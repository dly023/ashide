# Ashide Desktop — English (source-of-truth locale)
...
agent-management-source-local-agent = Ashide (local agent)
agent-management-source-cloud-agent = Ashide agent
...
agent-management-metadata-usage-used = Usage: { $usage }
...
workspace-menu-update-warp-manually = Update Ashide manually
...
workspace-menu-view-warp-logs = View Ashide logs
...
workspace-toast-ai-unavailable = AI usage is currently unavailable.
workspace-toast-provider-usage-limit = Check your provider usage limit.
...
app-menu-set-default-terminal = Set Ashide as Default Terminal
app-menu-file = File
app-menu-edit = Edit
app-menu-use-warp-prompt = Use Ashide's Prompt
...
app-menu-help = Help
app-menu-warp-documentation = Ashide Documentation...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = Ashide Slack Community...
workspace-update-and-relaunch-warp = Update and relaunch Ashide
workspace-updating-to-version = Updating to ({ $version })
workspace-update-warp-manually = Update Ashide manually
...
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with Ashide...{"  "}
...
terminal-model-use-own-provider = Use your own model
model-selector-no-provider-placeholder-name = No custom provider — add one in Settings → { $ai } → { $providers }
model-selector-no-provider-placeholder-model = Not configured
terminal-free-usage = Free usage
terminal-cloud-agent-run = Agent run
terminal-agent-header-for-terminal = for terminal
environment-runtime-choice-title = Choose how Ashide should set up this environment:
ssh-remote-choice-install-extension = Install Ashide's SSH extension
ssh-remote-choice-install-extension-desc = Install Ashide's extension to enable agent features like file browsing, code review, and intelligent command completions in this session.
...
ai-document-save-and-sync-tooltip = Save this plan to your Ashide Drive on this device
ai-document-show-in-local-drive = Show in Ashide Drive
...
ai-block-show-usage-details = Show usage details
...
ai-footer-enable-notifications-tooltip = Install the Warp plugin to enable rich agent notifications within Ashide
...
code-open-file-unavailable-environment-tooltip = Opening files is unavailable in this environment
...
ashide-launch-visit-repo = Visit the repo
ashide-launch-title = Ashide is now open-source
ashide-launch-description = You, our community, can participate in building Ashide using an agent-first workflow.
ashide-launch-contribute-title = Contribute
ashide-launch-contribute-description = Ashide's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.
ashide-launch-contribute-link-text = here
ashide-launch-oad-title = Open Automated Development
ashide-launch-oad-description = The Ashide repo is managed by an agent-first local workflow powered by Oz.
ashide-launch-auto-model-title = Introducing 'auto (open-weights)'
ashide-launch-auto-model-description = We've added a new auto model that picks the best open weight model for a task, like Kimi or MiniMax.
...
terminal-warpify-subshell-tooltip = Enable Ashide shell integration in this session
terminal-use-agent = Use agent
terminal-use-agent-tooltip = Ask the Ashide agent to assist
terminal-give-control-back-to-agent = Give control back to agent
terminal-resume-agent-tooltip = Ask the Ashide agent to resume
...
terminal-open-in-warp = Open in Ashide
terminal-open-conversation-in-warp-tooltip = Open this conversation in the Ashide desktop app
...
terminal-input-agent-hint-deploy-react-vercel = Ask Ashide, e.g. Deploy my React app to Vercel and set up environment variables
terminal-input-agent-hint-debug-python-ci = Ask Ashide, e.g. Help me debug why my Python tests are failing in CI
terminal-input-agent-hint-setup-microservice = Ask Ashide, e.g. Set up a new microservice with Docker and create the deployment pipeline
terminal-input-agent-hint-fix-node-memory-leak = Ask Ashide, e.g. Find and fix the memory leak in my Node.js application
terminal-input-agent-hint-backup-postgres = Ask Ashide, e.g. Create a backup script for my PostgreSQL database and schedule it
terminal-input-agent-hint-migrate-mysql-postgres = Ask Ashide, e.g. Help me migrate my data from MySQL to PostgreSQL
terminal-input-agent-hint-monitor-aws = Ask Ashide, e.g. Set up monitoring and alerts for my AWS infrastructure
terminal-input-agent-hint-build-fastapi = Ask Ashide, e.g. Build a REST API for my mobile app using FastAPI
terminal-input-agent-hint-optimize-sql = Ask Ashide, e.g. Help me optimize my SQL queries that are running slowly
terminal-input-agent-hint-github-actions = Ask Ashide, e.g. Create a GitHub Actions workflow to automatically deploy on merge
terminal-input-agent-hint-redis-cache = Ask Ashide, e.g. Set up Redis caching for my web application
terminal-input-agent-hint-kubernetes-pods = Ask Ashide, e.g. Help me troubleshoot why my Kubernetes pods keep crashing
terminal-input-agent-hint-bigquery-pipeline = Ask Ashide, e.g. Build a data pipeline to process CSV files and load them into BigQuery
terminal-input-agent-hint-ssl-https = Ask Ashide, e.g. Set up SSL certificates and configure HTTPS for my domain
terminal-input-agent-hint-refactor-legacy-code = Ask Ashide, e.g. Help me refactor this legacy code to use modern design patterns
terminal-input-agent-hint-unit-tests = Ask Ashide, e.g. Create unit tests for my authentication service
terminal-input-agent-hint-elk-logs = Ask Ashide, e.g. Set up log aggregation with ELK stack for my distributed system
terminal-input-agent-hint-oauth-express = Ask Ashide, e.g. Help me implement OAuth2 authentication in my Express.js app
terminal-input-agent-hint-optimize-docker = Ask Ashide, e.g. Optimize my Docker images to reduce build times and size
terminal-input-agent-hint-ab-testing = Ask Ashide, e.g. Set up A/B testing infrastructure for my web application
...
terminal-model-specs-description = Ashide's benchmarks for how well a model performs in our harness, usage intensity, and task speed.
terminal-model-specs-reasoning-level-title = Reasoning level
terminal-model-specs-reasoning-level-description = Increased reasoning levels use more tokens and have higher latency, but higher performance for complicated tasks.
...
onboarding-callout-meet-input-title = Meet the Ashide input
...
onboarding-callout-nl-support-text-prefix = Natural language input is off by default. If enabled, you can type requests in plain English and Ashide will autodetect queries for the agent. You can always override them using
onboarding-callout-enable-nl-detection = Enable Natural Language Detection
onboarding-callout-new-agent-title = Introducing Ashide's new agent experience
...
language-widget-secondary = Restart Ashide for the change to fully take effect.
language-restart-required-title = Language changed
language-restart-required-body = Ashide's UI language has been updated. Some text will switch immediately, but a full restart is required for the change to take effect everywhere.
...
# Ashide: settings-section-account removed alongside the Account settings page.
...
settings-section-warp-agent = Ashide Agent
...
# about_page.rs
settings-about-copyright = Copyright 2026 Ashide
settings-about-automatic-updates-label = Automatic updates
settings-about-automatic-updates-description = When enabled, Ashide checks for new versions in the background and downloads the installer to a local cache. The currently running Ashide is not touched until you click "Install now" to launch the installer yourself.
settings-about-update-checking = Checking for updates…
settings-about-update-up-to-date = Ashide is up to date.
...
settings-about-update-install-hint-macos = The installer will open — drag Ashide into your Applications folder to finish.
settings-about-update-install-hint-windows = The setup wizard will launch — follow the prompts to finish the upgrade.
settings-about-update-install-hint-linux = The AppImage will be replaced in place and Ashide will restart.
...
# Ashide: main_page.rs (Account / version / autoupdate) strings removed alongside
...
# ---- list_page.rs ----
settings-mcp-list-description = Add MCP servers to extend the Ashide Agent's capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins. Add a custom server, or use the presets to get started with popular servers.
...
settings-mcp-list-section-shared-by-ashide-and-other-devices = Shared by Ashide and from other devices
settings-mcp-list-section-shared-from-ashide = Shared from Ashide
...
settings-warpify-description-prefix = Configure whether Ashide attempts to "Warpify" (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-install-ssh-extension-description = Controls the installation behavior for Ashide's SSH extension when a remote host doesn't have it installed.
...
settings-network-proxy-password-save-failed = Could not save the proxy password to the OS keyring: { $error }. The password was not stored — try again.
...
# Section / sub-headers
settings-ai-warp-agent-header = Ashide Agent
...
settings-ai-command-denylist-description = Regular expressions to match commands that the Ashide Agent should always ask permission to execute.
settings-ai-command-allowlist = Command allowlist
settings-ai-command-allowlist-description = Regular expressions to match commands that can be automatically executed by the Ashide Agent.
...
settings-ai-mcp-allowlist-description = Allow the Ashide Agent to call these MCP servers.
settings-ai-mcp-denylist = MCP denylist
settings-ai-mcp-denylist-description = The Ashide Agent will always ask for permission before calling any MCP servers on this list.
...
settings-ai-base-model-description = This model serves as the primary engine behind the Ashide Agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Ashide may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
settings-ai-show-model-picker-in-prompt = Show model picker in prompt
settings-ai-codebase-context = Codebase Context
settings-ai-codebase-context-description = Allow the Ashide Agent to generate an outline of your codebase that can be used for context. No code is ever stored on our servers.
...
settings-ai-rules-description = Rules help the Ashide Agent follow your conventions, whether for codebases or specific workflows.
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in agent conversations started from the Ashide app.

# AWS Bedrock
settings-ai-aws-bedrock-toggle = Use AWS Bedrock credentials
settings-ai-aws-bedrock-description = Ashide loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = Ashide loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-ssh-wrapper = Ashide SSH Wrapper
settings-features-ssh-auto-discovery = Auto-discover SSH hosts
settings-features-receive-desktop-notifications = Receive desktop notifications from Ashide
...
settings-features-make-default-terminal = Make Ashide the default terminal
...
settings-features-default-terminal-current = Ashide is the default terminal
...
settings-features-restart-warp-to-apply = Restart Ashide for changes to take effect.
...
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Ashide may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from Ashide servers.
...
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (Ashide mode)
...
settings-appearance-tab-vertical-layout-label = Use Session Navigator
settings-appearance-tab-show-vertical-panel-in-restored-windows-label = Show Session Navigator in restored windows
settings-appearance-tab-show-vertical-panel-in-restored-windows-description = When enabled, reopening or restoring a window opens the Session Navigator even if it was closed when the window was last saved.
settings-appearance-tab-show-title-bar-search-bar-label = Show search bar in title bar
settings-appearance-tab-show-title-bar-search-bar-description = Show the "Search sessions, agents, files…" search bar in the middle of the title bar; click to open the command palette. Disable to leave the slot empty. Only applies when the Session Navigator is enabled.
...
settings-appearance-tab-use-prompt-as-title-description = Show the latest user prompt instead of the generated conversation title for built-in AI and third-party agent sessions in the Session Navigator.
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), Ashide-assisted setup, or manual configuration.
...
settings-agent-providers-title = Own keys and endpoints
settings-agent-providers-description = Use your own API key or compatible endpoint for Agent models. Metadata stays in local settings.toml; API keys stay in the system keychain.
settings-agent-providers-secret-save-failed = Could not save the API key to the system keychain: { $error }. The key was not stored — try again.
settings-agent-providers-sync-success = Synced from models.dev: { $updated } updated · { $added } added.
settings-agent-providers-sync-catalog-not-loaded = models.dev catalog not loaded yet — click [Refresh catalog] and try again.
settings-agent-providers-sync-no-match = No matching provider found on models.dev for "{ $name }". Check the name or base URL.
settings-agent-providers-empty = No keys or endpoints configured yet. Use [+ Add key] or [+ Add custom endpoint] to start.
settings-agent-providers-add-key-button = + Add key
settings-agent-providers-add-endpoint-button = + Add custom endpoint
settings-agent-providers-summary = { $keys } key(s) · { $endpoints } endpoint(s)
settings-agent-providers-status-configured = ✓ Key saved
settings-agent-providers-status-endpoint = ✓ Endpoint set
settings-agent-providers-status-missing = Not set up
settings-agent-providers-untitled = Untitled provider
...
quit-warning-title-app = Quit Ashide?
...
keybinding-desc-workspace-left-panel-local-drive = Left Panel: Ashide Drive
keybinding-desc-workspace-left-panel-ssh-manager = Left Panel: SSH Manager
keybinding-desc-workspace-left-panel-environment-provider-manager = Left Panel: Environment Providers
...
keybinding-desc-workspace-toggle-local-drive = Toggle Ashide Drive
...
keybinding-desc-workspace-search-drive = Search Ashide Drive
...
keybinding-desc-workspace-terminate-app = Quit Ashide
...
keybinding-desc-workspace-export-all-local-drive-objects = Export all Ashide Drive objects
...
# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Toggle Ashide AI
...
# Ashide: keybinding-desc-workspace-show-settings-account removed alongside the
...
keybinding-desc-workspace-show-settings-about-menu = About Ashide
...
keybinding-desc-workspace-view-logs = View Ashide logs
...
keybinding-desc-workspace-crash-macos = Crash the app (for testing local crash diagnostics)
keybinding-desc-workspace-crash-other = Crash the app (for testing local crash diagnostics)
...
keybinding-desc-workspace-open-ashide-launch-modal = [Debug] Open Ashide Launch Modal
keybinding-desc-workspace-reset-ashide-launch-modal-state = [Debug] Reset Ashide Launch Modal State
keybinding-desc-workspace-install-opencode-warp-plugin = [Debug] Install OpenCode Warp plugin
keybinding-desc-workspace-use-dev-checkout-opencode-warp-plugin = [Debug] Use dev checkout OpenCode Warp plugin (testing only)
...
keybinding-desc-input-show-network-log = Show Ashide network log
...
keybinding-desc-ai-assistant-close = Close Ashide AI
keybinding-desc-ai-assistant-focus-terminal-input = Focus Terminal Input From Ashide AI
keybinding-desc-ai-assistant-restart = Restart Ashide AI
...
keybinding-desc-terminal-ask-ai-selection = Ask Ashide AI about Selection
keybinding-desc-terminal-ask-ai-last-block = Ask Ashide AI about last block
keybinding-desc-terminal-ask-ai = Ask Ashide AI
...
toggle-suffix-ssh-wrapper = Ashide SSH wrapper
...
toggle-suffix-crash-reporting = local crash diagnostics
...
settings-external-editor-choose-layout = Choose a layout to open files in Ashide
...
settings-external-editor-prefer-markdown = Open Markdown files in Ashide's Markdown Viewer by default
...
menu-block-open-in-warp = Open in Ashide
...
menu-block-ask-warp-ai = Ask Ashide AI
...
menu-input-ask-warp-ai = Ask Ashide AI
...
slash-cmd-open-skill-desc = Open a skill's markdown file in Ashide's built-in editor
...
slash-cmd-open-file-desc = Open a file in Ashide's code editor
...
prompt-editor-warp-prompt-section = Ashide terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in Ashide without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make Ashide your own by choosing a built-in theme. Or create your own.
...
drive-import-file-import-error = Failed to import file
drive-import-folder-import-error = Failed to import folder
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with Ashide AI
...
workflow-ai-assist-error-rate-limited = AI usage is currently unavailable. Please try again later.
...
command-palette-conversations-new-conversation = New conversation
conversation-untitled = Untitled conversation
conversation-deleted = Deleted conversation
conversation-fallback-title = Conversation
workspace-session-bridge-fork-to-target = Fork to { $target }
workspace-session-bridge-edit-and-fork = Edit and Fork…
workspace-session-bridge-export-bundle = Export portable session bundle…
workspace-session-bridge-fork-unavailable = This conversation cannot be forked yet
workspace-session-bridge-edit-title = Edit and Fork session
workspace-session-bridge-edit-title-with-name = Edit and Fork { $name }
workspace-session-bridge-edit-target-label = Fork to
workspace-session-bridge-edit-message-draft = Messages
workspace-session-bridge-edit-save = Save and Fork to { $target }
...
workspace-session-bridge-edit-message-kept = Kept
workspace-session-bridge-edit-message-removed = Removed
workspace-session-bridge-edit-message-removed-help = This message will not be written to the forked session.
workspace-session-bridge-edit-remove-message = Remove
workspace-session-bridge-edit-restore-message = Restore
workspace-session-bridge-edit-error-missing-conversation = Conversation is missing. Close this dialog and try again.
workspace-session-bridge-edit-error-missing-target = Choose an Agent target for the fork.
workspace-session-bridge-edit-error-empty-draft = Keep at least one message.
workspace-session-bridge-forked-to-ashide = Forked to a new Ashide tab: { $title }
workspace-session-bridge-forked-to-target = Forked to a new { $target } tab: { $title }
workspace-session-bridge-forked-to-remote-target = Forked to a new remote { $target } tab: { $title }
workspace-session-bridge-open-tab-failed = Could not open the new { $target } tab
workspace-session-bridge-open-remote-tab-failed = Could not open the new remote { $target } tab
workspace-session-bridge-error-detail = { $context }: { $detail }
workspace-session-bridge-fork-failed = Forking the conversation failed
workspace-session-bridge-fork-cli-failed = Forking the CLI session failed
workspace-session-bridge-fork-remote-cli-failed = Forking the remote CLI session failed
workspace-session-bridge-edit-fork-failed = Save and Fork failed
workspace-session-bridge-bundle-exported = Exported SessionBridge bundle: { $path }
workspace-session-bridge-bundle-export-failed = Exporting the SessionBridge bundle failed
...
project-explorer-unavailable-environment-description = The Project Explorer requires access to the current app workspace, which isn’t supported in this environment.
...
workspace-left-panel-environment-provider-manager = Environment Providers
workspace-left-panel-server-file-browser = File Explorer
...
skill-manager-remote-connecting = Remote runtime is connecting. Skills will load after it is ready.
skill-manager-remote-loading = Reading skills from the environment runtime…
skill-manager-remote-error = Failed to read environment runtime skills: { $error }
skill-manager-remote-empty = No skills were found in this environment runtime. Ashide scans the current directory ancestors and HOME for .agents/skills / .claude/skills.
workspace-left-panel-ssh-manager-placeholder = SSH Manager
...
workspace-session-navigator-title = Sessions
workspace-session-navigator-subtitle = Live and recoverable CLI agent sessions. Click a row to focus or resume in its cwd.
workspace-session-navigator-empty = No sessions found yet.
workspace-session-navigator-no-matches = No sessions match your search.
workspace-session-navigator-terminal = Terminal
workspace-session-navigator-agent = Agent session
workspace-session-navigator-welcome = Welcome
workspace-session-navigator-session = Session
workspace-session-navigator-no-root = No workspace root captured
workspace-session-navigator-state-active = active
workspace-session-navigator-state-restored = restored
workspace-session-navigator-cue-live = live
workspace-session-navigator-cue-fork = fork
workspace-session-navigator-cue-indexed = indexed
workspace-session-navigator-cue-detected = detected
workspace-session-navigator-cue-source = source
workspace-session-navigator-menu-resume = Focus / Resume
workspace-session-navigator-menu-restoring = Restoring…
workspace-session-navigator-menu-focus = Focus session
workspace-session-navigator-menu-restore = Resume session
workspace-session-navigator-menu-rename-alias = Rename alias…
workspace-session-navigator-menu-clear-alias = Clear alias
workspace-session-navigator-menu-copy-id = Copy session ID
workspace-session-navigator-menu-copy-id-toast = Copied { $id } to clipboard
workspace-session-navigator-menu-pin = Pin
workspace-session-navigator-menu-unpin = Unpin
workspace-session-navigator-menu-delete = Delete permanently
workspace-session-preview-title = Session
workspace-session-preview-live = Live
workspace-session-preview-history = History
workspace-session-preview-environment-runtime = Environment · { $environment }
workspace-session-preview-session-id = session { $id }
workspace-session-preview-updated = updated { $time }
workspace-session-preview-focus = Focus
workspace-session-preview-resume = Resume
workspace-session-preview-delete = Delete…
workspace-session-preview-exit-delete = Exit and delete
...
workspace-environment-kind-local = Local
workspace-environment-kind-ssh = SSH
workspace-environment-kind-container = Container
workspace-environment-kind-wsl = WSL
workspace-environment-kind-custom = Custom
workspace-environment-state-connected = connected
workspace-environment-state-dormant = not connected
workspace-environment-state-connecting = connecting
workspace-environment-state-installing = installing runtime
workspace-environment-state-reconnecting = reconnecting
workspace-environment-state-error = error
workspace-environment-tooltip-local = Local environment
workspace-environment-tooltip-runtime-connected = Environment connected
workspace-environment-tooltip-runtime-dormant = Environment is not connected yet. Click to connect; the first connection installs or updates the Ashide helper on that host.
workspace-environment-tooltip-runtime-connecting = Connecting environment...
workspace-environment-tooltip-runtime-installing = Installing or starting the remote runtime...
workspace-environment-tooltip-runtime-reconnecting = Environment reconnecting...
workspace-environment-tooltip-runtime-error = Environment connection error. Click to retry.
workspace-environment-tooltip-generic = Environment
workspace-environment-tooltip-active-root = Active root: { $root }
workspace-environment-add-ssh = SSH
workspace-environment-provider-picker-title = Environment targets
workspace-environment-provider-picker-open-config = Open SSH config
workspace-environment-provider-picker-open = Open
...
workspace-restored-sessions-chip-label = Sessions · { $count } restored
workspace-restored-sessions-chip-single = { $agent }
workspace-restored-sessions-state-dormant = not connected
workspace-restored-sessions-agent-fallback = agent session
workspace-restored-sessions-terminal-fallback = Terminal
workspace-restored-sessions-welcome-fallback = Welcome
workspace-restored-sessions-fallback = Session
workspace-restored-sessions-tooltip = Restored workspace sessions
workspace-restored-sessions-tooltip-sublabel = Includes ordinary terminals and CLI agents captured from saved workspace metadata.
workspace-restored-sessions-toast = Restored CLI agent sessions are available. Resume UI is coming next.
workspace-restored-sessions-popover-title = Restored workspace sessions
workspace-restored-sessions-popover-subtitle = Ordinary terminals and CLI agents discovered from saved workspace metadata.
workspace-restored-sessions-popover-footer = Restore is explicit for now: no CLI command is executed automatically.
workspace-restored-sessions-popover-more = +{ $count } more sessions
workspace-restored-sessions-no-root = No workspace root captured
workspace-restored-sessions-no-command = no command captured
workspace-restored-sessions-no-environment = no environment
...
workspace-restored-sessions-origin-command-detected = detected command
workspace-restored-sessions-origin-plugin-observed = plugin observed
workspace-restored-sessions-origin-unknown = unknown
workspace-restored-sessions-action-resume = Resume
workspace-restored-sessions-action-promote = Promote
workspace-restored-sessions-action-terminal = Terminal
workspace-restored-sessions-action-pending = { $action } · { $state }
workspace-restored-sessions-action-state-adapter-pending = adapter pending
workspace-restored-sessions-action-state-unavailable = unavailable
workspace-restored-sessions-action-state-managed = already managed
workspace-restored-sessions-action-state-restored = restored

workspace-left-panel-ssh-manager-menu-open-environment = Open as Environment
...
workspace-left-panel-ssh-manager-pane-hint = Manage saved SSH servers here. Use Connect to open a terminal or Open as Environment for runtime-backed sessions.
...
search-filter-display-environment-providers = Environment Targets
search-filter-placeholder-environment-providers = Search environment targets...
...
server-file-browser-runtime-preparing = Preparing the remote runtime…
server-file-browser-runtime-reconnecting = Remote runtime connection is stale. Reconnecting…
server-file-browser-runtime-dormant = Environment runtime is not connected. Select the environment to reconnect.
server-file-browser-runtime-error = Environment runtime failed to start. Select the environment to retry.
...
server-file-browser-directory-listing-failed = Couldn’t list this directory. Refresh after the environment runtime reconnects.
server-file-browser-directory-listing-failed-detail = Couldn’t list this directory: { $error }
server-file-browser-path-resolution-failed = Couldn’t open this remote path. Refresh after the environment runtime reconnects.
server-file-browser-path-resolution-failed-detail = Couldn’t open this remote path: { $error }
...
global-search-environment-description = Global search requires access to the current app workspace, which is not supported in this environment
...
wasm-nux-open-desktop-title = Open in Ashide Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in Ashide
wasm-nux-download-title = Download Ashide Desktop?
wasm-nux-download-description = Ashide is the intelligent terminal with AI and your dev team's knowledge built-in.
wasm-nux-learn-more = Learn more
wasm-nux-download-confirm = Download
wasm-nux-object-kind-warp-links = Ashide links
...
cli-agent-plugin-claude-install-title = Install Ashide Plugin for Claude Code
...
cli-agent-plugin-claude-update-title = Update Ashide Plugin for Claude Code
...
cli-agent-plugin-gemini-install-title = Install Ashide Plugin for Gemini CLI
...
cli-agent-plugin-gemini-update-title = Update Ashide Plugin for Gemini CLI
...
cli-agent-plugin-opencode-install-title = Install Ashide Plugin for OpenCode
...
cli-agent-plugin-opencode-update-title = Update Ashide Plugin for OpenCode
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing Ashide SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing Ashide SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating Ashide SSH Extension...
...
agent-tip-command-palette = <keybinding> to open the Command Palette and access Ashide actions and shortcuts.
agent-tip-local-drive = Store reusable workflows, notebooks, and prompts in your
agent-tip-redirect-running-agent = Enter a new prompt to redirect the agent while it's running.
agent-tip-add-context = `@` to add context from files, blocks, or Ashide Drive objects to your prompt.
...
agent-tip-usage = `/usage` to show your current AI usage.
...
hoa-welcome-banner-title = Introducing universal agent support: level up any coding agent with Ashide
...
hoa-feature-native-code-review-description = Send inline comments from Ashide's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-ashide-section = Maximize Ashide
...
resource-center-command-palette-description = Access all of Ashide via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make Ashide your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up Ashide to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate Ashide with your IDE
resource-center-integrate-ide-description = Configure Ashide to launch from your most used development tools
...
onboarding-intention-title = Welcome to Ashide
...
onboarding-ai-feature-warp-agents = Ashide agents
...
onboarding-agent-title = Customize your Ashide Agent
...
onboarding-agent-disable-warp-agent = Disable Ashide Agent
onboarding-project-title = Open a project
onboarding-project-subtitle = Set up a project to optimize it for coding in Ashide.
...
onboarding-customize-title = Customize your Ashide
...
auth-local-privacy-note = Ashide stores onboarding choices locally on this device.
...
get-started-welcome-title = Welcome to Ashide
...
terminal-banner-p10k-supported = Powerlevel10k now supports Ashide!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in Ashide. You might consider one of the supported prompts as an alternative.{"  "}
terminal-loading-session = Loading session...
environment-runtime-placeholder-title = Environment
environment-runtime-placeholder-empty = Preparing environment terminal…
environment-runtime-placeholder-empty-detail = The terminal opens automatically after the connection is ready.
environment-runtime-placeholder-installing = Installing remote runtime…
environment-runtime-placeholder-installing-detail = The terminal opens automatically after installation finishes.
environment-runtime-placeholder-reconnecting = Reconnecting environment…
environment-runtime-placeholder-reconnecting-detail = The terminal resumes automatically after reconnecting.
environment-runtime-placeholder-opening = Opening environment terminal…
environment-runtime-placeholder-opening-detail = The terminal appears automatically after it is created.
environment-runtime-placeholder-dormant = Environment runtime is not connected
environment-runtime-placeholder-dormant-detail = Click the environment tab to reconnect.
environment-runtime-placeholder-error = Environment runtime failed to start
environment-runtime-placeholder-error-detail = Selecting the environment again retries automatically.
...
command-search-ai-unavailable-contact-admin = AI usage is currently unavailable. Check your provider usage limit.
command-search-ai-unavailable-prefix = AI usage is currently unavailable.{" "}
command-search-provider-usage-suffix = {" "}check your provider usage limit.
...
settings-import-reset-to-warp-defaults = Reset to Ashide defaults
...
workspace-delete-session-dialog-title-restoring = Cancel restore and delete “{ $title }”?
workspace-delete-session-dialog-message-restoring = This cancels the pending restore in this window and permanently deletes local session records for this logical session. It cannot be undone.
workspace-delete-session-dialog-confirm-restoring = Cancel restore and delete
workspace-delete-session-dialog-title-live = Exit and delete “{ $title }”?
workspace-delete-session-dialog-message-live = This closes the active pane or tab and permanently deletes local session records for this logical session. It cannot be undone.
workspace-delete-session-dialog-confirm-live = Exit and delete
workspace-delete-session-dialog-title = Delete “{ $title }” permanently?
workspace-delete-session-dialog-message = This permanently deletes local session records for this logical session. It cannot be undone.
workspace-delete-session-dialog-confirm = Delete permanently
workspace-close-session-confirm-title = Close session?
workspace-close-session-confirm-message = You are about to close a session that is currently being shared. Closing it will end sharing for everyone.
workspace-auto-reload = Auto-reload
workspace-add-new-repo = {" "}+ Add new repo
workspace-notification-permission-denied-toast = Ashide doesn't have permission to send desktop notifications.
workspace-troubleshoot-notifications-link = Troubleshoot notifications
workspace-plan-saved-to-local-drive-toast = Plan saved to your Ashide Drive
workspace-remote-control-link-copied-toast = Remote control link copied.
workspace-update-now = Update now
workspace-update-warp = Update Ashide
...
workspace-version-deprecation-without-permissions-banner = Some Ashide features may not work as expected without updating immediately, but Ashide is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but Ashide is unable to perform the update.
workspace-unable-to-launch-new-installed-version = Ashide was unable to launch the new installed version.
...
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in Ashide. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize Ashide for this codebase?
...
notifications-banner-permissions-denied-title = Ashide was denied permissions to send you notifications.
...
code-review-agent-code-review-requires-ai = Agent code review requires AI
...
notebook-local-save-conflict-resolution-message = This notebook could not be saved because changes were made while you were editing. Please copy your work and refresh.
notebook-local-save-feature-not-available-message = This notebook could not be fully saved because the local save path is temporarily unavailable. The changes are kept locally. Please retry later.
notebook-link-copied-toast = Link copied
settings-share-with-team = Save locally
tooltip-secrets-stay-local = *Secrets stay local.
...
sftp-action-reconnect = Reconnect
sftp-download-failed = Download failed: { $error }
sftp-not-connected-to-server = Not connected to server
sftp-download-not-connected = Download failed: not connected to server
sftp-rename-failed = Rename failed: { $error }
sftp-delete-title = Delete item
sftp-delete-description-one = Delete “{ $name }”? This cannot be undone.
sftp-delete-description-many = Delete { $count } items? This cannot be undone.
sftp-current-name = Current name: { $name }
sftp-transfer-status-pending = Pending
sftp-transfer-status-transferring = Transferring
sftp-transfer-status-completed = Completed
sftp-transfer-status-failed = Failed
sftp-transfer-status-cancelled = Cancelled
sftp-create-channel-failed = Failed to create SFTP channel: { $error }
sftp-read-server-config-failed = Failed to read server configuration: { $error }
sftp-list-directory-failed = Failed to list directory: { $error }
sftp-create-folder-failed = Failed to create folder: { $error }
sftp-server-config-not-found = Server configuration not found
sftp-environment-not-connected = This host's environment isn't connected. Open it as an environment first, then reopen Files.
sftp-connection-cancelled = Connection cancelled
sftp-upload-failed = Upload failed: { $error }
sftp-upload-not-connected = Upload failed: not connected to server
sftp-move-failed = Move failed: { $error }
sftp-folder-name-empty = Folder name cannot be empty
sftp-invalid-name-path-separators = Invalid name: path separators are not allowed
sftp-delete-failed = Delete failed: { $error }
sftp-file-name-invalid-characters = File name contains invalid characters
sftp-name-empty = Name cannot be empty
sftp-invalid-target-path = Invalid target path
