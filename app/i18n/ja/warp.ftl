...
common-skip = スキップ
common-get-warping = Ashide を始める
...
agent-management-source-local-agent = Ashide (ローカルエージェント)
agent-management-source-cloud-agent = Ashide (ローカルエージェント)
...
agent-management-metadata-usage-used = 使用量: { $usage }
...
workspace-menu-update-warp-manually = Ashide を手動で更新
...
workspace-menu-view-warp-logs = Ashide ログを表示
...
workspace-toast-ai-unavailable = AI 使用量は現在利用できません。
workspace-toast-provider-usage-limit = プロバイダーの使用上限を確認してください。
...
workspace-update-and-relaunch-warp = Ashide を更新して再起動
workspace-updating-to-version = ({ $version }) に更新中
workspace-update-warp-manually = Ashide を手動で更新
...
terminal-banner-shell-config-incompatible = シェルの設定が Ashide と互換性がありません...{"  "}
...
terminal-model-use-own-provider = 自分のモデルを使う
model-selector-no-provider-placeholder-name = カスタムプロバイダー未設定 — 設定 → { $ai } → { $providers } で追加
model-selector-no-provider-placeholder-model = 未設定
terminal-free-usage = 無料使用量
...
ai-document-save-and-sync-tooltip = この計画をこのデバイスの Ashide Drive に保存
ai-document-show-in-local-drive = Ashide Drive で表示
...
ai-block-show-usage-details = 使用量の詳細を表示
...
ai-footer-enable-notifications-tooltip = Warp プラグインをインストールして Ashide 内のリッチなエージェント通知を有効化
...
code-open-file-unavailable-environment-tooltip = この環境ではファイルを開けません
...
ashide-launch-visit-repo = リポジトリを見る
ashide-launch-title = Ashide がオープンソースになりました
ashide-launch-description = コミュニティの皆さんがエージェントファーストのワークフローで Ashide の構築に参加できます。
ashide-launch-contribute-title = コントリビュート
ashide-launch-contribute-description = Ashide のクライアントコードがオープンソースになりました。/feedback スキルで Issue を作成し、こちらのコントリビューションガイドラインに従ってください。
ashide-launch-contribute-link-text = こちら
ashide-launch-oad-title = Open Automated Development
ashide-launch-oad-description = Ashide リポジトリは、Oz のローカルエージェント体験を活用したエージェントファーストワークフローで管理されています。
ashide-launch-auto-model-title = 'auto (open-weights)' 登場
ashide-launch-auto-model-description = タスクに最適なオープンウェイトモデル (Kimi、MiniMax など) を選ぶ新しい auto モデルを追加しました。
...
terminal-warpify-subshell-tooltip = このセッションで Ashide シェル統合を有効化
terminal-use-agent = エージェントを使う
terminal-use-agent-tooltip = Ashide エージェントに支援を依頼
terminal-give-control-back-to-agent = 制御をエージェントに戻す
terminal-resume-agent-tooltip = Ashide エージェントに再開を依頼
...
terminal-open-in-warp = Ashide で開く
terminal-open-conversation-in-warp-tooltip = この会話を Ashide デスクトップアプリで開く
...
terminal-input-agent-hint-deploy-react-vercel = 何でも Ashide で。例: React アプリを Vercel にデプロイし環境変数を設定
terminal-input-agent-hint-debug-python-ci = 何でも Ashide で。例: CI で Python テストが失敗する原因をデバッグ
terminal-input-agent-hint-setup-microservice = 何でも Ashide で。例: Docker で新しいマイクロサービスをセットアップしデプロイパイプラインを作成
terminal-input-agent-hint-fix-node-memory-leak = 何でも Ashide で。例: Node.js アプリのメモリリークを発見して修正
terminal-input-agent-hint-backup-postgres = 何でも Ashide で。例: PostgreSQL DB のバックアップスクリプトを作成しスケジュール
terminal-input-agent-hint-migrate-mysql-postgres = 何でも Ashide で。例: MySQL から PostgreSQL へのデータ移行
terminal-input-agent-hint-monitor-aws = 何でも Ashide で。例: AWS インフラの監視とアラートを設定
terminal-input-agent-hint-build-fastapi = 何でも Ashide で。例: FastAPI でモバイルアプリ向け REST API を構築
terminal-input-agent-hint-optimize-sql = 何でも Ashide で。例: 遅い SQL クエリを最適化
terminal-input-agent-hint-github-actions = 何でも Ashide で。例: マージ時に自動デプロイする GitHub Actions ワークフローを作成
terminal-input-agent-hint-redis-cache = 何でも Ashide で。例: Web アプリに Redis キャッシュを設定
terminal-input-agent-hint-kubernetes-pods = 何でも Ashide で。例: Kubernetes Pod がクラッシュし続ける原因をトラブルシュート
terminal-input-agent-hint-bigquery-pipeline = 何でも Ashide で。例: CSV を処理して BigQuery に投入するデータパイプラインを構築
terminal-input-agent-hint-ssl-https = 何でも Ashide で。例: SSL 証明書を設定してドメインを HTTPS 化
terminal-input-agent-hint-refactor-legacy-code = 何でも Ashide で。例: レガシーコードをモダンな設計パターンにリファクタ
terminal-input-agent-hint-unit-tests = 何でも Ashide で。例: 認証サービスのユニットテストを作成
terminal-input-agent-hint-elk-logs = 何でも Ashide で。例: 分散システム向けに ELK スタックでログ集約を構築
terminal-input-agent-hint-oauth-express = 何でも Ashide で。例: Express.js アプリに OAuth2 認証を実装
terminal-input-agent-hint-optimize-docker = 何でも Ashide で。例: ビルド時間とサイズを削減するため Docker イメージを最適化
terminal-input-agent-hint-ab-testing = 何でも Ashide で。例: Web アプリ向けに A/B テスト基盤を構築
...
terminal-model-specs-description = Ashide のハーネスでのモデル性能、使用量の強度、タスク速度のベンチマーク。
terminal-model-specs-reasoning-level-title = 推論レベル
terminal-model-specs-reasoning-level-description = 推論レベルを上げるとトークン使用量とレイテンシが増えますが、複雑なタスクでの性能が向上します。
...
onboarding-callout-meet-input-title = Ashide 入力欄のご紹介
...
onboarding-callout-nl-support-text-prefix = 自然言語入力は既定で無効です。有効にすると、平易な英語でリクエストを入力でき、Ashide がエージェント向けクエリを自動判定します。次の方法でいつでも上書きできます:
onboarding-callout-enable-nl-detection = 自然言語判定を有効にする
onboarding-callout-new-agent-title = Ashide の新しいエージェント体験のご紹介
...
language-widget-secondary = 変更を完全に反映するには Ashide を再起動してください。
language-restart-required-title = 言語が変更されました
language-restart-required-body = Ashide の UI 言語が更新されました。一部のテキストは即座に切り替わりますが、すべての箇所に反映するには再起動が必要です。
# =============================================================================
...
# Ashide: settings-section-account は Account 設定ページとともに削除。
...
settings-section-warp-agent = Ashide エージェント
...
settings-about-automatic-updates-description = 有効にすると、Ashide はバックグラウンドで新バージョンを確認しダウンロードします。無効でも手動で更新を確認できます。
...
# Ashide: main_page.rs 関連の文字列は Account 設定ページとともに削除された。
...
# ---- list_page.rs ----
settings-mcp-list-description = MCP サーバーを追加して Ashide エージェントの機能を拡張します。MCP サーバーは標準化されたインターフェースを通じてデータソースやツールをエージェントに公開し、プラグインのように動作します。カスタムサーバーを追加するか、プリセットを使用して人気のサーバーから始められます。
...
settings-mcp-list-section-shared-by-ashide-and-other-devices = Ashide および別のデバイスから共有
settings-mcp-list-section-shared-from-ashide = Ashide から共有
...
settings-warpify-description-prefix = Ashide が特定のシェルを「Warpify」(ブロック・入力モード等のサポートを追加) するかを設定します。
...
settings-warpify-install-ssh-extension-description = リモートホストに Ashide の SSH 拡張がインストールされていない場合のインストール挙動を制御します。
...
settings-network-proxy-password-save-failed = プロキシパスワードを OS キーチェーンに保存できませんでした: { $error }。パスワードは保存されていません。もう一度お試しください。
...
# Section / sub-headers
settings-ai-warp-agent-header = Ashide エージェント
...
settings-ai-command-denylist-description = Ashide エージェントが実行する前に必ず権限を確認するコマンドにマッチする正規表現。
settings-ai-command-allowlist = コマンド許可リスト
settings-ai-command-allowlist-description = Ashide エージェントが自動的に実行できるコマンドにマッチする正規表現。
...
settings-ai-mcp-allowlist-description = Ashide エージェントによるこれらの MCP サーバーの呼び出しを許可します。
settings-ai-mcp-denylist = MCP 拒否リスト
settings-ai-mcp-denylist-description = Ashide エージェントはこのリストにある MCP サーバーを呼び出す前に必ず権限を確認します。
settings-ai-info-banner-managed-by-workspace = 一部の権限はワークスペースによって管理されています。

# Models / Profiles
settings-ai-base-model = ベースモデル
settings-ai-base-model-description = このモデルは Ashide エージェントの中心エンジンとして機能します。ほとんどのやり取りを担い、必要に応じて計画やコード生成などのタスクで他のモデルを呼び出します。Ashide はモデルの可用性や、会話要約などの補助タスクに応じて自動的に別のモデルへ切り替えることがあります。
settings-ai-show-model-picker-in-prompt = プロンプトにモデル選択を表示
settings-ai-codebase-context = コードベースのコンテキスト
settings-ai-codebase-context-description = Ashide エージェントによるコードベースの概要生成を許可し、コンテキストとして利用します。コードがサーバーに保存されることはありません。
...
settings-ai-rules-description = ルールは、Ashide エージェントがコードベースや特定のワークフローのお作法に沿うよう導きます。
...
settings-ai-cloud-agent-computer-use-description = Ashide アプリから開始したエージェント会話でコンピュータ操作を有効化します。

# AWS Bedrock
settings-ai-aws-bedrock-toggle = AWS Bedrock の認証情報を使用
settings-ai-aws-bedrock-description = Ashide はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。
settings-ai-aws-bedrock-description-managed = Ashide はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。この設定は組織によって管理されています。
...
settings-features-ssh-wrapper = Ashide SSH ラッパー
settings-features-ssh-auto-discovery = SSH ホストの自動検出
settings-features-receive-desktop-notifications = Ashide からのデスクトップ通知を受信
...
settings-features-make-default-terminal = Ashide を既定のターミナルにする
...
settings-features-default-terminal-current = Ashide が既定のターミナルです
...
settings-features-restart-warp-to-apply = 変更を反映するには Ashide を再起動してください。
...
settings-exec-profile-editor-base-model-desc = このモデルはエージェントの主要エンジンとして機能します。ほとんどのやり取りを駆動し、必要に応じて計画やコード生成などのために他モデルを呼び出します。Ashide はモデルの可用性や、会話の要約などの補助タスクのため、自動的に代替モデルに切り替える場合があります。
...
settings-show-blocks-confirm-dialog-text = このブロックを共有解除してもよろしいですか?
    ...
    リンク経由でアクセスできなくなり、Ashide サーバーから完全に削除されます。
...
settings-appearance-input-mode-pinned-bottom = 下部に固定 (Ashide モード)
...
settings-appearance-tab-vertical-layout-label = セッションナビゲーターを使用
...
settings-environments-page-description = 環境はアンビエントエージェントが実行される場所を定義します。GitHub (推奨)、Ashide 支援セットアップ、または手動構成で数分で設定できます。
...
settings-agent-providers-title = 自分のキーとエンドポイント
settings-agent-providers-description = 自分の API キーまたは互換エンドポイントで Agent モデルを使用します。メタデータはローカル settings.toml に保存され、API キーはシステムキーチェーンに保存されます。
settings-agent-providers-secret-save-failed = API キーをシステムキーチェーンに保存できませんでした: { $error }。キーは保存されていません。もう一度お試しください。
settings-agent-providers-sync-success = models.dev から同期しました: 更新 { $updated } 件 · 追加 { $added } 件。
settings-agent-providers-sync-catalog-not-loaded = models.dev のカタログがまだ読み込まれていません。[カタログを更新] をクリックして再試行してください。
settings-agent-providers-sync-no-match = 「{ $name }」に一致するプロバイダーが models.dev に見つかりませんでした。名前または Base URL を確認してください。
settings-agent-providers-empty = キーまたはエンドポイントがまだ構成されていません。[+ キーを追加] または [+ カスタムエンドポイントを追加] から始めてください。
settings-agent-providers-add-key-button = + キーを追加
settings-agent-providers-add-endpoint-button = + カスタムエンドポイントを追加
settings-agent-providers-summary = キー { $keys } 個 · エンドポイント { $endpoints } 個
settings-agent-providers-status-configured = ✓ キー保存済み
settings-agent-providers-status-endpoint = ✓ エンドポイント設定済み
settings-agent-providers-status-missing = 未設定
settings-agent-providers-untitled = 名称未設定のプロバイダー
...
quit-warning-title-app = Ashide を終了しますか?
...
server-file-browser-runtime-preparing = リモートランタイムを準備しています…
server-file-browser-runtime-reconnecting = リモートランタイム接続が古くなっています。再接続しています…
server-file-browser-runtime-dormant = 環境ランタイムは接続されていません。環境を選択すると再接続します。
server-file-browser-runtime-error = 環境ランタイムの起動に失敗しました。環境を再選択すると再試行します。
...
keybinding-desc-workspace-left-panel-local-drive = 左パネル: Ashide Drive
keybinding-desc-workspace-left-panel-ssh-manager = 左パネル: SSH マネージャー
keybinding-desc-workspace-left-panel-environment-provider-manager = 左パネル: 環境プロバイダー
keybinding-desc-workspace-open-global-search = グローバル検索を開く
keybinding-desc-workspace-open-global-search-menu = グローバル検索
keybinding-desc-workspace-toggle-local-drive = Ashide Drive を切り替え
...
keybinding-desc-workspace-search-drive = Ashide Drive を検索
...
keybinding-desc-workspace-terminate-app = Ashide を終了
...
keybinding-desc-workspace-export-all-local-drive-objects = すべての Ashide Drive オブジェクトをエクスポート
keybinding-desc-workspace-install-cli = Oz CLI コマンドをインストール
keybinding-desc-workspace-uninstall-cli = Oz CLI コマンドをアンインストール

# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Ashide AI を切り替え
...
# Ashide: keybinding-desc-workspace-show-settings-account は Account 設定ページとともに削除。
...
keybinding-desc-workspace-show-settings-about-menu = Ashide について
...
keybinding-desc-workspace-view-logs = Ashide ログを表示
...
keybinding-desc-workspace-crash-macos = アプリをクラッシュさせる (ローカルクラッシュ診断のテスト用)
keybinding-desc-workspace-crash-other = アプリをクラッシュさせる (ローカルクラッシュ診断のテスト用)
...
keybinding-desc-workspace-open-ashide-launch-modal = [Debug] Ashide Launch モーダルを開く
keybinding-desc-workspace-reset-ashide-launch-modal-state = [Debug] Ashide Launch モーダル状態をリセット
keybinding-desc-workspace-install-opencode-warp-plugin = [Debug] OpenCode Warp プラグインをインストール
keybinding-desc-workspace-use-dev-checkout-opencode-warp-plugin = [Debug] 開発 checkout の OpenCode Warp プラグインを使用 (テスト専用)
...
keybinding-desc-input-show-network-log = Ashide ネットワークログを表示
...
keybinding-desc-ai-assistant-close = Ashide AI を閉じる
keybinding-desc-ai-assistant-focus-terminal-input = Ashide AI からターミナル入力にフォーカス
keybinding-desc-ai-assistant-restart = Ashide AI を再起動
...
keybinding-desc-terminal-ask-ai-selection = 選択について Ashide AI に質問
keybinding-desc-terminal-ask-ai-last-block = 直前のブロックについて Ashide AI に質問
keybinding-desc-terminal-ask-ai = Ashide AI に質問
...
toggle-suffix-ssh-wrapper = Ashide SSH ラッパー
...
toggle-suffix-crash-reporting = ローカルクラッシュ診断
...
settings-external-editor-choose-layout = Ashide でファイルを開くレイアウトを選択
...
settings-external-editor-prefer-markdown = Markdown ファイルを既定で Ashide の Markdown ビューアで開く
...
menu-block-open-in-warp = Ashide で開く
...
menu-block-ask-warp-ai = Ashide AI に質問
...
menu-input-ask-warp-ai = Ashide AI に質問
...
prompt-editor-warp-prompt-section = Ashide ターミナルプロンプト
...
welcome-tips-command-palette-description = キーボードから手を離さずに Ashide の機能をすべて簡単に発見できます。
...
welcome-tips-theme-picker-description = 組み込みテーマから選んで Ashide を自分好みに。または自作も可能。
...
drive-import-file-import-error = ファイルのインポートに失敗しました
drive-import-folder-import-error = フォルダのインポートに失敗しました
...
workflow-ai-assist-tooltip = Ashide AI でタイトル・説明・パラメータを生成
...
workflow-ai-assist-error-rate-limited = AI 使用量は現在利用できません。しばらくしてから再度お試しください。
...
command-palette-conversations-new-conversation = 新しい会話
conversation-fallback-title = 会話
workspace-session-bridge-fork-to-target = { $target } に Fork
workspace-session-bridge-edit-and-fork = 編集して Fork…
workspace-session-bridge-export-bundle = ポータブルセッション bundle を書き出す…
workspace-session-bridge-fork-unavailable = この会話はまだ Fork できません
workspace-session-bridge-edit-title = セッションを編集して Fork
workspace-session-bridge-edit-title-with-name = { $name } を編集して Fork
workspace-session-bridge-edit-target-label = Fork 先
workspace-session-bridge-edit-message-draft = メッセージ
workspace-session-bridge-edit-save = { $target } に保存して Fork
...
workspace-session-bridge-edit-message-kept = 保持
workspace-session-bridge-edit-message-removed = 削除済み
workspace-session-bridge-edit-message-removed-help = このメッセージは Fork 後のセッションに書き込まれません。
workspace-session-bridge-edit-remove-message = 削除
workspace-session-bridge-edit-restore-message = 復元
workspace-session-bridge-edit-error-missing-conversation = 会話が見つかりません。このダイアログを閉じて再試行してください。
workspace-session-bridge-edit-error-missing-target = Fork 先の Agent を選択してください。
workspace-session-bridge-edit-error-empty-draft = 少なくとも 1 件のメッセージを残してください。
workspace-session-bridge-forked-to-ashide = Ashide の新しいタブに Fork しました: { $title }
workspace-session-bridge-forked-to-target = { $target } の新しいタブに Fork しました: { $title }
workspace-session-bridge-forked-to-remote-target = リモートの { $target } 新しいタブに Fork しました: { $title }
workspace-session-bridge-open-tab-failed = { $target } の新しいタブを開けませんでした
workspace-session-bridge-open-remote-tab-failed = リモートの { $target } 新しいタブを開けませんでした
workspace-session-bridge-error-detail = { $context }: { $detail }
workspace-session-bridge-fork-failed = 会話の Fork に失敗しました
workspace-session-bridge-fork-cli-failed = CLI セッションの Fork に失敗しました
workspace-session-bridge-fork-remote-cli-failed = リモート CLI セッションの Fork に失敗しました
workspace-session-bridge-edit-fork-failed = 保存して Fork に失敗しました
workspace-session-bridge-bundle-exported = SessionBridge バンドルをエクスポートしました: { $path }
workspace-session-bridge-bundle-export-failed = SessionBridge バンドルのエクスポートに失敗しました
...
workspace-left-panel-environment-provider-manager = 環境プロバイダー
workspace-left-panel-ssh-manager-placeholder = SSH マネージャー
...
workspace-left-panel-skill-manager = Skill マネージャー
skill-manager-search-placeholder = skill を検索
skill-manager-filter-all = すべて
skill-manager-filter-provider = ソース
skill-manager-meta-default = デフォルト
skill-manager-meta-duplicate = 重複
skill-manager-empty = 現在のフィルターに一致する skill はありません。
skill-manager-preview-empty = skill を選択して SKILL.md をプレビューします。
skill-manager-remote-connecting = リモートランタイムに接続中です。準備ができると skill を読み込みます。
skill-manager-remote-loading = リモート環境の skill を読み込んでいます…
skill-manager-remote-error = リモート環境の skill 読み込みに失敗しました: { $error }
skill-manager-remote-empty = このリモート環境では skill が見つかりませんでした。現在のディレクトリの祖先と HOME の .agents/skills / .claude/skills をスキャンします。
workspace-session-navigator-title = Sessions
workspace-session-navigator-subtitle = Live terminals and recoverable CLI agent sessions. Click a row to focus or resume in its cwd.
...
workspace-session-navigator-terminal = ターミナル
workspace-session-navigator-agent = Agent セッション
workspace-session-navigator-welcome = Welcome
workspace-session-navigator-session = セッション
workspace-session-navigator-no-root = ワークスペース root は記録されていません
...
workspace-session-navigator-menu-resume = フォーカス / 再開
workspace-session-navigator-menu-restoring = 復元中…
workspace-session-navigator-menu-focus = セッションにフォーカス
workspace-session-navigator-menu-restore = セッションを再開
workspace-session-navigator-menu-rename-alias = エイリアス名を変更…
workspace-session-navigator-menu-clear-alias = エイリアスを消去
workspace-session-navigator-menu-copy-id = セッション ID をコピー
workspace-session-navigator-menu-copy-id-toast = { $id } をクリップボードにコピーしました
workspace-session-navigator-menu-pin = ピン留め
workspace-session-navigator-menu-unpin = ピン留め解除
workspace-session-navigator-menu-delete = 完全に削除
workspace-session-preview-title = セッション
workspace-session-preview-live = アクティブ
workspace-session-preview-history = 履歴
workspace-session-preview-environment-runtime = 環境 · { $environment }
workspace-session-preview-session-id = session { $id }
workspace-session-preview-updated = 更新 { $time }
workspace-session-preview-focus = フォーカス
workspace-session-preview-resume = 再開
workspace-session-preview-delete = 削除…
workspace-session-preview-exit-delete = 終了して削除
...
workspace-environment-kind-local = ローカル
workspace-environment-kind-ssh = SSH
workspace-environment-kind-container = コンテナ
workspace-environment-kind-wsl = WSL
workspace-environment-kind-custom = カスタム
workspace-environment-state-connected = 接続済み
workspace-environment-state-dormant = 未接続
workspace-environment-state-connecting = 接続中
workspace-environment-state-installing = リモートランタイムをインストール中
workspace-environment-state-reconnecting = 再接続中
workspace-environment-state-error = エラー
workspace-environment-tooltip-local = ローカル環境
workspace-environment-tooltip-runtime-connected = 環境は接続済みです
workspace-environment-tooltip-runtime-dormant = 環境はまだ接続されていません。クリックして接続します。初回接続時にそのホストへ Ashide helper をインストールまたは更新します。
workspace-environment-tooltip-runtime-connecting = 環境に接続中...
workspace-environment-tooltip-runtime-installing = リモートランタイムをインストールまたは起動中...
workspace-environment-tooltip-runtime-reconnecting = 環境を再接続中...
workspace-environment-tooltip-runtime-error = 環境接続でエラーが発生しました。クリックして再試行します。
workspace-environment-tooltip-generic = 環境
workspace-environment-tooltip-active-root = アクティブ root: { $root }
workspace-environment-add-ssh = SSH
workspace-environment-provider-picker-title = 環境ターゲット
workspace-environment-provider-picker-open-config = SSH 設定を開く
workspace-environment-provider-picker-open = 開く
...
workspace-restored-sessions-chip-label = セッション · { $count } 件復元
workspace-restored-sessions-chip-single = { $agent }
workspace-restored-sessions-state-dormant = 未接続
...
workspace-left-panel-ssh-manager-menu-open-environment = 環境として開く
...
workspace-left-panel-ssh-manager-pane-hint = 保存済み SSH サーバーをここで管理します。「接続」でターミナルを開き、「環境として開く」でリモートランタイムのセッションを開始します。
...
search-filter-display-environment-providers = 環境ターゲット
search-filter-placeholder-environment-providers = 環境ターゲットを検索...
...
global-search-environment-description = Global search には現在の app ワークスペースへのアクセスが必要ですが、この環境ではサポートされていません
...
wasm-nux-open-desktop-title = Ashide デスクトップで開きますか?
wasm-nux-open-desktop-detail = 今後のリンクは自動的にデスクトップで開きます。
wasm-nux-open-desktop-confirm = Ashide で開く
wasm-nux-download-title = Ashide デスクトップをダウンロードしますか?
wasm-nux-download-description = Ashide は AI と開発チームのナレッジを内蔵したインテリジェントなターミナルです。
wasm-nux-learn-more = 詳しく見る
wasm-nux-download-confirm = ダウンロード
wasm-nux-object-kind-warp-links = Ashide リンク
...
cli-agent-plugin-codex-install-title = Codex の Ashide 通知を有効化
cli-agent-plugin-codex-install-subtitle = Codex を最新版に更新し、フォーカス時通知を有効にすることで Ashide が作業中に通知を表示できるようにします。
...
cli-agent-plugin-deepseek-install-title = DeepSeek の Ashide 通知を有効化
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Ashide SSH 拡張をインストール中... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Ashide SSH 拡張をインストール中...
terminal-bootstrapping-updating-warp-ssh-extension = Ashide SSH 拡張を更新中...
...
agent-tip-command-palette = <keybinding> でコマンドパレットを開き、Ashide のアクションやショートカットにアクセスできます。
agent-tip-local-drive = 再利用可能なワークフロー、ノートブック、プロンプトを保存する場所:
agent-tip-redirect-running-agent = 新しいプロンプトを入力すると、実行中のエージェントを別方向に向け直せます。
agent-tip-add-context = `@` でファイル、ブロック、Ashide Drive オブジェクトからコンテキストをプロンプトに追加できます。
...
agent-tip-usage = `/usage` で現在の AI 使用量を表示できます。
...
hoa-welcome-banner-title = ユニバーサルエージェントサポートのご紹介: Ashide であらゆるコーディングエージェントをレベルアップ
...
hoa-feature-native-code-review-description = Ashide のコードレビューからインラインコメントを Claude Code、Codex、OpenCode へ直接送信
resource-center-whats-new-section = 新着情報
resource-center-getting-started-section = はじめに
resource-center-maximize-ashide-section = Ashide を最大限に活用
...
resource-center-command-palette-description = キーボードから Ashide のすべての機能にアクセス。
resource-center-set-theme-title = テーマを設定
resource-center-set-theme-description = テーマを選んで Ashide を自分好みに。
resource-center-custom-prompt-title = カスタムプロンプトを使用
resource-center-custom-prompt-description = PS1 設定を尊重するよう Ashide をセットアップ
resource-center-view-documentation = ドキュメントを表示
resource-center-integrate-ide-title = Ashide を IDE と連携
resource-center-integrate-ide-description = よく使う開発ツールから Ashide を起動できるよう設定
...
onboarding-intention-title = Ashide へようこそ
...
onboarding-ai-feature-warp-agents = Ashide エージェント
...
onboarding-agent-title = Ashide エージェントをカスタマイズ
...
onboarding-agent-disable-warp-agent = Ashide エージェントを無効化
onboarding-project-title = プロジェクトを開く
onboarding-project-subtitle = Ashide でのコーディング向けにプロジェクトを設定します。
...
onboarding-customize-title = Ashide をカスタマイズ
...
auth-local-privacy-note = Ashide はオンボーディングの選択をこのデバイスにローカル保存します。
onboarding-free-user-title = はじめましょう。
onboarding-free-user-agent-title = Ashide 内蔵エージェントによるエージェント駆動開発
onboarding-free-user-agent-description = Ashide 内蔵エージェント Oz でローカルに反復・計画・構築。
...
get-started-welcome-title = Ashide へようこそ
...
terminal-banner-p10k-supported = Powerlevel10k が Ashide に対応しました！{"  "}
...
terminal-banner-pure-unsupported = Pure はまだ Ashide で対応していません。代替として対応プロンプトの利用をご検討ください。{"  "}
terminal-loading-session = セッションを読み込み中…
environment-runtime-placeholder-title = 環境
environment-runtime-placeholder-empty = 環境ターミナルを準備中…
environment-runtime-placeholder-empty-detail = 接続の準備ができると自動的にターミナルを開きます。
environment-runtime-placeholder-installing = リモートランタイムをインストール中…
environment-runtime-placeholder-installing-detail = インストール完了後に自動的にターミナルを開きます。
environment-runtime-placeholder-reconnecting = 環境に再接続中…
environment-runtime-placeholder-reconnecting-detail = 再接続後に自動的にターミナルを復元します。
environment-runtime-placeholder-opening = 環境ターミナルを開いています…
environment-runtime-placeholder-opening-detail = ターミナルの作成後に自動的に表示します。
environment-runtime-placeholder-dormant = 環境ランタイムは未接続です
environment-runtime-placeholder-dormant-detail = 環境タブをクリックして再接続します。
environment-runtime-placeholder-error = 環境ランタイムの起動に失敗しました
environment-runtime-placeholder-error-detail = 環境を再選択すると自動的に再試行します。
...
command-search-ai-unavailable-contact-admin = AI 使用量は現在利用できません。プロバイダーの使用上限を確認してください。
command-search-ai-unavailable-prefix = AI 使用量は現在利用できません。{" "}
command-search-provider-usage-suffix = {" "}プロバイダーの使用上限を確認してください。
sharing-invite = 招待
...
settings-import-reset-to-warp-defaults = Ashide の既定にリセット
...
workspace-delete-session-dialog-title-restoring = “{ $title }”の復元をキャンセルして削除しますか？
workspace-delete-session-dialog-message-restoring = このウィンドウで保留中の復元をキャンセルし、この論理セッションのローカル記録を完全に削除します。この操作は元に戻せません。
workspace-delete-session-dialog-confirm-restoring = 復元をキャンセルして削除
workspace-delete-session-dialog-title-live = “{ $title }”を終了して削除しますか？
workspace-delete-session-dialog-message-live = アクティブなペインまたはタブを閉じ、この論理セッションのローカル記録を完全に削除します。この操作は元に戻せません。
workspace-delete-session-dialog-confirm-live = 終了して削除
workspace-delete-session-dialog-title = “{ $title }”を完全に削除しますか？
workspace-delete-session-dialog-message = この論理セッションのローカル記録を完全に削除します。この操作は元に戻せません。
workspace-delete-session-dialog-confirm = 完全に削除
workspace-close-session-confirm-title = セッションを閉じますか？
workspace-close-session-confirm-message = 共有中のセッションを閉じようとしています。閉じると、全員の共有が終了します。
workspace-auto-reload = 自動再読み込み
workspace-add-new-repo = {" "}+ 新しいリポジトリを追加
workspace-notification-permission-denied-toast = Ashide にはデスクトップ通知を送る権限がありません。
workspace-troubleshoot-notifications-link = 通知のトラブルシューティング
workspace-plan-saved-to-local-drive-toast = プランを Ashide Drive に保存しました
workspace-remote-control-link-copied-toast = リモートコントロールリンクをコピーしました。
workspace-update-now = 今すぐ更新
workspace-update-warp = Ashide を更新
...
workspace-version-deprecation-without-permissions-banner = 直ちに更新しないと一部の Ashide 機能が想定どおり動作しない可能性がありますが、Ashide は更新を実行できません。
workspace-new-version-unable-to-update-banner = 新しいバージョンが利用可能ですが、Ashide は更新を実行できません。
workspace-unable-to-launch-new-installed-version = Ashide はインストール済みの新バージョンを起動できませんでした。
...
terminal-ssh-report-issue-prefix = Ashide の SSH 安定性向上に取り組んでいます。次の対応をご検討ください:{" "}
...
terminal-agent-mode-setup-title = このコードベース向けに Ashide を最適化しますか？
...
notifications-banner-permissions-denied-title = Ashide は通知送信の権限を拒否されました。
...
code-review-agent-code-review-requires-ai = エージェントによるコードレビューには AI が必要です
...
notebook-local-save-conflict-resolution-message = 編集中に変更が加えられたため、このノートブックを保存できませんでした。作業内容をコピーして再読み込みしてください。
notebook-local-save-feature-not-available-message = ローカル保存経路が一時的に利用できないため、このノートブックを完全には保存できませんでした。変更はローカルに保持されています。後でやり直してください。
notebook-link-copied-toast = リンクをコピーしました
settings-share-with-team = ローカルに保存
tooltip-secrets-stay-local = *シークレットはローカルにのみ保存されます。
...
sftp-status-connecting = 接続中…
sftp-status-disconnected = 切断されました
sftp-action-reconnect = 再接続
sftp-download-failed = ダウンロードに失敗しました: { $error }
sftp-not-connected-to-server = サーバーに接続されていません
sftp-download-not-connected = ダウンロードに失敗しました: サーバーに接続されていません
sftp-rename-failed = 名前の変更に失敗しました: { $error }
sftp-delete-title = 項目を削除
sftp-delete-description-one = “{ $name }”を削除しますか？この操作は元に戻せません。
sftp-delete-description-many = { $count } 個の項目を削除しますか？この操作は元に戻せません。
sftp-current-name = 現在の名前: { $name }
sftp-transfer-status-pending = 待機中
sftp-transfer-status-transferring = 転送中
sftp-transfer-status-completed = 完了
sftp-transfer-status-failed = 失敗
sftp-transfer-status-cancelled = キャンセル済み
sftp-create-channel-failed = SFTP チャンネルの作成に失敗しました: { $error }
sftp-read-server-config-failed = サーバー設定の読み取りに失敗しました: { $error }
sftp-list-directory-failed = ディレクトリの読み取りに失敗しました: { $error }
sftp-create-folder-failed = フォルダーの作成に失敗しました: { $error }
sftp-server-config-not-found = サーバー設定が見つかりません
sftp-environment-not-connected = このホストの環境が接続されていません。先に環境として開いてから、ファイルを再度開いてください。
sftp-connection-cancelled = 接続がキャンセルされました
sftp-upload-failed = アップロードに失敗しました: { $error }
sftp-upload-not-connected = アップロードに失敗しました: サーバーに接続されていません
sftp-move-failed = 移動に失敗しました: { $error }
sftp-folder-name-empty = フォルダー名を空にすることはできません
sftp-invalid-name-path-separators = 無効な名前: パス区切り文字は使用できません
sftp-delete-failed = 削除に失敗しました: { $error }
sftp-file-name-invalid-characters = ファイル名に無効な文字が含まれています
sftp-name-empty = 名前を空にすることはできません
sftp-invalid-target-path = 移動先パスが無効です
