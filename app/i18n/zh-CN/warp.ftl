...
agent-management-source-local-agent = Ashide（本地智能体）
agent-management-source-cloud-agent = Ashide（本地智能体）
...
agent-management-metadata-usage-used = 用量：{ $usage }
...
workspace-toast-provider-usage-limit = 请检查你的模型提供商用量限制。
...
ai-document-save-and-sync-tooltip = 将此计划保存到本设备的 Ashide Drive
ai-document-show-in-local-drive = 在 Ashide Drive 中查看
...
code-open-file-unavailable-environment-tooltip = 此环境暂不支持打开文件
...
settings-ai-codebase-context-description = 允许 Ashide 智能体生成代码库的概要作为上下文。代码从不存储到我们的服务器。
...
settings-ai-cloud-agent-computer-use-description = 在 Ashide 应用中启动的智能体会话中启用计算机使用。
...
settings-show-blocks-confirm-dialog-text = 确定要取消共享此命令块吗？
    ...
    取消后将无法通过链接访问，并将从 Ashide 服务器永久删除。
...
settings-environments-page-description = 环境定义了你的 ambient agent 在哪里运行。可通过 GitHub（推荐）、Ashide 辅助配置或手动配置在几分钟内创建一个。
...
quit-warning-title-app = 退出 Ashide？
...
keybinding-desc-workspace-init-project = 为 Ashide 初始化项目
...
# 终端输入相关 binding
keybinding-desc-input-show-network-log = 显示 Ashide 网络日志
...
# Ashide 已移除远端 Agent 入口，此 key 实际不会被渲染;保留以匹配 en 兜底链。
...
toggle-suffix-crash-reporting = 本地崩溃诊断
...
drive-import-file-import-error = 文件导入失败
drive-import-folder-import-error = 文件夹导入失败
...
workspace-session-bridge-fork-remote-cli-failed = Fork 远程 CLI 会话失败
...
workspace-left-panel-ssh-manager-placeholder = SSH 管理器
...
workspace-session-navigator-subtitle = 当前活跃终端和可恢复的 CLI agent 会话。点击一行在对应 cwd 聚焦或恢复。
...
workspace-session-navigator-agent = Agent 会话
...
workspace-session-navigator-menu-resume = 聚焦 / 恢复
...
workspace-restored-sessions-tooltip = 已恢复的工作区会话
workspace-restored-sessions-tooltip-sublabel = 包含普通终端和从工作区元数据捕获的 CLI agent。
workspace-restored-sessions-toast = 已发现可恢复的 CLI agent 会话。具体 resume UI 会在下一步接入。
...
workspace-restored-sessions-action-state-adapter-pending = adapter 待接入
workspace-restored-sessions-action-state-unavailable = 暂不可用
workspace-restored-sessions-action-state-managed = 已管理
workspace-restored-sessions-action-state-restored = 已恢复
...
workspace-left-panel-ssh-manager-pane-hint = 在这里管理已保存的 SSH 服务器。使用「连接」打开终端，或使用「作为环境打开」进入远程运行时会话。
...
onboarding-project-subtitle = 设置项目，让 Ashide 更适合编码。
...
onboarding-free-user-agent-title = 使用 Ashide 内置智能体进行开发
onboarding-free-user-agent-description = 使用 Oz（Ashide 内置智能体）迭代、规划和构建。本地运行，不依赖 Ashide 云服务。
...
command-search-ai-unavailable-contact-admin = 你的用量似乎已用完。请联系团队管理员请检查你的模型提供商用量限制。
command-search-ai-unavailable-prefix = 你的用量似乎已用完。{" "}
command-search-provider-usage-suffix = {" "}以获取更多用量。
sharing-invite = 邀请
...
sftp-create-channel-failed = 创建 SFTP 通道失败：{ $error }
...
sftp-connection-cancelled = 连接已取消
