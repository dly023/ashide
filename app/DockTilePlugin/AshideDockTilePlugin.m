#include "AshideDockTilePlugin.h"

@implementation AshideDockTilePlugin {
...
- (instancetype)init {
    ...
    if (self) {
        @try {
            ...
            NSString *timestamp = [formatter stringFromDate:[NSDate date]];
            NSString *logPath = [NSString stringWithFormat:@"/tmp/ashide_docktile_%@.log", timestamp];
            NSError *error = nil;
            [[NSFileManager defaultManager] createFileAtPath:logPath contents:nil attributes:nil];
            _logFileHandle = [NSFileHandle fileHandleForWritingAtPath:logPath];
            [self logMessage:@"AshideDockTilePlugin initialized"];
...
// Protocol method that is invoked by the system when the dock for Ashide is updated.
...
- (void)setDockTile:(NSDockTile *)dockTile {
    @try {
        ...
        if (dockTile) {
            ...
            NSBundle *pluginBundle = [NSBundle bundleForClass:[AshideDockTilePlugin class]];
            NSString *path = [[pluginBundle bundlePath] stringByAppendingPathComponent:@"Contents/Info.plist"];
            NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];    
            NSString *bundleId = dict[@"MainAppBundleIdentifier"];
...
- (void)dealloc {
    @try {
        [self logMessage:@"AshideDockTilePlugin deallocating"];
