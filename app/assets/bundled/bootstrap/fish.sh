...
function warp_bootstrapped
  ...
  set -l escaped_working_directory (warp_escape_json (pwd -P))
  set -l escaped_codex_home ""
  if set -q CODEX_HOME
    set escaped_codex_home (warp_escape_json "$CODEX_HOME")
  end
  set -l escaped_claude_config_dir ""
  if set -q CLAUDE_CONFIG_DIR
    set escaped_claude_config_dir (warp_escape_json "$CLAUDE_CONFIG_DIR")
  end
  set -l escaped_json "{\"hook\": \"Bootstrapped\", \"value\": {\"histfile\": \"$escaped_histfile\", \"shell\": \"fish\", \"home_dir\": \"$HOME\", \"path\": \"$PATH\", \"working_directory\": \"$escaped_working_directory\", \"codex_home\": \"$escaped_codex_home\", \"claude_config_dir\": \"$escaped_claude_config_dir\", \"editor\": \"$escaped_editor\", \"abbreviations\": \"$escaped_abbr\", \"aliases\": \"$escaped_aliases\", \"function_names\": \"$function_names\", \"env_var_names\": \"$env_var_names\", \"builtins\": \"$escaped_builtins\", \"keywords\": \"\", \"shell_version\": \"$FISH_VERSION\", \"vi_mode_enabled\": \"$vi_mode_enabled\", \"os_category\": \"$os_category\", \"linux_distribution\": \"$linux_distribution\", \"wsl_name\": \"$WSL_DISTRO_NAME\", \"shell_path\": \"$escaped_shell_path\"}}"
  warp_send_json_message $escaped_json
end
