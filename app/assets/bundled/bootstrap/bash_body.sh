  ...
  if kernel_name="$(uname)"; then
    ...
    function warp_bootstrapped () {
        ...
        if [ "$WARP_IN_MSYS2" = false ]; then
          ...
          local escaped_working_directory="$(warp_escape_json "$(pwd -P)")"
          local escaped_codex_home="$(warp_escape_json "${CODEX_HOME:-}")"
          local escaped_claude_config_dir="$(warp_escape_json "${CLAUDE_CONFIG_DIR:-}")"
        ...
        if [ "$WARP_IN_MSYS2" = true ]; then
          ...
          warp_send_hook_kv_pair "working_directory" "$(pwd -P)"
          warp_send_hook_kv_pair "codex_home" "${CODEX_HOME:-}"
          warp_send_hook_kv_pair "claude_config_dir" "${CLAUDE_CONFIG_DIR:-}"
