...
// _not_ within the Ashide binary (where it could cause a terminal to temporarily flash on
// Windows).
...
fn generate_channel_config_if_needed(target_family: &str, target_os: &str) {
    ...
    // configs. This is expected for external contributors building Ashide OSS.
...
/// Copies the DLLs needed to run Ashide on Windows.
///
...
fn embed_resource_file(target_dir: &Path) {
    ...
    // 默认值与 publisher 一致定为「Ashide」,与 `script/windows/bundle.ps1` OSS 分支
    // (`$APP_NAME = 'Ashide'`) + AUMID `dev.ashide.Ashide` + Cargo bundle
    // metadata 全局对齐。Windows 任务管理器的进程分组名实际取自 PE 资源中的
    // `FileDescription` / `ProductName`(不是窗口标题),所以这里若回退默认 "Ashide",
    // 直接 `cargo build` 出来的 dev 二进制在任务管理器里会显示成 `Ashide(N)`。
    let app_name = env::var("ASHIDE_APP_NAME").unwrap_or_else(|_| "Ashide".to_owned());
    let bin_name = env::var("CARGO_BIN_NAME").unwrap_or("oss".to_owned());
    // 以 `ASHIDE_APP_PUBLISHER` 覆盖；默认与 installer / AUMID 一致为「Ashide」。
    // 保持 installer `MyAppPublisher`、Cargo bundle metadata `copyright`、
    // 进程 AUMID `dev.ashide.Ashide` 三处全局对齐，避免 Windows Shell
    // 因 publisher / product name fingerprint 不一致而 miss 掉 icon cache。
    let publisher = env::var("ASHIDE_APP_PUBLISHER").unwrap_or_else(|_| "Ashide".to_owned());
