...
///
/// Used by tab configs to distinguish terminal, agent, and ambient agent panes.
...
pub enum PaneMode {
    ...
    Agent,
    /// An ambient agent pane with no local shell.
    AmbientAgent,
}
    ...
    fn try_from(snapshot: PaneNodeSnapshot) -> Result<Self, ()> {
        match snapshot {
            ...
            PaneNodeSnapshot::Leaf(leaf) => match leaf.contents {
                ...
                // Environment-management pane variants are transient provider
                // surfaces and are not launch-config material.
