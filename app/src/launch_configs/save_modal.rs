...
pub enum LaunchConfigModalEvent {
    ...
    /// already, we may as well save it "manually" to the local config model, while waiting for the update
...
impl TypedActionView for LaunchConfigSaveModal {
    ...
    fn handle_action(&mut self, action: &ActionRequest, ctx: &mut ViewContext<Self>) {
        ...
        // this will become moot when we move launch configs into a local registry.
