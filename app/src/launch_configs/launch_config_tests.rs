...
#[test]
fn pane_mode_ambient_agent_uses_hard_cut_wire_name() {
    let template = PaneTemplateType::PaneTemplate {
        is_focused: Some(true),
        cwd: PathBuf::from("/some/dir"),
        commands: vec![],
        pane_mode: PaneMode::AmbientAgent,
        shell: None,
    };

    let value = serde_json::to_value(&template).expect("serialize pane template");
    assert_eq!(value["pane_mode"], "ambient_agent");

    let stale_cloud = serde_json::json!({
        "cwd": "/some/dir",
        "commands": [],
        "is_focused": true,
        "pane_mode": "cloud",
    });
    assert!(serde_json::from_value::<PaneTemplateType>(stale_cloud).is_err());
}
