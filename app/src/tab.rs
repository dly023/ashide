...
pub struct TabData {
    ...
    /// Ashide environment authority for this tab. The window-level environment is
    /// derived from the active tab; persisted sessions use this to avoid local / SSH
    /// metadata bleeding across tabs.
...
pub struct TabComponent<'a> {
    ...
    workspace_root_label: Option<String>,
    uses_vertical_tabs: bool,
    is_drag_target: bool,
...
impl<'a> TabComponent<'a> {
    pub fn new(
        ...
        let visible_pane_count = tab.pane_group.as_ref(ctx).visible_pane_ids().len();
        let split_pane_count = (visible_pane_count > 1).then_some(visible_pane_count);
        ...
        let environment_label = Self::environment_label(tab.environment.as_ref());
        let workspace_root_label =
            Self::workspace_root_label(tab.environment.as_ref(), &tooltip_directory);
        let uses_vertical_tabs = uses_vertical_tabs(ctx);
    ...
    fn render_split_pane_count_badge(&self, count: usize) -> Box<dyn Element> {
        let theme = self.appearance.theme();
        let text_color = theme.sub_text_color(theme.background());
        Container::new(
            Text::new_inline(count.to_string(), self.appearance.ui_font_family(), 10.)
    ...
    fn environment_label(environment: Option<&EnvironmentSnapshot>) -> Option<String> {
        ...
        let label = display
            .chip_label
            .unwrap_or_else(|| display.kind_label.to_string());

        Some(label).filter(|label| !label.trim().is_empty())
    }

    fn workspace_root_label(
        environment: Option<&EnvironmentSnapshot>,
        tooltip_directory: &Option<String>,
    ) -> Option<String> {
        environment
            .and_then(|environment| environment.active_workspace_root.clone())
            .or_else(|| tooltip_directory.clone())
            .filter(|label| !label.trim().is_empty())
    }

    fn tab_subtitle(&self) -> Option<String> {
        match (&self.environment_label, &self.workspace_root_label) {
            (Some(environment), Some(root)) => Some(format!("{environment} · {root}")),
            (Some(environment), None) => Some(environment.clone()),
    ...
    fn render_tab_content(&self) -> Box<dyn Element> {
        ...
        if self.is_tab_being_renamed() {
            ...
            let font_size = self.styles.default.font_size.expect("Font size defined");
            let title = Text::new_inline(self.title.clone(), font_family, font_size)
            ...
            if self.uses_vertical_tabs {
                if let Some(subtitle) = self.tab_subtitle() {
                    let subtitle =
                        Text::new_inline(subtitle, font_family, (font_size - 1.0).max(10.0))
                            .with_clip(ClipConfig::end())
                            .with_color(internal_colors::neutral_5(self.appearance.theme()))
                            .finish();
                    ...
                    Flex::column()
                        ...
                        .with_child(title)
                        .with_child(Container::new(subtitle).with_margin_top(1.).finish())
                        .finish()
                } else {
                    title
                }
    ...
    fn render_tab_container_internal(
        ...
        let full_tab_content = {
            ...
            if let Some(count) = self.split_pane_count {
                flex_row.add_child(
                    Container::new(self.render_split_pane_count_badge(count))
                        .with_margin_left(4.)
                        .finish(),
                );
            }
