...
fn test_update_both_values_changed() {
    App::test((), |mut app| async move {
        ...
        model_handle.read(&app, |model, _| {
            assert!(model.contains_any_non_current_app_environment_blocks());
            assert!(!model.contains_any_restored_non_current_app_environment_blocks());
        });
...
fn test_update_only_remote_toggles() {
    App::test((), |mut app| async move {
        ...
        // Update with (false, true) - only non-current-app environment blocks changes
        model_handle.update(&mut app, |model, ctx| {
...
fn test_toolbar_command_map_deserialize_from_map() {
    ...
    assert_eq!(map.0["^opencode"], "OpenCode");
    assert_eq!(map.0["^codex"], "");
}

#[test]
fn test_toolbar_command_map_deserialize_from_legacy_vec() {
    let json = serde_json::json!(["^claude", "^opencode", "^custom"]);
    let map: ToolbarCommandMap = serde_json::from_value(json).unwrap();
    assert_eq!(map.0.len(), 3);
    ...
    assert_eq!(keys, vec!["^claude", "^opencode", "^custom"]);
}
...
#[test]
fn test_cli_agent_history_enabled_agents_default_to_all_discoverable_native_resumers() {
    App::test((), |mut app| async move {
        initialize_settings_for_tests(&mut app);
...
#[test]
fn test_cli_agent_history_enabled_agents_ignore_stale_names_and_deduplicate_selection() {
    App::test((), |mut app| async move {
        initialize_settings_for_tests(&mut app);
