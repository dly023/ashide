...
/// onboarding should preserve the user's object-store-backed default execution
...
///      `InlineLocal`.
///   2. User logs into an existing account; their object-store default profile
///      arrives via initial bulk load and `InitialLoadCompleted` fires.
///   3. The reconciliation handler promotes the local state to `ObjectBacked`
///      with the stored profile's ObjectStore ID.
...
fn apply_onboarding_settings_preserves_existing_profile_object_on_existing_user_login() {
    App::test((), |mut app| async move {
        ...
        let stored_profile_server_id = StableObjectId::from(7);
        let object_store_id = ObjectStoreId::StableId(stored_profile_server_id);
        let stored_model_id = LLMId::from("claude-existing-object-store-model");
        let local_profile = AIExecutionProfile {
            name: "Default".to_string(),
            is_default_profile: true,
            base_model: Some(stored_model_id.clone()),
        ...
        profile_model.read(&app, |model, ctx| {
            ...
            assert_eq!(info.object_store_id(), Some(object_store_id));
            assert_eq!(info.data().base_model, Some(stored_model_id.clone()));
        });
        ...
        // Post-condition: the stored profile retains its stored values.
