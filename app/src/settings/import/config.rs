...
pub enum HotkeyError {
    #[error("A hotkey window opens in a way Ashide does not support")]
...
/// Parsed terminal setting imported from another profile.
...
pub trait ParseableConfig: PartialEq + Sized + Send {
    ...
    /// Creates a Ashide-readable `Config`. Sets corresponding errors if values have
