...
bitflags! {
    ...
    /// bit 20 = cmd, bit 21 = numpad (which Ashide does not store as a modifier).
...
impl TryFrom<ITermKeystroke> for Keystroke {
    ...
    fn try_from(value: ITermKeystroke) -> Result<Self, Self::Error> {
        ...
        Ok(Keystroke {
            ...
            // Neither Ashide nor iTerm supports Meta in global hotkeys.
            meta: false,
            key,
        })
    }
...
pub struct ITermGlobalHotkeyWindow {
    ...
    /// -2 = screen with cursor (not supported in Ashide), and >= 0 is the index of the screen.
    screen: i64,
...
impl ParseableConfig for ITermProfile {
    ...
    fn parse(mut self, fonts: &[FontInfo]) -> Config {
        // iTerm stores its fonts with internal names and supports styles as default terminal text, whereas Ashide changes fonts based on display name.
        // Only import a font if there is only one font whose iTerm name starts with the display name of a font Ashide supports.
        ...
        let mouse_and_scroll_reporting = match (self.mouse_reporting, self.scroll_reporting) {
            // Since this is the Ashide default, return None.
            (true, true) => None,
    ...
    fn remove_default_values(mut self) -> Self {
        ...
        // Ashide's default is not to open windows with a custom size,
        ...
        // iTerm's presets are the same as Ashide's
