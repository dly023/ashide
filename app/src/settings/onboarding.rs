...
fn apply_ui_customization_settings(
    ...
    LocalDriveSettings::handle(app).update(app, |settings, ctx| {
        report_if_error!(settings
            .enable_local_drive
            .set_value(ui.show_local_drive, ctx));
    });
...
fn apply_agent_settings(agent_settings: &AgentDevelopmentSettings, app: &mut AppContext) {
    ...
    AIExecutionProfilesModel::handle(app).update(app, |profiles, ctx| {
        ...
        // Preserve the existing object-store default profile for users who are
        // already logged in (or who log in at the end of onboarding). A
        // `Some` object_store_id means the profile is backed by an ObjectStore object that
        ...
        // customizations. Fresh `InlineLocal` default profiles (brand-new
        // users, or users without any object-store default yet) still receive the
        // onboarding values.
        if default_profile_info.object_store_id().is_some() {
            log::info!(
                "Preserving existing object-store default execution profile; skipping \
