...
pub const DISMISSED_AI_ASSISTANT_WELCOME_KEY: &str = "DismissedAiAssistantWarmWelcome";
    ...
    pub fn mark_changelog_shown(changelog_version: &str, ctx: &mut AppContext) -> bool {
        ctx.private_user_preferences()
            ...
            .is_ok_and(|mut versions| {
                ...
                if let Err(error) = ctx.private_user_preferences().write_value(
                    CHANGELOG_VERSIONS,
                    serde_json::to_string(&versions).expect("changelog versions should serialize"),
                ) {
                    log::warn!("failed to persist shown changelog versions: {error}");
                }
                true
            })
    }
