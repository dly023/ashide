...
/// Checks if local crash diagnostics are currently enabled.
...
impl ToCrashReportTags for CrashRecoveryMetadata {
    fn to_crash_report_tags(&self) -> impl IntoIterator<Item = (&str, String)> {
        #[cfg(enable_crash_recovery)]
        {
            ...
            [
                (
                    "ashide.crash_recovery_process.running",
                    bool_to_crash_metadata_value(self.is_crash_recovery_process_running),
                ),
                (
                    "ashide.handled_by_crash_recovery_process",
...
/// Initializes local crash diagnostics. Returns whether diagnostics are active.
pub(crate) fn init(ctx: &mut AppContext) -> bool {
    if !FeatureFlag::CrashReporting.is_enabled() {
        log::info!(
            "CrashReporting FeatureFlag is disabled; not initializing local crash diagnostics."
        );
        return false;
    }
    ...
    if is_crash_reporting_enabled {
        ...
        log::info!(
            "Local crash diagnostics setting is disabled; not initializing local diagnostics."
        );
    }
...
/// Returns the local crash diagnostics environment label.
...
fn init_local_crash_reporting(
    ...
    log::info!("Ashide local crash diagnostics: panic 日志只写本地,不向远端上报");
...
/// Stops local minidump diagnostics if they were started by this process.
...
pub fn suspend_crash_reporting_for_child_spawn() {
    log::info!("Ashide local crash diagnostics: macOS 原生 crash reporter 已剥离,跳过暂停");
}

pub fn resume_crash_reporting_after_child_spawn() {
    log::info!("Ashide local crash diagnostics: macOS 原生 crash reporter 已剥离,跳过恢复");
}

pub fn crash() {
    panic!("Ashide local crash diagnostics: crash() invoked for panic-hook smoke test");
}

/// 登录状态回调仍会触发此入口,但本地崩溃诊断不写入用户身份。
...
/// Sets the Ashide client type local crash metadata.
pub fn set_client_type_tag(client_id: &str) {
    set_tag("ashide.client_type", client_id);
}

/// Initializes the Ashide virtual environment local crash metadata group.
...
impl ToCrashReportTags for ApplicationStage {
    fn to_crash_report_tags(&self) -> impl IntoIterator<Item = (&str, String)> {
        [("ashide.application_stage", self.to_string())]
    }
}
...
impl ToCrashReportTags for GPUDeviceInfo {
    fn to_crash_report_tags(&self) -> impl IntoIterator<Item = (&str, String)> {
        [
            ("ashide.gpu.device.name", self.device_name.to_string()),
            ("ashide.gpu.device.type", self.device_type.to_string()),
            ("ashide.gpu.backend", self.backend.to_string()),
            ("ashide.gpu.driver.name", self.driver_name.to_string()),
            ("ashide.gpu.driver.info", self.driver_info.to_string()),
        ]
    }
}
