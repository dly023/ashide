//! Native local crash diagnostics adapter that uses the [`minidumper`] crate to write local dumps.
...
/// Initialize local minidump diagnostics.
pub fn init() {
...
/// Uninitialize local minidump diagnostics.
pub fn uninit() {
...
/// Capture a local minidump. On certain platforms, this will produce local crash details
...
/// Handle for minidump state that must be kept in scope while local diagnostics are enabled.
...
pub fn run_server(socket_path: &Path) -> anyhow::Result<()> {
    ...
    // do if local diagnostics fail, so creating the log file itself is best-effort.
    let log_dir = warp_core::paths::state_dir().join(warp_core::paths::WARP_LOGS_DIR);
    let _ = std::fs::create_dir_all(&log_dir);
    let log_path = log_dir.join("ashide-minidump.log");
    ...
    impl minidumper::ServerHandler for Handler {
        fn create_minidump_file(&self) -> Result<(File, PathBuf), io::Error> {
            ...
            let dump_path =
                dump_dir.join(format!("ashide-minidump-{}.dmp", Uuid::new_v4().simple()));
        ...
        fn on_minidump_created(
            ...
            log_local_minidump(crash_details, result.as_ref().ok(), dump_path, tags);
...
fn log_local_minidump(
    ...
    log::error!(
        "Ashide local minidump captured (message: {message}, dump bytes: {dump_size}, path: {dump_path}, tags: {tags:?})"
    );
}

impl MinidumpGuard {
    /// Set up minidump-backed local diagnostics. This spawns a child process that writes local
    /// minidumps, and a crash handler which forwards crashes to that child process.
    pub fn start() -> anyhow::Result<Self> {
        ...
        // Ensure that the crash server process can ptrace Ashide.
        #[cfg(target_os = "linux")]
    ...
    /// Forward tags for the minidump server to attach to local crash logs.
