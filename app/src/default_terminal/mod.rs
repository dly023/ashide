...
mod non_mac {
    ...
    /// Sets Ashide as the default terminal
...
pub struct DefaultTerminal {
    /// Whether the OS will treat Ashide as the default app for scripts/executables.
...
impl DefaultTerminal {
    ...
    /// This is an OS-level setting. Unlike most other settings, where Ashide is the source-of-truth
    /// for the value of the setting, it can be changed outside of Ashide. We monitor if it gets
    /// changed externally by checking when Ashide is focused.
    ...
    /// This is a one-way operation. Once we set the default terminal to Ashide, we can't really
    ...
    pub fn make_warp_default(&mut self, ctx: &mut ModelContext<Self>) {
        if let Err(e) = set_warp_as_default_terminal() {
            log::error!("Error setting Ashide as default terminal: {e:#}");
        } else {
