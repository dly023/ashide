...
use crate::ai::blocklist::inline_action::code_diff_view::DiffFileBackend;
...
impl InlineDiffView {
    ...
    ///
    /// The `file_backend` determines whether the file lives in the current app
    /// filesystem or an Environment Runtime.
    /// For `CurrentAppFileSystem`, the file is registered by path on the current app filesystem.
    /// For `EnvironmentRuntime`, the file is registered against the environment backend so
    /// that `save()` / `delete()` dispatch over the runtime client.
    ///
    /// This must be called after construction for non-WASM environments.
    #[cfg(not(target_family = "wasm"))]
    pub fn register_file(&mut self, file_backend: &DiffFileBackend, ctx: &mut ViewContext<Self>) {
        let Some(file_path) = &self.file_path else {
            return;
        };

        let file_model = FileModel::handle(ctx);
        let file_id = match file_backend {
            DiffFileBackend::CurrentAppFileSystem => {
            ...
            DiffFileBackend::EnvironmentRuntime {
                host_id,
                session_id,
            } => {
                let host_id = host_id.clone();
                let session_id = *session_id;
                let environment_path = file_path.clone();
                file_model.update(ctx, |file_model, _ctx| {
                    file_model.register_remote_file(host_id, session_id, environment_path)
                })
            }
        };
