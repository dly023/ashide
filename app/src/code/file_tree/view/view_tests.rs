...
use repo_metadata::entry::{DirectoryEntry, Entry, FileMetadata, SymlinkEntry};
...
use super::{
    remove_file_tree_item, DirectoryLoadTracker, FileTreeItem, FileTreeView, RootDirectory,
};

fn symlink_item(
    path: &std::path::Path,
    target_kind: repo_metadata::SymlinkTargetKind,
) -> FileTreeItem {
    use repo_metadata::file_tree_store::FileTreeSymlinkMetadata;
    use std::sync::Arc;
    use warpui::elements::{DraggableState, MouseStateHandle};

    FileTreeItem::Symlink {
        metadata: FileTreeSymlinkMetadata {
            path: Arc::new(std_path(path)),
            target_kind,
            ignored: false,
            loaded: target_kind != repo_metadata::SymlinkTargetKind::Directory,
        },
        depth: 1,
...
#[test]
fn symlink_target_kind_is_the_only_navigation_capability_source() {
    use repo_metadata::SymlinkTargetKind;

    let temp = tempfile::TempDir::new().unwrap();
    let directory_link = symlink_item(
        &temp.path().join("directory-link"),
        SymlinkTargetKind::Directory,
    );
    let file_link = symlink_item(&temp.path().join("file-link"), SymlinkTargetKind::File);
    let broken_link = symlink_item(&temp.path().join("broken-link"), SymlinkTargetKind::Missing);
    let loop_link = symlink_item(&temp.path().join("loop-link"), SymlinkTargetKind::Other);

    assert!(directory_link.is_directory_like());
    assert!(!directory_link.is_file_like());
    assert!(!file_link.is_directory_like());
    assert!(file_link.is_file_like());
    assert!(!broken_link.is_directory_like());
    assert!(!broken_link.is_file_like());
    assert!(!loop_link.is_directory_like());
    assert!(!loop_link.is_file_like());
}

#[cfg(unix)]
#[test]
fn deleting_directory_symlink_removes_link_without_touching_target() {
    use repo_metadata::SymlinkTargetKind;
    ...
    let item = symlink_item(&link, SymlinkTargetKind::Directory);

    remove_file_tree_item(&item, &link).unwrap();
    assert!(!link.exists());
...
fn tree_with_symlink(
    ...
    Entry::Directory(DirectoryEntry {
        path: std_path(root),
        children: vec![Entry::Symlink(SymlinkEntry {
            path: std_path(&root.join("dir-link")),
            target_kind,
            children,
            ignored: false,
            loaded,
...
#[test]
fn replacing_tree_snapshot_reloads_expanded_directory_symlink() {
    use repo_metadata::SymlinkTargetKind;
    use std::collections::{HashMap, HashSet};

    let root = std::path::Path::new("/repo");
    let link = std_path(&root.join("dir-link"));
    let old_nested = std_path(&root.join("dir-link/old-nested"));
    let old_tree = tree_with_symlink(
        root,
        SymlinkTargetKind::Directory,
        true,
        vec![Entry::Directory(DirectoryEntry {
            path: old_nested.clone(),
            children: Vec::new(),
            ignored: false,
            loaded: true,
        })],
    );
    let mut root_directory = RootDirectory {
        entry: old_tree,
        expanded_folders: HashSet::from([link.clone(), old_nested.clone()]),
        items: Vec::new(),
    ...
    let paths_to_load = root_directory.replace_entry(tree_with_symlink(
        root,
        SymlinkTargetKind::Directory,
        false,
        Vec::new(),
    ));
    assert_eq!(paths_to_load, vec![link.clone()]);
    assert!(root_directory.expanded_folders.contains(&link));
    assert!(
        root_directory.expanded_folders.contains(&old_nested),
        "descendant expansion intent must survive while its ancestor is unloaded"
    );
    ...
    assert!(
        !root_directory.expanded_folders.contains(&old_nested),
        "once the replacement target is loaded, expansion state for missing old children is stale"
    );
    ...
    assert!(paths_to_load.is_empty());
    assert!(root_directory.expanded_folders.is_empty());
}

#[test]
fn directory_load_tracker_is_single_flight_until_completion() {
    use repo_metadata::SymlinkTargetKind;

    let root = std_path(std::path::Path::new("/repo"));
    let link = std_path(std::path::Path::new("/repo/dir-link"));
    let mut tracker = DirectoryLoadTracker::default();

    assert!(tracker.begin(&root, &link));
    assert!(!tracker.begin(&root, &link));

    let unloaded = tree_with_symlink(
        std::path::Path::new("/repo"),
        SymlinkTargetKind::Directory,
        false,
        Vec::new(),
    );
    tracker.reconcile_root(&root, &unloaded);
    assert!(
        !tracker.begin(&root, &link),
        "an unloaded replacement snapshot must not recursively start the same load"
    );

    tracker.finish(&root, &link);
    assert!(
        tracker.begin(&root, &link),
        "an explicit failure/completion boundary must make the path retryable"
    );

    let loaded = tree_with_symlink(
        std::path::Path::new("/repo"),
        SymlinkTargetKind::Directory,
        true,
        Vec::new(),
    );
    tracker.reconcile_root(&root, &loaded);
    assert!(
        tracker.begin(&root, &link),
        "a materialized snapshot must release the single-flight owner"
    );
}

#[test]
fn directory_load_tracker_releases_only_materialized_path() {
    use repo_metadata::SymlinkTargetKind;

    let root_path = std::path::Path::new("/repo");
    let root = std_path(root_path);
    let first = std_path(&root_path.join("first-link"));
    let second = std_path(&root_path.join("second-link"));
    let snapshot: FileTreeEntry = Entry::Directory(DirectoryEntry {
        path: root.clone(),
        children: vec![
    ...
    assert!(tracker.begin(&root, &first));
    assert!(tracker.begin(&root, &second));
    tracker.reconcile_root(&root, &snapshot);

    assert!(
        tracker.begin(&root, &first),
        "the materialized path must release its own single-flight owner"
    );
    assert!(
        !tracker.begin(&root, &second),
        "an unrelated unresolved sibling load must remain owned"
    );
}

#[test]
fn existing_root_entries_have_single_snapshot_replacement_gateway() {
    let source = include_str!("../view.rs");
    assert!(
        !source.contains("root_dir.entry ="),
        "existing root snapshots must go through replace_root_entry so expansion intent is reconciled"
    );
}
