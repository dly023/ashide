...
pub(super) fn sort_entries_for_file_tree(
    ...
    let is_dir_1 = entry_1.is_directory_like();
    let is_dir_2 = entry_2.is_directory_like();
...
impl FileTreeView {
    ...
    pub(super) fn create_new_file(&mut self, id: &FileTreeIdentifier, ctx: &mut ViewContext<Self>) {
        ...
        let (path, depth) = match root_dir.items.get(id.index) {
            ...
            Some(FileTreeItem::Symlink {
                metadata, depth, ..
            }) if metadata.target_kind == repo_metadata::SymlinkTargetKind::Directory => {
                (metadata.path.clone(), *depth)
            }
            Some(FileTreeItem::Symlink { .. }) | None => return,
        };
