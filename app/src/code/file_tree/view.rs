...
use repo_metadata::current_app_model::IndexedRepoState;
use repo_metadata::file_tree_store::{
    FileTreeDirectoryEntryState, FileTreeEntryState, FileTreeFileMetadata, FileTreeSymlinkMetadata,
};
use repo_metadata::RepoMetadataModel;
use repo_metadata::{FileTreeEntry, HiddenEntryPolicy, SymlinkTargetKind};
...
use crate::util::openable_file_type::{
    resolve_file_target_to_open_in_ashide, resolve_file_target_with_editor_choice,
};
...
enum FileTreeItem {
    ...
    /// A symbolic link whose lexical path remains the UI and mutation identity.
    Symlink {
        metadata: FileTreeSymlinkMetadata,
        depth: usize,
        mouse_state_handle: MouseStateHandle,
        draggable_state: DraggableState,
    },
...
impl FileTreeItem {
    fn path(&self) -> &StandardizedPath {
        match self {
            ...
            FileTreeItem::Symlink { metadata, .. } => &metadata.path,
        }
    }

    fn is_directory_like(&self) -> bool {
        matches!(self, Self::DirectoryHeader { .. })
            || matches!(
                self,
                Self::Symlink { metadata, .. }
                    if metadata.target_kind == SymlinkTargetKind::Directory
            )
    }
    ...
    fn is_file_like(&self) -> bool {
        matches!(self, Self::File { .. })
            || matches!(
                self,
                Self::Symlink { metadata, .. }
                    if metadata.target_kind == SymlinkTargetKind::File
            )
    }
}

fn remove_file_tree_item(item: &FileTreeItem, path: &Path) -> std::io::Result<()> {
    match item {
        FileTreeItem::File { .. } | FileTreeItem::Symlink { .. } => std::fs::remove_file(path),
        FileTreeItem::DirectoryHeader { .. } => std::fs::remove_dir_all(path),
    }
}
...
struct RootDirectory {
    ...
    /// The environment host this root belongs to, if any. `None` for terminal roots.
    environment_host_id: Option<HostId>,
    /// Exact Environment Runtime transport session for this root.
    environment_session_id: Option<warp_core::SessionId>,
}

impl RootDirectory {
    /// Returns whether this root is backed by a environment runtime.
    fn is_environment_root(&self) -> bool {
        self.environment_host_id.is_some()
    }

    /// Replaces the backing tree snapshot while preserving only valid expansion intent.
    ///
    /// Repository updates may replace an expanded directory (including a directory symlink)
    /// with an unloaded entry at the same lexical path. Expansion is a view-level intent, so
    /// replacing the snapshot must also return every expanded entry that now needs loading.
    /// Otherwise the row remains visually expanded but empty until the user collapses and
    /// expands it again.
    fn replace_entry(&mut self, entry: FileTreeEntry) -> Vec<StandardizedPath> {
        self.entry = entry;

        self.expanded_folders.retain(|path| {
            if let Some(entry) = self.entry.get(path) {
                return entry.is_directory_like();
            }

            // A descendant may be temporarily absent because its nearest materialized
            // ancestor is unloaded. Keep that expansion intent until the ancestor loads;
            // the next snapshot replacement will either restore or discard it.
            path.ancestors().skip(1).find_map(|ancestor| {
                self.entry
                    .get(&ancestor)
                    .map(|entry| entry.is_directory_like() && !entry.loaded())
            }) == Some(true)
        });

        let mut paths_to_load: Vec<_> = self
            .expanded_folders
            .iter()
            .filter(|path| {
                self.entry
                    .get(path)
                    .is_some_and(|entry| entry.is_directory_like() && !entry.loaded())
            })
            .cloned()
            .collect();
        paths_to_load.sort_by_key(|path| path.ancestors().count());
        paths_to_load
    }
}

#[derive(Default)]
struct DirectoryLoadTracker {
    in_flight: HashSet<(StandardizedPath, StandardizedPath)>,
}

impl DirectoryLoadTracker {
    fn begin(&mut self, root: &StandardizedPath, path: &StandardizedPath) -> bool {
        self.in_flight.insert((root.clone(), path.clone()))
    }

    fn finish(&mut self, root: &StandardizedPath, path: &StandardizedPath) {
        self.in_flight.remove(&(root.clone(), path.clone()));
    }

    fn finish_root(&mut self, root: &StandardizedPath) {
        self.in_flight.retain(|(load_root, _)| load_root != root);
    }

    fn reconcile_root(&mut self, root: &StandardizedPath, entry: &FileTreeEntry) {
        self.in_flight.retain(|(load_root, path)| {
            load_root != root
                || entry
                    .get(path)
                    .is_some_and(|entry| entry.is_directory_like() && !entry.loaded())
...
pub struct FileTreeView {
    ...
    /// [`RepoMetadataModel`] for file watching.
    #[cfg(feature = "local_fs")]
    ...
    /// Root/path-level single-flight guard for expanded directory loads.
    /// Prevents synchronous local load failures from recursively re-entering
    /// snapshot replacement and prevents duplicate remote load requests.
    directory_loads: DirectoryLoadTracker,
    project_explorer_visibility: Option<ModelHandle<ProjectExplorerVisibilityPreference>>,
    show_hidden_entries: bool,
    selected_path_identity: Option<StandardizedPath>,
}
...
impl FileTreeView {
    ...
    /// Returns whether the item identified by `id` belongs to a environment root.
    fn is_environment_item(&self, id: &FileTreeIdentifier) -> bool {
        self.root_directories
            .get(&id.root)
            .is_some_and(|r| r.is_environment_root())
    }
    ...
    pub fn set_is_active(&mut self, is_active: bool, ctx: &mut ViewContext<Self>) {
        self.set_is_active_current_app_file_system(is_active, ctx);
    }
    ...
    #[cfg(feature = "local_fs")]
    fn set_is_active_current_app_file_system(
        &mut self,
        is_active: bool,
        ...
        if is_active {
            ...
            // Skip environment-backed roots — their data comes from server pushes,
            // not from terminal DetectedRepositories / lazy-loading.
            let terminal_dirs: Vec<_> = self
                ...
                .cloned()
                .collect();
            self.update_directory_contents(&terminal_dirs, false, ctx);

            // Catch up on environment roots already in root_directories whose
            // content may have changed while inactive. We only refresh
            // existing roots — new environment roots are managed by the
            // workspace via `set_environment_root_directories`.
            let existing_environment_repo_ids: Vec<_> = self
                .root_directories
                .iter()
                .filter_map(|(_, root_dir)| {
                    let host_id = root_dir.environment_host_id.as_ref()?;
                ...
                .collect();
            if !existing_environment_repo_ids.is_empty() {
                self.insert_or_update_environment_roots(
                    &existing_environment_repo_ids,
                    None,
                    false,
                    ctx,
                );
    ...
    /// Inserts or updates environment root directories. When `insert_only` is
    ...
    #[cfg(feature = "local_fs")]
    fn insert_or_update_environment_roots(
        &mut self,
        environment_repo_ids: &[repo_metadata::RemoteRepositoryIdentifier],
        binding_session_id: Option<warp_core::SessionId>,
        insert_only: bool,
        ctx: &mut ViewContext<Self>,
    ) {
        ...
        for environment_repo_id in environment_repo_ids {
            let repo_path = environment_repo_id.path.clone();

            if insert_only && self.displayed_directories.contains(&repo_path) {
                continue;
            }

            let id = RepositoryIdentifier::Remote(environment_repo_id.clone());
            let Some(entry) = RepoMetadataModel::as_ref(ctx)
            ...
            self.displayed_directories.retain(|existing| {
                ...
                if dominated {
                    ...
                    self.directory_loads.finish_root(existing);
                }
                !dominated
            });

            let host_id = environment_repo_id.host_id.clone();
            if let Some(root_dir) = self.root_directories.get_mut(&repo_path) {
                root_dir.environment_host_id = Some(host_id);
                if let Some(binding_session_id) = binding_session_id {
                    root_dir.environment_session_id = Some(binding_session_id);
                }
                self.replace_root_entry(&repo_path, entry, ctx);
            } else {
                let Some(binding_session_id) = binding_session_id else {
                    log::warn!(
                        "refusing to create Environment file-tree root without a SessionId: {repo_path:?}"
                    );
                    continue;
                };
                self.root_directories.insert(
                    ...
                    RootDirectory {
                        ...
                        environment_host_id: Some(host_id),
                        environment_session_id: Some(binding_session_id),
                    },
                );
            }
    ...
    fn handle_repository_metadata_event(
        ...
        match event {
                ...
                if !root_paths.is_empty() {
                    ...
                    if let Some(entry) = entry {
                        for root_path in root_paths {
                            self.replace_root_entry(&root_path, entry.clone(), ctx);
                        }
            ...
            RepoMetadataEvent::RepositoryUpdated {
                id: RepositoryIdentifier::Remote(environment_repo_id),
            } => {
                // Only update existing environment roots — never add new ones.
                // New environment roots are pushed by the workspace via
                // `set_environment_root_directories` when `NavigatedToDirectory`
                ...
                let repo_path = &environment_repo_id.path;
                    ...
                    self.root_directories
                        ...
                        .is_some_and(|root_dir| {
                            root_dir
                                .environment_host_id
                                .as_ref()
                                .is_some_and(|h| *h == environment_repo_id.host_id)
                        });
                if belongs_to_this_tree {
                    self.insert_or_update_environment_roots(
                        std::slice::from_ref(environment_repo_id),
                        None,
                        false,
                        ctx,
                    );
            ...
            RepoMetadataEvent::RepositoryRemoved {
                ...
                self.directory_loads.finish_root(repo_path);
                self.rebuild_flattened_items();
                ctx.notify();
            }
            RepoMetadataEvent::UpdatingRepositoryFailed {
                ...
                log::warn!(
                    "unexpected coarse remote repository failure for {}; directory loads require exact path completion",
                    environment_repo_id.path
                );
            }
            RepoMetadataEvent::DirectoryLoadFinished {
                ...
                if self
                    .root_directories
                    .get(&environment_repo_id.path)
                    .is_some_and(|root| {
                        root.environment_host_id
                            .as_ref()
                            .is_some_and(|host_id| host_id == &environment_repo_id.host_id)
                    })
                {
                    self.directory_loads
                        .finish(&environment_repo_id.path, dir_path);
                }
                if !*succeeded {
                    log::warn!(
                        "remote directory load failed for {}/{}",
                        environment_repo_id.path,
                        dir_path
                    );
                }
            ...
            RepoMetadataEvent::DirectoryLoadFinished {
                ...
                log::warn!("unexpected local directory-load completion event for {path}");
            }
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        Self::new_with_visibility(None, ctx)
    }
    ...
    pub fn new_project_explorer(
        ...
        Self::new_with_visibility(Some(visibility), ctx)
    }
    ...
    fn new_with_visibility(
        ...
        let picker = Self {
            ...
            directory_loads: DirectoryLoadTracker::default(),
            project_explorer_visibility,
    ...
    /// Sets the environment root directories to display in the file tree.
    ///
    /// This is the environment runtime equivalent of [`set_root_directories`]. It
    /// inserts or updates the given environment repos and removes any existing
    /// environment roots that are NOT in `repos`. Terminal roots are unaffected.
    #[cfg(feature = "local_fs")]
    pub fn set_environment_root_directories(
        &mut self,
        repos: &[repo_metadata::RemoteRepositoryIdentifier],
        binding_session_id: warp_core::SessionId,
        ctx: &mut ViewContext<Self>,
    ) {
        // Remove environment roots that are no longer in the desired set.
        let desired_paths: HashSet<&StandardizedPath> = repos.iter().map(|id| &id.path).collect();
        let stale_environment_paths: Vec<StandardizedPath> = self
            .root_directories
            .iter()
            .filter(|(_, root_dir)| root_dir.is_environment_root())
            .filter(|(path, _)| !desired_paths.contains(path))
            .map(|(path, _)| path.clone())
            .collect();
        let mut changed = !stale_environment_paths.is_empty();
        for path in &stale_environment_paths {
            self.displayed_directories.retain(|p| p != path);
            self.root_directories.remove(path);
            self.directory_loads.finish_root(path);
        }

        // Insert or update the desired environment roots.
        // `insert_or_update_environment_roots` skips repos whose model data
        ...
        // in the tree.
        if !repos.is_empty() {
            // `insert_or_update_environment_roots` already rebuilds + notifies
            // when it mutates state. Track whether the placeholder loop
            // below adds anything new so we only rebuild a second time
            // when necessary.
            self.insert_or_update_environment_roots(repos, Some(binding_session_id), false, ctx);

            for environment_repo_id in repos {
                let repo_path = &environment_repo_id.path;
    ...
    ///
    /// This only manages terminal roots. Environment-backed roots (those with a
    /// `environment_host_id`) are managed by [`set_environment_root_directories`]
    /// and are preserved across calls to this method.
    ///
    /// When multiple input paths have an ancestor/descendant relationship
    /// among terminal roots, only the surviving ancestor is displayed and the
    ...
    pub fn set_root_directories(&mut self, paths: Vec<PathBuf>, ctx: &mut ViewContext<Self>) {
        ...
        // Collect existing environment directories so we can preserve them.
        // Environment CWDs are not present in `std_paths` because `normalize_cwd`
        // in the `WorkingDirectoriesModel` drops paths that cannot be
        // canonicalized on the terminal filesystem.
        let existing_environment_dirs: Vec<StandardizedPath> = self
            .displayed_directories
            .iter()
            .filter(|p| {
                self.root_directories
        ...
        // Only terminal paths participate in ancestor-dedup.
        let terminal_inputs: Vec<StandardizedPath> = std_paths
            .iter()
            .filter(|p| {
                !self
                    .root_directories
        ...
        // Ancestor-dedup only terminal inputs. Shared with `GlobalSearchView`
        // via `warp_util::path::group_roots_by_common_ancestor`.
        let grouping = warp_util::path::group_roots_by_common_ancestor(&terminal_inputs);

        // Final displayed order: terminal surviving roots (in input order),
        // followed by preserved environment roots (in their existing order).
        let new_displayed: Vec<StandardizedPath> = grouping
            .roots
            .iter()
            .cloned()
            .chain(existing_environment_dirs.iter().cloned())
            .collect();
        ...
        // Retain roots that are in `new_displayed`. Environment roots that
        // were properly pushed by `set_environment_root_directories` are
        // already included via the `existing_environment_dirs` chain above.
        self.root_directories
            .retain(|root, _| new_displayed.contains(root));
        self.displayed_directories = new_displayed.clone();
        // Only update terminal roots — environment roots are managed by
        // `set_environment_root_directories` and must not be passed to
        // `update_directory_contents` which would overwrite their
        // environment-backed entry with an empty terminal lazy-loaded one.
        #[cfg(feature = "local_fs")]
        {
            let terminal_displayed: Vec<_> = new_displayed
                .iter()
                .filter(|p| {
                    !self
                        .root_directories
                ...
                .cloned()
                .collect();
            self.update_directory_contents(&terminal_displayed, new_last_directory, ctx);
        }
        ...
        // Focus-follow: for the most-recent surviving current-app root, if it
    ...
    fn migrate_absorbed_root_state(
        ...
        for absorbed_root in absorbed {
            ...
            self.directory_loads.finish_root(absorbed_root);
    ...
    fn find_directory_header_id(
        ...
        let (index, _) = root_dir
            ...
            .iter()
            .enumerate()
            .find(|(_, item)| item.is_directory_like() && item.path() == directory_path)?;
        ...
        for root_path in paths {
                ...
                DetectedRepositories::as_ref(ctx).get_root_for_path(&root_local)
            {
                ...
                if let Some(repo_entry) = repo_entry {
                    ...
                    self.replace_root_entry(root_path, repo_entry, ctx);
                } else {
    ...
    fn ensure_loaded_path(
        ...
        if !target_item.is_directory_like() {
            return;
        }

        if !self.directory_loads.begin(root_path, path) {
            return;
        }
    ...
    fn load_directory_from_model(
        ...
        // Check if this is a environment-backed root.
        if self
            .root_directories
            .get(root_path)
            .is_some_and(|r| r.is_environment_root())
        {
            self.load_environment_directory(root_path, target_item, ctx);
            return;
        }

        let Some(root_dir) = self.root_directories.get(root_path) else {
            self.directory_loads.finish(root_path, target_item.path());
            return;
        };
        ...
        let entry = RepoMetadataModel::as_ref(ctx)
            ...
            .map(|state| state.entry.clone());
        if let Some(entry) = entry {
            self.replace_root_entry(root_path, entry, ctx);
        }
        self.directory_loads.finish(root_path, &dir_path);
    }

    /// Sends a `LoadRepoMetadataDirectory` request to the environment runtime for
    /// an unloaded subdirectory. The response flows back through
    /// `RepoMetadataEvent::FileTreeEntryUpdated { Remote }` which rebuilds
    /// the view automatically.
    #[cfg(feature = "local_fs")]
    fn load_environment_directory(
        &mut self,
        root_path: &StandardizedPath,
        target_item: &FileTreeEntryState,
        ctx: &mut ViewContext<Self>,
    ) {
        if !crate::workspace::environment_runtime::environment_runtime_feature_enabled() {
            self.directory_loads.finish(root_path, target_item.path());
            return;
        }
        ...
        let dir_path = target_item.path().clone();
        let Some(host_id) = root_dir.environment_host_id.as_ref() else {
            log::warn!("load_environment_directory: no host_id for {root_path}");
            self.directory_loads.finish(root_path, target_item.path());
            return;
        };

        if let Err(error) =
            crate::workspace::environment_runtime::load_repo_metadata_directory_for_host(
                host_id, repo_root, dir_path, ctx,
            )
        {
            log::warn!("load_environment_directory: {error}");
            self.directory_loads.finish(root_path, target_item.path());
        }
    }
    ...
    /// Installs a new model snapshot and immediately reconciles the view's expansion intent.
    /// This is the only update path for an existing root entry; callers must not assign
    /// `RootDirectory::entry` directly because doing so can leave expanded unloaded rows empty.
    fn replace_root_entry(
        &mut self,
        root_path: &StandardizedPath,
        entry: FileTreeEntry,
        ctx: &mut ViewContext<Self>,
        ...
        let paths_to_load = root_dir.replace_entry(entry);
        self.directory_loads
            .reconcile_root(root_path, &root_dir.entry);
        for path in paths_to_load {
            self.ensure_loaded_path(root_path, &path, ctx);
        }
    }

    /// Ensures a displayed standalone current-app directory is registered with
    /// [`RepoMetadataModel`] as a lazily-loaded path while the file tree is active, then
    ...
    fn register_and_refresh_lazy_loaded_directory(
        ...
        let entry = RepoMetadataModel::as_ref(ctx)
            .get_repository(&id, ctx)
            .map(|state| state.entry.clone());
        self.replace_root_entry(
            path,
            entry.unwrap_or_else(|| Self::create_empty_entry(path)),
            ctx,
        );
    }
    ...
    fn show_exceeded_file_limit_toast(ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            ...
            toast_stack.add_transient_toast(toast, window_id, ctx);
        });
    }

    fn show_unavailable_symlink_toast(target_kind: SymlinkTargetKind, ctx: &mut ViewContext<Self>) {
        let (message, object_id) = match target_kind {
            SymlinkTargetKind::Missing => (
                crate::t!("file-browser-broken-symlink"),
                "file_tree_broken_symlink",
            ),
            SymlinkTargetKind::Other => (
                crate::t!("file-browser-unsupported-symlink"),
                "file_tree_unsupported_symlink",
            ),
            SymlinkTargetKind::File | SymlinkTargetKind::Directory => return,
        };
        let window_id = ctx.window_id();
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::error(message).with_object_id(object_id.to_string());
            toast_stack.add_transient_toast(toast, window_id, ctx);
        });
    }
    ...
    fn rebuild_flatten_items_and_select_path(
        ...
        for root_path in self.displayed_directories.clone() {
            ...
            let selected_item_path = id_to_preserve
                ...
                .or_else(|| {
                    self.selected_path_identity
                        .as_ref()
                        .filter(|path| path.starts_with(&root_path))
                        .cloned()
                });
            if selected_item_path.is_some() {
                self.selected_path_identity = selected_item_path.clone();
            }
    ...
    fn flatten_entry_for_root(
        ...
        let ignored = entry_map
            .get(current_path)
            .is_some_and(|entry| entry.ignored());
        let policy = HiddenEntryPolicy::ProjectExplorer {
            show_user_hidden: self.show_hidden_entries,
        };
        if depth > 0 && !policy.allows_path(Path::new(current_path.as_str()), false, ignored) {
            return (None, false);
        }
        ...
        match entry_map.get(current_path).cloned() {
            ...
            Some(FileTreeEntryState::Symlink(symlink)) => {
                let is_expanded = symlink.target_kind == SymlinkTargetKind::Directory
                    && self.is_folder_expanded(root_path, &symlink.path);
                let symlink_std_path = (*symlink.path).clone();
                let (mouse_state_handle, draggable_state) = if let Some(root_dir) =
                    self.root_directories.get_mut(root_path)
                {
                    root_dir
                        .item_states
                        .entry(symlink_std_path)
                        .or_insert_with(|| (MouseStateHandle::default(), DraggableState::default()))
                        .clone()
                } else {
                    (MouseStateHandle::default(), DraggableState::default())
                ...
                items.push(FileTreeItem::Symlink {
                    metadata: symlink.clone(),
                    depth,
                    mouse_state_handle,
                    draggable_state,
                });
    ...
    fn is_item_expanded(&self, root_path: &StandardizedPath, item: &FileTreeItem) -> Option<bool> {
        match item {
            ...
            FileTreeItem::Symlink { metadata, .. } => (metadata.target_kind
                == SymlinkTargetKind::Directory)
                .then(|| self.is_folder_expanded(root_path, &metadata.path)),
        }
    }
    ...
    fn render_item(&self, id: &FileTreeIdentifier, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let hoverable = Hoverable::new(render_state.mouse_state.clone(), move |mouse_state| {
            ...
            // 环境文件已支持通过 buffer-sync 协议打开,不再显示「无法打开」提示。
        ...
        // current-app 和环境文件都可点击打开,统一用手型光标。
    ...
    fn selected_directory_like_path(&self) -> Option<StandardizedPath> {
        self.selected_item.as_ref().and_then(|id| {
            let root_dir = self.root_directories.get(&id.root)?;
            let item = root_dir.items.get(id.index)?;
            item.is_directory_like().then(|| item.path().clone())
        })
    }
    ...
    fn open_file(
        ...
        let target = if editor_layout.is_some() {
            resolve_file_target_to_open_in_ashide(path, settings, editor_layout)
        } else {
    ...
    fn select_and_execute_item_at_id(
        ...
        let is_environment_root = root_dir.is_environment_root();

        match item {
            FileTreeItem::File { metadata, .. } => {
                if !is_environment_root {
                    let path = metadata.path.to_local_path_lossy();
                    self.open_file(&path, None, ctx);
                } else {
                    // Environment runtime 文件:走 buffer-sync 协议打开。
                    #[cfg(feature = "local_tty")]
                    if let Some(host_id) = root_dir.environment_host_id.clone() {
                        let Some(binding_session_id) = root_dir.environment_session_id else {
                            log::error!(
                                "Environment file-tree root is missing its transport SessionId"
                            );
                            return;
                        };
                        let environment_file_path =
                            crate::code::buffer_location::EnvironmentFilePath::new(
                                host_id,
                                (*metadata.path).clone(),
                            );
                        ctx.emit(FileTreeEvent::OpenEnvironmentFile {
            ...
            FileTreeItem::Symlink { metadata, .. } => match metadata.target_kind {
                SymlinkTargetKind::Directory => {
                    let path = (*metadata.path).clone();
                    self.toggle_folder_expansion(&id.root, &path, ctx);
                }
                SymlinkTargetKind::File if !is_environment_root => {
                    self.open_file(&metadata.path.to_local_path_lossy(), None, ctx);
                }
                ...
                SymlinkTargetKind::Other | SymlinkTargetKind::Missing => {
                    Self::show_unavailable_symlink_toast(metadata.target_kind, ctx);
                }
            },
        }
    ...
    fn context_menu_items(
        ...
        let is_environment_root = self.is_environment_item(id);

        let mut items = vec![];

        if is_environment_root {
            // Environment file trees only support a limited set of actions:
            // copying paths and attaching as context. File opening,
            // creation, rename, delete, cd, and reveal are unavailable
            // because there is no terminal filesystem or editor support.
        } else {
    ...
    fn delete_item(&mut self, id: &FileTreeIdentifier, ctx: &mut ViewContext<Self>) {
        ...
        let result = remove_file_tree_item(item, &path);
        if let Err(e) = result {
    ...
    fn render_file_tree(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if num_items == 0 {
            let has_filtered_entries = !self.show_hidden_entries
                && self.displayed_root_directories().any(|(_, root)| {
                    root.entry
                        .child_paths(root.entry.root_directory())
                        .any(|path| {
                            let ignored = root.entry.get(path).is_some_and(|entry| entry.ignored());
...
pub enum FileTreeEvent {
    ...
    /// 在环境文件树里点击一个文件时发出,请求以 Environment Runtime buffer 方式打开它。
...
impl View for FileTreeView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        self.render_error_state(
            crate::t!("project-explorer-unavailable-environment-description"),
            app,
        )
    }
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if matches!(
            self.enablement,
            CodingPanelEnablementState::PendingRuntimeSession
        ) {
            return self.render_loading_state(app);
        }

        if self.displayed_directories.is_empty() {
            if let CodingPanelEnablementState::RuntimeSession {
                has_environment_runtime,
            } = self.enablement
            {
                // When the session has an Environment Runtime connection, show a
                // loading state: repo metadata may arrive momentarily. For legacy
                // nested SSH modes (tmux/subshell), no data will arrive, so show
                // the disabled error instead.
                return if has_environment_runtime {
        ...
        match action {
            ...
            FileTreeAction::Expand => {
                if let Some(selected_item) = self.selected_item.clone() {
                    if let Some(sp) = self.selected_directory_like_path() {
            ...
            FileTreeAction::NewFileBelowDirectory { id } => {
                if !self.is_environment_item(id) {
