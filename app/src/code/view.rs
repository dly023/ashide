...
pub fn init(app: &mut AppContext) {
    ...
    super::current_app_code_editor::init(app);
...
#[derive(Clone)]
pub struct TabData {
    /// Complete open intent. Environment tabs keep their stable transport
    /// SessionId here while `location` remains host+path identity.
    source: CodeSource,
    /// current-app 文件路径。**仅 current-app 文件**有值 —— Environment Runtime 文件没有 current-app 路径。
    /// 仅用于显示 / repo 检测 / rename 等 current-app 专属逻辑。
    path: Option<PathBuf>,
    /// tab 的统一身份(current-app 路径或 Environment Runtime `EnvironmentFilePath`),用于 tab 去重 / 聚焦。
    /// 新建文件(无 backing)为 `None`。
    location: Option<BufferLocation>,
    editor_view: ViewHandle<CurrentAppCodeEditorView>,
...
impl TabData {
    ...
    /// tab 的统一身份(current-app 路径或 Environment Runtime `EnvironmentFilePath`)。
...
pub struct CodeView {
    ...
    /// 等待Environment Runtime SaveBuffer 异步完成的 callback,key 为 `CurrentAppCodeEditorView`
    /// 的 `EntityId`。Environment Runtime buffer 的保存调用会立刻返回 `Ok(())`,daemon 实际
    /// 落盘要等 `CurrentAppCodeEditorEvent::FileSaved` / `FailedToSave` 异步事件;
    /// "关 tab"对应的 callback 必须延迟到这条事件到达,否则保存失败时 tab 已被
    /// 关闭,用户看不到错误。
    pending_environment_save_callbacks: HashMap<EntityId, SaveCallback>,
}
...
impl CodeView {
    fn new_internal(source: CodeSource, ctx: &mut ViewContext<Self>) -> Self {
        ...
        Self {
            ...
            pending_environment_save_callbacks: HashMap::new(),
        }
    }
    ...
    pub fn new(
        ...
        view.open_or_focus_existing(source, line_col, ctx);
        #[cfg(feature = "local_fs")]
        {
    ...
    pub fn restore(
        ...
        for tab_snapshot in tabs {
            // 持久化恢复只包含 current-app 文件 —— Environment Runtime pane 不持久化。
            let tab_source = tab_snapshot.path.clone().map_or(
                CodeSource::New {
                    default_directory: None,
                },
                |path| CodeSource::Link {
                    path,
                    range_start: None,
                    range_end: None,
                },
            );
            let tab_data = view.build_tab_data(tab_source, false, ctx);
    ...
    pub fn new_preview(source: CodeSource, ctx: &mut ViewContext<Self>) -> Self {
        ...
        if source.location().is_some() {
            view.open_in_preview_or_promote(source, ctx);
            #[cfg(feature = "local_fs")]
            {
                view.update_markdown_mode_segmented_control(ctx);
            }
    ...
    fn construct_shared_buffer_editor_from_path(
        ...
        ctx.add_typed_action_view(|ctx| {
            let mut editor = CurrentAppCodeEditorView::new_with_global_buffer(
                path,
                |buffer_state, ctx| {
                    ctx.add_typed_action_view(|ctx| {
                        CodeEditorView::new(
    ...
    /// 构造一个绑定到Environment Runtime buffer 的编辑器视图。结构与
    /// [`Self::construct_shared_buffer_editor_from_path`] 一致,但 buffer 经
    /// `GlobalBufferModel` 的 `BufferLocation::EnvironmentRuntime` 路径打开,内容由
    /// buffer-sync 协议异步填充。
    #[cfg(feature = "local_tty")]
    fn construct_shared_buffer_editor_from_environment(
        &mut self,
        environment_file_path: &super::buffer_location::EnvironmentFilePath,
        binding_session_id: warp_core::SessionId,
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<CurrentAppCodeEditorView> {
        let environment_file_path = environment_file_path.clone();
        ctx.add_typed_action_view(|ctx| {
            let mut editor = CurrentAppCodeEditorView::new_with_environment_buffer(
            ...
            // 与 current-app shared-buffer 编辑器一致:补上 footer / 保存冲突 UI。
            editor.add_footer(ctx);
            editor
        })
    }
    ...
    fn construct_new_file_editor(
        ...
        ctx.add_typed_action_view(|ctx| {
            let mut current_file_editor =
                CurrentAppCodeEditorView::new(editor, None, false, None, ctx);
            if FeatureFlag::HoaCodeReview.is_enabled() {
                current_file_editor = current_file_editor
    ...
    fn build_tab_data(
        ...
        // current-app 路径(仅 `CurrentAppFileSystem` 变体有),用于显示 / repo 检测 / rename。
        let path = location.as_ref().and_then(|loc| match loc {
            BufferLocation::CurrentAppFileSystem(p) => Some(p.clone()),
        ...
        let code_editor = match &source {
            CodeSource::Link { path, .. }
            | CodeSource::ProjectRules { path }
            | CodeSource::FileTree { path }
            | CodeSource::Finder { path }
            | CodeSource::Skill { path, .. } => {
            ...
            CodeSource::EnvironmentFileTree {
                environment_file_path,
                binding_session_id,
            } => self.construct_shared_buffer_editor_from_environment(
                environment_file_path,
                *binding_session_id,
                ctx,
            ),
            ...
            CodeSource::New { .. } | CodeSource::AIAction { .. } => {
                self.construct_new_file_editor(ctx)
            }
        };
        ...
        if path.is_none() && matches!(source, CodeSource::New { .. }) {
            let default_directory = source.default_directory().cloned();
            code_editor.update(ctx, |current_file_editor, _ctx| {
                current_file_editor.set_new_file(true);
                current_file_editor.set_default_directory(default_directory);
            });
        }
        ...
        ctx.subscribe_to_view(&code_editor, |me, emitter, event, ctx| match event {
            ...
            CurrentAppCodeEditorEvent::FailedToLoad { error: err } => {
                ...
                // 通过事件发出者(CurrentAppCodeEditorView 句柄)定位是哪个 tab 失败,
            ...
            CurrentAppCodeEditorEvent::SelectionAddedAsContext {
                relative_file_path,
                line_range,
                selected_text,
            } => {
            ...
            CurrentAppCodeEditorEvent::FileSaved => {
                me.sync_active_tab_path(ctx);
                me.set_title_after_content_update(ctx);
                CodeView::display_save_success(ctx.window_id(), ctx);
                ctx.notify();
                // Environment Runtime异步 Save 完成:触发暂存的 close-tab callback。
                if let Some(cb) = me.pending_environment_save_callbacks.remove(&emitter.id()) {
                    cb(SaveOutcome::Succeeded, me, ctx);
                }
            }
            CurrentAppCodeEditorEvent::FailedToSave { error: err } => {
                log::warn!("Failed to load file. {err:?}");
                CodeView::display_save_failure(ctx.window_id(), ctx);
                // Environment Runtime异步 Save 失败:tab 必须留下,告知 callback 走 Failed 分支。
            ...
            CurrentAppCodeEditorEvent::DiffAccepted => {
                CodeManager::handle(ctx).update(ctx, |code_manager, ctx| {
                    code_manager.complete_pending_diffs(me.source.clone(), ctx);
                });
            }
            CurrentAppCodeEditorEvent::DiffRejected => {
                CodeManager::handle(ctx).update(ctx, |code_manager, ctx| {
                    code_manager.complete_pending_diffs(me.source.clone(), ctx);
                });
            }
            CurrentAppCodeEditorEvent::DiffStatusUpdated => (),
            CurrentAppCodeEditorEvent::UserEdited => (),
            CurrentAppCodeEditorEvent::VimMinimizeRequested => (),
            CurrentAppCodeEditorEvent::ViewportUpdated => (),
            CurrentAppCodeEditorEvent::LayoutInvalidated => (),
            CurrentAppCodeEditorEvent::DiscardUnsavedChanges { path } => {
                #[cfg(feature = "local_fs")]
                GlobalBufferModel::handle(ctx).update(ctx, |global_buffer, ctx| {
                    global_buffer.discard_unsaved_changes(path, ctx);
                });
            }
            CurrentAppCodeEditorEvent::CommentSaved { .. }
            | CurrentAppCodeEditorEvent::RequestOpenComment(_)
            | CurrentAppCodeEditorEvent::DeleteComment { .. } => {
                // Comment events are handled by CodeReviewView, not CodeView
            }
            CurrentAppCodeEditorEvent::RunTabConfigSkill { path } => {
                ctx.emit(CodeViewEvent::RunTabConfigSkill { path: path.clone() });
            }
            CurrentAppCodeEditorEvent::DelayedRenderingFlushed => (),
        });
    ...
    pub fn current_app_path(&self, ctx: &AppContext) -> Option<PathBuf> {
    ...
    /// active and editable. `location` 统一覆盖 current-app 与 Environment Runtime 文件。
    pub fn open_in_preview_or_promote(&mut self, source: CodeSource, ctx: &mut ViewContext<Self>) {
        let Some(location) = source.location() else {
            log::warn!("Preview CodeView source has no location");
            return;
        };
        ...
        if let Some((preview_index, _)) = self.preview_tab() {
            let new_tab = self.build_tab_data(source.clone(), true, ctx);
        ...
        let new_tab = self.build_tab_data(source, true, ctx);

        self.tab_group.push(new_tab);
        let active_tab_index = self.tab_group.len() - 1;
        self.set_active_tab_index(active_tab_index, ctx);

        // `FileOpened` 仅对 current-app 文件有意义(repo 检测 / OpenedFilesModel)。
        if let BufferLocation::CurrentAppFileSystem(file_path) = location {
    ...
    pub fn open_in_preview_or_promote_and_jump(
        ...
        self.open_in_preview_or_promote(source, ctx);
    ...
    pub fn open_or_focus_existing(
        ...
        self.open_new_tab_for_source(source, line_col, ctx);
    }
    ...
    fn open_new_tab_for_source(
        ...
        let location = source.location();
        let new_tab = self.build_tab_data(source, false, ctx);
        self.tab_group.push(new_tab);
        let active_tab_index = self.tab_group.len() - 1;
    ...
    fn set_title(&self, _unsaved_changes: bool, ctx: &mut ViewContext<Self>) {
        ...
        // 标题优先用 current-app 路径(完整路径),Environment Runtime 文件无 current-app 路径时回退到
        // `location` 派生的显示名,避免Environment Runtime tab 显示成 "Untitled"。
        let title = if let Some(file) = self.current_app_path(ctx) {
    ...
    fn save_current_file(
        ...
        // Environment Runtime buffer 的保存调用仅触发 `SaveBuffer` 协议消息,daemon 实际
        // 落盘要等 `CurrentAppCodeEditorEvent::FileSaved` / `FailedToSave` 异步事件。
        // 这里记下 editor entity id 与 "是否Environment Runtime",在 `Ok` 分支按需暂存 callback。
        let environment_save_target = self.tab_at(index).and_then(|tab| {
            let editor = tab.editor_view.as_ref(ctx);
            let file_id = editor.file_id()?;
            let is_environment_runtime_buffer =
                GlobalBufferModel::as_ref(ctx).is_environment_runtime_buffer(file_id);
            is_environment_runtime_buffer.then(|| tab.editor_view.id())
        });
        ...
        let result = self
            ...
            .map(|tab| {
                tab.editor_view
                    .update(ctx, |code_diff, ctx| code_diff.save_current_file(ctx))
            })
        ...
        match result {
            ...
            Ok(()) => match (environment_save_target, callback) {
                (Some(editor_id), Some(cb)) => {
                    // Environment Runtime:暂存 callback,等 FileSaved / FailedToSave 事件再触发。
                    self.pending_environment_save_callbacks
                        .insert(editor_id, cb);
    ...
    fn has_unsaved_changes(tab: &TabData, ctx: &AppContext) -> bool {
        let current_file_editor = tab.editor_view.as_ref(ctx);
        current_file_editor.has_unsaved_changes(ctx)
    }
    ...
    /// Update the TabData path for the active tab to match the current-app editor metadata.
    ...
    fn sync_active_tab_path(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(tab) = self.tab_group.get_mut(self.active_tab_index) {
            let new_path = tab
                ...
                .file_path()
                .map(|p| p.to_path_buf());
            tab.location = new_path.clone().map(BufferLocation::CurrentAppFileSystem);
            tab.path = new_path;
        }
    }
    ...
    pub fn cleanup_all_tabs(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // 同时让 daemon 释放Environment Runtime buffer 内存。
    ...
    fn remove_tab_with_confirmation(
        ...
        if let Some(tab) = self.tab_at(index) {
            // 用 `location` 派生文件名:Environment Runtime 文件没有 current-app `path`,否则会丢失文件名。
    ...
    /// 用于跨 pane / 跨窗口拖拽:tab 只是搬家,buffer 状态(及Environment Runtime buffer
    ...
    fn remove_tab_data_index_inner(
        ...
        // Environment Runtime buffer 下次打开会复用包含未保存编辑的旧状态,造成"看着已保存"
    ...
    pub fn remove_tab_for_move(
        ...
        let source = self.tab_at(index)?.source.clone();
        self.remove_tab_data_index_for_move(index, ctx);
        Some(CodePane::new(source, None, ctx))
    }
            ...
            Some(PendingSaveIntent::Save) => {
                self.save_current_file(
                    ...
                    Some(Box::new(move |outcome, me, ctx| {
                        // 仅在保存成功时才关 tab。Environment Runtime保存通过异步事件回填
    ...
    pub fn rename_tabs_with_path(
        ...
        for tab in self.tab_group.iter_mut() {
            if tab.path.as_ref().is_some_and(|path| path == old_path) {
                ...
                tab.location = Some(BufferLocation::CurrentAppFileSystem(new_path.to_path_buf()));
                tab.editor_view.update(ctx, |editor, ctx| {
                            ...
                            move |ctx: &mut AppContext| {
                                if let Some(view) = handle.upgrade(ctx) {
                                    view.update(ctx, |editor, ctx| {
                                        let _ = editor.save_current_file(ctx);
                                    });
                                }
                            }
                        };
    ...
    fn overflow_menu_items(&self, ctx: &AppContext) -> Vec<MenuItem<CodeViewAction>> {
        ...
        #[cfg(feature = "local_fs")]
        if let Some(path) = self.current_app_path(ctx) {
            items.extend([
                MenuItem::Separator,
...
impl TypedActionView for CodeView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            CodeViewAction::SaveFile => {
                self.save_current_file(self.active_tab_index, None, ctx);
            }
            CodeViewAction::SaveFileAs => {
            ...
            CodeViewAction::RenderMarkdown => {
                let path = self.current_app_path(ctx).or_else(|| {
                    self.tab_at(self.active_tab_index)
                        .and_then(|t| t.path.clone())
                });
