...
impl CodeView {
    ...
    pub fn restore(
        ...
        for tab_snapshot in tabs {
            ...
            let location = tab_snapshot
                .path
                .clone()
                .map(BufferLocation::CurrentAppFileSystem);
    ...
    fn construct_shared_buffer_editor_from_environment(
        ...
        ctx.add_typed_action_view(|ctx| {
            ...
            // Environment Runtime buffer 创建时内容为空,初始内容经 buffer-sync 协议异步到达。
    ...
    fn build_tab_data(
        ...
        let code_editor = match location.as_ref() {
            Some(BufferLocation::CurrentAppFileSystem(path)) => {
                self.construct_shared_buffer_editor_from_path(path, ctx)
            }
            #[cfg(feature = "local_tty")]
            Some(BufferLocation::EnvironmentRuntime(environment_file_path)) => {
                self.construct_shared_buffer_editor_from_environment(environment_file_path, ctx)
            }
            #[cfg(not(feature = "local_tty"))]
            Some(BufferLocation::EnvironmentRuntime(_)) => {
        ...
        ctx.subscribe_to_view(&code_editor, |me, emitter, event, ctx| match event {
            CurrentAppCodeEditorEvent::FileLoaded => {
                // Environment Runtime buffer 初始内容到达后,解除加载期施加的编辑锁。
                // current-app 文件创建时即可编辑,从不被锁,这里也就是 no-op。
                if let Some(tab) = me.tab_group.iter().find(|tab| tab.editor_view == emitter) {
                    if matches!(tab.location, Some(BufferLocation::EnvironmentRuntime(_))) {
                        emitter.update(ctx, |current_file_editor, ctx| {
                            current_file_editor
                                .editor()
                                .update(ctx, |code_editor, ctx| {
                                    code_editor
                                        .set_interaction_state(InteractionState::Editable, ctx);
                                });
                        });
                    }
                }
        ...
        self.tab_at(index)
            ...
            .map(|location| {
                let source = match location {
                    BufferLocation::CurrentAppFileSystem(path) => CodeSource::Link {
                        path,
                        range_start: None,
                        range_end: None,
                    },
                    BufferLocation::EnvironmentRuntime(environment_file_path) => {
