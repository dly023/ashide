...
pub struct CodeEditorColorOverride {
    foreground: ColorU,
    background: ColorU,
}

impl CodeEditorColorOverride {
...
pub struct CodeEditorRenderOptions {
    ...
    color_override: Option<CodeEditorColorOverride>,
}
...
impl CodeEditorRenderOptions {
    ...
    pub fn color_override(mut self, color_override: CodeEditorColorOverride) -> Self {
...
impl CodeEditorView {
    ...
    fn handle_appearance_or_font_change(&mut self, ctx: &mut ViewContext<Self>) {
        let new_styles = code_text_styles_with_color_override(
...
impl View for CodeEditorView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let (nonactive_scrollbar_thumb_background, active_scrollbar_thumb_background) =
            if let Some(color_override) = self.color_override {
                (
                    Fill::Solid(coloru_with_opacity(color_override.foreground, 40)),
                    Fill::Solid(coloru_with_opacity(color_override.foreground, 72)),
                )
            } else {
                (
...
fn code_text_styles_with_color_override(
    appearance: &Appearance,
    font_settings: &FontSettings,
    line_height_override: Option<f32>,
    color_override: Option<CodeEditorColorOverride>,
) -> RichTextStyles {
    let mut styles = code_text_styles(appearance, font_settings, line_height_override);
    if let Some(color_override) = color_override {
        apply_color_override(&mut styles, color_override);
    }
    styles
}

fn apply_color_override(styles: &mut RichTextStyles, color_override: CodeEditorColorOverride) {
    let foreground = color_override.foreground;
    let background = color_override.background;

    styles.base_text.text_color = foreground;
    styles.code_text.text_color = foreground;
    styles.embedding_text.text_color = foreground;
    styles.code_background = Fill::Solid(background);
    styles.embedding_background = Fill::Solid(background);
    styles.placeholder_color = coloru_with_opacity(foreground, 48);
    styles.selection_fill = Fill::Solid(coloru_with_opacity(foreground, 24));
    styles.cursor_fill = Fill::Solid(foreground);
    styles.inline_code_style.background = coloru_with_opacity(foreground, 14);
    styles.inline_code_style.font_color = foreground;
    styles.check_box_style.border_color = coloru_with_opacity(foreground, 72);
    styles.check_box_style.background = background;
    styles.check_box_style.hover_background = coloru_with_opacity(foreground, 14);
    styles.horizontal_rule_style.color = coloru_with_opacity(foreground, 32);
    styles.table_style.border_color = coloru_with_opacity(foreground, 32);
    styles.table_style.header_background = coloru_with_opacity(foreground, 12);
    styles.table_style.cell_background = background;
    styles.table_style.alternate_row_background = Some(coloru_with_opacity(foreground, 8));
    styles.table_style.text_color = foreground;
    styles.table_style.header_text_color = foreground;
    styles.table_style.scrollbar_nonactive_thumb_color = coloru_with_opacity(foreground, 40);
    styles.table_style.scrollbar_active_thumb_color = coloru_with_opacity(foreground, 72);
}
