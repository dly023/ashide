...
type SaveCallback = Box<
    dyn FnOnce(SaveOutcome, &mut ViewContext<CurrentAppCodeEditorView>) + Send + Sync + 'static,
>;

pub fn init(app: &mut AppContext) {
    app.register_fixed_bindings([FixedBinding::new(
        "cmdorctrl-l",
        CurrentAppCodeEditorAction::InsertSelectedTextToInput,
        id!("CurrentAppCodeEditorView") & !id!("IMEOpen"),
    )]);
}

pub enum CurrentAppCodeEditorEvent {
    FileLoaded,
    FailedToLoad {
...
enum LoadedFileMetadata {
    /// Current-app file with both FileId and path (for files that are actually opened)
    CurrentAppFile { id: FileId, path: PathBuf },
...
#[derive(Debug, Clone)]
pub enum CurrentAppCodeEditorAction {
...
pub struct CurrentAppCodeEditorView {
...
impl CurrentAppCodeEditorView {
    pub fn new(
        ...
        ctx.subscribe_to_view(&editor, |me, _, event, ctx| match event {
            CodeEditorEvent::UnifiedDiffComputed(_) => {
                ctx.emit(CurrentAppCodeEditorEvent::DiffAccepted);
            }
            CodeEditorEvent::ContentChanged { origin, .. } => {
                ...
                if origin.from_user() {
                    me.was_edited = true;
                    ctx.emit(CurrentAppCodeEditorEvent::UserEdited);
                }
            }
            CodeEditorEvent::VimEscapeInNormalMode => {
                ctx.emit(CurrentAppCodeEditorEvent::VimMinimizeRequested);
            }
            CodeEditorEvent::EscapePressed => {}
            CodeEditorEvent::DiffUpdated => {
                ctx.emit(CurrentAppCodeEditorEvent::DiffStatusUpdated);
            }
            ...
            CodeEditorEvent::CommentSaved { comment } => {
                ctx.emit(CurrentAppCodeEditorEvent::CommentSaved {
                    comment: comment.clone(),
                });
            }
            CodeEditorEvent::DeleteComment { id } => {
                ctx.emit(CurrentAppCodeEditorEvent::DeleteComment { id: *id });
            }
            CodeEditorEvent::RequestOpenComment(uuid) => {
                ctx.emit(CurrentAppCodeEditorEvent::RequestOpenComment(*uuid));
            }
            CodeEditorEvent::ViewportUpdated => {
                ctx.emit(CurrentAppCodeEditorEvent::ViewportUpdated);
            }
            CodeEditorEvent::LayoutInvalidated => {
                ctx.emit(CurrentAppCodeEditorEvent::LayoutInvalidated);
            }
            CodeEditorEvent::DelayedRenderingFlushed => {
                ctx.emit(CurrentAppCodeEditorEvent::DelayedRenderingFlushed);
            }
    ...
    fn perform_save(&mut self, file_id: FileId, ctx: &mut ViewContext<Self>) {
        ...
        // Environment Runtime 文件:走 buffer-sync 的 `SaveBuffer` 协议落盘。不能用下面的当前 app
        // `FileModel` 路径 —— Environment Runtime buffer 没有 current-app 路径,会得到 `NoFilePath`。
        // 用户的编辑已通过 `BufferEdit` 实时同步到 daemon,这里只触发 daemon 落盘。
        #[cfg(feature = "local_tty")]
        {
            let is_environment_runtime_buffer = GlobalBufferModel::handle(ctx)
                .as_ref(ctx)
                .is_environment_runtime_buffer(file_id);
            if is_environment_runtime_buffer {
        ...
        if let Err(err) = result {
            ...
            ctx.emit(CurrentAppCodeEditorEvent::FailedToSave {
                error: Rc::new(err),
            });
        }
    }
    ...
    #[cfg(test)]
    pub(crate) fn load_completion_reached(&self) -> bool {
        self.file_loaded.is_set()
    }
        ...
        T: FnOnce(BufferState, &mut ViewContext<Self>) -> ViewHandle<CodeEditorView>,
    {
        let buffer_state = GlobalBufferModel::handle(ctx).update(ctx, |model, ctx| {
            model.open_current_app(path.to_path_buf(), ctx)
        });
        ...
        current_file_editor.metadata = Some(LoadedFileMetadata::CurrentAppFile {
            id: file_id,
    ...
    /// 构造一个绑定到Environment Runtime buffer 的编辑器视图。
    ///
    /// 通过 [`GlobalBufferModel::open`] 以 [`BufferLocation::EnvironmentRuntime`] 打开 Environment Runtime 文件,
    /// 内容由 buffer-sync 协议异步填充。语言识别复用Environment Runtime路径的后缀。
    #[cfg(feature = "local_tty")]
    pub fn new_with_environment_buffer<T>(
    ...
    where
        T: FnOnce(BufferState, &mut ViewContext<Self>) -> ViewHandle<CodeEditorView>,
    {
        // Environment Runtime路径用于语言识别(后缀)。
        let language_path = std::path::PathBuf::from(environment_file_path.path.as_str());
        let buffer_state = GlobalBufferModel::handle(ctx).update(ctx, |model, ctx| {
            model.open_environment_buffer(environment_file_path.clone(), binding_session_id, ctx)
        });
        let file_id = buffer_state.file_id;
        let canonical_content_version = buffer_state.buffer.as_ref(ctx).version();
        let editor = editor_constructor(buffer_state, ctx);
        ...
        // Project the canonical loaded state after installing the subscription.
        // This covers both orderings: a future BufferLoaded event, and a second
        // consumer opening a buffer that was already materialized before this
        // view existed.
        let is_loaded = GlobalBufferModel::as_ref(ctx).buffer_loaded(file_id);
        current_file_editor.editor.update(ctx, |editor, ctx| {
        ...
        if is_loaded {
            current_file_editor.base_content_version = Some(canonical_content_version);
            current_file_editor.on_file_loaded(ctx);
        }
    ...
    pub(crate) fn add_footer(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.subscribe_to_view(&footer, |_, _, event, ctx| match event {
            CodeFooterViewEvent::RunTabConfigSkill { path } => {
                ctx.emit(CurrentAppCodeEditorEvent::RunTabConfigSkill { path: path.clone() });
            }
        });
        ...
        ctx.subscribe_to_model(&GlobalBufferModel::handle(ctx), move |me, _, event, ctx| {
            ...
            match event {
                GlobalBufferModelEvent::BufferLoaded {
                    ...
                    if matches!(
                        me.metadata,
                        Some(LoadedFileMetadata::EnvironmentFile { .. })
                    ) {
                    ...
                    ctx.emit(CurrentAppCodeEditorEvent::FileLoaded);
                }
                GlobalBufferModelEvent::FailedToLoad { error, .. } => {
                    ...
                    ctx.emit(CurrentAppCodeEditorEvent::FailedToLoad {
                        error: error.clone(),
                    });
                }
                ...
                GlobalBufferModelEvent::FileSaved { .. } => {
                    ctx.emit(CurrentAppCodeEditorEvent::FileSaved);
                }
                ...
                // Environment Runtime buffer 同步事件由 GlobalBufferModel / ServerModel 内部消费,
                // 当前编辑器视图不关心。
    ...
    /// Save the current file through its active file-system backend.
    ...
    fn handle_save_as(
        ...
        let file_id = buffer_state.file_id;
        me.metadata = Some(LoadedFileMetadata::CurrentAppFile {
            id: file_id,
            path: path.clone(),
        });
    ...
    pub fn file_id(&self) -> Option<FileId> {
        self.metadata.as_ref().map(|metadata| match metadata {
            LoadedFileMetadata::CurrentAppFile { id, .. }
            | LoadedFileMetadata::EnvironmentFile { id, .. } => *id,
        })
    }

    pub fn file_path(&self) -> Option<&Path> {
        match self.metadata.as_ref()? {
            LoadedFileMetadata::CurrentAppFile { path, .. } => Some(path.as_path()),
            // Environment Runtime 文件没有 current-app 路径。
            LoadedFileMetadata::EnvironmentFile { .. } => None,
        }
    }
    ...
    pub fn apply_rename(
        ...
        let file_id = buffer_state.file_id;
        self.metadata = Some(LoadedFileMetadata::CurrentAppFile {
            id: file_id,
    ...
    fn insert_selected_text_to_input(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.emit(CurrentAppCodeEditorEvent::SelectionAddedAsContext {
            relative_file_path,
            line_range,
            selected_text,
        });
...
impl DiffViewer for CurrentAppCodeEditorView {
    ...
    /// Automatically accept and save this diff. Unlike [`Self::accept_diff`] and [`Self::save_current_file`], this
    ...
    fn accept_and_save_diff(&self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(self.file_loaded.wait(), move |me, _, ctx| {
            me.accept_diff(ctx);
            if let Err(err) = me.save_current_file(ctx) {
                log::error!("{err:?}");
    ...
    fn reject_diff(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.emit(CurrentAppCodeEditorEvent::DiffRejected);
    }
...
impl Entity for CurrentAppCodeEditorView {
    type Event = CurrentAppCodeEditorEvent;
}

impl View for CurrentAppCodeEditorView {
    fn ui_name() -> &'static str {
        "CurrentAppCodeEditorView"
    }
...
impl TypedActionView for CurrentAppCodeEditorView {
    type Action = CurrentAppCodeEditorAction;
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CurrentAppCodeEditorAction::SaveFile => {
                if let Err(ImmediateSaveError::FailedToSave(err)) = self.save_current_file(ctx) {
                    log::error!("Failed to save file {err:?}");
                    ctx.emit(CurrentAppCodeEditorEvent::FailedToSave {
                        error: Rc::new(err),
                    });
            ...
            CurrentAppCodeEditorAction::DiscardUnsavedChanges => {
                if let Some(path) = self.file_path().map(Path::to_path_buf) {
                    self.base_content_version = Some(self.editor().as_ref(ctx).version(ctx));
                    ctx.emit(CurrentAppCodeEditorEvent::DiscardUnsavedChanges { path });
                }
            }
        }
    }
        ...
        .with_child(
            appearance
                ...
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(CurrentAppCodeEditorAction::DiscardUnsavedChanges)
                })
                .finish(),
        )
        .with_child(
            Container::new(
                appearance
                    ...
                    .build()
                    .on_click(move |ctx, _, _| {
                        ctx.dispatch_typed_action(CurrentAppCodeEditorAction::SaveFile)
                    })
                    .finish(),
            )
            .with_margin_left(4.)
