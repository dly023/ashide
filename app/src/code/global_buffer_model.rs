...
impl GlobalBufferModel {
    ...
    pub fn close_buffer(&mut self, file_id: FileId, ctx: &mut ModelContext<Self>) {
        ...
        if let Some(state) = self.buffers.get(&file_id) {
            ...
            {
                let host_id = environment_file_path.host_id.clone();
                let path_str = environment_file_path.path.as_str().to_string();
                if let Some(client) =
                    crate::workspace::environment_runtime::client_for_host(&host_id, ctx)
                {
    ...
    pub fn open(&mut self, location: BufferLocation, ctx: &mut ModelContext<Self>) -> BufferState {
        match location {
            #[cfg(feature = "local_fs")]
            BufferLocation::CurrentAppFileSystem(path) => {
                self.open_current_app_file_system(path, false, ctx)
            }
            #[cfg(not(feature = "local_fs"))]
            BufferLocation::CurrentAppFileSystem(_) => {
                unimplemented!("Current-app file-system buffers require the local_fs feature")
            }
            BufferLocation::EnvironmentRuntime(environment_file_path) => {
                self.open_environment_buffer(environment_file_path, ctx)
            }
        }
    }
    ...
    fn open_environment_buffer(
        ...
        // TODO(environment-runtime): buffer 在初始内容(OpenBufferResponse)到达前即可编辑,
        ...
        #[cfg(feature = "local_tty")]
        {
            ...
            let Some(client) =
                ...
                log::warn!("No environment runtime client for host {host_id:?}");
            ...
            ctx.spawn(
                ...
                move |me, result, ctx| match result {
                    Ok(response) => {
                        ...
                        if let BufferSource::EnvironmentRuntime { sync_clock, .. } =
                            &mut state.source
                        {
    ...
    pub fn handle_buffer_updated_push(
        ...
        let file_id = self.buffers.iter().find_map(|(id, state)| {
            ...
            {
                if environment_file_path.host_id == *host_id
    ...
    pub fn apply_client_edit(
        ...
        let BufferSource::ServerCurrentAppFileSystem { sync_clock, .. } = &mut state.source else {
            return false;
        };
    ...
    pub fn save_environment_buffer(&self, file_id: FileId, ctx: &mut ModelContext<Self>) {
        ...
        let Some(client) = crate::workspace::environment_runtime::client_for_host(&host_id, ctx)
        else {
            log::warn!("save_environment_buffer: host {host_id:?} 无 environment runtime client");
