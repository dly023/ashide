...
use crate::uri::web_intent_parser;
...
pub enum WasmNUXDialogAction {
    ...
    OpenNativeAndClose,
    /// Open the Ashide download page
    OpenDownloadDesktopAppLink,
    /// Open a link to learn more about Ashide
    LearnMore,
}
...
/// * Download Ashide if they haven't already
...
impl WasmNUXDialog {
    ...
    /// behavior: if Ashide is installed, redirect to it; otherwise stay on the web.
...
impl View for WasmNUXDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // 1. The user doesn't have Ashide installed - treat them as a potential new user and encourage downloading Ashide.
        // 2. The user has Ashide installed, but clicked through to the web - ask if they want to always default to web.
        ...
        let dialog = if self.requested_download {
            ...
            let object_kind = crate::t!("wasm-nux-object-kind-warp-links");
...
impl TypedActionView for WasmNUXDialog {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            WasmNUXDialogAction::OpenNativeAndClose => {
                // We intentionally do not set the native preference here, in case the user hasn't actually installed Ashide.
                // If they have, on subsequent loads, we'll detect that Ashide is installed and redirect to the desktop.
                ctx.emit(WasmNUXDialogEvent::Close);
            ...
            WasmNUXDialogAction::OpenDownloadDesktopAppLink => {
                ctx.open_url("https://github.com/dly023/ashide");
