...
macro_rules! t_with_fallback {
    ($key:literal, $fallback:literal) => {{
        let translated = crate::t!($key);
        if translated == $key {
            $fallback.to_string()
        } else {
            translated
    ...
    V::Action: Clone + From<ContextMenuAction>,
{
    ...
    pub fn show_context_menu(&mut self, source: MenuSource, ctx: &mut ViewContext<V>) {
        ...
        if has_selection && can_edit {
            let item = MenuItemFields::new(t_with_fallback!("common-cut", "Cut"))
        ...
        if has_selection {
            let item = MenuItemFields::new(t_with_fallback!("common-copy", "Copy"))
        ...
        if can_edit {
            let item = MenuItemFields::new(t_with_fallback!("common-paste", "Paste"))
    ...
    fn split_pane_menu_items(&self, ctx: &mut ViewContext<V>) -> Vec<MenuItem<V::Action>> {
        ...
        if ContextFlag::CreateNewSession.is_enabled() {
            items.extend([
                MenuItemFields::new(t_with_fallback!(
                    "settings-pane-split-right",
                    "Split pane right"
                ))
                ...
                .into_item(),
                MenuItemFields::new(t_with_fallback!("settings-pane-split-up", "Split pane up"))
                    .with_on_select_action(V::Action::from(ContextMenuAction::EmitPaneEvent(
        ...
        if split_pane_state.is_in_split_pane() {
            ...
            items.push(
                MenuItemFields::new(t_with_fallback!("settings-pane-close", "Close pane"))
                    .with_on_select_action(V::Action::from(ContextMenuAction::EmitPaneEvent(
                        PaneEvent::Close,
                    )))
