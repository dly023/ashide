...
pub enum LinkTarget {
    ...
    CurrentAppFile {
        ...
        session: Arc<Session>,
        /// Whether or not this file is a Markdown file viewable in Ashide.
        is_markdown: bool,
    },
    CurrentAppDirectory {
        path: PathBuf,
...
impl LinkTarget {
    ...
    pub fn secondary_action(&self) -> Option<SecondaryAction> {
        match self {
            LinkTarget::CurrentAppDirectory { .. } => Some(SecondaryAction {
            ...
            LinkTarget::Url(_) | LinkTarget::CurrentAppFile { .. } => None,
        }
    }
}
...
impl PartialEq for LinkTarget {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (
                Self::CurrentAppDirectory { path: my_path },
                Self::CurrentAppDirectory { path: other_path },
            ) => my_path == other_path,
            _ => false,
        }
    }
...
impl fmt::Display for LinkTarget {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            LinkTarget::CurrentAppFile { path, .. } => path.display().fmt(f),
            LinkTarget::CurrentAppDirectory { path, .. } => path.display().fmt(f),
        }
    }
}
...
impl NotebookLinks {
    ...
    /// it's treated as an URL. Otherwise, it's treated as a current app filesystem path, possibly with a line
                    ...
                    Some(base_directory) => {
                        cfg_if::cfg_if! {
                            if #[cfg(feature = "local_fs")] {
                                ...
                                // If we don't have a current app filesystem, we append the path naively.
    ...
    async fn resolve_file(
        ...
        Ok(if metadata.is_dir() {
            ...
            LinkTarget::CurrentAppDirectory { path }
        } else {
            LinkTarget::CurrentAppFile {
    ...
    /// * Markdown files are opened in Ashide (if the `FileNotebooks` feature flag is enabled).
    ...
    pub fn open(&self, link: LinkTarget, ctx: &mut ModelContext<Self>) {
        match link {
            ...
            LinkTarget::CurrentAppDirectory { path, .. } => ctx.open_file_path(&path),
        }
    }
    ...
    pub fn secondary_action(&self, link: &LinkTarget, ctx: &mut ModelContext<Self>) {
        match link {
            LinkTarget::CurrentAppDirectory { path } => {
                ctx.emit(LinkEvent::StartEnvironmentSession { path: path.clone() })
            }
            LinkTarget::CurrentAppFile {
                path,
                line_and_column,
                ...
                // The default action for Markdown file links is to open them in Ashide. As a
...
// The `line_and_column` argument is unused when there is no current app filesystem.
...
pub enum LinkEvent {
    ...
    /// This event tells the parent pane group to ask the workspace for a new session in the
    /// given directory. The workspace owns local-vs-remote environment routing.
    StartEnvironmentSession { path: PathBuf },
    /// Signal to views that they should re-resolve links because the backing context for
    /// resolution has changed.
    RefreshLinks,
    #[cfg(feature = "local_fs")]
    /// Emitted when a file should be opened in Ashide (code editor or markdown viewer).
    OpenFileWithTarget {
        path: PathBuf,
        target: FileTarget,
    ...
    fn base_directory<'a>(&'a self, ctx: &'a AppContext) -> Option<&'a Path> {
        match self {
            ...
            SessionSource::Active(window_id) => {
                ActiveSession::as_ref(ctx).current_app_path(*window_id)
            }
        }
    }
}
