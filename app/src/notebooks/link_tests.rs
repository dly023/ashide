...
fn current_app_directory(path: impl Into<PathBuf>) -> LinkTarget {
    LinkTarget::CurrentAppDirectory { path: path.into() }
}

fn current_app_file(path: impl Into<PathBuf>) -> LinkTarget {
    let path = path.into();
...
fn init_link_model(app: &mut App, base_directory: Option<&Path>) -> ModelHandle<NotebookLinks> {
    ...
    app.add_singleton_model(|ctx| {
        ...
        session.set_session_for_test(
            ...
            TEST_SESSION.clone(),
            base_directory,
            base_directory.map(|dir| dir.to_string_lossy().into_owned()),
            None,
            ctx,
        );
        session
...
fn test_resolve_bare_url() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            ...
            current_app_file(base_path.join("nodot/slash"))
        );
        assert_eq!(
            resolve(&app, &links, ".vscode/settings.json").await,
            current_app_file(base_path.join(".vscode/settings.json"))
        );
        ...
        assert_eq!(
            ...
            current_app_file(base_path.join("myfile.swift"))
        );
        assert_eq!(
            resolve(&app, &links, "license.txt").await,
            current_app_file(base_path.join("license.txt"))
        );
        ...
        assert_eq!(
            ...
            current_app_file(base_path.join("app/src/main.rs"))
        );
    });
}
...
fn test_open_local_image_uses_system_generic_target() {
    App::test((), |mut app| async move {
        ...
        links.update(&mut app, |links, ctx| {
            links.open(current_app_file(&image_path), ctx);
        });
...
fn test_resolve_relative_file_no_base() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            ...
            current_app_file(absolute_path)
        );
        ...
        assert_eq!(
            ...
            current_app_directory(absolute_directory)
        );
...
fn test_resolve_relative_file_base() {
    ...
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            ...
            current_app_file(base_path.join("relative/path.txt"))
        );
        assert_eq!(
            resolve(&app, &links, "./dotted.txt").await,
            current_app_file(base_path.join("dotted.txt"))
        );
        assert_eq!(
            resolve(&app, &links, "./relative/../dotted.txt").await,
            current_app_file(base_path.join("relative/../dotted.txt"))
        );

        assert_eq!(
            resolve(&app, &links, "./relative").await,
            current_app_directory(base_path.join("relative"))
        );
...
fn test_resolve_file_with_line() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            ...
            current_app_file_location(base_path.join("src/main.rs"), 123, None)
        );

        assert_eq!(
            resolve(&app, &links, "path/to/index.html:99:51").await,
            current_app_file_location(base_path.join("path/to/index.html"), 99, Some(51))
        );
    });
}
