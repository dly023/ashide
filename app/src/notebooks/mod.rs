...
use crate::{
    ...
    drive::{
        items::{notebook::LocalDriveNotebook, LocalDriveItem},
        ObjectTypeAndId,
    },
    object_store::ids::{ObjectStoreId, StableObjectId},
    object_store::{GenericStoredObject, ObjectType, Owner, StoredObjectModel},
...
impl StoredObjectModel for NotebookObjectModel {
    ...
    fn to_local_drive_item(
        ...
        Some(Box::new(LocalDriveNotebook::new(
...
pub struct NotebookId(StableObjectId);
crate::stable_object_id_traits! { NotebookId, "Notebook" }

impl From<NotebookId> for ObjectStoreId {
...
pub enum NotebookLocation {
    ...
    PersonalDrive,
    /// A notebook backed by the current app filesystem.
    #[serde(rename = "LocalFile")]
    CurrentAppFile,
    /// A notebook backed by an Environment Runtime filesystem.
...
impl From<Owner> for NotebookLocation {
    fn from(owner: Owner) -> Self {
        ...
        NotebookLocation::PersonalDrive
    }
}
