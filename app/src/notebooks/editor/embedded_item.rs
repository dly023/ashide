...
use crate::{
    ...
    object_store::ids::{HashableId, ToStableObjectId},
    object_store::{model::persistence::ObjectStoreModel, StoredObject},
...
impl EmbeddedWorkflow {
    ...
    fn get_workflow<'a>(&self, app: &'a AppContext) -> Option<&'a WorkflowObject> {
        ...
        // sqlite hash id -> uid
        let uid =
            WorkflowId::from_hash(&self.hashed_id).map(|id| id.to_stable_object_id().uid())?;
...
impl EmbeddedItem for EmbeddedWorkflow {
    fn layout(&self, text_layout: &TextLayout, app: &AppContext) -> Box<dyn LaidOutEmbeddedItem> {
        ...
        let object_store_workflow = self.get_workflow(app);
        ...
        let Some(workflow) = object_store_workflow.and_then(|workflow| {
        ...
        let is_agent_mode_prompt =
            object_store_workflow.is_some_and(|w| w.model().data.is_agent_mode_workflow());
    ...
    fn to_rich_format(&self, app: &AppContext) -> EmbeddedItemRichFormat<'_> {
        ...
        // attributes so we could re-parse the ID and metadata when pasted into Ashide.
...
impl RenderableEmbeddedWorkflow {
    pub fn new(
        ...
        let (icon, icon_color) = if is_agent_mode_prompt {
            (
                Icon::Prompt,
                local_drive_icon_color(appearance, DriveObjectType::AgentModeWorkflow),
            )
        } else {
            (
                Icon::Workflow,
