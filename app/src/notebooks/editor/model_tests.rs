...
use crate::object_store::{Revision, StoredObjectMetadata, StoredObjectPermissions};
...
// Mock out a stable object-store workflow with the given i64 ID.
fn mock_server_workflow(id: i64, app: &mut App) {
    let stable_id: StableObjectId = id.into();
    let workflow_id: WorkflowId = stable_id.into();
    let object_store_id = ObjectStoreId::StableId(workflow_id.into());
    ...
    ObjectStoreModel::handle(app).update(app, |object_store_model, _| {
        object_store_model.add_object(object_store_id, workflow);
    });
}
