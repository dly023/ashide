...
use crate::{
    ...
    notebooks::interaction::EmbeddedObjectInfo,
    object_store::ids::ObjectStoreId,
    object_store::{model::persistence::ObjectStoreModel, ObjectIdType, Space},
...
impl RichTextEditorView {
    ...
    fn insert_embedded_workflow(&mut self, id: &ObjectStoreId, ctx: &mut ViewContext<Self>) {
        self.insert_block(
        ...
        let _ = ObjectStoreModel::as_ref(ctx).get_workflow(id);
        ctx.emit(EditorViewEvent::InsertedEmbeddedObject(
            EmbeddedObjectInfo::Workflow {
                workflow_id: id.into_stable().map(Into::into),
            },
        ))
    }
