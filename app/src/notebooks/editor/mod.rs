...
/// within a notebook.
///
/// This may be a command block that's part of the notebook text, or an embedded Ashide Drive workflow.
...
impl NotebookWorkflow {
    pub fn from_object_store_workflow(object_store_workflow: Box<WorkflowObject>) -> Self {
        Self {
            source: Some(object_store_workflow.permissions.owner.into()),
            workflow: UserInput::new(Arc::new(WorkflowType::ObjectStore(object_store_workflow))),
        }
    }
