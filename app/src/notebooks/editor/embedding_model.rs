...
use crate::{
    ...
    notebooks::{
        interaction::{ActionEntrypoint, BlockInfo},
...
impl NotebookEmbed {
    ...
    fn maybe_get_workflow<'a>(&self, ctx: &'a AppContext) -> Option<&'a WorkflowObject> {
        ...
        object_store_model
            .get_by_uid(&id.to_stable_object_id().uid())
    ...
    fn render_footer_for_workflow(
        ...
        let workflow_info =
            NotebookWorkflow::from_object_store_workflow(Box::new(workflow.clone()));
        let block_info = BlockInfo::EmbeddedWorkflow {
            workflow_id: workflow_id.into_stable().map(Into::into),
        };
...
impl ChildModelHandle for ModelHandle<NotebookEmbed> {
    ...
    fn executable_workflow(&self, app: &AppContext) -> Option<NotebookWorkflow> {
        // Currently we are only supporting embedded workflows. We could support
        // more drive objects in the future.
        self.as_ref(app).maybe_get_workflow(app).map(|workflow| {
            NotebookWorkflow::from_object_store_workflow(Box::new(workflow.clone()))
        })
    }
