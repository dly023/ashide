...
use crate::{
    auth::{AuthManager, AuthStateProvider, TEST_USER_UID},
    ...
    object_store::{
        model::{
            ...
            view::{Editor, ObjectStoreViewModel},
        },
...
fn open_notebook(
    ...
    let load_future = handle.update(app, |view, ctx| {
        view.load(notebook, &LocalDriveObjectSettings::default(), ctx)
    });
    app.update(|ctx| ctx.await_spawned_future(load_future.future_id()))
}

fn stored_notebook(title: impl Into<String>, data: impl Into<String>) -> NotebookObject {
    local_notebook_with_id(ObjectStoreId::ClientId(ClientId::new()), title, data)
}

fn local_notebook_with_id(
    id: crate::object_store::ids::ObjectStoreId,
...
fn mock_stored_notebook(title: impl Into<String>, data: impl Into<String>) -> NotebookObject {
    local_notebook_with_id(StableObjectId::from(123).into(), title, data)
}
...
"#,
        ...
        // `load` completes edit-access work, while command child models are created by the
        // editor layout pipeline. Wait for that actual readiness boundary instead of racing the
        // first layout pass under a highly parallel workspace test run.
        let layout_complete = app.read(|ctx| {
            ...
            assert!(
                events.contains(&NotebookEvent::RunWorkflow {
                    ...
                    source: WorkflowSource::Notebook {
                        notebook_id: None,
                        location: NotebookLocation::PersonalDrive,
                    },
                }),
...
fn test_focus_tracking() {
    App::test((), |mut app| async move {
        ...
        open_notebook(
            &mut app,
            &notebook,
            stored_notebook("Test Notebook", "This is a notebook"),
        )
        .await;
...
/// Legacy collaborator metadata is ignored in the local-only object store, so a stale
/// "current user" editor marker must not reintroduce cloud baton behavior.
#[test]
fn test_legacy_current_user_editor_metadata_opens_local_editing() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        ...
        let mut stored_notebook = stored_notebook("Test Notebook", r#"A notebook"#);

        // Set legacy collaborator metadata; local-only notebook editing must ignore it.
        stored_notebook.metadata.current_editor_uid = Some(TEST_USER_UID.to_string().clone());

        // Add the notebook to object store
        ObjectStoreModel::handle(&app).update(&mut app, |model, _| {
            model.add_object(stored_notebook.id, stored_notebook.clone())
        });

        // Open the notebook
        open_notebook(&mut app, &notebook_view, stored_notebook).await;

        // Assert that the legacy editor marker is ignored by the local object-store view.
        notebook_view.update(&mut app, |notebook, ctx| {
            assert_eq!(
                notebook
                    ...
                    .as_ref(ctx)
                    .current_editor(ctx),
                Some(Editor::no_editor())
            )
        });

        let mode = notebook_view.read(&app, |notebook, ctx| notebook.mode(ctx));
        // Assert that local notebooks open directly into edit mode.
        assert_eq!(mode, Mode::Editing);
    });
}

/// Legacy collaborator metadata is ignored in the local-only object store, so another
/// stale editor must not block local editing.
#[test]
fn test_legacy_other_editor_metadata_opens_local_editing() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        ...
        // Set legacy collaborator metadata; local-only notebook editing must ignore it.
        stored_notebook.metadata.current_editor_uid = Some(uid.clone());
...
/// Legacy collaborator metadata loaded after startup is ignored by the local-only object store.
#[test]
fn test_legacy_editor_metadata_loaded_offline_opens_local_editing() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        ...
        let stored_notebook = updated_notebook.clone();

        // Add the notebook to the object store, with no editor.
        ObjectStoreModel::handle(&app).update(&mut app, |object_store_model, _| {
            object_store_model.add_object(stored_notebook.id, stored_notebook.clone());
        });

        // Open the notebook, before initial load has finished.
        let open_future = open_notebook(&mut app, &notebook_view, stored_notebook);
        ...
        // The notebook should ignore the legacy editor marker and open in local edit mode.
        open_future.await;
...
/// Removing legacy collaborator metadata during startup keeps the local-only edit path active.
#[test]
fn test_legacy_editor_metadata_removed_offline_opens_local_editing() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
...
fn test_close_unmodified() {
    ...
    App::test((), |mut app| async move {
        ...
        let stored_notebook = mock_stored_notebook("Test", "Some text");
        let notebook_id = stored_notebook.id;
