...
///
/// The overall flow is:
/// 1. A `Workspace` is asked to open a notebook (from the local Drive index, universal search, etc.).
...
impl NotebookManager {
    ...
    pub fn notebook_raw_text(&self, notebook_id: ObjectStoreId) -> Option<&str> {
        match self
            .raw_text_by_hashed_id
            .get(&notebook_id.uid())
    ...
    pub fn notebook_raw_text_shared(&self, notebook_id: ObjectStoreId) -> Option<Arc<str>> {
        match self
            .raw_text_by_hashed_id
            .get(&notebook_id.uid())
            .unwrap_or(&NotebookRawTextStatus::NotParsed)
    ...
    fn update_raw_text_for_notebook(
        &mut self,
