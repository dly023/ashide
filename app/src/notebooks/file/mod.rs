...
/// View for a read-only notebook backed by a file, rather than Ashide Drive.
pub struct FileNotebookView {
    ...
    /// File watcher id for the currently opened file (if any). Only needed when current-app file watching is available
    /// access.
    #[cfg(feature = "local_fs")]
...
enum SourceFile {
    CurrentAppFile {
        /// The full path to the open current-app file.
        ///
        /// See [this comment](https://docs.google.com/document/d/18h7VzSAl6r5a94CovShlpPSahYqECX9WZliLjUqUsko/edit?disco=AAAA5Y1THuk);
        /// Environment Runtime paths are represented outside this current-app file state.
...
impl SourceFile {
    fn current_app_path(&self) -> Option<&Path> {
        match self {
            SourceFile::CurrentAppFile {
                current_app_path, ..
            } => Some(current_app_path.as_path()),
    ...
    fn display_name(&self) -> String {
        match self {
            SourceFile::CurrentAppFile {
                current_app_path, ..
            } => current_app_path.display().to_string(),
...
impl FileState {
    /// The path to the open current-app file, if it exists.
    fn current_app_path(&self) -> Option<&Path> {
        self.source().and_then(|src| src.current_app_path())
    }
...
impl FileNotebookView {
    ...
    pub fn set_content(&mut self, content: &str, ctx: &mut ViewContext<Self>) {
        let doc_path = self.file_state.current_app_path().map(|p| p.to_path_buf());
    ...
    /// Asynchronously open a current-app file, watching for file changes.
    pub fn open_current_app_file(
        &mut self,
        path: impl Into<PathBuf>,
        session: Option<Arc<Session>>,
        ctx: &mut ViewContext<Self>,
    ) {
        let current_app_path = path.into();

        // If a session is available, initialize the location and link context now. Otherwise,
        // we'll wait until one is available.
        if let Some(session) = &session {
            self.set_context(&current_app_path, session.clone(), ctx);
        } else {
            // Set a temporary title until the context is available.
            self.pane_configuration.update(ctx, |pane_config, ctx| {
                pane_config.set_title(current_app_path.display().to_string(), ctx);
            });
        }

        self.file_state = FileState::Loading(SourceFile::CurrentAppFile {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            let file_id = file_model.update(ctx, |m, ctx| m.open(&current_app_path, true, ctx));
                ...
                move |me, file_model: ModelHandle<FileModel>, event: &FileModelEvent, ctx| {
                    ...
                    match event {
                        FileModelEvent::FileLoaded { content, .. } => {
                            ...
                            {
                                me.file_state = FileState::Loaded(SourceFile::CurrentAppFile {
                                    current_app_path: canonical_path,
                                    session: session.clone(),
                                });
                            }
                        ...
                        FileModelEvent::FailedToLoad { error, .. } => {
                            safe_warn!(
                                safe: ("Unable to read current-app notebook file"),
                                full: ("Unable to read current-app notebook file: {error}")
                            );
                            me.file_state =
        ...
        {
            // WASM builds should never call `open_current_app_file`, so we should never get here!
            safe_warn!(
                safe: ("Current-app filesystem access is not available in this build"),
                full: ("Current-app filesystem access is not available in this build (feature \"local_fs\" disabled)")
            );
            self.file_state = FileState::Error(SourceFile::CurrentAppFile {
                current_app_path,
                session,
            });
            ctx.notify();
    ...
    fn reload_file(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let (current_app_path, session) =
            match mem::replace(&mut self.file_state, FileState::NoFile) {
                FileState::NoFile => return,
                FileState::Loading(source)
                | FileState::Error(source)
                | FileState::Loaded(source) => match source {
                    SourceFile::CurrentAppFile {
                        current_app_path,
                        session,
                    } => (current_app_path, session),
                    SourceFile::Static { .. } => return,
                },
            };
        self.open_current_app_file(current_app_path, session, ctx);
    }

    #[cfg(feature = "local_fs")]
    fn open_as_code(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(path) = self.current_app_path() {
    ...
    /// The path to the currently-open current-app file, if any.
    pub fn current_app_path(&self) -> Option<PathBuf> {
        self.file_state.current_app_path().map(Path::to_path_buf)
    }
        ...
        match event {
            ...
            EditorViewEvent::RunWorkflow(workflow) => {
                ...
                let source = workflow.source.unwrap_or(WorkflowSource::Notebook {
                    notebook_id: None,
                    location: NotebookLocation::CurrentAppFile,
                });
    ...
    fn handle_active_session_change(
        ...
        if self.location.is_none() {
            let Some(path) = self.current_app_path() else {
                return;
            };
            if let Some(active_session) = handle.as_ref(ctx).session(ctx.window_id()) {
                if active_session.uses_current_app_environment() {
...
impl BackingView for FileNotebookView {
    ...
    fn pane_header_overflow_menu_items(
        ...
        let mut actions = vec![];
        if let Some(SourceFile::CurrentAppFile {
            current_app_path: _current_app_path,
            ..
        }) = self.file_state.source()
        {
            ...
            #[cfg(feature = "local_fs")]
            {
                ...
                // duplicate it in the overflow menu. Keep "Open in editor" available for current-app files.
                actions.push(
