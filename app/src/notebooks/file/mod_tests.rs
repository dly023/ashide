    ...
    App::test((), |mut app| async move {
        ...
        handle
            .update(&mut app, |file_notebook, ctx| {
                file_notebook.open_current_app_file("../README.md", Some(session), ctx);
...
fn test_load_before_session() {
    ...
    App::test((), |mut app| async move {
        ...
        handle
            .update(&mut app, |file_notebook, ctx| {
                file_notebook.open_current_app_file("../README.md", None, ctx);
                match &file_notebook.file_state {
                    FileState::Loading(source) => {
                        assert_eq!(source.current_app_path(), Some(Path::new("../README.md")))
                    }
        ...
        handle.read(&app, |view, _| {
            ...
            match &view.file_state {
                FileState::Loaded(source) => {
                    assert_eq!(source.current_app_path(), Some(expected_path.as_path()));
                }
