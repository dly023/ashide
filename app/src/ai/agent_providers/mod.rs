...
use crate::ai::llms::{
    AvailableLLMs, DisableReason, LLMContextWindow, LLMInfo, LLMProvider, ModelsByFeature,
};
...
fn placeholder_llm_info() -> LLMInfo {
    LLMInfo {
        // Reference the live settings section labels so the pointer can never
        // drift out of sync with the actual Settings UI again.
        display_name: crate::t!(
            "model-selector-no-provider-placeholder-name",
            ai = crate::t!("settings-section-ai"),
            providers = crate::t!("settings-section-agent-providers")
        ),
        base_model_name: crate::t!("model-selector-no-provider-placeholder-model"),
