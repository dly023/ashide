...
pub const BLOCKED_BYOP_REQUEST_MESSAGE: &str =
    "Can't continue this conversation: an earlier tool result is missing or corrupted in this conversation's history, so Ashide can't safely send the request to your provider. Start a new conversation or fork from an earlier point to continue.";
