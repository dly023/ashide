...
///
/// Ashide 中 plan 只写本地 SQLite,不写云端 Drive,因此
...
impl AIDocumentModel {
    ...
    ///
    /// Ashide 本地化:原 `create_document_from_notebook` 依赖云 notebook 提供
    /// title + object_store_id;本地路径只需要 id + content,仅用于「会话恢复
    /// 时 plan 不在内存中」的占位重建。
    ...
    pub fn create_new_version_and_apply_diffs(
        ...
        if let Some(doc) = self.create_new_document_version(id, ctx) {
            ...
            // Ashide 本地化:不再向云 notebook 推送新版本内容,全走本地 SQLite。
            Some(version)
        } else {
            None
        }
    ...
    pub fn restore_document(
        ...
        // Ashide 本地化:plan 只存本地 SQLite,不再从云 notebook 反查 object_store_id。
    }
    ...
    fn handle_save_request(
        ...
        if self
            ...
            .unwrap_or(false)
        {
            // Ashide 本地化:不再向云 notebook 推送内容,只写本地 SQLite。
