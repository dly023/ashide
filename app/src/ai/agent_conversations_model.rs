...
use crate::ai::blocklist::{format_usage_units, BlocklistAIHistoryEvent, BlocklistAIHistoryModel};
...
impl ConversationOrTask<'_> {
    ...
    pub(super) fn request_usage(&self, app: &AppContext) -> Option<f32> {
        match self {
            ConversationOrTask::Task(task) => task.usage_units(),
            ConversationOrTask::Conversation(metadata) => {
                let history_model = BlocklistAIHistoryModel::as_ref(app);
                history_model
                    .conversation(&metadata.nav_data.id)
                    .map(|conv| conv.usage_units())
                    .or_else(|| {
                        history_model
                            .get_conversation_metadata(&metadata.nav_data.id)
                            .and_then(|m| m.usage_units)
                    })
            }
        }
    }
    ...
    pub fn display_request_usage(&self, app: &AppContext) -> Option<String> {
        self.request_usage(app).map(format_usage_units)
    }
    ...
    /// Returns navigation data for persisted user conversations, used for emitting the Navigate event.
    ...
    pub fn run_time(&self) -> Option<String> {
        match self {
            ...
            // Persisted user conversations don't currently track run time
    ...
    fn matches_owner_and_creator(
        ...
        let passes_owner = match owner_filter {
            ...
            OwnerFilter::PersonalOnly => match self {
                ...
                // Persisted user conversations are always owned by the current user
...
pub struct AgentConversationsModel {
    ...
    /// 本地化后无轮询,仅作为 register_view_open/closed 的占位记录使用。
...
impl AgentConversationsModel {
    ...
    /// Returns true if we have tasks or persisted user conversations in this view
    ...
    /// Returns the app conversation ID represented by the given task, if this task and a
    /// conversation entry both point at the same underlying interactive run.
    ///
    /// We first match using the run ID, and then use the conversation token for cases where the task only carries conversation identity
    ...
    pub fn conversation_id_for_task_id(
        &self,
        ...
        let history_model = BlocklistAIHistoryModel::as_ref(app);
        Self::conversation_id_shadowed_by_task(task, history_model).or_else(|| {
            task.conversation_id.as_ref().and_then(|conversation_id| {
                AIConversationId::try_from(conversation_id.clone()).ok()
            })
        })
    }
    ...
    pub fn get_all_creators(&self, app: &AppContext) -> Vec<(String, String)> {
        ...
        // Include the current user since they may have persisted user conversations
