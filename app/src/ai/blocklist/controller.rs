...
pub struct SessionContext {
    ...
    /// Ashide:legacy SSH session(用户在本地 PTY 手敲 `ssh xxx@yyy`,
...
impl SessionContext {
    pub fn from_session(session: &ActiveSession, app: &AppContext) -> Self {
        ...
        let is_legacy_ssh = session_arc
            ...
            .unwrap_or(false);
        SessionContext {
            session_id: session_arc.as_ref().map(|session| session.id()),
    ...
    pub fn session_id(&self) -> Option<warp_core::SessionId> {
        self.session_id
    }
    ...
    /// Returns the environment host ID if this session has a connected runtime
    /// client.
    ...
    /// Returns `true` if this session uses an Environment Runtime, regardless
    /// of whether the runtime client is connected.
    ...
    /// Ashide:legacy SSH 连接信息(host/port),仅在 `is_legacy_ssh()` 为 true 时有意义。
    pub fn ssh_connection_info(&self) -> Option<&InteractiveSshCommand> {
        self.ssh_connection_info.as_ref()
    }

    /// Ashide:本会话是否为 legacy SSH(用户手敲 ssh,远端无 warp hook)。
...
pub struct BlocklistAIController {
    ...
    /// conversation, because the ambient agent task driver owns the entire Ashide window working on a task, and any
...
impl BlocklistAIController {
    ...
    fn maybe_populate_plans_for_ai_document_model(
        ...
        for attachment in referenced_attachments.values() {
            ...
            // Ashide 本地化:plan 只存本地 SQLite。会话恢复时如果 plan 不在内存中,
                ...
                error.downcast_ref::<crate::ai::agent::conversation::UpdateConversationError>()
            {
                if matches!(
                    ...
                    return self.complete_byop_blocked_request(
                        ...
                        request_params.model.clone(),
                        is_queued_prompt,
                        "Ashide couldn't save the BYOP conversation state needed to send this \
    ...
    fn handle_response_stream_event(
        ...
        match event {
                    ...
                    Ok(event) => {
                        ...
                        match event {
                            ...
                            warp_multi_agent_api::response_event::Type::Finished(
                                ...
                                // Ashide BYOP 本地会话压缩:在 stream finished 前拿 summarization 标志
            ...
            ResponseStreamEvent::AfterStreamFinished { cancellation } => {
                ...
                // Ashide BYOP:收集本轮新加 message id,稍后用于在 EMPTY 分支检测
                ...
                if let Some(stream_cancellation) = &cancellation {
                    ...
                    // Ashide:LRC tag-in 首轮自动授权 agent 工具执行。
                    //
                    ...
                    // Ashide BYOP:from_args 解析失败时,chat_stream 走 fallback emit
                    ...
                    // 同标记反复命中导致死循环。
                    // Ashide BYOP:没有进入 AIAgentAction 队列的 synthetic
                    // ToolCallResult 需要 auto-resume,否则 exchange 静默结束,模型卡死等结果。
                    // 1. invalid_arguments — from_args 解析失败兜底(原始)。
                    // 2. _byop_intercepted — todowrite / webfetch / websearch 等app-side intercepted
                    //    工具结果。这类工具不走 protobuf executor,直接合成 result,
                    //    没有 AIAgentAction 入队。
                    let needs_byop_synthetic_resume = conversation.all_tasks().any(|task| {
                        task.messages().any(|msg| {
                    ...
                    if needs_byop_synthetic_resume {
                        log::info!(
                            "[byop] detected synthetic app-handled tool_result (invalid_arguments \
    ...
    pub(super) fn handle_response_stream_finished(
        ...
        // Ashide BYOP 本地会话压缩:在 token_usage move 进下面 closure 前先聚合,
        ...
        match finished_event.reason {
            Some(warp_multi_agent_api::response_event::stream_finished::Reason::Done(_)) | None => {
                ...
                // Ashide BYOP 本地会话压缩 - auto overflow 触发(对齐 opencode `processor.ts:395-403`)
                // 仅在本流不是摘要本身时检查,防止递归。
...
/// Ashide BYOP 专用:LRC tag-in / agent-monitored 场景下提取 RunningCommand。
///
/// 上游 `get_running_command` 在 `is_agent_monitoring()` 时返回 None — 因为 Ashide 自家
