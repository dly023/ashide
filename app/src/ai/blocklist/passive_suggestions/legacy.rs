...
use crate::ai::blocklist::{read_current_app_file_context, BlocklistAIPermissions};
...
impl PassiveSuggestionsModel {
    ...
    fn generate_prompt_suggestions(
        ...
        // Ashide 已剥离 Ashide Inc 云端,无 BYOP 配置时静默 no-op。
    ...
    fn maybe_generate_passive_code_diff(
        ...
        let can_read_file = BlocklistAIPermissions::as_ref(ctx)
            ...
            .is_allowed();
        let should_skip_for_environment_runtime = self
            .active_session
            .as_ref(ctx)
            .session_type(ctx)
        ...
        self.code_diff_preflight_future_handle = Some(ctx.spawn(
            async move {
                let file_future =
                    read_current_app_file_context(&files, current_working_directory, shell, None, None);
