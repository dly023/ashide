    ...
    ai::{
        ...
        blocklist::{
            inline_action::code_diff_view::{
                CodeDiffView, CodeDiffViewEvent, DiffFileBackend, FileDiff,
            },
...
impl RequestFileEditsExecutor {
    ...
    pub(super) fn should_autoexecute(
        ...
        if uses_environment_runtime {
            return BlocklistAIPermissions::as_ref(ctx)
                .can_write_environment_files(&conversation_id, Some(self.terminal_view_id), ctx)
                .is_allowed();
        }
    ...
    pub(super) fn execute(
        ...
        ctx.subscribe_to_view(diff_view, move |_, event, _| match event {
    ...
    fn on_diffs_applied(
        ...
        // Set the file backend on the diff view so save/delete/create routes
        // through the correct FileModel backend.
        let active_session = self.active_session.as_ref(ctx);
        let diff_file_backend = active_session
            .session_type(ctx)
            .as_ref()
            .and_then(SessionType::environment_runtime_host_id)
            .cloned()
            .zip(active_session.session(ctx).map(|session| session.id()))
            .map(
                |(host_id, session_id)| DiffFileBackend::EnvironmentRuntime {
                    host_id,
                    session_id,
                },
            )
            .unwrap_or(DiffFileBackend::CurrentAppFileSystem);

        diff_view.update(ctx, |diff_view, ctx| {
            diff_view.set_diff_file_backend(diff_file_backend);
