...
impl CallMCPToolExecutor {
    ...
    pub(super) fn execute(
        ...
        #[cfg(not(target_family = "wasm"))]
        {
                        ...
                        .active_server_is_current_app_file_based(*installation_id, ctx)
                    {
                        return ActionExecution::Sync(AIAgentActionResultType::CallMCPTool(
                            CallMCPToolResult::Error(
                                "Current-app file-based MCP tools are not available in Environment Runtime sessions"
                                    .to_owned(),
                            ),
                        ));
                    }
            ...
            if let Some(schema) = templatable_mcp_manager.tool_input_schema_in_mcp_scope(
                *server_id,
                name.as_str(),
            ...
            let templatable_peer = if let Some(installation_id) = server_id {
                ...
                templatable_mcp_manager.server_with_tool_name_in_mcp_scope(
                    name.as_str(),
                    include_current_app_file_based_mcp,
                    ctx,
                )
            ...
            ActionExecution::new_async(
                ...
                move |res, _| handle_call_tool_result(res),
            )
        }
    }
