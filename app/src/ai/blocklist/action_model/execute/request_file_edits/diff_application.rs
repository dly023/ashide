...
///
/// This is the common currency between filesystem-backed and environment
/// runtime-backed file-reading paths so that all diff application logic can be
/// shared.
...
pub(crate) enum DiffApplicationError {
    ...
    /// File read/write operations are not available in this Environment Runtime session.
    /// This covers both connection-dropped and unsupported environment session types.
    EnvironmentFileOperationsUnsupported,
}
    ...
    fn to_conversation_message(&self) -> String {
        match self {
            ...
            DiffApplicationError::EnvironmentFileOperationsUnsupported => {
                "The file read/edit tool is not available in this Environment Runtime session. Try using a different tool.".to_string()
            }
        }
    }
...
///
/// Errors are returned for display and for the agent response.
...
/// Shared implementation used by normal diff application and debug conversation import paths.
