...
use crate::terminal::model::session::{
    active_session::ActiveSession, BootstrapSessionType, SessionId, SessionInfo, Sessions,
};
use crate::terminal::model_events::ModelEventDispatcher;
use watcher::HomeDirectoryWatcher;

fn add_read_skill_executor(
    app: &mut App,
    ...
    model_events.update(app, |dispatcher, _ctx| {
        dispatcher.set_active_session_id(session_id);
    });

    let sessions_for_active = sessions.clone();
    let model_events_for_active = model_events.clone();
    let active_session = app.add_model(move |ctx| {
        ActiveSession::new(sessions_for_active, model_events_for_active, ctx)
    });
...
fn test_read_skill_executor_success() {
    ...
    App::test((), |mut app| async move {
        ...
        let executor_handle = add_read_skill_executor(&mut app, BootstrapSessionType::Local);
...
#[test]
fn test_read_skill_executor_refuses_current_app_fallback_for_environment_runtime_session() {
    let temp_dir = TempDir::new().unwrap();
    let skill_path = create_test_skill_file(&temp_dir, "remote-skill", "Should not read locally");
    ...
    App::test((), |mut app| async move {
        ...
        let executor_handle =
            add_read_skill_executor(&mut app, BootstrapSessionType::EnvironmentRuntime);

        let action = AIAgentAction {
            id: AIAgentActionId::from("remote-read-skill-action".to_string()),
            action: AIAgentActionType::ReadSkill(ReadSkillRequest {
                skill: SkillReference::Path(skill_path),
            }),
            task_id: TaskId::new("remote-read-skill-task".to_string()),
            requires_result: false,
        };
        ...
        executor_handle.update(&mut app, |executor, ctx| {
            ...
            match result {
                AnyActionExecution::Sync(AIAgentActionResultType::ReadSkill(
                    ...
                    assert!(
                        msg.contains("Environment Runtime") && msg.contains("finishes connecting"),
                        "msg={msg}"
                    );
                }
                _ => panic!("Environment Runtime read_skill must not read current-app skills"),
            }
        });
    });
}
...
#[test]
fn test_plain_skill_name_rejects_paths() {
    assert!(is_plain_skill_name("frontend"));
    assert!(!is_plain_skill_name("../frontend"));
    assert!(!is_plain_skill_name("."));
    assert!(!is_plain_skill_name(""));
}
