...
/// Per-file byte limit for [`read_current_app_file_context`]. Binary files larger
/// than this are skipped; text files are truncated at this limit.
#[cfg(feature = "local_fs")]
const MAX_FILE_READ_BYTES: usize = 1_000_000;

/// The results of a [`read_current_app_file_context`] call.
...
pub async fn read_current_app_file_context(
    ...
    return Err(anyhow::anyhow!(
        "Can't read files when the current app filesystem is unavailable"
    ));
...
/// path in [`read_current_app_file_context`], `false` if it should be read as text.
///
...
/// Runs a shell command on the session so it works for both current-app and Environment Runtime sessions.
