...
use crate::{
    ...
    code_review::pane_entrypoint::CodeReviewPaneEntrypoint,
...
pub enum CodeDiffViewEvent {
    ...
    SavedAcceptedDiffs {
        ...
        /// files from the active backend when building context for
        /// the LLM.
...
/// by the CodeDiffModel for each file 2) wait for the changes to be saved through the active backend.
///
...
/// File backend used by code diffs when registering editable files.
#[derive(Clone, Debug)]
pub enum DiffFileBackend {
...
pub struct CodeDiffView {
    ...
    /// Filesystem backend targeted by this diff view.
    diff_file_backend: DiffFileBackend,
}
...
impl CodeDiffView {
    ...
    fn build(
        ...
        Self {
            ...
            should_show_speedbump,
            session_platform,
            diff_file_backend: DiffFileBackend::CurrentAppFileSystem,
        }
    }

    /// Set the session type for this diff view.
    ///
    /// `set_candidate_diffs` registers files with this backend instead of assuming
    /// the current-app filesystem.
    pub fn set_diff_file_backend(&mut self, file_backend: DiffFileBackend) {
        self.diff_file_backend = file_backend;
    }
    ...
    pub fn set_candidate_diffs(&mut self, diffs: Vec<FileDiff>, ctx: &mut ViewContext<Self>) {
        ...
        let pending_diffs = diffs
            ...
            .map(|(idx, diff)| {
                ...
                #[cfg(not(target_family = "wasm"))]
                {
                    let session_type = &self.diff_file_backend;
    ...
    fn render_header_contents(
        ...
        if let Some(skill) = skill {
            if let SkillReference::Path(skill_path) = skill.reference.clone() {
                let skill_reference = skill.reference.clone();
        ...
        // config and a Ashide config at once) show no badge to avoid misleading attribution.
