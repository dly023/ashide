...
//! to known documents (e.g. Ashide Drive objects).
...
use crate::{
    ai::agent::AIAgentCitation, object_store::model::persistence::ObjectStoreModel,
...
pub(crate) fn is_command_copied_from_document(
    ...
    match document {
        AIAgentCitation::LocalDriveObject { uid } => {
            is_command_copied_from_local_drive_object(command, uid, shell_type, ctx)
        }
        _ => false,
    }
}
...
/// Ashide Drive object identified by `object_uid`.
fn is_command_copied_from_local_drive_object(
    command: &str,
    object_uid: &str,
    shell_type: Option<ShellType>,
    ctx: &AppContext,
