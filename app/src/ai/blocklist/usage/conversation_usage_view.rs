...
impl ConversationUsageView {
    ...
    fn render_unified_layout(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        if self.usage_info.usage_units_for_last_block.is_some() {
            let last_block_usage_units = self.usage_info.usage_units_for_last_block.unwrap();
            labels.push(render_label_text("Usage (last response)", appearance));
            values.push(render_value_text(
                format_usage_units(last_block_usage_units),
                appearance,
            ));

            labels.push(render_label_text("Usage (total)", appearance));
            values.push(render_value_text(
                format_usage_units(self.usage_info.usage_units),
                appearance,
            ));
        } else {
    ...
    /// Render the footer card container.
