...
impl AgentViewZeroStateBlock {
    ...
    pub fn new(
        ...
        let (current_working_directory, active_session_id) = {
            let terminal_model = terminal_model.lock();
            (
                current_working_directory_for_zero_state(&terminal_model),
                terminal_model.block_list().active_block().session_id(),
            )
        };
        let active_session_uses_current_app_environment =
            active_session_uses_current_app_environment_for_zero_state(
                active_session_id,
                sessions.as_ref(ctx),
            );
        let cached_recent_conversations = if active_session_uses_current_app_environment {
            ...
            // Conversation history metadata is still keyed by cwd only. In an
            // Environment Runtime session, the same string path (for example
            // /root/project) can exist on many hosts and must not be matched
            // against current-app history. Keep remote zero-state history empty
            // until conversation metadata carries an Environment authority key.
            Vec::new()
        };
...
fn active_session_uses_current_app_environment_for_zero_state(
    active_session_id: Option<crate::terminal::model::session::SessionId>,
    sessions: &Sessions,
) -> bool {
    active_session_id
        .and_then(|session_id| sessions.get(session_id))
        .is_none_or(|session| session.uses_current_app_environment())
}
...
pub enum AgentViewZeroStateAction {
    ...
    /// 用户可在「设置 → Ashide 智能体 → AI 输入」中重新开启。
    HideZeroStateHints,
}
...
fn current_working_directory_for_zero_state(terminal_model: &TerminalModel) -> Option<String> {
    terminal_model
        ...
        .cloned()
        .or_else(|| {
            let is_bootstrapping_environment_session = terminal_model.has_pending_ssh_session()
                || terminal_model
                    .get_pending_session_info()
                    .as_ref()
                    .is_some_and(|pending_session_info| {
                        pending_session_info.session_type.uses_environment_runtime()
                    });
            (!terminal_model.block_list().is_bootstrapped()
                && !is_bootstrapping_environment_session)
                .then(|| {
                    terminal_model
