use super::{
    active_session_uses_current_app_environment_for_zero_state, display_working_directory,
    format_session_location,
};
...
use crate::terminal::model::{
    session::{Session, SessionId, SessionInfo, Sessions},
    TerminalModel,
};
use crate::terminal::{
    color::{self, Colors},
...
#[test]
fn cwd_for_recent_conversations_uses_startup_path_before_current_app_bootstrap() {
...
#[test]
fn cwd_for_recent_conversations_does_not_use_startup_path_for_pending_environment_session() {
    let mut terminal = prebootstrap_terminal_with_startup_path("/startup/path");
...
#[test]
fn recent_conversation_lookup_is_allowed_for_current_app_session() {
    let mut sessions = Sessions::new_for_test();
    sessions.register_session_for_test(SessionInfo::new_for_test().with_id(0));

    assert!(active_session_uses_current_app_environment_for_zero_state(
        Some(SessionId::from(0)),
        &sessions,
    ));
}

#[test]
fn recent_conversation_lookup_is_disabled_for_environment_runtime_session() {
    let mut sessions = Sessions::new_for_test();
    sessions.register_session_for_test(
        SessionInfo::new_for_test()
            .with_id(0)
            .with_ssh_socket_path(PathBuf::from("/tmp/ashide-test.sock")),
    );
