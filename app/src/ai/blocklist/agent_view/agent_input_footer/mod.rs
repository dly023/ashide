pub(super) mod chips;
pub mod editor;
// Ashide Wave 7-3:`environment_selector` was removed with the hosted-mode footer.
...
use crate::{
    ...
    terminal::{
        cli_agent_sessions::{
            listener::session_supports_rich_status, CLIAgentInputState, CLIAgentSession,
...
// Ashide Wave 7-3:`warp_cli::agent::Harness` import was removed with the hosted-mode footer.
...
// Ashide Wave 7-3:`EnvironmentSelector` / `EnvironmentSelectorEvent` re-export was removed
...
// Ashide Wave 7-3: removed the hosted-mode footer gap constant with the old footer.
...
/// Returns `"<agent_prefix>"` for current-app sessions or `"<agent_prefix>@<host>"` for an environment host.
fn plugin_chip_key(agent_prefix: &str, environment_host_key: &Option<String>) -> String {
    match environment_host_key {
        Some(host) => format!("{agent_prefix}@{host}"),
        None => agent_prefix.to_owned(),
    }
}

fn plugin_chip_key_for_session(session: &CLIAgentSession) -> String {
    plugin_chip_key(
        session.agent.command_prefix(),
        &session.environment_host_key,
    )
}
...
pub struct AgentInputFooter {
    ...
    // Ashide Wave 7-3:`environment_selector` field was removed with the hosted-mode footer.
...
impl AgentInputFooter {
    ...
    pub fn new(
        ...
        // Ashide Wave 7-3:`EnvironmentSelector` 初始化 + 订阅 + ambient_agent
        ...
        let mut me = Self {
            ...
            // Ashide Wave 7-3:`environment_selector` field init was removed with hosted-mode UI.
            // 子系统物理删。
    ...
    // Ashide Wave 7-3: hosted-mode footer rendering was removed.
    ...
    fn plugin_chip_kind(&self, app: &AppContext) -> Option<PluginChipKind> {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            let chip_key = plugin_chip_key_for_session(session);
            ...
            // For environment-host sessions, we can't check the filesystem.
            if session.uses_environment_host() {
            ...
            // Not installed in the current app filesystem.
    ...
    fn should_use_manual_mode(&self, app: &AppContext) -> bool {
        ...
        sessions_model.has_plugin_auto_failed(session.agent, &session.environment_host_key)
    }
    ...
    fn record_plugin_auto_failure_and_notify(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(agent) = self.cli_agent(ctx) {
            let environment_host_key = CLIAgentSessionsModel::as_ref(ctx)
                .session(self.terminal_view_id)
                .and_then(|s| s.environment_host_key.clone());
            CLIAgentSessionsModel::handle(ctx).update(ctx, |model, _| {
                model.record_plugin_auto_failure(agent, environment_host_key);
            });
        }
        let window_id = ctx.window_id();
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
        ...
        Fut: std::future::Future<Output = Result<(), PluginInstallError>> + Send + 'static,
    {
        ...
        let (shell_path, shell_type) = match shell_data {
            ...
            // needing the Ashide plugin to integrate with it.
    ...
    pub fn has_open_chip_menu(&self, app: &AppContext) -> bool {
        // Ashide Wave 7-3:`environment_selector` is_menu_open() check was removed with hosted-mode UI.
        // 子系统物理删。
    ...
    pub fn toggle_cli_voice_input(
        ...
        match &self.cli_voice_input_state {
            CLIVoiceInputState::Stopped => {
                // Ashide(Phase 3c A1):删除 `AIRequestUsageModel::can_request_voice`
    ...
    fn update_context_window_button(&mut self, ctx: &mut ViewContext<Self>) {
            ...
            BlocklistAIHistoryModel::as_ref(ctx).active_conversation(self.terminal_view_id)
        {
            ...
            // and is only written when a stream finishes with usage metadata. Keep
            // the button's neutral initial tooltip instead of showing "100% remaining".
            if usage == 0.0 {
...
impl View for AgentInputFooter {
    ...
    fn render(&self, app: &warpui::AppContext) -> Box<dyn warpui::Element> {
        ...
        // Ashide Wave 7-3: removed environment-selector ambient-agent chip injection.
...
pub enum AgentInputFooterEvent {
    ...
    // Ashide Wave 7-3:`OpenEnvironmentManagementPane` event was removed with hosted-mode UI.
    // 物理删。
...
/// Green-accented theme for the "Install Ashide plugin" chip.
...
async fn write_install_log(agent: CLIAgent, err: &PluginInstallError) -> Option<PathBuf> {
    ...
    let contents = format!(
        "Ashide plugin installation — {agent:?}\n\
         {now}\n\
         \n\
         {log}",
        log = err.log,
