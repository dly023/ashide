...
impl SuggestedRuleView {
    ...
    pub fn is_edit_allowed(&self, ctx: &mut ViewContext<Self>) -> bool {
        let Some(SuggestedRuleAndId { .. }) = &self.rule_and_id else {
            return false;
        };
    ...
    fn handle_object_store_event(&mut self, event: &ObjectStoreEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ObjectStoreEvent::ObjectUpdated {
                ...
                if let Some(rule_and_id) = &self.rule_and_id {
                    if rule_and_id.object_store_id.into_client() == id.into_client() {
                        self.load_rule(ctx);
                    }
                }
            }
            ...
            | ObjectStoreEvent::ObjectDeleted {
                ...
                if let Some(rule_and_id) = &self.rule_and_id {
                    if rule_and_id.object_store_id == *id {
                        self.reset_rule(ctx);
                    }
                }
            }
    ...
    pub fn add_rule(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(owner) = self.owner {
            ...
            update_manager.update(ctx, |update_manager, ctx| {
                if let Some(client_id) = object_store_id.into_client() {
