...
fn test_detect_secrets_no_regexes_configured() {
    // Explicitly clear the process-wide secret regex globals so this test asserts on a known
    // "no regexes configured" state rather than relying on ambient state. Other (serial) tests
    // in this module populate the global registry, so we must reset it to be deterministic when
