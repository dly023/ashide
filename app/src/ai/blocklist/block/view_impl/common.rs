    ...
    ai::{
        ...
        blocklist::{
            ...
            code_block::{
                ...
                CodeBlockStyle, CodeSnippetButtonHandles,
            },
            inline_action::{
...
const INTERNAL_WARP_ERROR: &str = "Internal Ashide error.";
...
pub fn render_warping_indicator_base(
    ...
    // Unicode code point for the app glyph that is embedded in the version of Roboto we bundle
    ...
    if let Some(sub_element) = secondary_element {
        // Our warping indicator text prepends the app glyph (and a space) to the label.
...
pub struct TextSectionsProps<'a, V, A: 'static> {
    ...
    pub code_block_style: Option<&'a CodeBlockStyle>,
...
struct VisualMarkdownBlockOptions<A: 'static> {
    ...
    /// app tooltip primitive so hovering surfaces the CommonMark image title.
...
fn render_visual_markdown_block<A: Action>(
    ...
    // Wrap the rendered image in the standard app tooltip when the source
...
fn render_table_section(
    ...
    if let AgentOutputTableRendering::PlainText { content } = &table.rendering {
        return render_plain_text_table_section(
            content,
...
fn render_plain_text_table_section(
    content: &str,
...
pub fn render_code_output_section<A: Action>(
    ...
    let language_text = props.language.map(|language| {
        ...
        .with_color(code_block_style.sub_text)
        .with_selection_color(if props.is_ai_input_enabled {
            theme.text_selection_as_context_color().into_solid()
        } else {
            theme.text_selection_color().into_solid()
    ...
    match props.editor_view {
        Some(view) => render_code_block_with_warp_text(
            CodeBlockOptions {
                ...
                style: Some(code_block_style.clone()),
            },
            view,
            app,
            source,
...
pub fn render_failed_output(props: FailedOutputProps, app: &AppContext) -> Box<dyn Element> {
    ...
    let error_text = match props.error {
        RenderableAIError::QuotaLimit => {
            // 本地化后额度刷新时间不适用，仅保留通用错误文案。
            format!("{ERROR_APOLOGY_TEXT}\n\n{INTERNAL_WARP_ERROR}")
        }
        RenderableAIError::ServerOverloaded => {
            "Ashide is currently overloaded. Please try again later.".to_string()
        }
...
pub(crate) fn render_debug_footer<V: View>(
    ...
    // 不再用 `Expanded` —— alt-screen / 长命令 take-over 场景下,父容器
    // 沿主轴是 infinite constraint(error block 渲染路径),`Flex + Expanded`
    // 会 panic `flex contains flexible children but has an infinite constraint`。
