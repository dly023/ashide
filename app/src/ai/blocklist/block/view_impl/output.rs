...
use crate::ai::blocklist::view_util::format_usage_units;
...
pub(crate) struct Props<'a> {
    ...
    pub(super) code_block_style: Option<&'a crate::ai::blocklist::code_block::CodeBlockStyle>,
                            ...
                            if !are_all_text_sections_empty(sections) =>
                        {
                            ...
                            let text_sections = render_text_sections(
                                TextSectionsProps {
                                    ...
                                    code_block_style: props.code_block_style,
                        ...
                        AIAgentOutputMessageType::Action(AIAgentAction {
                            ...
                            if !status.is_streaming() || !files.is_empty() {
                                ...
                                let skill = common_path(&file_paths)
                                    ...
                                    .and_then(|skill_path| {
                                        crate::ai::skills::environment_skill_inventory::skill_descriptor_for_path(
                                            &skill_path, app,
                                        )
                                    });
                        ...
                        Ordering::Equal | Ordering::Less => {
                            output_items.add_child(
                                render_informational_footer(
                                    app,
                                    "Sorry you had a bad experience with this interaction. We've adjusted this request usage. We appreciate your feedback!"
                                        .to_string(),
                                )
                                .with_agent_output_item_spacing(app)
                                .finish(),
                        ...
                        Ordering::Greater => {
                            output_items.add_child(
                                render_informational_footer(
                                    app,
                                    format!(
                                        "Sorry you had a bad experience with this interaction. We've adjusted usage for {request_refunded_count} requests. We appreciate your feedback!"
                                    ),
                                )
                                .with_agent_output_item_spacing(app)
                                .finish(),
...
fn render_read_skill(
    ...
    let appearance = Appearance::as_ref(app);
    let skill = crate::ai::skills::environment_skill_inventory::skill_descriptor_for_reference(
        skill_reference,
        app,
    );
    ...
    if let Some(skill) = skill {
        if !skill.is_bundled() {
            let Some(path) = (match &skill.reference {
...
fn render_read_files(
    ...
    file_names: impl IntoIterator<Item = impl AsRef<str>>,
    app: &AppContext,
    parsed_skill: Option<SkillDescriptor>,
    action_index: usize,
) -> Box<dyn Element> {
    ...
    if let Some(skill) = parsed_skill {
        let SkillReference::Path(path) = skill.reference.clone() else {
            return renderable_action.render(app).finish();
        };
        let source = CodeSource::Skill {
...
fn render_usage_button(props: Props, app: &AppContext) -> Box<dyn Element> {
    ...
    let has_any_usage = conversation.usage_units() > 0.0
        || conversation.usage_units_for_last_block().is_some()
    ...
    let total_usage_units = conversation.usage_units();
    let mut usage_text = format_usage_units(total_usage_units);
    if let Some(usage_units_for_last_block) = conversation.usage_units_for_last_block() {
        // Only show the usage units for the last block if it is different from the total usage units
        // and the last block has non-zero usage.
        // Avoid showing last-block usage if the request failed, as it may be adjusted later.
        if usage_units_for_last_block > 0.0
            && total_usage_units != usage_units_for_last_block
            && props.model.status(app).error().is_none()
        {
            // If the first part of the decimal is 0, we just display the whole number.
            if usage_units_for_last_block.fract() < 0.1 {
                usage_text = format!(
                    "{usage_text} (+{})",
                    usage_units_for_last_block.trunc() as i32
                );
            } else {
                usage_text = format!("{usage_text} (+{usage_units_for_last_block:.1})");
            }
        }
    }
        ...
        |mouse_state| {
            ...
            if mouse_state.is_hovered() || mouse_state.is_clicked() {
                ...
                let tooltip = ui_builder
                    .tool_tip(crate::t!("ai-block-show-usage-details"))
                    .build()
                    .finish();
