...
pub fn render_citation(
    ...
    let (icon, name) = match citation {
        AIAgentCitation::LocalDriveObject { uid } => {
            let item = ObjectStoreModel::as_ref(app)
                .get_by_uid(uid)?
                .to_local_drive_item(appearance)?;
            (
        ...
        AIAgentCitation::WarpDocumentation { .. } => {
            ...
            let name = String::from("Ashide Docs");
            (Some(icon), name)
        }
...
impl View for AIBlock {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let terminal_colors = terminal_model.colors();
        drop(terminal_model);
        let code_block_style =
            crate::ai::blocklist::code_block::CodeBlockStyle::from_terminal_colors(
                Appearance::as_ref(app),
                terminal_colors[color_index::FOREGROUND],
                terminal_colors[color_index::BACKGROUND],
            );
        ...
        contents.add_child(output::render(
            output::Props {
                ...
                code_block_style: Some(&code_block_style),
                detected_links_state: &self.detected_links_state,
                secret_redaction_state: &self.secret_redaction_state,
