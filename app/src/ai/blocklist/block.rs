...
use crate::code::editor::view::{CodeEditorColorOverride, CodeEditorRenderOptions};
...
impl AIBlock {
    ...
    pub fn new(
        ...
        // Ashide:原这里订阅 GetRelevantFilesController 的 Success 事件以在 RAG
        ...
        // Ashide(Phase 3c A1):删除对 `AIRequestUsageModelEvent::RequestBonusRefunded`
    ...
    fn spawn_link_detection(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            let cwd = if uses_environment_runtime {
                None
            } else {
            ...
            && AISettings::as_ref(ctx).is_rule_suggestions_enabled(ctx)
        {
            ...
            for rule in suggestions.rules.into_iter() {
                ...
                ctx.subscribe_to_view(&rule_view, |_me, _view, event, ctx| match event {
                    SuggestedChipViewEvent::OpenAIFactCollection { object_store_id } => {
                        ctx.emit(AIBlockEvent::OpenAIFactCollection {
        ...
        if FeatureFlag::SuggestedAgentModeWorkflows.is_enabled() && self.suggested_rules.is_empty()
        {
            if let Some(workflow) = suggestions.agent_mode_workflows.first() {
                ...
                ctx.subscribe_to_view(&workflow_view, |_me, _view, event, ctx| match event {
                    SuggestedChipViewEvent::OpenWorkflow { object_store_id } => {
                        ctx.emit(AIBlockEvent::OpenWorkflow {
                            object_store_id: *object_store_id,
                        });
                    }
    ...
    fn embedded_code_editor_color_override(&self) -> CodeEditorColorOverride {
        let terminal_model = self.terminal_model.lock();
        let colors = terminal_model.colors();
        CodeEditorColorOverride::new(
            colors[color_index::FOREGROUND],
            colors[color_index::BACKGROUND],
        )
    }
    ...
    fn handle_code_section_stream_update(
        ...
        let color_override = self.embedded_code_editor_color_override();
        match self.code_editor_views.get_mut(index) {
            Some(embedded_view) => {
                embedded_view.view.update(ctx, |view, ctx| {
                    view.set_color_override(Some(color_override), ctx);
...
impl AIBlock {
    ...
    fn update_imported_comments_disabled_state(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        {
            if self.has_imported_comments {
                self.update_own_imported_comments_disabled_state(None, ctx);
...
pub enum AIBlockEvent {
    ...
    /// Open a Ashide Text instance with the requested code diff.
    OpenCodeWithDiff {
        view: ViewHandle<CodeDiffView>,
    },
...
pub enum AIBlockAction {
    ...
    CopyDebugId(String),
    /// Open Ashide feedback documentation
    OpenFeedbackDocs,
