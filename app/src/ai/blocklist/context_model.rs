...
pub struct BlocklistAIContextModel {
    ...
    directory_context_uses_environment_runtime: bool,
...
impl BlocklistAIContextModel {
    pub fn new(
            ...
            ModelEvent::BlockMetadataReceived(block_metadata_received) => {
                ...
                if let Some(session_id) = session_id {
                    ...
                    if let Some(active_session) = active_session {
                        let uses_environment_runtime =
                            active_session.session_type().uses_environment_runtime();
        ...
        Self {
            ...
            directory_context_uses_environment_runtime: false,
    ...
    pub fn pending_context(&self, app: &AppContext, is_user_query: bool) -> Vec<AIAgentContext> {
        ...
        // outline 功能未启用,语义搜索上下文恒 false。
        let is_pwd_indexed = false;

        let project_rules = if !self.directory_context_uses_environment_runtime {
            pwd.clone()
                .and_then(|path| {
        ...
        if is_user_query {
            ...
            // Ashide P0/P1: 把 PendingFile 同步读入并以 AIAgentContext::File 推进 context。
    ...
    pub fn update_directory_context(
        ...
        self.directory_context_uses_environment_runtime = uses_environment_runtime;
