...
pub struct AIConversationMetadata {
    ...
    /// Whether the full conversation data exists in the app database.
    pub has_persisted_conversation_data: bool,
...
impl From<&AIConversation> for AIConversationMetadata {
    fn from(conversation: &AIConversation) -> Self {
        ...
        Self {
            ...
            usage_units: Some(conversation.usage_units()),
            server_conversation_token: conversation.server_conversation_token().cloned(),
            has_persisted_conversation_data: true,
...
pub struct BlocklistAIHistoryModel {
    ...
    /// conversations out of user history and navigation.
    ...
    /// Metadata for both persisted user and ambient agent conversations.
    ...
    /// Reverse index from server-side agent identifier to app conversation ID.
    ///
    ...
    /// ambient-agent task on every conversation refresh.
    ...
    /// Populated at startup from the app database and kept in sync at runtime
...
impl BlocklistAIHistoryModel {
    pub(crate) fn new(
        ...
        // Initialize historical conversations from app database
    ...
    /// Ashide BYOP 专用:见 `Conversation::create_optimistic_cli_subagent_task_silent` 文档。
    ...
    /// Assigns a `run_id` to a conversation that was spawned as a child
    ...
    /// Resolves a server-side agent identifier to an app conversation ID.
    ...
    pub fn delete_conversation(
        ...
        // Session Navigator) can refresh its cache. Historical-only
    ...
    /// Returns user conversation metadata,
    /// (excluding conversations from ambient agent runs).
    pub fn get_user_conversations_metadata(&self) -> impl Iterator<Item = &AIConversationMetadata> {
    ...
    pub fn insert_forked_conversation_from_tasks(
        ...
        // This is harmless if we're opening the conversation immediately, but ensures it is indexed for Session Navigator right away if we fork in the background.
    ...
    pub fn insert_historical_conversation_from_tasks(
        &mut self,
        conversation_id: AIConversationId,
        tasks: Vec<warp_multi_agent_api::Task>,
        conversation_data: AgentConversationData,
        ...
        let agent_link_id = conversation_data
            .run_id
            .clone()
            .or_else(|| conversation_data.server_conversation_token.clone());
        let conversation =
            AIConversation::new_restored(conversation_id, tasks, Some(conversation_data))?;
        let metadata = AIConversationMetadata::from(&conversation);
        if let Some(key) = agent_link_id.or_else(|| agent_id_key(&conversation)) {
            self.agent_id_to_conversation_id
                .insert(key, conversation_id);
        }
        if let Some(token) = &metadata.server_conversation_token {
        ...
        self.all_conversations_metadata
            .insert(conversation_id, metadata);
        self.conversations_by_id.insert(conversation_id, conversation);
...
pub enum BlocklistAIHistoryEvent {
    ...
    /// Session Navigator for a session that was never opened in this run).
    DeletedConversation {
        terminal_view_id: Option<EntityId>,
        conversation_id: AIConversationId,
