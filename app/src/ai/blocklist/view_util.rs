...
/// Formats usage units to display as whole numbers when the value is effectively a whole number,
/// otherwise displays with one decimal place.
pub fn format_usage_units(usage_units: f32) -> String {
    // If the first part of the decimal is 0, we just display the whole number.
    if usage_units.fract() < 0.1 {
        let whole = usage_units.trunc() as i32;
        if whole == 1 {
        ...
        format!("{usage_units:.1} usage units")
    }
}
