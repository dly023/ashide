...
impl BlocklistAIPermissions {
    ...
    pub fn permissions_profile_for_id(
        ...
        AIExecutionProfile {
            ...
            auto_save_plans_to_local_drive: profile_data.auto_save_plans_to_local_drive,
    ...
    /// 判断 Agent Mode 是否可以自动读取非 current-app Environment 里的文件。
    ///
    /// 这里故意不接收 `PathBuf`:Environment Runtime 路径属于已连接主机,
    /// 不能拿来匹配本机 current-app allowlist。
    pub fn can_read_environment_files_with_conversation(
        &self,
        conversation_id: &AIConversationId,
        terminal_view_id: Option<EntityId>,
        ctx: &AppContext,
    ...
    /// Returns whether or not Ashide can auto-read the given files (e.g. for codebase indexing).
    ...
    /// 判断 Agent Mode 是否可以自动写入非 current-app Environment 里的文件。
    ///
    /// Environment 路径由已连接 runtime 解释,本机 protected-path/allowlist
    /// 检查不能把它们当成这台机器上的文件。
    pub fn can_write_environment_files(
        &self,
        conversation_id: &AIConversationId,
        terminal_view_id: Option<EntityId>,
        ctx: &AppContext,
    ...
    pub fn can_ask_user_question(
        ...
        // Ashide 改:auto-approve(ctrl+shift+i)只对 shell/编辑等执行类工具自动通过,
