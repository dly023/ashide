...
impl BlocklistAIActionModel {
    ...
    /// Marks an action as peer-executing on the viewer side.
    /// This is called when a viewer receives a CommandExecutionStarted event from the sharer,
    /// allowing the viewer's UI to show the action as running even though the viewer is not executing it.
    pub fn mark_action_as_peer_executing(
        &mut self,
    ...
    ///
    /// 用途:Ashide BYOP 路径下 LRC tag-in 场景 — 用户主动 SetInputModeAgent 把
    ...
    fn handle_preprocess_actions_results(
        ...
        if auto_accept && !auto_accept_ids.is_empty() {
            // Ashide:LRC tag-in 自动授权路径。Bypass 默认的
    ...
    pub fn apply_finished_action_result(
        ...
        // and apply the associated actions to the viewer document version
        // (or create a new document if the given doc does not exist).
        self.maybe_sync_view_only_documents_with_viewer_model(
            conversation_id,
            &mut action_result,
            ctx,
        );
    ...
    /// are backed by documents in the viewer `AIDocumentModel` and that their
    /// `DocumentContext` versions match. For CreateDocuments, restore missing documents
    /// (using titles from the original action); for EditDocuments, apply edits to viewer
    /// documents and align versions, so headers and "View" buttons stay accurate.
    fn maybe_sync_view_only_documents_with_viewer_model(
        &self,
        conversation_id: AIConversationId,
                ...
                doc_model.update(ctx, |doc_model, doc_ctx| {
                    for doc_context in updated_documents.iter_mut() {
                        ...
                        if let Some(new_version) = doc_model.restore_document_edit(
                            ...
                            // Align the header's version with the viewer-restored doc
