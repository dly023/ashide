...
impl BlocklistAIController {
    ...
    fn on_shared_init(
        ...
        // 如果共享会话 conversation token 已经绑定到 app 对话,直接复用。
        // 否则,当当前 agent view 对话为空时复用该 app 对话 ID,并绑定传入的共享 token。
    ...
    fn on_shared_finished(
        ...
        self.handle_response_stream_finished(
            ...
            conversation_id,
            did_exchange_contain_user_query,
            // shared session 路径不会触发 current-app 压缩(来自 peer viewer 同步流),始终 None
            None,
            ctx,
        );
    }

    /// 查找与共享会话 conversation token 对应的 app 对话。
    ...
    /// Marks an action as peer-executing when a viewer receives a CommandExecutionStarted event.
    /// This allows the viewer's UI to show the action as running rather than queued.
    pub fn mark_action_as_peer_executing_in_shared_session(
        &mut self,
        ...
        self.action_model.update(ctx, |action_model, ctx| {
            action_model.mark_action_as_peer_executing(action_id, conversation_id, ctx);
        });
    }
