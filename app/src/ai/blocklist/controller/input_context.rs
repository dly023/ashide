...
use crate::{
    ai::{
        ...
        skills::environment_skill_inventory::{
...
lazy_static! {
    ...
    // Regex to match Ashide Drive objects inserted via at-context. Ex: <notebook:[workflow_id]>
    ...
    if FeatureFlag::ListSkills.is_enabled() {
            ...
            .is_some_and(|session_type| session_type.uses_environment_runtime())
        {
            active_session
                .session(app)
                .map(|session| {
                    cached_environment_skills_for_session(
                        session.id(),
                        active_session
            ...
            current_app_skills_for_working_directory(
