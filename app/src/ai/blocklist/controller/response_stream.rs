...
fn byop_dispatch_info(
    ...
    // 任一模型不是 BYOP 编码(比如 fallback 到非 BYOP 默认),则跳过 — Ashide 主路径都是 BYOP,
...
pub struct ResponseStream {
    ...
    /// AI identifiers used for BYOP conversation lookup and stream output correlation.
    ai_identifiers: AIIdentifiers,
...
impl ResponseStream {
    ...
    /// Ashide BYOP 本地会话压缩:返回本流是否在跑 SummarizeConversation,
...
async fn byop_required_response_stream(
    ...
    log::debug!("No BYOP provider selected for Ashide agent request");
    let error_stream = futures::stream::once(async {
        Err(Arc::new(AIApiError::Other(anyhow!(
            "Ashide requires a configured BYOP provider in Settings"
        ))))
    })
