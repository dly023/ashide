//! This module contains functions for loading conversation data from the app database.
...
impl BlocklistAIHistoryModel {
    /// Loads conversation data from memory or the app database.
    ///
    /// This method automatically determines whether to load from memory or app storage:
    /// - If the conversation is already in memory, returns it immediately
    /// - If has_persisted_conversation_data is true, loads from the app database synchronously
    ///
    ...
    pub fn load_conversation_data(
        ...
        if metadata.has_persisted_conversation_data {
            // Load from the app database synchronously
            let result = self
            ...
            log::warn!(
                "Cannot load conversation {conversation_id}: no persisted conversation data"
            );
            box_future(futures::future::ready(None))
        }
    }

    /// Loads a conversation from the app database and returns it.
    ...
    pub(super) fn initialize_historical_conversations(
        ...
        let collected: HashMap<AIConversationId, AIConversationMetadata> = conversations
            ...
            .filter_map(|agent_conv| {
                ...
                let usage_units = conversation_data
                    .as_ref()
                ...
                let artifacts = conversation_data
                    ...
                    .unwrap_or_default();
                if let Some(run_id) = conversation_data
                    .as_ref()
                    .and_then(|data| data.run_id.as_deref())
                {
                ...
                let server_conversation_token = conversation_data
                    .as_ref()
                    .and_then(|data| data.server_conversation_token.clone())
