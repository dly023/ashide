...
impl AIDocumentView {
    ...
    fn update_header_buttons(&mut self, ctx: &mut ViewContext<Self>) {
        // Ashide 本地化:plan 不再同步到云 notebook,原本这里从 object_store_id
        // 取 server_id 传给 sharing UI;object_store_id 已删,这里仅重发 header overflow menu。
        self.pane_configuration.update(ctx, |pc, ctx| {
    ...
    fn render_sync_element(
        ...
        match save_status {
            // Ashide 不使用云端同步，Plan 已自动保存到本地 SQLite，
            ...
            // Ashide 中 NotSaved 仅表示文档不存在(调用侧占位),不画任何 UI。
    ...
    fn export(&self, ctx: &mut ViewContext<Self>) {
        ...
        // Export writes to the current app filesystem. Do not use a raw runtime
        // pwd here: remote Environment paths such as `/root/project` are not
        // valid local save-panel defaults.
        ...
        match action {
            ...
            AIDocumentAction::SendUpdatedPlan => {
                ...
                terminal_view.update(ctx, |terminal_view, ctx| {
                    ...
                    // Reuse the existing user-query path so the updated local plan is attached.
                    terminal_view
                        .ai_controller()
