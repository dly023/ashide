...
pub(crate) fn active_terminal_environment_skill_inventory_refresh_args(
    active_session: &TerminalActiveSession,
    app: &AppContext,
) -> Option<(Arc<EnvironmentRuntimeClient>, SessionId, Option<String>)> {
    active_terminal_environment_skill_inventory_refresh_args_with(
        active_session,
        app,
        |session_id| environment_runtime::client_for_session(session_id, app),
    )
}

fn active_terminal_environment_skill_inventory_refresh_args_with(
    active_session: &TerminalActiveSession,
    app: &AppContext,
    client_for_session: impl FnOnce(SessionId) -> Option<Arc<EnvironmentRuntimeClient>>,
) -> Option<(Arc<EnvironmentRuntimeClient>, SessionId, Option<String>)> {
...
mod tests {
    ...
    use crate::terminal::model::session::{
        active_session::ActiveSession, BootstrapSessionType, SessionInfo, Sessions,
    };
    ...
    #[test]
    fn active_environment_inventory_uses_active_session_binding() {
        App::test((), |mut app| async move {
            let first_session_id = SessionId::from(41);
            let active_session_id = SessionId::from(42);
            let sessions = app.add_model(|_| Sessions::new_for_test());
            sessions.update(&mut app, |sessions, _ctx| {
                for session_id in [first_session_id, active_session_id] {
            ...
            let dispatcher = app.add_model(move |ctx| {
                ModelEventDispatcher::new(events_rx, sessions_for_dispatcher, ctx)
            });
            dispatcher.update(&mut app, |dispatcher, _ctx| {
                dispatcher.set_active_session_id(active_session_id);
            });

            let sessions_for_active = sessions.clone();
            let dispatcher_for_active = dispatcher.clone();
            let active_session = app.add_model(move |ctx| {
                ActiveSession::new(sessions_for_active, dispatcher_for_active, ctx)
            });
            let (active_client, _event_rx) = EnvironmentRuntimeClient::new(
                futures::io::empty(),
                futures::io::sink(),
                &app.background_executor(),
            );
            let active_client = Arc::new(active_client);
            let requested_session_id = Cell::new(None);

            let (resolved_client, resolved_session_id, cwd) = app.read(|ctx| {
                active_terminal_environment_skill_inventory_refresh_args_with(
                    active_session.as_ref(ctx),
                    ctx,
                    |session_id| {
                        requested_session_id.set(Some(session_id));
                        (session_id == active_session_id).then(|| active_client.clone())
                    },
                )
                .expect("active Environment session must resolve its exact transport")
            });

            assert_eq!(requested_session_id.get(), Some(active_session_id));
            assert_eq!(resolved_session_id, active_session_id);
            assert!(Arc::ptr_eq(&resolved_client, &active_client));
            assert_eq!(cwd, None);
        });
    }
}
