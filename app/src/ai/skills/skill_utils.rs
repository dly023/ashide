...
///    例如 `Agents > Ashide > Claude > …`。
/// 2. **同 rank 时 reference 路径短者胜**:取为稳定 tiebreak。
///
/// 该实现覆盖了三种场景:
/// - `npx skills` 软链同名 skill 到 `~/.agents/skills/` / `~/.ashide/skills/` / `~/.claude/skills/`
...
pub fn skill_path_from_file_path(file_path: &Path) -> Option<PathBuf> {
    for definition in SKILL_PROVIDER_DEFINITIONS.iter() {
        let home_skill_dirs = if definition.provider == SkillProvider::Ashide {
