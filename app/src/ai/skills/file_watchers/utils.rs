...
use anyhow::Error;
use regex::Regex;
use repo_metadata::{current_app_model::GetContentsArgs, RepoContent, RepoMetadataModel};
...
pub fn find_skill_directories_in_tree(
    ...
    repo_metadata
        ...
        .map(|content| match content {
            ...
            RepoContent::Symlink(symlink) => symlink.path.to_local_path_lossy(),
        })
        .collect()
}
...
pub fn is_home_provider_path(path: &Path) -> bool {
    SKILL_PROVIDER_DEFINITIONS.iter().any(|provider| {
        if provider.provider == SkillProvider::Ashide {
