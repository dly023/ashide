...
fn resolve_from_skill_dirs_by_directory_scan_resolves_home_skill_dir() -> Result<()> {
    ...
    let skill_dir = temp_dir.path().join(".ashide").join("skills");
    ...
    assert!(resolved.instructions.contains("Global Ashide skill"));
...
fn resolve_from_root_path_by_directory_scan_respects_directory_precedence() -> Result<()> {
    ...
    let ashide_skill = root.join(".ashide/skills/my-skill/SKILL.md");
    ...
    write_skill_file(
        ...
        "my-skill",
        "desc",
        "# Claude version\n\nDo not pick this when .ashide exists.",
    )?;
    write_skill_file(
        &codex_skill,
        "my-skill",
    ...
    assert!(!resolved.instructions.contains("Ashide version"));
...
fn parse_org_from_git_url_supports_ssh_and_https() {
    assert_eq!(
        parse_org_from_git_url("git@github.com:example-org/example-internal.git"),
        Some("example-org".to_string())
    );

    assert_eq!(
        parse_org_from_git_url("https://github.com/example-org/example-internal.git"),
        Some("example-org".to_string())
    );
}
...
fn resolve_simple_name_uses_directory_precedence() -> Result<()> {
    ...
    // Note: .agents/skills has highest precedence, followed by .ashide, then .claude.
    let agents_skill = root.join(".agents/skills/my-skill/SKILL.md");
    write_skill_file(
        &agents_skill,
        "my-skill",
