...
pub struct SkillManager {
    ...
    /// Skills bundled into Ashide, each with activation condition and icon.
...
impl SkillManager {
    ...
    ///
    /// 同名多份时按 [`provider_rank`] 升序取第一(`Agents > Ashide > Claude > …`),
    ...
    /// Load skill definitions bundled with Ashide.
...
///
/// Supported variables:
/// - `{{warp_server_url}}` - Empty in Ashide.
/// - `{{warp_cli_binary_name}}` - The CLI binary name (e.g., `ashide` or `ashide-cli`)
/// - `{{warp_url_scheme}}` - The URL scheme (e.g., `ashide`, `ashidedev`, `ashidepreview`)
...
/// branded icon; everything else falls back to the Ashide logo.
