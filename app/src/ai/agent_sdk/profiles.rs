...
use crate::object_store::model::generic_string_model::StringModel;
...
impl ProfilesCommandRunner {
    fn list(&self, global_options: GlobalOptions, ctx: &mut ModelContext<Self>) {
        ...
        let initial_load = ObjectStoreModel::as_ref(ctx).initial_load_complete();

        ctx.spawn(initial_load, move |_, _, ctx| {
            ...
            let profiles: Vec<_> = profile_ids
                ...
                .map(|profile| {
                    ...
                    let id = match profile.object_store_id() {
                        Some(ObjectStoreId::StableId(server_id)) => server_id.to_string(),
                        _ => "Inline local".to_string(),
                    };
                    ProfileInfo { id, name }
                })
                .collect();
