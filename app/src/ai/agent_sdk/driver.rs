...
/// Signals to Claude child-harness hooks that Ashide already owns the background
...
/// self-managed listener path so older Ashide builds and standalone plugin
...
/// that Ashide and the Claude hook scripts share.
...
/// Options for initializing a current-app agent driver.
///
/// This driver is the CLI/current-app runner used by `oz agent run`. Workspace
/// UI entrypoints for Environment Runtime sessions must route through
/// `EnvironmentRuntimeEntryIntent` instead of constructing this driver directly:
/// the working directory and terminal pane here are current-app filesystem
/// values by design.
pub struct CurrentAppAgentDriverOptions {
    /// Initial current-app working directory for the agent's terminal session.
...
/// `AgentDriver` is a model for driving an ambient Ashide agent to completion.
///
...
pub struct Task {
    ...
    /// ID of the profile to run as (ObjectStoreId string). If None, use the default profile.
...
pub enum AgentRunPrompt {
    /// Prompt has already been resolved to a plain string.
    ResolvedText(String),
}
...
pub enum AgentDriverError {
    ...
    ProfileError(String),
    #[error("Current user state is unavailable. Restart Ashide and try again.")]
    NotLoggedIn,
...
impl AgentDriver {
    pub fn new(
        options: CurrentAppAgentDriverOptions,
        ctx: &mut ModelContext<Self>,
    ) -> Result<Self, AgentDriverError> {
        let CurrentAppAgentDriverOptions {
            working_dir,
            task_id,
            parent_run_id,
            should_share,
        ...
        // Ashide 启动时会初始化 current user;走到这里说明 auth singleton 未正确初始化。
        ...
        let terminal_driver = terminal::TerminalDriver::create(
            terminal::CurrentAppTerminalDriverOptions {
    ...
    /// installing the Ashide plugin and platform plugin, if applicable.
    ///
    ...
    async fn prepare_harness(
        ...
        let AgentRunPrompt::ResolvedText(prompt_text) = prompt;
    ...
    fn configure_terminal(
        ...
        if let Some(profile) = profile {
            let server_id = StableObjectId::try_from(profile.as_str())
                .map_err(|_| AgentDriverError::ProfileError(profile.clone()))?;
            let object_store_id = ObjectStoreId::StableId(server_id);
            AIExecutionProfilesModel::handle(ctx).update(ctx, |model, ctx| {
                if let Some(profile_id) = model.get_profile_id_by_object_store_id(&object_store_id)
                {
    ...
    fn execute_run(
        ...
        // Ashide 不同步 plan 到 Ashide Drive,原 "plan_artifact_created" CLI 输出依赖云 notebook_link,
        ...
        self.terminal_driver.update(ctx, |td, ctx| {
            td.with_terminal_view(ctx, |terminal, ctx| {
                let AgentRunPrompt::ResolvedText(prompt_str) = task_prompt;
