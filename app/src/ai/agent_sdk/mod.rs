...
use crate::ai::agent_sdk::driver::{AgentRunPrompt, CurrentAppAgentDriverOptions, Task};
...
fn dispatch_command(
    ...
    match command {
        ...
        CliCommand::Recall(recall_args) => memory::recall(ctx, global_options, recall_args),
        CliCommand::Remember(remember_args) => memory::remember(ctx, global_options, remember_args),
        CliCommand::MCP(mcp_cmd) => mcp::run(ctx, global_options, mcp_cmd),
        CliCommand::Memory(memory_cmd) => memory::run(ctx, global_options, memory_cmd),
        ...
        CliCommand::SessionBridge(session_bridge_cmd) => {
            session_bridge::run(ctx, global_options, session_bridge_cmd)
        }
...
fn run_agent(
    ...
    match command {
        AgentCommand::Run(args) => {
            ...
            if args.harness == Harness::OpenCode {
                return Err(anyhow::anyhow!(
                    "The opencode harness is only supported for current-app child agent launches."
                ));
            }
        ...
        AgentCommand::List(_) => Err(anyhow::anyhow!("Agent skill listing is disabled in Ashide")),
    }
}
...
fn build_merged_config_and_task(
    ...
    // Combine base_prompt with the user prompt before dispatch.
    let resolved_prompt = match (merged_config.base_prompt.as_deref(), prompt) {
    ...
    let task = Task {
        prompt: AgentRunPrompt::ResolvedText(resolve_prompt(&resolved_prompt, ctx)?),
...
impl AgentDriverRunner {
    async fn setup_and_run_driver(
        ...
        // Wait for Ashide Drive to sync before building the task config, since
        // prompt resolution (SavedPrompt -> workflow lookup) depends on it.
        if foreground
            .spawn(|_, ctx| common::refresh_local_drive(ctx))
            .await?
            .await
            .is_err()
        {
            return Err(AgentDriverError::LocalDriveSyncFailed);
        }
            ...
            match &task.harness {
                HarnessKind::Unsupported(harness) => {
                    return Err(AgentDriverError::HarnessSetupFailed {
                        ...
                        reason: format!(
                            "The {harness} harness is only supported for current-app child agent launches."
                        ),
                    });
                }
    ...
    /// cloned first since it may not exist in the current-app filesystem. Otherwise we resolve directly
    /// against the current-app filesystem.
    ...
    /// Build the current-app agent driver options and Task.
    async fn build_driver_options_and_task(
        foreground: &ModelSpawner<Self>,
        args: RunAgentArgs,
    ) -> Result<(CurrentAppAgentDriverOptions, Task), AgentDriverError> {
        ...
        // Build the AgentConfigSnapshot, Task, and current-app driver options.
        ...
        let (task, mut driver_options) = foreground
            .spawn(move |_, ctx| -> anyhow::Result<_> {
                ...
                let driver_options = driver::CurrentAppAgentDriverOptions {
    ...
    /// Creates current-app driver task state for a new agent run.
    ///
    /// Ashide current-app runs do not create provider-owned ambient-agent task records.
    /// The driver keeps `task_id` as `None`, so downstream code naturally skips provider-owned task branches.
    async fn initialize_new_task(
        driver_options: &mut CurrentAppAgentDriverOptions,
    ) -> Result<(), AgentDriverError> {
    ...
    fn create_and_run_driver(
        ctx: &mut AppContext,
        driver_options: driver::CurrentAppAgentDriverOptions,
        output_format: OutputFormat,
        task: driver::Task,
    ) {
...
fn command_requires_auth(command: &CliCommand) -> bool {
    match command {
        ...
        CliCommand::Recall(_) | CliCommand::Remember(_) | CliCommand::Memory(_) => false,
        CliCommand::Whoami => true,
        CliCommand::SessionBridge(_) => false,
...
/// If auth is required and the current user is available, dispatches immediately.
fn launch_command(
    ctx: &mut AppContext,
    command: CliCommand,
    global_options: GlobalOptions,
    ...
    if !auth_state.is_logged_in() {
        return Err(anyhow::anyhow!(
            "No current user is available. Restart Ashide and try again."
        ));
    }
...
fn report_fatal_error(err: anyhow::Error, ctx: &mut AppContext) {
    ...
    #[cfg(not(target_family = "wasm"))]
    {
        if let Ok(path) = log_file_path() {
            let _ = write!(
                message,
                "\n\nFor more information, check Ashide logs at {}",
                path.display()
            );
        }
    }
