...
///
/// Each new external harness (e.g. Claude, Codex) implements this to be used with current-app agent runs.
...
fn task_env_vars_for_harness_name(
    ...
    // `OZ_HARNESS` is consumed by the child CLI when it emits `run message *`
    // events.
    env_vars.insert(
...
pub(crate) trait HarnessRunner: Send + Sync {
    /// Create current-app conversation state and start the harness command in the terminal.
    ///
