...
use crate::{
    ...
    terminal::{model::block::BlockId, shared_session::IsSharedSessionCreator, TerminalView},
    util::sync::Condition,
};
...
/// Options for creating the current-app terminal view before constructing a
/// [`TerminalDriver`].
///
/// This is intentionally named current-app: it opens
/// [`NewWorkspaceSource::ExplicitCurrentAppTerminalSession`]. Environment
/// Runtime terminal/agent/codex entrypoints must use the workspace runtime
/// intent path instead of this helper.
pub(crate) struct CurrentAppTerminalDriverOptions {
...
/// Create a current-app terminal window and extract the [`ViewHandle<TerminalView>`].
///
...
fn create_current_app_terminal_view(
    options: CurrentAppTerminalDriverOptions,
    ctx: &mut AppContext,
...
impl TerminalDriver {
    ...
    pub(crate) fn create(
        ...
        let terminal_view = create_current_app_terminal_view(options, ctx)?;
