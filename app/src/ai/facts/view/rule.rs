...
use crate::object_store::ids::{ClientId, ObjectStoreId};
...
impl RuleView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        // 本地 ObjectStoreEvent 已覆盖本地写入后的规则列表刷新。
    ...
    pub fn edit_ai_rule(
        ...
        let (is_autogenerated, suggested_logging_id) = ObjectStoreModel::as_ref(ctx)
            .get_object_of_type::<GenericStringObjectId, AIFactObjectModel>(&object_store_id)
            .map(|ai_fact| {
        ...
        update_manager.update(ctx, |update_manager, ctx| {
            ...
            update_manager.update_ai_fact(ai_fact, object_store_id, revision_ts, ctx);
        });
    }

    pub fn delete_ai_rule(&mut self, id: ObjectStoreId, ctx: &mut ViewContext<Self>) {
        let update_manager = UpdateManager::handle(ctx);
        update_manager.update(ctx, |update_manager, ctx| {
    ...
    fn render_global_rule_row(
        ...
        // 本地规则不依赖网络在线状态或远端对象 ID。
        // 规则永远可编辑,点击动作无条件挂载。
        hoverable
            .with_cursor(Cursor::PointingHand)
            .on_click(move |ctx, _, _| {
                ctx.dispatch_typed_action(RuleViewAction::Edit(ai_row.fact.object_store_id()));
            })
            .finish()
    }
...
impl TypedActionView for RuleView {
    ...
    fn handle_action(&mut self, action: &RuleViewAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            RuleViewAction::Edit(object_store_id) => {
                ctx.emit(RuleViewEvent::Edit(*object_store_id));
            }
