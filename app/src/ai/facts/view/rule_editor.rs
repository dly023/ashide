...
use crate::object_store::{Revision, StoredObject};
...
impl RuleEditorView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        // Ashide(本地化,Phase 2d-1):原 NetworkStatus 订阅用于在线/离线状态变化时重绘
    ...
    pub fn set_ai_rule(
        ...
        if let Some(object_store_id) = object_store_id {
...
impl View for RuleEditorView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Ashide(本地化,Phase 2d-1):原 `is_delete_allowed` 依赖网络在线+server_id,
            ...
            RuleEditorViewAction::Save => {
                ...
                if let Some(ai_fact) = &self.ai_fact {
                    ctx.emit(RuleEditorViewEvent::Edit {
                        name,
                        content,
                        object_store_id: ai_fact.object_store_id(),
