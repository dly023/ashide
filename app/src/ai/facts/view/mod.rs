// Pane 容器视图保留,负责在 Rules / RuleEditor 两页之间切换。
...
impl AIFactView {
    ...
    fn handle_rule_view_event(&mut self, event: &RuleViewEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            RuleViewEvent::Edit(object_store_id) => {
                self.update_page(
                    AIFactPage::RuleEditor {
    ...
    fn handle_rule_editor_view_event(
        ...
        match event {
            ...
            RuleEditorViewEvent::Delete { object_store_id } => {
                self.rule_view.update(ctx, |rule_view, ctx| {
                    rule_view.delete_ai_rule(*object_store_id, ctx);
                });
            }
            _ => {}
        }
    ...
    pub fn update_page(&mut self, page: AIFactPage, ctx: &mut ViewContext<Self>) {
        ...
        if let AIFactPage::RuleEditor { object_store_id } = page {
            self.rule_editor_view.update(ctx, |rule_editor_view, ctx| {
                rule_editor_view.set_ai_rule(object_store_id, ctx);
            });
        }
        self.focus(ctx);
        ctx.notify();
