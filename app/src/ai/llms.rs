...
impl DisableReason {
    ...
    pub fn tooltip_text(&self) -> &'static str {
        match self {
            DisableReason::DisabledByPolicy => "This model is disabled by local policy.",
    ...
    fn default() -> Self {
        Self {
            agent_mode: AvailableLLMs {
                ...
                choices: vec![LLMInfo {
                    display_name: "auto (balanced)".to_owned(),
                    base_model_name: "auto (balanced)".to_owned(),
...
impl LLMPreferences {
    ...
    pub fn get_base_llm_choices_for_agent_mode(&self) -> impl Iterator<Item = &LLMInfo> {
        ...
        self.models_by_feature
            ...
            .choices
            .iter()
            .filter(|llm| !matches!(llm.disable_reason, Some(DisableReason::DisabledByPolicy)))
    }
    ...
    /// Returns the preferred Codex model, if set locally.
    ...
    pub fn refresh_byop_models(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        if new != self.models_by_feature {
            self.apply_model_update(new, ctx);
        }
    }

    fn apply_model_update(&mut self, update: ModelsByFeature, ctx: &mut ModelContext<Self>) {
