...
impl FileBasedMCPManager {
    ...
    /// - For `MCPProvider::Ashide`: the logical root for `~/.ashide*/.mcp.json`.
        ...
        self.file_based_servers_by_root
            ...
            .any(|(root_path, provider_map)| {
                provider_map.iter().any(|(provider, hashes)| {
                    ...
                    match provider {
                        MCPProvider::Ashide => Self::is_global_warp_root(root_path),
    ...
    /// Ashide config (`~/.ashide/.mcp.json`). Global Ashide servers always auto-spawn.
    ...
    fn spawn_file_based_servers(
        ...
        // - Global Ashide: always auto-spawn.
        // - Global non-Ashide: auto-spawn iff the toggle is on.
    ...
    fn handle_file_based_mcp_enabled_change(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // - Global Ashide servers always spawn regardless of the toggle.
        ...
        if !AISettings::as_ref(ctx).is_file_based_mcp_enabled(ctx) {
            ...
            // Toggle on: spawn global third-party servers (global Ashide servers are
    ...
    /// exception: global Ashide installs are discovered in `~/.ashide*/`, which
    /// isn't a useful cwd for spawned processes, so they are remapped to the
    /// home directory instead.
    /// - Project-scoped installations: the repo root.
    /// - Global installations (`~/.ashide/.mcp.json`, `~/.claude.json`, etc.): the
    ///   home directory.
    ///
    ...
    pub fn spawn_root_for_installation(&self, uuid: Uuid) -> Option<PathBuf> {
        ...
        // Global Ashide installs live under `~/.ashide*/`, which is internal Ashide
        // state rather than a meaningful working directory. Map them to the
        // home dir so all global installs (Ashide and third-party) share a
        // consistent cwd.
