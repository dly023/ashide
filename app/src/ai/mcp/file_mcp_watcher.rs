...
/// `.ashide/.mcp.json`), capturing the parent directory component.
...
/// deep (e.g. `.codex/config.toml` → `.codex`, `.ashide/.mcp.json` → `.ashide`). Returns `None`
    ...
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        if let Some(home_dir) = dirs::home_dir() {
            for provider in MCPProvider::iter() {
                if provider == MCPProvider::Ashide {
                    continue;
                }
...
async fn parse_mcp_config_file(
    ...
    let json = match provider {
        ...
        MCPProvider::Claude | MCPProvider::Ashide | MCPProvider::Agents => file_contents,
    };
