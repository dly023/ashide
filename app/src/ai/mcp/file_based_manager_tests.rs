...
/// An Ashide-global installation detected from the managed `~/.ashide*/.mcp.json`
...
fn test_global_warp_server_from_managed_home_root_always_spawns() {
    ...
    let parsed = parse_mcp_json(r#"{"global-ashide": {"command": "npx", "args": ["ashide"]}}"#);

    App::test((), |mut app| async move {
        let manager = setup_app(&mut app);
        let events = subscribe_events(&mut app, &manager);

        // Toggle is off by default; the watcher-produced Ashide root should still
        // be classified as the global Ashide config and auto-spawn.
        ...
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.spawned_uuids.len(),
                1,
                "Managed Ashide MCP config should auto-spawn regardless of toggle"
            );
        });

        // Flipping the toggle must not despawn the global Ashide server.
        ...
        events.update(&mut app, |e, _| {
            assert!(
                ...
                "Managed Ashide MCP config should never be despawned by toggle changes, got: {:?}",
                e.despawned_uuids
            );
        });
    });
}
/// A globally-scoped non-Ashide installation only auto-spawns when the toggle is on.
#[test]
    ...
    App::test((), |mut app| async move {
        ...
        events.update(&mut app, |e, _| {
            assert!(
                ...
                "Global non-Ashide server must not auto-spawn while toggle is off, got: {:?}",
                e.spawned_uuids
            );
        });
        ...
        // Toggle on: the global non-Ashide server should be spawned.
        set_file_based_mcp_enabled(&mut app, true);
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.spawned_uuids,
                vec![installation_uuid],
                "Global non-Ashide server should spawn when toggle flips on"
            );
        });

        // Toggle off: the global non-Ashide server should be despawned.
        set_file_based_mcp_enabled(&mut app, false);
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.despawned_uuids,
                vec![installation_uuid],
                "Global non-Ashide server should despawn when toggle flips off"
            );
        });
    });
}

/// Project-scoped installations (both Ashide and third-party) never auto-spawn on
/// detection, and the toggle must not spawn or despawn them either.
#[test]
fn test_project_scoped_servers_never_auto_spawn() {
    let _flag_guard = FeatureFlag::FileBasedMcp.override_enabled(true);
    let repo_path = PathBuf::from("/tmp/ashide-test-repo");
    let claude_parsed =
        parse_mcp_json(r#"{"proj-claude": {"command": "npx", "args": ["proj-claude"]}}"#);
    let warp_parsed =
        parse_mcp_json(r#"{"proj-ashide": {"command": "npx", "args": ["proj-ashide"]}}"#);
    ...
    App::test((), |mut app| async move {
        ...
        manager.update(&mut app, |m, ctx| {
            ...
            m.apply_parsed_servers(repo_path.clone(), MCPProvider::Ashide, warp_parsed, ctx);
        });
...
/// is considered global (and thus gated only by the toggle for non-Ashide providers).
#[test]
fn test_server_referenced_from_both_global_and_project_is_global() {
    ...
    let repo_path = PathBuf::from("/tmp/ashide-test-repo-shared");
