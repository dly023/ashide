...
impl TemplatableMCPServerManager {
    ...
    #[cfg(not(target_family = "wasm"))]
    fn active_server_in_mcp_scope(
        &self,
        installation_uuid: Uuid,
        include_current_app_file_based: bool,
        app: &AppContext,
    ) -> bool {
        include_current_app_file_based
            || FileBasedMCPManager::as_ref(app)
                .get_hash_by_uuid(installation_uuid)
                .is_none()
    }

    /// Returns true when an active server comes from a current-app file-based MCP config.
    ///
    /// Environment Runtime sessions intentionally do not inherit current-app file-based MCP
    /// servers. The prompt context already omits them; this helper lets execution-time fallbacks
    /// reject stale tool/resource calls that were produced before switching environments.
    #[cfg(not(target_family = "wasm"))]
    pub fn active_server_is_current_app_file_based(
        &self,
        installation_uuid: Uuid,
        app: &AppContext,
    ) -> bool {
    ...
    /// Returns a reconnecting peer for a server that has the given tool, scoped by whether
    /// current-app file-based MCP servers are allowed for the active session.
    #[cfg(not(target_family = "wasm"))]
    pub fn server_with_tool_name_in_mcp_scope(
        &self,
        tool_name: &str,
        include_current_app_file_based: bool,
        app: &AppContext,
        ...
        self.active_servers
            .iter()
            .find(|(installation_uuid, server)| {
                self.active_server_in_mcp_scope(
                    **installation_uuid,
                    include_current_app_file_based,
                    app,
                ) && server.tools.iter().any(|t| t.name == tool_name)
            })
            .map(|(installation_uuid, _)| {
                super::reconnecting_peer::ReconnectingPeer::new(*installation_uuid, spawner.clone())
            })
    ...
    #[cfg(not(target_family = "wasm"))]
    pub fn server_with_installation_id(
        &self,
        installation_uuid: Uuid,
    ) -> Option<super::reconnecting_peer::ReconnectingPeer> {
        let spawner = self.spawner.as_ref()?;
    ...
    /// Returns the resource and reconnecting peer for the first matching active server in scope.
    #[cfg(not(target_family = "wasm"))]
    pub fn resource_with_server_in_mcp_scope(
        &self,
        name: &str,
        uri: Option<&str>,
        include_current_app_file_based: bool,
        ...
        rmcp::model::Resource,
        super::reconnecting_peer::ReconnectingPeer,
    )> {
        let spawner = self.spawner.as_ref()?;
        self.active_servers
            .iter()
            .find_map(|(installation_uuid, server)| {
                if !self.active_server_in_mcp_scope(
                    *installation_uuid,
                    include_current_app_file_based,
                    app,
                ) {
                ...
                server
                    ...
                    .map(|resource| {
                        (
                            resource,
                            super::reconnecting_peer::ReconnectingPeer::new(
                                *installation_uuid,
                                spawner.clone(),
                            ),
                        )
    ...
    #[cfg(not(target_family = "wasm"))]
    pub fn tool_input_schema_in_mcp_scope(
        &self,
        installation_id: Option<Uuid>,
        tool_name: &str,
        include_current_app_file_based: bool,
        ...
        let candidates: Box<dyn Iterator<Item = (Uuid, &TemplatableMCPServerInfo)>> =
            if let Some(uuid) = installation_id {
                Box::new(
                    self.active_servers
                        .get(&uuid)
                        .map(|server| (uuid, server))
                        .into_iter(),
                )
            } else {
                Box::new(
                    self.active_servers
                        .iter()
                        .map(|(uuid, server)| (*uuid, server)),
                )
            };

        candidates
            .filter(|(uuid, _)| {
                self.active_server_in_mcp_scope(*uuid, include_current_app_file_based, app)
            })
