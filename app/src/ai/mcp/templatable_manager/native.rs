...
use crate::object_store::update_manager::{InitiatedBy, UpdateManager};
use crate::object_store::{Space, StoredObject, StoredObjectMetadataExt};
...
use crate::{
    ...
    object_store::{GenericStringObjectFormat, JsonObjectType},
...
enum SpawnMode {
    /// Initial spawn - clears logs and persists running state.
    Initial {
...
/// An MCP server integration that Ashide ships with bundled skills for.
...
impl TemplatableMCPServerManager {
            ...
            ObjectStoreEvent::ObjectCreated {
                type_and_id:
                    ObjectTypeAndId::GenericStringObject {
                        object_type:
                            GenericStringObjectFormat::Json(JsonObjectType::TemplatableMCPServer),
                        id: new_object_store_id,
                    },
            } => {
                log::debug!(
                    "A new MCP server template was found with sync id {new_object_store_id}"
                );
                me.refresh_server_objects(ctx);
            }
            _ => {}
    ...
    fn spawn_server_impl(
        ...
        if let TransportType::CLIServer(cli_server) = &mut server.transport_type {
            ...
            // (repo root for project-scoped configs, ~/.ashide/ or ~ for globals). This
            ...
            // continue to inherit Ashide's process cwd.
    ...
    pub fn purge_file_based_server_credentials(
        ...
        for hash in installation_hashes {
            if let Some(old) = self.file_based_server_credentials.remove(hash) {
                removed.push((*hash, old));
            }
        }
        ...
        .is_err()
        {
            // Keychain write failed: restore every removed entry so the cache
            // matches what is actually persisted.
            for (hash, old) in removed {
            ...
            .map_err(|err| {
                if err.kind() == std::io::ErrorKind::NotFound {
                    let cwd_display = cwd_for_log
                        .as_deref()
                        .unwrap_or("<inherited from Ashide's process cwd>");
                    logger.log(format!(
