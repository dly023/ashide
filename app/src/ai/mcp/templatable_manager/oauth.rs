...
/// updates to Ashide secure storage via a channel.
///
...
/// runtime token auto-refreshes are written back to Ashide secure storage.
///
...
pub async fn make_authenticated_client(
    ...
    oauth_state
        .start_authorization(&[], &redirect_uri, Some("Ashide"))
        .await?;
    ...
    let config = match auth_manager.register_client("Ashide", &redirect_uri).await {
        Ok(config) => config,
...
impl TemplatableMCPServerManager {
    ...
    pub fn save_credentials_to_secure_storage(
        ...
        if let Some(hash) = FileBasedMCPManager::as_ref(app).get_hash_by_uuid(installation_uuid) {
            let prev = self.file_based_server_credentials.insert(hash, credentials);
            if write_to_secure_storage(
                app,
            ...
            .is_err()
            {
                // Keychain write failed: undo the in-memory insert so the cache
                // doesn't claim a credential is saved.
                match prev {
                    Some(old) => {
                        self.file_based_server_credentials.insert(hash, old);
                    }
                    None => {
        ...
        if let Some(template_uuid) = self.get_template_uuid(installation_uuid) {
            let prev = self.server_credentials.insert(template_uuid, credentials);
            if write_to_secure_storage(
                app,
            ...
            .is_err()
            {
                match prev {
                    Some(old) => {
                        self.server_credentials.insert(template_uuid, old);
                    }
                    None => {
    ...
    pub fn delete_credentials_from_secure_storage(
        ...
        if let Some(template_uuid) = self.get_template_uuid(installation_uuid) {
            let removed = self.server_credentials.remove(&template_uuid);
            if write_to_secure_storage(
                app,
                TEMPLATABLE_MCP_CREDENTIALS_KEY,
                &self.server_credentials,
            ...
            .is_err()
            {
                // Keychain write failed: restore the removed entry so the cache
...
///
/// RRA5-05: returns the error instead of swallowing it with `.ok()`. Callers
/// roll back their in-memory cache on failure so it never claims a credential
/// is persisted when the keychain write actually failed (which would make
/// `has_oauth_credentials_*` lie and silently break the next token use).
pub(crate) fn write_to_secure_storage<T: Serialize>(
    ...
    let json = serde_json::to_string(credentials).map_err(|err| {
        ...
        anyhow::anyhow!("serialize MCP credentials: {err}")
    })?;
    app.secure_storage().write_value(key, &json).map_err(|err| {
        log::error!("Failed to write MCP credentials to secure storage: {err:#}");
        anyhow::anyhow!("write MCP credentials to secure storage: {err}")
    })?;
    Ok(())
}
