...
pub(crate) fn convert_input_context(context: Option<&api::InputContext>) -> Arc<[AIAgentContext]> {
    ...
    if let (Some(os), Some(shell)) = (&context.operating_system, &context.shell) {
        result.push(AIAgentContext::ExecutionEnvironment(AiExecutionContext {
            os: AiOsContext {
    ...
    fn into_exchanges(self) -> Vec<AIAgentExchange> {
        ...
        for api_message in self.messages.iter() {
            ...
            let added_message_as_exchange_input = match message {
                ...
                // persisted lifecycle timestamps survive roundtrip.
...
pub(crate) fn convert_tool_call_result_to_input(
    ...
    match tool_call_result.result.as_ref() {
        ...
        Some(ToolCallResultType::UploadFileArtifact(_)) => {
            // Upload artifact 已物理切除,对应 result 直接丢弃。
            let _ = tool_call_id;
            let _ = task_id;
            let _ = context;
            None
            ...
            let edit_result = match &result.result {
                Some(api::apply_file_diffs_result::Result::Success(success)) => {
                    ...
                    RequestFileEditsResult::Success {
                        diff: "".to_string(), // Field is not populated by current conversion output
                        updated_files,
                        deleted_files: success
                            ...
                            .collect(),
                        // Line counts are not available in persisted imported data
                        lines_added: 0,
                        lines_removed: 0,
                    }
                }
        ...
        | Some(ToolCallResultType::RequestComputerUseResult(_)) => {
            // Computer Use 已被移除,遇到这两类 result 直接忽略。
            None
        }
