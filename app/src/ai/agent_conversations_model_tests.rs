...
fn test_get_tasks_and_conversations_prefers_task_when_task_id_matches_conversation_run_id() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            ...
            assert_eq!(
                model.conversation_id_for_task_id(&task.task_id, ctx),
...
///
/// `harness` semantics:
/// - `None`            → leaves `agent_config_snapshot = None` (task without config snapshot).
...
fn test_harness_filter_matches_only_selected_harness() {
    App::test((), |mut app| async move {
        ...
        // Snapshot present but no harness set → Some(Oz), matches the built-in Ashide agent.
        ...
        // Local conversation: effectively the built-in Ashide agent.
        ...
        app.update(|ctx| {
            ...
            // All → everything, including the unknown-harness task.
            ...
            // Ashide Agent / Oz → default-snapshot task and local conversation.
            // The task with no snapshot resolves to `harness() == None` and
            // is deliberately excluded from any specific-harness filter.
            let oz_items = items_for(HarnessFilter::Specific(Harness::Oz));
            assert_eq!(
                oz_items.len(),
                2,
                "expected 2 Ashide Agent matches, got {oz_items:?}"
            );
            ...
            assert!(
                ...
                "task with no snapshot should not match the Ashide Agent filter"
            );
        });
    });
}
...
fn test_agent_management_filters_serde_defaults_missing_harness_to_all() {
    ...
    // Unknown harness strings deserialize to All.
    let forward = r#"{
        "owners": "PersonalOnly",
        "status": "All",
        "source": "All",
