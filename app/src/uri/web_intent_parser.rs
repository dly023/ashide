...
/// Attempts to rewrite an Ashide web URL into a native desktop intent URL.
/// Returns `None` if the URL is not a recognized Ashide web intent.
...
/// On the WASM build, fires an event to try and open the given link on the desktop app.
#[cfg(target_family = "wasm")]
pub fn open_url_on_desktop(url: &Url) {
    match WebIntent::try_from_url(url) {
        Ok(WebIntent::ConversationView(intent)) | Ok(WebIntent::Action(intent)) => {
