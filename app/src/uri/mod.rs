...
use crate::util::openable_file_type::{
    is_file_openable_in_ashide, is_markdown_file, is_runnable_shell_script, starts_with_shebang,
};
...
pub enum UriHost {
    ...
    Conversation,
    /// Supports opening Ashide settings via URI
    Settings,
...
impl UriHost {
    fn handle(&self, primary_window_id: Option<WindowId>, url: &Url, ctx: &mut AppContext) {
        ...
        match self {
            UriHost::Auth => {
                safe_info!(
                    safe: ("Ignored auth url because Ashide has no hosted login flow"),
                    full: ("Ignored auth url {url} because Ashide has no hosted login flow")
                );
            }
            UriHost::Action => {
            ...
            UriHost::Settings => {
                ...
                if let Some(settings_sub_page) = settings_sub_page {
                    match settings_sub_page.as_str() {
                        "environments" => {
                            // The old settings/environments URI route is gone; keep only the
                            // GitHub auth completion notification for independent listeners.
                        ...
                        "platform" => {
                            log::warn!("settings/platform URI route is disabled in Ashide; ignoring request");
                        }
                        "appearance" => {
                            dispatch_action_in_new_or_existing_window(
                                primary_window_id,
    ...
    fn window_behavior_hint(&self) -> WindowBehaviorHint {
        ...
        match self {
            ...
            Self::NewTab => W::ShowPrimaryWindow(WindowActivationFallbackBehavior::Notify {
                ...
                description: "Go to Ashide to see your new tab.".to_owned(),
            }),
...
enum OpenFileAction {
    ...
    Notebook,
    /// Open in Ashide's code/text editor pane.
    Editor,
...
fn classify_open_file_action(path: &Path) -> OpenFileAction {
    ...
    if path.is_file() {
        ...
        // shebang scripts that `is_file_openable_in_ashide` rejects on extension alone
        ...
        if is_file_openable_in_ashide(path).is_some() || starts_with_shebang(path) {
    ...
    if action == OpenFileAction::Notebook {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            use crate::util::{
                ...
                openable_file_type::resolve_file_target_to_open_in_ashide,
            };

            // Open text/code files in Ashide's code editor, respecting the user's layout preference.
            let editor_settings = EditorSettings::as_ref(ctx);
            let target = resolve_file_target_to_open_in_ashide(&path, editor_settings, None);

            let window_id = if let Some((wid, _)) = primary_window_and_view {
                wid
            } else {
                open_new_with_workspace_source(NewWorkspaceSource::DefaultEnvironmentTerminal, ctx)
                    .0
            };
