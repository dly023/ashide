...
mod env_vars;
mod environment_authority;
#[allow(dead_code)]
#[path = "remote_server/mod.rs"]
mod environment_runtime_transport;
mod experiments;
...
// Ashide(本地化,Phase 4):`ServerVoiceTranscriber` 原用于默认 VoiceTranscriber 注入,现走 `VoiceTranscriber::disabled()`,同名 import 暂收。
#[cfg(feature = "local_fs")]
...
// Re-export the safe logging macros at the crate root level for internal callers.
...
/// Launch mode for how to start up Ashide.
...
pub enum LaunchMode {
    ...
    /// Run the Ashide command-line SDK.
    CommandLine {
...
impl LaunchMode {
    fn args(&self) -> Cow<'_, warp_cli::AppArgs> {
        match self {
            ...
            | LaunchMode::Test { .. }
            | LaunchMode::EnvironmentRuntimeProxy
            | LaunchMode::EnvironmentRuntimeDaemon => Cow::Owned(warp_cli::AppArgs::default()),
        }
    }
    ...
    fn is_integration_test(&self) -> bool {
        match self {
            ...
            | LaunchMode::CommandLine { .. }
            | LaunchMode::EnvironmentRuntimeProxy
            | LaunchMode::EnvironmentRuntimeDaemon => false,
        }
    }

    /// Returns `true` only for the regular desktop GUI process that completed
    /// singleton ownership arbitration before shared application startup.
    fn owns_desktop_gui_singleton(&self) -> bool {
        match self {
            LaunchMode::App { .. } => true,
            LaunchMode::CommandLine { .. }
            | LaunchMode::Test { .. }
    ...
    fn take_test_driver(&mut self) -> Option<TestDriver> {
        match self {
            ...
            | LaunchMode::CommandLine { .. }
            | LaunchMode::EnvironmentRuntimeProxy
            | LaunchMode::EnvironmentRuntimeDaemon => None,
        }
    }
    ...
    fn execution_mode(&self) -> ExecutionMode {
        match self {
            ...
            // EnvironmentRuntimeProxy and EnvironmentRuntimeDaemon don't use execution
            // mode, but Sdk is the closest match (headless, no GUI).
            LaunchMode::EnvironmentRuntimeProxy | LaunchMode::EnvironmentRuntimeDaemon => {
                ExecutionMode::Sdk
            }
        }
    }
    ...
    /// Returns `true` if Ashide should run headlessly, without a visible UI.
    fn is_headless(&self) -> bool {
        match self {
            ...
            LaunchMode::EnvironmentRuntimeProxy | LaunchMode::EnvironmentRuntimeDaemon => true,
    ...
    fn log_destination(&self) -> Option<LogDestination> {
        match self {
            ...
            LaunchMode::EnvironmentRuntimeProxy => Some(LogDestination::Stderr),
            LaunchMode::EnvironmentRuntimeDaemon => Some(LogDestination::File),
...
pub fn run() -> Result<()> {
    ...
    if let Some(command) = args.command() {
        ...
        match command {
            ...
            #[cfg(not(target_family = "wasm"))]
            warp_cli::Command::Worker(warp_cli::WorkerCommand::EnvironmentRuntimeProxy(args)) => {
                init_common(&LaunchMode::EnvironmentRuntimeProxy, None)?;
                return workspace::environment_runtime::run_worker_proxy(args.identity_key.clone());
            }
            #[cfg(not(target_family = "wasm"))]
            warp_cli::Command::Worker(warp_cli::WorkerCommand::EnvironmentRuntimeDaemon(args)) => {
                init_common(&LaunchMode::EnvironmentRuntimeDaemon, None)?;
                return workspace::environment_runtime::run_worker_daemon(
            ...
            warp_cli::Command::CommandLine(cmd) => {
                if matches!(
                    cmd.as_ref(),
                    warp_cli::CliCommand::Memory(warp_cli::memory::MemoryCommand::McpServer)
                ) {
                    init_common(
                        &LaunchMode::CommandLine {
                    ...
                    return crate::agent_memory::mcp_server::run_stdio();
                }
    ...
    let is_cli_binary = cfg!(feature = "standalone")
        ...
        || std::env::var_os("ASHIDE_CLI_MODE").is_some();
    if is_cli_binary {
...
pub fn run_integration_test(driver: TestDriver) -> Result<()> {
    // 集成测试绕过 `run()`,因此必须在构造任何 UI 前显式初始化固定语言。
    // 使用英文让文本断言与 fixture 保持确定性,同时覆盖真实启动的 i18n 初始化合同。
    i18n::init(Some("en"));
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    if launch_mode.owns_desktop_gui_singleton() {
        match app_services::arbitrate_desktop_gui_singleton(launch_mode.args().as_ref())? {
            app_services::DesktopSingletonArbitration::ContinueAsOwner => {}
            app_services::DesktopSingletonArbitration::ForwardedToOwner => std::process::exit(0),
        }
    }
    ...
    // Sets up a Job Object that we associate with the Ashide process to handle
    ...
    // 公共设置始终存放在 TOML 设置文件中,这里用它做 app 启动前的读取。
    #[cfg_attr(
    ...
    let mut app_builder = if launch_mode.is_headless() {
        warpui::platform::AppBuilder::new_headless(
            app_callbacks(
                ...
                launch_mode.owns_desktop_gui_singleton(),
            ),
            Box::new(ASSETS),
...
fn initialize_app(
    ...
    // Ashide:保留 AuthStateProvider singleton 仅用于遗留调用点读取本地占位用户态。
    ctx.add_singleton_model(|_ctx| AuthStateProvider::new(auth_state.clone()));

    // Ashide Wave 3-1:AuthManager 已本地化为 stub,不再注入 server_api / auth_client。
    ...
    // Rewrite recognized Ashide web URLs (sessions, Drive, settings, home) into local
    ...
    if user_is_logged_in {
        // Ashide 本地 auth facade 在 `AuthState::initialize` 时已把身份快照装载完毕。
    ...
    #[cfg(feature = "local_fs")]
    {
        ...
        ctx.add_singleton_model(|ctx| {
            ...
            workspace::environment_runtime::subscribe_to_repo_metadata_updates(ctx);
            model
        });
    }
    ...
    ai::init(ctx);
    if launch_mode.owns_desktop_gui_singleton() {
        app_services::init_desktop_gui_owner(ctx);
    }
    #[cfg(not(target_family = "wasm"))]
    ...
    // Ashide Wave 7-2:`settings_view::update_environment_form::init` 随 cloud ambient agent
    // 主体子系统物理删。
    ...
    // 注册 CLI agent 安装状态 singleton model(后台异步扫描 PATH,完成后 emit 事件驱动 UI)
    ...
    ctx.add_singleton_model(crate::ssh_manager::SshTargetCatalog::new);
    ...
    #[cfg(feature = "local_fs")]
    {
        let file_model = ctx.add_singleton_model(FileModel::new);
        file_model.update(ctx, |file_model, _ctx| {
            file_model.set_environment_file_runtime(
                workspace::environment_runtime::environment_file_runtime(),
            );
        });
    }
    ctx.add_singleton_model(|ctx| {
        let model = GlobalBufferModel::new(ctx);
        // 客户端 app:订阅 Environment Runtime 的 buffer push 事件。daemon 不做
        // 这一步(daemon 不注册 runtime transport manager),所以这段不能放进
        // GlobalBufferModel::new。runtime transport manager 已在前面注册过。
        #[cfg(feature = "local_tty")]
        if workspace::environment_runtime::environment_runtime_feature_enabled() {
            GlobalBufferModel::subscribe_to_environment_runtime_buffer_updates(ctx);
        }
        model
    });
    #[cfg(windows)]
    ...
    ctx.add_singleton_model(|_| {
        // Ashide(本地化,Phase 4):原默认注入 `ServerVoiceTranscriber` 走云端 Wispr STT。
    ...
    // 本地 object-store 写入后即完成,不再维护远端 pending-change 队列。
    ...
    timer.mark_interval_end("OBJECT_STORE_MODEL_INITIALIZED");
    ...
    // Ashide:UpdateManager 只负责本地对象的内存/SQLite 同步,不再注入云端 client。
    ...
    // Ashide(本地化,Phase 5):`PreferencesSyncer` 已物理删除。原同步器负责本地
    ...
    // TemplatableMCPServerManager must be registered after UpdateManager and FileBasedMCPManager.
    ctx.add_singleton_model(|ctx| {
    ...
    // SkillManager is used to cache SKILL.md files for all active current-app terminal views and their working directories.
    ctx.add_singleton_model(SkillManager::new);
    ctx.add_singleton_model(EnvironmentSkillInventoryCache::new);
    ...
    // 本地构建仅注册本地 ObjectStoreModel(无云端 listener)。
...
fn app_callbacks(
    is_integration_test: bool,
    owns_desktop_gui_singleton: bool,
    ...
    warpui::platform::AppCallbacks {
        ...
        on_will_terminate: Some(Box::new(move |ctx| {
            ...
            // autoupdate, to ensure we're not running any extra Ashide processes
            ...
            if owns_desktop_gui_singleton {
                app_services::teardown_desktop_gui_owner(ctx);
            }
...
fn launch(ctx: &mut warpui::AppContext, app_state: Option<AppState>, launch_mode: LaunchMode) {
    ...
    match &launch_mode {
        ...
        // EnvironmentRuntimeProxy and EnvironmentRuntimeDaemon never go through
...
pub fn enabled_features() -> HashSet<FeatureFlag> {
    ...
    workspace::environment_runtime::install_debug_runtime_feature_flags(&mut flags);

    // Issue #72: HTTP 代理设置页面。不走 channel 判断,所有 channel 含 ashide
    // 默认启用,作为企业 VPN / 公司代理场景的基本能力。
    flags.insert(FeatureFlag::HttpProxySettings);

    // Ashide UI model: horizontal bar is the Environment Strip; vertical tabs
    // are the default Session Navigator for sessions/panes inside an environment.
    flags.insert(FeatureFlag::VerticalTabs);
    ...
    let extra_flags: &[FeatureFlag] = &[
        ...
        // Ashide Wave 7-2:`CloudEnvironments` FeatureFlag 随 cloud ambient agent 主体子系统
        ...
        #[cfg(feature = "save_ambient_plans_to_local_drive")]
        FeatureFlag::SaveAmbientPlansToLocalDrive,
        ...
        // Ashide(本地化):ambient agent / agent management view 的云端入口已物理下线。
...
/// `ASHIDE_UNSTABLE_FEATURES` 接受的不稳定功能名 -> FeatureFlag 映射。
/// 这里登记的功能在 release 构建下默认隐藏,设置对应 token 后才会出现;
/// dev 构建走 debug_assertions 分支默认启用,无需该变量。
/// 注:`server_file_browser` 已在 `RELEASE_FLAGS` 默认开放,不再登记于此。
