//! Voltron (https://en.wikipedia.org/wiki/Voltron) in Ashide is a code name for the UI element used
...
pub struct VoltronMetadata {
    ...
    pub active_session_current_app_path: Option<PathBuf>,
...
impl Voltron {
    ...
    pub fn refresh_active_session_current_app_path(
        &mut self,
        path: Option<PathBuf>,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.metadata.active_session_current_app_path == path {
            return;
        }
        self.metadata.active_session_current_app_path = path.clone();
            ...
            match current_feature.feature_view_handle {
                VoltronFeatureViewHandle::Workflows(view_handle) => {
                    view_handle.update(ctx, |view, ctx| {
                        view.on_active_session_current_app_path_changed(path, ctx);
                    });
                }
            }
        }
