...
pub struct EditorModal {
    ...
    /// Dropdown to select the separator for the Ashide prompt, in the case of
    /// same line prompt. This separator is added at the end of the Ashide prompt.
    warp_prompt_separator_dropdown: ViewHandle<Dropdown<EditorModalAction>>,
    /// The separator currently selected for the Ashide prompt.
...
impl EditorModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let warp_prompt_separator = match SessionSettings::as_ref(ctx).saved_prompt.value() {
            ...
            // If the "default Ashide prompt" i.e. no context chips, is selected, then default to no Ashide prompt separator.
    ...
    /// Updates the state of the Ashide prompt separator dropdown to be enabled/disabled based on the current state of the modal.
    fn update_warp_separator_dropdown_state(&mut self, ctx: &mut ViewContext<Self>) {
        // If we are using the Ashide prompt and SLP is enabled, then we enable the dropdown. Otherwise, disable it.
    ...
    fn save_prompt_to_settings(&mut self, ctx: &mut ViewContext<Self>) {
        if self.is_dirty {
            match self.prompt_type {
                PromptType::PS1 => {
                    // TODO: we need to stop the Ashide prompt generators from running at this point
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            Self::Action::Chip(chip_action) => {
                ...
                if mutated {
                    self.is_dirty = true;
                    self.prompt_type = PromptType::Ashide;
                }
                ctx.notify();
            }
            Self::Action::UsePS1 => {
                ...
                // Disable the Ashide separator dropdown (only applies to Ashide prompt).
            ...
            Self::Action::UseWarpPrompt => {
                ...
                // Enable the Ashide separator dropdown, if SLP is on.
                self.update_warp_separator_dropdown_state(ctx);
                ctx.notify();
            }
            Self::Action::ResetWarpPrompt => {
                ...
                // Disable the Ashide separator dropdown, since SLP is off for the default Ashide prompt.
                self.update_warp_separator_dropdown_state(ctx);
            ...
            Self::Action::ToggleSameLinePrompt => {
                ...
                // In case we had previously picked default Ashide prompt, but now the user toggled
...
impl EditorModal {
    ...
    // TODO: consider supporting SLP with the new Ashide prompt.
    #[allow(dead_code)]
    ...
    fn render_warp_prompt_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        self.render_prompt_section(
            appearance,
            matches!(self.prompt_type, PromptType::Ashide | PromptType::WarpDefault),
            header_row,
            None,
            body,
            self.mouse_state_handles
    ...
    fn render_buttons(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        // - the Ashide prompt is used but there are no chips selected
        let save_disabled = !self.is_dirty
            || (matches!(self.prompt_type, PromptType::Ashide)
