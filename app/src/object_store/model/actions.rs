...
use crate::{
    object_store::ids::{parse_sqlite_id_to_uid, HashedSqliteId, ObjectUid},
...
/// We track object actions through this type. A single ObjectAction represents an
/// object_id/action pair and a subtype that contains data about the action(s).
/// Each ObjectAction either represents one action or a summary of identical
/// actions that occurred at different times. We summarize old actions to keep
/// the local in-memory footprint bounded.
#[derive(Clone, Debug, PartialEq)]
...
impl TryFrom<PersistedObjectAction> for ObjectAction {
    ...
    fn try_from(other: PersistedObjectAction) -> Result<Self, Self::Error> {
        ...
        let action_subtype = if let Some(count) = other.count {
            ...
            // to the latest processed_at_timestamp in the bundle. Bundled actions
            // are already persisted snapshots, so this is a value, not an option.
            ...
            // The processed_at_timestamp is still None when the action is still pending locally.
        ...
        // The object_object_store_id stored in SQLite is the historical hashed id that's
        // used to index into the ObjectActions model.
...
pub enum ObjectActionSubtype {
    SingleAction {
        ...
        // When the action was durably processed (used to order actions against each other).
        // None if the action is still pending locally.
        ...
        // Whether or not this action has been successfully persisted locally.
        pending: bool,
    },
    BundledActions {
...
/// A singleton model representing actions that have occurred on a per-object
/// basis. These represent local actions taken by the user. The actions have a
/// pending status that is true until the local persistence path has processed
/// them.
