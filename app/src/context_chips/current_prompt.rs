...
pub struct CurrentPrompt {
    ...
    /// The separator to use as a trailing character at the end of Ashide prompt, if any.
...
impl CurrentPrompt {
    ...
    /// Whether same line prompt is enabled for the Ashide prompt.
    pub fn same_line_prompt_enabled(&self) -> bool {
        self.same_line_prompt_enabled
    }

    /// The separator for the current Ashide prompt.
    ...
    fn build_chip_fingerprint(
        ...
        for input in inputs {
            ...
            match input {
                ...
                ChipFingerprintInput::SessionUsesCurrentAppEnvironment => {
                    context
                        .active_session
                        .map(Session::uses_current_app_environment)
                        .hash(&mut hasher);
                }
