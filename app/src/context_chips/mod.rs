...
///
/// Used as a fallback when `GitRepoStatusModel` is unavailable (e.g. Environment Runtime sessions,
/// current-app subshells).
    ...
    chips.iter().find_map(|chip| {
        if matches!(chip.kind(), ContextChipKind::GitDiffStats) {
            chip.value().map(|value| match value {
                ...
                // Raw shell command output (Environment Runtime sessions) — parse.
