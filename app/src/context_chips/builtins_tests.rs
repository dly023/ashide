...
#[test]
fn test_environment_runtime_sessions() {
    let current_app_environment_session = Session::test();
    let environment_runtime_session = Session::new(
    ...
    let local_ctx = GeneratorContext {
        active_block_metadata: &BlockMetadata::new(
            Some(current_app_environment_session.id()),
            None,
        ),
        active_session: Some(&current_app_environment_session),
        current_environment: &Default::default(),
    };

    let remote_ctx = GeneratorContext {
        active_block_metadata: &BlockMetadata::new(Some(environment_runtime_session.id()), None),
        active_session: Some(&environment_runtime_session),
        current_environment: &Default::default(),
    };
    ...
    // The SSH chip is only shown for Environment Runtime sessions.
