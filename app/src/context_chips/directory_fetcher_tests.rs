...
fn context_chip_declares_and_tests_its_hidden_categories() {
    ...
    assert!(policy.allows_path(std::path::Path::new("visible.rs"), false, false));
    assert!(!policy.allows_path(std::path::Path::new(".env"), false, false));
    assert!(!policy.allows_path(std::path::Path::new("secret"), true, false));
    assert!(!policy.allows_path(std::path::Path::new("target"), false, true));
    assert!(!policy.allows_path(std::path::Path::new(".git/HEAD"), false, false));
}
