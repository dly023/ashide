...
impl DirectoryFetcher {
    ...
    pub fn refetch_directory(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // Always use async method - SessionContext works for both current-app and Environment Runtime sessions
    ...
    async fn fetch_files_async(
        ...
        // Use SessionContext to get directory entries (works for both current-app and Environment Runtime sessions)
        let entries = session_context.list_directory_entries(typed_path).await;

        // Context chip 显式选择共享策略，不再拥有第二套 hard filter。
        let mut items: Vec<DirectoryItem> = entries
            .iter()
            .filter(|entry| context_chip_allows_entry(entry))
...
fn context_chip_allows_entry(entry: &EngineDirEntry) -> bool {
    HiddenEntryPolicy::ContextChip.allows_path(
