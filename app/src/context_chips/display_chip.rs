...
impl DisplayChip {
    ...
    fn new_internal(
        ...
        let quota_reset_popup = ctx.add_typed_action_view(|_| {
            FeaturePopup::alert_icon(NewFeaturePopupLabel::FromString(
                "Monthly AI usage reset!".to_string(),
            ))
        });
    ...
    fn git_diff_stats_chip(
        ...
        let uses_current_app_environment_session = self
            .session_context
            .as_ref()
            .map(|ctx| ctx.session.uses_current_app_environment())
            .unwrap_or(true);

        let diff_stats_display = if uses_current_app_environment_session {
