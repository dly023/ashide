...
pub struct ChipRuntimeCapabilities {
    ...
    pub session_uses_current_app_environment: Option<bool>,
...
impl ChipRuntimeCapabilities {
    ...
    pub fn from_session_with_external_command_queries<'a>(
        ...
        Self {
            ...
            session_uses_current_app_environment: Some(session.uses_current_app_environment()),
            external_commands,
        }
    }
}
...
impl ChipDisabledReason {
    pub fn tooltip_text(&self) -> String {
        match self {
            Self::RequiresCurrentAppEnvironment => {
                "Requires the current app environment".to_string()
            }
...
pub enum ChipFingerprintInput {
    SessionId,
    SessionUsesCurrentAppEnvironment,
    WorkingDirectory,
    GitBranch,
    PythonVirtualenv,
    CondaEnvironment,
...
/// executables it requires, whether it's restricted to the current app environment, its shell command
/// timeout, and which inputs form its cache fingerprint.
#[derive(Clone, Debug, Default)]
pub struct ChipRuntimePolicy {
    required_executables: Vec<String>,
    current_app_environment_only: bool,
...
impl ChipRuntimePolicy {
    ...
    pub fn availability(&self, capabilities: &ChipRuntimeCapabilities) -> ChipAvailability {
        if self.current_app_environment_only
            && matches!(
                capabilities.session_uses_current_app_environment,
                Some(false)
            )
        {
            return ChipAvailability::Disabled(ChipDisabledReason::RequiresCurrentAppEnvironment);
        }
