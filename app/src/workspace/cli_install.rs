...
fn create_symlink_with_admin(source: &Path, target: &Path) -> Result<()> {
    ...
    let script = format!(
        "do shell script \"ln -sf {escaped_source} {escaped_target}\" with prompt \"Ashide needs administrator privileges to install the command in /usr/local/bin.\" with administrator privileges"
    );
...
fn remove_file_with_admin(target: &Path) -> Result<()> {
    ...
    let script = format!(
        "do shell script \"rm {escaped_target}\" with prompt \"Ashide needs administrator privileges to uninstall the command from /usr/local/bin.\" with administrator privileges"
    );
...
///
/// This function:
/// 1. Detects the current Ashide channel and finds the appropriate binary
