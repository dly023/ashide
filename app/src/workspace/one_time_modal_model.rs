...
pub struct OneTimeModalModel {
    /// Whether the Ashide launch modal is currently being shown.
    is_ashide_launch_modal_open: bool,
...
impl OneTimeModalModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
            ...
            if is_existing_user {
                ...
                GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                    if let Err(e) = settings
                        .did_check_to_trigger_ashide_launch_modal
                        .set_value(true, ctx)
                    {
                        log::warn!("Failed to mark Ashide launch modal as dismissed: {e}");
                    }
                });
            }
        });
        ...
        Self {
            is_ashide_launch_modal_open: false,
    ...
    /// Returns whether the Ashide launch modal is currently open.
    pub fn is_ashide_launch_modal_open(&self) -> bool {
        self.is_ashide_launch_modal_open && self.target_window_id.is_some()
    }

    pub fn mark_ashide_launch_modal_dismissed(&mut self, ctx: &mut ModelContext<Self>) {
        self.set_ashide_launch_modal_open(false, ctx);
    }
    ...
    pub fn is_any_modal_open(&self) -> bool {
        (self.is_ashide_launch_modal_open || self.is_hoa_onboarding_open)
            && self.target_window_id.is_some()
    }

    #[cfg(debug_assertions)]
    pub fn force_open_ashide_launch_modal(&mut self, ctx: &mut ModelContext<Self>) {
        self.set_ashide_launch_modal_open(true, ctx);
    }
    ...
    fn set_ashide_launch_modal_open(&mut self, is_open: bool, ctx: &mut ModelContext<Self>) -> bool {
        if self.is_ashide_launch_modal_open != is_open {
            self.is_ashide_launch_modal_open = is_open;
    ...
    fn check_and_trigger_all_modals(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        if self.check_and_trigger_ashide_launch_modal(ctx) {
            return;
        }
    ...
    fn check_and_trigger_ashide_launch_modal(&mut self, ctx: &mut ModelContext<Self>) -> bool {
        // Only show if the feature flag is enabled.
        if !FeatureFlag::AshideLaunchModal.is_enabled() {
            return false;
        }

        let general_settings = GeneralSettings::as_ref(ctx);
        let ashide_modal_shown = *general_settings
            .did_check_to_trigger_ashide_launch_modal
            .value();

        if ashide_modal_shown {
            return false;
        }
        ...
        let should_show_ashide_modal = !matches!(ChannelState::channel(), Channel::Integration);
        self.set_ashide_launch_modal_open(should_show_ashide_modal, ctx);
        should_show_ashide_modal
    }
}
