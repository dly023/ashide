...
fn initialize_app(app: &mut App) {
    ...
    // don't depend on app-level persistence bootstrap.
...
fn test_ashide_session_bridge_fork_opens_focused_conversation_tab() {
    ...
    App::test((), |mut app| async move {
        ...
        workspace.update(&mut app, |workspace, ctx| {
            ...
            let expected_session_id = Workspace::ashide_conversation_session_id(target_conversation_id);
            let expected_conversation_id = target_conversation_id.to_string();
            assert!(workspace.session_navigator_sessions(ctx).iter().any(|session| {
...
fn test_pending_environment_runtime_agent_view_entry() -> PendingEnvironmentRuntimeAgentViewEntry {
    PendingEnvironmentRuntimeAgentViewEntry {
...
) -> PendingEnvironmentRuntimeForkedConversationEntry {
    PendingEnvironmentRuntimeForkedConversationEntry {
