...
mod session_bridge_edit_dialog;
pub mod sync_inputs;
...
use crate::modal;
use crate::notebooks;
use crate::pane_group::{Direction, TabBarHoverIndex};
...
pub use action::{
    CommandSearchOptions, InitContent, RestoreConversationLayout, SessionBridgeForkTarget,
...
pub use util::{
    active_terminal_in_window, PaneViewLocator, TabMovement, TerminalSessionFallbackBehavior,
};
pub use view::{
...
pub(crate) fn active_window_environment_allows_current_app_skill_manager(app: &AppContext) -> bool {
    ...
    WorkspaceRegistry::as_ref(app)
        ...
        .map(|workspace| {
            workspace
                .as_ref(app)
                .active_environment_allows_current_app_skill_manager(app)
        })
        .unwrap_or(true)
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct SkillManagerEnvironmentRuntimeScope {
    pub(crate) session_id: Option<warp_core::SessionId>,
    pub(crate) host_id: Option<warp_core::HostId>,
    pub(crate) current_working_directory: Option<String>,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum SkillManagerEnvironmentScope {
    CurrentApp,
    EnvironmentRuntime(SkillManagerEnvironmentRuntimeScope),
}

impl SkillManagerEnvironmentScope {
    pub(crate) fn environment_runtime_scope(&self) -> Option<&SkillManagerEnvironmentRuntimeScope> {
        match self {
            Self::CurrentApp => None,
            Self::EnvironmentRuntime(scope) => Some(scope),
        }
    }
}

/// Resolves a connected environment-runtime `HostId` for an SSH config
/// `node_id` within a specific window, used by the SFTP file browser to decide
/// whether file ops can ride the daemon's shared ControlMaster connection.
///
...
pub(crate) fn connected_environment_runtime_target_for_node(
    window_id: warpui::WindowId,
    node_id: &str,
    app: &AppContext,
) -> Option<environment_runtime::EnvironmentRuntimeTarget> {
    WorkspaceRegistry::as_ref(app)
        .get(window_id, app)?
        .as_ref(app)
...
pub(crate) fn active_window_skill_manager_environment_scope(
    app: &AppContext,
    ...
    WorkspaceRegistry::as_ref(app)
        ...
        .map(|workspace| {
            workspace
                .as_ref(app)
                .active_skill_manager_environment_scope(window_id, app)
        })
        .unwrap_or(SkillManagerEnvironmentScope::CurrentApp)
}

pub(crate) fn reconnect_active_window_skill_manager_environment_if_stale(app: &mut AppContext) {
    let Some(window_id) = app.windows().active_window() else {
        return;
    };
    ...
    workspace.update(app, |workspace, ctx| {
        workspace.reconnect_active_skill_manager_environment_if_stale(window_id, ctx);
    });
}
...
use crate::workspace::view::{
    ...
    LEFT_PANEL_SKILL_MANAGER_BINDING_NAME, NEW_AGENT_TAB_BINDING_NAME, NEW_TAB_BINDING_NAME,
    NEW_TERMINAL_TAB_BINDING_NAME, OPEN_GLOBAL_SEARCH_BINDING_NAME,
    TOGGLE_LOCAL_DRIVE_BINDING_NAME, TOGGLE_NOTIFICATION_MAILBOX_BINDING_NAME,
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_editable_bindings([
        EditableBinding::new(
            ...
            WorkspaceAction::AddTerminalPane(Direction::Left),
        )
        ...
        EditableBinding::new(
            ...
            WorkspaceAction::AddTerminalPane(Direction::Up),
        )
        .with_context_predicate(id!("Workspace") & !id!("Workspace_PaneDragging"))
        ...
        EditableBinding::new(
            ...
            WorkspaceAction::AddTerminalPane(Direction::Down),
        )
        .with_context_predicate(id!("Workspace") & !id!("Workspace_PaneDragging"))
    ...
    if ChannelState::enable_debug_features() {
        ...
        #[cfg(debug_assertions)]
        {
            app.register_editable_bindings([
                ...
                .with_context_predicate(id!("Workspace")),
                EditableBinding::new(
                    "workspace:open_ashide_launch_modal",
                    crate::t!("keybinding-desc-workspace-open-ashide-launch-modal"),
                    WorkspaceAction::OpenAshideLaunchModal,
                )
                .with_context_predicate(id!("Workspace")),
                EditableBinding::new(
                    "workspace:reset_ashide_launch_modal_state",
                    crate::t!("keybinding-desc-workspace-reset-ashide-launch-modal-state"),
                    WorkspaceAction::ResetAshideLaunchModalState,
                )
                .with_context_predicate(id!("Workspace")),
                EditableBinding::new(
                ...
                .with_context_predicate(id!("Workspace")),
                EditableBinding::new(
                    "workspace:use_dev_checkout_opencode_warp_plugin",
                    crate::t!("keybinding-desc-workspace-use-dev-checkout-opencode-warp-plugin"),
                    WorkspaceAction::UseDevCheckoutOpenCodeWarpPlugin,
                )
                .with_context_predicate(id!("Workspace")),
                EditableBinding::new(
    ...
    app.register_editable_bindings([
        ...
        .with_context_predicate(id!("Workspace") & id!(flags::ENABLE_LOCAL_DRIVE)),
        EditableBinding::new(
        ...
        .with_context_predicate(
            id!("Workspace") & id!(flags::ENABLE_LOCAL_DRIVE) & id!("IsOnline"),
        ),
        EditableBinding::new(
        ...
        .with_context_predicate(id!("Workspace"))
        .with_custom_action(CustomAction::ToggleLocalDrive),
        EditableBinding::new(
        ...
        EditableBinding::new(
            ...
            BindingDescription::new(crate::t!(
                "keybinding-desc-workspace-left-panel-local-drive"
            )),
            WorkspaceAction::ToggleLocalDrive,
        )
        .with_group(bindings::BindingGroup::Navigation.as_str())
        .with_context_predicate(id!("Workspace") & id!(flags::ENABLE_LOCAL_DRIVE))
        ...
        EditableBinding::new(
            LEFT_PANEL_ENVIRONMENT_PROVIDER_MANAGER_BINDING_NAME,
            BindingDescription::new(crate::t!(
                "keybinding-desc-workspace-left-panel-environment-provider-manager"
            )),
            WorkspaceAction::ToggleEnvironmentProviderManager,
        )
        .with_group(bindings::BindingGroup::Navigation.as_str())
        .with_context_predicate(id!("Workspace"))
        ...
        EditableBinding::new(
            ...
            BindingDescription::new(crate::t!("keybinding-desc-workspace-toggle-local-drive"))
                .with_custom_description(
                    bindings::MAC_MENUS_CONTEXT,
                    crate::t!("keybinding-desc-workspace-toggle-local-drive-menu"),
                ),
            WorkspaceAction::ToggleLocalDrive,
        )
        .with_context_predicate(id!("Workspace") & id!(flags::ENABLE_LOCAL_DRIVE)),
    ...
    if cfg!(not(target_family = "wasm")) {
        app.register_editable_bindings([EditableBinding::new(
            "workspace:export_all_local_drive_objects",
            crate::t!("keybinding-desc-workspace-export-all-local-drive-objects"),
            WorkspaceAction::ExportAllLocalDriveObjects,
        )
        .with_group(bindings::BindingGroup::Settings.as_str())
        .with_context_predicate(id!("Workspace") & id!(flags::ENABLE_LOCAL_DRIVE))]);
    }
    ...
    app.register_editable_bindings([
        ...
        // two mac menu items for Agent Mode vs Ashide AI since they are mutually exclusive.
        .with_custom_action(CustomAction::NewAgentModePane),
    ]);
    ...
    app.register_editable_bindings([
        ...
        .with_context_predicate(
            id!("Workspace") & id!(flags::ENABLE_LOCAL_DRIVE) & id!(flags::IS_ANY_AI_ENABLED),
        ),
    ]);
...
fn add_overflow_menu_items_as_editable_binding(app: &mut AppContext) {
    ...
    // 本地/去中心化构建无云端账号,故无 "Invite People..."(ShowReferralSettingsPage)动作。
    app.register_editable_bindings([
        EditableBinding::new(
