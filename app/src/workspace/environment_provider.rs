...
pub(crate) struct EnvironmentProviderCandidateLoadResult {
    pub(crate) config_open_target: Option<std::path::PathBuf>,
...
mod tests {
    ...
    fn catalog_with_aliases(aliases: &[&str]) -> crate::ssh_manager::SshTargetCatalog {
        crate::ssh_manager::SshTargetCatalog::with_snapshot(warp_ssh_manager::LoadResult {
            path: Some(PathBuf::from(r"C:\Users\Alice\.ssh\config")),
            outcome: warp_ssh_manager::LoadOutcome::Loaded(
                aliases
                    .iter()
                    .map(|alias| warp_ssh_manager::SshConfigCandidate {
    ...
    #[test]
    fn ssh_config_runtime_lookup_uses_only_committed_catalog_generation() {
        let mut catalog = catalog_with_aliases(&[
            "remote-fixture-old-a",
            "remote-fixture-old-b",
            "remote-fixture-old-c",
        ]);
        catalog.commit_for_test(warp_ssh_manager::LoadResult {
            path: Some(PathBuf::from(r"C:\Users\Alice\.ssh\config")),
            outcome: warp_ssh_manager::LoadOutcome::Loaded(vec![
                warp_ssh_manager::SshConfigCandidate {
                    alias: "remote-fixture-new-a".into(),
                    hostname: None,
                    user: None,
                    port: None,
                    identity_file: None,
                ...
                warp_ssh_manager::SshConfigCandidate {
                    alias: "remote-fixture-new-b".into(),
                    hostname: None,
                    user: None,
                    port: None,
                    identity_file: None,
        ...
        assert!(runtime_transport_descriptor_for_connection_ref(
            "ssh-config:remote-fixture-old-a",
            &catalog
        )
        .is_none());
        let descriptor = runtime_transport_descriptor_for_connection_ref(
            "ssh-config:remote-fixture-new-b",
            &catalog,
        )
        .expect("runtime lookup must use the current committed catalog generation");
        assert_eq!(
            descriptor.connection_ref(),
            "ssh-config:remote-fixture-new-b"
        );
        assert_eq!(descriptor.target(), "remote-fixture-new-b");
    }
}
...
pub(crate) fn runtime_transport_descriptor_for_connection_ref(
    ...
    source_saved_ssh::runtime_transport_descriptor_for_connection_ref(connection_ref, catalog)
}

pub(crate) fn describe_runtime_transport_descriptor_lookup_failure(
    connection_ref: &str,
    catalog: &crate::ssh_manager::SshTargetCatalog,
) -> String {
    source_saved_ssh::describe_runtime_transport_descriptor_lookup_failure(connection_ref, catalog)
}

pub(crate) fn target_for_provider_candidate(
    alias: &str,
    catalog: &crate::ssh_manager::SshTargetCatalog,
) -> Option<EnvironmentProviderTarget> {
    source_saved_ssh::target_for_provider_candidate(alias, catalog)
}

pub(crate) fn provider_candidates(
    catalog: &crate::ssh_manager::SshTargetCatalog,
) -> EnvironmentProviderCandidateLoadResult {
    source_saved_ssh::provider_candidates(catalog)
}
