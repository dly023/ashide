...
/// Used to show a Ashide-native modal dialog above a [`super::Workspace`]. The first button is [`ButtonVariant::Accent`].
...
impl NativeModal {
    ...
    fn trigger_button_callback(&mut self, idx: usize, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(mut dialog) = self.alert_dialog.take() {
            ...
            (button.on_click)(ctx);
            if dialog.show_disable_checkbox && self.dont_show_again {
...
impl View for NativeModal {
    ...
    fn render(&self, app: &warpui::AppContext) -> Box<dyn warpui::Element> {
        ...
        let mut dialog_column_contents = Vec::new();
        if alert_dialog.show_disable_checkbox {
            dialog_column_contents.push(
