...
use super::{
    ...
    split_pane_count_for_tab_display, summary_overflow_count, summary_search_text_fragments, terminal_kind_badge_label,
...
#[test]
fn tabs_granularity_returns_first_pane_when_split_in_tabs_mode() {
    let pane_1 = pane_id();
    let pane_2 = pane_id();
    let pane_3 = pane_id();
    ...
    assert_eq!(
        ...
        vec![pane_1],
        "split tabs mode should keep the first visible pane as the tab title source"
    );
}
...
#[test]
fn pane_ids_for_display_granularity_uses_first_pane_when_split_in_tabs_mode() {
    let first = pane_id();
    let second = pane_id();
    let focused = pane_id();
...
#[test]
fn pane_ids_for_display_granularity_uses_focused_pane_when_not_split() {
    let first = pane_id();
    let second = pane_id();
    let visible = vec![first, second];
...
#[test]
fn split_pane_count_for_tab_display_only_shows_for_split_tabs_mode() {
    let panes = vec![pane_id(), pane_id()];
    assert_eq!(
        split_pane_count_for_tab_display(&panes, VerticalTabsDisplayGranularity::Tabs),
        Some(2)
    );
    assert_eq!(
        split_pane_count_for_tab_display(&[pane_id()], VerticalTabsDisplayGranularity::Tabs),
        None
    );
    assert_eq!(
        split_pane_count_for_tab_display(&panes, VerticalTabsDisplayGranularity::Panes),
        None
    );
}
