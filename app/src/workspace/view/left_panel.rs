...
impl LeftPanelView {
    pub fn new(
        ...
        let environment_project_explorer_view =
            ctx.add_typed_action_view(ServerFileBrowserView::new);
        ...
        ctx.subscribe_to_view(&environment_project_explorer_view, |_me, _, event, ctx| {
            ctx.emit(LeftPanelEvent::ServerFileBrowser(event.clone()));
        });
