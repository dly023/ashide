...
impl Workspace {
    ...
    pub(super) fn raw_session_navigator_sessions(
        ...
        let pinned_session_ids = Self::pinned_cli_agent_session_ids();
        let deleted_session_ids = Self::deleted_cli_agent_session_ids();
        ...
        let sources = live_sessions
            ...
            .filter(|session| {
                let source_is_live = session.id.starts_with("tab:");
                Self::session_matches_current_environment(session, &current_authority)
                    && (source_is_live
                        || (!deleted_session_ids.contains(&session.id)
                            && !deleted_session_ids
                                .contains(&Self::workspace_session_logical_key(session))))
            });
        let mut sessions =
            WorkspaceSessionSnapshot::merge_for_session_navigator(sources, &pinned_session_ids);
        ...
        for session in &mut sessions {
            if let Some(alias) = self.workspace_session_alias(session) {
    ...
    pub(super) fn workspace_session_alias(
        ...
        Self::cli_agent_session_alias_for_keys(&keys)
    }

    pub(super) fn cli_agent_session_alias_for_keys(keys: &[String]) -> Option<String> {
        crate::terminal::cli_agent_session_index::session_alias_for_keys(
    ...
    pub(super) fn set_workspace_session_alias_for_keys(
        keys: &[String],
        alias: Option<&str>,
    ) -> Result<(), String> {
        for key in keys {
            Self::set_cli_agent_session_alias(key, alias)?;
        }
        Ok(())
    }
    ...
    pub(super) fn deleted_cli_agent_session_ids() -> HashSet<String> {
        crate::terminal::cli_agent_session_index::deleted_session_ids()
    }
    ...
    pub(super) fn set_cli_agent_session_deleted(
        session_id: &str,
        deleted: bool,
    ) -> Result<(), String> {
        crate::terminal::cli_agent_session_index::set_session_deleted(session_id, deleted)
    }
    ...
    pub(super) fn finish_workspace_session_alias_rename(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let result = if alias.is_empty() {
            Self::set_workspace_session_alias_for_keys(&keys, None)
        } else {
            Self::set_workspace_session_alias_for_keys(&keys, Some(&alias))
        };
    ...
    pub(super) fn clear_workspace_session_alias(
        ...
        let keys = self.workspace_session_alias_keys(&session);
        if let Err(error) = Self::set_workspace_session_alias_for_keys(&keys, None) {
    ...
    pub(super) fn copy_workspace_session_id(
        ...
        ctx.clipboard()
            .write(warpui::clipboard::ClipboardContent::plain_text(id_to_copy.clone()));
        self.show_workspace_session_success_toast(
            crate::t!("workspace-session-navigator-menu-copy-id-toast", id = id_to_copy.as_str()),
            ctx,
        );
        ctx.notify();
    }
    ...
    pub(super) fn toggle_workspace_session_pinned(
        ...
        for key in keys {
            if let Err(error) = Self::set_cli_agent_session_pinned(&key, pinned) {
    ...
    pub(super) fn delete_workspace_session(
        ...
        let mut delete_keys = self.workspace_session_alias_keys(&session);
        ...
        for backing_session in &backing_sessions {
            delete_keys.extend(Self::workspace_session_alias_keys_for_session(
                backing_session,
            ));
        }
        delete_keys.sort();
        ...
        // Delete each CLI source in its environment FIRST; only tombstone +
        // remove from Ashide once the deletes confirm. This fixes the prior
        // ordering where a local tombstone was written before the (async,
        // possibly-failing) remote delete — leaving the row hidden locally but
        // still present remotely on failure. NOTE: this UI-ordering change is
        // NOT headless-verifiable — needs a real-machine delete pass (local +
        // remote, including a forced remote-delete failure).
        #[cfg(all(feature = "local_fs", not(target_family = "wasm")))]
        {
            ...
            ctx.spawn(future, move |workspace, results, ctx| {
                ...
                for key in &delete_keys {
                    if let Err(error) = Self::set_cli_agent_session_deleted(key, true) {
                        log::warn!("delete_workspace_session: failed to tombstone {key}: {error}");
                    }
                }
                ...
                for key in &pin_keys {
                    if let Err(error) = Self::set_cli_agent_session_pinned(key, false) {
                        log::warn!("delete_workspace_session: failed to clear pin {key}: {error}");
                    }
                }
                for key in &delete_keys {
                    if let Err(error) = Self::set_cli_agent_session_alias(key, None) {
                        log::warn!(
                            "delete_workspace_session: failed to clear alias {key}: {error}"
                        );
                    }
                }
        ...
        #[cfg(any(not(feature = "local_fs"), target_family = "wasm"))]
        {
            ...
            for backing_session in &backing_sessions {
                if !deleted_ids.insert(backing_session.id.clone()) {
                    continue;
                }
                if Self::is_environment_cli_agent_session_source_id(&backing_session.id) {
                    self.schedule_environment_cli_agent_session_source_action(
                        backing_session,
                        EnvironmentCliAgentSessionSourceAction::Delete,
                        ctx,
                    );
            ...
            for backing_session in &backing_sessions {
                for key in Self::workspace_session_pin_keys(backing_session) {
                    if let Err(error) = Self::set_cli_agent_session_pinned(&key, false) {
                        log::warn!("delete_workspace_session: failed to clear pin {key}: {error}");
                    }
                }
            ...
            for key in &delete_keys {
                if let Err(error) = Self::set_cli_agent_session_alias(key, None) {
                    log::warn!("delete_workspace_session: failed to clear alias {key}: {error}");
                }
            }
            ...
            if let Some(error) = session_source_delete_errors.first() {
                self.show_workspace_session_error_toast(
                    format!("已从 Ashide 隐藏，但清理会话来源记录失败：{error}"),
                    ctx,
                );
            } else {
    ...
    pub(super) fn reselect_workspace_session_after_delete(
        ...
        if let Some(tab_index) = self.tab_index_for_environment_authority(target_authority) {
            ...
            if !active_is_local {
                ...
                {
                    self.activate_tab_internal(local_tab_index, ctx);
                } else {
            ...
            self.ensure_environment_tab_for_authority(target_authority, ctx);
        }
        self.sync_session_navigator_sessions(ctx);
    }
