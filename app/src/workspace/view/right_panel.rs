...
pub enum ReviewDestination {
    ...
    None,
    /// A Ashide agent terminal is available (input box visible, not executing).
    Ashide,
...
impl ReviewTerminalUnavailableReason {
    fn label(&self) -> &'static str {
        match self {
            ...
            Self::SessionPathUnavailable => {
                "session cwd is unavailable for current-app file access"
            }
            Self::SessionOutsideSelectedRepo => "session cwd is not inside selected repo",
            Self::AIDisabled => "AI is disabled for Ashide review destinations",
...
impl RightPanelView {
        ...
        let Some(selected_repo_path) = selected_repo_path else {
            ...
            let no_repo_body = {
                ...
                if let Some(env) = &self.code_review_session_env {
                    if env.uses_environment_runtime {
                        CodeReviewView::render_environment_runtime_state(appearance, button)
                    } else if env.is_wsl {
    ...
    fn review_terminal_status(
        ...
        tv.read(ctx, |t, ctx| {
            let active_session_path = t.active_session_current_app_path(ctx);
    ...
    /// considered available (non-CLI Ashide terminals require AI to be on).
            ...
            .map(|tv| {
                tv.read(ctx, |t, ctx| {
                    t.active_cli_agent(ctx)
                        ...
                        .unwrap_or(ReviewDestination::Ashide)
                })
            })
