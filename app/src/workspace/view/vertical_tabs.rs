...
fn render_restored_session_icon_with_status(
    ...
    let status_color = if is_resuming {
        Some(theme.accent().into())
    } else if session.is_active {
        Some(coloru_with_opacity(theme.accent().into(), 85))
    } else if is_live {
    ...
    let Some(status_color) = status_color else {
        return icon;
    };

    let status = if is_resuming {
        ConstrainedBox::new(
            UiIcon::Loading
                .to_warpui_icon(WarpThemeFill::Solid(status_color))
                .finish(),
        )
        .with_width(10.)
        .with_height(10.)
        ...
        render_restored_session_activity_dot(status_color)
    };
...
fn render_restored_session_row(
    ...
    let hover_session_id = session.id.clone();
    let hover_environment_authority_key = session.environment_authority_key.clone();
    let row_position_id =
        vtab_session_row_position_id(&session.id, session.environment_authority_key.as_deref());
    let detail_target = Some(VerticalTabsDetailTarget::Session {
        session_id: hover_session_id,
        environment_authority_key: hover_environment_authority_key,
    });
            ...
            workspace
                ...
                .is_some_and(|target| {
                    target.session_id == session.id
                        && target.environment_authority_key == session.environment_authority_key
                }),
    ...
    pub(super) fn reconcile_visibility_for_mouse_position(
        ...
        let row_rect = match &active_target {
            ...
            VerticalTabsDetailTarget::Session {
                ...
                self.window_id,
                vtab_session_row_position_id(session_id, environment_authority_key.as_deref()),
            ),
        };
        let sidecar_rect = app.element_position_by_id_at_last_frame(
            self.window_id,
...
fn render_groups(
    ...
    if let Some(sessions) = render_restored_sessions_group(
        _state,
        workspace,
        &workspace.session_navigator_sessions(app),
        query,
        appearance,
        app,
    ) {
...
fn render_workspace_session_detail_sidecar(
    ...
    let target = WorkspaceSessionActionTarget::new(
        session_id.to_owned(),
        environment_authority_key.map(str::to_owned),
    );
    let session = workspace
        .session_navigator_sessions(app)
        .into_iter()
        .find(|session| Workspace::workspace_session_matches_action_target(session, &target))?;
    ...
    let anchor_position_id = vtab_session_row_position_id(session_id, environment_authority_key);
    ...
    let source_row_mouse_state = state
        .session_row_mouse_states
        .borrow()
        .get(&vtab_session_row_key(session_id, environment_authority_key))
        .cloned();
    ...
    let active_target = VerticalTabsDetailTarget::Session {
        ...
        environment_authority_key: environment_authority_key.map(str::to_owned),
    };
...
pub(super) fn render_detail_sidecar(
    ...
    } = &active_target
    {
        return render_workspace_session_detail_sidecar(
            ...
            app,
            session_id,
            environment_authority_key.as_deref(),
        );
    }
