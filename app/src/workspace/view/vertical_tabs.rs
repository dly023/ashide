...
fn render_restored_session_row(
    ...
    let row = Hoverable::new(mouse_state, move |hover| {
        ...
        if is_selected {
            ...
            container = container.with_background(ElementFill::Solid(coloru_with_opacity(
                theme.accent().into(),
                18,
            )));
        } else if hovered {
...
fn pane_row_background(
    ...
    if let Some(color) = pane_color {
        let opacity = if is_being_dragged || is_selected || is_hovered {
...
fn render_tab_group_internal(
    ...
    let split_pane_count = split_pane_count_for_tab_display(&visible_pane_ids, display_granularity);
    ...
    let mut group_element = Hoverable::new(group_mouse_state, move |group_state| {
        ...
        let group_content = if uses_outer_group_container {
            ...
            let background = if is_drag_target || is_active {
                internal_colors::fg_overlay_2(theme)
...
fn render_terminal_row_content(
    ...
    let first_line_element = wrap_title_line_with_trailing_accessories(
        first_line,
        props,
        theme,
        appearance,
...
fn render_split_pane_count_badge(count: usize, appearance: &Appearance) -> Box<dyn Element> {
    let theme = appearance.theme();
    let sub_text_color = theme.sub_text_color(theme.background());
    render_badge_container(
...
fn wrap_title_line_with_trailing_accessories(
    ...
    let mut trailing = Flex::row()
        .with_cross_axis_alignment(CrossAxisAlignment::Center)
        .with_spacing(4.);
    if let Some(count) = props.split_pane_count {
        trailing.add_child(render_split_pane_count_badge(count, appearance));
    }
...
fn pane_ids_for_display_granularity(
    ...
    match granularity {
        ...
        VerticalTabsDisplayGranularity::Tabs => {
            let pane_id = if visible_pane_ids.len() > 1 {
                visible_pane_ids.first().copied()
            } else {
                visible_pane_ids
                    .iter()
                    .copied()
            ...
            pane_id.into_iter().collect()
        }
    }
}

fn split_pane_count_for_tab_display(
    visible_pane_ids: &[PaneId],
    granularity: VerticalTabsDisplayGranularity,
) -> Option<usize> {
    if matches!(granularity, VerticalTabsDisplayGranularity::Tabs) && visible_pane_ids.len() > 1 {
        Some(visible_pane_ids.len())
    } else {
        None
    }
}
...
fn render_compact_pane_row(props: PaneProps<'_>, app: &AppContext) -> Box<dyn Element> {
    ...
    // Title row with optional split count / indicator accessories.
    let title_row = wrap_title_line_with_trailing_accessories(
        title_element,
        &props,
        theme,
