...
use crate::drive::{local_drive_object_styling::local_drive_icon_color, DriveObjectType};
...
const DETAIL_SIDECAR_SECTION_PADDING: f32 = 10.;
...
impl SessionNavigatorRowIdentity {
    fn row_key(&self) -> String {
        vtab_session_row_key(&self.row_id, Some(&self.environment_navigation_key))
    }

    fn row_position_id(&self) -> String {
        vtab_session_row_position_id(&self.row_id, Some(&self.environment_navigation_key))
    }
}
...
enum RestoredSessionActivityIndicator {
    Running,
    Attention,
}
...
) -> Option<RestoredSessionActivityIndicator> {
    match status {
        Some(CLIAgentSessionStatus::InProgress) => Some(RestoredSessionActivityIndicator::Running),
        Some(CLIAgentSessionStatus::Blocked { .. }) => {
            Some(RestoredSessionActivityIndicator::Attention)
        }
        Some(CLIAgentSessionStatus::Success) | None => None,
    }
}
...
fn live_cli_agent_activity_indicator(
    workspace: &Workspace,
    session: &WorkspaceSessionSnapshot,
    app: &AppContext,
) -> Option<RestoredSessionActivityIndicator> {
    ...
    let container_uuid = session
        .container_uuid
        .as_deref()
        .expect("Session Navigator live pane must have a stable container UUID");
    let expected_agent = session
        .cli_agent
        .as_deref()
        .map(CLIAgent::from_serialized_name)
        .filter(|agent| !matches!(agent, CLIAgent::Unknown))?;
    let terminal_view_id = workspace.tabs.iter().find_map(|tab| {
        tab.pane_group
            .as_ref(app)
            .terminal_view_id_for_container_uuid(container_uuid, app)
    })?;
    let cli_agent_session = CLIAgentSessionsModel::as_ref(app).session(terminal_view_id)?;
    if cli_agent_session.agent != expected_agent || !session_supports_rich_status(cli_agent_session)
    {
        return None;
    }

    restored_session_activity_indicator(Some(&cli_agent_session.status))
}

fn render_restored_session_icon_with_status(
    session: &WorkspaceSessionSnapshot,
    is_resuming: bool,
    activity_indicator: Option<RestoredSessionActivityIndicator>,
    appearance: &Appearance,
    ...
    let status = match badge {
        RestoredSessionIconBadge::Resuming => ConstrainedBox::new(
            UiIcon::Loading
                .to_warpui_icon(WarpThemeFill::Solid(theme.accent().into()))
                .finish(),
        )
        .with_width(10.)
        .with_height(10.)
        .finish(),
        RestoredSessionIconBadge::Activity(indicator) => {
            let color = match indicator {
                RestoredSessionActivityIndicator::Running => {
...
fn render_restored_session_trailing_slot(
    is_pinned: bool,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let theme = appearance.theme();
    ...
    let pin_slot = if is_pinned {
        ConstrainedBox::new(
            UiIcon::BookmarkFilled
                .to_warpui_icon(WarpThemeFill::Solid(coloru_with_opacity(sub_text, 72)))
                .finish(),
        )
        .with_width(SESSION_PIN_ICON_SIZE)
        .with_height(SESSION_PIN_ICON_SIZE)
        .finish()
    } else {
        Empty::new().finish()
    };
...
fn render_restored_session_row(
    ...
    let context_session_target = session_target.clone();
    let row_position_id = render_identity.row_position_id();
    let detail_target = Some(VerticalTabsDetailTarget::Session {
        row_id: render_identity.row_id,
        environment_navigation_key: render_identity.environment_navigation_key,
    });
    ...
    let row = Hoverable::new(mouse_state, move |hover| {
        ...
        let title = match (same_window_split_group_number, is_renaming) {
            (Some(group_number), false) => {
                wrap_title_line_with_same_window_split_badge(title, group_number, appearance)
            }
    ...
    .on_right_click(move |ctx, _, position| {
        ctx.dispatch_typed_action(WorkspaceAction::ShowWorkspaceSessionContextMenu {
            target: context_session_target.clone(),
            position,
        });
    })
    .on_hover(move |is_hovered, ctx, app, position| {
        ...
        if is_hovered {
            if !suppress_hover && overlay_state.active_target != detail_target {
                overlay_state.active_target = detail_target.clone();
                changed = true;
            }
...
fn detail_target_kind(target: &VerticalTabsDetailTarget) -> Option<VerticalTabsDetailTargetKind> {
    match target {
        VerticalTabsDetailTarget::Pane { .. } => Some(VerticalTabsDetailTargetKind::Pane),
        VerticalTabsDetailTarget::Tab { .. } => Some(VerticalTabsDetailTargetKind::Tab),
        VerticalTabsDetailTarget::Session { .. } => None,
    }
}
...
fn pane_row_background(
    ...
    if let Some(color) = pane_color {
        let opacity = if is_being_dragged || is_selected {
    ...
    } else if is_being_dragged || (is_selected && is_hovered) {
...
fn restored_session_row_background(
    is_selected: bool,
    is_resuming: bool,
    is_hovered: bool,
    theme: &WarpTheme,
) -> Option<ThemeFill> {
    match (is_selected, is_hovered, is_resuming) {
        (true, _, _) => Some(internal_colors::fg_overlay_2(theme)),
        (false, true, _) => Some(internal_colors::fg_overlay_1(theme)),
...
enum VerticalTabsDetailTarget {
    ...
    Session {
        row_id: String,
        environment_navigation_key: String,
    },
}

impl VerticalTabsDetailTarget {
    fn pane_group_id(&self) -> Option<EntityId> {
        match self {
            Self::Pane { pane_group_id, .. } | Self::Tab { pane_group_id, .. } => {
                Some(*pane_group_id)
            }
            Self::Session { .. } => None,
        }
    ...
    fn source_pane_id(&self) -> Option<PaneId> {
        match self {
            Self::Pane { pane_id, .. } => Some(*pane_id),
            Self::Tab { source_pane_id, .. } => Some(*source_pane_id),
            Self::Session { .. } => None,
        }
    }
}
...
impl VerticalTabsDetailHoverState {
    pub(super) fn reconcile_visibility_for_mouse_position(
        ...
        let mut overlay_state = self
            ...
            .lock()
            .expect("vertical tabs detail overlay lock poisoned");
        let Some(active_target) = overlay_state.active_target.clone() else {
            return false;
        };

        let row_rect = match &active_target {
            VerticalTabsDetailTarget::Pane {
                pane_group_id,
                pane_id,
            } => app.element_position_by_id_at_last_frame(
                self.window_id,
                vtab_pane_row_position_id(*pane_group_id, *pane_id),
            ),
            VerticalTabsDetailTarget::Tab {
                ...
                self.window_id,
                vtab_pane_row_position_id(*pane_group_id, *source_pane_id),
            ),
            VerticalTabsDetailTarget::Session {
                row_id,
                environment_navigation_key,
            } => app.element_position_by_id_at_last_frame(
                self.window_id,
                vtab_session_row_position_id(row_id, Some(environment_navigation_key)),
            ),
        };
        let sidecar_rect = app.element_position_by_id_at_last_frame(
            self.window_id,
        ...
        let matches = self
            ...
            .map(|overlay_state| {
                overlay_state
                    .active_target
                    .as_ref()
                    .and_then(VerticalTabsDetailTarget::pane_group_id)
                    .is_some_and(|active_pane_group_id| active_pane_group_id == pane_group_id)
            })
            .unwrap_or(false);
        if matches {
...
struct PaneProps<'a> {
    ...
    split_group_number: Option<usize>,
}
...
enum SummaryPaneKind {
    ...
    Settings,
    EnvVarCollection, // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
    AIFact,
    AIDocument,
...
fn render_detail_kind_badge_icon(
    ...
    match &props.typed {
        ...
        typed => {
            let fill = typed
                .local_drive_object_type()
                .map(|object_type| {
                    WarpThemeFill::Solid(local_drive_icon_color(appearance, object_type))
                })
                .unwrap_or(sub_text);
...
fn render_tab_group_internal(
    ...
    let representative_pane_ids =
        pane_ids_for_display_granularity(&visible_pane_ids, focused_pane_id, display_granularity);
    ...
    let displayed_tab_title_override = (!uses_outer_group_container)
        .then(|| pane_group.custom_title(app))
        .flatten();
    let split_group_number = workspace.same_window_split_group_number_for_tab(tab_index, app);
    ...
    let group_container_is_selected =
        tab_group_container_is_selected(is_active, resolved_mode, pane_ids_to_render.len());
    ...
    let mut group_element = Hoverable::new(group_mouse_state, move |group_state| {
        ...
        let group_content = if uses_outer_group_container {
            ...
            let background = if is_drag_target || group_container_is_selected {
                internal_colors::fg_overlay_2(theme)
            } else if group_state.is_hovered() {
            ...
            } else if group_container_is_selected {
                container =
                    container.with_border(Border::all(1.).with_border_fill(ThemeFill::Solid(
                        coloru_with_opacity(theme.main_text_color(theme.background()).into(), 32),
                    )));
...
fn resolve_icon_with_status_variant(
    ...
    match typed {
        ...
        // Settings and environment management use the foreground color per design spec    // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
        TypedPane::Settings => IconWithStatusVariant::Neutral {
            icon: typed.icon(),
            icon_color: main_text,
        },
        // Ashide Drive object types use their established index colors
...
fn render_pane_row(props: PaneProps<'_>, app: &AppContext) -> Box<dyn Element> {
    ...
    let text_content = if let TypedPane::Terminal(terminal_pane) = &props.typed {
        ...
        let title_row = wrap_title_line_with_trailing_accessories(
...
impl TypedPane<'_> {
    fn summary_pane_kind(&self, title: &str, app: &AppContext) -> SummaryPaneKind {
        match self {
            ...
            TypedPane::EnvVarCollection => SummaryPaneKind::EnvVarCollection, // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
    ...
    fn local_drive_object_type(&self) -> Option<DriveObjectType> {
        typed_pane_local_drive_object_type(self)
    }

    fn supports_vertical_tabs_detail_sidecar(&self) -> bool {
        matches!(self, TypedPane::Terminal(_) | TypedPane::Code(_))
            || self.local_drive_object_type().is_some()
    }
    fn kind_label(&self) -> String {
        match self {
            ...
            } // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
            // 物理删。
    ...
    fn badge(&self, app: &AppContext) -> Option<String> {
        match self {
            ...
            | TypedPane::Workflow { .. }
            | TypedPane::Settings
            | TypedPane::EnvVarCollection    // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
            | TypedPane::AIFact
    ...
    fn icon(&self) -> WarpIcon {
        match self {
            ...
            TypedPane::Settings => WarpIcon::Gear, // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
...
impl PaneGroup {
    fn resolve_pane_type(&self, pane_id: PaneId, app: &AppContext) -> TypedPane<'_> {
        match pane_id.pane_type() {
            ...
            | IPaneType::EnvironmentRuntimePlaceholder
            | IPaneType::Welcome
...
fn render_terminal_row_content(
    ...
    let first_line_element =
        wrap_title_line_with_trailing_accessories(first_line, props, theme, appearance, app);
...
fn render_summary_pane_kind_icon_circle(
    ...
    let (icon_element, background): (Box<dyn Element>, ElementFill) = match kind {
        ...
        | SummaryPaneKind::EnvVarCollection    // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
...
fn summary_pane_kind_icon(
    ...
    match kind {
        ...
        SummaryPaneKind::Settings => (WarpIcon::Gear, main_text), // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
...
fn render_same_window_split_badge(
    group_number: usize,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let theme = appearance.theme();
    ...
    render_badge_container(
        Text::new_inline(
            crate::t!(
                "workspace-session-navigator-split-group-badge",
                number = group_number
            ),
            appearance.ui_font_family(),
            10.,
...
fn wrap_title_line_with_same_window_split_badge(
    title_line: Box<dyn Element>,
    group_number: usize,
    appearance: &Appearance,
) -> Box<dyn Element> {
    Flex::row()
        ...
        .with_child(
            Container::new(render_same_window_split_badge(group_number, appearance))
                .with_margin_left(4.)
                .finish(),
        )
        .finish()
...
fn wrap_title_line_with_trailing_accessories(
    title_line: Box<dyn Element>,
    props: &PaneProps<'_>,
    theme: &WarpTheme,
    appearance: &Appearance,
    ...
    let split_group_number = props.split_group_number;
    let has_unread = has_unread_activity(&props.typed, app);
    let has_typed_badge = props.typed.badge(app).is_some();
    if split_group_number.is_none() && !has_unread && !has_typed_badge {
        return title_line;
    }
    ...
    let mut trailing = Flex::row()
        .with_cross_axis_alignment(CrossAxisAlignment::Center)
        .with_spacing(4.);
    if let Some(group_number) = split_group_number {
        trailing.add_child(render_same_window_split_badge(group_number, appearance));
    }
    if has_unread || has_typed_badge {
    ...
    let per_pane: HashMap<PaneId, Option<AnsiColorIdentifier>> = visible_pane_ids
        ...
        .map(|&pane_id| {
            let color = if let Some(tv) = pane_group.terminal_view_from_pane_id(pane_id, app) {
                // Directory tab colors are current-app settings. Do not compare a
                // runtime-backed environment cwd against current-app directory
                // color rules; provider-scoped colors should be a separate setting.
                tv.as_ref(app)
                    .active_session_current_app_path(app)
                    .as_deref()
                    .and_then(|cwd| {
                        dir_colors
                            .color_for_directory(cwd)
...
fn pane_ids_for_display_granularity(
    visible_pane_ids: &[PaneId],
    _focused_pane_id: PaneId,
    granularity: VerticalTabsDisplayGranularity,
) -> Vec<PaneId> {
    match granularity {
        VerticalTabsDisplayGranularity::Panes => visible_pane_ids.to_vec(),
        VerticalTabsDisplayGranularity::Tabs => visible_pane_ids.to_vec(),
    }
}

fn pane_row_is_selected(
    is_active_tab: bool,
    pane_id: PaneId,
    focused_pane_id: PaneId,
    rendered_pane_count: usize,
) -> bool {
    is_active_tab && (rendered_pane_count <= 1 || pane_id == focused_pane_id)
}

fn tab_group_container_is_selected(
    is_active_tab: bool,
    resolved_mode: VerticalTabsResolvedMode,
    rendered_pane_count: usize,
) -> bool {
    is_active_tab
        && (matches!(resolved_mode, VerticalTabsResolvedMode::Summary) || rendered_pane_count <= 1)
}
...
fn detail_sidecar_offset_and_max_height(
    ...
    const DETAIL_SIDECAR_MAX_HEIGHT: f32 = 360.;
    const DETAIL_SIDECAR_HORIZONTAL_GAP: f32 = 8.;
...
fn render_local_drive_object_detail_section(
    props: &PaneProps<'_>,
    appearance: &Appearance,
    app: &AppContext,
) -> Box<dyn Element> {
...
fn typed_pane_local_drive_object_type(typed: &TypedPane<'_>) -> Option<DriveObjectType> {
    match typed {
        ...
        | TypedPane::CodeDiff
        | TypedPane::File
        | TypedPane::Settings    // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
        | TypedPane::ExecutionProfileEditor
        | TypedPane::Other => None,
    }
}
...
fn render_detail_section(
    ...
    match &props.typed {
        ...
        | TypedPane::EnvVarCollection
        | TypedPane::AIFact
        | TypedPane::AIDocument => render_local_drive_object_detail_section(props, appearance, app),
        TypedPane::CodeDiff
        | TypedPane::File
        | TypedPane::Settings    // Legacy environment-management pane variants were removed with the ambient-agent UI subsystem.
        | TypedPane::ExecutionProfileEditor
...
fn render_session_preview_action(
    label: String,
...
fn render_workspace_session_detail_sidecar_contents(
    ...
    let text_colors = detail_sidecar_text_colors(theme);
    let is_live =
        session.is_active || Workspace::locator_from_restored_session_id(&session.id).is_some();
    let root = restored_session_root_label(session)
        .unwrap_or_else(|| crate::t!("workspace-session-navigator-no-root"));
    let environment_label = restored_session_environment_label(session);
    let status = if is_live {
        crate::t!("workspace-session-preview-live")
    } else {
        crate::t!("workspace-session-preview-history")
    };
    ...
    let header = Flex::column()
        ...
        .with_child(
            Text::new_inline(
                crate::t!("workspace-session-preview-title"),
                appearance.ui_font_family(),
                11.,
            )
        ...
        .with_child(
            // The agent identity is already carried by the row icon and the
            // title above (restored_session_label); this line only conveys the
            // live/history status, never repeating the agent name.
            Text::new_inline(status, appearance.monospace_font_family(), 10.5)
    ...
    let mut facts = Flex::column()
        .with_cross_axis_alignment(CrossAxisAlignment::Start)
        .with_spacing(4.);
    facts.add_child(render_detail_wrapping_text(
        root,
        12.,
        text_colors.main,
        None,
    ...
    if let Some(environment_label) = environment_label {
        facts.add_child(
            Text::new_inline(environment_label, appearance.monospace_font_family(), 10.5)
                .with_color(text_colors.sub.into())
                .with_clip(ClipConfig::ellipsis())
                .finish(),
        );
    }
    if let Some(session_id) = session.cli_agent_session_id.as_deref() {
        facts.add_child(
            Text::new_inline(
                crate::t!("workspace-session-preview-session-id", id = session_id),
                appearance.monospace_font_family(),
                10.5,
            )
            .with_color(text_colors.sub.into())
    ...
    if let Some(updated_at) = session.updated_at_unix_ms {
        ...
        facts.add_child(
            Text::new_inline(
                crate::t!("workspace-session-preview-updated", time = updated_at),
                appearance.monospace_font_family(),
                10.5,
            )
            .with_color(text_colors.disabled.into())
    ...
    let delete_label = if is_live {
        ...
        crate::t!("workspace-session-preview-delete")
    };
    let session_target = WorkspaceSessionActionTarget::new(
        session.id.clone(),
        session.environment_authority_key.clone(),
    ...
    let primary_actions = Flex::row()
        ...
        .with_child(render_session_preview_action(
            if is_live {
                crate::t!("workspace-session-preview-focus")
            } else {
                crate::t!("workspace-session-preview-resume")
            },
            Some(WorkspaceAction::ActivateRestoredWorkspaceSession {
                target: session_target.clone(),
            }),
            appearance,
        ));

    let mut action_rows = Flex::column()
        .with_cross_axis_alignment(CrossAxisAlignment::Start)
        .with_spacing(6.)
        .with_child(primary_actions.finish());
...
fn render_workspace_session_detail_sidecar(
    state: &VerticalTabsPanelState,
    workspace: &Workspace,
    side: super::PanelPosition,
    app: &AppContext,
    ...
    let session = navigator_model
        ...
        .iter()
        .find(|session| {
            Workspace::workspace_session_row_id(session, &navigator_model.state) == row_id
                && ParsedEnvironmentAuthority::parse(session_authority_or_terminal_bootstrap(
        ...
        .clone();
    if workspace.show_tab_right_click_menu.is_some() {
    ...
    let source_row_mouse_state = state
        .session_row_mouse_states
        .borrow()
        .get(&vtab_session_row_key(
            row_id,
            Some(environment_navigation_key),
        ))
        .cloned();
    let detail_overlay_state = state.detail_overlay_state.clone();
    let window_id = workspace.window_id;
    let active_target = VerticalTabsDetailTarget::Session {
    ...
    .on_hover(move |is_hovered, ctx, app, position| {
        ...
        if !is_hovered {
            ...
            if !row_hovered && overlay_state.active_target == Some(active_target.clone()) {
                overlay_state.active_target = None;
                overlay_state.safe_triangle.set_target_rect(None);
                ctx.notify();
            }
...
pub(super) fn render_detail_sidecar(
    ...
    let active_target = state
        ...
        .lock()
        .ok()
        .and_then(|overlay_state| overlay_state.active_target.clone())?;
    if let VerticalTabsDetailTarget::Session {
        row_id,
        environment_navigation_key,
    } = &active_target
    {
        return render_workspace_session_detail_sidecar(
            state,
            workspace,
            side,
            app,
    ...
    let pane_group_id = active_target.pane_group_id()?;
    let source_pane_id = active_target.source_pane_id()?;
    ...
    let pane_group = tab.pane_group.as_ref(app);
    let Some(pane_ids) = pane_ids_for_detail_target(pane_group, active_target.clone(), app) else {
        state.clear_detail_sidecar();
        return None;
    };
    ...
    let theme = appearance.theme();
    let sidecar_background = detail_sidecar_background(theme);
    let anchor_position_id = vtab_pane_row_position_id(pane_group_id, source_pane_id);
    let (offset, max_height, width, bounds, parent_anchor, child_anchor) =
        detail_sidecar_offset_and_max_height(&anchor_position_id, side, workspace.window_id, app);
    let source_row_mouse_state = state
        .pane_row_mouse_states
    ...
    for (index, pane_id) in pane_ids.iter().enumerate() {
        let Some(props) =
            detail_pane_props(state, workspace, pane_group, pane_group_id, *pane_id, app)
        else {
            state.clear_detail_sidecar();
            return None;
        };
...
fn render_compact_pane_row(props: PaneProps<'_>, app: &AppContext) -> Box<dyn Element> {
    ...
    // Title row with optional same-window split marker / indicator accessories.
    let title_row =
        wrap_title_line_with_trailing_accessories(title_element, &props, theme, appearance, app);
