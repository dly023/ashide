...
impl ServerFileBrowserView {
    ...
    fn finish_resolve_and_open(
        ...
        match result {
            ...
            Ok(resolved) if resolved.kind == EnvironmentRuntimeFileKind::File => {
                ...
                if let Some(parent) = environment_parent(&resolved.canonical_path) {
    ...
    fn commit_rename(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.spawn(
            ...
            move |me, result, ctx| {
                ...
                match result {
                    Ok(()) => {
                        let new_path = environment_parent(&from_path)
                            .map(|parent| join_environment_path(&parent, &new_name))
    ...
    fn upload_next_task(
        ...
        let (current_app_path, staging_environment_path, uploaded_bytes) = {
            let batch = self
                .upload_batches
                .get(batch_index)
        ...
        let verify_tasks: Vec<(String, u64)> = self
            ...
            .map(|batch| {
                batch
                    ...
                    .iter()
                    .filter(|task| matches!(task.status, UploadTaskStatus::Completed))
                    .map(|task| (task.staging_environment_path.clone(), task.total_bytes))
                    .collect()
            })
            .unwrap_or_default();
    ...
    fn promote_staging_batch(
        ...
        let Some(environment_session_id) = operation_scope.session_id else {
    ...
    fn spawn_cleanup_staging(
        ...
        ctx.spawn(
            async move { cleanup_staging_root(client, environment_session_id, staging_root).await },
            |_, _result, _ctx| {},
        );
    }
...
async fn list_terminal_directory(
    ...
    let mut reader = tokio::fs::read_dir(&canonical_path)
        .await
        .map_err(|error| error.to_string())?;
...
fn collect_upload_tasks(
    ...
    for current_app_path in current_app_paths {
        ...
        } else if current_app_path.is_file() {
...
async fn create_environment_entry(
    ...
    let (canonical_directory, entries) =
        list_directory(client.clone(), environment_directory).await?;
    ...
    let path = join_environment_path(&canonical_directory, &name);
    match kind {
        NewEnvironmentEntryKind::File => create_environment_file(client, path.clone()).await?,
        NewEnvironmentEntryKind::Directory => {
            create_environment_directory(client, path.clone()).await?
        }
    }
...
async fn create_environment_file(
    ...
    crate::workspace::environment_runtime::write_file_chunk(
        &client,
        environment_path,
        0,
        Vec::new(),
...
async fn upload_file_with_progress(
    ...
    let bytes = tokio::fs::read(current_app_path)
        .await
        .map_err(|error| error.to_string())?;
    let mut offset = 0;
...
fn staging_cleanup_shell_commands(staging_root: &str) -> String {
    ...
    if let Some(staging_parent) = environment_parent(staging_root) {
...
async fn promote_staging_files(
    client: Arc<EnvironmentFileBrowserClient>,
    environment_session_id: SessionId,
    staging_root: String,
    conflict_policy: UploadConflictPolicy,
    directory_overwrite_roots: HashSet<String>,
    ...
    for (staging_path, final_path) in promote_pairs {
        let Some(parent) = environment_parent(&final_path) else {
            continue;
        };
    ...
    execute_environment_shell_script(client, environment_session_id, script).await
}
...
async fn collect_download_files(
    ...
    tokio::fs::create_dir_all(&current_app_directory)
        .await
        .map_err(|error| error.to_string())?;
    let mut files = Vec::new();
...
async fn download_file_with_progress(
    ...
    let mut output = tokio::fs::File::create(&current_app_path)
        .await
        .map_err(|error| error.to_string())?;
...
async fn delete_environment_path(
    client: Arc<EnvironmentFileBrowserClient>,
    environment_session_id: Option<SessionId>,
    path: String,
    is_directory: bool,
) -> Result<(), String> {
    if is_directory {
        let Some(environment_session_id) = environment_session_id else {
        ...
        return execute_environment_shell_script(client, environment_session_id, script).await;
    }
...
async fn execute_environment_shell_script(
    client: Arc<EnvironmentFileBrowserClient>,
    environment_session_id: SessionId,
    script: String,
) -> Result<(), String> {
    crate::workspace::environment_runtime::run_command_success(
        &client,
        environment_session_id,
        script,
    )
