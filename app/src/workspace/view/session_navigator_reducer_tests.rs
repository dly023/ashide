...
fn test_oracle_combinatorial_action_invariants() {
    ...
    let actions: Vec<SessionNavigatorAction> = vec![
        ...
        SessionNavigatorAction::Refresh {
            ...
            pinned_identity_keys: HashSet::from([logical_key(&virtual_a)]),
        },
    ];
...
fn test_session_navigator_spec_matrix_is_complete_and_linked() {
    ...
    let scan_commit_tests = include_str!("session_navigator_scan_tests.rs");
    let workspace_tests = include_str!("../view_test.rs");
    let vertical_tabs_tests = include_str!("vertical_tabs_tests.rs");
    ...
    let session_bridge_tests = include_str!("../../session_bridge/tests.rs");
    let environment_authority_tests = include_str!("../../environment_authority.rs");
    ...
    for row in rows {
        ...
        let tests = row["test"].as_str().expect("matrix test 必须是字符串");
        for test in tests.split(" + ") {
            ...
            assert!(
                reducer_tests.contains(&format!("fn {test}"))
                    || scan_commit_tests.contains(&format!("fn {test}"))
                    || workspace_tests.contains(&format!("fn {test}"))
                    || vertical_tabs_tests.contains(&format!("fn {test}"))
                    ...
                    || environment_authority_tests.contains(&format!("fn {test}"))
    ...
    for required_id in [
        ...
        "SN-INTRA-TITLE-PARSER-PARITY-01",
        "SN-INTRA-SESSION-BRIDGE-TITLE-01",
...
#[test]
fn test_live_and_runtime_placeholder_projection_only_use_canonical_pane_binding() {
    let source = include_str!("session_navigator.rs");
    let projection = source
        .split("pub(super) fn live_workspace_sessions")
        .nth(1)
        .and_then(|source| {
            source
                .split("pub(crate) fn live_or_pending_workspace_session_locator")
                .next()
        })
        .expect("live Workspace session projection must exist");

    assert!(projection.contains("session_binding_for_pane_id"));
    assert!(projection.contains("binding.apply_to_workspace_session"));
    assert!(!projection.contains("CLIAgentSessionsModel::as_ref"));
    assert!(!projection.contains("apply_registered_cli_agent_session_to_live_snapshot"));
    assert!(!projection.contains("custom_command_prefix"));
    assert!(!source.contains("fn apply_registered_cli_agent_session_to_live_snapshot"));
}

#[test]
fn test_missing_pane_content_is_rejected_before_snapshot_or_identity_projection() {
    let source = include_str!("../../pane_group/mod.rs");
    let snapshot = source
        .split("pub fn snapshot_for_node")
        .nth(1)
        .and_then(|source| source.split("pub fn container_uuid_for_pane_id").next())
        .expect("PaneGroup::snapshot_for_node must exist");

    assert!(!snapshot.contains("Uuid::new_v4"));
    assert!(!snapshot.contains("TerminalPaneSnapshot"));
    assert!(!snapshot.contains("InputConfig::new"));
    let validation = snapshot
        .find("pane tree leaf 必须存在对应内容")
        .expect("missing pane content must fail closed");
    let projection = snapshot
        .find("let contents = pane_configuration.snapshot(app)")
        .expect("typed pane snapshot projection must remain");
    let configuration = snapshot
        .find(".pane_configuration()")
        .expect("pane configuration projection must remain");
...
#[test]
fn test_session_navigator_initial_cache_uses_typed_refresh_boundary() {
    let workspace_view = include_str!("../view.rs");
    let session_navigator = include_str!("session_navigator.rs");
    let environment_table = include_str!("../environment_table.rs");
    let environment_backend = include_str!("../environment_backend.rs");
    let local_index = include_str!("../../terminal/cli_agent_session_index.rs");

    assert!(!workspace_view.contains("indexed_cli_agent_sessions: Vec<WorkspaceSessionSnapshot>"));
    assert!(environment_table.contains(
        "indexed_cli_agent_sessions_by_navigation_key: HashMap<String, Vec<WorkspaceSessionSnapshot>>"
    ));
    assert!(workspace_view.contains("refresh_indexed_sessions_for_authority"));
    assert!(environment_backend.contains("fn refresh_indexed_sessions("));
    assert!(session_navigator.contains("try_scan_terminal_cli_agent_session_discovery"));
    assert!(!session_navigator.contains("pub(super) fn scan_terminal_cli_agent_sessions()"));
    assert!(!local_index.contains("pub(crate) fn scan_current_app_cli_agent_sessions("));
}
...
#[test]
fn test_live_source_order_changes_preserve_container_rows_and_display_order() {
    let mut target = make_live_session("tab:0:leaf:1", "local", 3_000);
    target.container_uuid = Some(vec![0x10]);
    let mut unrelated_a = make_live_session("tab:0:leaf:0", "local", 1_000);
    unrelated_a.container_uuid = Some(vec![0x20]);
    let mut unrelated_b = make_live_session("tab:0:leaf:2", "local", 2_000);
    unrelated_b.container_uuid = Some(vec![0x30]);
    let initial_sessions = vec![target.clone(), unrelated_a.clone(), unrelated_b.clone()];
    let initial = reduce(
        Vec::new(),
        SessionNavigatorState::new(),
        SessionNavigatorAction::Refresh {
            new_sessions: initial_sessions.clone(),
            pinned_identity_keys: HashSet::new(),
        },
        &PaneGroupInfo::new(),
    );
    let initial_rows: Vec<_> = initial
        .sessions
        .iter()
        .map(|session| row_id_for_session(session, &initial.state))
        .collect();
    let initial_order = initial.state.display_order.clone();

    let refreshed = reduce(
        initial.sessions,
        initial.state,
        SessionNavigatorAction::Refresh {
            new_sessions: vec![unrelated_b, target, unrelated_a],
            pinned_identity_keys: HashSet::new(),
        },
        &PaneGroupInfo::new(),
    );
    let refreshed_rows: HashSet<_> = refreshed
        .sessions
        .iter()
        .map(|session| row_id_for_session(session, &refreshed.state))
        .collect();

    assert_eq!(refreshed.sessions.len(), 3);
    assert_eq!(refreshed_rows, initial_rows.into_iter().collect());
    assert_eq!(refreshed.state.display_order, initial_order);
}

#[test]
fn test_session_navigator_refresh_collapses_materialized_alias_to_live_row() {
    let virtual_session =
        make_virtual_session("agy-restore-source", "ssh:remote-fixture-primary", 2_000);
    let mut live_session = make_live_session("tab:3:leaf:0", "ssh:remote-fixture-primary", 0);
    live_session.cli_agent_session_id = None;
    live_session.label = Some("Antigravity live".to_owned());
    ...
    let initial = reduce(
        ...
        SessionNavigatorAction::Refresh {
            new_sessions: vec![virtual_session.clone()],
            pinned_identity_keys: HashSet::from([source_identity.clone()]),
        },
        &PaneGroupInfo::new(),
    );
    let source_row_id = row_id_for_session(&virtual_session, &initial.state);
    ...
    let refreshed = reduce(
        ...
        aliased.state,
        SessionNavigatorAction::Refresh {
            new_sessions: vec![virtual_session, live_session.clone()],
            pinned_identity_keys: HashSet::from(["agy-restore-source".to_owned()]),
        },
        &PaneGroupInfo::new(),
    );
    ...
    assert_eq!(refreshed.sessions[0].id, live_session.id);
    assert!(refreshed.sessions[0].is_pinned);
    assert_eq!(
        row_id_for_session(&refreshed.sessions[0], &refreshed.state),
        source_row_id
    );
    assert_eq!(
        refreshed.state.selected_row_id.as_deref(),
...
#[test]
fn test_restore_refresh_preserves_target_carrier_and_unrelated_rows_for_local_and_runtime() {
    for environment in ["local", "ssh:remote-fixture-primary"] {
        let target = make_virtual_session("codex:restore-target", environment, 3_000);
        let unrelated_older = make_virtual_session("codex:unrelated-older", environment, 1_000);
        let unrelated_newer = make_virtual_session("codex:unrelated-newer", environment, 2_000);
        let mut live_target = make_live_session("tab:4:leaf:0", environment, 4_000);
        live_target.cli_agent_session_id = None;
        ...
        let initial_order = initial
            .sessions
            .iter()
            .map(|session| row_id_for_session(session, &initial.state))
            .collect::<Vec<_>>();
        let target_row_id = row_id_for_session(&target, &initial.state);

        let started = reduce(
            initial.sessions,
            initial.state,
            SessionNavigatorAction::RestoreStarted {
                session_keys: vec![target.logical_key(), live_target.logical_key()],
                selected_logical_key: Some(target.logical_key()),
            },
            &PaneGroupInfo::new(),
        );
        let source_temporarily_absent = reduce(
            started.sessions,
            started.state,
            SessionNavigatorAction::Refresh {
                new_sessions: vec![unrelated_older.clone(), unrelated_newer.clone()],
                pinned_identity_keys: HashSet::new(),
            },
            &PaneGroupInfo::new(),
        );

        assert_eq!(source_temporarily_absent.sessions.len(), 3, "{environment}");
        assert_eq!(
            source_temporarily_absent
                .sessions
                .iter()
                .map(|session| row_id_for_session(session, &source_temporarily_absent.state))
                .collect::<Vec<_>>(),
            initial_order,
            "{environment}: source-missing frame must preserve cardinality, RowId and order"
        );
        assert!(source_temporarily_absent
            .sessions
            .iter()
            .any(
                |session| row_id_for_session(session, &source_temporarily_absent.state)
                    == target_row_id
            ));

        let materialized = reduce(
            source_temporarily_absent.sessions,
            source_temporarily_absent.state,
            SessionNavigatorAction::Refresh {
                new_sessions: vec![unrelated_older, live_target.clone(), unrelated_newer],
                pinned_identity_keys: HashSet::new(),
            },
            &PaneGroupInfo::new(),
        );
        assert_eq!(materialized.sessions.len(), 3, "{environment}");
        let target_after = materialized
            .sessions
            .iter()
            .find(|session| row_id_for_session(session, &materialized.state) == target_row_id)
            .expect("materialized target must retain its canonical row");
        assert!(target_after.is_live_container, "{environment}");
        assert_eq!(target_after.id, live_target.id, "{environment}");
        assert!(!materialized
            .state
            .restoring_row_ids
            .contains(&target_row_id));
        validate_state(&materialized.sessions, &materialized.state).unwrap();
    }
}

#[test]
fn test_restore_finished_removes_carrier_only_after_confirmed_source_absence() {
    let target = make_virtual_session("codex:restore-target", "local", 3_000);
    let unrelated_older = make_virtual_session("codex:unrelated-older", "local", 1_000);
    let unrelated_newer = make_virtual_session("codex:unrelated-newer", "local", 2_000);
    let initial = reduce(
        Vec::new(),
        SessionNavigatorState::new(),
        SessionNavigatorAction::Refresh {
    ...
    let started = reduce(
        ...
        initial.state,
        SessionNavigatorAction::RestoreStarted {
            session_keys: vec![target.logical_key()],
            selected_logical_key: Some(target.logical_key()),
        },
        &PaneGroupInfo::new(),
    );
    ...
    assert_eq!(source_temporarily_absent.sessions.len(), 3);
    ...
    assert_eq!(finished.sessions.len(), 3);
    assert!(!finished.state.restoring_row_ids.contains(&target_row_id));

    let confirmed_absent = reduce(
        finished.sessions,
        finished.state,
        SessionNavigatorAction::Refresh {
            new_sessions: vec![unrelated_older, unrelated_newer],
            pinned_identity_keys: HashSet::new(),
        },
        &PaneGroupInfo::new(),
    );
    assert_eq!(confirmed_absent.sessions.len(), 2);
