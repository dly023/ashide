...
const HERO_IMAGE_PATH: &str = "async/png/onboarding/ashide_launch_banner.png";
...
fn feature_items() -> [FeatureItem; 3] {
    [
        FeatureItem {
            icon: Icon::HeartHand,
            title: crate::t!("ashide-launch-contribute-title"),
            description: crate::t!("ashide-launch-contribute-description"),
            inline_link: Some(InlineLink {
                text: crate::t!("ashide-launch-contribute-link-text"),
                url: CONTRIBUTING_URL,
            }),
        },
        FeatureItem {
            icon: Icon::Oz,
            title: crate::t!("ashide-launch-oad-title"),
            description: crate::t!("ashide-launch-oad-description"),
            inline_link: None,
        },
        FeatureItem {
            icon: Icon::MessageChatSquare,
            title: crate::t!("ashide-launch-auto-model-title"),
            description: crate::t!("ashide-launch-auto-model-description"),
            inline_link: None,
        },
    ]
}
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings([FixedBinding::new(
        "escape",
        AshideLaunchModalAction::Close,
        id!(AshideLaunchModal::ui_name()),
    )]);
}

#[derive(Clone, Debug)]
pub enum AshideLaunchModalAction {
    Close,
    VisitRepo,
}

#[derive(Clone, Debug)]
pub enum AshideLaunchModalEvent {
    Close,
}
...
pub struct AshideLaunchModal {
    close_button: ViewHandle<ActionButton>,
    cta_button: ViewHandle<ActionButton>,
}

impl AshideLaunchModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let close_button = ctx.add_view(|_ctx| {
            ActionButton::new("", CloseButtonTheme)
                .with_icon(Icon::X)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| ctx.dispatch_typed_action(AshideLaunchModalAction::Close))
        });

        let cta_button = ctx.add_view(|_ctx| {
            ActionButton::new(crate::t!("ashide-launch-visit-repo"), CtaButtonTheme)
                .with_full_width(true)
                .on_click(|ctx| ctx.dispatch_typed_action(AshideLaunchModalAction::VisitRepo))
        });
    ...
    fn render_description(appearance: &Appearance) -> Box<dyn Element> {
        Text::new(
            crate::t!("ashide-launch-description"),
            appearance.ui_font_family(),
            14.,
        )
...
impl Entity for AshideLaunchModal {
    type Event = AshideLaunchModalEvent;
}

impl View for AshideLaunchModal {
...
impl TypedActionView for AshideLaunchModal {
    type Action = AshideLaunchModalAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            AshideLaunchModalAction::Close => {
                ctx.emit(AshideLaunchModalEvent::Close);
            }
            AshideLaunchModalAction::VisitRepo => {
