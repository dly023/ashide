...
use crate::environment_authority::{
    session_authority_uses_runtime_environment, ParsedEnvironmentAuthority,
};
...
pub(in crate::workspace::view) fn vtab_session_row_key(
    row_id: &str,
    environment_navigation_key: Option<&str>,
) -> String {
    format!(
        "{}::{}",
        environment_navigation_key
            .filter(|navigation_key| !navigation_key.trim().is_empty())
            .unwrap_or("local"),
        row_id
    )
}
...
pub(in crate::workspace::view) fn vtab_session_row_position_id(
    ...
    format!(
        ...
        vtab_session_row_key(row_id, environment_navigation_key)
    )
}
...
fn restored_session_uses_environment_runtime(session: &WorkspaceSessionSnapshot) -> bool {
    session_authority_uses_runtime_environment(session.environment_authority_key.as_deref())
}
...
pub(in crate::workspace::view) fn restored_session_environment_label(
    ...
    let label = ParsedEnvironmentAuthority::parse(authority).display_label()?;
    Some(crate::t!(
...
mod tests {
    ...
    fn restored_session_search_fragments_include_stable_identity_and_authority() {
        ...
        session.environment_authority_key =
            Some("ssh:ssh-config:remote-fixture-primary".to_string());
        ...
        assert!(fragments
            .iter()
            .any(|fragment| fragment == "ssh:ssh-config:remote-fixture-primary"));
        assert!(fragments
            .iter()
    ...
    #[test]
    fn vtab_session_row_key_scopes_canonical_row_id_by_environment() {
        assert_ne!(
            vtab_session_row_key("row:7", Some("local")),
            vtab_session_row_key("row:7", Some("ssh:ssh-config:remote-fixture-primary"))
        );
        assert_eq!(vtab_session_row_key("row:7", None), "local::row:7");

        let source_id = "external-index:codex:source";
        let live_locator = "tab:4:leaf:0";
        assert_ne!(source_id, live_locator);
        assert_eq!(
            vtab_session_row_key("row:7", Some("local")),
            vtab_session_row_key("row:7", Some("local")),
            "physical carrier replacement must not change the Vertical Tabs render identity"
        );
    }
}
