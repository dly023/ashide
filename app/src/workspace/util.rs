...
use warpui::{
    elements::{ClippedScrollStateHandle, MouseStateHandle},
...
impl PaneViewLocator {
    /// Constructs a placeholder locator for non-live sessions surfaced into
    /// the command-palette search (restored / indexed / historical sessions).
    /// Never used to focus a pane — activation routes through
    /// `ActivateRestoredWorkspaceSession` — so the locator only needs to be
    /// constructible without a live view.
    pub fn placeholder() -> Self {
        Self {
            pane_group_id: EntityId::from_usize(0),
            pane_id: PaneId::placeholder(),
        }
    }
}
...
pub(super) struct WorkspaceMouseStates {
    ...
    pub(super) environment_provider_picker_refresh_button: MouseStateHandle,
...
pub struct WorkspaceState {
    ...
    pub is_session_bridge_edit_dialog_open: bool,
    ...
    pub is_workspace_session_restore_popover_open: bool,
    pub is_environment_provider_picker_open: bool,
...
impl WorkspaceState {
    ...
    pub fn is_any_non_palette_modal_open(&self, app: &AppContext) -> bool {
        self.is_theme_creator_modal_open
            ...
            || self.is_session_bridge_edit_dialog_open
            ...
            || {
                ...
                one_time_modal.is_ashide_launch_modal_open()
            }
    }
    ...
    pub fn close_all_modals(&mut self) {
        ...
        self.is_workspace_session_restore_popover_open = false;
        self.is_environment_provider_picker_open = false;
        ...
        self.is_session_bridge_edit_dialog_open = false;
    ...
    pub fn close_all_left_panels(&mut self) {
        self.is_local_drive_open = false;
...
/// that.
///
/// Note that "active" is not the same as "focused" in Ashide's pane management.
