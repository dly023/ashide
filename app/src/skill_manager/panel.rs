...
use crate::ai::skills::{
    environment_skill_inventory::{
        cached_environment_skills_for_session, inventory_items_from_skill_descriptors,
        EnvironmentSkillInventoryCache, EnvironmentSkillInventoryCacheEvent,
    },
...
use crate::workspace::{ActiveSession, SkillManagerEnvironmentRuntimeScope};
...
impl SkillManagerPanel {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let active_session = ActiveSession::handle(ctx);
        ctx.subscribe_to_model(&active_session, |me, _active_session, _, ctx| {
            me.refresh_active_environment_inventory(ctx);
            let inventory = me.active_inventory(ctx);
        ...
        let environment_skill_inventory = EnvironmentSkillInventoryCache::handle(ctx);
        ctx.subscribe_to_model(
            &environment_skill_inventory,
            |me, _cache, event, ctx| match event {
                EnvironmentSkillInventoryCacheEvent::InventoryChanged => {
                    let inventory = me.active_inventory(ctx);
                    if me.provider_filter.is_some_and(|provider| {
                        !Self::providers_in_inventory(&inventory).contains(&provider)
                    }) {
    ...
    fn rebuild_source_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        let inventory = self.active_inventory(ctx);
    ...
    fn active_environment_runtime_scope(
        app: &AppContext,
    ) -> Option<SkillManagerEnvironmentRuntimeScope> {
        crate::workspace::active_window_skill_manager_environment_scope(app)
            .environment_runtime_scope()
            .cloned()
    }

    fn active_inventory(&self, app: &AppContext) -> Vec<SkillInventoryItem> {
        if let Some(environment_scope) = Self::active_environment_runtime_scope(app) {
            let Some(session_id) = environment_scope.session_id else {
                return Vec::new();
            };
            return inventory_items_from_skill_descriptors(cached_environment_skills_for_session(
                session_id,
                environment_scope.current_working_directory.as_deref(),
                app,
            ));
        }

        SkillManager::as_ref(app).list_skill_inventory(app)
    }

    fn refresh_active_environment_inventory(&self, ctx: &mut ViewContext<Self>) {
        let Some(environment_scope) = Self::active_environment_runtime_scope(ctx) else {
            return;
        };
        let Some(session_id) = environment_scope.session_id else {
            return;
        ...
        EnvironmentSkillInventoryCache::handle(ctx).update(ctx, |cache, ctx| {
            cache.refresh_for_session(
                client,
                session_id,
                environment_scope.current_working_directory,
                ctx,
            );
        });
    }
    ...
    pub fn on_left_panel_focused(&mut self, ctx: &mut ViewContext<Self>) {
        crate::workspace::reconnect_active_window_skill_manager_environment_if_stale(ctx);
        self.refresh_active_environment_inventory(ctx);
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SkillManagerPanelAction::EditSkill(path) => {
                ...
                if let Some(environment_scope) = Self::active_environment_runtime_scope(ctx) {
                    let Some(binding_session_id) = environment_scope.session_id else {
                        log::warn!(
                            "refusing to open environment skill path {path:?}: environment session is unavailable"
                        );
                        ctx.notify();
                        return;
                    };
                    let Some(host_id) = environment_scope.host_id else {
                        log::warn!(
                            "refusing to open environment skill path {path:?}: environment runtime is not connected"
                        );
                        ctx.notify();
                        return;
                    };
                    match warp_util::standardized_path::StandardizedPath::try_new(
                        &path.to_string_lossy(),
                    ) {
                        Ok(standardized_path) => {
                            ctx.emit(SkillManagerPanelEvent::OpenEnvironmentSkillFile {
                                environment_file_path: EnvironmentFilePath::new(
                                    host_id,
                                    standardized_path,
                                ),
                                binding_session_id,
                        ...
                        Err(error) => {
                            log::warn!("failed to open environment skill path {path:?}: {error}");
                        }
                    }
                } else {
...
impl View for SkillManagerPanel {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let inventory = self.active_inventory(app);
        let providers = Self::providers_in_inventory(&inventory);
        let query = self.query(app);
        let empty_message = self.empty_inventory_message(inventory.is_empty(), &query, app);
        let items = Self::filter_inventory(&inventory, &query, self.provider_filter);
        ...
        Container::new(
            Flex::column()
                ...
                .with_child(
                    Shrinkable::new(
                        1.0,
                        self.render_skill_list(&items, empty_message, appearance),
                    )
                    .finish(),
                )
                .finish(),
