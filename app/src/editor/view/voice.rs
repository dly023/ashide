...
impl EditorView {
        ...
        match *source {
            ...
            voice_input::VoiceInputToggledFrom::Key { state } => {
                ...
                match &self.voice_input_state {
                    ...
                    // to Ashide and let it go - we should NOT activate voice input in this case.
                    ...
                    // if the user is not focused on Ashide (since we lose the ability to listen to modifier
    ...
    fn show_microphone_access_toast(ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, move |toast_stack, ctx| {
            ...
            toast_stack.add_transient_toast(toast, active_window_id, ctx);
        });
    }
