...
/// Ashide: 某个环境目录（cwd）下真实子项的快照。
///
/// 由 daemon 的 `ListDirectory` RPC 返回的结果填充。终端链接检测器
/// 在环境会话里用它做精确校验:把 `ls -l` 整行候选子串里真正的文件名
/// 切出来 —— 这正是当前应用环境会话里 `fs::metadata` 存在性校验所起的作用。
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct EnvironmentDirListing {
    /// 该目录的绝对路径(环境 cwd)。
    pub dir: PathBuf,
    /// 目录下的直接子项:文件名 -> 是否为目录。
...
/// Ashide: 终端文件链接的校验来源。
///
/// 当前应用环境会话用当前应用文件系统 `fs::metadata` 判断路径是否存在；
/// Environment Runtime 会话的文件不在当前应用文件系统上，因此改用 helper
/// `ListDirectory` RPC 缓存下来的真实目录列表做精确校验。
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub enum LinkValidationContext {
    /// 当前应用环境会话：用当前应用文件系统校验路径是否真实存在。
    #[default]
    Local,
    /// Environment Runtime 会话：用缓存下来的环境 cwd 目录列表精确校验。
    ///
    /// `None` 表示该 cwd 的目录列表尚未缓存(异步拉取中或拉取失败),
    /// 此时本轮校验一律视为"无效"(不高亮),等列表到达后 re-render 再点亮。
    Environment(Option<Arc<EnvironmentDirListing>>),
}
...
fn is_path_valid(
    ...
    // Environment Runtime 会话的文件不在当前应用文件系统上，`fs::metadata` 必然失败。
    // 改用 helper `ListDirectory` 缓存下来的真实目录列表精确校验：候选解析
    // 路径有效 ⇔ 其父目录恰好等于缓存的 cwd 且其文件名是该目录下的已知子项。
    // 这给链接检测器的子串搜索提供了和当前应用文件系统 `fs::metadata` 等价的消歧依据,
    // 能从 `ls -l` 整行里准确切出真正的文件名。
    if let LinkValidationContext::Environment(listing) = validation_ctx {
...
/// Ashide: 判断一个已解析的环境路径是否指向目录。
///
/// 仅在环境会话点击链接、需要决定"打开文件 vs `cd` 进目录"时调用;
/// 依据是缓存下来的环境 cwd 目录列表。列表未缓存或路径不在其中时返回
/// `false`(按文件处理,与不缓存时的保守行为一致)。
pub fn environment_path_is_dir(path: &Path, listing: &EnvironmentDirListing) -> bool {
    let Some(file_name) = path.file_name().and_then(|n| n.to_str()) else {
        return false;
    };
    if path.parent() != Some(listing.dir.as_path()) {
