...
pub fn open_file_path_with_line_and_col(
    ...
    if full_path.is_file() {
        ...
        // Ashide channel.
        // When the resolved default is a different Ashide, open with the
...
fn is_ashide_bundle(bundle_id: &str) -> bool {
    AppId::parse(bundle_id)
        .map(|id| id.qualifier() == "dev" && id.organization() == "ashide")
        .unwrap_or(false)
}
