...
pub enum CustomAction {
    ...
    #[cfg(windows)]
    WindowsCopy,
    /// Also applies to the legacy AI panel (toggles the panel)
    NewAgentModePane,
    /// Also applies to the legacy AI panel (attaches the selection to the panel editor)
...
lazy_static! {
    ...
    /// know about particular Ashide-specific actions, so it deals with all actions
    /// as plain isizes. Within Ashide though we want to deal with them as the enum type.
