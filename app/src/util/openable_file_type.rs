//! File type detection utilities for determining if files can be opened in Ashide.
...
/// The type of file that can be opened in Ashide. The in-product treatment for "opening" a file
/// depends on its type.
...
pub enum FileTarget {
    /// Open in Ashide's Markdown viewer.
    MarkdownViewer(EditorLayout),
    /// Open in Ashide's Code Editor.
...
/// "Open with Ashide" is invoked from Finder/another app via a `file://` URL.
///
...
pub fn is_runnable_shell_script(path: &Path) -> bool {
    ...
    // A file `chmod 070` belongs to a group, not to the user invoking Ashide.
...
/// Determines if a file can be opened in Ashide and returns its type.
/// Returns `None` if the file is binary and should not be opened.
pub fn is_file_openable_in_ashide(path: &Path) -> Option<OpenableFileType> {
...
/// Only use this for UI elements that must explicitly open a file in Ashide (i.e. "Open in New Tab").
/// Prefer `resolve_file_target` for all other cases to respect users' preferences.
/// This would also force any binary file to be opened in Ashide's Code Editor, so you should likely check
/// `is_file_openable_in_ashide` before rendering any such UI Elements.
#[cfg(feature = "local_fs")]
pub fn resolve_file_target_to_open_in_ashide(
    path: &Path,
    settings: &EditorSettings,
    layout: Option<EditorLayout>,
) -> FileTarget {
    let openable_file_type = is_file_openable_in_ashide(path);
...
pub fn resolve_file_target_with_editor_choice(
    ...
    let is_openable_in_ashide = is_file_openable_in_ashide(path);
    let is_markdown = matches!(is_openable_in_ashide, Some(OpenableFileType::Markdown));
    let layout = layout.unwrap_or(default_layout);
    let is_openable_in_ashide = is_openable_in_ashide.is_some();
    ...
    // 2. Ashide Code Editor (Explicit user preference)
    if is_openable_in_ashide && matches!(editor_choice, EditorChoice::Ashide) {
    ...
    match editor_choice {
        ...
        EditorChoice::Ashide | EditorChoice::EnvEditor => unreachable!("Already matched above"),
    }
}
...
mod tests {
    ...
    fn test_binary_files_not_openable() {
        assert!(is_file_openable_in_ashide(Path::new("image.png")).is_none());
        assert!(is_file_openable_in_ashide(Path::new("video.mp4")).is_none());
        assert!(is_file_openable_in_ashide(Path::new("binary.exe")).is_none());
        assert!(is_file_openable_in_ashide(Path::new("archive.zip")).is_none());
    }

    #[test]
    #[cfg(feature = "local_fs")]
    fn test_open_code_panels_file_editor_default_is_ashide() {
    ...
    #[test]
    #[cfg(feature = "local_fs")]
    fn test_resolve_file_target_ashide_uses_default_layout() {
        let target = resolve_file_target_with_editor_choice(
            Path::new("data.txt"),
            EditorChoice::Ashide,
            true, /* prefer_markdown_viewer */
    ...
    fn test_markdown_files() {
        assert_eq!(
            is_file_openable_in_ashide(Path::new("README.md")),
            Some(OpenableFileType::Markdown)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("doc.markdown")),
            Some(OpenableFileType::Markdown)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("README")),
            Some(OpenableFileType::Markdown)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("CHANGELOG")),
            Some(OpenableFileType::Markdown)
        );
    }
    ...
    fn test_code_files() {
        assert_eq!(
            is_file_openable_in_ashide(Path::new("main.rs")),
            Some(OpenableFileType::Code)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("app.js")),
            Some(OpenableFileType::Code)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("script.py")),
            Some(OpenableFileType::Code)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("config.json")),
            Some(OpenableFileType::Code)
        );
    }
    ...
    fn test_text_files() {
        ...
        assert_eq!(
            is_file_openable_in_ashide(Path::new("data.txt")),
            Some(OpenableFileType::Text)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("data.csv")),
            Some(OpenableFileType::Text)
        );
        assert_eq!(
            is_file_openable_in_ashide(Path::new("file.svg")),
            Some(OpenableFileType::Text)
        );
    }
