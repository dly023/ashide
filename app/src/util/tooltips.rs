    ...
    let tooltip_element = if is_enterprise_secret_redaction_enabled {
        let tooltip_column = Flex::column()
            ...
            .with_child(
                appearance
                    .ui_builder()
                    .span(crate::t!("tooltip-secrets-stay-local"))
                    .with_style(UiComponentStyles {
                        font_size: Some(12.),
...
/// Returns whether "Open in Ashide" should be offered for the given file path.
///
/// This checks:
/// - Whether Ashide is already the default editor (skip if so)
/// - Whether this file is openable in Ashide (skips binary files and directories)
/// - Whether Ashide is an OS-level default editor (skips Markdown files)
#[cfg(feature = "local_fs")]
pub fn should_show_open_in_ashide_link(path: &Path, app: &AppContext) -> bool {
    use crate::{
    ...
    if matches!(*settings.open_file_editor, EditorChoice::Ashide) {
        return false;
    }
...
pub fn should_show_open_in_ashide_link(_path: &std::path::Path, _app: &AppContext) -> bool {
    false
}
