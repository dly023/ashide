//! Fluent-based localization layer for Ashide Desktop.
//!
//! 加载链:
...
mod tests {
    ...
    fn fallback_chain_works() {
        // 用一个**本地** loader 验证 fallback,而不是去动进程级的 `LANGUAGE_LOADER`
        // OnceLock:后者是 init-once 的,若别的测试先 init 成 en,这里的
        // `init(Some("zh-CN"))` 就成了 no-op,且并行下还会污染其它测试的 locale。
        // 这里复刻 init() 内部的构造流程,完全隔离。
        let loader = fluent_language_loader!();
        loader
            .load_fallback_language(&Localizations)
            .expect("load en fallback bundle");
        let zh: LanguageIdentifier = "zh-CN".parse().unwrap();
        i18n_embed::select(&loader, &Localizations, &[zh]).expect("select zh-CN");
        // common-ok 中文已译
    ...
    // SSTAB-007 copy-id menu item: both the en fallback and zh-CN bundles must
    // resolve the keys, otherwise the context menu renders
    // `No localization for id: "workspace-..."` (observed when running a build
    // compiled before the ftl entries were added).
    #[test]
    fn session_copy_id_keys_resolve_in_en_and_zh() {
        let loader = fluent_language_loader!();
        loader
            .load_fallback_language(&Localizations)
            .expect("load en fallback bundle");
        // English fallback must have both keys.
        assert_eq!(
            loader.get("workspace-session-navigator-menu-copy-id"),
            "Copy session ID"
        );
        let en_toast = loader.get("workspace-session-navigator-menu-copy-id-toast");
        assert!(
            en_toast.contains("Copied") && en_toast.contains("clipboard"),
            "en toast should mention copied+clipboard, got: {en_toast:?}"
        );

        // zh-CN must override the label.
        let zh: LanguageIdentifier = "zh-CN".parse().unwrap();
        i18n_embed::select(&loader, &Localizations, &[zh]).expect("select zh-CN");
        assert_eq!(
            loader.get("workspace-session-navigator-menu-copy-id"),
