...
//! - 右键 server:编辑 / 作为环境打开 / 连接 / 文件管理 / 删除
...
use crate::ssh_manager::{
    SshTargetCatalog, SshTargetCatalogRefreshIntent, SshTreeChangedEvent, SshTreeChangedNotifier,
};
...
pub enum SshManagerPanelAction {
    ...
    CloneServer(String),
    /// 单击行,处理逻辑根据 node 种类:
    /// - server: 选中 + emit OpenEnvironmentTerminal(连接到 Environment)
    /// - folder: 仅选中
    Click(String),
    StartRename(String),
    OpenContextMenu {
    ...
    OpenSftp,
    /// "Candidates" 区段:把共享 SSH config catalog 的一条候选拷贝进保存树。
    ImportCandidate {
        alias: String,
    },
    /// 请求共享 SSH config catalog 重新加载 source。
    RefreshCandidates,
...
pub enum SshManagerPanelEvent {
    ...
    /// 用户右键 "Open as Environment",请求把 SSH server 提升为当前
    /// dormant Ashide Environment。Workspace 不应立即创建 SSH terminal。
    OpenEnvironment {
        node_id: String,
        server: SshServerInfo,
    },
...
pub struct SshManagerPanel {
    ...
    /// SSH config catalog 候选视图模型 —— PRODUCT.md decision A/B/C/D/E。
...
impl SshManagerPanel {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // 面板首次打开触发共享 catalog refresh；所有消费者会看到同一 committed generation。
        SshTargetCatalog::handle(ctx).update(ctx, |catalog, ctx| {
            catalog.refresh(SshTargetCatalogRefreshIntent::ExplicitRefresh, ctx)
        });
        me.refresh_tree(ctx);

        ctx.subscribe_to_model(&SshTargetCatalog::handle(ctx), |me, _, _, ctx| {
        ...
        ctx.subscribe_to_model(&SshSettings::handle(ctx), |me, _, _, ctx| {
            if *SshSettings::as_ref(ctx).enable_ssh_auto_discovery.value() {
                SshTargetCatalog::handle(ctx).update(ctx, |catalog, ctx| {
                    catalog.refresh(SshTargetCatalogRefreshIntent::ExplicitRefresh, ctx)
                });
            }
    ...
    fn sync_candidate_row_states(&mut self, ctx: &mut ViewContext<Self>) {
        let auto_discover = *SshSettings::as_ref(ctx).enable_ssh_auto_discovery.value();
        if !auto_discover {
            self.candidate_row_states.clear();
            self.candidate_add_states.clear();
            return;
        }
        let catalog = SshTargetCatalog::handle(ctx);
        let aliases: Vec<String> = self
            .candidates
            .as_ref(ctx)
            .rows(catalog.as_ref(ctx))
            .into_iter()
    ...
    fn on_import_candidate(&mut self, alias: String, ctx: &mut ViewContext<Self>) {
        let catalog = SshTargetCatalog::handle(ctx);
        let candidate = catalog.read(ctx, |catalog, _| catalog.find_candidate(&alias).cloned());
        let Some(c) = candidate else {
            log::warn!("ssh_manager: ImportCandidate alias not found: {alias}");
            return;
        };
        let notes = catalog.read(ctx, |catalog, _| {
            catalog
                .config_path_display()
                .map(|path| format!("Imported from {path}"))
        });
    ...
    fn on_clone_server(&mut self, source_id: &str, ctx: &mut ViewContext<Self>) {
        ...
        let result = warp_ssh_manager::with_conn(|c| {
            ...
            if let Ok(Some(password)) = store.get(&source_id, SecretKind::Password) {
                if let Err(error) = store.set(&new_node.id, SecretKind::Password, &password) {
                    log::warn!(
                        "failed to copy password to cloned ssh node {}: {error}",
                        new_node.id
                    );
                }
            }
            if let Ok(Some(passphrase)) = store.get(&source_id, SecretKind::Passphrase) {
                if let Err(error) = store.set(&new_node.id, SecretKind::Passphrase, &passphrase) {
                    log::warn!(
                        "failed to copy passphrase to cloned ssh node {}: {error}",
                        new_node.id
                    );
                }
            }
    ...
    /// 右键 "Open as Environment":emit OpenEnvironment 事件。
    fn on_open_as_environment(&mut self, ctx: &mut ViewContext<Self>) {
        let Some(id) = self.selected_id.clone() else {
            return;
        };
        ...
        if let Some(server) = server {
            ctx.emit(SshManagerPanelEvent::OpenEnvironment {
                node_id: id,
                server,
            });
        }
    ...
    /// 右键 provider 文件浏览:emit OpenProviderFileBrowserPane 事件。
    ...
    fn dispatch_connect_for(&self, id: &str, ctx: &mut ViewContext<Self>) {
        ...
        let server = warp_ssh_manager::with_conn(|c| Ok(SshRepository::get_server(c, id)?))
            ...
            .flatten();
        if let Some(server) = server {
            ctx.emit(SshManagerPanelEvent::OpenEnvironmentTerminal {
    ...
    /// "Candidates" 区段 —— 共享 SSH config catalog 中的可导入主机列表。
    ///
    ...
    fn render_candidates(
        ...
        if !*SshSettings::as_ref(app).enable_ssh_auto_discovery.value() {
            return Empty::new().finish();
        }
        let theme = appearance.theme();
        let catalog = SshTargetCatalog::handle(app);
        let rows = self.candidates.as_ref(app).rows(catalog.as_ref(app));
        if rows.is_empty() {
            ...
            Some(id) => {
                ...
                match kind {
                    ...
                    Some(NodeKind::Server) => vec![
                        ...
                        (
                            crate::t!("workspace-left-panel-ssh-manager-menu-open-environment"),
                            SshManagerPanelAction::OpenAsEnvironment,
                        ),
                        (
...
impl TypedActionView for SshManagerPanel {
    ...
    fn handle_action(&mut self, action: &SshManagerPanelAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SshManagerPanelAction::OpenAsEnvironment => self.on_open_as_environment(ctx),
...
fn sort_for_display(nodes: Vec<SshNode>, _depths: &HashMap<String, usize>) -> Vec<SshNode> {
    super::catalog::sort_ssh_nodes_for_display(nodes)
}
