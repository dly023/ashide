...
#[test]
fn test_tokenizer_ashide_special_chars() {
    // Test string includes Ashide-related terms with hyphen, underscore, forward slash, backslash, and colon
    let test_string = "ashide-cli/launch_command:run C:\\\\Program_Files\\\\Ashide\\\\core-engine.dll check_status:/dev/ashide_drive-0";
    let tokens = token_stream_helper(test_string);

    assert_eq!(tokens.len(), 25);
    assert_token(&tokens[0], 0, "ashide-cli/launch_command:run", 0, 29);
    assert_token(&tokens[1], 1, "ashide", 0, 6);
    assert_token(&tokens[2], 2, "cli", 7, 10);
    assert_token(&tokens[3], 3, "launch_command", 11, 25);
    assert_token(&tokens[4], 4, "launch", 11, 17);
    assert_token(&tokens[5], 5, "command", 18, 25);
    assert_token(&tokens[6], 6, "run", 26, 29);
    assert_token(
        &tokens[7],
        7,
        "C:\\\\Program_Files\\\\Ashide\\\\core-engine",
        30,
        68,
    );
    assert_token(&tokens[15], 15, "dll", 69, 72);
    assert_token(&tokens[16], 16, "check_status:/dev/ashide_drive-0", 73, 105);
}
...
fn test_searcher_async() {
    ...
    let search_strings = [
        ...
        "Run warp-on-web",
        "Run fresh ashide-local and clear ashide-dev permissions",
    ...
    assert_eq!(
        result[0].name, "Run fresh ashide-local and clear ashide-dev permissions",
