...
cfg_if::cfg_if! {
    if #[cfg(feature = "local_fs")] {
        ...
        use repo_metadata::current_app_model::GetContentsArgs;
...
pub struct FileSearchModel {
    ...
    repo_contents_cache: RefCell<HashMap<StandardizedPath, Arc<Vec<FileSearchResult>>>>,
}
...
impl FileSearchModel {
        ...
        ctx.subscribe_to_model(
            ...
            |me, event, _ctx| match event {
                RepoMetadataEvent::FileTreeUpdated { ids } => {
                    for id in ids {
                        me.invalidate_cached_repository(id);
                    }
                }
                ...
                | RepoMetadataEvent::DirectoryLoadFinished { id, .. } => {
                    me.invalidate_cached_repository(id);
                }
    ...
    #[cfg(feature = "local_fs")]
    fn invalidate_cached_repository(&self, id: &repo_metadata::RepositoryIdentifier) {
        let repo_metadata::RepositoryIdentifier::Local(path) = id else {
            return;
        };
        self.repo_contents_cache.borrow_mut().remove(path);
    }
    ...
    /// Gets repository contents (files and directories) from the RepoMetadataModel for the current working directory.
    ...
    pub fn get_repo_contents(&self, app: &AppContext) -> Arc<Vec<FileSearchResult>> {
        ...
        let repo_metadata::RepositoryIdentifier::Local(repo_root) = &id else {
            return Arc::new(Vec::new());
        };
        ...
        self.repo_contents_cache
            .borrow_mut()
            .insert(repo_root.clone(), arc.clone());
        arc
    }
    ...
    /// Gets repository contents from the RepoMetadataModel for the current working directory (WASM stub)
    #[cfg(not(feature = "local_fs"))]
            ...
            contents
                ...
                .filter_map(|content| match content {
                    repo_metadata::RepoContent::File(file_metadata) => {
                        Self::repo_search_result(&file_metadata.path, repo_root, false)
                    }
                    repo_metadata::RepoContent::Directory(dir_entry) => {
                        Self::repo_search_result(&dir_entry.path, repo_root, true)
                    }
                    repo_metadata::RepoContent::Symlink(symlink) => {
                        let is_directory =
                            symlink.target_kind == repo_metadata::SymlinkTargetKind::Directory;
                        Self::repo_search_result(&symlink.path, repo_root, is_directory)
                    }
                })
                .collect()
        } else {
    ...
    #[cfg(feature = "local_fs")]
    fn repo_search_result(
        entry_path: &StandardizedPath,
        repo_root: &StandardizedPath,
        is_directory: bool,
        ...
        let mut path = entry_path.strip_prefix(repo_root)?.to_owned();
        if is_directory && !path.ends_with(std::path::MAIN_SEPARATOR) {
            path.push(std::path::MAIN_SEPARATOR);
        }
        Some(FileSearchResult {
            path,
            project_directory: repo_root.as_str().to_owned(),
            is_directory,
        })
    }
