...
mod file_search_model_tests {
    ...
    #[cfg(unix)]
    #[test]
    fn repo_search_result_preserves_lexical_symlink_root() {
        use std::os::unix::fs::symlink;
        use warp_util::standardized_path::StandardizedPath;

        let temp = tempfile::tempdir().unwrap();
        let target = temp.path().join("target-repo");
        let lexical_root = temp.path().join("repo-link");
        std::fs::create_dir(&target).unwrap();
        symlink(&target, &lexical_root).unwrap();

        let root = StandardizedPath::try_from_local(&lexical_root).unwrap();
        let entry = StandardizedPath::try_from_local(&lexical_root.join("dir-link")).unwrap();
        let result = FileSearchModel::repo_search_result(&entry, &root, true).unwrap();

        assert_eq!(
            result.path,
            format!("dir-link{}", std::path::MAIN_SEPARATOR)
        );
        assert_eq!(result.project_directory, lexical_root.to_string_lossy());
        assert_ne!(result.project_directory, target.to_string_lossy());
        assert!(result.is_directory);
    }

    #[test]
    fn completed_tree_mutations_invalidate_cached_repo_contents() {
        ...
        App::test((), |mut app| async move {
            ...
            let repo_path = std::env::temp_dir().join("ashide-file-search-cache-repo");
            let standardized = StandardizedPath::try_from_local(&repo_path).unwrap();
            let id = RepositoryIdentifier::local(standardized.clone());
            let seed_cache = |app: &mut App| {
                FileSearchModel::handle(app).update(app, |model, _ctx| {
                    model
                        .repo_contents_cache
                        .borrow_mut()
                        .insert(standardized.clone(), Arc::new(Vec::new()));
                });
            };
            let cache_contains_repo = |app: &mut App| {
                FileSearchModel::handle(app).read(app, |model, _ctx| {
                    model
                        .repo_contents_cache
                        .borrow()
                        .contains_key(&standardized)
                })
            };

            seed_cache(&mut app);
            RepoMetadataModel::handle(&app).update(&mut app, |_model, ctx| {
                ctx.emit(RepoMetadataEvent::FileTreeEntryUpdated { id: id.clone() });
            });
            assert!(!cache_contains_repo(&mut app));

            seed_cache(&mut app);
            RepoMetadataModel::handle(&app).update(&mut app, |_model, ctx| {
                ctx.emit(RepoMetadataEvent::DirectoryLoadFinished {
                    id,
                    dir_path: standardized.clone(),
                    succeeded: true,
                });
            });
            assert!(!cache_contains_repo(&mut app));
