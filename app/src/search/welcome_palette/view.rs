...
use crate::search::command_palette::{launch_config, local_drive, CommandPaletteMixer};
...
impl WelcomePalette {
    pub fn new(
        ...
        let mixer = ctx.add_model(|ctx| {
            ...
            mixer.add_sync_source(local_drive_data_source.clone(), HashSet::new());
            ...
            CommandPaletteItemAction::ExecuteWorkflow { id } => {
                ...
                self.dispatch_typed_action_on_view(
                    &WorkspaceAction::RunWorkflow {
                        workflow: Arc::new(WorkflowType::ObjectStore(Box::new(workflow.clone()))),
