...
fn conversation_matches_active_environment_scope(
    conversation: &ConversationNavigationData,
    app: &AppContext,
) -> bool {
    ...
    // Persisted/historical conversation metadata is current-app scoped today:
    // it has cwd strings but no Environment authority. While an Environment
    // Runtime session is active, only show live conversations whose terminal
    // view is also runtime-backed; otherwise command-palette navigation can
    // jump from the selected Environment back into current-app history.
    let Some(window_id) = conversation.window_id else {
        return false;
    };
    let Some(terminal_view_id) = conversation.terminal_view_id else {
        return false;
    };
    app.view_with_id::<TerminalView>(window_id, terminal_view_id)
        .and_then(|terminal_view| {
            terminal_view
                .as_ref(app)
                .active_session_uses_environment_runtime(app)
        })
        .unwrap_or(false)
}
...
impl FuzzyConversationSearcher {
    ...
    pub fn searchable_conversations(&self, app: &AppContext) -> Vec<ConversationNavigationData> {
        let conversations = match self.filter {
        ...
        conversations
            .into_iter()
            .filter(|conversation| conversation_matches_active_environment_scope(conversation, app))
            .collect()
    }
}
