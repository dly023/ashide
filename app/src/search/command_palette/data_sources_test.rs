...
use crate::object_store::ids::ObjectStoreId::{self};
use crate::object_store::{Owner, StoredObjectMetadata, StoredObjectPermissions};
...
fn mock_workflow(id: WorkflowId, owner: Owner) -> WorkflowObject {
    let object_store_id = ObjectStoreId::StableId(id.into());
    WorkflowObject::new(
        object_store_id,
...
fn test_drive_data_source_correctly_filters_drive_filter() {
    App::test((), |mut app| async move {
        ...
        let data_source_handle = app.add_model(local_drive::DataSource::new);
