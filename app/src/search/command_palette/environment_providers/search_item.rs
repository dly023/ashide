...
#[derive(Debug)]
pub struct EnvironmentProviderSearchItem {
    pub target: EnvironmentProviderTarget,
    /// 显示用的 provider detail。
    pub detail: String,
    /// 节点名(用作主标签)。
...
impl EnvironmentProviderSearchItem {
    pub fn new(target: EnvironmentProviderTarget, detail: String, display_name: String) -> Self {
        Self {
            target,
            detail,
            display_name,
    ...
    fn render_label(
        ...
        // `format!("{display_name} {detail}")`(单空格)的索引。combined 用
        // 双空格分隔,索引会偏。我们重新只在 display_name 部分做高亮,detail
        // 部分作为附属信息单独画(更直观)。
...
impl SearchItem for EnvironmentProviderSearchItem {
