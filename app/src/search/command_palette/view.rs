...
use crate::workspace::{
    active_terminal_in_window, ActiveSession, ForkedConversationDestination, WorkspaceAction,
};
use warpui::elements::{
...
pub enum Event {
    ...
    /// View the relevant object in the Ashide Drive sidebar.
    ViewInLocalDrive {
        id: ObjectTypeAndId,
    },
    ...
    /// Open an Environment Runtime file via buffer-sync.
    OpenEnvironmentFile {
        host_id: HostId,
    ...
    /// Change directory inside the active Environment Runtime terminal.
...
impl TypedActionView for View {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            Action::Close => self.close(ctx, false),
...
impl View {
    ...
    #[cfg(feature = "integration_tests")]
    pub fn selected_binding_description(&self, app: &AppContext) -> Option<String> {
        let selected_result = self.search_bar_state.as_ref(app).selected_result()?;
        match selected_result.accept_result() {
            CommandPaletteItemAction::AcceptBinding { binding } => Some(
                binding
                    .description
            ...
            | CommandPaletteItemAction::ViewInLocalDrive { .. }
            ...
            | CommandPaletteItemAction::OpenEnvironmentDirectory { .. }
            ...
            | CommandPaletteItemAction::OpenEnvironmentProviderTerminal { .. }
            | CommandPaletteItemAction::NoOp => None,
        }
    }
    ...
    pub fn is_mode_enabled(&self, mode: PaletteMode, app: &AppContext) -> bool {
        ...
        matches!(
            ...
            (PaletteMode::Command, QueryFilter::Actions)
                ...
                | (PaletteMode::LocalDrive, QueryFilter::Drive)
        )
    }
    ...
    fn close(&mut self, ctx: &mut ViewContext<Self>, accepted_action: bool) {
        ...
        if ctx.root_view_id(ctx.window_id()).is_some() {
            ctx.emit(Event::Close { accepted_action });
        }
    }
    ...
    fn handle_result_accepted(
        ...
        match result_action.clone() {
            ...
            CommandPaletteItemAction::ActivateRestoredWorkspaceSession { target } => {
            ...
            CommandPaletteItemAction::ViewInLocalDrive { id } => {
                ctx.emit(Event::ViewInLocalDrive { id })
            }
            ...
            CommandPaletteItemAction::OpenEnvironmentProviderTerminal { target } => {
                self.dispatch_typed_action_on_view(
                    &crate::workspace::WorkspaceAction::OpenEnvironmentRuntimeTerminal { target },
                    ctx,
                );
            }
            CommandPaletteItemAction::OpenFile {
                ...
                if active_window_uses_environment_runtime(ctx) {
                    log::warn!(
                        "Ignoring stale current-app command-palette OpenFile while an Environment Runtime session is active: {project_directory}/{path}"
                    );
                    return;
                }
            ...
            CommandPaletteItemAction::OpenEnvironmentFile {
                ...
                ctx.emit(Event::OpenEnvironmentFile {
                    host_id,
                    session_id,
                    path,
                    line_and_column_arg,
            ...
            CommandPaletteItemAction::OpenDirectory {
                ...
                if active_window_uses_environment_runtime(ctx) {
                    log::warn!(
                        "Ignoring stale current-app command-palette OpenDirectory while an Environment Runtime session is active: {project_directory}/{path}"
                    );
                    return;
                }
                let absolute_path = std::path::Path::new(&project_directory)
            ...
            CommandPaletteItemAction::OpenEnvironmentDirectory {
                ...
                ctx.emit(Event::OpenEnvironmentDirectory {
                    host_id,
                    session_id,
                    path,
                });
            ...
            CommandPaletteItemAction::CreateFile {
                ...
                if active_window_uses_environment_runtime(ctx) {
                    log::warn!(
                        "Ignoring stale current-app command-palette CreateFile while an Environment Runtime session is active: {current_directory}/{file_name}"
                    );
                    return;
                }
