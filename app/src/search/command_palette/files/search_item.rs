...
pub struct FileSearchItem {
    ...
    pub environment_host_id: Option<HostId>,
    pub environment_session_id: Option<SessionId>,
}
    ...
    fn accept_result(&self) -> Self::Action {
        if self.is_directory {
            if let (Some(host_id), Some(session_id)) =
                (&self.environment_host_id, self.environment_session_id)
            {
                let path = std::path::Path::new(&self.project_directory)
                    .join(&self.path)
                    .to_string_lossy()
                    .to_string();
        ...
        } else if let (Some(host_id), Some(session_id)) =
            (&self.environment_host_id, self.environment_session_id)
        {
            let path = std::path::Path::new(&self.project_directory)
                .join(&self.path)
