...
fn render_session_label(
    ...
    } else if let Some(label) = &session.prompt_elements().display_label {
        // Restored / indexed / historical sessions have no live prompt grid or
        // chip snapshot; render their display label as a plain styled text span
        // so the row is legible and consistent with the Session Navigator.
        render_prompt_display_label(label, appearance)
    } else {
...
fn render_prompt_display_label(label: &str, appearance: &Appearance) -> Box<dyn Element> {
    appearance
        .ui_builder()
