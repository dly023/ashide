...
fn collect_search_sessions(app: &AppContext) -> Vec<SessionNavigationData> {
    ...
    let live_keys: HashSet<String> = live
        ...
        .map(|s| {
            format!(
                "tab::{}::{}",
                s.pane_view_locator().pane_group_id, s.pane_view_locator().pane_id
            )
        })
        .collect();
    ...
    for window_id in app.window_ids().collect::<Vec<_>>() {
        ...
        for workspace_handle in workspace_handles.into_iter() {
            ...
            let snapshots: Vec<WorkspaceSessionSnapshot> =
                workspace.workspace_session_snapshots_for_search(app);
...
impl SessionSearcher for FuzzySessionSearcher {
    fn search(
        ...
        let all_sessions = {
            let mut s = collect_search_sessions(app);
            s.sort_by_key(|x| x.last_focus_ts());
            s
        };
...
mod full_text_searcher {
    ...
    impl SessionSearcher for FullTextSessionSearcher {
        fn search(
            ...
            let documents =
                super::collect_search_sessions(app)
                    .into_iter()
                    .enumerate()
                    .map(|(idx, session)| {
