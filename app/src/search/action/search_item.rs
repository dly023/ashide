...
impl SearchItemIcon for BindingGroup {
    ...
    fn icon_color(&self, appearance: &Appearance) -> ColorU {
        match self {
            ...
            Self::AiAssistant if !FeatureFlag::AgentMode.is_enabled() => {
                ColorU::from_u32(colors::WARP_AI)
            }
            Self::AiAssistant => appearance.theme().foreground().into_solid(),
            Self::Workflow => local_drive_icon_color(appearance, DriveObjectType::Workflow),
            Self::Notebooks => local_drive_icon_color(
                appearance,
