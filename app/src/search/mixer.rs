...
impl<T: Action + Clone> SearchMixer<T> {
    ...
    pub fn add_async_source(
        ...
        self.run_current_query_for_source_if_matching(data_source_id, ctx);
    }
    ...
    fn run_current_query_for_source_if_matching(
        &mut self,
        data_source_id: DataSourceId,
        ctx: &mut ModelContext<Self>,
    ) {
        let matches_current_query = self.query.as_ref().is_some_and(|query| {
            self.sources
                .get(&data_source_id)
                .is_some_and(|source| source.matches_query(query))
        });
        if !matches_current_query {
            return;
        }

        let reopened_committed_query = if self.pending_results.is_none() {
            self.pending_results = Some(self.results.clone());
            self.initial_results_emitted = false;
            ctx.emit(SearchMixerEvent::ResultsChanged);
            true
        } else {
        ...
        self.run_query_internal(data_source_id, false, ctx);

        if reopened_committed_query && self.pending_results.is_some() && self.is_loading() {
            self.schedule_initial_results_timeout(ctx);
        }
    }
    ...
    /// Returns whether the visible result set belongs to the current query rather than the
    /// previous query that remains visible while matching sources are still loading.
    pub fn has_committed_current_query_results(&self) -> bool {
        self.query.is_some() && self.initial_results_emitted
    }
    ...
    fn schedule_initial_results_timeout(&self, ctx: &mut ModelContext<Self>) {
        let query_generation = self.query_generation;
        let _ = ctx.spawn(
