...
use crate::search::ai_context_menu::files::data_source::{
    file_data_source_for_active_directory, file_data_source_for_current_repo,
};
...
impl AIContextMenu {
    ...
    pub(crate) fn get_categories_for_mode(
        ...
        let show_local_drive = LocalDriveSettings::is_local_drive_enabled(app);

        // 只计算一次,CLI agent、AI 模式和 terminal 模式都会用到。
        // Environment Runtime 会话不能拿远端 `/root/...` 去问本机 repo detector;
        // 在 runtime-backed repo metadata 接入本菜单前,远端只暴露当前目录文件分类。
        let active_session_uses_environment_runtime = {
        ...
        let is_active_dir_in_git_repo = {
            ...
            #[cfg(not(target_family = "wasm"))]
            {
                if active_session_uses_environment_runtime {
                    false
                } else {
                    let active_window_id = app.windows().state().active_window;
                    let active_dir = active_window_id.and_then(|window_id| {
                        ActiveSession::as_ref(app).current_app_path(window_id)
                    });
        ...
        if is_ai_or_autodetect_mode {
            ...
            if show_local_drive && FeatureFlag::DriveObjectsAsContext.is_enabled() {
                categories.push(AIContextMenuCategory::Workflows);
                categories.push(AIContextMenuCategory::Notebooks);
                categories.push(AIContextMenuCategory::Plans);
            }
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Subscribe to repository detection so categories update when a git repo is found.
        #[cfg(not(target_family = "wasm"))]
            ...
            NavigationState::Category(AIContextMenuCategory::CurrentFolderFiles) => {
                self.mixer.update(ctx, |mixer, ctx| {
                    mixer.add_async_source(
                        file_data_source_for_active_directory(ctx),
                        [QueryFilter::Files],
