...
impl ConversationDataSource {
    /// Merges persisted user conversations and ambient-agent tasks, deduplicated by
    ...
    fn collect_conversations(app: &AppContext) -> Vec<ConversationContextItem> {
        ...
        let active_session_uses_environment_runtime = app
            .windows()
            .active_window()
        ...
        // Source 1: persisted + historical conversations (excludes ambient agent conversations).
        // These rows are current-app conversation metadata today: their working
        // directory fields have no Environment authority, and the server token
        // alone does not identify which runtime should own restored file/tool
        // context. In an Environment Runtime session, do not offer them as
        // @conversation context until the persisted metadata carries an
        // Environment key.
        ...
        // corresponding task, so this covers conversations without persisted task state.
