...
use super::{
    ai_assistant::AiAssistantDataSource,
...
impl CommandSearchView {
    ...
    fn selected_result_renderer<'a>(
        ...
        if !mixer.has_committed_current_query_results() {
            return None;
        }
...
impl CommandSearchView {
    ...
    pub fn has_committed_results(&self, ctx: &AppContext) -> bool {
        self.mixer.as_ref(ctx).has_committed_current_query_results()
            && self
                .search_bar_state
                .as_ref(ctx)
                .query_result_renderers()
