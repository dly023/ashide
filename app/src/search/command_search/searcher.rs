...
///
/// Cloud workflows carry only a `ObjectStoreId` so the handler can resolve the full
...
pub enum CommandSearchItemAction {
    ...
    /// The user accepted the search item to open Ashide AI.
    OpenAiAssistant,

    /// The user accepted the search item to translate the query to a command using Ashide AI.
    TranslateUsingAiAssistant,
}
