...
use crate::drive::settings::{LocalDriveSettings, LocalDriveSettingsChangedEvent};
...
impl CommandSearchZeroStateView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        ctx.subscribe_to_model(&LocalDriveSettings::handle(ctx), |_, _, event, ctx| {
            if let LocalDriveSettingsChangedEvent::EnableLocalDrive { .. } = event {
                ctx.notify();
            }
        });
...
fn valid_query_filters(app: &AppContext) -> Vec<QueryFilter> {
    ...
    if LocalDriveSettings::is_local_drive_enabled(app) {
