...
#[test]
fn test_async_source_added_after_query_runs_current_matching_query() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let mixer = app.add_model(|_| SearchMixer::<TestAction>::new());
        mixer.update(&mut app, |mixer, ctx| {
            mixer.run_query(
                Query {
                    text: "history command".to_string(),
        ...
        app.read(|app| {
            let mixer = mixer.as_ref(app);
            assert!(mixer.is_loading());
            assert!(!mixer.has_committed_current_query_results());
            assert!(mixer.results().is_empty());
        });
...
#[test]
fn test_async_source_added_after_commit_preserves_results_but_reopens_query_readiness() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let mixer = app.add_model(|_| SearchMixer::<TestAction>::new());
        mixer.update(&mut app, |mixer, ctx| {
