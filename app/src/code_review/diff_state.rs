...
impl FileDiff {
    ...
    pub fn requires_detached_buffer(&self) -> bool {
        self.is_symlink || matches!(self.status, GitFileStatus::Deleted)
    }
...
impl DiffStateModel {
    ...
    fn handle_updated_metadata_for_repo(
        ...
        match metadata {
            ...
            Err(err) => {
                log::debug!("Failed to refresh code review metadata: {err}");
                self.metadata = None;
            }
        }
    ...
    async fn num_lines_in_file_if_non_binary(file_path: &Path) -> Result<Option<usize>> {
        ...
        if metadata.file_type().is_symlink() {
            ...
            return Ok(Some(Self::symlink_target_line_count(
    ...
    fn symlink_target_line_count(target: &str) -> usize {
        target.split_terminator('\n').count().max(1)
    }

    fn read_symlink_target(repo_path: &Path, file_path: &Path) -> Result<Option<PathBuf>> {
        ...
        std::fs::read_link(full_path).map(Some).map_err(Into::into)
    }

    fn new_symlink_diff_output(target: &Path) -> String {
        let target = target.to_string_lossy();
        let added_lines = target
            .split_terminator('\n')
    ...
    async fn get_file_diff(
        ...
        let symlink_target = Self::read_symlink_target(repo_path, file_path)?;
        let is_symlink = symlink_target.is_some();
        let symlink_diff_output = status
            .is_new_file()
            .then(|| symlink_target.as_deref().map(Self::new_symlink_diff_output))
            .flatten();
        ...
        let diff_output = if let Some(output) = symlink_diff_output {
            output
        } else {
            log::debug!(
            ...
            match run_git_command(repo_path, &diff_args).await {
                Ok(output) => output,
                Err(error) => {
                    log::info!(
