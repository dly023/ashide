...
use crate::{
    ...
    code::{
        current_app_code_editor::{
            render_unsaved_circle_with_tooltip, CurrentAppCodeEditorEvent, CurrentAppCodeEditorView,
        },
        editor::{
...
use crate::{
    ...
    terminal::view::{TerminalAction, TerminalView},
...
impl LoadedState {
    ...
    fn editor_absolute_file_paths(
        &self,
        repo_path: &Path,
    ) -> Vec<(ViewHandle<CurrentAppCodeEditorView>, PathBuf)> {
        self.file_states
            .values()
...
impl CodeReviewView {
    ...
    fn clear_comment_locations(
        &self,
        editor_file_paths: &[(ViewHandle<CurrentAppCodeEditorView>, PathBuf)],
        ctx: &mut ViewContext<Self>,
    ) {
        for (editor, _) in editor_file_paths {
            editor.update(ctx, |current_file_editor, ctx| {
                current_file_editor
                    .editor()
    ...
    /// `DiffSelectorEvent::SelectMode`: updates the diff state model.
    ...
    fn update_search_decorations(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        for file_state in state.file_states.values() {
            ...
            editor_state.editor.update(ctx, |current_file_editor, ctx| {
    ...
    fn handle_edit_comment(&mut self, comment_id: &CommentId, ctx: &mut ViewContext<Self>) {
        ...
        match &comment.target {
            AttachedReviewCommentTarget::Line {
                ...
                editor_state
                    .editor()
                    .update(ctx, |current_file_editor, ctx| {
                        current_file_editor.editor().update(ctx, |editor, ctx| {
            ...
            #[cfg(not(target_family = "wasm"))]
            {
                ...
                ctx.subscribe_to_view(
                    ...
                    move |view, editor_view, event, ctx| {
                        if let CurrentAppCodeEditorEvent::ViewportUpdated = event {
    ...
    fn build_view_state_for_file_diffs(
        ...
        for file in files {
            let editor_state = {
                #[cfg(not(target_family = "wasm"))]
                {
                    if file.file_diff.requires_detached_buffer() {
    ...
    fn session_env(&self, app: &AppContext) -> Option<GitSessionState> {
        ...
        terminal_view.read(app, |terminal, ctx| {
            ...
            let uses_environment_runtime = terminal
                .active_session_uses_environment_runtime(ctx)
                .unwrap_or(false);
            let is_wsl = session.as_ref().map(|s| s.is_wsl()).unwrap_or(false);

            let enablement = if uses_environment_runtime {
                CodingPanelEnablementState::RuntimeSession {
                    has_environment_runtime: false,
                }
            } else if is_wsl {
    ...
    fn render_no_repo_for_env(
        ...
        match self.session_env(app) {
            ...
            {
                self.render_environment_runtime_state_with_buttons(appearance)
            }
            Some(state)
                if matches!(
                    state.enablement,
    ...
    fn create_code_review_model_with_global_buffer(
        ...
        if file.file_diff.is_binary || file.content_at_head.is_none() {
            ...
            let local_code_view = ctx.add_typed_action_view(|ctx| {
                let editor = CurrentAppCodeEditorView::new_with_global_buffer(
                    &full_file_path,
                    |buffer_state, ctx| {
            ...
            ctx.subscribe_to_view(&local_code_view, {
                ...
                move |me, editor, event, ctx| {
                    me.handle_current_app_code_editor_events(
                        editor,
                        event,
                        &full_file_path,
                        &diff_file_path,
        ...
        if file.file_diff.is_binary {
            ...
            let local_code_view = ctx.add_typed_action_view(|ctx| {
                let mut local_code_view =
                    CurrentAppCodeEditorView::new(code_editor_view, None, false, None, ctx);
    ...
    fn handle_current_app_code_editor_events(
        &mut self,
        editor: ViewHandle<CurrentAppCodeEditorView>,
        event: &CurrentAppCodeEditorEvent,
        full_file_path: &Path,
        diff_file_path: &Path,
        ctx: &mut ViewContext<Self>,
    ) {
        match event {
            ...
            CurrentAppCodeEditorEvent::FailedToSave { error } => {
            ...
            CurrentAppCodeEditorEvent::FailedToLoad { error } => {
            ...
            CurrentAppCodeEditorEvent::DiscardUnsavedChanges { path: _path } => {
                #[cfg(feature = "local_fs")]
                GlobalBufferModel::handle(ctx).update(ctx, |global_buffer, ctx| {
                    global_buffer.discard_unsaved_changes(_path, ctx);
                });
            }
            CurrentAppCodeEditorEvent::CommentSaved { comment } => {
                let Some(file_path) = editor.as_ref(ctx).file_path() else {
                    log::error!(
                        "Attempted to attach code review comment to a CurrentAppCodeEditorView without a file path"
                    );
                    return;
                };
            ...
            CurrentAppCodeEditorEvent::DeleteComment { id } => {
                self.delete_comment_by_id(*id, ctx);
            }
            CurrentAppCodeEditorEvent::RequestOpenComment(comment_id) => {
                ...
                match &existing_comment.target {
                    AttachedReviewCommentTarget::Line { line, .. } => {
                        ...
                        editor.update(ctx, |current_app_code_editor, ctx| {
                            current_app_code_editor
                                .editor()
                                .update(ctx, |code_editor, ctx| {
    ...
    /// This is called when CurrentAppCodeEditorEvent::DelayedRenderingFlushed or FailedToLoad fires.
    ...
    fn apply_diff_to_code_editor(
        code_editor_view: &ViewHandle<CurrentAppCodeEditorView>,
        file: &FileDiffAndContent,
        ...
        if let Some(file_content) = &file.content_at_head {
            ...
            // 删除项没有可加载路径；symlink blob 若走 GlobalBufferModel 会解引用并泄露目标内容。
            let requires_detached_buffer = file.file_diff.requires_detached_buffer();

            code_editor_view.update(ctx, |current_file_editor, ctx| {
                ...
                let saved_selections = if !is_initial_setup {
                    Some(Self::capture_cursor_position(
                        current_file_editor.editor().as_ref(ctx),
                        ctx,
                    ))
                } else {
                    None
                ...
                // detached buffer 必须从基线和 hunk 重建；普通文件继续由 GlobalBufferModel 提供当前内容。
                if requires_detached_buffer {
                    ...
                    // 先装载基线，再在下方应用 hunk 得到当前内容。
                    current_file_editor.reset_with_state(state, ctx);
                }
                #[cfg(not(target_family = "wasm"))]
                if !requires_detached_buffer {
                    // 文件尚未加载时，后续 file-load event 会负责重新计算 diff。
                    let file_loaded = current_file_editor.file_loaded(ctx);
                ...
                current_file_editor.editor().update(ctx, |editor, ctx| {
                    // GlobalBufferModel 自己管理隐藏行；detached buffer 在这里显式配置。
                    if requires_detached_buffer {
        ...
        let relocated_comments = comments
            ...
            .map(|mut comment| {
                ...
                let (new_location, new_content, used_fallback) =
                    editor_view.update(ctx, |current_file_editor, ctx| {
                        current_file_editor.editor().update(ctx, |editor, ctx| {
                            editor.model.update(ctx, |model, ctx| {
    ...
    pub fn render_environment_runtime_state(
        appearance: &Appearance,
        open_repo_button: Option<Box<dyn Element>>,
    ) -> Box<dyn Element> {
    ...
    #[cfg(not(target_family = "wasm"))]
    fn render_environment_runtime_state_with_buttons(
        &self,
        appearance: &Appearance,
    ) -> Box<dyn Element> {
        ...
        match result {
            ReviewSubmissionResult::Success => {
                ...
                ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                    ...
                    stack.add_transient_toast(toast, self.window_id, ctx);
                });
            ...
            ReviewSubmissionResult::Error => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    ...
                    toast_stack.add_transient_toast(toast, self.window_id, ctx);
                });
            }
        }
    }
    ...
    /// Renders the file content (hunks for text files using CurrentAppCodeEditorView, placeholder for binary)
        ...
        if let CodeReviewViewState::Loaded(state) = self.state() {
            if let Some(file_state) = state.file_states.get(path) {
                if let Some(editor) = file_state.editor_state.as_ref().map(|state| state.editor()) {
                    if let Err(err) = editor.update(ctx, |current_file_editor, ctx| {
                        current_file_editor.save_current_file(ctx)
                    }) {
                        safe_error!(
    ...
    pub(super) fn editor_for_path(
        ...
        path: &Path,
        ctx: &AppContext,
    ) -> Option<ViewHandle<CurrentAppCodeEditorView>> {
        match self.state() {
    ...
    pub(super) fn editor_handles(
        &self,
    ) -> Box<dyn Iterator<Item = ViewHandle<CurrentAppCodeEditorView>> + '_> {
        match self.state() {
    ...
    fn close(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if !unsaved_file_paths.is_empty() && ChannelState::channel() != Channel::Integration {
            ...
            if cfg!(all(not(target_family = "wasm"), target_os = "macos")) {
                ...
                // Find the workspace to show the Ashide-native modal
