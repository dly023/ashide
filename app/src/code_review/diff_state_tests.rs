...
#[cfg(unix)]
#[test]
fn test_untracked_symlink_diff_uses_link_target_without_dereferencing() {
    use std::os::unix::fs::symlink;

    let tempdir = tempfile::tempdir().unwrap();
    let repo = tempdir.path().join("repo");
    let directory_target = tempdir.path().join("directory-target");
    std::fs::create_dir_all(&repo).unwrap();
    std::fs::create_dir_all(&directory_target).unwrap();
    std::fs::write(directory_target.join("child.txt"), "target contents\n").unwrap();
    std::fs::write(tempdir.path().join("file-target.txt"), "file contents\n").unwrap();

    for (name, target) in [
        ("directory-link", "../directory-target"),
        ("file-link", "../file-target.txt"),
        ("broken-link", "../missing-target"),
    ] {
        symlink(target, repo.join(name)).unwrap();

        let diff = async_io::block_on(DiffStateModel::get_file_diff(
            &repo,
            &PathBuf::from(name),
        ...
        assert!(diff.is_symlink);
        assert!(diff.requires_detached_buffer());
        assert_eq!(diff.hunks.len(), 1);
        assert_eq!(diff.hunks[0].lines.len(), 1);
        assert_eq!(diff.hunks[0].lines[0].text, target);
        assert_eq!(diff.additions(), 1);
        assert_eq!(
            async_io::block_on(DiffStateModel::num_lines_in_file_if_non_binary(
                &repo.join(name)
            ))
            .unwrap(),
            Some(1)
...
#[cfg(unix)]
#[test]
fn test_untracked_symlink_diff_is_not_affected_by_newlines_in_path_or_target() {
    use std::os::unix::fs::symlink;
    ...
    assert_eq!(diff.file_path, PathBuf::from(name));
    assert!(diff.is_symlink);
    assert!(diff.requires_detached_buffer());
    assert_eq!(diff.hunks.len(), 1);
    assert_eq!(diff.hunks[0].lines.len(), 2);
    assert_eq!(diff.hunks[0].lines[0].text, "../missing");
    assert_eq!(diff.hunks[0].lines[1].text, "second");
