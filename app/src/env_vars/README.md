...
This document describes the current local object-store implementation. All referenced files are present in this directory unless specified otherwise.
...
Context: EVCs are built on GenericStringObjects (GSOs). In Ashide, that means the object is stored in the local object store and persisted through SQLite. The `JsonObjectType::EnvVarCollection` discriminator is the object kind used by current local storage.

The `EnvVarCollectionObject` implementation in `mod.rs` specifies local object-store behavior such as rendering in Ashide Drive and supporting export.

The implementation of EVCs as an Ashide Drive object is in `app/src/drive/items/env_var_collection.rs`, where code for the Ashide Drive preview and click action is located.

Edit-collision and liveness behavior lives in the local object-store/update-manager path. Legacy server file names in older docs should be read as historical context, not as active network behavior.

## Client Side

### Panes

EVCs, like most objects in Ashide, are children of a pane. Our implementation is defined in `app/src/pane_group/pane/env_var_collection_pane.rs`, which is essentially identical to other pane implementations. The `EnvVarCollectionPane` is closely coupled with the `EnvVarCollectionManager`, defined in `manager.rs`. The manager is responsible for creating, destroying, and registering all EVC panes, whereas the pane itself contains the EVC view.
