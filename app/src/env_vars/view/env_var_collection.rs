...
use crate::{
    ...
    drive::{access::ContentEditability, items::LocalDriveItemId},
    editor::EditorView,
    env_vars::{
...
pub const EDUCATION_TEXT: &str = "Add secret or command. Ashide never stores external secrets";
...
pub enum EnvVarCollectionEvent {
    Pane(PaneEvent),
    UpdatedEnvVarCollection(ObjectStoreId),
...
impl EnvVarCollectionView {
    ...
    fn invoke_env_var_collection(&self, ctx: &mut ViewContext<Self>) {
            ...
            .active_env_var_collection
        {
            ActiveEnvVarCollection::CommittedEnvVarCollection(id) => {
                ...
                if let Some(local_env_var) = object_store_model.get_env_var_collection(id) {
                    ctx.emit(EnvVarCollectionEvent::Invoke(EnvVarCollectionType::Object(
                        Box::new(local_env_var.clone()),
                    )));
                } else {
    ...
    fn save_env_var_collection(&self, ctx: &mut ViewContext<Self>) {
        ...
        match active_env_var_collection {
            // If the EVC has already been committed, update local object-store
            // state via the update manager.
    ...
    fn handle_active_env_var_collection_change(
        ...
        // Refresh the overflow menu to show actions that only apply to persisted EVCs.
    ...
    pub fn env_var_collection_id<C: ModelAsRef>(&self, ctx: &C) -> Option<ObjectStoreId> {
    ...
    fn view_in_local_drive(&mut self, id: LocalDriveItemId, ctx: &mut ViewContext<Self>) {
        ctx.emit(EnvVarCollectionEvent::ViewInLocalDrive(id));
    }
                ...
                ConstrainedBox::new(
                    Align::new(
                        Container::new(render_breadcrumbs(
                            ...
                            |ctx, _, breadcrumb| {
                                ctx.dispatch_typed_action(
                                    EnvVarCollectionAction::ViewInLocalDrive(
...
impl TypedActionView for EnvVarCollectionView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            EnvVarCollectionAction::ViewInLocalDrive(id) => self.view_in_local_drive(*id, ctx),
        }
    }
}
