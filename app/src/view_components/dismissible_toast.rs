...
impl ToastIdentity {
    fn coalesced(source: impl Into<String>, category: ToastCategory) -> Self {
        Self {
            source: source.into(),
            stable_key: None,
            category,
    ...
    fn keyed(
        ...
        Self {
            source: source.into(),
            stable_key: Some(stable_key.into()),
            category,
        }
    }
    ...
    fn matches(&self, other: &Self) -> bool {
        self.source == other.source
            && match (&self.stable_key, &other.stable_key) {
                (Some(key), Some(other_key)) => key == other_key,
                (None, None) => self.category == other.category,
...
impl ToastPolicy {
    ...
    pub fn action_required(source: impl Into<String>, stable_key: impl Into<String>) -> Self {
        Self::ActionRequired {
            source: source.into(),
    ...
    fn resolve(self, category: ToastCategory) -> (ToastIdentity, ToastLifetime) {
        match self {
...
impl<A: Action + Clone> DismissibleToastStack<A> {
    ...
    pub fn add_toast(
        ...
        let (uuid, generation) = self.upsert(toast, policy);
        if self
            .toasts
            .iter()
            .any(|toast| toast.uuid == uuid && toast.lifetime == ToastLifetime::Transient)
        {
            let abort_handle = ctx.spawn_abortable(
                Timer::after(self.timeout),
                move |view, _, ctx| view.dismiss_toast_generation(&uuid, generation, ctx),
                |_, _| {},
            );
            if let Some(toast) = self.toasts.iter_mut().find(|toast| toast.uuid == uuid) {
                toast.abort_handle = Some(abort_handle);
            }
        }
        ctx.notify();
    }
    ...
    #[track_caller]
    pub fn add_transient_toast(&mut self, toast: DismissibleToast<A>, ctx: &mut ViewContext<Self>) {
        self.add_toast(
            toast,
    ...
    pub fn add_action_required_toast(
        ...
        self.add_toast(toast, ToastPolicy::action_required(source, stable_key), ctx);
    }

    fn upsert(&mut self, toast: DismissibleToast<A>, policy: ToastPolicy) -> (Uuid, u64) {
        let (identity, lifetime) = policy.resolve(toast.category());
        if let Some(existing) = self
            .toasts
            .iter_mut()
            .find(|toast| toast.identity.matches(&identity))
        {
            if let Some(abort_handle) = existing.abort_handle.take() {
                abort_handle.abort();
            }
            existing.dismissible_toast = toast;
            existing.identity = identity;
            existing.lifetime = lifetime;
            existing.generation = existing.generation.wrapping_add(1);
            return (existing.uuid, existing.generation);
        }
    ...
    fn dismiss_toast_generation(
        &mut self,
        uuid: &Uuid,
        generation: u64,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.remove_toast_generation(uuid, generation) {
            ctx.notify();
        }
    }

    fn remove_toast_generation(&mut self, uuid: &Uuid, generation: u64) -> bool {
        let Some(index) = self
            .toasts
            .iter()
            .position(|toast| toast.uuid == *uuid && toast.generation == generation)
        else {
            return false;
        };
    ...
    fn visible_toast_count(&self) -> usize {
    ...
    fn hidden_toast_count(&self) -> usize {
        self.toasts.len().saturating_sub(self.visible_toast_count())
    }
...
impl<A: Action + Clone> View for DismissibleToastStack<A> {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let visible_toast_count = self.visible_toast_count();
        let hidden_toast_count = self.hidden_toast_count();
        for toast in self.toasts.iter().rev().take(visible_toast_count) {
        ...
        if hidden_toast_count > 0 {
            let appearance = Appearance::as_ref(app);
            let summary = appearance
                .ui_builder()
                .wrappable_text(format!("{hidden_toast_count} more notifications"), true)
                .with_style(UiComponentStyles {
                    font_size: Some(appearance.ui_font_subheading()),
                    font_color: Some(ToastFlavor::Default.text_color(appearance)),
                    ..Default::default()
                })
                .build()
                .finish();
            rendered_toasts.add_child(
                Container::new(summary)
                    ...
                    .with_background(Fill::black().with_opacity(60))
                    .with_corner_radius(CornerRadius::with_all(Radius::Pixels(TOAST_CORNER_RADIUS)))
                    .with_margin_bottom(5.)
                    .finish(),
            );
        }
...
impl<A: Action + Clone> DismissibleToast<A> {
    ...
    fn category(&self) -> ToastCategory {
        match self.flavor {
            ToastFlavor::Default => ToastCategory::Info,
            ToastFlavor::Success => ToastCategory::Success,
            ToastFlavor::Error => ToastCategory::Error,
        }
    }
