use crate::integration_testing::command_palette::assertions::{
    ...
    assert_command_palette_selected_binding,
};
...
///
/// Opens the palette, types the action text, and waits until the exact requested binding owns the
/// selected result before pressing Enter.
pub fn open_command_palette_and_run_action(action: &str) -> Vec<TestStep> {
    vec![
        TestStep::new(format!("Open command palette for {action}").as_str())
            .with_keystrokes(&[cmd_or_ctrl_shift("p")])
            .add_assertion(assert_command_palette_is_open()),
        TestStep::new(format!("Type {action} in command palette").as_str())
            .with_typed_characters(&[action])
            .add_assertion(assert_command_palette_selected_binding(action)),
