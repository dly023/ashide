...
pub fn assert_local_drive_is_open() -> AssertionCallback {
    Box::new(move |app, window_id| {
        let workspace = workspace_view(app, window_id);

        workspace.read(app, |workspace, _| {
            async_assert!(
                workspace.is_local_drive_open(),
                "Expected Ashide Drive to be open, but it was closed"
            )
        })
    })
}

pub fn assert_local_drive_is_closed() -> AssertionCallback {
    Box::new(move |app, window_id| {
        let workspace = workspace_view(app, window_id);
        ...
        workspace.read(app, |workspace, _| {
            async_assert!(
                !workspace.is_local_drive_open(),
                "Expected Ashide Drive to be closed, but it was open"
            )
        })
    })
}
