...
use crate::{
    ...
    object_store::{model::persistence::ObjectStoreModel, update_manager::UpdateManager, Space},
    workspaces::user_workspaces::UserWorkspaces,
};

/// Create a personal rule and save its object store ID into the step data.
...
pub fn open_rule_pane(window_key: impl Into<String>, key: impl Into<String>) -> TestStep {
    ...
    TestStep::new("Open rule pane").with_action(move |app, _, data| {
        ...
        let fact_id: &ObjectStoreId = data.get(&key).expect("No saved rule ID");
...
pub fn update_rule_content(
    ...
    TestStep::new("Update rule content").with_action(move |app, _, data| {
        let object_store_id: &ObjectStoreId = data.get(&fact_key).expect("No saved rule ID");
        UpdateManager::handle(app).update(app, |update_manager, ctx| {
            let ai_fact = crate::ai::facts::AIFact::Memory(AIMemory {
                is_autogenerated: false,
                name: None,
            ...
            update_manager.update_ai_fact(ai_fact, *object_store_id, None, ctx);
        });
    })
}
