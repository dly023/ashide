...
use crate::{
    ...
    integration_testing::view_getters::{
        input_view, single_input_view_for_tab, single_terminal_view, single_terminal_view_for_tab,
        terminal_view,
    },
...
pub fn assert_shell_ready_for_input(tab_index: usize, pane_index: usize) -> AssertionCallback {
    Box::new(move |app, window_id| {
...
pub fn assert_no_block_executing(tab_index: usize, pane_index: usize) -> AssertionCallback {
    Box::new(move |app, window_id| {
        ...
        terminal_view.read(app, |view, _ctx| {
            let model = view.model.lock();
            let block = model.block_list().active_block();
            let block_is_executing = match block.state() {
                BlockState::BeforeExecution => block.started(),
                BlockState::Executing => true,
                BlockState::Background => !block.finished(),
                BlockState::DoneWithExecution
                | BlockState::DoneWithNoExecution
                | BlockState::Static => false,
            };
            async_assert!(
                !block_is_executing,
                "Expected no block to be executing, but active block state is {:?} and output is:\n{}\n",
                block.state(),
                block.output_with_secrets_unobfuscated()
            )
        })
...
pub fn assert_focused_editor_in_tab(tab_index: usize) -> AssertionCallback {
    assert_focused_editor(tab_index, 0)
}

pub fn assert_focused_editor(tab_index: usize, pane_index: usize) -> AssertionCallback {
    Box::new(move |app, window_id| {
        let input_view = input_view(app, window_id, tab_index, pane_index);
...
pub fn assert_active_session_local_path(expected_path: &'static str) -> AssertionCallback {
    Box::new(move |app, window_id| {
        ActiveSession::handle(app).read(app, |active_session, _| {
            ...
            let pwd = active_session.current_app_path(window_id);
...
pub fn assert_terminal_current_app_path(
    tab_index: usize,
