...
pub fn open_prompt_context_menu_for_input() -> TestStep {
    new_step_with_default_assertions("Open prompt context menu")
        .with_right_click_on_saved_position_fn(|app, window_id| {
            let input = single_input_view_for_tab(app, window_id, 0);
            input.read(app, |input, _ctx| input.prompt_save_position_id())
        })
        .add_assertion(assert_context_menu_is_open(true))
        .add_saved_position_readiness_assertion("Copy prompt")
}

pub fn open_rprompt_context_menu_for_input() -> TestStep {
    new_step_with_default_assertions("Open right prompt context menu")
        .with_right_click_on_saved_position_fn(|app, window_id| {
            let input = single_input_view_for_tab(app, window_id, 0);
            format!("rprompt_area_{}", input.id())
        })
        .add_assertion(assert_context_menu_is_open(true))
        .add_saved_position_readiness_assertion("Copy right prompt")
}
