...
use crate::{
    ai::blocklist::ai_brand_color, object_store::ids::ObjectStoreId, settings::InputModeSettings,
...
use crate::{
    ...
    object_store::{model::actions::ObjectActions, StoredObjectMetadataExt},
};
use crate::{object_store::model::actions::ObjectActionType, terminal::view::TerminalAction};
...
struct ButtonMouseStates {
    ...
    edit_object_store_workflow: MouseStateHandle,
...
impl WorkflowsMoreInfoView {
    ...
    pub fn set_environment_variables_selection(
        &mut self,
        env_vars_id: Option<ObjectStoreId>,
        ctx: &mut ViewContext<Self>,
    ) {
    ...
    fn render_edit_button(
        ...
        render_hoverable_card_button(
            icons::Icon::Rename,
            Some(label.to_owned()),
            self.button_mouse_states.edit_object_store_workflow.clone(),
    ...
    fn render_info_box(
        ...
        match &self.workflow {
            WorkflowType::ObjectStore(workflow) => {
...
pub enum WorkflowsInfoBoxViewEvent {
    PrefixCommandWithEnvironmentVariables(Option<ObjectStoreId>),
}

#[derive(Debug, Clone)]
pub enum WorkflowsInfoBoxViewAction {
    CollapseOrExpand,
    SelectEnvironmentVariables(Option<ObjectStoreId>),
}
