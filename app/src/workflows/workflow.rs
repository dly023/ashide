...
pub enum Workflow {
    ...
    Command {
        ...
        #[serde(default)]
        environment_variables: Option<ObjectStoreId>,
    },
}
...
impl Workflow {
    ...
    pub fn get_enum_ids(&self) -> Vec<ObjectStoreId> {
        self.arguments()
            .iter()
    ...
    pub fn default_env_vars(&self) -> Option<ObjectStoreId> {
        match self {
            Workflow::Command {
    ...
    pub fn replace_object_id(&mut self, old_id: ObjectStoreId, new_id: ObjectStoreId) -> bool {
    ...
    D: Deserializer<'de>,
{
    ...
    let arg_type = match value.get("arg_type").and_then(|value| value.as_str()) {
        ...
        Some("Enum") => {
            ...
            let deserialized_id = ObjectStoreId::deserialize(enum_id)
