...
pub enum EnvVarSelectorAction {
    Select(Option<ObjectStoreId>),
}

pub enum EnvVarSelectorEvent {
    SelectionChanged(Option<ObjectStoreId>),
    Refreshed,
}
...
impl EnvVarSelector {
    ...
    fn refresh_dropdown_items(&mut self, ctx: &mut ViewContext<Self>) {
        let mut env_vars = ObjectStoreModel::as_ref(ctx)
            ...
            .map(|collection| (collection.display_name(), collection.object_store_id()))
            .collect_vec();
