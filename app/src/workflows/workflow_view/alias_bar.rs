...
impl AliasBar {
    pub fn new(workflow_id: ObjectStoreId, ctx: &mut ViewContext<Self>) -> Self {
    ...
    pub fn set_workflow_id(&mut self, workflow_id: ObjectStoreId, ctx: &mut ViewContext<Self>) {
    ...
    pub fn set_current_env_vars(
        ...
        if let Some(alias) = self
            ...
            .and_then(|index| self.aliases.get_mut(index))
        {
            if alias.env_vars != object_store_id {
                alias.env_vars = object_store_id;
                self.mark_dirty(true, ctx);
            }
        }
    }
    ...
    pub fn current_env_vars(&self) -> Option<ObjectStoreId> {
        self.selected_alias
            .and_then(|index| self.aliases.get(index))
