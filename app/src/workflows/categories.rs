...
use crate::{
    object_store::model::persistence::ObjectStoreModel, workspaces::user_workspaces::UserWorkspaces,
};
use crate::{
...
pub enum WorkflowViewType {
    All,
    LocalPersonal, // represents both local + personal Drive
    Project,
...
#[cfg(feature = "local_fs")]
#[derive(Default)]
struct ProjectWorkflowLoadState {
    requested_path: Option<PathBuf>,
    generation: u64,
}

#[cfg(feature = "local_fs")]
impl ProjectWorkflowLoadState {
    fn begin(&mut self, path: Option<PathBuf>) -> u64 {
        self.generation = self.generation.wrapping_add(1);
        self.requested_path = path;
        self.generation
    }

    fn owns(&self, generation: u64, path: &Option<PathBuf>) -> bool {
        self.generation == generation && &self.requested_path == path
    }
}

#[cfg(feature = "local_fs")]
struct ProjectWorkflowLoadResult {
    path: Option<PathBuf>,
    generation: u64,
...
pub struct CategoriesView {
    ...
    search_term: String,
    #[cfg(feature = "local_fs")]
    project_workflow_load_state: ProjectWorkflowLoadState,
}
...
impl CategoriesView {
    pub fn new(
        ...
        let mut workflows_view = Self {
            ...
            #[cfg(feature = "local_fs")]
            project_workflow_load_state: Default::default(),
        };
    ...
    #[cfg(feature = "local_fs")]
    fn apply_project_workflows_load(
        &mut self,
        result: ProjectWorkflowLoadResult,
        ctx: &mut ViewContext<Self>,
    ) {
        if !self
            .project_workflow_load_state
            .owns(result.generation, &result.path)
        {
            log::info!(
                "discarding stale project workflow load generation={} path={:?}",
                result.generation,
                result.path
            );
            return;
    ...
    #[cfg(feature = "local_fs")]
    fn load_project_workflows_for_path(
        &mut self,
        path: Option<PathBuf>,
        force_refresh: bool,
        ctx: &mut ViewContext<Self>,
    ) {
        if !force_refresh && self.project_workflow_load_state.requested_path == path {
            return;
        }

        let generation = self.project_workflow_load_state.begin(path.clone());
        let Some(path) = path else {
            self.apply_project_workflows_load(
        ...
        let _ = ctx.spawn(
            async move {
                ...
                ProjectWorkflowLoadResult {
                    workflows: super::local_workflows::load_project_workflows(&path),
                    path: Some(path),
                    generation,
                }
            },
        ...
        log::info!("loading project workflows generation={generation} path={requested_path:?}");
    }

    #[cfg(feature = "local_fs")]
    pub(crate) fn on_active_session_current_app_path_changed(
        &mut self,
        path: Option<PathBuf>,
        ctx: &mut ViewContext<Self>,
    ) {
        self.load_project_workflows_for_path(path, false, ctx);
    }

    pub fn load_object_store_workflows(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        for space in user_workspaces.all_user_spaces(ctx) {
            ...
            let new_workflows_in_space = Self::categorize_workflows(
                ...
                workflows_in_space
                    ...
                    .map(|w| Arc::new(WorkflowType::ObjectStore(Box::new(w.clone())))),
            );
            self.workflows_by_source
        ...
        self.active_workflows = match &self.selected_workflow_type {
            ...
            WorkflowViewType::LocalPersonal => {
                ...
                let personal_drive = self
                    .workflows_by_source
                    .get(&WorkflowSource::PersonalDrive)
                    .map(|categorized_workflows| {
                        Self::create_workflow_source_pair(
                            categorized_workflows.values(),
...
impl VoltronFeatureViewMeta for CategoriesView {
    ...
    fn on_load(&mut self, metadata: VoltronMetadata, ctx: &mut ViewContext<Self>) {
        #[cfg(feature = "local_fs")]
        self.load_project_workflows_for_path(metadata.active_session_current_app_path, true, ctx);

        self.load_object_store_workflows(ctx);
