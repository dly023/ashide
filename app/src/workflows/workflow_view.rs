...
use crate::{
    ...
    drive::{
        ...
        DriveObjectType, LocalDriveObjectSettings, ObjectTypeAndId,
    },
    editor::{
...
struct EnvironmentVariablesState {
    default_env_vars: Option<ObjectStoreId>,
    is_dirty: bool,
}
...
pub struct WorkflowView {
    ...
    /// Name of an argument whose row should be focused/revealed once the
    /// content edit has propagated into `arguments_rows`. Set by
    /// [`Self::add_argument`] and consumed in [`Self::handle_content_editor_event`]
    /// after `update_arguments_rows`, because the derived row does not exist yet
    /// on the same frame the `{{argument}}` token is inserted (the editor's
    /// `Edited` event is flushed after the current dispatch completes).
    pending_focus_argument_name: Option<String>,
    show_enum_creation_dialog: bool,
    enum_creation_dialog: ViewHandle<EnumCreationDialog>,
    all_workflow_enums: HashMap<ObjectStoreId, WorkflowEnumData>,
...
impl WorkflowView {
    ...
    fn new_internal(
        ...
        let workflow_id = ObjectStoreId::ClientId(ClientId::default());
    ...
    pub fn open_new_workflow(
        ...
        self.set_workflow_id(object_store_id, ctx);
    ...
    fn handle_object_store_event(&mut self, event: &ObjectStoreEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ObjectStoreEvent::ObjectUpdated {
                type_and_id: ObjectTypeAndId::Workflow(object_store_id),
                source: _,
            } => {
                if self.workflow_id() == *object_store_id && !self.is_editable() {
                    self.reset(ctx);
                }
            }
    ...
    fn handle_update_manager_event(
        ...
        {
            if let Some(workflow) = self.get_object_store_workflow(ctx) {
                let updated_id = result
                    .stable_id
                    .map(ObjectStoreId::StableId)
                    .or_else(|| result.client_id.map(ObjectStoreId::ClientId));
                if updated_id.is_some_and(|updated_id| updated_id == self.workflow_id) {
                    self.load(
                        workflow,
    ...
    fn reset(&mut self, ctx: &mut ViewContext<Self>) {
        let workflow = self.get_object_store_workflow(ctx);
        ...
        ctx.spawn(initial_load_complete, move |me, _, ctx| {
            ...
            if let Some(workflow) = workflow {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_transient_toast_by_type(
    ...
    pub fn refresh_pane_overflow_menu(&self, ctx: &mut ViewContext<Self>) {
        if let ContainerConfiguration::Pane(pane_config) = &self.container_configuration {
            // Refresh the overflow menu to show actions that only apply to persisted workflows.
            pane_config.update(ctx, |pane_config, ctx| {
    ...
    pub fn workflow_id(&self) -> ObjectStoreId {
        self.workflow_id
    }

    fn set_workflow_id(&mut self, id: ObjectStoreId, ctx: &mut ViewContext<Self>) {
        self.workflow_id = id;
    ...
    fn handle_content_editor_event(&mut self, event: &EditorEvent, ctx: &mut ViewContext<Self>) {
        match event {
            EditorEvent::Edited(_) => {
                ...
                // UX closure: if an "+ Add argument" click just appended a new
                // argument, the derived row now exists — focus its first editor
                // so the user lands on it (focusing an argument editor also
                // reveals it within the scroll container, mirroring how the enum
                // dialog reopen focuses `arg_type_editor`).
                if let Some(name) = self.pending_focus_argument_name.take() {
                    if let Some(row) = self.arguments_rows.iter().find(|row| row.name == name) {
                        let editor = row.description_editor.clone();
                        ctx.focus(&editor);
                    }
                }
    ...
    fn handle_enum_creation_dialog_event(
        ...
        match event {
            ...
            EnumCreationDialogEvent::EditEnum(enum_data, did_reusability_change) => {
                workflow_arg_type_helpers::edit_enum(
                    enum_data,
                    *did_reusability_change,
                    &mut self.all_workflow_enums,
                    &self.arguments_rows,
                    &mut self.pending_argument_editor_row,
                    ctx,
    ...
    fn save_argument_objects(&self, ctx: &mut ViewContext<Self>) {
        let mut sent_requests: HashSet<ObjectStoreId> = HashSet::new();
    ...
    fn copy_to_command_line(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.is_workflow_dirty(ctx) {
            ...
            if let Some(workflow) = self.get_object_store_workflow(ctx) {
                let mut cloned_object_store_workflow = workflow.clone();
                cloned_object_store_workflow.set_model(WorkflowObjectModel::new(new_workflow));
                if let Some(owner) = self.owner {
                    ctx.emit(WorkflowViewEvent::RunWorkflow {
                        workflow: Arc::new(WorkflowType::ObjectStore(Box::new(
                            cloned_object_store_workflow,
                        ))),
                        source: owner.into(),
        ...
        } else if let Some(workflow) = self.get_object_store_workflow(ctx) {
            if let Some(owner) = self.owner {
                ctx.emit(WorkflowViewEvent::RunWorkflow {
                    workflow: Arc::new(WorkflowType::ObjectStore(Box::new(workflow))),
                    source: owner.into(),
    ...
    fn display_error_toast(&self, message: String, ctx: &mut ViewContext<Self>) {
        ...
        crate::workspace::ToastStack::handle(ctx).update(ctx, |stack, ctx| {
            stack.add_transient_toast(DismissibleToast::error(message), window_id, ctx);
        });
        ctx.notify();
    }
    ...
    /// unnecessarily modify the backing Ashide Drive object.
    ...
    fn add_argument(&mut self, ctx: &mut ViewContext<Self>) {
        let argument_name = self.content_editor.update(ctx, |editor, ctx| {
        ...
        // UX closure: the new argument's row is derived from the content edit we
        // just made, but the editor's `Edited` event (which rebuilds
        // `arguments_rows`) is flushed only after this dispatch returns, so the
        // row does not exist yet here. Record the name and focus/reveal the row
        // once `handle_content_editor_event` has rebuilt the rows.
        self.pending_focus_argument_name = Some(argument_name);
    }
    ...
    fn render_edit_toggle_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        if let Some((mode_text, mut edit_button)) = text_and_button {
            // Ashide UX: 与 notebook details_bar 一致,给 workflow 编辑
    ...
    fn get_object_store_workflow(&mut self, ctx: &mut ViewContext<Self>) -> Option<WorkflowObject> {
        ...
        if let Some((label, icon)) = label_and_icon {
                ...
                && !self.is_for_agent_mode
            {
                ...
                let button_with_tool_tip = appearance.ui_builder().tool_tip_on_element(
                    "Generate a title, descriptions, or parameters with Ashide AI".to_string(),
    ...
    fn view_in_local_drive(&mut self, id: LocalDriveItemId, ctx: &mut ViewContext<Self>) {
        ctx.emit(WorkflowViewEvent::ViewInLocalDrive(id));
    }
        ...
        row.add_child(
            Shrinkable::new(
                ...
                Container::new(render_breadcrumbs(
                    ...
                    appearance,
                    |ctx, _, breadcrumb| {
                        ctx.dispatch_typed_action(WorkflowAction::ViewInLocalDrive(
...
impl TypedActionView for WorkflowView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            WorkflowAction::ViewInLocalDrive(id) => self.view_in_local_drive(*id, ctx),
