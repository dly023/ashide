...
/// Singleton model that loads and caches local (non-local Drive) workflows.
...
impl LocalWorkflows {
    ...
    /// Returns an iterator over hardcoded "application" workflows included in the Ashide binary.
    pub fn app_workflows(&self) -> impl Iterator<Item = &Workflow> {
        self.app_workflows.iter()
    }

    /// Returns an iterator over the static set of workflows for 3rd party tools loaded from Ashide's
