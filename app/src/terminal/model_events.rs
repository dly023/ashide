...
impl ModelEventDispatcher {
    ...
    fn handle_terminal_model_event(&mut self, event: Event, ctx: &mut ModelContext<Self>) {
        let event_to_emit = match event {
            ...
            Event::Handler(HandlerEvent::Bootstrapped(bootstrapped_event)) => {
                ...
                // environment runtime is enabled, the environment runtime client
                // is wired up independently: `Sessions::new` subscribes to
                // runtime session events and attaches the client to the session's
                // runtime command executor when the connection lands, so it's safe
                // to initialize the session before the runtime finishes connecting.
            ...
            Event::EnvironmentRuntimeReady { session_id } => {
                log::info!("Environment runtime ready for session {session_id:?}");
                return;
            }
            Event::EnvironmentRuntimeFailed { session_id, error } => {
                log::warn!(
                    "Environment runtime setup failed for session {session_id:?}, falling back to \
                     ControlMaster: {error}"
                );
                return;
            }
    ...
    ///
    /// For legacy SSH sessions using environment runtime, this also
    /// sends the `SessionBootstrapped` notification through the runtime
    /// transport manager.
    fn complete_bootstrapped_session(
        ...
        let uses_environment_runtime = session_info.session_type.uses_environment_runtime();

        if crate::workspace::environment_runtime::environment_runtime_feature_enabled()
            && uses_environment_runtime
        {
            if let Some(runtime_session_id) = session_info.environment_runtime_transport_session_id
            {
                crate::workspace::environment_runtime::register_terminal_bootstrap_session_alias(
                    session_id,
                    runtime_session_id,
                    ctx,
                );
        ...
        if crate::workspace::environment_runtime::environment_runtime_feature_enabled()
            && (is_legacy_ssh || uses_environment_runtime)
        {
            crate::workspace::environment_runtime::notify_bootstrapped_session(
                session_id,
                &shell_type_name,
                shell_path.as_deref(),
                ctx,
    ...
    /// Emits an event so `TerminalView` can render the environment runtime setup-choice block.
    pub fn request_environment_runtime_setup_choice(
        &mut self,
        session_id: SessionId,
        ctx: &mut ModelContext<Self>,
    ) {
        ctx.emit(ModelEvent::EnvironmentRuntimeSetupChoiceRequested { session_id });
    }
}
...
pub enum ModelEvent {
    ...
    PromptUpdated,
    /// Emitted when the honor_ps1 state of the shell is out-of-sync with Ashide's settings.
    /// This can happen in cases such as when the user changes between PS1 and Ashide prompt inside
    ...
    /// the environment runtime setup flow. `EnvironmentRuntimeController` subscribes to
    /// this instead of `Handler(InitShell)` so `PtyController` never sees it.
    SshInitShell {
        pending_session_info: Box<SessionInfo>,
    },
    /// Emitted by `ModelEventDispatcher::request_environment_runtime_setup_choice`
    /// when the environment runtime helper is missing and the user must choose.
    EnvironmentRuntimeSetupChoiceRequested {
        session_id: SessionId,
    },
    /// Emitted right before the remote shell for a session exits. Used to
    /// tear down per-session resources (e.g. the environment-runtime-proxy ssh
