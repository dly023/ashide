...
impl ModelEventDispatcher {
    ...
    ///
    /// For sessions using Environment Runtime, this also sends the complete
    /// target execution snapshot through the runtime transport manager.
    fn complete_bootstrapped_session(
        ...
        let (
            ...
            session_info.environment_runtime_transport_session_id,
            session_info.shell.shell_type().name().to_owned(),
            session_info.shell.shell_path().clone(),
            session_info.execution_context.clone(),
        );
