...
pub type FilterBuilderFn = dyn Fn(
    ...
    Option<BlockIndex>,           /* filter_editor_block_index */
...
pub struct BlockListElement {
    ...
    filter_editor_block_index: Option<BlockIndex>,
    ...
    /// the cursor cell is skipped. For agents that let Ashide draw the cursor
...
impl BlockListElement {
    ...
    pub fn new(
        ...
        Self {
            ...
            filter_editor_block_index: None,
    ...
    pub fn with_active_block_filter_editor(
        mut self,
        filter_editor_block_index: BlockIndex,
    ) -> Self {
        self.filter_editor_block_index = Some(filter_editor_block_index);
        self
    }
    ...
    fn draw_block(
        ...
        *grid_origin += vec2f(0., prompt_height_offset);
        ...
        // If Ashide prompt (non-PS1) is being used, the command is drawn below the prompt,
        ...
        *grid_origin += vec2f(
        ...
        if !block.should_hide_output_grid() {
            ...
            // Check if the "hide cursor" escape sequence is present.
            && block.is_mode_set(TermMode::SHOW_CURSOR)
            // Don't draw the Ashide cursor when rich input is hiding
            // the CLI agent's cursor cell — agents like OpenCode and Codex
            // rely on Ashide's cursor, so we suppress it here too.
...
impl Element for BlockListElement {
    fn layout(
        ...
        let elements = (self.filter_elements_builder)(
            visible_block_indices.clone(),
            self.hovered_block_index,
            self.filter_editor_block_index,
            ...
            match block_entry {
                VisibleItem::Block {
                    ...
                    let block_has_active_filter_icon = self
                        ...
                        || self.filter_editor_block_index == Some(*block_index);
