...
pub enum Event {
    ...
    /// per-session resources (e.g. the `ssh … environment-runtime-proxy` child that
    ...
    /// Emitted when the environment runtime helper has been successfully checked or
    /// installed and is ready. The session is initialized independently on
    /// `Bootstrapped`; when the environment runtime later connects, the client is
    /// attached to the existing session's runtime command executor via
    /// the environment runtime session subscription in `Sessions::new`.
    EnvironmentRuntimeReady {
        session_id: SessionId,
    },
    /// Emitted when the environment runtime setup failed. The session falls back to
    /// the legacy SSH ControlMaster executor.
    EnvironmentRuntimeFailed {
        session_id: SessionId,
        error: String,
    },
    ...
    /// External programs can use this to trigger notifications in Ashide.
    ///
    /// References:
...
pub struct BlockCompletedEvent {
    ...
    pub restored_block_used_current_app_environment: Option<bool>,
}
...
pub struct AfterBlockCompletedEvent {
    ...
    pub object_store_workflow_id: Option<ObjectStoreId>,

    /// If the completed block had an env var object associated.
    pub local_env_var_collection_id: Option<ObjectStoreId>,
}
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::EnvironmentRuntimeReady { session_id } => {
                write!(f, "EnvironmentRuntimeReady(session: {session_id:?})")
            }
            Event::EnvironmentRuntimeFailed {
                session_id,
                ref error,
            } => {
                write!(
                    f,
                    "EnvironmentRuntimeFailed(session: {session_id:?}, error: {error})"
                )
            }
