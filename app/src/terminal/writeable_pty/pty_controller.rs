...
use crate::terminal::{
    ...
    model::{escape_sequences, session::SessionInfo},
...
/// restore the PS1 from the saved PS1 value (we had unset the PS1 for Ashide prompt).
const SWITCH_TO_PS1_ESCAPE_SEQUENCE: &[u8] = &[escape_sequences::C0::ESC, b'p'];
/// Used to let the shell know we are switching to the Ashide prompt via a bindkey \ew. This will
/// unset the PS1 to ensure we don't have a double prompt (PS1 and Ashide prompt).
...
impl<T: EventLoopSender> PtyController<T> {
    pub(crate) fn take_command_output_ownership(
        ...
        // A command takes ownership of subsequent PTY output. This is a
        // cancellation boundary, not a synthetic protocol end marker: partial
        // generator output must be discarded rather than decoded.
        model.cancel_in_band_command_output();
    }
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&model_event_dispatcher, |me, event, ctx| match event {
            ...
            ModelEvent::HonorPS1OutOfSync => {
                // We force re-sync the PS1 state of Ashide settings with the shell's environment variable, $WARP_HONOR_PS1, via
    ...
    pub fn write_command(
    ...
    ) {
        {
            ...
            Self::take_command_output_ownership(&mut model, source);
        }
