    ...
    ctx.subscribe_to_view(terminal_view, move |_view, event, ctx| {
        ...
        match event {
            ...
            view::Event::ExecuteCommand(event) => {
                ...
                model_clone.lock().block_list_mut().active_block_mut().set_object_store_workflow_state(event.workflow_id);
                controller.update(ctx, |controller, ctx| {
...
/// Wires up bi-directional communication between the EnvironmentRuntimeController and the TerminalView.
/// This handles the flow to render the environment runtime setup block and forward the user's choice to the controller.
///
/// Note that this interaction can't live in the TerminalView because the view must be manager-agnostic.
///
/// NOTE: we cannot simply use the strong references (the handle arguments to this wire_up fn)
...
pub fn wire_up_environment_runtime_controller_with_view<T: EventLoopSender>(
    environment_runtime_controller: &ModelHandle<
        super::environment_runtime_controller::EnvironmentRuntimeController<T>,
    >,
    terminal_view: &ViewHandle<TerminalView>,
    ctx: &mut AppContext,
) {
    let controller_weak = environment_runtime_controller.downgrade();
    ctx.subscribe_to_view(terminal_view, move |_view, event, ctx| {
        let Some(controller) = controller_weak.upgrade(ctx) else {
            return;
        };
        match event {
            view::Event::EnvironmentRuntimeInstallRequested { session_id } => {
                controller.update(ctx, |ctrl, ctx| {
                    ctrl.handle_environment_runtime_install(*session_id, ctx);
                });
            }
            view::Event::EnvironmentRuntimeSkipRequested { session_id } => {
                controller.update(ctx, |ctrl, ctx| {
                    ctrl.handle_environment_runtime_skip(*session_id, ctx);
                });
            }
            _ => {}
        }
...
/// Creates a [`EnvironmentRuntimeController`] that orchestrates the SSH init flow.
#[cfg(not(target_family = "wasm"))]
pub fn init_environment_runtime_controller<Sender: EventLoopSender>(
    pty_controller: &ModelHandle<PtyController<Sender>>,
    model_events: &ModelHandle<ModelEventDispatcher>,
    ctx: &mut AppContext,
) -> ModelHandle<super::environment_runtime_controller::EnvironmentRuntimeController<Sender>> {
    let pty_weak = pty_controller.downgrade();
    ctx.add_model(|ctx| {
        super::environment_runtime_controller::EnvironmentRuntimeController::new(
            pty_weak,
            model_events.clone(),
            ctx,
        )
