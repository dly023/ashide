...
pub(super) struct State {
    ...
    /// correctly. Notably, Zsh does NOT support this well in bracketed paste mode (which we use for all Ashide
...
impl ansi::Handler for GridHandler {
    ...
    fn identify_terminal<W: std::io::Write>(&mut self, writer: &mut W, intermediate: Option<char>) {
        match intermediate {
            ...
            Some('>') => {
                ...
                // Ashide, rather than passing the higher `Pv` value, we decided to use one of the
    ...
    fn report_xtversion<W: std::io::Write>(&mut self, writer: &mut W) {
        ...
        let _ = writer.write_all(format!("\x1bP>|Ashide({version})\x1b\\").as_bytes());
    }
