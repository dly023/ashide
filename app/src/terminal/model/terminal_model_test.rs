...
use crate::terminal::{event_listener::ChannelEventListener, input::CommandExecutionSource};
...
#[test]
fn force_environment_runtime_session_type_overrides_current_app_hostname_heuristic() {
    let mut terminal = TerminalModel::mock(None, None);
    let current_app_hostname = crate::terminal::model::session::get_current_app_hostname()
    ...
    assert_eq!(
        ...
        Some(&BootstrapSessionType::EnvironmentRuntime)
    );
}

#[test]
fn cancel_in_band_command_output_is_idempotent_while_idle() {
    let mut terminal = TerminalModel::mock(None, None);

    assert!(!terminal.is_receiving_in_band_command_output());
    terminal.cancel_in_band_command_output();
    terminal.cancel_in_band_command_output();
...
#[test]
fn cancel_in_band_command_output_discards_partial_generator_payload() {
    let mut terminal = TerminalModel::mock(None, None);

    Handler::start_in_band_command_output(&mut terminal);
    for character in "partial-generator-payload".chars() {
        Handler::input(&mut terminal, character);
    }
    assert!(terminal.is_receiving_in_band_command_output());
...
fn write_command_takeover_fixture() -> (
    TerminalModel,
    ...
    let event_proxy = ChannelEventListener::builder_for_test()
        ...
        .build();
    (TerminalModel::mock(None, Some(event_proxy)), events_rx)
}

struct NoopEventLoopSender;

impl crate::terminal::writeable_pty::pty_controller::EventLoopSender for NoopEventLoopSender {
    fn send(
        &self,
        message: crate::terminal::writeable_pty::Message,
    ) -> Result<(), crate::terminal::writeable_pty::pty_controller::EventLoopSendError> {
        drop(message);
        Ok(())
    }
}

fn take_write_command_ownership(
    terminal: &mut TerminalModel,
    events_rx: &async_channel::Receiver<crate::terminal::event::Event>,
) {
    while events_rx.try_recv().is_ok() {}
    crate::terminal::writeable_pty::PtyController::<NoopEventLoopSender>::take_command_output_ownership(
        terminal,
    ...
    assert_eq!(
        events_rx.len(),
        0,
        "command takeover emitted a terminal event"
    );
}

#[test]
fn write_command_takeover_is_idle_no_op_with_zero_error_events() {
    let (mut terminal, events_rx) = write_command_takeover_fixture();

    assert!(!terminal.is_receiving_in_band_command_output());
    take_write_command_ownership(&mut terminal, &events_rx);
}

#[test]
fn write_command_takeover_discards_partial_payload_with_zero_error_events() {
    let (mut terminal, events_rx) = write_command_takeover_fixture();
...
#[test]
fn forced_environment_runtime_session_carries_transport_session_id() {
    let mut terminal = TerminalModel::mock(None, None);
    let current_app_hostname = crate::terminal::model::session::get_current_app_hostname()
        .unwrap_or_else(|_| "localhost".to_string());
    ...
    assert_eq!(
        terminal
            .get_pending_session_info()
            .as_ref()
            .and_then(|session| session.environment_runtime_transport_session_id),
...
fn non_inline_iterm_image_save_is_current_app_scoped() {
    ...
    terminal.bootstrapped(BootstrappedValue {
        shell: "bash".to_string(),
        ..Default::default()
    });

    assert!(terminal.active_block_uses_environment_runtime());
    assert!(!terminal.should_save_non_inline_iterm_image_to_current_app_fs());
}
