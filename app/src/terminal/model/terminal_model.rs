...
pub enum ConversationTranscriptViewerStatus {
    ...
    Loading,
    /// Viewing a user conversation (not from ambient agent).
    ViewingUserConversation,
...
pub enum TmuxInstallationState {
    /// This means tmux was installed by Ashide in this session, successfully or unsuccessfully.
...
pub struct TerminalModel {
    ...
    /// Screen/grid that owns the current IME composition. It is fixed when
    /// marked text is set, so a late clear cannot target a newer active block.
    marked_text_carrier: Option<MarkedTextCarrier>,
    ...
    /// Force bootstrapped sessions in this terminal to be classified as Environment Runtime.
    /// Used by native Environment Runtime PTYs, where the transport is already known
    /// by construction and should not rely on hostname heuristics.
    force_environment_runtime_session_type: bool,

    /// The long-lived environment-owned runtime session that backs native PTYs
    /// opened by this terminal model.
    environment_runtime_transport_session_id: Option<SessionId>,

    /// Session IDs that have bootstrapped as Environment Runtime sessions.
    ///
    /// The terminal model is the layer that receives iTerm/kitty/image escape
    /// sequences, but it does not own the canonical `Sessions` registry. Keep
    /// this small local cache in sync from `BootstrappedEvent` construction so
    /// model-level filesystem side effects can remain current-app scoped.
    environment_runtime_session_ids: HashSet<SessionId>,
...
impl TerminalModel {
    ...
    pub fn new_for_test(
        ...
        use super::session::get_current_app_hostname;
        ...
        // We need to set the hostname to the current app host to ensure that this
        // test model uses the current-app environment rather than Environment Runtime.
        // See `SessionInfo::determine_session_type()` for details.
        let hostname = get_current_app_hostname().unwrap_or_else(|_| "localhost".to_string());
    ...
    fn new_internal(
        ...
        Self {
            ...
            force_environment_runtime_session_type: false,
            environment_runtime_transport_session_id: None,
            environment_runtime_session_ids: HashSet::new(),
            obfuscate_secrets,
    ...
    /// Returns whether the underlying PTY lifecycle has reached its terminal state.
    ///
    /// Session ownership uses this as the release truth for a closing durable
    /// CLI-agent process. UI detachment alone is not sufficient: the child can
    /// still be alive briefly after its pane/window disappears.
    ...
    pub fn pending_session_uses_environment_runtime(&self) -> bool {
        self.pending_session_info
            .as_ref()
            .map(|session_info| session_info.session_type.uses_environment_runtime())
            .unwrap_or(false)
    }
    ...
    pub fn set_force_environment_runtime_session_type(
        &mut self,
        force_environment_runtime_session_type: bool,
    ) {
        self.force_environment_runtime_session_type = force_environment_runtime_session_type;
    }
    ...
    pub fn active_block_uses_environment_runtime(&self) -> bool {
        if let Some(session_id) = self.active_block_metadata().session_id() {
            self.environment_runtime_session_ids.contains(&session_id)
        } else {
            self.pending_session_uses_environment_runtime()
                || self.force_environment_runtime_session_type
        }
    }

    fn should_save_non_inline_iterm_image_to_current_app_fs(&self) -> bool {
        !self.active_block_uses_environment_runtime()
    }
    ...
    /// Starts the active block and resets block-to-block state. For current-app sessions, this is called
    ...
    /// Cancels an incomplete in-band generator payload because a user command is
    /// taking ownership of the terminal. This is not a protocol end marker and
    /// therefore must not decode partial output or warn when no receiver exists.
    pub fn cancel_in_band_command_output(&mut self) {
...
impl TerminalModel {
    ...
    pub fn set_marked_text(&mut self, marked_text: &str, selected_range: &Range<usize>) {
        ...
        self.marked_text_carrier = if self.alt_screen_active {
            self.alt_screen.set_marked_text(marked_text, selected_range);
            Some(MarkedTextCarrier::AltScreen)
        } else {
            self.block_list
                .set_marked_text(marked_text, selected_range)
                .map(MarkedTextCarrier::Block)
        };
    }
    ...
    pub fn clear_marked_text(&mut self) {
        ...
        match self.marked_text_carrier.take() {
            Some(MarkedTextCarrier::AltScreen) => self.alt_screen.clear_marked_text(),
            Some(MarkedTextCarrier::Block(block_id)) => {
                self.block_list.clear_marked_text_for_block(&block_id)
            }
            None => {}
        }
    }
...
impl ansi::Handler for TerminalModel {
    ...
    fn bootstrapped(&mut self, value: BootstrappedValue) {
        ...
        if fully_populated_session_info
            .session_type
            .uses_environment_runtime()
        {
            self.environment_runtime_session_ids
                .insert(fully_populated_session_info.session_id);
        }
    ...
    fn init_shell(&mut self, data: InitShellValue) {
        if !self.ignore_bootstrapping_messages {
            ...
            let mut pending_session_info = SessionInfo::create_pending(
                shell_type,
                data,
                subshell_info,
            ...
            if self.force_environment_runtime_session_type {
                pending_session_info.environment_runtime_transport_session_id =
                    self.environment_runtime_transport_session_id;
            }
        ...
        match std::mem::replace(
            ...
            IsReceivingITermImageData::Yes { mut pending } => {
                ...
                if !pending.metadata.inline {
                    #[cfg(not(target_family = "wasm"))]
                    {
                        // Non-inline iTerm images ask the terminal to write a
                        // file under the active shell cwd. For Environment
                        // Runtime sessions that cwd is remote. Do not treat it
                        // as a current-app path; a future environment file API
                        // can implement the remote write explicitly.
                        if self.should_save_non_inline_iterm_image_to_current_app_fs() {
