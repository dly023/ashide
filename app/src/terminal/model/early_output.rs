...
pub enum TypeaheadMode {
    /// The shell reports its input buffer to Ashide, and we use that for typeahead.
    ShellReported,
    /// Ashide matches user input against characters echoed to the PTY to estimate typeahead.
    /// This is only used on bash 3.2 and should be removed if we stop supporting
    /// such old bash versions.
    InputMatching,
}

/// Model for "early" terminal output. Early output is output that Ashide receives
/// from the PTY while no block is running. In concrete terms, it's output received
/// after a `BlockFinished` hook but before Ashide has written the next command from
...
impl EarlyOutput {
    ...
    pub fn preexec(block_list: &mut BlockList) {
        if block_list.early_output().mode == TypeaheadMode::ShellReported {
            ...
            // When the command doesn't come from Ashide, however, the active block isn't
            // started until we receive the preexec hook. At this point, the shell has
            // already written the command to the pty, resulting in Ashide treating it as
            // background output.
