...
#[test]
fn terminal_bootstrap_captures_cli_agent_execution_context() {
    let pending = SessionInfo::create_pending(
        ShellType::Zsh,
        ansi::InitShellValue {
            shell: "zsh".into(),
            ..Default::default()
    ...
    let session_info = pending.merge_from_bootstrapped_value(
        ansi::BootstrappedValue {
            shell: "zsh".into(),
            shell_path: Some("/bin/zsh".into()),
            home_dir: Some("/home/target".into()),
            path: Some("/target/bin".into()),
            working_directory: Some("/workspace/target".into()),
            codex_home: Some("/target/codex".into()),
            claude_config_dir: Some("/target/claude".into()),
            ..Default::default()
        },
        false,
    );
    ...
    assert_eq!(
        session_info.execution_context.working_directory.as_deref(),
        Some("/workspace/target")
    );
    assert_eq!(
        session_info.execution_context.environment_variables,
