...
/// Indicates that the following JSON-encoded message is hex-encoded for Ashide's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Ashide's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Ashide's lifecycle
/// hooks via key-value pairs.
/// In OSC< it is used as the first parameter.
pub(super) const UNENCODED_KV_MARKER: char = 'k';

/// Enum representing all possible JSON payloads for Ashide's DCS's.
    ...
    pub fn populate_field(&mut self, key: String, v: String) {
        ...
        match self {
            ...
            DProtoHook::Bootstrapped { value } => match key.as_ref() {
                ...
                "working_directory" => {
                    value.working_directory = map_empty_to_none(v);
                }
                "codex_home" => {
                    value.codex_home = map_empty_to_none(v);
                }
                "claude_config_dir" => {
                    value.claude_config_dir = map_empty_to_none(v);
                }
                "editor" => {
...
/// Received from the pty after the shell has finished executing Ashide's
/// bootstrap script.
...
/// Received from the pty when Ashide's SSH wrapper is executed, prior to
...
/// created; if the session is for a subshell, this triggers Ashide's bootstrap process.
...
/// The binding is triggered when Ashide writes the `ESC-i` escape sequence to the pty.
/// Ashide usually does this after a block completes, to collect any typeahead
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Ashide client drop
/// per-session resources — in particular the `ssh … environment-runtime-proxy`
