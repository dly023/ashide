...
pub trait Handler {
    ...
    /// Callback for the Ashide CommandFinished hook.
    ...
    /// Callback for the Ashide precmd hook.
    fn precmd(&mut self, _data: PrecmdValue) {}

    /// Callback for the Ashide preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Ashide bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Ashide pre-interactive SSH session hook - called once
    ...
    /// Callback for the Ashide ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Ashide exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Ashide client a chance to drop per-session
    /// resources (e.g. the `ssh … environment-runtime-proxy` child) before the outer
    ...
    /// input buffer (the reporting is itself triggered by Ashide).
    ...
    /// created. If the session is for a subshell, this should triggers Ashide's bootstrap process.
    ...
    /// These allow external programs to trigger notifications in Ashide.
