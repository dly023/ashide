...
//! executing one of Ashide's DCS hooks, etc. [`Handler`] should be implemented by
...
/// Marks an OSC as one that is sent by Ashide logic registered in the shell.
///
...
/// of checks ensuring that Ashide's grids and ConPTY's grid are in sync.
...
/// between each update.
///
/// There are two mechanisms to prevent Ashide from falling too behind:
    ...
    W: io::Write + 'a,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Received a Ashide OSC used for in-band generators.
            WARP_IN_BAND_GENERATOR_OSC_MARKER => match params.get(1) {
                Some(&WARP_IN_BAND_GENERATOR_START_BYTE) => {
                    log::info!("Received a Ashide OSC marker for starting in-band command output.");
                ...
                _ => {
                    log::warn!("Received a Ashide OSC marker missing required param.");
                }
            },

            // Received a Ashide OSC used for shell hooks.
            WARP_OSC_MARKER => {
                ...
                match json_marker_char {
                    HEX_ENCODED_JSON_MARKER => {
                        ...
                        let Some(data_str) = params
                            ...
                            log::error!("Ashide OSC marker did not contain payload");
                            return;
                        };
                        safe_debug!(
                            safe: ("Received Ashide OSC string for shell hook"),
                            full: ("Received Ashide OSC string for shell hook with JSON payload: {:?}", data_str)
                        );
            ...
            WARP_RESET_GRID_OSC_MARKER => {
                log::debug!("Received Ashide OSC string for reset grid");
                self.handler.on_reset_grid();
            }

            // Received a Ashide OSC used for completions.
            WARP_COMPLETIONS_OSC_MARKER => match params.get(1) {
                Some(&WARP_COMPLETIONS_START_BYTE) => {
                    let Some(format) = params
                        ...
                        log::warn!("Ashide start completions OSC marker contained invalid format.");
                        return;
                    };
                ...
                Some(&WARP_COMPLETIONS_MATCH_RESULT_BYTE) => {
                    ...
                    let Some(data_str) = params
                        ...
                        log::warn!(
                            "Ashide completions match result OSC marker did not contain payload"
                        );
                        return;
                    };
                ...
                Some(bytes) if bytes.starts_with(WARP_COMPLETIONS_MATCH_UPDATE_METADATA) => {
                    ...
                    let Some(data_str) = params
                        ...
                        log::warn!(
                            "Ashide completions match metadata OSC marker did not contain payload"
                        );
                        return;
                    };
                    ...
                    match &parameter[2..] {
                        ...
                        _ => {
                            log::warn!("Invalid Ashide OSC marker parameter for completions match metadata: {parameter}");
                        }
                    }
                }
                ...
                _ => {
                    log::warn!("Received a Ashide OSC completions marker missing required param.");
                }
            },
