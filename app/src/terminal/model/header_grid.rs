...
//! Ashide. This struct is abstracted away from Block, for the purposes of enabling same-line prompt,
...
pub struct HeaderGrid {
    ...
    /// The height of the Ashide prompt in lines (non-PS1).
...
impl HeaderGrid {
    ...
    pub fn set_honor_ps1(&mut self, honor_ps1: bool) {
        ...
        if !self.honor_ps1 {
            // If we are switching to Ashide prompt (from PS1), we need to clear the cached prompt end point
    ...
    fn is_command_finished_and_empty(&self) -> bool {
        if !self.honor_ps1 {
            // If we are using Ashide prompt, we expect the combined grid cursor to be at the start, if
    ...
    fn precmd(&mut self, data: PrecmdValue) {
        if let Some(honor_ps1) = data.honor_ps1 {
            if honor_ps1 != self.honor_ps1 {
                ...
                // to be emitted. This may be de-synced from Ashide settings in particular niche cases (which are
