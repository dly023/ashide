...
pub fn test_restore_blocks_with_local_status() {
    ...
    assert_eq!(
        block_list.blocks[0].restored_block_used_current_app_environment(),
        Some(true)
    );
    assert_eq!(
        block_list.blocks[1].restored_block_used_current_app_environment(),
        Some(false)
    );
    assert_eq!(
        block_list.blocks[2].restored_block_used_current_app_environment(),
        None
    );
...
pub fn test_block_heights_combined_prompt_command_grid_warp_prompt() {
    ...
    // Note that this test is using the Ashide prompt, hence the prompt is not included in the combined grid.
    ...
    // we have the built-in Ashide prompt, so there's padding between that prompt and the combined grid.
...
pub fn test_block_heights_combined_prompt_command_grid_ps1() {
    ...
    // We have a 2-line prompt, adding 1 extra line to the combined grid (vs 0.6 default for Ashide prompt).
