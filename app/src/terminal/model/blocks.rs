...
impl BlockList {
    ...
    fn seek_down_to_next_grid(
        ...
        match grid_type {
            GridType::Output => {
                ...
                // vs the next "command" (in the Ashide prompt case), when using the combined grid, rather than
    ...
    #[cfg(test)]
    pub fn create_new_block_with_current_app_environment_status(
        &mut self,
        block_id: BlockId,
        bootstrap_stage: BootstrapStage,
        precmd_value: Option<PrecmdValue>,
        restored_block_used_current_app_environment: bool,
    ) {
        self.create_new_block(
            block_id,
            bootstrap_stage,
            precmd_value,
            Some(restored_block_used_current_app_environment),
        );
    }
    ...
    fn create_new_block(
        ...
        if let Some(is_local) = restored_block_used_current_app_environment {
            block.set_restored_block_used_current_app_environment(is_local);
        }
    ...
    pub fn create_restored_command_block(
        ...
        self.create_new_block(
            ...
            BootstrapStage::PostBootstrapPrecmd,
            Some(precmd_value),
            None, // restored_block_used_current_app_environment
        );
    ...
    pub fn reinit_shell(&mut self) {
        ...
        self.create_new_block(
            ...
            BootstrapStage::WarpInput,
            None, /* precmd_value */
            None, /* restored block current-app status */
        );
        self.bootstrap_stage = BootstrapStage::WarpInput;
    }
    ...
    fn restore_block(
        ...
        self.event_proxy
            .send_terminal_event(AfterBlockCompleted(AfterBlockCompletedEvent {
                ...
                // We do not track if a restored block was an object-store workflow execution.
    ...
    fn send_after_block_completed_event(&self, finished_block: &Block) {
        let block_type = finished_block.into();
        self.event_proxy
            .send_terminal_event(AfterBlockCompleted(AfterBlockCompletedEvent {
                block_type,
                num_secrets_obfuscated: finished_block.num_secrets_obfuscated(),
                object_store_workflow_id: finished_block.object_store_workflow_state(),
                local_env_var_collection_id: finished_block.local_env_var_collection_state(),
            }));
    }
            ...
            if !background_block.is_empty(&agent_view_state) {
                ...
                self.event_proxy.send_terminal_event(AfterBlockCompleted(
                    AfterBlockCompletedEvent {
                        ...
                        // Background blocks are not tracked as object-store workflow executions.
                        object_store_workflow_id: None,
                        local_env_var_collection_id: None,
                    },
                ));
    ...
    pub(in crate::terminal) fn clear_marked_text_for_block(&mut self, block_id: &BlockId) {
        if let Some(block) = self.mut_block_from_id(block_id) {
            block.clear_marked_text();
        }
    }
...
impl ansi::Handler for BlockList {
    ...
    fn clear_screen(&mut self, mode: ClearMode) {
        ...
        match mode {
            ...
            ClearMode::All => {
                ...
                // Ashide's custom logic for "clear". It's not immediately straightforward because a
                ...
                // quite right with Ashide's custom clear screen logic.
            }
            _ => {}
        }
    ...
    fn precmd(&mut self, data: PrecmdValue) {
        self.latest_block_finished_time.take();
        ...
        if let Some(previous_block) = previous_block {
            self.send_after_block_completed_event(previous_block);
        } else {
            self.event_proxy
