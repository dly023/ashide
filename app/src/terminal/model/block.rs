...
use crate::{
    ...
    terminal::{
        ...
        event::{
            BlockCompletedEvent, BlockMetadataReceivedEvent, BlockType, Event, UserBlockCompleted,
        },
...
pub struct Block {
    ...
    /// If the command is an object-store workflow, this is set to its id. If the block was not a workflow,
    /// this is None.
    object_store_workflow_id: Option<ObjectStoreId>,

    /// If the command included an env var invocation. If not this will be None.
    local_env_var_collection_id: Option<ObjectStoreId>,
    ...
    /// Only set on restored blocks. Indicates whether the block used the current app environment.
    restored_block_used_current_app_environment: Option<bool>,
...
pub enum BlockState {
    ...
    /// This block holds static content and is programmatically added to the blocklist by Ashide. An
...
impl Block {
    ...
    pub fn set_restored_block_used_current_app_environment(
        &mut self,
        used_current_app_environment: bool,
    ) {
        debug_assert!(
            self.bootstrap_stage == BootstrapStage::RestoreBlocks,
            "set_restored_block_used_current_app_environment should only be called for restored blocks"
        );
        self.restored_block_used_current_app_environment = Some(used_current_app_environment);
    }

    pub fn restored_block_used_current_app_environment(&self) -> Option<bool> {
        self.restored_block_used_current_app_environment
    }
    ...
    /// line prompt, we render on the same line for PS1, but not for Ashide prompt!
    ...
    pub fn finish(&mut self, exit_code: impl Into<ExitCode>) {
        ...
        self.event_proxy
            .send_terminal_event(Event::BlockCompleted(BlockCompletedEvent {
                ...
                restored_block_used_current_app_environment: self
                    .restored_block_used_current_app_environment,
            }));
    }
    ...
    /// when the shell echoes the command bytes that Ashide wrote to the PTY.
    ...
    pub fn prompt_and_command_grid_offset(&self) -> Lines {
        if self.header_grid.honor_ps1() {
            ...
            // Grid is drawn below custom Ashide prompt in finished blocks.
            self.block_banner_height()
                + self.padding_top()
                + self.prompt_height()
                + self.command_padding_top()
    ...
    /// In the case of combined grid: for Ashide prompt, this includes the height of both the Ashide prompt
    ...
    pub fn prompt_and_command_height(&self) -> Lines {
        if !self.ready_to_render() {
            ...
            // Handle the case of Ashide built-in prompt with combined grid.
    ...
    pub fn set_local_env_var_state(&mut self, env_var_collection_id: Option<ObjectStoreId>) {
        self.local_env_var_collection_id = env_var_collection_id;
    }

    pub fn local_env_var_collection_state(&self) -> Option<ObjectStoreId> {
        self.local_env_var_collection_id
    }

    pub fn set_object_store_workflow_state(&mut self, workflow_id: Option<ObjectStoreId>) {
        self.object_store_workflow_id = workflow_id;
    }

    pub fn object_store_workflow_state(&self) -> Option<ObjectStoreId> {
        self.object_store_workflow_id
    }
...
impl ansi::Handler for Block {
    ...
    fn linefeed(&mut self) -> ScrollDelta {
        ...
        // prompt, but this is not needed in Ashide due to us visually separating
        // blocks.
        match self.header_grid.receiving_chars_for_prompt {
