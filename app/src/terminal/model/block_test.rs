...
/// Regression test (CORE-1947): checks Ashide prompt case for is_command_empty. Specifically,
...
/// be against (0, 0) in the combined grid, for the Ashide prompt case, rather than checking against the
...
fn test_command_is_empty_warp_prompt() {
    ...
    // Note that we are indicating Ashide prompt, not PS1 here!
    let mut block = create_test_block_with_grids(
        block_index,
        prompt_and_command_grid,
        rprompt_grid,
...
#[test]
fn test_restored_block_used_current_app_environment() {
    // Test default value is None
    let mut block = TestBlockBuilder::new()
        .with_bootstrap_stage(BootstrapStage::RestoreBlocks)
        .build();
    assert_eq!(block.restored_block_used_current_app_environment(), None);

    // Test setting to true
    block.set_restored_block_used_current_app_environment(true);
    assert_eq!(
        block.restored_block_used_current_app_environment(),
        Some(true)
    );

    // Test changing value
    block.set_restored_block_used_current_app_environment(false);
    assert_eq!(
        block.restored_block_used_current_app_environment(),
        Some(false)
    );
