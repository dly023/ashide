...
#[cfg(feature = "local_tty")]
pub(crate) mod environment_runtime_executor;
#[cfg(feature = "local_tty")]
mod legacy_ssh_command_executor;
...
#[cfg(feature = "local_tty")]
pub use legacy_ssh_command_executor::LegacySshCommandExecutor;
#[cfg(feature = "local_tty")]
pub use local_command_executor::{LocalCommandExecutionContext, LocalCommandExecutor};
...
fn new_command_executor_for_local_tty_session(
    ...
    use environment_runtime_executor::EnvironmentRuntimeCommandExecutor;
    ...
    // Environment Runtime sessions must not use the in-band executor. Native
    // runtime PTYs are visible shell sessions that alias to an
    // environment-owned transport session, so `client_for_session` resolves the
    // alias before returning the connected runtime client.
    //
    // Legacy SSH sessions still keep the explicit ControlMaster fallback when
    // runtime setup was skipped or failed. Native Environment Runtime sessions
    // have no safe legacy shell transport to fall back to, so use NoOp until a
    // connected runtime client exists instead of sending generator commands into
    // the live PTY.
    if crate::workspace::environment_runtime::environment_runtime_feature_enabled()
        && (session_info.session_type.uses_environment_runtime() || is_legacy_ssh_session)
    {
        let session_id = session_info.session_id;
        let maybe_client =
            crate::workspace::environment_runtime::client_for_session(session_id, ctx);
        if let Some(client) = maybe_client {
            log::info!("creating an environment runtime executor for session {session_id:?}");
            return Arc::new(EnvironmentRuntimeCommandExecutor::new(session_id, client));
        }
        if session_info.session_type.uses_environment_runtime() {
            log::warn!(
                "environment runtime session {session_id:?} has no connected client; \
                 using no-op command executor instead of in-band"
            );
            return Arc::new(NoOpCommandExecutor::new());
        }
        log::info!(
            "environment runtime enabled but no connected client for legacy SSH session \
             {session_id:?}; falling back to legacy SSH ControlMaster executor"
        );
    }
    ...
    match &session_info.session_type {
                ...
                && !force_use_in_band_generators =>
        {
            if let IsLegacySSHSession::Yes { socket_path } = &session_info.is_legacy_ssh_session {
                ...
                Arc::new(LegacySshCommandExecutor::new(
                    socket_path.clone(),
                    wsl_distro,
                ))
            } else {
        ...
        _ => {
            if *should_force_disable_in_band_generators {
                ...
                // codepath is only accessible via a user defaults command that a Ashide
