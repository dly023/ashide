...
/// Legacy SSH fallback `CommandExecutor` that executes the given `command` in a forked
/// process by opening a one-off SSH channel through the active session's
/// ControlMaster/ControlPath socket. This is only used when the native
/// Environment Runtime executor is unavailable or explicitly bypassed.
#[derive(Debug)]
pub struct LegacySshCommandExecutor {
    control_socket_path: PathBuf,
    wsl_distro: Option<String>,
}

impl LegacySshCommandExecutor {
...
#[async_trait]
impl CommandExecutor for LegacySshCommandExecutor {
    ...
    /// The legacy SSH ControlMaster executor cannot be run in parallel because an SSH host could have a value
