...
use crate::workspace::environment_runtime::{self, EnvironmentRuntimeSessionEvent};
#[cfg(feature = "local_tty")]
use command_executor::environment_runtime_executor::EnvironmentRuntimeCommandExecutor;
...
use crate::workspace::environment_runtime::EnvironmentRuntimeSetupState;
...
pub struct Sessions {
    ...
    /// Tracks the environment runtime setup state for SSH sessions. Keyed by the pending session ID.
    environment_runtime_setup_states: HashMap<SessionId, EnvironmentRuntimeSetupState>,
}
...
impl Sessions {
    pub fn new(
        ...
        // Track the connected host_id and reconnected client on the `Session`
        // type so downstream code can distinguish hosts and generator
        // commands keep using the current runtime transport. Native
        // Environment Runtime terminals are visible shell sessions aliased to
        // an environment-owned transport session, so updates for the transport
        // session must also apply to its visible aliases.
        #[cfg(feature = "local_tty")]
        if environment_runtime::environment_runtime_feature_enabled() {
            environment_runtime::subscribe_to_session_events(
                ctx,
                |sessions, event, ctx| match event {
                    EnvironmentRuntimeSessionEvent::Connected {
                        session_id: sid,
                        host_id,
                    } => {
                        for session in sessions.sessions_for_environment_runtime_transport(sid) {
                            session.set_environment_runtime_host_id(Some(host_id.clone()));
                        }
                    }
                    EnvironmentRuntimeSessionEvent::Disconnected { session_id: sid } => {
                        for session in sessions.sessions_for_environment_runtime_transport(sid) {
                            session.set_environment_runtime_host_id(None);
                        }
                    }
                    EnvironmentRuntimeSessionEvent::SetupStateChanged { session_id, state } => {
                        sessions.set_environment_runtime_setup_state(session_id, state);
                        ctx.notify();
                    }
                    EnvironmentRuntimeSessionEvent::Reconnected {
                        ...
                        for session in sessions.sessions_for_environment_runtime_transport(sid) {
                            ...
                            let new_executor = Arc::new(EnvironmentRuntimeCommandExecutor::new(
                                session_id,
                                client.clone(),
                            ));
                            session.set_command_executor(new_executor);
                            log::info!(
                                "Swapped command executor for session {session_id:?} after \
                                 runtime transport {sid:?} reconnected"
                            );
                        }
                    }
                },
        ...
        Self {
            ...
            environment_runtime_setup_states: Default::default(),
        }
    }

    #[cfg(feature = "local_tty")]
    fn sessions_for_environment_runtime_transport(
        &self,
        ...
        self.sessions
            .values()
            .filter(|session| {
                session.id() == transport_session_id
                    || session.info.environment_runtime_transport_session_id
    ...
    pub fn any_session_uses_environment_runtime(&self) -> bool {
        self.sessions
            .values()
            .any(|session| !session.uses_current_app_environment())
    }

    pub fn session_uses_environment_runtime(&self, session_id: SessionId) -> bool {
        self.sessions
            .get(&session_id)
            .map(|session| !session.uses_current_app_environment())
            .unwrap_or(false)
    }
    ...
    pub fn set_environment_runtime_setup_state(
        &mut self,
        session_id: SessionId,
        state: EnvironmentRuntimeSetupState,
    ) {
        self.environment_runtime_setup_states
            .insert(session_id, state);
    }

    /// Returns the current remote server setup state for the given session, if any.
    pub fn environment_runtime_setup_state(
        &self,
        session_id: SessionId,
    ) -> Option<&EnvironmentRuntimeSetupState> {
        self.environment_runtime_setup_states.get(&session_id)
    }
    ...
    pub fn initialize_bootstrapped_session(
        ...
        // For environment-runtime sessions, pick up the current host_id from
        // the manager so the session host identity is populated without
        // waiting for the next SessionConnected event. The
        // EnvironmentRuntimeCommandExecutor already has its client baked in, so
        // nothing else needs to be wired here.
        #[cfg(feature = "local_tty")]
        if environment_runtime::environment_runtime_feature_enabled()
            && matches!(
                session_info.session_type,
                BootstrapSessionType::EnvironmentRuntime
            )
        {
            if let Some(host_id) = environment_runtime::host_id_for_session(session_id, ctx) {
                session.set_environment_runtime_host_id(Some(host_id));
            }
        }
...
impl From<SessionType> for command_corrections::SessionType {
    fn from(session_type: SessionType) -> Self {
        match session_type {
            SessionType::EnvironmentRuntime { .. } => command_corrections::SessionType::Remote,
...
pub struct SessionInfo {
    ...
    pub session_type: BootstrapSessionType,
    /// For native Environment Runtime PTYs, this is the long-lived
    /// environment-owned transport session that backs the visible shell
    /// session. It lets runtime host/client lookups resolve without treating
    /// the visible shell as an owning remote-server connection.
    pub environment_runtime_transport_session_id: Option<SessionId>,
    pub host_info: HostInfo,
    pub tmux_control_mode: bool,
    pub wsl_name: Option<String>,
    /// If this is a subshell or Environment Runtime session, store the parent session ID here.
...
impl SessionInfo {
    ...
    pub fn create_pending(
        ...
        force_environment_runtime_session: bool,
        ...
        // to determine whether this should use the current app environment or Environment Runtime.
        let session_type = Self::determine_session_type(
            &init_shell_value,
            force_environment_runtime_session
                || matches!(&is_legacy_ssh_session, IsLegacySSHSession::Yes { .. }),
        );

        if launch_data.is_none()
            && is_legacy_ssh_session == IsLegacySSHSession::No
            && session_type.uses_current_app_environment()
        {
            log::warn!("pending shell launch data was None for a current-app session");
        }

        let spawning_session_id =
            if matches!(session_type, BootstrapSessionType::EnvironmentRuntime)
        ...
        SessionInfo {
            ...
            session_type,
            environment_runtime_transport_session_id: None,
            subshell_info,
    ...
    fn determine_session_type(
        ...
        match get_current_app_hostname() {
            Ok(local_hostname) => {
    ...
    fn determine_session_type(
        ...
        _force_environment_runtime_session: bool,
    ) -> BootstrapSessionType {
        // When the `remote_tty` feature is enabled--the session is always considered Environment Runtime.
        BootstrapSessionType::EnvironmentRuntime
    }
    ...
    pub fn merge_from_bootstrapped_value(
        ...
        let mut execution_environment_variables = HashMap::new();
        if let Some(home_dir) = &home_dir {
            execution_environment_variables.insert("HOME".to_owned(), home_dir.clone());
        }
        if let Some(path) = &bootstrapped_value.path {
            execution_environment_variables.insert("PATH".to_owned(), path.clone());
        }
        if let Some(codex_home) = &bootstrapped_value.codex_home {
            execution_environment_variables.insert("CODEX_HOME".to_owned(), codex_home.clone());
        }
        if let Some(claude_config_dir) = &bootstrapped_value.claude_config_dir {
            execution_environment_variables
                .insert("CLAUDE_CONFIG_DIR".to_owned(), claude_config_dir.clone());
        }
        if let Err(error) =
            remote_server::session_execution_context::mark_validated_target_session_snapshot(
                bootstrapped_value.shell_path.as_deref(),
                bootstrapped_value.working_directory.as_deref(),
                &mut execution_environment_variables,
            )
        {
            log::warn!(
                "terminal bootstrap did not produce a complete remote execution context: {error}"
            );
        }
        let execution_context = SessionExecutionContext {
            working_directory: bootstrapped_value.working_directory,
            environment_variables: execution_environment_variables,
        };
        ...
        SessionInfo {
            ...
            environment_runtime_transport_session_id: self.environment_runtime_transport_session_id,
...
/// information is not available until the environment runtime helper handshake completes,
/// which happens *after* the session is bootstrapped.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum BootstrapSessionType {
    /// The session host is the same host where Ashide is running.
    Local,

    /// The session host is a different host from where Ashide is running.
    EnvironmentRuntime,
}
...
pub enum SessionType {
    ...
    ///
    /// `host_id` is `Some` when Environment Runtime has completed the
    /// connection handshake. It is
    /// `None` when the feature flag is off or the connection hasn't been
    /// established yet.
    EnvironmentRuntime { host_id: Option<warp_core::HostId> },
}
...
impl From<BootstrapSessionType> for SessionType {
    fn from(bst: BootstrapSessionType) -> Self {
        match bst {
            ...
            BootstrapSessionType::EnvironmentRuntime => {
                SessionType::EnvironmentRuntime { host_id: None }
            }
        }
    }
}
impl BootstrapSessionType {
    pub fn uses_environment_runtime(&self) -> bool {
        matches!(self, BootstrapSessionType::EnvironmentRuntime)
    }

    pub fn uses_current_app_environment(&self) -> bool {
        !self.uses_environment_runtime()
    }
}

impl SessionType {
    pub fn uses_environment_runtime(&self) -> bool {
        matches!(self, SessionType::EnvironmentRuntime { .. })
    }
    ...
    pub fn environment_runtime_host_id(&self) -> Option<&warp_core::HostId> {
        match self {
            SessionType::EnvironmentRuntime { host_id } => host_id.as_ref(),
            SessionType::Local => None,
        }
    }

    pub fn is_environment_runtime_disconnected(&self) -> bool {
        matches!(self, SessionType::EnvironmentRuntime { host_id: None })
    }
}
...
pub struct Session {
    ...
    /// when Environment Runtime reports a connected session (to fill in the
...
impl Session {
    ...
    /// Updates the environment runtime `host_id` after the helper handshake
    /// completes (or clears it on disconnect).
    pub fn set_environment_runtime_host_id(&self, host_id: Option<warp_core::HostId>) {
        let mut st = self.session_type.lock();
        if let SessionType::EnvironmentRuntime { host_id: ref mut h } = *st {
            *h = host_id;
        }
    }
    ...
    pub fn is_subshell_or_ssh(&self) -> bool {
        matches!(self.session_type(), SessionType::EnvironmentRuntime { .. })
    ...
    /// server reconnect to swap in a new `EnvironmentRuntimeCommandExecutor`
    ...
    pub fn uses_current_app_environment(&self) -> bool {
        self.session_type().uses_current_app_environment()
    }

    async fn read_history_for_current_app_session(
        &self,
    ...
    async fn read_history_for_environment_runtime_session(&self) -> Vec<String> {
        let histfile = &self.info.histfile;
        let shell_type = self.info.shell.shell_type();
    ...
    pub async fn read_history(&self, is_kaspersky_running: bool) -> Vec<String> {
        if self.info.session_type.uses_environment_runtime() {
            self.read_history_for_environment_runtime_session().await
        } else {
            self.read_history_for_current_app_session(is_kaspersky_running)
                .await
        }
    }
...
/// Returns the hostname for the current app host.
pub fn get_current_app_hostname() -> Result<String> {
    cfg_if::cfg_if! {
        if #[cfg(not(target_family = "wasm"))] {
            ...
            gethostname()
                .into_string()
                .map_err(|os_string| {
                    anyhow::anyhow!("Failed to convert current app hostname OsString {os_string:?} into String.")
                })
        } else {
...
pub mod testing {
    ...
    impl SessionInfo {
        ...
        pub fn with_ssh_socket_path(mut self, socket_path: PathBuf) -> Self {
            if let BootstrapSessionType::Local = self.session_type {
                self.session_type = BootstrapSessionType::EnvironmentRuntime;
            }
    ...
    impl Session {
        ...
        pub fn test_remote() -> Self {
            Self::test_remote_with_command_executor(Arc::new(TestCommandExecutor::default()))
        }

        pub fn test_remote_with_command_executor(
            command_executor: Arc<dyn CommandExecutor>,
        ) -> Self {
            let info = SessionInfo::new_for_test()
                .with_session_type(BootstrapSessionType::EnvironmentRuntime)
                .with_shell_type(ShellType::Bash); // We only support UNIX-based Environment Runtime sessions.
            let session_type = SessionType::from(info.session_type.clone());
            Self {
                info,
                external_commands: Default::default(),
