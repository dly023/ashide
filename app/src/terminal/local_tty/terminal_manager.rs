...
use crate::terminal::writeable_pty::terminal_manager_util::{
    init_environment_runtime_controller, init_pty_controller_model,
    wire_up_environment_runtime_controller_with_view, wire_up_pty_controller_with_view,
};
...
type EnvironmentRuntimeController =
    writeable_pty::environment_runtime_controller::EnvironmentRuntimeController<
...
pub struct TerminalManager {
    ...
    /// The manager is responsible for managing the lifetime of the environment runtime controller.
    #[expect(dead_code)]
...
impl TerminalManager {
    ...
    pub fn create_model(
        ...
        shell_starter_override: Option<ShellStarter>,
        ...
        let wsl_name_or_shell_starter = shell_starter_override
            .map(|starter| ShellStarterSource::Override(starter).into())
            .or_else(|| ShellStarter::init(preferred_shell.clone()));
        ...
        // Initialize the EnvironmentRuntimeController.
        let environment_runtime_controller =
            init_environment_runtime_controller(&pty_controller, &model_events, ctx);
        ...
        wire_up_environment_runtime_controller_with_view(
    ...
    fn on_shell_determined(
        ...
        let shell_starter_source = match shell_starter_source {
            ...
            Err(error) => {
                log::error!("Could not resolve the selected shell: {error:#}");
                self.view.update(ctx, |terminal_view, ctx| {
                    terminal_view.on_pty_spawn_failed(error, ctx);
                });
        ...
        {
            match wsl_shell_starter.home_directory() {
                Ok(home_directory) => Some(home_directory),
                Err(error) => {
                    log::error!("Could not resolve the selected WSL home directory: {error:#}");
                    self.view.update(ctx, |terminal_view, ctx| {
                        terminal_view.on_pty_spawn_failed(error, ctx);
                    });
                    self.model().lock().exit(ExitReason::ShellNotFound);
    ...
    /// Sends bindkey to notify shell process to switch to Ashide prompt logic for prompt
...
pub fn get_shell_starter(
    chosen_shell: Option<AvailableShell>,
    ctx: &mut AppContext,
) -> anyhow::Result<Option<ShellStarter>> {
    let preferred_shell = chosen_shell.unwrap_or_else(|| {
        AvailableShells::handle(ctx).read(ctx, |shells, ctx| shells.get_user_preferred_shell(ctx))
    });
    let Some(shell_starter_or_wsl_name) = ShellStarter::init(preferred_shell) else {
        return Ok(None);
    };

    // TODO(alokedesai): Further refactor this function to make it clear that it's expensive.
    let shell_starter_source = warpui::r#async::block_on(async {
        shell_starter_or_wsl_name.to_shell_starter_source().await
    })?;
    Ok(shell_starter_source.map(get_shell_starter_internal))
}
...
fn get_shell_starter_internal(shell_starter_source: ShellStarterSource) -> ShellStarter {
    match shell_starter_source {
        ...
        ShellStarterSource::Fallback {
            ...
            if let Some(shell) = unsupported_shell {
                log::debug!("Falling back from unsupported shell {shell}");
            }
            ShellStarter::Direct(starter)
        }
    }
