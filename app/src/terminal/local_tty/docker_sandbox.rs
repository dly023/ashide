...
//!
//! This module owns everything specific to running a Ashide shell inside a
...
///
/// Lives under the Ashide per-user cache directory rather than `/tmp` so:
...
/// Resolves the absolute path to the `sbx` CLI binary using the Ashide
/// process's `PATH`.
///
/// Ashide's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Ashide is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Ashide panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Ashide writes this sandbox's bash init script.
