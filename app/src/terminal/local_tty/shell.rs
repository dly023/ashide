...
/// * On macOS, this includes `$APP_PATH/Contents/Resources/bin`, in which we put a wrapper around the Ashide CLI.
...
/// executable binary for one of Ashide's supported shell types (bash, fish, zsh).
...
impl ShellStarter {
    ...
    /// used to spawn a shell process for a new top-level Ashide session. If a WSL Distribution is
...
pub struct DirectShellStarter {
    ...
    /// Arguments to be passed to the shell binary at [`shell_path`] when spawning a new Ashide
    /// session.
    args: Vec<OsString>,
}
    ...
    pub async fn to_shell_starter_source(self) -> anyhow::Result<Option<ShellStarterSource>> {
        match self {
            ...
            ShellStarterSourceOrWslName::WSLName { distro_name } => {
                let starter = WslShellStarter::init_from_wsl_distribution(&distro_name).await;
                resolved_wsl_shell_starter_source(&distro_name, starter)
            }
        }
    }
...
impl WslShellStarter {
    async fn init_from_wsl_distribution(distribution: &str) -> anyhow::Result<Self> {
        ...
        let command_result = command::r#async::Command::new(Self::wsl_command())
            .args(wsl_exec_arguments(distribution, "printenv", ["SHELL"]))
            .output()
            .await;
        let shell_path = decode_wsl_path_result(command_result, "querying the login shell")?
            .to_string_lossy()
            .into_owned();
        ...
        let shell_type = if shell_path.contains("bash") {
            ...
            bail!(
                "The selected WSL distribution {distribution:?} uses unsupported shell {shell_path:?}"
            );
        };
    ...
    pub(super) fn home_directory(&self) -> anyhow::Result<PathBuf> {
        let command_result = command::blocking::Command::new(Self::wsl_command())
            .args(wsl_exec_arguments(&self.distribution, "printenv", ["HOME"]))
            .output();
        let home_dir = decode_wsl_path_result(command_result, "querying the home directory")?;
        ...
        .with_context(|| {
            format!(
                "Could not map the home directory for WSL distribution {:?} to a Windows path",
                self.distribution
            )
        })
    }
...
fn resolved_wsl_shell_starter_source(
    distribution: &str,
    starter: anyhow::Result<WslShellStarter>,
) -> anyhow::Result<Option<ShellStarterSource>> {
    starter
        .with_context(|| format!("Failed to start the selected WSL distribution {distribution:?}"))
        .map(|starter| Some(ShellStarterSource::Override(ShellStarter::Wsl(starter))))
}
...
fn arguments_for_session_spawning_command(
    ...
    match shell_type {
        ShellType::Zsh => {
            ...
            // begins with a space. We use this to hide Ashide bootstrap commands from the history.
            vec![
                "-c".to_owned().into(),
        ...
        ShellType::Bash => {
             ...
             * 5. Send an InitShell message to Ashide through escape sequences.
        ...
        ShellType::Fish => {
            ...
            vec![
                ...
                format!(
                    ...
                    // Fish's implementation of this breaks Ashide by emitting `OSC 133 A` but not
                    // `OSC 133 B` afterwards, which we have assumed. This is a temporary workaround.
                    // See this issue: https://github.com/dly023/ashide/issues/7588
...
fn wsl_arguments_for_session_spawning_command(
    ...
    let mut args = wsl_exec_arguments(distribution, shell_path, std::iter::empty::<OsString>());
    // Note we typically go through bash so that we can launch the user's shell
    // with a leading '-', making it a login shell.
    match shell_type {
...
/// Decode the output of a wsl.exe query while preserving the operation that failed.
fn decode_wsl_path_result(
    result: io::Result<process::Output>,
    operation: &str,
) -> anyhow::Result<UnixPathBuf> {
    let output = result.with_context(|| format!("Could not execute wsl.exe while {operation}"))?;
    if !output.status.success() {
        ...
        cfg_if::cfg_if! {
            ...
            // snippet does need to be guarded.
            if #[cfg(windows)] {
                let utf16 = output.stdout
                    .chunks_exact(2)
                    .map(|bytes| u16::from_le_bytes([bytes[0], bytes[1]]))
                    .collect_vec();
                let wsl_err_msg = String::from_utf16_lossy(&utf16);
            } else {
                let wsl_err_msg = String::new();
            }
        }
        // If wsl.exe was correctly invoked but the Linux command had an error, that will
        // be UTF-8.
        let detail = if wsl_err_msg.trim().is_empty() {
            String::from_utf8_lossy(&output.stderr).trim().to_owned()
        } else {
            wsl_err_msg.trim().to_owned()
        };
        bail!(
            "wsl.exe failed while {operation} with status {}: {detail}",
            output.status
        );
    }

    let value = take_until_utf16_crlf(output.stdout)
        .into_iter()
        .take_while(|byte| *byte != b'\n')
        .collect_vec();
    if value.is_empty() {
        bail!("wsl.exe returned an empty value while {operation}");
    }

    Ok(UnixPathBuf::from(value))
}
