...
fn build_host_shell_command(
    ...
    // Without it, Ashide can't render structured CLI agent notifications,
    ...
    unsafe {
        ...
        command.pre_exec(move || {
            ...
            if is_isolated {
                ...
                // in case of OOM. If the Ashide process is killed while hosting an ambient
...
/// Ashide panes/sandboxes don't race on or share the same host directories.
///
...
/// under the Ashide cache dir, and (2) the stopped `warp-sandbox-<id>`
...
fn prepare_docker_sandbox(starter: &DockerSandboxShellStarter) -> Result<()> {
    ...
    // the per-user Ashide cache dir rather than `/tmp`) prevents the init
    // script from being read or symlink-attacked by anyone other than the
    // Ashide user. The file itself is left at the default mode so the
