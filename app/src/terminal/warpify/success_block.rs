        ...
        let auto_warpify_snippet = auto_warpify_snippet.map(|(output_grid, can_write_to_rc)| {
            AutoWarpifySnippet {
                description: (if !output_grid.is_empty() {
                    ...
                    "In remote subshells, Ashide runs commands in the background to power completions, syntax highlighting, and other features."
                }).into(),
...
impl TypedActionView for WarpifySuccessBlock {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            WarpifySuccessBlockAction::AshideifySettings => {
                ctx.emit(WarpifySuccessBlockEvent::AshideifySettings);
            }
