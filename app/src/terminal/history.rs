...
use crate::{
    object_store::ids::{ClientId, HashableId as _, ObjectStoreId},
...
pub struct PersistedCommand {
    ...
    pub workflow_id: Option<ObjectStoreId>,
...
impl From<crate::persistence::model::Command> for PersistedCommand {
    fn from(command: crate::persistence::model::Command) -> Self {
        PersistedCommand {
            ...
            workflow_id: command.object_store_workflow_id.and_then(|workflow_id| {
                if let Some(client_id) = ClientId::from_hash(workflow_id.as_str()) {
                    Some(ObjectStoreId::ClientId(client_id))
                } else {
    ...
    pub fn linked_workflow(&self, ctx: &AppContext) -> Option<(WorkflowType, WorkflowSource)> {
        match self {
            LinkedWorkflowData::Id(id) => {
                ...
                let workflow_source = WorkflowSource::PersonalDrive;
                workflow.map(|workflow| {
                    (
                        WorkflowType::ObjectStore(Box::new(workflow.clone())),
                        workflow_source,
                    )
                })
            }
