...
use crate::ai::skills::environment_skill_inventory::{
    active_terminal_session_skill_inventory, EnvironmentSkillInventoryCache,
    EnvironmentSkillInventoryCacheEvent,
};
...
impl SlashCommandDataSource {
    pub fn new(args: DataSourceArgs, ctx: &mut ModelContext<Self>) -> Self {
        ...
        ctx.subscribe_to_model(&active_session, |me, event, ctx| match event {
            ActiveSessionEvent::UpdatedPwd | ActiveSessionEvent::Bootstrapped => {
                me.refresh_environment_skill_inventory(ctx);
                me.recompute_active_commands(ctx);
            }
        });
        ctx.subscribe_to_model(
            &EnvironmentSkillInventoryCache::handle(ctx),
    ...
    fn refresh_environment_skill_inventory(&self, ctx: &mut ModelContext<Self>) {
        let Some((client, session_id, cwd)) = self.environment_skill_inventory_refresh_args(ctx)
        else {
            return;
        };
        EnvironmentSkillInventoryCache::handle(ctx).update(ctx, |cache, ctx| {
            cache.refresh_for_session(client, session_id, cwd, ctx);
        });
    }

    fn environment_skill_inventory_refresh_args(
        &self,
        app: &AppContext,
    ) -> Option<(
        std::sync::Arc<environment_runtime::EnvironmentRuntimeClient>,
        warp_core::SessionId,
        Option<String>,
    )> {
        ...
        let host_id = session_type
            .as_ref()
            .and_then(|session_type| session_type.environment_runtime_host_id())?;
        let client = environment_runtime::client_for_host(host_id, app)?;
        let session = active_session.session(app)?;
        let cwd = active_session.current_working_directory().cloned();
        Some((client, session.id(), cwd))
    }
    ...
    fn recompute_active_commands(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        let uses_current_app_environment = self
            .active_session
            .as_ref(ctx)
            .session_type(ctx)
            .is_some_and(|st| st.uses_current_app_environment());
        if uses_current_app_environment {
            session_context |= Availability::CURRENT_APP_ENVIRONMENT;
        }
...
impl SyncDataSource for SlashCommandDataSource {
    ...
    fn run_query(
        ...
        if FeatureFlag::ListSkills.is_enabled() && AISettings::as_ref(app).is_any_ai_enabled(app) {
            ...
            let skills = active_terminal_session_skill_inventory(active_session, app)
                .into_skills_for_cli_agent_providers(cli_agent_providers, true, app);
    ...
    pub(super) fn from_skill(skill: &SkillDescriptor, app: &AppContext) -> Self {
        ...
        let icon = if let Some(override_icon) = skill.icon_override {
            ...
            match skill.provider {
                SkillProvider::Ashide => WarpIcon::WarpLogoLight,
