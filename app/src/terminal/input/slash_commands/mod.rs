...
use crate::object_store::{model::persistence::ObjectStoreModel, ObjectType};
    ...
    pub(super) fn execute_slash_command(
        ...
        match command.name {
                            ...
                            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                                toast_stack.add_transient_toast(
                                    DismissibleToast::error(
                                        "The /open-file command is only available in the current app environment"
                                            .to_owned(),
                                    ),
                                    window_id,
                                    ctx,
            ...
            compact if command.name == commands::COMPACT.name => {
                // `/compact` 与 `/compact-and` 共用本地会话压缩链路：
                // initial_prompt: None 表示只静默摘要，不在压缩后继续发送 prompt；
                // 自定义指令(`/compact <指令>`)会作为 prompt 传入摘要流程。
                if self
                    .ai_context_model
                    .as_ref(ctx)
                    .selected_conversation_id(ctx)
