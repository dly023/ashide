...
use warpui::elements::{ChildView, CrossAxisAlignment, Flex, MainAxisSize, ParentElement as _};
use warpui::{
...
impl InlineModelSelectorView {
    pub fn new(
        ...
        let menu_view = if FeatureFlag::InlineMenuHeaders.is_enabled() {
            ...
            // Discoverable BYOK entry: from the model picker, jump straight to
            // Settings > AI > Providers to add your own key / custom endpoint
            // (no cloud account or subscription). See BYOK-002.
            let use_own_model_button = ctx.add_view(|_| {
                ActionButton::new(
                    crate::t!("terminal-model-use-own-provider"),
                    ManageDefaultsTheme,
                )
                .with_icon(Icon::Plus)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(WorkspaceAction::ShowSettingsPageWithSearch {
                        search_query: String::new(),
                        section: Some(SettingsSection::AgentProviders),
                    });
                })
            });
            let header_config = InlineMenuHeaderConfig {
                ...
                trailing_element: Some(Box::new(move |_app: &AppContext| {
                    Flex::row()
                        ...
                        .with_children([
                            ChildView::new(&use_own_model_button).finish(),
