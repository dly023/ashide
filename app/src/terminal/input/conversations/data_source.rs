...
impl SyncDataSource for ConversationMenuDataSource {
    ...
    fn run_query(
        ...
        let active_session_uses_environment_runtime = active_session
            .session_type(app)
            .is_some_and(|session_type| session_type.uses_environment_runtime());
        let session_pwd = if filter_by_cwd && !active_session_uses_environment_runtime {
            active_session.current_working_directory().cloned()
        } else {
            None
        };

        // Conversation history metadata is still keyed by cwd only, with no
        // Environment authority. A runtime-backed session may share a path string
        // with the current app host, so the "Current Directory" filter must not
        // match those cwd-only current-app history rows. Until persisted
        // conversation metadata carries an Environment key, keep runtime-backed
        // current-directory results empty instead of leaking cross-environment
        // conversations.
