...
impl super::TerminalView {
    /// 判断给定会话是否由环境运行时托管。
    fn session_uses_environment_runtime(
        &self,
        ...
        #[cfg(all(feature = "local_tty", not(target_family = "wasm")))]
        {
            ...
            if let Some(session_id) = session_id {
                return environment_runtime::host_id_for_session(session_id, ctx).is_some();
            }
        }
    ...
    fn environment_dir_listing_context(
        &mut self,
        ...
        use crate::util::file::{EnvironmentDirListing, LinkValidationContext};
        use crate::workspace::environment_runtime;
        ...
        // 命中缓存(已就绪或拉取中)直接返回。
        if let Some(entry) = self.environment_dir_listing_cache.get(&cache_key) {
            return LinkValidationContext::Environment(entry.clone());
        }

        // 取该会话的环境运行时客户端。
        let Some(client) = environment_runtime::client_for_session(session_id, ctx) else {
            return LinkValidationContext::Environment(None);
        };
        ...
        while self.environment_dir_listing_cache.len() >= MAX_ENTRIES {
            // shift_remove_index 保持插入顺序;FIFO 头部是最旧的。
            self.environment_dir_listing_cache.shift_remove_index(0);
        }
        self.environment_dir_listing_cache
        ...
        ctx.spawn(
            async move {
                crate::workspace::environment_runtime::list_directory(&client, cwd_for_request)
                    .await
            },
            move |me, result, ctx| {
                // 拉取期间用户可能已经切换 cwd / 清空缓存,只有占位还在才写入。
                if !me.environment_dir_listing_cache.contains_key(&key_for_store) {
                    return;
                }
                match result {
                    Ok(listing) => {
                        ...
                        me.environment_dir_listing_cache
                            .insert(key_for_store.clone(), Some(listing));
                        // 列表到达,触发 re-render 让链接重新扫描并点亮。
                        ctx.notify();
                    }
                    Err(err) => {
                        log::warn!("环境运行时 ListDirectory 失败 {cwd_for_store:?}: {err}");
                        me.environment_dir_listing_cache.shift_remove(&key_for_store);
                    }
                }
            },
        );

        LinkValidationContext::Environment(None)
    }
    ...
    fn scan_for_file_path(
        ...
        // 判断被悬停 block 所属会话是否由环境运行时托管。
        // 环境运行时的文件不在当前应用文件系统上,需要用 `LinkValidationContext::Environment`
        // 携带运行时拉取来的真实目录列表做精确校验。
        ...
        let uses_environment_runtime = self.session_uses_environment_runtime(block_session_id, ctx);
        ...
        //
        // 环境运行时会话的 block `pwd()` 是 shell-integration 上报的运行时 cwd,
        // 拼接后即得到正确的环境绝对路径,因此运行时 block 也参与扫描(不再跳过)。
        let pwd_to_scan_for = match position {
            WithinModel::AltScreen(_) => {
                if uses_environment_runtime {
                    // 环境运行时会话:`pwd()` 返回的是 shell-integration 上报的活动 cwd。
                    self.pwd()
                } else {
                    self.current_app_pwd(ctx)
                }
            }
            WithinModel::BlockList(inner) => self
                .model
        ...
        // 环境运行时会话用缓存的 cwd 目录列表精确校验;当前应用环境会话保持 `Local`。
        let validation_ctx = match (&pwd_to_scan_for, block_session_id) {
            #[cfg(all(feature = "local_tty", not(target_family = "wasm")))]
            (Some(cwd), Some(session_id)) if uses_environment_runtime => {
                self.environment_dir_listing_context(session_id, cwd, ctx)
            }
