...
//! Codex), it displays a specialized footer with additional functionality.
...
impl TerminalView {
    ...
    pub(super) fn should_render_use_agent_footer(
        ...
        if cli_agent.is_some() {
            ...
            // disable Ashide AI still get the footer for third-party coding agents.
    ...
    pub(super) fn detected_cli_agent_resume_session_id_from_model(
        &self,
        model: &TerminalModel,
        agent: CLIAgent,
    ) -> Option<String> {
...
/// For CLI agent commands (e.g., Claude Code, Codex), displays a specialized
