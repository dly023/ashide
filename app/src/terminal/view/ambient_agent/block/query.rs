...
/// Renders the submitted prompt immediately while the environment is still being set up, so ambient-agent setup feels like interactive agent mode even before the first exchange is available in history.
