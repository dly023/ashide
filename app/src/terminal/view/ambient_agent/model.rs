...
pub struct AmbientAgentViewModel {
    ...
    /// The app conversation associated with this ambient-agent run, if any.
    /// Set for worker-owned child agents spawned via `start_agent` so the `run_id`
...
impl AmbientAgentViewModel {
    ...
    /// Sets the app conversation ID associated with this ambient-agent run.
    ...
    pub fn spawn_agent(
        ...
        let config = Some(AgentConfigSnapshot {
            environment_id: self.active_environment_id(ctx),
    ...
    fn spawn_internal(&mut self, mut request: SpawnAgentRequest, ctx: &mut ModelContext<Self>) {
        if let Some(environment_id) = self.active_environment_id(ctx) {
            let config = request
                .config
                .get_or_insert_with(AgentConfigSnapshot::default);
                            ...
                            match ai_api_error {
                                AIApiError::QuotaLimit => {
                                    me.handle_spawn_error(
                                        USAGE_LIMIT_TASK_FAILURE_MESSAGE.to_string(),
                                        ctx,
                                    );
                                    ctx.emit(AmbientAgentViewModelEvent::ShowAIUsageLimitNotice);
                                    return;
                                }
    ...
    fn active_environment_id(&self, ctx: &ModelContext<Self>) -> Option<String> {
        ActiveSession::as_ref(ctx)
            .environment_id_for_terminal_view(self.terminal_view_id)
            .map(str::to_owned)
    }
...
pub enum AmbientAgentViewModelEvent {
    ...
    /// Request to show the agent usage-limit notice.
    ShowAIUsageLimitNotice,
    ...
    /// for claude / other third-party harnesses.
