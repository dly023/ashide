...
fn test_any_text_file_type_supported() {
    ...
    // .svg is a non-markdown file, and we allow opening it in the Ashide Editor (as a "code" file).
    assert_eq!(
        result,
        Some(OpenablePath {
...
fn test_single_code_file() {
    let result = async_io::block_on(check_openable_in_warp(
        "cat app/src/bin/ashide.rs".to_string(),
        Some(workspace_root().to_string_lossy().to_string()),
        TopLevelCommandCaseSensitivity::CaseInsensitive,
        EscapeChar::Backslash,
    ));
    assert_eq!(
        result,
        Some(OpenablePath {
            path: workspace_root().join("app/src/bin/ashide.rs"),
