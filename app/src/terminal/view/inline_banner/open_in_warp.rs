...
/// Given an openable file, format a file-specific title for the Open in Ashide banner.
fn file_title_text(openable_path: &OpenablePath) -> String {
    match openable_path.file_type {
        OpenableFileType::Markdown => {
            "Did you know that Ashide can directly display Markdown files?".to_string()
        }
            ...
            cfg_if::cfg_if! {
                if #[cfg(not(target_family = "wasm"))] {
                    ...
                    match language.as_ref().map(|language| language.display_name()) {
                        Some(display_name) => {
                            format!("Did you know that Ashide can directly edit {display_name} files?")
                        }
                        None => "Did you know that Ashide can directly edit code?".to_string(),
                    }
                } else {
                    // The `languages` crate is not available on WASM, so use a fallback message.
                    "Did you know that Ashide can directly edit code?".to_string()
                }
            }
        }
    }
...
pub fn render_open_in_warp_banner(
    ...
    let button_text = match state.target.file_type {
        OpenableFileType::Markdown => "View in Ashide",
        OpenableFileType::Code | OpenableFileType::Text => "Edit in Ashide",
    };
