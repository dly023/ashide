...
//! Ashide's SSH extension on the remote host the shell just connected to,
...
//!   cards ("Install Ashide's SSH extension" / "Continue without installing"),
...
#[derive(Clone, Debug)]
pub enum EnvironmentRuntimeSetupChoiceViewAction {
    Install,
    Skip,
    ToggleDoNotAskAgain,
    AshideifySettings,
...
#[derive(Clone, Debug)]
pub enum EnvironmentRuntimeSetupChoiceViewEvent {
    Install,
    Skip,
    AshideifySettings,
}
...
pub struct EnvironmentRuntimeSetupChoiceView {
...
impl EnvironmentRuntimeSetupChoiceView {
        ...
        let buttons = ctx.add_typed_action_view(|_| {
            KeyboardNavigableButtons::new(vec![
                rich_navigation_button(
                    ...
                    EnvironmentRuntimeSetupChoiceViewAction::Install,
                ),
                rich_navigation_button(
                    ...
                    /* recommended */ false,
                    MouseStateHandle::default(),
                    EnvironmentRuntimeSetupChoiceViewAction::Skip,
                ),
            ])
        });
    ...
    fn render_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        HeaderConfig::new(crate::t!("environment-runtime-choice-title"), app)
    ...
    fn render_footer(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let checkbox = appearance
            ...
            .on_click(|ctx, _, _| {
                ctx.dispatch_typed_action(
                    EnvironmentRuntimeSetupChoiceViewAction::ToggleDoNotAskAgain,
                );
            })
            .finish();
        ...
        let manage_settings_link = appearance
            ...
            .link(
                ...
                Some(Box::new(|ctx| {
                    ctx.dispatch_typed_action(
                        EnvironmentRuntimeSetupChoiceViewAction::AshideifySettings,
                    );
                })),
...
impl Entity for EnvironmentRuntimeSetupChoiceView {
    type Event = EnvironmentRuntimeSetupChoiceViewEvent;
}

impl View for EnvironmentRuntimeSetupChoiceView {
    fn ui_name() -> &'static str {
        "EnvironmentRuntimeSetupChoiceView"
    }
...
impl TypedActionView for EnvironmentRuntimeSetupChoiceView {
    type Action = EnvironmentRuntimeSetupChoiceViewAction;
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            EnvironmentRuntimeSetupChoiceViewAction::Install => {
                ...
                ctx.emit(EnvironmentRuntimeSetupChoiceViewEvent::Install);
            }
            EnvironmentRuntimeSetupChoiceViewAction::Skip => {
                if self.do_not_ask_again {
                ...
                ctx.emit(EnvironmentRuntimeSetupChoiceViewEvent::Skip);
            }
            EnvironmentRuntimeSetupChoiceViewAction::ToggleDoNotAskAgain => {
            ...
            EnvironmentRuntimeSetupChoiceViewAction::AshideifySettings => {
                ctx.emit(EnvironmentRuntimeSetupChoiceViewEvent::AshideifySettings);
            }
        }
    }
}
