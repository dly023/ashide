...
impl OnboardingAgenticSuggestionsBlock {
    pub fn new(
        ...
        let matrix_save_directory = themes_dir()
            .into_os_string()
            .into_string()
            .unwrap_or("the Ashide themes directory.".to_string());
        ...
        let agent_suggestions = vec![
            ...
            (
                AgenticSuggestionsContent {
                    ...
                    prompt: format!("First check if {matrix_save_directory} exists, and create this path if it doesn't already exist. Then create a matrix theme for my Ashide terminal without a background image field, following exact YAML structure on the warp website without any extra or missing fields. Call it matrix.yaml and save it in the directory we previously created. Once you've verified that the theme is correct and ready to be applied, let me know by only saying 'The matrix theme is now available at <path>.'."),
    ...
    fn render_text(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const WELCOME_TEXT_LINE_ONE: &str = "Welcome to Ashide!";
