...
impl OnboardingPromptBlock {
    ...
    fn render_text(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const LINE_ONE: &str = "Next, let’s set up your prompt. Ashide has a custom prompt builder or you can select PS1 to honor your pre-existing prompt configuration.";
        const LINE_TWO: &str =
            "Ashide works with many custom prompts like oh-my-zsh, Starship, Powerlevel10K. ";
    ...
    fn render_existing_prompt_button_interior(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const LINK_DESTINATION: &str = "https://github.com/dly023/ashide/issues/new?assignees=&labels=Bug&projects=&template=01_bug_report.yml";
    ...
    fn render_warp_prompt_button_interior(&self, appearance: &Appearance) -> Box<dyn Element> {
        // Pixel values pulled from Figma mocks
        // https://www.figma.com/file/y888viqzWBoMpFTxQqkQEN/Activation?node-id=568:1595&mode=dev
        const HEADER_TEXT: &str = "Ashide prompt";
        const HEADER_MARGIN_LEFT: f32 = 4.;
