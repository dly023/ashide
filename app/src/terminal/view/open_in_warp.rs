...
use crate::{
    ...
    util::openable_file_type::{is_file_openable_in_ashide, OpenableFileType},
};
...
/// A path to a file that can be opened in Ashide, along with its type.
...
impl TerminalView {
    ...
    /// Whether or not the "Open in Ashide" banner is open.
        ...
        match action {
                ...
                match &self.inline_banners_state.open_in_warp_banner {
                    Some(banner_state) => {
                        ActionAccessibilityContent::Custom(AccessibilityContent::new_without_help(
                            format!("Open {} in Ashide", banner_state.target.path.display()),
            ...
            OpenInWarpBannerAction::LearnMore => {
                ActionAccessibilityContent::Custom(AccessibilityContent::new(
                    ...
                    "Learn more about opening Markdown files in Ashide",
                    WarpA11yRole::UserAction,
                ))
            }
        }
...
/// Examines `command` for a file openable in Ashide, returning the resolved path and type if found.
    ...
    for command in all_parsed_commands(command, escape_char) {
        ...
        if let Some(positionals) = positionals {
            for arg in positionals.iter() {
                ...
                let Some(file_type) = is_file_openable_in_ashide(relative_path) else {
                    continue;
                };
                ...
                if async_fs::metadata(&resolved).await.is_ok() {
                    // We've found a file that exists and can be opened in Ashide.
