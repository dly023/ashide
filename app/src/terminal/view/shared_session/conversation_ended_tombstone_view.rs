...
use crate::ai::blocklist::{format_usage_units, BlocklistAIHistoryModel};
...
impl TombstoneDisplayData {
    fn from_conversation(
        ...
        Self {
            ...
            skill_name: None,
            run_time,
            usage: Some(format_usage_units(conversation.usage_units())),
...
/// Displays metadata, artifacts, and actions like "Continue in current app".
pub struct ConversationEndedTombstoneView {
    ...
    #[cfg(not(target_family = "wasm"))]
    continue_in_current_app_button: Option<ViewHandle<ActionButton>>,
...
impl ConversationEndedTombstoneView {
    pub fn new(
        ...
        #[cfg(not(target_family = "wasm"))]
        let continue_in_current_app_button = conversation_id.map(|conv_id| {
                ...
                ActionButton::new(crate::t!("terminal-continue-locally"), PrimaryTheme)
                    ...
                    .on_click(move |ctx| {
                        ctx.dispatch_typed_action(
                            ConversationEndedTombstoneAction::ContinueInCurrentApp(conv_id),
                        );
                    })
            })
        });

        // In wasm, continuing in the current app is impossible so we instead
        // offer to open the conversation in warp (where you can continue in the current app).
        #[cfg(target_family = "wasm")]
        ...
        let view = Self {
            ...
            #[cfg(not(target_family = "wasm"))]
            continue_in_current_app_button,
            #[cfg(target_family = "wasm")]
            open_in_warp_button,
        };
        ...
        ctx.subscribe_to_view(
            ...
            |_, _, event, ctx| match event {
                ArtifactButtonsRowEvent::OpenPlan { document_uid } => {
                    // Ashide 本地化:在 shared session tombstone 中同样走本地
    ...
    fn render_metadata_row(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        if !UserWorkspaces::as_ref(app).is_byo_api_key_enabled() {
            if let Some(usage) = &self.display_data.usage {
    ...
    fn render_action_buttons(
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            // forked into a local Ashide conversation. Unknown harness (None) is
            ...
            if self.continue_in_current_app_button.is_some()
                && AISettings::as_ref(app).is_any_ai_enabled(app)
                && harness_allows_continue
            {
                row.add_child(
                    ChildView::new(self.continue_in_current_app_button.as_ref().unwrap()).finish(),
                );
                has_button = true;
            }
        }
...
pub enum ConversationEndedTombstoneAction {
    #[cfg(not(target_family = "wasm"))]
    ContinueInCurrentApp(AIConversationId),
    #[cfg(target_family = "wasm")]
...
impl TypedActionView for ConversationEndedTombstoneView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            #[cfg(not(target_family = "wasm"))]
            ConversationEndedTombstoneAction::ContinueInCurrentApp(conversation_id) => {
                ctx.dispatch_typed_action(&WorkspaceAction::ContinueConversationInCurrentApp {
