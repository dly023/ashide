...
use crate::ai::ambient_agents::task::{
    AgentConfigSnapshot, AmbientAgentTaskState, HarnessConfig, RequestUsage, TaskCreatorInfo,
};
use crate::ai::ambient_agents::AmbientAgentTask;
use crate::ai::artifacts::Artifact;
use crate::ai::blocklist::format_usage_units;
...
fn task_with_run_time_and_usage() -> AmbientAgentTask {
...
fn task_without_run_time_or_usage() -> AmbientAgentTask {
    let mut task = task_with_run_time_and_usage();
...
fn data_with_conversation_values() -> TombstoneDisplayData {
    TombstoneDisplayData {
        ...
        usage: Some("conv usage".to_string()),
        ..Default::default()
    }
}
...
#[test]
fn task_overrides_run_time_and_usage_when_present() {
    let task = task_with_run_time_and_usage();
    ...
    let expected_usage = format_usage_units((INFERENCE_COST + COMPUTE_COST) as f32);
    assert_eq!(data.run_time, Some(expected_run_time));
    assert_eq!(data.usage, Some(expected_usage));
}

#[test]
fn conversation_values_preserved_when_task_lacks_run_time_and_usage() {
    let task = task_without_run_time_or_usage();
    let mut data = data_with_conversation_values();

    data.enrich_from_task(task);

    assert_eq!(data.run_time.as_deref(), Some("conv run time"));
    assert_eq!(data.usage.as_deref(), Some("conv usage"));
}
