...
use crate::cli_agent_jsonl::canonical_codex_session_id;

use self::listener::CLIAgentSessionListener;
use super::CLIAgent;
use event::{CLIAgentEvent, CLIAgentEventType};

fn canonical_provider_session_id(
    agent: CLIAgent,
    session_id: Option<String>,
) -> Result<Option<String>, ()> {
    match (agent, session_id) {
        (CLIAgent::Codex, Some(session_id)) => {
            canonical_codex_session_id(&session_id).map(Some).ok_or(())
        }
        (_, session_id) => Ok(session_id),
    }
}
...
pub struct CLIAgentSessionContext {
    ...
    /// Stable title supplied by the session index / restored row before a live
    /// plugin event reports a fresher prompt or summary.
...
impl CLIAgentSessionContext {
    fn clean_title_text(text: Option<&str>) -> Option<String> {
        text.map(str::trim)
    ...
    pub(crate) fn latest_user_prompt(&self) -> Option<String> {
        Self::clean_title_text(self.query.as_deref())
    }
    ...
    pub(crate) fn title_like_text(&self) -> Option<String> {
        Self::clean_title_text(self.summary.as_deref())
            .or_else(|| Self::clean_title_text(self.fallback_title.as_deref()))
    }
}
...
pub struct CLIAgentSession {
    ...
    /// `None` when the session uses the current app environment.
    /// `Some("user@hostname")` when bound to a non-current-app environment host.
    /// Used as a key for per-host plugin install failure tracking.
    pub environment_host_key: Option<String>,
...
impl CLIAgentSession {
    pub fn uses_environment_host(&self) -> bool {
        self.environment_host_key.is_some()
    }
    ...
    fn apply_event(&mut self, event: &CLIAgentEvent) -> Option<CLIAgentSessionStatus> {
        let event_session_id =
            canonical_provider_session_id(self.agent, event.session_id.clone()).ok()?;
        ...
        self.session_context.session_id =
            event_session_id.or(self.session_context.session_id.take());
...
pub struct CLIAgentSessionsModel {
    ...
    /// Tracks (agent, environment_host_key) pairs where an auto plugin operation (install or update) has failed.
...
impl CLIAgentSessionsModel {
    ...
    pub fn register_listener(
        ...
        environment_host_key: Option<String>,
        should_auto_toggle_input: bool,
        listener: ModelHandle<CLIAgentSessionListener>,
        ctx: &mut ModelContext<Self>,
    ) {
        let Ok(session_id) = canonical_provider_session_id(agent, session_id) else {
            return;
        };
            ...
            .filter(|s| s.agent == agent)
        {
            ...
            session.environment_host_key = environment_host_key;
    ...
    /// `environment_host_key` is `None` for current-app sessions, `Some("user@hostname")` for an environment host.
    #[cfg(not(target_family = "wasm"))]
    pub fn record_plugin_auto_failure(
        &mut self,
        agent: CLIAgent,
        environment_host_key: Option<String>,
    ) {
        self.plugin_auto_failures
            .insert((agent, environment_host_key));
    }
    ...
    pub fn has_plugin_auto_failed(
        ...
        self.plugin_auto_failures
            .contains(&(agent, environment_host_key.clone()))
    }
}
