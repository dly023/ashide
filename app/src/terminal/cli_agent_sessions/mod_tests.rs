...
#[test]
fn codex_plugin_event_rejects_noncanonical_session_id_before_model_commit() {
    let body = r#"{"v":1,"agent":"codex","event":"session_start","session_id":"msg_06435be93b11cbcc016a55deda46808197a7e8894330ebe948"}"#;

    assert!(parse_event(Some("warp://cli-agent"), body).is_none());

    let canonicalized = parse_event(
        Some("warp://cli-agent"),
        r#"{"v":1,"agent":"codex","event":"session_start","session_id":"rollout-2026-07-15T12-00-00-11111111-1111-4111-8111-111111111111"}"#,
    )
    .unwrap();
    assert_eq!(
        canonicalized.session_id.as_deref(),
    ...
    let mut session = CLIAgentSession {
        ...
        session_context: CLIAgentSessionContext {
            session_id: Some(canonical_id.to_owned()),
            ..Default::default()
        },
    ...
    let event = CLIAgentEvent {
        ...
        session_id: Some("msg_06435be93b11cbcc016a55deda46808197a7e8894330ebe948".to_owned()),
        cwd: Some("/must-not-commit".to_owned()),
        project: None,
        payload: CLIAgentEventPayload {
            query: Some("must not commit".to_owned()),
            ..Default::default()
        },
    };
    ...
    assert!(session.session_context.cwd.is_none());
...
fn cli_agent_session_context_title_like_text_uses_trimmed_summary() {
    let context = CLIAgentSessionContext {
        ...
        fallback_title: Some("Indexed title".to_string()),
...
#[test]
fn cli_agent_session_context_title_like_text_uses_fallback_title_without_summary() {
    let context = CLIAgentSessionContext {
        fallback_title: Some("  Indexed session title  ".to_string()),
        ..Default::default()
    };

    assert_eq!(
        context.title_like_text(),
        Some("Indexed session title".to_string())
    );
}
...
#[test]
fn uses_environment_host_returns_true_when_environment_host_key_is_set() {
    let session = CLIAgentSession {
        ...
        plugin_version: None,
        draft_text: None,
        environment_host_key: Some("user@devbox".to_owned()),
        custom_command_prefix: None,
    };
    assert!(session.uses_environment_host());
}

#[test]
fn uses_environment_host_returns_false_when_environment_host_key_is_none() {
    let session = CLIAgentSession {
        agent: CLIAgent::Claude,
        status: CLIAgentSessionStatus::InProgress,
        session_context: CLIAgentSessionContext::default(),
...
#[test]
fn current_app_failure_is_shared_across_current_app_sessions() {
...
#[test]
fn current_app_failure_does_not_affect_environment_host_key() {
    let mut model = CLIAgentSessionsModel::new();

    model.record_plugin_auto_failure(CLIAgent::Claude, None);

    let environment_host_key = Some("user@devbox".to_owned());
    assert!(!model.has_plugin_auto_failed(CLIAgent::Claude, &environment_host_key));
}
