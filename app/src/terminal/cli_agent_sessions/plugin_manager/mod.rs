...
/// Manages the Ashide notification plugin for a specific CLI agent.
///
...
pub(crate) trait CliAgentPluginManager: Send + Sync {
    /// The minimum plugin version required by this Ashide build.
    ...
    /// Whether the Ashide notification plugin is installed.
    ...
    /// Install the Ashide notification plugin.
    ...
    /// Update the Ashide notification plugin to the latest version.
    /// Default returns an error — only agents with `can_auto_install() == true` should override.
    ...
    fn install_success_message(&self) -> &'static str {
        "Ashide plugin installed. Please restart the session to activate."
    }

    /// Toast message shown after a successful auto-update.
    fn update_success_message(&self) -> &'static str {
        "Ashide plugin updated. Please restart the session to activate."
    }
...
/// doesn't have Ashide notification plugin support.
