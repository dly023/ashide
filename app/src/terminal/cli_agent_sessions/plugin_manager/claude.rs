...
impl ClaudeCodePluginManager {
    pub(super) fn new(
        ...
        Self {
            executor: LocalCommandExecutor::new(shell_path, shell_type, Default::default()),
            path_env_var,
        }
    }
...
impl CliAgentPluginManager for ClaudeCodePluginManager {
    ...
    fn install_success_message(&self) -> &'static str {
        "Ashide plugin installed. Please run /reload-plugins to activate."
    }

    fn update_success_message(&self) -> &'static str {
        "Ashide plugin updated. Please run /reload-plugins to activate."
    }
...
/// Reads the installed version string for the Ashide plugin, if present.
...
/// Checks `CLAUDE_CONFIG_DIR` first, matching Claude Code, then falls back to `~/.claude`.
fn claude_home_dir() -> io::Result<PathBuf> {
    ...
    Ok(CliAgentStoreRoots::for_current_process(home_dir).claude_config_dir)
}
