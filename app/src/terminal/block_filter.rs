...
pub struct BlockFilterEditor {
    edit_session: Option<BlockFilterEditSession>,
    next_generation: u64,
...
pub struct BlockFilterEditSession {
...
pub enum BlockFilterEditorEvent {
    UpdateFilter {
        session: BlockFilterEditSession,
        query: BlockFilterQuery,
    },
    Close(BlockFilterEditSession),
}
...
impl BlockFilterEditor {
    ...
    pub fn open_and_set_filter(
        ...
        self.edit_session = Some(BlockFilterEditSession {
            block_index,
    ...
    pub fn edit_session(&self) -> Option<BlockFilterEditSession> {
        self.edit_session
    }

    pub fn is_current_session(&self, session: BlockFilterEditSession) -> bool {
        self.edit_session == Some(session)
    }

    fn close(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(session) = self.edit_session.take() {
            ctx.emit(BlockFilterEditorEvent::Close(session));
        }
    }
    ...
    fn update_query(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let Some(session) = self.edit_session else {
            return;
        };
