...
use crate::notifications::{
    item::NotificationSourceAgent, model::NotificationsModel, NotificationCategory,
    NotificationFilter,
};
...
#[test]
fn test_pending_session_launch_data_query_treats_bootstrap_as_unavailable() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let _shell_selector = FeatureFlag::ShellSelector.override_enabled(true);
        let terminal = add_window_with_terminal(&mut app, None);
        let session_id = SessionId::from(92);
        ...
        assert_eventually!(
            terminal.read(&app, |view, ctx| view
                .sessions
                .as_ref(ctx)
                .tracks_session(session_id)),
            "InitShell should register the session as pending"
        );
        ...
        terminal.read(&app, |view, ctx| {
            assert!(view.sessions.as_ref(ctx).get(session_id).is_none());
            assert!(view.current_app_shell_launch_data(ctx).is_none());
        });
    })
}
...
#[test]
fn test_create_new_block_with_current_app_environment_status() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        ...
        assert_eventually!(
            terminal.read(&app, |view, ctx| !view
                .active_block_uses_non_current_app_environment(ctx)),
            "Block should be local"
        );

        // No non-current-app environment blocks should exist
        assert_eventually!(
            terminal.read(&app, |view, _ctx| !view
                .contains_restored_non_current_app_environment_blocks()),
            "No non-current-app environment blocks should exist"
        );
        ...
        assert_eventually!(
            terminal.read(&app, |view, _ctx| !view
                .any_session_contains_non_current_app_environment_blocks),
            "No non-current-app environment blocks should exist"
        );
...
fn clear_buffer_action_in_fullscreen_agent_view_starts_new_conversation() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let _agent_view_flag_guard = FeatureFlag::AgentView.override_enabled(true);
...
fn test_clear_session_flag_state() {
    ...
    App::test((), |mut app| async move {
        ...
        remote_block.is_local = Some(false); // Mark it as a non-current-app environment block
        ...
        // Create terminal with the restored non-current-app environment block
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            assert!(
                !view.any_session_contains_non_current_app_environment_blocks,
                "Terminal should not have non-current-app environment blocks"
            );
            assert!(
                view.any_session_contains_restored_non_current_app_environment_blocks,
                "Terminal should have restored non-current-app environment blocks"
            );

            {
                ...
                // The first block should be our restored non-current-app environment block
                assert!(!blocks.is_empty(), "At least one block should exist");
                if let Some(first_block) = blocks.first() {
                    assert_eq!(
                        first_block.restored_block_used_current_app_environment(),
                        Some(false),
            ...
            assert!(
                !view.any_session_contains_non_current_app_environment_blocks,
                "Terminal should not have non-current-app environment blocks after clearing"
            );
            assert!(
                !view.any_session_contains_restored_non_current_app_environment_blocks,
                "Terminal should not have restored non-current-app environment blocks after clearing"
            );
        });
    })
}
...
fn test_not_bootstrapped() {
    App::test((), |mut app| async move {
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            assert_eq!(view.active_session_current_app_path(ctx), None);
        });
    })
}
...
fn test_bash_vim_banner_already_shown() {
    App::test((), |mut app| async move {
        ...
        // Ensure Ashide's vim keybindings are off.
...
fn test_bash_vim_banner_off() {
    App::test((), |mut app| async move {
        ...
        // Ensure Ashide's vim keybindings are on.
        AppEditorSettings::handle(&app).update(&mut app, |editor_settings, ctx| {
...
fn test_first_onboarding_block_exists() {
    App::test((), |mut app| async move {
        ...
        let _settings_import_flag_guard = FeatureFlag::SettingsImport.override_enabled(false);
        terminal.update(&mut app, |terminal_view, ctx| {
...
fn cli_agent_rich_input_hint_text_mentions_active_cli_agent() {
    App::test((), |mut app| async move {
        ...
        // Fluent wraps `{ $agent }` substitutions in bidi isolate marks
        // (U+2068 FIRST STRONG ISOLATE / U+2069 POP DIRECTIONAL ISOLATE) because
        // the loader uses the default `use_isolating = true`. The Unknown case
        // uses a separate key with no substitution, so it has no isolate marks.
...
#[test]
fn command_detected_structured_agent_stop_reaches_notification_model_without_session_start() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        ...
        terminal.read(&app, |view, ctx| {
            ...
            let items = NotificationsModel::as_ref(ctx)
                .notifications()
