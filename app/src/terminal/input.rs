...
use crate::{
    ai::{
        ...
        blocklist::{
            ...
            render_ai_follow_up_icon, BlocklistAIContextEvent, BlocklistAIContextModel,
    ...
    app_interaction::CommandXRayTrigger,
    ...
    object_store::{
        model::{actions::ObjectActionType, persistence::ObjectStoreModel},
...
use super::{
    ...
    view::{
        ...
        inline_banner::{PromptSuggestionBannerState, ZeroStatePromptSuggestionType},
...
const AGENT_MODE_AI_DISABLED_AUTODETECTION_DISABLED_HINT_FALLBACK: &str = "Run commands";
const CLI_AGENT_RICH_INPUT_HINT_FALLBACK: &str = "Tell the agent what to build...";
...
fn translate_cli_agent_rich_input_hint_key() -> String {
    crate::i18n::t_or(
        CLI_AGENT_RICH_INPUT_HINT_KEY,
        CLI_AGENT_RICH_INPUT_HINT_FALLBACK,
    )
}

fn translate_enter_prompt_for_cli_agent(agent_display_name: &str) -> String {
    if crate::i18n::loader().is_some() {
        crate::t!(
            "terminal-input-enter-prompt-for-agent",
            agent = agent_display_name
        )
    } else {
        format!("Enter prompt for {agent_display_name}...")
    }
}
...
pub enum CommandExecutionSource {
    /// A non-shared command execution request from Ashide AI.
...
pub enum Event {
    ...
    CreateDockerSandbox,
    /// Exit ambient agent and start a new parent terminal agent conversation in the root terminal.
    ///
    /// If `initial_prompt` is `Some`, it should prefill the parent terminal agent prompt but not auto-send.
    ExitAmbientAgentAndStartParentTerminalAgent {
        initial_prompt: Option<String>,
    },
    ScrollToExchange {
...
struct EnvVarCollectionState {
    selected_env_vars: Option<ObjectStoreId>,
}
...
pub struct Input {
    ...
    /// Incoming peer edits that are not yet applied
    ...
    /// if any of these pending peer edits can be flushed.
    ///
    /// Today, we only expect to use this for shared session viewers.
    deferred_peer_operations: DeferredPeerOperations,
...
/// A map of peer buffer operations that were deferred because
/// the corresponding block ID was not active when these operations
/// were received.
struct DeferredPeerOperations {
...
impl Input {
    ...
    pub(crate) fn new(
        ...
        let deferred_peer_operations =
            DeferredPeerOperations::new(model.lock().block_list().active_block_id().clone());
    ...
    fn handle_inline_prompts_menu_event(
        ...
        self.show_workflows_info_box_on_workflow_selection(
            WorkflowType::ObjectStore(Box::new(workflow)),
            WorkflowSource::AiAssistant,
            ...
            .skill_by_reference(&reference)
        {
            ...
            None => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_transient_toast(
    ...
    fn open_file_in_code_editor(
        ...
        if !session.uses_current_app_environment() {
            return Err(
                "Tried to open file in code editor for an Environment Runtime session".to_string(),
            );
        }
    ...
    fn agent_mode_hint_text(&mut self, app: &AppContext) -> String {
        ...
        let key = match (
            ...
            (InputType::AI, _) => {
                ...
                //  * `None` (no conversation, empty, passive, or untitled) => new conversation => "Ashide anything"
    ...
    /// Predicts the next action using an AI model and past context on blocks within Ashide.
    ...
    fn cli_agent_rich_input_hint_text(&self, ctx: &ViewContext<Self>) -> Cow<'static, str> {
        if self.is_locked_in_shell_mode(ctx) {
            return Cow::Owned(crate::i18n::t_or(
                AGENT_MODE_AI_DISABLED_AUTODETECTION_DISABLED_HINT_KEY,
                AGENT_MODE_AI_DISABLED_AUTODETECTION_DISABLED_HINT_FALLBACK,
            ));
        }
        ...
        CLIAgentSessionsModel::as_ref(ctx)
            ...
            .map(|session| match session.agent {
                CLIAgent::Unknown => Cow::Owned(translate_cli_agent_rich_input_hint_key()),
                _ => Cow::Owned(translate_enter_prompt_for_cli_agent(
                    session.agent.display_name(),
                )),
            })
            .unwrap_or_else(|| Cow::Owned(translate_cli_agent_rich_input_hint_key()))
    }
    ...
    /// Try to execute a command in the current-app session that was
    ...
    fn try_execute_command_from_source(
        ...
        self.model
            ...
            .block_list_mut()
            .active_block_mut()
            .set_local_env_var_state(env_var_collection_id);
    ...
    pub fn workflows_info_box_open_workflow_cloud_id(&self) -> Option<ObjectStoreId> {
    ...
    fn env_vars_command_prefix(
        &self,
        env_vars_id: &ObjectStoreId,
        ctx: &AppContext,
    ) -> Option<String> {
        ...
        if shell_type == ShellType::Fish {
            // Ashide currently doesn't support newlines in Fish, just prepend the vars
    ...
    fn reset_workflow_state(
        &mut self,
        env_vars: Option<ObjectStoreId>,
        ctx: &mut ViewContext<Input>,
    ) {
    ...
    fn conversation_attachment_for_ai_context_menu(
        ...
        if let Some(app_conversation_id) =
            history_model.find_conversation_id_by_server_token(&conversation_token)
        {
            if let Some(conversation) = history_model.conversation(&app_conversation_id) {
                let content =
        ...
        match event {
            EditorEvent::Edited(edit_origin) => {
                ...
                let should_run_ai_input_detection = match edit_origin {
                    ...
                    // Peer edits from shared session viewers should trigger autodetection
                ...
                match action {
                    ...
                    AIContextMenuSearchableAction::InsertFilePath { file_path } => {
                        ...
                        let file_path = if is_ai_mode
                            || self.active_session_current_app_path(ctx).is_none()
                        {
                            file_path.to_string()
                        } else {
                            #[cfg(feature = "local_fs")]
    ...
    fn can_attach_on_filepaths_paste_or_dragdrop(&self, ctx: &mut ViewContext<Self>) -> bool {
        // Shared session viewers cannot attach images in Ashide.
        let is_viewer = self.model.lock().shared_session_status().is_viewer();
        if is_viewer {
            return false;
        }
    ...
    fn open_completion_suggestions(
        ...
        // However, completions are disabled on environment runtime hosts because
        // in-band generators don't work in this context (with CLI agent).
        let is_cli_agent_shell_mode = self.is_locked_in_shell_mode(ctx)
            && CLIAgentSessionsModel::as_ref(ctx).is_input_open(self.terminal_view_id)
            && !self
                .active_session(ctx)
                .is_some_and(|s| s.session_type().uses_environment_runtime());
    ...
    pub(crate) fn input_enter(&mut self, ctx: &mut ViewContext<Self>) {
            ...
            && self.ai_input_model.as_ref(ctx).is_ai_input_enabled()
        {
                ...
                .is_configuring_ambient_agent()
            {
                ...
                // Collect pending files for the spawn request. Cloud mode image attachments are disabled in Ashide.
            ...
            if FeatureFlag::WorkflowAliases.is_enabled() {
                ...
                if let Some(alias) = WorkflowAliases::as_ref(ctx).match_alias(&command_string) {
                        ...
                        ObjectStoreModel::as_ref(ctx).get_workflow(&alias.workflow_id)
                    {
                        ...
                        let workflow_type = WorkflowType::ObjectStore(Box::new(workflow.clone()));
    ...
    fn input_cmd_enter(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match &mode {
            ...
            _ => {
                ...
                // new parent terminal agent conversation in the root terminal. This should work whether the
                ...
                if self.ambient_agent_view_model.as_ref(ctx).is_ambient_agent() {
                    ...
                    ctx.emit(Event::ExitAmbientAgentAndStartParentTerminalAgent {
    ...
    fn submit_ai_query(
        ...
        if let Some(workflow_state) = self.workflows_state.selected_workflow_state.as_ref() {
            if let WorkflowType::ObjectStore(workflow) = &workflow_state.workflow_type {
    ...
    pub fn process_shared_session_peer_edits(
        &mut self,
        block_id: &BlockId,
        ...
        if block_id == &self.deferred_peer_operations.latest_block_id {
            self.editor.update(ctx, |editor, ctx| {
                editor.apply_remote_operations(operations, ctx);
            });
        } else {
            self.deferred_peer_operations
    ...
    pub fn refresh_deferred_peer_operations(&mut self, ctx: &mut ViewContext<Self>) {
        let latest_block_id = self.model.lock().block_list().active_block_id().clone();
        self.deferred_peer_operations.latest_block_id = latest_block_id;
        self.flush_deferred_peer_operations(ctx);
    }

    /// Flushes any deferred peer operations for the latest known block ID.
    fn flush_deferred_peer_operations(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(operations) = self.deferred_peer_operations.flush() {
            self.editor.update(ctx, |editor, ctx| {
                editor.apply_remote_operations(operations, ctx);
            });
        }
    ...
    pub fn handle_block_completed_event(
        ...
        if let BlockType::User(user_block) = &block_completed_event.block_type {
            ...
            if should_clear_buffer {
                ...
                if self.deferred_peer_operations.latest_block_id != latest_block_id {
                    self.deferred_peer_operations.latest_block_id = latest_block_id;
                    self.editor
    ...
    pub fn handle_after_block_completed_event(
                ...
                .is_bootstrapping_precmd_done()
        {
            ...
            self.refresh_deferred_peer_operations(ctx);
    ...
    pub fn set_active_block_metadata(
        ...
        if self.is_voltron_open {
            let active_session_current_app_path =
                self.active_session_current_app_path(ctx).map(PathBuf::from);
            self.voltron_view.update(ctx, |voltron, ctx| {
                voltron
                    .refresh_active_session_current_app_path(active_session_current_app_path, ctx);
            });
        }
    ...
    fn active_session_current_app_path(&self, ctx: &ViewContext<Self>) -> Option<&Path> {
        self.active_block_session_id().and_then(|session_id| {
            let current_session = self.sessions.as_ref(ctx).get(session_id)?;
            if current_session.uses_current_app_environment() {
                self.active_block_metadata
                    .as_ref()
                    .and_then(BlockMetadata::current_working_directory)
                    .map(Path::new)
    ...
    fn select_and_refresh_voltron(
        ...
        let active_session_current_app_path = self.active_session_current_app_path(ctx);
        let menu_positioning = self.menu_positioning(ctx);
        let metadata = VoltronMetadata {
            active_session_current_app_path: active_session_current_app_path
                .map(|path| path.into()),
    ...
    fn handle_action(&mut self, action: &InputAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            InputAction::InsertZeroStatePromptSuggestion(suggestion_type) => {
                self.insert_zero_state_prompt_suggestion(*suggestion_type, ctx);
            }
            InputAction::EnableAutoDetection => {
    ...
    fn on_focus(&mut self, focus_ctx: &FocusContext, ctx: &mut ViewContext<Self>) {
        if focus_ctx.is_self_focused() {
            ...
            if let Some(focus_handle) = &self.focus_handle {
                focus_handle.commit_application_focus(ctx);
            }
        }
    }
