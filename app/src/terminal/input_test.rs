...
use crate::ai::skills::{
    environment_skill_inventory::EnvironmentSkillInventoryCache, SkillManager,
};
...
pub fn initialize_app(app: &mut App) {
    ...
    app.add_singleton_model(
        |_| crate::object_store::model::actions::ObjectActions::new(Vec::new()),
    );
...
fn create_object_store_workflow(app: &mut App, workflow: Workflow) -> ObjectStoreId {
...
pub async fn add_window_with_bootstrapped_terminal_and_window_id(
    ...
    let shell_starter_source = ShellStarter::init(Default::default())
        ...
        .await
        .expect("Could not resolve the shell starter source")
...
fn test_slash_menu_saved_prompt_inserts_context_reference() {
    App::test((), |mut app| async move {
        ...
        let workflow_id = create_object_store_workflow(
            &mut app,
            Workflow::AgentMode {
...
fn test_ai_context_menu_enters_ai_mode_for_ai_only_context_items() {
    App::test((), |mut app| async move {
        ...
        let terminal = add_window_with_bootstrapped_terminal(&mut app, None, None).await;
        let input = terminal.read(&app, |terminal, _| terminal.input().clone());
        let notebook_id = create_notebook_object(&mut app, "Notebook", "notebook context", None);
        let rule_id = create_cloud_rule(&mut app, "Rule", "rule context");
        let plan_document_id = AIDocumentId::new();
        create_notebook_object(&mut app, "Plan", "plan context", Some(plan_document_id));
        let cases = vec![
            (
                AIContextMenuSearchableAction::InsertDriveObject {
