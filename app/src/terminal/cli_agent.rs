...
//! like Claude Code, Codex, Amp, and Droid.
...
use crate::cli_agent_jsonl::{canonical_codex_session_id, AgentSessionDiscoveryProvider};
...
/// Ashide 对单个 Agent 的静态集成能力。
///
/// 安装、连接和运行时 readiness 不属于这里；这些动态状态由对应 backend 持有。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AgentCapabilities {
    pub can_detect: bool,
    pub can_bind_live_session: bool,
    pub can_index_sessions: bool,
    pub can_list_sessions: bool,
    pub can_cold_restore: bool,
    pub can_resume: bool,
    pub can_attach: bool,
    pub can_target_environment_runtime: bool,
    pub can_read_session_ir: bool,
    pub can_fork: bool,
    pub can_edit_preview: bool,
    pub can_write_native_history: bool,
    pub can_launch_derived_session: bool,
}

/// Agent identity 与 capability 的唯一静态 registry record。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AgentRecord {
    pub agent: CLIAgent,
    pub capabilities: AgentCapabilities,
    pub(crate) session_discovery_provider: Option<AgentSessionDiscoveryProvider>,
}
...
impl CLIAgent {
    ...
    pub fn resume_session_id_from_command(&self, command: &str) -> Option<String> {
        ...
        match self {
            ...
            CLIAgent::Omp => args.iter().enumerate().find_map(|(index, arg)| {
                if let Some(session_id) = arg.strip_prefix("--resume=") {
                    return non_empty_session_id(session_id);
                }
                if arg == "--resume" || arg == "-r" {
    ...
    pub fn record(self) -> AgentRecord {
        ...
        let capabilities = match self {
            ...
            CLIAgent::Claude | CLIAgent::Codex => AgentCapabilities {
                can_detect: true,
                can_bind_live_session: true,
                can_index_sessions: true,
                can_list_sessions: true,
    ...
    pub(crate) fn session_discovery_provider(self) -> Option<AgentSessionDiscoveryProvider> {
        self.record().session_discovery_provider
    }

    /// Serialized version of the CLIAgent name (e.g. "Claude", "Codex"). Used for the
    ...
    ///
    /// TODO(advait): Check whether Amp, Droid, and Copilot support `!` bash
...
impl CLIAgentInstallModel {
    ...
    #[cfg(test)]
    pub(crate) fn set_installed_agents_for_test(
        &mut self,
        installed: impl IntoIterator<Item = CLIAgent>,
    ) {
        let mut cache = enum_iterator::all::<CLIAgent>()
            .map(|agent| (agent, false))
            .collect::<HashMap<_, _>>();
        for agent in installed {
            cache.insert(agent, true);
        }
