...
pub(crate) fn scan_current_app_cli_agent_sessions(
    ...
    sessions
        ...
        .filter_map(|session| {
            ...
            (!deleted_session_ids.contains(&snapshot.id)
                && !deleted_session_ids.contains(&snapshot.logical_key()))
            .then_some(snapshot)
        })
        .collect()
}
...
pub(crate) fn deleted_session_ids() -> HashSet<String> {
    let Some(path) = deleted_sessions_path() else {
        return HashSet::new();
    };
...
pub(crate) fn set_session_deleted(session_id: &str, deleted: bool) -> Result<(), String> {
    let session_id = session_id.trim();
    if session_id.is_empty() {
        return Err("deleted session id is empty".to_owned());
    }

    let Some(path) = deleted_sessions_path() else {
        return Err("home directory is unavailable".to_owned());
    };
    let mut ids = deleted_session_ids();
    if deleted {
    ...
    let contents = serde_json::to_string_pretty(&ids)
        .map_err(|error| format!("failed to encode deleted sessions: {error}"))?;
    fs::write(&path, contents)
        .map_err(|error| format!("failed to write {}: {error}", path.display()))
}

pub(crate) fn session_aliases() -> HashMap<String, String> {
    let Some(path) = session_aliases_path() else {
        return HashMap::new();
    };
    let Ok(contents) = fs::read_to_string(path) else {
        return HashMap::new();
    ...
    serde_json::from_str::<HashMap<String, String>>(&contents)
        ...
        .filter_map(|(key, alias)| {
            ...
            (!key.trim().is_empty() && !alias.is_empty()).then_some((key, alias))
        })
        .collect()
}

pub(crate) fn session_alias_for_keys<'a>(
    keys: impl IntoIterator<Item = &'a str>,
) -> Option<String> {
    let aliases = session_aliases();
    keys.into_iter().find_map(|key| aliases.get(key).cloned())
}
...
pub(crate) fn set_session_alias(key: &str, alias: Option<&str>) -> Result<(), String> {
    ...
    let mut aliases = session_aliases();
    ...
    let contents = serde_json::to_string_pretty(&aliases)
        .map_err(|error| format!("failed to encode session aliases: {error}"))?;
    fs::write(&path, contents)
        .map_err(|error| format!("failed to write {}: {error}", path.display()))
}

pub(crate) fn pinned_session_ids() -> HashSet<String> {
    let Some(path) = pinned_sessions_path() else {
        return HashSet::new();
    };
    let Ok(contents) = fs::read_to_string(path) else {
        return HashSet::new();
...
pub(crate) fn set_session_pinned(session_id: &str, pinned: bool) -> Result<(), String> {
    ...
    let mut ids = pinned_session_ids();
    if pinned {
        ids.insert(session_id.to_owned());
    } else {
        ids.remove(session_id);
...
fn pinned_sessions_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home_dir| home_dir.join(".ashide/session_pins.json"))
}

fn deleted_sessions_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home_dir| home_dir.join(".ashide/session_deleted.json"))
}

fn session_aliases_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home_dir| home_dir.join(".ashide/session_aliases.json"))
}
