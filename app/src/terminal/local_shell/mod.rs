...
pub async fn execute_command(
    ...
    let local_command_executor =
        LocalCommandExecutor::new(Some(shell_path), shell_type, Default::default());

    // Build environment variables map.
    // Always include HOME to ensure the shell can expand ~ in rc files - this is critical
    // when Ashide is launched via launchd (Finder, Dock) with a minimal environment.
...
async fn capture_interactive_shell_env(
    ...
    // controlling terminal with `tcsetpgrp` [1]. If the Ashide process itself tries to read from
