...
use crate::terminal::remote_tty::event_loop::{EnvironmentRuntimePtyConfig, EventLoop};
...
impl TerminalManager {
    ...
    pub fn create_environment_runtime_model(
        resources: TerminalViewResources,
        initial_size: Vector2F,
        model_event_sender: Option<SyncSender<ModelEvent>>,
        window_id: WindowId,
        initial_input_config: Option<InputConfig>,
        environment_runtime_pty: EnvironmentRuntimePtyConfig,
        ctx: &mut AppContext,
    ) -> ModelHandle<Box<dyn crate::terminal::TerminalManager>> {
        Self::create_model_inner(
            resources,
            ...
            window_id,
            initial_input_config,
            Some(environment_runtime_pty),
            ctx,
        )
    }

    fn create_model_inner(
        resources: TerminalViewResources,
        initial_size: Vector2F,
        model_event_sender: Option<SyncSender<ModelEvent>>,
        window_id: WindowId,
        ...
        let mut model = model;
        if let Some(environment_runtime_pty) = environment_runtime_pty.as_ref() {
            model.set_force_environment_runtime_session_type(true);
            model.set_environment_runtime_transport_session_id(environment_runtime_pty.session_id);
        }
    ...
    fn create_and_start_event_loop(
        ...
        ctx.add_model(|ctx| {
            if let Some(environment_runtime_pty) = environment_runtime_pty {
                EventLoop::start_environment_runtime(
                    terminal_model,
                    message_receiver,
