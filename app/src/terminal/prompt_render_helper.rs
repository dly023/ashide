...
pub fn should_render_ps1_prompt(terminal_model: &TerminalModel, app: &AppContext) -> bool {
    ...
    // if the sharer is using PS1 and the viewer is not (using Ashide prompt in non-SLP mode).
...
pub fn should_render_prompt_on_same_line(
    ...
    // 2. The user has the same line prompt setting enabled for their Ashide prompt.
...
impl PromptRenderHelper {
    ...
    fn bootstrapping_shell_message(&self, model: &TerminalModel, sessions: &Sessions) -> String {
        ...
        if let Some(pending_session_id) = model.pending_session_id() {
            if let Some(state) = sessions.environment_runtime_setup_state(pending_session_id) {
                return match state {
                    EnvironmentRuntimeSetupState::Checking => crate::t!("terminal-starting-shell"),
                    EnvironmentRuntimeSetupState::Installing {
                    ...
                    EnvironmentRuntimeSetupState::Updating => {
                        crate::t!("terminal-bootstrapping-updating-warp-ssh-extension")
                    }
                    EnvironmentRuntimeSetupState::Initializing => {
                        crate::t!("terminal-bootstrapping-initializing")
                    }
                    EnvironmentRuntimeSetupState::Ready => crate::t!("terminal-starting-shell"),
                    EnvironmentRuntimeSetupState::Failed { .. }
                    | EnvironmentRuntimeSetupState::Unsupported { .. } => {
