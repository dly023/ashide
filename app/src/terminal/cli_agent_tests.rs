...
fn test_resume_session_id_from_command_parses_explicit_restore_commands() {
    assert_eq!(
        CLIAgent::Codex.resume_session_id_from_command("codex resume abc-123"),
        Some("abc-123".to_owned())
    );
    assert_eq!(
        CLIAgent::Codex.resume_session_id_from_command("cd /repo && codex resume abc-123"),
        Some("abc-123".to_owned())
    );
    assert_eq!(
...
#[test]
fn test_first_class_agent_adapter_capabilities_are_gated() {
    let codex = CLIAgent::Codex.adapter_capabilities();
    assert!(codex.can_list_sessions);
    assert!(codex.can_resume);
    assert!(codex.can_promote_detected_terminal);
    assert!(codex.can_target_environment_runtime);

    let claude = CLIAgent::Claude.adapter_capabilities();
    assert!(claude.can_resume);

    let agy = CLIAgent::Antigravity.adapter_capabilities();
    assert!(agy.can_resume);

    let gemini = CLIAgent::Gemini.adapter_capabilities();
    assert!(!gemini.can_resume);
    assert!(!gemini.can_promote_detected_terminal);
}
