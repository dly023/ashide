...
/// Duration between filesystem watch events for the Ashide managed paths watcher, in milliseconds.
#[cfg(not(target_family = "wasm"))]
...
pub(crate) fn ensure_warp_watch_roots_exist() {
    ...
    if let Err(err) = fs::create_dir_all(&data_dir) {
        log::warn!(
            "Failed to create Ashide data directory {}: {err}",
            data_dir.display()
        );
    }
    ...
    if config_local_dir != data_dir {
        if let Err(err) = fs::create_dir_all(&config_local_dir) {
            log::warn!(
                "Failed to create Ashide config directory {}: {err}",
                ...
                (warp_home_config_dir(), warp_home_mcp_config_file_path())
            {
                        ...
                        || !warp_home_config_dir.starts_with(&config_local_dir))
                {
                    Self::register_path(
                        ...
                        "Ashide home MCP config directory",
                    );
                }
            }
        }
...
mod tests {
    ...
    fn warp_managed_mcp_config_path_contains_only_warp_home_path() {
        match (
            ...
            (_, _, None) => {}
            _ => panic!("Expected Ashide MCP path when home directory is available"),
        }
    }

    #[test]
    fn filter_repository_update_by_prefix_keeps_only_matching_paths() {
        let skills_dir = PathBuf::from("/tmp/.ashide-local/skills");
        let other_dir = PathBuf::from("/tmp/.ashide-local/worktrees/repo");
    ...
    fn filter_repository_update_by_prefix_converts_cross_boundary_moves() {
        let skills_dir = PathBuf::from("/tmp/.ashide-local/skills");
        let skill_file = skills_dir.join("deploy").join("SKILL.md");
        let ignored_file = PathBuf::from("/tmp/.ashide-local/worktrees/repo/SKILL.md");
