    ...
    fn render(
        ...
        if !self.disabled {
            ...
            ret = ret.on_click(move |ctx, _, _| {
                ctx.dispatch_typed_action(MenuAction::Activate {
                    depth,
                    selection: SelectAction::Index {
                        row: row_index,
                        item: item_index,
                    },
                });
...
impl<A: Action + Clone> Menu<A> {
    ...
    pub fn mimic_confirm(&mut self, ctx: &mut ViewContext<Self>) {
        self.handle_action(&MenuAction::Enter, ctx);
    }
}
...
impl<A: Action + Clone> SubMenu<A> {
    ...
    fn delegate_pointer_action_to_selected_submenu(
        ...
        let event_depth = match action {
            ...
            MenuAction::Activate { depth, .. } => *depth,
        };
    ...
    fn handle_action(
        ...
        match action {
            ...
            MenuAction::Activate { depth, selection } => {
                if *depth != self.depth {
                    return;
                }
                self.select(*selection, position_namespace, ctx);
                self.activate_selected_item(dispatch_item_actions, position_namespace, ctx);
            }
            MenuAction::OpenSubmenu => {
    ...
    fn activate_selected_item(
        ...
        let Some(action) = self.selected_action_for_enter(position_namespace, ctx) else {
            return;
        };
