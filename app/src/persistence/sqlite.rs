...
use persistence::model::ENVIRONMENT_RUNTIME_PLACEHOLDER_PANE_KIND;
use uuid::Uuid;
...
use super::model::{
    self, CurrentUserInformation, NewApp, NewCommand, NewFolder, NewNotebook, NewServerExperiment,
    NewTab, NewTeam, NewWindow, NewWorkspace, NewWorkspaceTeam, ObjectMetadata, ObjectPermissions,
    Project, Tab, Window, AI_DOCUMENT_PANE_KIND, AI_FACT_PANE_KIND, CODE_PANE_KIND,
    ENV_VAR_COLLECTION_PANE_KIND, EXECUTION_PROFILE_EDITOR_PANE_KIND, NOTEBOOK_PANE_KIND,
...
use crate::app_state::{
    ...
    EnvVarCollectionPaneSnapshot, EnvironmentSnapshot, LeftPanelSnapshot, PaneSessionBinding,
...
use crate::object_store::ids::{
    ClientId, HashableId, ObjectStoreId, StableObjectId, ToStableObjectId,
};
use crate::object_store::model::actions::{ObjectAction, ObjectActionSubtype};
use crate::object_store::model::generic_string_model::{GenericStringObjectId, StoredStringObject};
...
use crate::{
    app_state::{
        ...
        TabSnapshot, TerminalPaneSnapshot, WindowSnapshot, WorkspaceSessionSnapshot,
    },
    workspaces::user_profiles::UserProfileWithUID,
};
use crate::{
    notebooks::NotebookObjectModel,
    object_store::{NumInFlightRequests, Revision, StoredObjectMetadata},
};
use crate::{
    object_store::{StoredObjectPermissions, StoredObjectStatuses, StoredObjectSyncStatus},
...
use crate::persistence::stored_objects::{upsert_stored_object, StoredObjectId};

const ASHIDE_SQLITE_FILE_NAME: &str = "ashide.sqlite";

/// 删除本地 stored object 时使用的回调。入参是待删除对象的 id。
/// 传入的 conn 已经启动 transaction。
type DeleteStoredObjectFn =
...
/// Returns a writable connection to the sqlite database and runs pending migrations.
pub fn establish_rw_connection() -> Result<SqliteConnection> {
    init_db()
}
...
unsafe fn init_logging() {
    ...
    extern "C-unwind" fn log_callback(_data: *mut c_void, err_code: c_int, msg: *const c_char) {
        ...
        let level = match (primary_error_code, err_code) {
            ...
            // This is mostly outside of Ashide's control (e.g. the user or some system program is
...
pub fn database_file_path() -> PathBuf {
    warp_core::paths::state_dir().join(ASHIDE_SQLITE_FILE_NAME)
}
    ...
    conn.transaction::<(), Error, _>(|conn| {
        ...
        for (idx, window) in app_state.windows.iter().enumerate() {
            ...
            let new_window = NewWindow {
                ...
                local_drive_index_width: window.local_drive_index_width,
                    ...
                    match pane_node {
                        ...
                        PaneNodeSnapshot::Leaf(pane) => {
                            let session_binding_json =
                                pane.session_binding.as_ref().map(|binding| {
                                    serde_json::to_string(binding).expect(
                                        "PaneSessionBinding serialization must be infallible",
                                    )
                                });
                            diesel::insert_into(
...
fn save_pane_state(
    ...
    match &snapshot.contents {
        LeafContents::Terminal(terminal_snapshot) => {
            ...
            let terminal = model::NewTerminalPane {
                ...
                active_profile_id: terminal_snapshot
                    .active_profile_id
                    .as_ref()
                    .and_then(|object_store_id| serde_json::to_string(object_store_id).ok()),
                conversation_ids,
        ...
        LeafContents::EnvironmentRuntimePlaceholder => {
            // Stateless; the pane kind row is sufficient.
        }
...
fn upsert_mcp_server_installation(
    ...
    let new_installation = model::NewMCPServerInstallation {
        ...
        // TODO(pei): Change this to be the timestamp of the stored object.
...
fn save_workspace(conn: &mut SqliteConnection, workspace: WorkspaceMetadata) -> Result<()> {
    ...
    for team in workspace.teams {
        ...
        let new_team = NewTeam {
            ...
            workspace_policy_json: serde_json::to_string(&team.workspace_policy).ok(),
        };
...
fn increment_retry_count(
    conn: &mut SqliteConnection,
    stable_id_string: String,
) -> Result<(), Error> {
    use schema::object_metadata::dsl::*;
    conn.transaction::<(), Error, _>(|conn| {
        diesel::update(object_metadata.filter(server_id.eq(Some(stable_id_string))))
...
/// 删除 `object_store_id` 标识的本地 stored object 的 helper。如果找到有效 object metadata row,
/// 就调用 `delete_object_fn` 删除实际对象。
fn delete_stored_object(
    conn: &mut SqliteConnection,
    object_store_id: ObjectStoreId,
    object_id_type: ObjectIdType,
    delete_object_fn: DeleteStoredObjectFn,
) -> Result<(), Error> {
    use schema::object_metadata::dsl::*;

    // Filter to find metadata row.
    // The diesel types for `filter`s are dependent on the columns being filtered
    // so while the `hashed_object_store_id` will only match one of `client_id` and `server_id`,
    // we filter on both here for ergonomics.
    let hashed_object_store_id = object_store_id.sqlite_uid_hash(object_id_type);
    ...
    diesel::delete(
        schema::object_actions::dsl::object_actions
            .filter(schema::object_actions::hashed_object_id.eq(hashed_object_store_id)),
    )
    .execute(conn)?;
...
fn update_object_metadata(
    ...
    let folder_id_str = metadata
        .folder_id
        .map(|folder_object_store_id| folder_object_store_id.sqlite_uid_hash(ObjectIdType::Folder));
...
fn upsert_workflows(
    conn: &mut SqliteConnection,
    object_store_workflows: Vec<WorkflowObject>,
) -> Result<(), Error> {
    use schema::workflows::dsl::*;
    conn.transaction::<(), Error, _>(|conn| {
        // todo: wrap in an arc to avoid unnecessary cloning.
        for workflow in object_store_workflows {
...
fn upsert_notebooks(
    conn: &mut SqliteConnection,
    local_notebooks: Vec<NotebookObject>,
) -> Result<(), Error> {
...
fn read_node(conn: &mut SqliteConnection, node: model::PaneNode) -> Result<PaneNodeSnapshot> {
    match node.is_leaf {
        true => {
            ...
            let contents = match pane.kind.as_ref() {
                ...
                NOTEBOOK_PANE_KIND => {
                    ...
                    let notebook_id = notebook_pane.notebook_id.and_then(|id| {
                        ClientId::from_hash(&id)
                            .map(ObjectStoreId::ClientId)
                            .or_else(|| {
                                NotebookId::from_hash(&id)
                                    .map(|id| ObjectStoreId::StableId(id.into()))
                            })
                    });
                    ...
                    // to an uneditable notebook. In that case, bias towards object-store notebooks. If
                    // both are null, it's more likely that the pane was a new, empty cloud
                    // notebook than an unreadable current app filesystem file.
                    LeafContents::Notebook(match local_path {
                        Some(path) => {
                            NotebookPaneSnapshot::CurrentAppFileNotebook { path: Some(path) }
                        }
                        None => NotebookPaneSnapshot::NotebookObject {
                            notebook_id,
                            settings: LocalDriveObjectSettings::default(),
                        },
                    })
                }
                WORKFLOW_PANE_KIND => {
                ...
                ENVIRONMENT_RUNTIME_PLACEHOLDER_PANE_KIND => {
                    LeafContents::EnvironmentRuntimePlaceholder
                }
                WELCOME_PANE_KIND => {
            ...
            let container = schema::pane_container_identities::dsl::pane_container_identities
                .find(node.id)
                .first::<model::PaneContainerIdentity>(conn)?;
            let session_binding = container
                .session_binding_json
                .map(|binding| serde_json::from_str::<PaneSessionBinding>(&binding))
                .transpose()
                .with_context(|| format!("pane node {} 的 session binding JSON 无效", node.id))?;
            anyhow::ensure!(
                !container.uuid.is_empty(),
...
fn read_sqlite_data(
    ...
    let mut stored_objects: Vec<Box<dyn StoredObject>> = Vec::new();
            ...
            .filter_map(|workflow| {
                metadata_by_id
                    ...
                    .and_then(|metadata| {
                        ...
                        let stored_object_permissions =
                            to_stored_object_permissions(permissions, current_user_id)?;
                        workflow_content
                            .zip(workflow_id)
                            .map(|(content, workflow_id)| {
                                let boxed: Box<dyn StoredObject> = Box::new(WorkflowObject::new(
                                    ...
                                    to_stored_object_metadata(metadata),
                                    stored_object_permissions,
                                ));
                                boxed
                            })
                    })
                ...
                metadata_by_id
                    ...
                    .and_then(|metadata| {
                        ...
                        let json_object_type: JsonObjectType = metadata
                            ...
                            .try_into()
                            .ok()?;
                        object_id.and_then(|stable_id| match json_object_type {
    ...
    let teams: Vec<TeamMetadata> = db_teams
        ...
        .map(|team| {
            ...
            TeamMetadata::from_local_cache(
                StableObjectId::from_string_lossy(team.server_uid),
                team.name,
                team_settings,
                workspace_policy,
                members,
                    ...
                    .filter_map(|workspace_team| {
                        if workspace_team.workspace_server_uid == workspace.server_uid {
                            teams.iter().find(|team| {
                                team.uid
                                    == StableObjectId::from_string_lossy(
                                        &workspace_team.team_server_uid,
                                    )
                            })
                        } else {
                            None
    ...
    let time_of_next_force_object_refresh: Option<DateTime<Utc>> =
        schema::stored_objects_refreshes::dsl::stored_objects_refreshes
...
fn id_from_metadata<K: HashableId + ToStableObjectId>(
    metadata: &ObjectMetadata,
) -> Option<ObjectStoreId> {
    match (&metadata.server_id, &metadata.client_id) {
        (Some(stable_id), _) => {
            K::from_hash(stable_id).map(|id| ObjectStoreId::StableId(id.to_stable_object_id()))
        }
        (None, Some(client_id)) => ClientId::from_hash(client_id).map(ObjectStoreId::ClientId),
        _ => None,
    }
}

fn to_stored_object_metadata(metadata: &ObjectMetadata) -> StoredObjectMetadata {
    StoredObjectMetadata {
        ...
        folder_id: metadata.folder_id.as_ref().and_then(|folder_id_str| {
            // First, attempt to convert the string into a stable object id.
            let as_stable_id =
                FolderId::from_hash(folder_id_str).map(|id| ObjectStoreId::StableId(id.into()));

            // If the string cannot be converted to stable object id, it may be a client id.
            if as_stable_id.is_none() {
                ClientId::from_hash(folder_id_str).map(ObjectStoreId::ClientId)
            } else {
                as_stable_id
            }
        }),
...
fn to_stored_object_permissions(
...
impl From<StartedCommandMetadata> for model::NewCommand {
    fn from(metadata: StartedCommandMetadata) -> Self {
        Self {
            ...
            object_store_workflow_id: metadata
                .object_store_workflow_id
...
fn record_time_of_next_refresh(
    ...
    use schema::stored_objects_refreshes::dsl::*;
    let refresh = NewObjectStoreRefresh {
        time_of_next_refresh: timestamp.naive_utc(),
    };
    conn.transaction::<(), Error, _>(|conn| {
        diesel::delete(stored_objects_refreshes).execute(conn)?;
        diesel::insert_into(stored_objects_refreshes)
            .values(refresh)
            .execute(conn)?;
        Ok(())
    })
...
fn delete_objects(
    conn: &mut SqliteConnection,
    ids: Vec<(ObjectStoreId, ObjectIdType)>,
) -> Result<(), Error> {
    conn.transaction::<(), Error, _>(|conn| {
        for (object_store_id, object_id_type) in ids {
            match object_id_type {
                ObjectIdType::Notebook => delete_stored_object(
                    conn,
                    object_store_id,
                    object_id_type,
                ...
                ObjectIdType::Workflow => delete_stored_object(
                    conn,
                    object_store_id,
                    object_id_type,
                ...
                ObjectIdType::Folder => delete_stored_object(
                    conn,
                    object_store_id,
                    object_id_type,
                ...
                ObjectIdType::GenericStringObject => delete_stored_object(
                    conn,
                    object_store_id,
                    object_id_type,
                    Box::new(|conn, gso_id| {
