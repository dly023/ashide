...
#[derive(Debug, thiserror::Error)]
pub(crate) enum InsertConversationError {
    #[error("conversation already exists: {0}")]
    AlreadyExists(String),
    #[error("Failed to serialize conversation data: {0:?}")]
    Serialization(#[from] serde_json::Error),
    #[error("Failed to insert conversation to sqlite: {0:?}")]
    DB(#[from] diesel::result::Error),
}

pub(crate) fn insert_agent_conversation<'a>(
    conn: &mut SqliteConnection,
    conversation_id_param: &str,
    tasks: impl IntoIterator<Item = &'a api::Task>,
    conversation_data_param: AgentConversationData,
) -> Result<(), InsertConversationError> {
    use diesel::ExpressionMethods;
    use diesel::QueryDsl;
    use schema::agent_conversations::dsl::*;
    ...
    conn.transaction::<_, InsertConversationError, _>(|conn| {
        let exists = agent_conversations::table()
            ...
            .is_some();
        if exists {
            return Err(InsertConversationError::AlreadyExists(
                conversation_id_param.to_owned(),
            ));
        }
...
pub(crate) fn read_agent_conversations(
    conn: &mut SqliteConnection,
