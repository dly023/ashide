...
use super::super::runtime_paths;

/// Path to the daemon's Unix domain socket.
pub(super) fn socket_path(identity_key: &str) -> PathBuf {
    let dir = runtime_paths::remote_server_daemon_dir(identity_key);
...
pub fn run(identity_key: &str) -> anyhow::Result<()> {
    ...
    if !daemon_running {
        ...
        cmd.arg("environment-runtime-daemon")
            .arg("--identity-key")
            .arg(identity_key)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
...
/// length-prefixed framing.  The framing is handled at the endpoints (Ashide
/// client and daemon).
///
