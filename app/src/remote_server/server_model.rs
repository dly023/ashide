...
use super::proto::{
    ...
    ServerMessage, SessionBootstrapped, WriteFile, WriteFileResponse, WriteFileSuccess, WritePty,
};
...
#[cfg(feature = "local_fs")]
use super::proto::{
    create_directory_response, get_cli_agent_session_user_state_response, list_directory_response,
    mutate_cli_agent_session_response, mutate_cli_agent_session_user_state_response,
    read_cli_agent_session_response, read_file_chunk_response, rename_file_response,
    resolve_conflict_response, resolve_path_response, save_buffer_response,
    scan_cli_agent_sessions_response, write_file_chunk_response, BufferEdit, BufferUpdatedPush,
    CliAgentSessionMutation, CliAgentSessionRecord, CliAgentSessionUserState,
    CliAgentSessionUserStateMutation, CloseBuffer, CreateDirectory, CreateDirectoryResponse,
    CreateDirectorySuccess, DirEntry, FileSystemEntryKind, GetCliAgentSessionUserState,
    GetCliAgentSessionUserStateResponse, GetCliAgentSessionUserStateSuccess, ListDirectory,
    ListDirectoryResponse, ListDirectorySuccess, MutateCliAgentSession,
    MutateCliAgentSessionResponse, MutateCliAgentSessionSuccess, MutateCliAgentSessionUserState,
    MutateCliAgentSessionUserStateResponse, MutateCliAgentSessionUserStateSuccess, OpenBuffer,
    OpenBufferResponse, ReadCliAgentSession, ReadCliAgentSessionResponse,
    ReadCliAgentSessionSuccess, ReadFileChunk, ReadFileChunkResponse, ReadFileChunkSuccess,
    RenameFile, RenameFileResponse, RenameFileSuccess, ResolveConflict, ResolveConflictResponse,
    ResolveConflictSuccess, ResolvePath, ResolvePathResponse, ResolvePathSuccess, SaveBuffer,
    SaveBufferResponse, SaveBufferSuccess, ScanCliAgentSessions, ScanCliAgentSessionsResponse,
    ScanCliAgentSessionsSuccess, TextEdit, WriteFileChunk, WriteFileChunkResponse,
...
pub struct ServerModel {
    ...
    /// per SSH session / Ashide tab connecting to this host.  Each entry maps
    ...
    /// Persistent native remote PTYs owned by this daemon runtime.
...
impl ServerModel {
    ...
    pub fn deregister_connection(&mut self, conn_id: ConnectionId, ctx: &mut ModelContext<Self>) {
        ...
        #[cfg(unix)]
        {
            let ptys_before = self.ptys.len();
            self.ptys.retain(|_, pty| pty.conn_id != conn_id);
            let removed_ptys = ptys_before.saturating_sub(self.ptys.len());
            if removed_ptys > 0 {
                log::info!(
                    "Daemon: closed {removed_ptys} PTY session(s) for disconnected connection {conn_id}"
                );
            }
        }
    ...
    pub fn handle_message(
        ...
        let outcome = match msg.message {
            ...
            #[cfg(feature = "local_fs")]
            Some(client_message::Message::RenameFile(msg)) => self.handle_rename_file(msg),
            #[cfg(feature = "local_fs")]
            ...
            Some(
                ...
                | client_message::Message::RenameFile(_)
    ...
    fn spawn_remote_pty(
        ...
        let Some(conn_tx) = self.connection_senders.get(&conn_id).cloned() else {
    ...
    fn handle_write_pty(&mut self, req: WritePty) {
        #[cfg(unix)]
        if let Some(pty) = self.ptys.get_mut(&req.pty_id) {
            if let Err(e) = pty.master.write_all(&req.bytes) {
                log::warn!("Failed to write to remote PTY {}: {e}", req.pty_id);
            }
        } else {
            log::warn!("Ignoring WritePty for unknown PTY {}", req.pty_id);
        }
    ...
    fn handle_resize_pty(&mut self, req: ResizePty) {
        #[cfg(unix)]
        if let Some(pty) = self.ptys.get(&req.pty_id) {
            let size = winsize {
            ...
            log::warn!("Ignoring ResizePty for unknown PTY {}", req.pty_id);
        }
    ...
    fn handle_close_pty(&mut self, req: ClosePty) {
        if self.ptys.remove(&req.pty_id).is_none() {
            log::debug!("Ignoring ClosePty for unknown PTY {}", req.pty_id);
        }
    }
    ...
    #[cfg(feature = "local_fs")]
    fn handle_rename_file(&self, msg: RenameFile) -> HandlerOutcome {
        let from = expand_user_path(&msg.from_path);
        let to = expand_user_path(&msg.to_path);
        let result = match std::fs::rename(&from, &to) {
            Ok(()) => rename_file_response::Result::Success(RenameFileSuccess {}),
            Err(err) => rename_file_response::Result::Error(FileOperationError {
                message: format!(
        ...
        HandlerOutcome::Sync(server_message::Message::RenameFileResponse(
            RenameFileResponse {
                result: Some(result),
            },
        ))
    ...
    fn handle_scan_cli_agent_sessions(&self, msg: ScanCliAgentSessions) -> HandlerOutcome {
        let records = super::cli_agent_sessions::scan_sessions(msg.limit as usize)
            .into_iter()
        ...
        HandlerOutcome::Sync(server_message::Message::ScanCliAgentSessionsResponse(
            ScanCliAgentSessionsResponse {
                result: Some(scan_cli_agent_sessions_response::Result::Success(
                    ScanCliAgentSessionsSuccess { records },
                )),
            },
        ))
    }
    ...
    fn handle_read_cli_agent_session(&self, msg: ReadCliAgentSession) -> HandlerOutcome {
        let result = match super::cli_agent_sessions::read_session(&msg.source) {
            Ok(session) => {
    ...
    fn handle_mutate_cli_agent_session(&self, msg: MutateCliAgentSession) -> HandlerOutcome {
        ...
        let result = match super::cli_agent_sessions::mutate_session(&msg.source, mutation) {
            Ok(()) => {
