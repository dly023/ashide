...
//! rsync/SSH-stdin upload primitives those two entry points build on.
//!
...
static DEV_REMOTE_INSTALL_SINGLEFLIGHT: LazyLock<futures::lock::Mutex<()>> =
...
pub(crate) async fn dev_install_local_binary(socket_path: &Path, ssh_target: &str) -> Result<()> {
    // RemoteServerManager 按 session 调度安装；同一进程中的 terminal runtime、
    // Environment runtime 或多个 Workspace 可能同时发现 helper 过期。整个 dev
    // install 必须 single-flight，而不能只依赖 Cargo 的 target lock：等待者进入
    // gate 后会从 platform/freshness 开始重检，从而复用首个任务的本地产物与
    // 远端 stamp，避免重复 build、重复上传和 daemon replacement。
    let _install_guard = DEV_REMOTE_INSTALL_SINGLEFLIGHT.lock().await;
    ...
    if uploaded_with_rsync {
        ...
        log::warn!(
            "dev remote-server: rsync 不可用，改用 SSH stdin 流式上传到临时文件 {remote_temp_binary}"
        );
        // dev 产物有数百 MB，使用 DEV_UPLOAD_TIMEOUT。
...
mod tests {
    ...
    fn dev_remote_install_is_process_single_flight() {
        ...
        let install = source
            .split("pub(crate) async fn dev_install_local_binary")
            .nth(1)
            .and_then(|source| source.split("#[cfg(test)]").next())
            .expect("dev install function must exist");
        let gate = install
            .find("DEV_REMOTE_INSTALL_SINGLEFLIGHT.lock().await")
            .expect("dev install must acquire the process-wide single-flight gate");
        let platform = install
            .find("detect_remote_platform")
            .expect("dev install must detect the remote platform");
        let build = install
            .find("cross_compile_remote_server")
            .expect("dev install must build or reuse the local helper");
        let remote_freshness = install
            .find("skip_dev_upload_if_remote_binary_matches")
            .expect("dev install must recheck the remote stamp before upload");

        assert!(gate < platform);
        assert!(platform < build);
        assert!(build < remote_freshness);
    }
    ...
    fn release_workflow_publishes_versioned_remote_helper_assets() {
        ...
        for (line_index, line) in workflow_lines.iter().enumerate() {
            if !line.contains("gh release ") {
                continue;
            }
            ...
            while command.trim_end().ends_with('\\') {
                ...
                command.push(' ');
                command.push_str(workflow_lines[continuation_index].trim());
            }
            release_commands.push(command);
        }
        assert!(!release_commands.is_empty());
        for command in &release_commands {
            assert!(
                command.contains("--repo \"$GITHUB_REPOSITORY\""),
                "release command must pin the target repository: {command}"
            );
        }
        for subcommand in ["view", "edit", "create", "upload"] {
            assert!(
                release_commands
                    .iter()
                    .any(|command| command.contains(&format!("gh release {subcommand}"))),
                "release workflow must exercise gh release {subcommand}"
            );
        }
        ...
        assert!(artifact_script.contains("build_linux_helper x86_64-unknown-linux-musl x86_64"));
        assert!(artifact_script.contains("build_linux_helper aarch64-unknown-linux-musl aarch64"));
        assert!(artifact_script.contains("TARGET_ARCH=\"all\""));
        assert!(artifact_script.contains("--arch"));
        assert!(artifact_script.contains("all|x86_64|aarch64"));
        assert!(
            artifact_script.contains("$TARGET_ARCH\" == \"all\" || \"$TARGET_ARCH\" == \"x86_64")
        );
        assert!(
            artifact_script.contains("$TARGET_ARCH\" == \"all\" || \"$TARGET_ARCH\" == \"aarch64")
        );
        assert!(!artifact_script.contains("Skipping x86_64 Linux helper build"));
    }

    #[test]
    fn helper_fallback_uses_ssh_stdin_not_scp() {
        ...
        assert!(ssh_transport.contains("futures_lite::io::copy"));
        assert!(ssh_transport.contains("cat > {}"));
        assert!(!ssh_transport.contains("Command::new(\"scp\")"));
    }

    #[test]
    fn helper_upload_promotes_only_after_stream_success() {
        let installer = include_str!("dev_remote_install.rs")
            .split_once("#[cfg(test)]")
            .expect("installer test boundary must exist")
            .0;

        for function in ["release_install_local_binary", "dev_install_local_binary"] {
            let body = installer
                .split_once(&format!("async fn {function}"))
                .and_then(|(_, after_signature)| after_signature.split_once("\n}\n"))
                .map(|(body, _)| body)
            ...
            let upload_completion = fallback
                .find(".await?;")
                .expect("SSH stdin upload must finish before promotion");
            let promote = fallback
                .find("promote_uploaded_binary(")
                .expect("successful upload must be atomically promoted");

            assert!(
                upload_completion < promote,
    ...
    #[test]
    fn release_workflow_separates_source_ref_from_product_version() {
        let workflow = include_str!("../../../.github/workflows/release.yml");
        let product_tag_expression = "${{ github.event_name == 'workflow_dispatch' && github.event.inputs.tag || github.ref_name }}";

        assert!(workflow.contains("SOURCE_REF: ${{ github.ref }}"));
        assert_eq!(workflow.matches(product_tag_expression).count(), 2);
        assert!(!workflow
            .contains("github.ref_type == 'tag' && github.ref_name || github.event.inputs.tag"));
        assert!(workflow.contains("validate-release-identity:"));
        assert!(workflow.contains("Product release tag must match vMAJOR.MINOR.PATCH"));
        assert_eq!(
            workflow.matches("needs: validate-release-identity").count(),
            4
        );
        let mut current_job = None;
        let mut identity_gated_producers = Vec::new();
        for line in workflow.lines() {
            if let Some(candidate) = line.strip_prefix("  ") {
                if !candidate.starts_with(' ') && candidate.ends_with(':') {
                    current_job = Some(candidate.trim_end_matches(':'));
                }
            }
            if line == "    needs: validate-release-identity" {
                identity_gated_producers
                    .push(current_job.expect("identity gate must belong to a job"));
            }
        }
        identity_gated_producers.sort_unstable();
        assert_eq!(
            identity_gated_producers,
            [
                "build-linux-cli",
    ...
    fn release_builds_are_reproducibly_locked() {
        ...
        assert!(macos_bundle.contains("cargo fetch --locked"));
        assert!(macos_bundle.contains("CARGO_BUNDLE_SKIP_BUILD=1"));
        assert!(macos_bundle.contains("CARGO_NET_OFFLINE=true cargo bundle"));
    ...
    #[test]
    fn macos_dmg_is_repository_owned_state_driven_and_verified() {
        let workflow = include_str!("../../../.github/workflows/release.yml");
        let macos_bundle = include_str!("../../../script/macos/bundle");
        let dmg_builder = include_str!("../../../script/macos/create_ashide_dmg");
        let finder_layout = include_str!("../../../script/macos/configure_ashide_dmg.applescript");

        assert!(macos_bundle.contains("script/macos/create_ashide_dmg"));
        assert!(!macos_bundle.contains("create-dmg"));
        assert!(!workflow.contains("brew install create-dmg"));

        assert!(dmg_builder.contains("diskutil image create blank"));
        assert!(dmg_builder.contains("local args=(image attach --plist)"));
        assert!(dmg_builder.contains("diskutil image resize --size min"));
        assert!(dmg_builder.contains("diskutil image create from --format UDZO"));
        assert!(dmg_builder.contains("args+=(--readOnly)"));
        assert!(dmg_builder.contains("rm -f \"$OUTPUT_DMG\""));
        assert!(dmg_builder.contains("Created and verified"));
        for deprecated_operation in [
            "hdiutil create",
            "hdiutil attach",
            "hdiutil detach",
            "hdiutil resize",
            ...
            assert!(!dmg_builder.contains(deprecated_operation));
        }

        assert!(finder_layout.contains("set expectedBounds to {10, 60, 771, 560}"));
        ...
        assert!(finder_layout.contains("closed-window generation boundary"));
        assert!(finder_layout.contains("if visible of container window then return"));
        assert!(finder_layout.contains("my verifyPersistedLayout"));
        assert!(!finder_layout
            .lines()
            .any(|line| { matches!(line.trim(), "delay 2" | "delay 4" | "delay 5" | "delay 10") }));
        assert!(!dmg_builder
            .lines()
            .any(|line| matches!(line.trim(), "sleep 2" | "sleep 4" | "sleep 5" | "sleep 10")));
    }

    #[test]
    fn third_party_license_generation_is_version_pinned_and_fail_closed() {
        let workflow = include_str!("../../../.github/workflows/release.yml");
        let installer = include_str!("../../../script/install_cargo_release_deps");
        let unix_resources = include_str!("../../../script/prepare_bundled_resources");
        let windows_resources =
            include_str!("../../../script/windows/prepare_bundled_resources.ps1");
        let install_command = "cargo install --locked --force --features cli cargo-about@0.9.1";

        assert!(installer.contains(install_command));
        assert!(workflow.contains(install_command));
        assert!(unix_resources.contains("cargo-about 0.9.1"));
        assert!(windows_resources.contains("cargo-about 0.9.1"));

        for resource_script in [unix_resources, windows_resources] {
            assert!(resource_script.contains("cargo about generate"));
            assert!(resource_script.contains("--workspace"));
            assert!(resource_script.contains("--frozen"));
            assert!(resource_script.contains("--fail"));
        }
    }

    #[test]
    fn ci_workflow_runs_local_remote_parity_gate() {
        let workflow = include_str!("../../../.github/workflows/ci.yml");

        assert!(workflow.contains("pull_request:"));
        assert!(workflow.contains("push:"));
        assert!(workflow.contains("cargo fmt --all -- --check"));
        assert!(workflow.contains("./script/check_local_remote_parity"));
        assert!(workflow.contains("cargo check --locked"));
    }

    #[test]
    fn tracker_capability_matrix_references_are_complete() {
        let tracker = include_str!("../../../docs/design/local-remote-parity-tracker.yaml");
        let matrix = include_str!("../../../docs/design/local-remote-capability-matrix.csv");
        let matrix_capabilities = matrix
            .lines()
            .skip(1)
            .filter_map(|line| line.split(',').next())
            .collect::<Vec<_>>();

        let mut unique_matrix_capabilities = matrix_capabilities.clone();
        unique_matrix_capabilities.sort_unstable();
        unique_matrix_capabilities.dedup();
        assert_eq!(
            unique_matrix_capabilities.len(),
            matrix_capabilities.len(),
            "capability matrix contains duplicate capability rows"
        );

        let tracker_capabilities = tracker.lines().filter_map(|line| {
            line.strip_prefix("    capability_matrix: \"")
                .and_then(|value| value.strip_suffix('\"'))
        });
        for capability in tracker_capabilities {
            assert!(
                unique_matrix_capabilities.contains(&capability),
                "tracker capability_matrix reference has no matrix row: {capability}"
            );
        }
    }
