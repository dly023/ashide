...
/// These are shipped with Ashide and user-editable (Ashide does not overwrite modifications).
#[cfg_attr(target_family = "wasm", expect(dead_code))]
pub fn default_tab_configs_dir() -> PathBuf {
    base_dir().join("default_tab_configs")
}

/// Returns whether the path points to a tab config TOML file under one of Ashide's
...
/// Ensures the default Ashide worktree tab config exists, creating it
