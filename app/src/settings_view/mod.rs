...
/// Wider sidebar used for the settings-file footer. Sized to
...
pub enum SettingsViewEvent {
    Pane(PaneEvent),
    StartResize,
    // Ashide 去中心化分支:`CheckForUpdate` / `LocalDrive` 变体随 Account
...
impl Display for SettingsSection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s: String = match self {
            ...
            SettingsSection::LocalDrive => crate::t!("settings-section-local-drive"),
...
impl SettingsSection {
    const AGENT_SUBPAGES: &'static [Self] = &[
        Self::WarpAgent,
        Self::AgentProfiles,
        Self::AgentProviders,
    ...
    pub fn parent_page_section(&self) -> Self {
        self.route().backing_page_section()
    }
...
enum SettingsRoute {
    ...
    AgentsSubpage {
        backing_page: SettingsSection,
        ai_subpage: Option<AISubpage>,
    },
    CodeSubpage {
        backing_page: SettingsSection,
    },
...
impl SettingsRoute {
    fn from_section(section: SettingsSection) -> Self {
        match section {
            SettingsSection::AgentMCPServers => Self::AgentsSubpage {
                backing_page: SettingsSection::MCPServers,
                ai_subpage: None,
            },
            section if SettingsSection::AGENT_SUBPAGES.contains(&section) => Self::AgentsSubpage {
                backing_page: SettingsSection::AI,
                ai_subpage: AISubpage::from_section(section),
            },
            SettingsSection::EditorAndCodeReview => Self::CodeSubpage {
                backing_page: SettingsSection::Code,
            },
    ...
    fn is_subpage(self) -> bool {
        matches!(self, Self::AgentsSubpage { .. } | Self::CodeSubpage { .. })
    }

    fn is_agents_subpage(self) -> bool {
        matches!(self, Self::AgentsSubpage { .. })
    }

    fn backing_page_section(self) -> SettingsSection {
        match self {
            Self::Page(section) => section,
            Self::AgentsSubpage { backing_page, .. } | Self::CodeSubpage { backing_page, .. } => {
                backing_page
            }
        }
    }

    fn ai_subpage(self) -> Option<AISubpage> {
        match self {
            Self::AgentsSubpage { ai_subpage, .. } => ai_subpage,
            Self::Page(_) | Self::CodeSubpage { .. } => None,
        }
    }
}
...
impl FromStr for SettingsSection {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "About" => Ok(Self::About),
            "AI" => Ok(Self::AI),
            "Ashide Agent" => Ok(Self::WarpAgent),
            ...
            "LocalDrive" | "Ashide Drive" => Ok(Self::LocalDrive),
...
pub mod flags {
    ...
    pub const ENABLE_LOCAL_DRIVE: &str = "EnableLocalDrive";
...
pub enum SettingsAction {
    ...
    LocalDrive(local_drive_page::LocalDriveSettingsPageAction),
...
macro_rules! update_page {
    ($handle:expr, $update:expr, $ctx:expr) => {
        match $handle {
            ...
            SettingsPageViewHandle::LocalDrive(handle) => $ctx.update_view(handle, $update),
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Ashide Drive page
        let local_drive_page_handle =
            ctx.add_typed_action_view(local_drive_page::LocalDriveSettingsPageView::new);
        ...
        let mut settings_pages = vec![
            ...
            SettingsPage::new(local_drive_page_handle),
        ];
            ...
            EditorEvent::Edited(_) => {
                ...
                if is_search_active {
                    ...
                    for &subpage_section in SettingsSection::ai_subpages() {
                        if let Some(subpage) = subpage_section.route().ai_subpage() {
                ...
                if is_search_active {
                    ...
                    if let Some(subpage) = current.route().ai_subpage() {
                        self.ai_page_handle.update(ctx, |view, ctx| {
                            view.set_active_subpage(Some(subpage), ctx);
                        });
                    }
                        ...
                        if let SettingsNavItem::Umbrella(umbrella) = item {
                            let has_match = umbrella.subpages.iter().any(|subpage_section| {
                                self.subpage_filter
                                    ...
                                    .map(|md| md.is_truthy())
                                    .unwrap_or_else(|| {
                                        // Subpages with their own backing page fall back to pages_filter.
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        if section.is_subpage() {
            if let Some(subpage) = section.route().ai_subpage() {
                self.ai_page_handle.update(ctx, |view, ctx| {
                    view.set_active_subpage(Some(subpage), ctx);
                });
            }
        ...
        {
            crate::crash_reporting::set_tag("ashide.settings_page", section.to_string());
        }
    ...
    fn should_render_page(&self, settings_page: &SettingsPage, app: &AppContext) -> bool {
        match &settings_page.view_handle {
            ...
            SettingsPageViewHandle::LocalDrive(v) => v.as_ref(app).should_render(app),
            // Issue #72: 全局 HTTP 代理设置页。
...
impl TypedActionView for SettingsView {
    ...
    fn handle_action(&mut self, action: &SettingsAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SettingsAction::LocalDrive(local_drive_action) => {
                if let Some(local_drive_page) = self.settings_page(SettingsSection::LocalDrive) {
                    if let SettingsPageViewHandle::LocalDrive(view) = &local_drive_page.view_handle
                    {
                        view.update(ctx, |view, ctx| {
                            view.handle_action(local_drive_action, ctx);
                        })
                    }
                }
            }
