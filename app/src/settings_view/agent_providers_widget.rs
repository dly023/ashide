...
use warpui::elements::{
    ...
    MouseStateHandle, ParentElement, Radius, SavePosition, Text, Wrap,
};
...
pub(super) fn clear_expanded_models_for_provider(provider_id: &str) {
    ...
    EXPANDED_PROVIDERS.with(|m| {
        m.borrow_mut().remove(provider_id);
    });
}

std::thread_local! {
    /// {provider_id => 是否展开完整编辑表单}。只记录用户显式 toggle 过的卡片;
    /// 未记录的用调用方给的 default(新建/未配置 → 展开;已配置 → 折叠)。
    /// 与 EXPANDED_MODELS 一样 process-local、不持久化。
    static EXPANDED_PROVIDERS: RefCell<HashMap<String, bool>> = RefCell::new(HashMap::new());
}

/// provider 卡片是否展开:用户显式 toggle 过则用该值,否则回落到 `default_expanded`
/// (默认规则:未配置 key 且无自定义 endpoint 的"空白卡"展开,已配置的折叠成摘要)。
pub(super) fn is_provider_expanded(provider_id: &str, default_expanded: bool) -> bool {
    EXPANDED_PROVIDERS.with(|m| {
        m.borrow()
            .get(provider_id)
            .copied()
...
/// 显式设置某 provider 卡片的展开态(新建时设 true,使其默认打开供填写)。
pub(super) fn set_provider_expanded(provider_id: &str, expanded: bool) {
    EXPANDED_PROVIDERS.with(|m| {
        m.borrow_mut().insert(provider_id.to_string(), expanded);
    });
}

// ---------------------------------------------------------------------------
// 新建条目后的"待聚焦"目标(UX closure):创建 provider / model / header 后,
// rebuild 会丢弃旧 EditorView 并新建一批,因此无法在 action handler 里直接拿到
// 新 editor handle 去 focus。改用 process-local 一次性请求:action handler 记下
// "下次 build_row 时给这个 provider 的某个 editor 聚焦",`build_row` 持有刚建好的
// handle,命中后立即 `ctx.focus` 并清掉请求(只生效一次)。
// 与 EXPANDED_* 一样 thread_local、不持久化。
// ---------------------------------------------------------------------------

#[derive(Clone, Copy, PartialEq)]
pub(super) enum PendingFocusTarget {
    /// 聚焦卡片的 name 编辑器(新建 provider)。
    ProviderName,
    /// 聚焦最后一条 model 行的 id 编辑器(新增 model)。
    LastModelId,
    /// 聚焦最后一条 header 行的 key 编辑器(新增 header)。
    LastHeaderKey,
}

std::thread_local! {
    /// {provider_id => 待聚焦目标}。由 add_* action handler 写入,build_row 消费(取走)。
    static PENDING_FOCUS: RefCell<HashMap<String, PendingFocusTarget>> = RefCell::new(HashMap::new());
}

/// 记录"下次重建该 provider 的卡片时,聚焦指定 editor"。仅生效一次。
pub(super) fn request_focus(provider_id: &str, target: PendingFocusTarget) {
    PENDING_FOCUS.with(|m| {
        m.borrow_mut().insert(provider_id.to_string(), target);
    });
}

/// 取走某 provider 的待聚焦请求(消费语义:取出后即清除)。
fn take_pending_focus(provider_id: &str) -> Option<PendingFocusTarget> {
    PENDING_FOCUS.with(|m| m.borrow_mut().remove(provider_id))
}

/// provider 卡片用于 scroll-to 的 `SavePosition` id。新建卡片后用它把卡片滚进视野。
pub(super) fn provider_card_position_id(provider_id: &str) -> String {
    format!("agent_provider_card_{provider_id}")
}
...
struct ProviderRow {
    ...
    /// 卡片级折叠/展开 chevron 的鼠标状态。
    card_expand_button_state: MouseStateHandle,
    /// 折叠态 header 上的快速删除(×)鼠标状态(展开态用底部的删除按钮)。
    card_quick_remove_button_state: MouseStateHandle,
...
pub(super) struct AgentProvidersWidget {
    add_key_button_state: MouseStateHandle,
    add_endpoint_button_state: MouseStateHandle,
...
impl AgentProvidersWidget {
    pub(super) fn new(ctx: &mut ViewContext<AISettingsPageView>) -> Self {
        ...
        Self {
            add_key_button_state: MouseStateHandle::default(),
            add_endpoint_button_state: MouseStateHandle::default(),
    ...
    fn build_row(
        ...
        // UX closure:刚新建 provider / model / header 时,聚焦相应的新 editor,
        // 让用户点完"添加"就能直接打字。请求只生效一次(取走即清)。
        if let Some(target) = take_pending_focus(&provider_id) {
            match target {
                PendingFocusTarget::ProviderName => ctx.focus(&name_editor),
                PendingFocusTarget::LastModelId => {
                    if let Some(last) = model_rows.last() {
                        ctx.focus(&last.id_editor);
                    }
                }
                PendingFocusTarget::LastHeaderKey => {
                    if let Some(last) = header_rows.last() {
                        ctx.focus(&last.key_editor);
                    }
                }
            }
        }
        ...
        ProviderRow {
            ...
            base_url_editor,
            api_key_editor,
            card_expand_button_state: MouseStateHandle::default(),
            card_quick_remove_button_state: MouseStateHandle::default(),
    ...
    fn render_provider_card(
        ...
        // 已配置状态:以 keychain 实际读回为准(写失败时 RRA5-03 会回滚内存,
        // 所以这里不会把"写库失败"误显示为已配置)。状态在卡片 header 常显,
        // 折叠时也能一眼看到;不渲染 key 值本身。
        let key_configured = AgentProviderSecrets::as_ref(app)
            .get(&provider.id)
        ...
        // ---- 卡片折叠/展开:已配置卡默认折叠成一行摘要,空白/新建卡默认展开 ----
        let default_expanded = !key_configured && provider.base_url.trim().is_empty();
        let is_expanded = is_provider_expanded(&provider.id, default_expanded);

        let chevron = Self::render_card_button(
            if is_expanded { "▾" } else { "▸" },
            row.card_expand_button_state.clone(),
        ...
        // 状态以"是否可用"为准,不是"有没有 key":
        //   有 key            → 绿 ✓ Key saved
        //   无 key 但有 endpoint → 绿 ✓ Endpoint set(Ollama/本地/自建无鉴权也算配好,
        //                          见 openai_compatible.rs:key 为空时不发 auth header)
        //   既无 key 又无 endpoint → 灰 Not set up(真·空白卡,默认就展开)
        let has_endpoint = !provider.base_url.trim().is_empty();
        let status_chip = if key_configured {
            Text::new(
                crate::t!("settings-agent-providers-status-configured"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            ...
            Text::new(
                crate::t!("settings-agent-providers-status-endpoint"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            .with_color(appearance.theme().ansi_fg_green().into())
            ...
            Text::new(
                crate::t!("settings-agent-providers-status-missing"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            .with_color(appearance.theme().disabled_ui_text_color().into())
        ...
        let mut header_row = Flex::row()
            ...
            .with_child(status_chip);
        // 折叠态下提供快速删除,免得为删一个已配置 provider 还得先展开。
        // 展开态用底部的删除按钮(避免两个删除按钮共享同一 mouse state)。
        if !is_expanded {
            let quick_remove = Self::render_card_button(
                "×",
                row.card_quick_remove_button_state.clone(),
                AISettingsPageAction::RemoveAgentProvider {
                    provider_id: provider.id.clone(),
                },
                appearance,
            ...
            header_row = header_row.with_child(
                Container::new(quick_remove)
                    .with_margin_left(MODEL_ROW_GAP)
                    .finish(),
            );
        }
        ...
        if is_expanded {
            card.add_child(Container::new(name_field).with_margin_top(10.).finish());
            card.add_child(api_type_field);
            card.add_child(base_url_field);
            ...
            card.add_child(Container::new(bottom_row).with_margin_top(10.).finish());
        }

        let card_el = Container::new(card.finish())
            .with_background(appearance.theme().surface_1())
            .with_uniform_padding(12.)
            .with_corner_radius(CornerRadius::with_all(Radius::Pixels(6.)))
            .with_margin_bottom(8.)
            .finish();
        // 包一层 SavePosition,新建卡片后可用其 id 滚动进视野(UX closure)。
        SavePosition::new(card_el, &provider_card_position_id(&provider.id)).finish()
    }
}
...
impl SettingsWidget for AgentProvidersWidget {
    ...
    fn render(
        ...
        // 已配置摘要,与每张卡片的状态分档一致(互斥、不重复计数):
        //   key 数      = 有 keychain key 的 provider(key 优先)
        //   endpoint 数 = 无 key 但有 base_url 的 provider(keyless 本地/自建)
        // 两者之和 = 已配好的 provider 总数。
        let (configured_key_count, configured_endpoint_count) = {
            let secrets = AgentProviderSecrets::as_ref(app);
            let mut keys = 0usize;
            let mut endpoints = 0usize;
            for p in &providers {
                if secrets.get(&p.id).is_some_and(|key| !key.is_empty()) {
                    keys += 1;
        ...
        let add_key_button = Self::render_card_button(
            crate::t!("settings-agent-providers-add-key-button"),
            self.add_key_button_state.clone(),
            AISettingsPageAction::AddAgentProviderKey,
            appearance,
        );
        let add_endpoint_button = Self::render_card_button(
            crate::t!("settings-agent-providers-add-endpoint-button"),
            self.add_endpoint_button_state.clone(),
            ...
            Flex::row()
                ...
                .with_child(
                    Container::new(add_endpoint_button)
                        .with_margin_left(HEADER_PADDING)
                        .finish(),
                )
                .finish(),
        )
        ...
        // ---- 已配置摘要行(仅在已有 provider 时显示) ----
        if !providers.is_empty() {
            let summary = Container::new(
                Text::new(
                    crate::t!(
                        "settings-agent-providers-summary",
                        keys = configured_key_count,
                        endpoints = configured_endpoint_count
                    ),
                    appearance.ui_font_family(),
                    appearance.ui_font_size(),
                )
