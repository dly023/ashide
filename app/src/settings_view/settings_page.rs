...
use super::{
    ...
    local_drive_page::LocalDriveSettingsPageView,
...
pub enum SettingsPageViewHandle {
    ...
    LocalDrive(ViewHandle<LocalDriveSettingsPageView>),
    /// 全局 HTTP 代理设置页。
...
impl SettingsPageViewHandle {
    pub fn child_view(&self) -> Box<dyn Element> {
        ...
        match self {
            ...
            LocalDrive(view_handle) => ChildView::new(view_handle).finish(),
...
/// Optional per-row annotation used by a few settings widgets.
...
impl<V: warpui::View> PageType<V> {
    ...
    /// Scroll an arbitrary `SavePosition` id (e.g. a sub-element nested inside a
    /// widget, like an individual agent-provider card) into view. Unlike
    /// [`scroll_to_widget`], the id is a runtime `String` and no widget
    /// highlight is applied.
    pub fn scroll_to_position_id(&mut self, position_id: String) {
        match self {
            Self::Monolith {
