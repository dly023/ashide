...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        ...
        .with_group(bindings::BindingGroup::AiAssistant)],
        app,
    );
    ...
    );
    {
        ...
        let mode_bindings: Vec<FixedBinding> = ThinkingDisplayMode::iter()
            .map(|mode| {
                ...
                .with_group(bindings::BindingGroup::AiAssistant.as_str())
            })
            .collect();
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // third-party coding agents rather than Ashide's own AI.
        Self::update_editor_interaction_state(
    ...
    /// Scroll a newly-created agent-provider card into view. Call this *after*
    /// `rebuild_current_page` so the card's `SavePosition` is part of the next
    /// paint; the scroll target is resolved during that paint.
    fn scroll_to_provider_card(&mut self, provider_id: &str) {
        let position_id = super::agent_providers_widget::provider_card_position_id(provider_id);
        self.page.scroll_to_position_id(position_id);
    }
    ...
    fn save_agent_provider_edits(
        ...
        let result = crate::ai::agent_providers::AgentProviderSecrets::handle(ctx)
            .update(ctx, |secrets, ctx| {
                secrets.set(provider_id, api_key.to_owned(), ctx)
            });
        if let Err(e) = result {
            Self::add_secret_error_toast(e, ctx);
        }
    }

    /// Surfaces an agent-provider API-key keychain write failure as an error
    /// toast. `AgentProviderSecrets` rolls back its in-memory state on failure,
    /// so a failed save is never shown as "configured".
    fn add_secret_error_toast(error: impl std::fmt::Display, ctx: &mut ViewContext<Self>) {
        let message = crate::t!(
            "settings-agent-providers-secret-save-failed",
            error = error.to_string()
        );
        let window_id = ctx.window_id();
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            toast_stack.add_transient_toast(DismissibleToast::error(message), window_id, ctx);
        });
    }

    /// Surfaces a success/info toast (reuses the same ephemeral toast stack as
    /// [`add_secret_error_toast`]). Used to give visible feedback after a
    /// models.dev sync that otherwise mutates state silently.
    fn add_success_toast(message: String, ctx: &mut ViewContext<Self>) {
        let window_id = ctx.window_id();
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            toast_stack.add_transient_toast(DismissibleToast::success(message), window_id, ctx);
        });
    }

    /// Surfaces an error toast (reuses the ephemeral toast stack). Used for the
    /// models.dev sync early-return failure cases that previously only logged.
    fn add_error_toast(message: String, ctx: &mut ViewContext<Self>) {
        let window_id = ctx.window_id();
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            toast_stack.add_transient_toast(DismissibleToast::error(message), window_id, ctx);
        });
...
pub enum AISettingsPageAction {
    ...
    ToggleCLIAgentHistoryAgent(CLIAgent),
    ...
    AddAgentProvider,
    /// BYOK: 新增一个"已知厂商 + 自带 key"的 provider — base_url 预填为所选
    /// api_type 的默认 endpoint,用户只需补 key。
    AddAgentProviderKey,
    /// BYOK: 新增一个"自定义 OpenAI 兼容 endpoint"的 provider — base_url 留空,
    /// 强制用户填自己的服务地址。
    AddAgentProviderEndpoint,
    RemoveAgentProvider {
        provider_id: String,
    },
    ...
    /// 折叠/展开整张 provider 卡片(轻量摘要 ↔ 完整编辑表单)。
    ToggleAgentProviderExpanded {
        provider_id: String,
        expand: bool,
    },
    // ----- 单条模型条目 detail panel -----
...
impl TypedActionView for AISettingsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AISettingsPageAction::ToggleCLIAgentHistoryAgent(agent) => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    let mut enabled = settings.cli_agent_history_enabled_agents.value().clone();
                    let serialized = agent.to_serialized_name();
                    if enabled.iter().any(|name| name == &serialized) {
                        enabled.retain(|name| name != &serialized);
                    } else {
                        enabled.push(serialized);
                    }
                    report_if_error!(settings
            ...
            AISettingsPageAction::ToggleShowAgentTips => {
                InputSettings::handle(ctx).update(ctx, |input_settings, ctx| {
                    if let Err(e) = input_settings.show_agent_tips.toggle_and_save_value(ctx) {
            ...
            AISettingsPageAction::ToggleLocalDriveContext => {
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    let _ = settings
                        .local_drive_context_enabled
                        .toggle_and_save_value(ctx);
                });
                ctx.notify();
            }
            ...
            AISettingsPageAction::AddAgentProvider => {
                ...
                let new_id = new_provider.id.clone();
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    let mut providers = settings.agent_providers.value().clone();
                    providers.push(new_provider);
                    let _ = settings.agent_providers.set_value(providers, ctx);
                });
                // UX closure:聚焦新卡片的 name 编辑器并把卡片滚进视野。
                super::agent_providers_widget::request_focus(
                    &new_id,
                    super::agent_providers_widget::PendingFocusTarget::ProviderName,
                );
                self.rebuild_current_page(ctx);
                self.scroll_to_provider_card(&new_id);
            }
            AISettingsPageAction::AddAgentProviderKey => {
                let mut provider = crate::settings::AgentProvider::new_empty();
                // "自带 key"流:预填所选 api_type 的默认 endpoint,用户只需补 key。
                provider.base_url = provider.api_type.default_base_url().to_owned();
                let new_id = provider.id.clone();
                // 新建卡片默认展开,点完按钮即可直接填写。
                super::agent_providers_widget::set_provider_expanded(&new_id, true);
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    let mut providers = settings.agent_providers.value().clone();
                    providers.push(provider);
                    let _ = settings.agent_providers.set_value(providers, ctx);
                });
                // UX closure:聚焦 name 编辑器并把新卡片滚进视野。
                super::agent_providers_widget::request_focus(
                    &new_id,
                    super::agent_providers_widget::PendingFocusTarget::ProviderName,
                );
            ...
            AISettingsPageAction::AddAgentProviderEndpoint => {
                // "自定义 endpoint"流:base_url 留空(new_empty 即如此),强制用户填服务地址。
                let provider = crate::settings::AgentProvider::new_empty();
                let new_id = provider.id.clone();
                super::agent_providers_widget::set_provider_expanded(&new_id, true);
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    let mut providers = settings.agent_providers.value().clone();
            ...
            AISettingsPageAction::AddAgentProviderModel { provider_id } => {
                ...
                // UX closure:聚焦新模型行的 id 编辑器并把卡片滚进视野(长列表时新行在折叠区外)。
                super::agent_providers_widget::request_focus(
                    provider_id,
                    super::agent_providers_widget::PendingFocusTarget::LastModelId,
                );
                self.rebuild_current_page(ctx);
                self.scroll_to_provider_card(provider_id);
            }
            ...
            AISettingsPageAction::AddAgentProviderHeader { provider_id } => {
                ...
                // UX closure:聚焦新 header 行的 key 编辑器并把卡片滚进视野。
                super::agent_providers_widget::request_focus(
                    provider_id,
                    super::agent_providers_widget::PendingFocusTarget::LastHeaderKey,
                );
                self.rebuild_current_page(ctx);
                self.scroll_to_provider_card(provider_id);
            }
            ...
            AISettingsPageAction::SyncProviderModelsFromModelsDev { provider_id } => {
                ...
                let Some(catalog) = models_dev::cached() else {
                    ...
                    Self::add_error_toast(
                        crate::t!("settings-agent-providers-sync-catalog-not-loaded"),
                        ctx,
                    );
                    return;
                };
                ...
                let Some((_, cat_provider)) = cat_provider else {
                    ...
                    Self::add_error_toast(
                        crate::t!(
                            "settings-agent-providers-sync-no-match",
                ...
                // 在变更前算好计数:updated = 本地已有且 catalog 也有的 id;
                // added = catalog 多出的 id。用于成功 toast 反馈。
                let local_ids: std::collections::HashSet<String> =
                    local.models.iter().map(|m| m.id.clone()).collect();
                let updated_count = local
                    .models
                    .iter()
                    .filter(|m| cat_models.contains_key(&m.id))
                    .count();
                let added_count = cat_models
                    .keys()
                ...
                self.rebuild_current_page(ctx);
                // UX closure:同步成功后用 toast 反馈"更新 X 项 / 新增 Y 项"。
            ...
            AISettingsPageAction::ToggleAgentProviderExpanded {
                provider_id,
                expand,
            } => {
                super::agent_providers_widget::set_provider_expanded(provider_id, *expand);
                self.rebuild_current_page(ctx);
            }
...
impl UsageWidget {
    fn render_request_usage_count(
        ...
        let request_count_label = if is_unlimited {
    ...
    fn render_ai_usage_limit_row(
        ...
        let request_usage_details = Flex::column()
            ...
            .with_child(self.render_request_usage_count(used, limit, is_unlimited, appearance));
        ...
        let subtext = {
            let subtext_fragments = vec![
                FormattedTextFragment::plain_text(
                    "You haven't added any MCP servers yet. Once you do, you'll be able to control how much autonomy the Ashide Agent has when interacting with them. ",
                ),
...
impl SettingsWidget for MCPServersWidget {
    ...
    fn render(
        ...
        let mcp_description = vec![
            FormattedTextFragment::plain_text(
                "Add MCP servers to extend the Ashide Agent's capabilities. \
...
impl SettingsWidget for AIFactWidget {
    ...
    fn search_terms(&self) -> &str {
        "agent oz ai a.i. knowledge fact memory memories rules Ashide Drive context workflows notebooks environment variables"
    }
    ...
    fn render(
        ...
        // 去中心化分支:不再渲染 "Ashide Drive as agent context" 开关。
        let _ = self;
        let _ = view;
...
impl VoiceWidget {
    fn render_voice_section(
        ...
        let voice_input_description_text_fragments = vec![
            FormattedTextFragment::plain_text(
                "Voice input allows you to control Ashide by speaking directly to your terminal (powered by ",
            ),
...
impl SettingsWidget for OtherAIWidget {
    ...
    fn search_terms(&self) -> &str {
        "other use agent footer toolbar layout chip chips rearrange re-arrange thinking expanded reasoning collapse never show hide"
    }
...
struct CLIAgentWidget {
    ...
    history_agent_toggles: HashMap<CLIAgent, SwitchStateHandle>,
}

impl Default for CLIAgentWidget {
    fn default() -> Self {
        Self {
            cli_agent_footer_toggle: Default::default(),
            auto_toggle_rich_input_toggle: Default::default(),
            auto_toggle_rich_input_info_tooltip: Default::default(),
            auto_open_rich_input_on_cli_agent_start_toggle: Default::default(),
            auto_dismiss_rich_input_toggle: Default::default(),
            history_agent_toggles: crate::settings::ai::cli_agent_history_discoverable_agents()
                .into_iter()
                .map(|agent| (agent, SwitchStateHandle::default()))
                .collect(),
        }
    }
}
...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn search_terms(&self) -> &str {
        "third party cli coding agent claude codex toolbar footer layout chip chips rearrange re-arrange bar command regex auto show rich input dismiss"
    }
    ...
    fn render(
        ...
        let history_agents = crate::settings::ai::cli_agent_history_discoverable_agents();
        column.add_child(
            Container::new(
                FormattedTextElement::new(
                    FormattedText::new([FormattedTextLine::Line(vec![
                        FormattedTextFragment::plain_text(crate::t!(
                            "settings-ai-discover-agent-sessions-description"
                        )),
                    ])]),
                    appearance.ui_font_size(),
                    appearance.ui_font_family(),
        ...
        for agent in history_agents {
            let switch_state = self
                .history_agent_toggles
                .get(&agent)
                .expect("discoverable CLI agent must have a settings switch state");
            column.add_child(build_toggle_element(
                render_body_item_label::<AISettingsPageAction>(
                    crate::t!(
                        "settings-ai-discover-agent-sessions",
                ...
                render_ai_feature_switch(
                    switch_state.clone(),
                    ai_settings.is_cli_agent_history_enabled(agent),
                    true,
                    AISettingsPageAction::ToggleCLIAgentHistoryAgent(agent),
                    app,
                ),
