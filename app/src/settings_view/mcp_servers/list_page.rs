...
use crate::{
    ...
    workspace::{TerminalSessionFallbackBehavior, Workspace},
};
...
impl MCPServersListPageView {
    ...
    fn open_logs_for_server(&self, log_file_path: &PathBuf, ctx: &mut ViewContext<Self>) {
        ...
        workspace_view_handle.update(ctx, |workspace, ctx| {
            let Some(terminal_view_handle) = workspace.focus_terminal_input(
                None,
    ...
    fn handle_server_card_event(&mut self, event: &ServerCardEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            ServerCardEvent::Share(item_id) => {
                log::debug!("Ashide: MCP sharing is disabled for {item_id:?}");
            }
        ...
        if !has_any_content {
                ...
                && filtered_file_based_cards.is_empty()
            {
                ...
                if !shared_server_cards.is_empty() {
                    ...
                    page.add_child(self.render_server_cards_section(
                        &crate::t!("settings-mcp-list-section-shared-by-ashide-and-other-devices"),
                        &shared_server_cards,
                        appearance,
                        app,
                    ));
                    ...
                    page.add_child(self.render_server_cards_section(
                        &crate::t!("settings-mcp-list-section-shared-from-ashide"),
    ...
    fn file_based_root_chip_text(root_path: &PathBuf) -> Option<String> {
        ...
        // If the path is the Ashide data directory (for example, ~/.ashide or an app-channel
        // data dir), set the text to "global". The Ashide provider stores its data directory as
        // the root path rather than the home directory, unlike other providers that store the
