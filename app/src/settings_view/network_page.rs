...
impl NetworkPageView {
    ...
    /// Surfaces a proxy-password keychain write failure as an error toast.
    /// `ProxyCredentials` rolls back its in-memory password on failure, so the
    /// editor still reflects the previously stored value.
    fn add_proxy_password_error_toast(error: impl std::fmt::Display, ctx: &mut ViewContext<Self>) {
        let message = crate::t!(
            "settings-network-proxy-password-save-failed",
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            NetworkPageAction::SaveProxyPassword => {
                ...
                let result = ProxyCredentials::handle(ctx)
                    .update(ctx, |creds, ctx| creds.set_password(value, ctx));
                if let Err(e) = result {
                    Self::add_proxy_password_error_toast(e, ctx);
                }
                ctx.notify();
            }
            NetworkPageAction::ClearProxyPassword => {
                let result = ProxyCredentials::handle(ctx)
                    .update(ctx, |creds, ctx| creds.set_password(String::new(), ctx));
                if let Err(e) = result {
                    Self::add_proxy_password_error_toast(e, ctx);
                }
                ctx.notify();
