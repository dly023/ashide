...
use crate::drive::{LocalDriveObjectArgs, LocalDriveObjectSettings, ObjectTypeAndId};
...
use crate::object_store::{GenericStringObjectFormat, JsonObjectType, ObjectType};
...
///
/// 取 `ChannelState::app_id().application_name()` 是为了让 OSS 构建显示 `Ashide`、
/// 而 Dev/Preview/Local 等变体显示各自的 `AshideDev` / `AshidePreview` / `AshideLocal`,
/// 避免在 fork 中跨多处硬编码字符串(Windows 任务管理器按窗口标题做进程分组,
/// 硬编码 `"Ashide"` 会让开发版和日用版混在一起)。
///
...
pub fn init(app: &mut AppContext) {
    ...
    app.add_global_action(
        ...
        open_local_drive_object,
    );
    app.add_action(
        "root_view:open_drive_object_existing_window",
        RootView::open_local_drive_object_in_existing_window,
    );
...
///
/// This is the canonical way to open a new Ashide window - all other entrypoints should delegate to
/// it if possible.
...
pub(crate) fn open_new_from_path(
    ...
    let source = path_if_directory(&arg.path)
        .map(|path| NewWorkspaceSource::DirectoryInCurrentEnvironment {
            path: path.into(),
            hide_homepage: false,
        })
        .unwrap_or(NewWorkspaceSource::DefaultEnvironmentTerminal);
    open_new_with_workspace_source(source, ctx)
}
...
fn open_local_drive_object(arg: &LocalDriveObjectArgs, ctx: &mut AppContext) {
    match arg.object_type {
        ObjectType::Notebook => open_new_workspace_with_notebook_open(
            ObjectStoreId::StableId(arg.stable_id),
            arg.settings.clone(),
            ctx,
        ),
...
/// This action will show or hide all of Ashide's windows except the quake window
///
/// - If Ashide is active and has any windows, hide those windows.
/// - If Ashide is hidden, show all windows.
/// - If Ashide is active but has 0 normal windows, create a new window with a new session.
fn show_or_hide_non_quake_mode_windows(_: &(), ctx: &mut AppContext) {
    ...
    // Now there is at least one window. If a Ashide window is active, hide the app.
...
pub enum NewWorkspaceSource {
    ...
    DefaultEnvironmentTerminal,
    DirectoryInCurrentEnvironment {
        path: PathBuf,
        hide_homepage: bool,
    },
    ExplicitCurrentAppTerminalSession {
...
/// before the local auth facade is available.
...
impl RootView {
    ...
    fn add_session_at_path(&mut self, path: &PathBuf, ctx: &mut ViewContext<Self>) -> bool {
        ...
        if let AuthOnboardingState::Terminal(handle) = &self.auth_onboarding_state {
            handle.update(ctx, |view, ctx| {
                if let Some(path) = path_if_directory(path) {
                    view.open_directory_tab_in_current_environment(path.into(), false, ctx);
                } else {
                    view.add_terminal_tab(false, ctx);
                }
                ctx.windows().show_window_and_focus_app(window_id);
                ctx.notify();
            })
    ...
    pub fn open_local_drive_object_in_existing_window(
        &mut self,
        arg: &LocalDriveObjectArgs,
        ctx: &mut ViewContext<Self>,
    ) -> bool {
        if let AuthOnboardingState::Terminal(handle) = &self.auth_onboarding_state {
            ...
            match arg.object_type {
                ObjectType::Notebook => {
                    handle.update(ctx, |workspace, ctx| {
                        let initialized_section_states =
                            workspace.has_local_drive_initialized_sections(ctx);
                        let notebook_id = ObjectStoreId::StableId(arg.stable_id);
                ...
                ObjectType::Workflow => {
                    handle.update(ctx, |workspace, ctx| {
                        let initialized_section_states =
                            workspace.has_local_drive_initialized_sections(ctx);
                        let workflow_id = ObjectStoreId::StableId(arg.stable_id);
                        let settings = arg.settings.clone();
                        let _ = ctx.spawn(initialized_section_states, move |workspace, _, ctx| {
                ...
                ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                    ...
                    let item_id =
                        LocalDriveItemId::Object(ObjectTypeAndId::from_generic_string_object(
                    ...
                    handle.update(ctx, |workspace, ctx| {
                        let initialized_section_states =
                            workspace.has_local_drive_initialized_sections(ctx);
                        let _ = ctx.spawn(initialized_section_states, move |workspace, _, ctx| {
                            workspace.view_in_and_focus_local_drive(item_id, ctx);
                        });
                    });
                }
                ObjectType::Folder => {
                    ...
                    let item_id = LocalDriveItemId::Object(ObjectTypeAndId::Folder(
                        ObjectStoreId::StableId(arg.stable_id),
                    ));
                    handle.update(ctx, |workspace, ctx| {
                        let initialized_section_states =
            ...
            log::warn!("Auth not complete before trying to open Ashide Drive object");
        }
        true
    }
        ...
        match event {
            AuthManagerEvent::AuthComplete => {
                ...
                } else if let AuthOnboardingState::NeedsSsoLink { .. } = &self.auth_onboarding_state
                {
                    ...
                    // since we should err on the side of them _not_ being able to use Ashide.
                ...
                UserAuthenticationError::UserAccountDisabled => {
                    cfg_if! {
                        if #[cfg(target_family = "wasm")] {
                            ...
                            // 本地/去中心化构建无云端账号,native 不做强制登出(无 log_out UI 入口)。
                            log::warn!("User account disabled; ignoring (Ashide 已移除 log_out)");
                        }
                    }
                }
    ...
    fn handle_auth_override_warning_modal_event(
        ...
        match event {
            AuthOverrideWarningModalEvent::Close => {
                // 本地/去中心化构建无云端账号,关闭时不触发登出(无 log_out 入口)。
            }
    ...
    /// Ashide(本地化,Phase 5):原 `handle_preferences_syncer_event` 在云端
