...
pub(super) fn relaunch() -> Result<()> {
    ...
    // Ashide OSS(Channel::Oss):没有代码签名,无法用 RENAME_SWAP 在原地替换 bundle。
    ...
    // 启动新版 Ashide 前先等待当前进程退出，避免 Dock 中短暂出现多个图标。
    ...
    if let Ok(path) = env::var("ASHIDE_CHANNEL_VERSIONS_PATH") {
        let quoted_path = shell_escape::escape(path.into());
        open_args.push_str(&format!(
            " --env ASHIDE_CHANNEL_VERSIONS_PATH={quoted_path}"
        ));
    }
...
fn oss_open_installer() -> Result<()> {
    ...
    let dmg = find_latest_dmg(&autoupdate_dir)
        .ok_or_else(|| anyhow!("Ashide OSS: 找不到已下载的 dmg(目录: {autoupdate_dir:?})"))?;

    log::info!("Ashide OSS: 准备打开安装 dmg {dmg:?}");
...
/// Determines if the user needs authorization in order to update Ashide.
...
/// * Ashide can create files in the directory
/// * Ashide can modify the permissions of created files
...
fn parse_code_signature_team_identifier(metadata: &[u8]) -> Result<String> {
    let metadata = str::from_utf8(metadata).context("codesign metadata is not valid UTF-8")?;
    let team_identifier = metadata
        .lines()
        .find_map(|line| line.strip_prefix("TeamIdentifier="))
        .context("codesign metadata is missing TeamIdentifier")?;
    ensure!(
        team_identifier.len() == 10
            && team_identifier
                .bytes()
                .all(|byte| byte.is_ascii_uppercase() || byte.is_ascii_digit()),
        "codesign TeamIdentifier is not a valid target-owned Apple Team ID"
    );
    Ok(team_identifier.to_owned())
}

async fn code_signature_team_identifier(path: &Path) -> Result<String> {
    let output = Command::new("/usr/bin/codesign")
        ...
        "Failed to inspect current code signature: {output:?}"
    );
    parse_code_signature_team_identifier(&output.stderr)
}

/// Verifies that the staged bundle path has a valid macOS code signature, and that its
/// team identifier matches the currently running Ashide bundle.
...
pub(super) async fn download_update_and_cleanup(
    ...
    // Ashide OSS(Channel::Oss):没有 Apple Developer ID 签名,不能走官方
...
async fn oss_download_dmg(
    ...
    log::info!(
        "Ashide OSS: 下载更新 dmg, version {} on channel {channel}",
...
async fn apply_update(channel: Channel, version_info: &VersionInfo, update_id: &str) -> Result<()> {
    ...
    if async_fs::metadata(executable_temp_file.as_path())
        ...
        .is_ok()
    {
        // If we performed this process already but didn't relaunch Ashide, the old executable will
...
async fn download_and_extract_binary(
    ...
    let current_bundle_path = PathBuf::from(get_bundle_path()?);
    let expected_team_identifier = code_signature_team_identifier(&current_bundle_path)
        .await
        .context("Failed to read the current bundle signing authority")?;
    future::try_zip(
        verify_code_signature("bundle", &target, &expected_team_identifier),
        verify_code_signature(
            "executable",
            executable_path_buf.as_path(),
            &expected_team_identifier,
        ),
    )
    .await?;
...
async fn mount_dmg(dmg_dir: &Path, update_id: &str) -> Result<PathBuf> {
    ...
    // -nobrowse: Do not show the Ashide DMG in Finder or similar apps.
    // -noautoopen: Do not open the Ashide DMG in Finder.
...
fn update_url(channel: Channel, version: &str) -> String {
    ...
    if matches!(channel, Channel::Oss) {
        ...
        if let Some(release) = github::cached_release() {
            ...
            log::warn!(
                "Ashide OSS: cached release tag {} 没有名为 {asset} 的资产,回退到 tag URL",
                release.tag_name
            );
        }
        return format!("https://github.com/dly023/ashide/releases/download/v{version}/{asset}");
    }
    format!(
        "{}/{}",
...
fn app_name_prefix(channel: Channel) -> &'static str {
    match channel {
        ...
        Channel::Preview => "AshidePreview",
        Channel::Local => "AshideLocal",
        Channel::Integration => "AshideIntegration",
        Channel::Dev => "AshideDev",
        Channel::Oss => "Ashide",
    }
}
