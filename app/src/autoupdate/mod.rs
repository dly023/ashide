...
pub(crate) fn verify_oss_asset_sha256(
    ...
    let Some(release) = github::cached_release() else {
        log::info!("Ashide OSS: 没有 cached release,跳过 SHA-256 校验");
        return Ok(());
    };
    let Some(asset) = release.find_asset(asset_name) else {
        log::warn!("Ashide OSS: cached release 中找不到 {asset_name},跳过 SHA-256 校验");
        return Ok(());
    };
    let Some(expected) = asset.sha256_hex() else {
        log::info!(
            "Ashide OSS: asset {asset_name} 没有可识别的 digest(可能是 sha256 之外的算法),跳过校验"
        );
        return Ok(());
    };
    ...
    if actual == expected {
        log::info!("Ashide OSS: SHA-256 校验通过 ({asset_name})");
        Ok(())
    } else {
        Err(anyhow!(
...
pub enum AutoupdateStage {
    ...
    /// A new version was installed, but Ashide hasn't restarted yet.
    ///
...
impl AutoupdateState {
    ...
    fn should_update(&mut self, version: VersionInfo, update_id: String) -> UpdateReady {
        ...
        // Ashide OSS 注:`ChannelState::app_version()` 返回的是注入的 `GIT_RELEASE_TAG`
        ...
        // - Ashide release:tag 一律带 `v`,两边 trim 后仍相等,行为不变。
        // - Ashide OSS:trim 后才相等,正确识别同版本。
    ...
    fn on_update_check_complete(
        ...
        match &update_available {
            Ok(UpdateReady::CanDownload {
                ...
                // Ashide OSS(Channel::Oss):走和官方一致的下载流程,平台 download_update_and_cleanup
...
pub fn accessibility_content(
    ...
    match (request_type, update_available) {
        ...
        (RequestType::ManualCheck, Ok(UpdateReady::Yes { .. })) => Some(AccessibilityContent::new(
            "Update available.",
            "Use the command palette to install and relaunch Ashide",
...
async fn fetch_version(
    ...
    // Ashide OSS 走 GitHub Releases(dly023/ashide),不再请求旧上游更新服务
...
pub fn apply_update(
    ...
    // Ashide OSS:三平台 apply_update 都走与官方一致的入口,只是各平台 relaunch()
...
/// Relaunch Ashide to apply an update.
///
/// This will:
...
{
    // Ashide OSS(Channel::Oss):mac 平台 dmg 已下载,无需 mount/swap/verify,直接
