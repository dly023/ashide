...
pub(super) async fn download_update_and_cleanup(
    ...
    match UpdateMethod::detect() {
        ...
        UpdateMethod::PackageManager(package_manager) => {
            log::info!("Detected that Ashide was installed using {package_manager:?}");
...
pub(super) fn apply_update() -> Result<ReadyForRelaunch> {
    ...
    match UpdateMethod::detect() {
        ...
        UpdateMethod::PackageManager(package_manager) => bail!(
            "Ashide does not support package-manager autoupdate for {package_manager}; install the new release manually"
        ),
    }
}
...
mod appimage {
    ...
    pub(super) async fn download_update_and_cleanup(
        ...
        // Ashide OSS:从 GitHub Release 缓存里取真实下载 URL,绕开空的 releases_base_url。
        // 官方 channel 仍然走 release_assets_directory_url。
        let url = if matches!(channel, warp_core::channel::Channel::Oss) {
            // OSS Linux AppImage 资产名与 script/linux/bundle 输出保持一致。
            let asset = "Ashide-x86_64.AppImage";
            if let Some(release) = crate::autoupdate::github::cached_release() {
                if let Some(found) = release.find_asset(asset) {
                    ...
                    format!(
                        "https://github.com/dly023/ashide/releases/download/v{}/{asset}",
        ...
        // Ashide OSS:在覆盖原 AppImage 之前先对临时文件做 SHA-256 校验,
        ...
        if matches!(channel, warp_core::channel::Channel::Oss) {
            ...
            if let Err(e) =
                crate::autoupdate::verify_oss_asset_sha256(&temp_path, "Ashide-x86_64.AppImage")
            {
        ...
        // Move new AppImage over the one that launched the current Ashide instance.
    ...
    pub(super) fn relaunch(appimage_path: &Path) -> Result<()> {
        ...
        if let Ok(path) = std::env::var("ASHIDE_CHANNEL_VERSIONS_PATH") {
            command.env("ASHIDE_CHANNEL_VERSIONS_PATH", path);
        }

        log::info!("Relaunching Ashide for update...");
        command.spawn()?;
        Ok(())
    }
}
...
mod package_manager {
    ...
    pub(super) fn relaunch() -> Result<()> {
        ...
        // Add any arguments that were passed to Ashide, skipping the first
...
/// Returns which method should be used to update Ashide.
#[derive(Debug)]
pub(crate) enum UpdateMethod {
    /// We don't know how to update Ashide.
    Unknown,
    /// Ashide is running as an AppImage and should be updated in-place.
    AppImage(PathBuf),
    /// Ashide can be updated using the given package manager.
...
impl PackageManager {
    ...
    /// 把"用户应该跑的升级命令"写到日志里。OSS 用户翻 ~/.local/share/dev.ashide.Ashide/
