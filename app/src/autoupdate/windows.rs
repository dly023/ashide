...
/// Download the Inno Setup install wizard, the same one users run on the first Ashide install, and
...
pub(super) async fn download_update_and_cleanup(
    ...
    // Ashide OSS:从 GitHub Release 缓存里取真实下载 URL。其他 channel 走 release_assets_directory_url。
    let url = if matches!(channel, Channel::Oss) {
        if let Some(release) = github::cached_release() {
            if let Some(found) = release.find_asset(&installer_file_name) {
                ...
                log::warn!(
                    "Ashide OSS: cached release tag {} 没有名为 {installer_file_name} 的资产,回退到 tag URL",
                    release.tag_name
                );
                format!(
                    "https://github.com/dly023/ashide/releases/download/v{}/{installer_file_name}",
    ...
    // Ashide OSS:校验 GitHub Release 元数据里的 SHA-256,防御 CDN 中间人/损坏。
...
const UPDATE_LOG_FILENAME: &str = "ashide_update.log";
...
pub(super) fn check_and_report_update_errors(ctx: &mut AppContext) {
    ...
    if has_unable_to_close {
        log::warn!("Windows auto-update log reported that setup could not close all applications");
    }
    ...
    .is_some();
    if has_file_in_use {
        log::warn!("Windows auto-update log reported a file-in-use error");
    }

    // Fired when the mutex polling loop timed out and a force-kill was attempted.
    let has_mutex_timeout = memchr::memmem::find(
        &contents_lowercase,
        b"ashide mutex still held after timeout",
    )
    .is_some();
    if has_mutex_timeout {
        log::warn!("Windows auto-update log reported a mutex timeout");
    }
    ...
    if has_forcekill_failed {
        log::warn!("Windows auto-update log reported a failed force-kill");
    }

    // Ashide OSS 不上传 autoupdate 失败日志,仅在本地记录错误计数;完整日志文件会被下方
    ...
    #[cfg(feature = "crash_reporting")]
    {
        ...
        if error_count > 0 {
            log::error!(
                "Ashide OSS: Windows auto-update log contains {error_count} error(s) (log: {:?})",
                log_path
            );
        }
        let _ = &contents;
...
pub(super) fn relaunch() -> Result<()> {
    ...
    if matches!(channel, Channel::Oss) {
        ...
        // The Inno Setup install wizard will run without user input. It will re-launch Ashide after
        ...
        cmd.args([
            ...
            &log_arg,
            "/update=1",
            // Do not forcibly kill Ashide via RestartManager. The installer will wait for
            // Ashide to exit naturally by polling the single-instance mutex instead.
            "/NOCLOSEAPPLICATIONS",
            &format!("/DIR={}", install_dir.display()),
        ]);
    }
...
fn installer_file_name() -> Result<String> {
    ...
    // For example, on arm64 this is AshideSetup-arm64.exe and on x64 this is
    // AshideSetup.exe.
