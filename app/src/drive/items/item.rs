...
use crate::{
    ...
    object_store::{model::persistence::ObjectStoreModel, StoredObject, StoredObjectMetadataExt},
};
...
use crate::{drive::items::LocalDriveItem, object_store::StoredObjectLocation};

use super::LocalDriveItemId;
...
struct LocalDriveItemStyles {
    // Height of each item
    item_height: f32,
    /// Default styles of the LocalDriveItem
...
impl LocalDriveItemStyles {
    ...
    fn default(appearance: &Appearance) -> LocalDriveItemStyles {
        let theme = appearance.theme();
        let item_height = ITEM_FONT_SIZE * 2.0 - ITEM_MARGIN_BOTTOM;
        let background = theme.background();
        LocalDriveItemStyles {
            item_height,
...
/// A UI wrapper around a row in Ashide Drive that holds important UI state for the row and implements
/// a unified look for all rows in Ashide Drive, like padding and hover states.
///
/// The item-specific information like icon, name, click_action, and preview modal are abstracted as much as
/// possible into the LocalDriveType enum.
pub struct LocalDriveRow<'a> {
    item: Box<dyn LocalDriveItem>,
    space: Space,
...
impl<'a> LocalDriveRow<'a> {
    ...
    pub fn new(
        ...
        let local_drive_item_id = item.local_drive_id();
        ...
        Some(Self {
            ...
            folder_depth,
            can_move,
            styles: LocalDriveItemStyles::default(appearance),
            menu_open,
            share_dialog_open,
            is_selected,
            is_focused,
    ...
    pub fn new_from_stored_object(
        ...
        let item = object.to_local_drive_item(appearance)?;
        Self::new(
            item,
            item_states,
            space,
    ...
    /// Returns a Box<dyn Element> representing a displayable view of this object-store object history, including
    ...
    fn render_secondary_icon(&self, style: UiComponentStyles) -> Box<dyn Element> {
        let icon_to_render = match self.item.local_drive_id() {
            LocalDriveItemId::Object(ObjectTypeAndId::Folder(_)) => self
                .item
    ...
    pub fn render_item(&self, style: UiComponentStyles) -> Box<dyn Element> {
        ...
        let local_drive_item_id = self.item.local_drive_id();
        match local_drive_item_id {
            LocalDriveItemId::Object(_)
...
/// Generate a callback for calculating the Drag bounds within Ashide Drive
fn drag_bounds_callback() -> impl Fn(&PositionCache, Vector2F) -> Option<RectF> {
    move |position_cache, window: Vector2F| {
        let drive_index = position_cache.get_position(LOCAL_DRIVE_POSITION_ID)?;
...
impl UiComponent for LocalDriveRow<'_> {
    ...
    fn build(self) -> Self::ElementType {
        ...
        match self.item.local_drive_id() {
            LocalDriveItemId::Object(item) => {
                ...
                SavePosition::new(
                    save_position_child,
                    &self.item.local_drive_id().drive_row_position_id(),
                )
            }
            LocalDriveItemId::AIFactCollection | LocalDriveItemId::MCPServerCollection => {
                SavePosition::new(
                    hoverable_item,
                    &self.item.local_drive_id().drive_row_position_id(),
                )
