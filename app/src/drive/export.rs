...
#[cfg(feature = "local_fs")]
use crate::{
    notebooks::export_notebook, object_store::update_manager::get_duplicate_object_name,
...
/// Singleton model for exporting from Ashide Drive.
...
impl ExportManager {
    ...
    pub fn export(
        ...
        for object in objects {
            match ObjectStoreModel::as_ref(ctx).get_by_uid(&object.uid()) {
                ...
                Some(stored_object) => {
                    let id = ExportId(*object, stored_object.space(ctx));
                    ids.push(id);
    ...
    fn export_one(
        ...
        let name = if name.is_empty() {
            crate::i18n::t_or("common-untitled", "Untitled")
        } else {
            safe_filename(&name)
        };
    ...
    pub fn display_name(self, ctx: &AppContext) -> Option<String> {
        ObjectStoreModel::as_ref(ctx)
            ...
            .map(|object| {
                ...
                if name.is_empty() {
                    name.push_str(&crate::i18n::t_or("common-untitled", "Untitled"))
                }
                name
            })
    }
