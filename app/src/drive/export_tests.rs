...
fn add_workflow(id: ObjectStoreId, workflow: Workflow, app: &mut App) {
...
fn test_export_workflow_success() {
    App::test((), |mut app| async move {
        ...
        let workflow_id = ObjectStoreId::StableId(WorkflowId::from(123).into());
...
fn test_export_notebook_with_embeds() {
    App::test((), |mut app| async move {
        ...
        let notebook_id = ObjectStoreId::StableId(NotebookId::from(456).into());
        add_notebook(
            notebook_id,
            "Test notebook",
            format!(
