...
use crate::{
    ...
    object_store::{model::persistence::ObjectStoreModel, Revision},
...
pub enum EnumCreationDialogEvent {
    ...
    /// The boolean value represents if the visibility of the enum changed (went from non-reusable to reusable
...
pub struct WorkflowEnumData {
    ...
    pub name: String,
    /// If the enum is reusable or not, used when determining if an enum should be displayed in the dropdown
...
pub struct EnumCreationDialog {
    ...
    /// The `object_store_id` of the enum in the dialog if it already exists,
    /// `None` if this is a new enum.
    object_store_id: Option<ObjectStoreId>,
    /// The revision timestamp of the enum, if it has already been persisted locally.
...
impl EnumCreationDialog {
    ...
    fn load_enum(
        ...
        self.object_store_id = Some(enum_id);
    ...
    pub fn load_from_data(
        ...
        self.load_enum(name, enum_id, is_reusable, enum_data, ctx);
    }

    // Load an enum from memory
    pub fn load_from_object_store_model(
        &mut self,
        enum_id: ObjectStoreId,
        ctx: &mut ViewContext<Self>,
    ) {
        ...
        if let Some(workflow_enum) = workflow_enum {
            self.load_enum(
                &workflow_enum.name,
                enum_id,
                workflow_enum.is_reusable,
                &workflow_enum.variants,
                ctx,
            );
        } else {
            // If we couldn't find an enum with this ObjectStoreId, open an empty dialog
            self.initialize(ctx);
        }
    }
        ...
        match self.object_store_id {
            ...
            None => {
                ctx.emit(EnumCreationDialogEvent::CreateEnum(WorkflowEnumData {
                    id: ObjectStoreId::ClientId(ClientId::default()),
                    name: self.name_editor.as_ref(ctx).buffer_text(ctx),
                    is_reusable: true,
                    revision_ts: self.revision_ts.clone(),
                    new_data: Some(variants),
    ...
    fn render_dialog_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        let text = match self.object_store_id {
    ...
    fn render_footer_buttons(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let save_button_label = match self.object_store_id {
