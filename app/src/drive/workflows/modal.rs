...
use crate::{
    ...
    drive::{
        items::LocalDriveItemId, local_drive_object_styling::local_drive_icon_color,
...
pub struct WorkflowModal {
    ...
    /// Only present if the workflow already exists in the local object store.
    ...
    clicked_breadcrumb: Option<LocalDriveItemId>,
...
impl WorkflowModal {
    ...
    fn populate(&mut self, workflow: Workflow, ctx: &mut ViewContext<Self>) {
        ...
        // Necessary since Ashide AI command search sometimes provides arguments in an invalid argument format.
    ...
    fn view_in_local_drive(&mut self, id: LocalDriveItemId, ctx: &mut ViewContext<Self>) {
        ctx.emit(WorkflowModalEvent::ViewInLocalDrive(id));
    ...
    fn menu_items(&self) -> Vec<MenuItem<WorkflowModalAction>> {
    ...
    fn handle_object_store_event(&mut self, event: &ObjectStoreEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            | ObjectStoreEvent::ObjectPermissionsUpdated { type_and_id, .. } => {
                // Update breadcrumbs if the workflow moved elsewhere or its reusable state changed.
                if let Some(workflow_id) = self.workflow_id {
        ...
        let workflow_icon = Container::new(
            ConstrainedBox::new(
                Icon::from(DriveObjectType::Workflow)
                    .to_warpui_icon(
                        local_drive_icon_color(appearance, DriveObjectType::Workflow).into(),
                    )
                    .finish(),
            )
            ...
            let rendered_breadcrumbs = breadcrumb::render_breadcrumbs(
                ...
                |ctx, _, object| {
                    let item_id = match object.kind {
                        ContainingObjectKind::Object(id) => LocalDriveItemId::Object(id),
                        ContainingObjectKind::Space(space) => LocalDriveItemId::Space(space),
                    };
                    ctx.dispatch_typed_action(WorkflowModalAction::ViewInLocalDrive(item_id));
                },
            );
        ...
        match action {
            ...
            WorkflowModalAction::ForceClose => {
                ...
                if let Some(id) = self.clicked_breadcrumb {
                    self.view_in_local_drive(id, ctx);
                }
            }
            WorkflowModalAction::AiAssist => self.issue_request(ctx),
            WorkflowModalAction::ViewInLocalDrive(id) => {
