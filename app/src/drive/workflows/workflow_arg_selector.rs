...
pub struct WorkflowArgSelector {
    ...
    all_workflow_enums: HashMap<ObjectStoreId, EnumMenuItem>,
...
impl WorkflowArgSelector {
    ...
    pub fn get_selected_enum(&self) -> Option<ObjectStoreId> {
        self.selected_enum
    }

    pub fn set_selected_enum(&mut self, id: Option<ObjectStoreId>, ctx: &mut ViewContext<Self>) {
        if id.is_some() {
    ...
    pub fn get_created_enums(&self) -> Vec<ObjectStoreId> {
        self.created_enums.clone()
    }

    pub fn set_workflow_enums(
        &mut self,
        workflow_enums: &HashMap<ObjectStoreId, WorkflowEnumData>,
        ctx: &mut ViewContext<Self>,
    ) {
        self.all_workflow_enums = workflow_enums
            .iter()
            .filter_map(|(id, enum_data)| {
                if enum_data.is_reusable || Some(*id) == self.get_selected_enum() {
    ...
    /// Used when an enum created in another row is edited to be "non-reusable".
    pub fn remove_enum_from_menu(&mut self, enum_id: &ObjectStoreId, ctx: &mut ViewContext<Self>) {
    ...
    fn edit_enum(&mut self, id: ObjectStoreId, ctx: &mut ViewContext<Self>) {
