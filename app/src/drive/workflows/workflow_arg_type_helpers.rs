...
/// Get all workflow enums in the space, filtering to only show the reusable ones
pub fn load_workflow_enums_with_owner<V>(
    owner: Owner,
    ctx: &mut warpui::ViewContext<V>,
) -> HashMap<ObjectStoreId, WorkflowEnumData>
where
    V: warpui::View,
{
    ...
    object_store_model
        ...
        .filter(|workflow_enum| workflow_enum.model().string_model.is_reusable)
        .map(|workflow_enum| {
            let enum_id = workflow_enum.id;
            let enum_data = WorkflowEnumData {
                name: workflow_enum.model().string_model.name.clone(),
                id: enum_id,
                is_reusable: workflow_enum.model().string_model.is_reusable,
...
pub fn load_argument_into_selector(
    ...
    argument: &Argument,
    all_workflow_enums: &mut HashMap<ObjectStoreId, WorkflowEnumData>,
    ...
    if let ArgumentType::Enum { enum_id } = argument.arg_type {
        ...
        if let Some(enum_data) = all_workflow_enums.get(&enum_id) {
            ...
            // Grab the revision_ts, enum name, and reusability status from the object store
            let object_store_model = ObjectStoreModel::as_ref(ctx);
            ...
            let enum_data = workflow_enum_model.map(|workflow_enum| {
                ...
                (workflow_enum.name.clone(), workflow_enum.is_reusable)
            });

            // If we found an enum in memory, add the enum to the global list
            if let Some((enum_name, is_reusable)) = enum_data {
...
/// Given arg type data and a ViewContext, saves the data represented by arg type data to the local object store.
pub fn save_enum<V>(
    ...
    V: warpui::View,
{
    ...
    match enum_data.id {
        ObjectStoreId::ClientId(client_id) => {
        ...
        ObjectStoreId::StableId(_) => {
            // Update the persisted local enum.
            UpdateManager::handle(ctx).update(ctx, |update_manager, ctx| {
    ...
    if let Some(ArgumentEditorRowIndex(index)) = pending_argument_editor_row {
        arguments_rows[*index]
            ...
            .update(ctx, |selector, ctx| {
                // Insert into the menu, which we might not have done earlier if the enum is not reusable
                if !enum_data.is_reusable {
    ...
    V: warpui::View,
{
    ...
    // Update the enum to each argument row's list, in case its name was updated, if it is reusable
    if enum_data.is_reusable {
        arguments_rows.iter().for_each(|row| {
            row.arg_type_editor().update(ctx, |editor, ctx| {
                editor.insert_enum_into_menu(enum_id, enum_name.clone(), ctx);
    ...
    // Otherwise, remove the enum from the dropdown list for every row if it is newly "non-reusable"
    else if !enum_data.is_reusable && did_reusability_change {
        arguments_rows.iter().for_each(|row| {
            row.arg_type_editor().update(ctx, |editor, ctx| {
    ...
    if let Some(ArgumentEditorRowIndex(index)) = pending_argument_editor_row {
        arguments_rows[*index]
            .arg_type_editor()
            .update(ctx, |selector, ctx| {
                // Insert into the menu, which we might have undone earlier if the enum is non-reusable
                if !enum_data.is_reusable {
                    selector.insert_enum_into_menu(enum_id, enum_name.clone(), ctx);
                }
            });
    ...
    match all_workflow_enums.get(id) {
        ...
        Some(WorkflowEnumData {
            ...
            enum_creation_dialog.update(ctx, |dialog, ctx| {
                dialog.load_from_data(name, *id, *is_reusable, new_data, ctx);
            });
            true
        }
