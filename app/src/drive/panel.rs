...
use crate::{
    ...
    object_store::{
        ...
        Owner, Space, StoredObjectEventEntrypoint,
    },
...
pub const LOCAL_DRIVE_POSITION_ID: &str = "local_drive";

/// The sidebar that houses Ashide Drive.
/// `DrivePanel` is different from `DriveIndex` in that it is responsible for
/// how Ashide Drive interacts with the workspace and the rest of the app, whereas
/// `DriveIndex` is the main Ashide Drive view and responsible for the internals of Ashide Drive.
...
pub enum DrivePanelAction {
    ...
    OpenSearch,
    /// Focus Local Drive panel (via single click)
    FocusDriveIndex,
}
...
impl DrivePanel {
    ...
    fn handle_index_view_event(&mut self, event: &DriveIndexEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            DriveIndexEvent::ExportObject(_stored_object_type_and_id) => {
                // No-op when no local filesystem.
            }
            DriveIndexEvent::RunObject(id) => {
                let object_store_model = ObjectStoreModel::as_ref(ctx);
                let object = object_store_model.get_by_uid(&id.uid());
                if let Some(stored_object) = object {
                    let workflow: Option<&WorkflowObject> = stored_object.into();
                    let env_var_collection: Option<&EnvVarCollectionObject> = stored_object.into();
                    if let Some(workflow) = workflow {
            ...
            DriveIndexEvent::FocusLocalDrive => ctx.emit(DrivePanelEvent::FocusLocalDrive),
    ...
    pub fn open_existing_notebook(&self, notebook_id: ObjectStoreId, ctx: &mut ViewContext<Self>) {
    ...
    pub fn open_object_dialog(
        ...
        self.index_view.update(ctx, |index_view, ctx| {
            index_view.handle_action(
                &DriveIndexAction::create_object(stored_object_type, space, initial_folder_id),
                ctx,
            )
        });
        ctx.notify();
    ...
    /// Recomputes and initializes the section states for the Local Drive index. This is needed after
    ...
    /// This functions scrolls the relevant Ashide Drive item into view.
    ...
    /// This functions sets the index of a focused Ashide Drive item.
    ...
    pub fn set_focused_item(&mut self, item_id: LocalDriveItemId, ctx: &mut ViewContext<Self>) {
        self.index_view.update(ctx, |index, ctx| {
    ...
    pub fn reset_focused_index_in_local_drive(
        ...
        self.index_view.update(ctx, |index, ctx| {
            index.reset_focused_index_in_local_drive(should_scroll, ctx);
        })
    }
    ...
    fn handle_action(&mut self, action: &DrivePanelAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            DrivePanelAction::FocusDriveIndex => {
                ctx.focus(&self.index_view);
                // should_scroll is set to false here in order to not let menu clicks autoscroll Local Drive
