use crate::{
    drive::{local_drive_object_styling::local_drive_icon_color, DriveObjectType},
...
use crate::{
    appearance::Appearance, notebooks::file::is_markdown_file, object_store::ids::ClientId,
...
pub(super) struct FolderNode {
    parent_id: FolderId,
    local_object_client_id: ClientId,
    name: String,
    children: Vec<ImportedNode>,
    object_uid: Option<String>,
    all_children_saved_locally: bool,
    status: ImportStatus,
...
impl FolderNode {
    fn new(name: String, parent_id: FolderId) -> Self {
        Self {
            name,
            parent_id,
            local_object_client_id: ClientId::new(),
            children: Vec::new(),
            object_uid: None,
            all_children_saved_locally: false,
    ...
    pub(super) fn local_object_client_id(&self) -> ClientId {
        self.local_object_client_id
    }
    ...
    fn render(
        ...
        let icon_color =
            override_color.unwrap_or(local_drive_icon_color(appearance, DriveObjectType::Folder));
        ...
        let folder_row = match &self.status {
            ImportStatus::Loaded(object_uid) if allow_click_to_open_target => {
...
impl ImportStatus {
    ...
    fn render_status_icon(&self, appearance: &Appearance) -> Box<dyn Element> {
        let status_icon_element = match &self {
            ImportStatus::SavedLocally => Icon::Laptop
                .to_warpui_icon(
                    appearance
                        .theme()
                        .sub_text_color(appearance.theme().surface_1()),
                ...
                .finish(),
            ImportStatus::Loading => Icon::Refresh
                .to_warpui_icon(
                    appearance
                        .theme()
                        .sub_text_color(appearance.theme().surface_1()),
                ...
                .finish(),
            ImportStatus::Loaded(_) => Icon::Check
                .to_warpui_icon(Fill::Solid(ColorU::new(11, 142, 71, 255)))
                .finish(),
            ImportStatus::Error(_) => Icon::AlertTriangle
...
impl FileNode {
    ...
    pub(super) fn saved_locally(&mut self) {
        if !self.status.is_loaded() {
            self.status = ImportStatus::SavedLocally;
        }
    }
        ...
        let icon_element = match &self.file_type {
            FileType::Workflow => Icon::Workflow
                .to_warpui_icon(Fill::Solid(override_color.unwrap_or(
                    local_drive_icon_color(appearance, DriveObjectType::Workflow),
                )))
                .finish(),
...
pub(super) async fn expand_dirs(dirs: Vec<PathBuf>) -> FileImportState {
    ...
    FileImportState::new(folder_id_to_node, file_id_to_node)
}

pub(super) struct FileImportState {
...
impl FileImportState {
    ...
    // Get the local object client id for the provided folder id. Returns None
    // if the folder is the root node folder.
    pub(super) fn folder_local_client_id(&self, folder_id: FolderId) -> Option<ClientId> {
        if folder_id == FolderId::root_id() {
            // Root folder should not have a client id.
            return None;
        }
        match self.folder_id_to_node.get(&folder_id) {
            Some(folder_node) => Some(folder_node.local_object_client_id()),
            None => {
    ...
    pub(super) fn file_name_and_parent_local_client_id(
        &self,
        file_id: FileId,
    ) -> Option<(String, Option<ClientId>)> {
        let file_node = self.file_id_to_node.get(&file_id)?;
        let parent_client_id = self.folder_local_client_id(file_node.parent_id);
        Some((file_node.name.clone(), parent_client_id))
    }

    pub(super) fn mark_folder_saved_locally(&mut self, result: ImportResult, folder_id: FolderId) {
        let parent_id = if let Some(folder) = self.folder_id_to_node.get_mut(&folder_id) {
            let should_update_upstream_folders = match result {
                // If creating the folder is not successful, its children will not import.
                // Mark the folder as errored and update upstream folders.
                ImportResult::Error(e) => {
                    folder.status = ImportStatus::Error(e);
                    true
                }
                // If the folder has no children, mark the folder as completed and update
                // upstream folders.
                ImportResult::Success(object_uid) => {
                    folder.object_uid = Some(object_uid.clone());

                    // If a folder has no children or all of its children have been saved locally,
                    // we need to bubble the state up in the folder hierarchy tree.
                    if folder.children().is_empty() || folder.all_children_saved_locally {
                        folder.status = ImportStatus::Loaded(object_uid);
                        true
                    } else {
                        false
                    }
    ...
    pub(super) fn set_file_and_parent_to_loading(&mut self, file_id: FileId) {
        let parent_id = match self.file_id_to_node.get_mut(&file_id) {
            Some(file_node) => {
                file_node.status = ImportStatus::Loading;
    ...
    /// File import is completed if the root node is marked as complete.
        ...
        while let Some(folder_node) = self.folder_id_to_node.get(&next_folder_to_update) {
            ...
            if folder_node.status.is_loaded() {
                let folder_node = self
                    ...
                    .expect("Should exist");
                folder_node.all_children_saved_locally = false;
                folder_node.status = ImportStatus::Loading
            } else {
                break;
            }
        ...
        while let Some(folder_node) = self.folder_id_to_node.get(&next_folder_to_update) {
                ...
                && folder_node.are_children_loaded(&self.folder_id_to_node, &self.file_id_to_node)
            {
                let folder_node = self
                    ...
                    .get_mut(&next_folder_to_update)
                    .expect("Should exist");
                folder_node.all_children_saved_locally = true;

                if let Some(id) = &folder_node.object_uid {
                    folder_node.status = ImportStatus::Loaded(id.clone());
                } else if folder_is_root_node {
                    folder_node.status = ImportStatus::Loaded(String::new());
                } else {
                    // Normally a folder should have been created for a file to be imported.
                    ...
                    // the folder is not created but all of its children complete.
                    break;
                }
            } else {
                break;
    ...
    pub(super) fn update_tree_with_file_import_result(
        &mut self,
        result: ImportResult,
        file_id: FileId,
    ) -> bool {
        ...
        file_node_to_update.status = match result {
            ImportResult::Success(id) => ImportStatus::Loaded(id),
            ImportResult::Error(e) => ImportStatus::Error(format!("Failed to parse file: {e}")),
        };
