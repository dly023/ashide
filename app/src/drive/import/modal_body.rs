...
use super::{
    modal::BODY_HEIGHT,
    nodes::{
        expand_dirs, parse_file, FileContent, FileId, FileImportState, FolderId, ImportResult,
    },
...
/// 4. Insert all folders we need to create into the import queue
...
pub struct ImportModalBody {
    ...
    // Queue local import object creation so parent folders exist before children.
...
impl ImportModalBody {
        ...
        if let ImportState::PathExpanded(state) = &mut self.state {
            match event {
                ImportQueueEvent::FileCompleted {
                    ...
                    if state.update_tree_with_file_import_result(
                        ImportResult::Success(object_uid.clone()),
                        *file_id,
                    ) {
                        ctx.notify();
                    }
                ...
                ImportQueueEvent::FolderCompleted {
                    ...
                    state.mark_folder_saved_locally(
                        ImportResult::Success(object_uid.clone()),
                        *folder_id,
                    );
                    ctx.notify();
            ...
            if state.all_files_saved_locally() {
                ...
                ctx.emit(ImportModalBodyEvent::ImportCompleted);
            }
        }
    }

    pub fn set_new_target(&mut self, owner: Owner, initial_folder_id: Option<ObjectStoreId>) {
        // TODO: this should take an owner OR folder.
        self.owner = Some(owner);
        self.initial_folder_id = initial_folder_id;
    }

    // Push a new local import update to the queue.
    fn push_new_update(&mut self, arg: ImportQueueArgs, ctx: &mut ViewContext<Self>) {
        self.import_queue
            .update(ctx, |queue, ctx| queue.enqueue(arg, ctx));
    }

    // Whether there is an active import in progress (If all imports are completed,
    // we don't consider the import modal import to be in progress).
    pub fn import_in_progress(&self, app: &AppContext) -> bool {
        let _ = app;

        match &self.state {
            ImportState::PickFiles => false,
    ...
    pub fn before_import(&self) -> bool {
        matches!(&self.state, ImportState::PickFiles | ImportState::Loading)
    }

    fn parent_id_for_import(&self, parent_folder_client_id: Option<ClientId>) -> ParentId {
        match parent_folder_client_id {
            Some(id) => ParentId::PendingFolder(id),
            None => ParentId::InitialFolder(self.initial_folder_id),
        }
    }

    /// Queue creation of local folder objects for the expanded import tree.
    pub(super) fn populate_folder_objects(
        &mut self,
        state: &mut FileImportState,
        ...
        // Push all folders to the queue in order so child imports can depend on
        // their parent folder's local ObjectStoreId.
        while let Some(node) = state.folder_id_to_node.get(&id) {
            ...
            let parent_folder_client_id = state.folder_local_client_id(parent_id);
            self.push_new_update(
                ImportQueueArgs {
                    owner,
                    parent_id: self.parent_id_for_import(parent_folder_client_id),
                    content: RequestContent::Folder {
                        name: node.name(),
                        client_id: node.local_object_client_id(),
                        folder_id: id,
                    },
                },
                ctx,
    ...
    /// Parse the next file that has not been imported. We determine the next file by iteratively
    /// by adding 1 to the previous imported file id.
    fn parse_next_file(
        &mut self,
        file_id: FileId,
                ...
                let metadata = match &mut view.state {
                    ImportState::PathExpanded(state) => {
                        ...
                        if let Err(e) = &response {
                            state.update_tree_with_file_import_result(
                                ImportResult::Error(e.to_string()),
                                file_id,
                            );
                        }

                        state.file_name_and_parent_local_client_id(file_id)
                    }
                    _ => None,
                };

                let Some((file_name, parent_client_id)) = metadata else {
                    return;
                };
                ...
                match response {
                    Ok(FileContent::Notebook(data)) => {
                        ...
                        view.push_new_update(
                            ImportQueueArgs {
                                owner,
                                parent_id: view.parent_id_for_import(parent_client_id),
    ...
    pub fn reset(&mut self, ctx: &mut ViewContext<Self>) {
        self.state = ImportState::PickFiles;
    ...
    fn render_picker_state(&self, appearance: &Appearance) -> Box<dyn Element> {
    ...
    fn render_loaded_state(
        &self,
        file_import_state: &FileImportState,
        appearance: &Appearance,
    ) -> Box<dyn Element> {
        let mut column = Flex::column();
        let folder_id_to_node = &file_import_state.folder_id_to_node;
        let file_id_to_node = &file_import_state.file_id_to_node;
...
impl View for ImportModalBody {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        match &self.state {
            ImportState::PickFiles | ImportState::Loading | ImportState::PathLoaded => {
                self.render_picker_state(appearance)
            }
            ImportState::PathExpanded(paths) => self.render_loaded_state(paths, appearance),
        }
    }
}
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            ImportModalBodyAction::FilePickerCancelled => {
                self.state = ImportState::PickFiles;
                ctx.emit(ImportModalBodyEvent::ImportSelected);
                ctx.notify();
            }
            ...
            ImportModalBodyAction::PathsSelected(paths) => {
                ...
                let handle = ctx.spawn(
                    ...
                    move |view, mut import_state, ctx| {
                        view.populate_folder_objects(&mut import_state, ctx);
                        view.state = ImportState::PathExpanded(import_state);
