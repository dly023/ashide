...
use crate::drive::import::nodes::{ImportResult, ImportStatus};

use super::{FileId, FileImportState, FileNode, FileType, FolderId, FolderNode, ImportedNode};

fn mock_tree() -> FileImportState {
...
fn test_state_update_in_tree() {
    ...
    state.mark_folder_saved_locally(
        ImportResult::Success("mock-folder".to_string()),
        FolderId(1),
    );
    ...
    state.update_tree_with_file_import_result(
        ImportResult::Success("mock-markdown".to_string()),
        FileId(1),
    );
    assert_eq!(
        state
            ...
            .expect("Should exist")
            .status,
        ImportStatus::Loaded("mock-folder".to_string())
    );
    ...
    state.update_tree_with_file_import_result(
        ImportResult::Success("mock-root".to_string()),
        FileId(0),
    );
    assert_eq!(
        state
            ...
            .expect("Should exist")
            .status,
        ImportStatus::Loaded(String::new())
    );
...
fn test_empty_folders_update() {
    ...
    // Errored imports should also be considered complete imports.
    state.mark_folder_saved_locally(ImportResult::Error("Failure".to_string()), FolderId(2));
    assert_eq!(
        state
            .folder_id_to_node
            .get(&FolderId(0))
