use crate::{
    ...
    object_store::{model::persistence::ObjectStoreModel, Owner, StoredObject},
...
impl ImportModal {
    ...
    pub fn open_file_picker(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let file_picker_config = FilePickerConfiguration::new()
            .allow_multi_select()
            .allow_folder()
    ...
    fn handle_import_body_event(
        ...
        match event {
            ...
            ImportModalBodyEvent::ImportCompleted
            | ImportModalBodyEvent::AllFileSavedLocally
            | ImportModalBodyEvent::ImportSelected => ctx.notify(),
        }
    }
    ...
    fn render_footer(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let button_text = if !self.import_modal.as_ref(app).import_in_progress(app) {
...
impl View for ImportModal {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if !self.import_modal.as_ref(app).before_import() {
