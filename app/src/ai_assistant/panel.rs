...
use crate::terminal::resizable_data::{ModalType, ResizableData, DEFAULT_AI_ASSISTANT_WIDTH};
...
use super::execution_context::AiExecutionContext;
...
const ZERO_STATE_HELP_TEXT: &str =
    "Shift + ctrl + space a block or text selection to ask Ashide AI.";
...
impl AIAssistantPanelView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let resizable_state_handle = match resizable_data_handle
            .as_ref(ctx)
            .get_handle(ctx.window_id(), ModalType::AiAssistantWidth)
        {
            Some(handle) => handle,
            None => {
                log::error!("Couldn't retrieve legacy AI resizable state handle.");
                resizable_state_handle(DEFAULT_AI_ASSISTANT_WIDTH)
            }
        };
