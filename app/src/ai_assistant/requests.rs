...
/// Tracks the current request status for legacy Ashide AI requests.
...
impl Requests {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        // Ashide 不接 hosted AI Assistant 后端,初始 request_limit_info 不再向服务端拉取;
    ...
    /// Records a legacy Ashide AI request and returns the local disabled response.
    pub fn issue_request(&mut self, request: String, ctx: &mut ModelContext<Self>) {
        ...
        let response = "Legacy Ashide AI Assistant requests are disabled in Ashide. Use Agent Mode with a configured BYOP model instead.".to_owned();
