...
fn make_new_view_menu(ctx: &AppContext) -> Menu {
    let mut items = vec![
        updateable_custom_item_without_checkmark(CustomAction::ToggleLocalDrive, ctx),
        MenuItem::Separator,
...
fn make_new_drive_menu(ctx: &AppContext) -> Menu {
    ...
    // Ashide Phase 2a: removed `Share pane contents` menu item (cloud
...
fn debug_menu_items() -> Vec<MenuItem> {
    ...
    if FeatureFlag::DebugMode.is_enabled() {
        ...
        // Ashide Wave 3-1:"Create anonymous user" debug menu item 随
...
/// Dispatch event to open a new Ashide window
