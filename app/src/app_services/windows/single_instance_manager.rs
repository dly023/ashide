...
pub(super) fn uri_named_pipe_name() -> String {
    format!("Ashide{:?}_URI_CHANNEL", ChannelState::channel())
}
...
fn try_create_mutex() -> Result<Option<MutexHandle>, Error> {
    ...
    // `script/windows/windows-installer.iss`, which the installer uses to detect whether Ashide is
    // running.
    let name = format!("Local\\Ashide{:?}_SingleInstance", ChannelState::channel())
        .encode_utf16()
...
/// A singleton model that is responsible for ensuring there is only one instance of Ashide running.
...
impl SingleInstanceManager {
    /// Attempts to upgrade the current Ashide instance to the "main" instance (i.e. the one that
    ...
    /// Returns whether or not this process should be treated as the main instance of Ashide.
    ///
