...
pub enum StartupArgsForwardingError {
    /// There's no instance of Ashide already running.
...
impl From<zbus::fdo::Error> for StartupArgsForwardingError {
    fn from(value: zbus::fdo::Error) -> Self {
        ...
        // registering a service, so this really means that Ashide is not already
        // running.
...
// A D-Bus client for connecting to an already-running instance of Ashide and
// invoking org.freedesktop.Application IPC methods.
//
// No `default_service` / `default_path` are declared: the proxy is always
// constructed via `ExistingApplicationProxy::builder(&conn)` with an explicit
// `.destination(well_known_name())` / `.path(application_service_path())`
// (see `pass_startup_args_to_existing_instance`), and the path is derived at
// runtime from the app id (`/dev/ashide/Ashide` for the OSS default). Hardcoded
// defaults would only be dead constants that could mislead a future
// `Proxy::new` caller, so they are intentionally omitted.
#[proxy(
