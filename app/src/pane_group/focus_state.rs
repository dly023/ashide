...
impl PaneFocusHandle {
    ...
    /// Commits focus already established by the application focus system.
    /// This updates only the canonical pane-group model and never moves UI focus.
    pub fn commit_application_focus(&self, ctx: &mut AppContext) {
        self.focus_state.update(ctx, |focus_state, ctx| {
            focus_state.set_focused_pane(self.pane_id, ctx);
            focus_state.set_active_session(self.pane_id.as_terminal_pane_id(), ctx);
        });
    }
