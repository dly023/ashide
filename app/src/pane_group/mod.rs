...
pub use pane::environment_runtime_placeholder_pane::EnvironmentRuntimePlaceholderPane;
...
///    MSYS2/WSL on Windows) that a raw `PATH` lookup would miss when Ashide is
...
// Preferred width for a newly created Agent Mode pane so that it is legible. Small windows may
// reduce it only as far as needed to preserve the framework minimum extent of every existing pane.
pub const AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH: f32 = 400.;

/// Shared placement and sizing contract for split-pane Agent entry.
///
/// Runtime materialization timing may differ, but local panes, connected-runtime panes, and
/// connecting placeholders must all consume this same intent.
#[derive(Debug, Clone, Copy)]
pub(crate) struct AgentSplitLayoutIntent {
    pub(crate) direction: Direction,
    minimum_width: f32,
}

impl Default for AgentSplitLayoutIntent {
    fn default() -> Self {
        Self {
            direction: Direction::Right,
            minimum_width: AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH,
        }
    }
}
...
pub enum Event {
    ...
    OpenObjectStoreWorkflowForEdit(ObjectStoreId),
    ...
    /// tell the workspace to open a file within Ashide.
    OpenFileInWarp {
    ...
    MaximizePaneToggled,
    /// An environment runtime resolved the repo root for a session in this pane group.
    EnvironmentRepoNavigated {
    ...
    TerminalSessionBootstrapped {
        pane_id: PaneId,
    },
    TerminalSessionBootstrapFailed {
        pane_id: PaneId,
    },
    FocusPaneGroup,
    FocusPane {
    ...
    /// A pane requested a new agent pane. Workspace owns Environment routing,
    /// so PaneGroup must surface this as an intent instead of spawning a
    /// terminal-bootstrap pane directly.
    NewPaneInAIMode {
    ...
    LocalDriveObjectInPane(ObjectUid),
    ...
    /// An environment terminal pane has initiated a file upload
    /// using a current-app session.
    FileUploadCommand {
        upload_id: FileUploadId,
        command: String,
        environment_pane_id: TerminalPaneId,
        current_app_pane_id: TerminalPaneId,
    },
    /// A terminal pane managing a file upload is requesting a password.
    FileUploadPasswordPending {
        current_app_pane_id: TerminalPaneId,
    },
    /// A terminal pane managing a file upload has completed its task.
    FileUploadFinished {
        current_app_pane_id: TerminalPaneId,
        exit_code: ExitCode,
    },
    ...
    /// Ashide:在终端里 Ctrl/Cmd+点击环境会话输出中的文件路径时发出,
    /// 由 workspace 走 buffer-sync 协议在编辑器中打开环境文件。
    #[cfg(all(feature = "local_tty", feature = "local_fs"))]
    OpenEnvironmentFileFromTerminal {
...
/// A terminal process to spawn directly, without first launching the current-app shell bootstrap.
#[derive(Debug, Clone)]
pub struct DirectTerminalProcess {
    pub executable_path: PathBuf,
    pub args: Vec<OsString>,
    pub shell_type: ShellType,
}

pub type EnvironmentRuntimePtyClient =
    crate::workspace::environment_runtime::EnvironmentRuntimeClient;

#[derive(Debug, Clone)]
pub struct EnvironmentRuntimePtyProcess {
    pub client: Arc<EnvironmentRuntimePtyClient>,
...
/// Options that can be set when adding a new terminal pane.
#[derive(Debug, Default, Clone)]
pub struct NewTerminalOptions {
    /// The particular shell to spawn (if not the default).
    pub shell: Option<AvailableShell>,
    /// A process to spawn directly instead of bootstrapping the current-app shell.
    pub direct_process: Option<DirectTerminalProcess>,
    /// A environment-runtime native PTY to create inside an already connected environment runtime.
    pub environment_runtime_pty: Option<EnvironmentRuntimePtyProcess>,
...
struct AddPaneOptions {
    ...
    agent_split_layout: Option<AgentSplitLayoutIntent>,
}
...
impl NewTerminalOptions {
    ...
    pub fn with_direct_process(mut self, direct_process: DirectTerminalProcess) -> Self {
        self.direct_process = Some(direct_process);
        self
    }

    pub fn with_environment_runtime_pty(
        mut self,
        environment_runtime_pty: EnvironmentRuntimePtyProcess,
    ) -> Self {
        self.environment_runtime_pty = Some(environment_runtime_pty);
        self
    }
}

#[derive(Debug)]
pub struct EnvironmentRuntimePaneTemplate {
    pub options: NewTerminalOptions,
...
#[derive(Debug)]
pub enum EnvironmentRuntimePaneTemplateNode {
    Pane(EnvironmentRuntimePaneTemplate),
    Branch {
        split_direction: launch_config::SplitDirection,
        panes: Vec<EnvironmentRuntimePaneTemplateNode>,
    },
}
...
pub enum PanesLayout {
    ...
    EnvironmentRuntimeTemplate(EnvironmentRuntimePaneTemplateNode),
...
impl PanesLayout {
    pub fn is_terminal_bootstrap(&self) -> bool {
        matches!(self, Self::SingleTerminal(_))
    }

    pub fn is_snapshot_restoration(&self) -> bool {
...
#[derive(Clone, Copy)]
enum PaneReplacementFocus {
    Replacement,
    FollowOriginal,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum TerminalPaneReplacementPhase {
    Prepared { terminal_pane_id: PaneId },
    RolledBack { terminal_pane_id: PaneId },
}
...
impl PaneGroup {
    ...
    fn pane_tree_from_template_recursive(
        ...
        match root {
            PaneTemplateType::PaneTemplate {
                ...
                if !commands.is_empty() && !matches!(pane_mode, PaneMode::AmbientAgent) {
    ...
    #[allow(clippy::too_many_arguments)]
    fn pane_tree_from_environment_runtime_template(
        root: EnvironmentRuntimePaneTemplateNode,
        resources: TerminalViewResources,
        ctx: &mut ViewContext<Self>,
        pane_contents: &mut HashMap<PaneId, Box<dyn AnyPaneContent>>,
        is_left_pane: bool,
        ...
        let (_, pane_data, focus) = Self::pane_tree_from_environment_runtime_template_recursive(
            root,
            resources,
            ctx,
            pane_contents,
    ...
    #[allow(clippy::too_many_arguments)]
    fn pane_tree_from_environment_runtime_template_recursive(
        root: EnvironmentRuntimePaneTemplateNode,
        resources: TerminalViewResources,
        ctx: &mut ViewContext<Self>,
        pane_contents: &mut HashMap<PaneId, Box<dyn AnyPaneContent>>,
        ...
        match root {
            EnvironmentRuntimePaneTemplateNode::Pane(template) => {
                let (view, terminal_manager) = PaneGroup::create_session(
                    template.options.initial_directory,
                    template.options.env_vars,
                    template.options.is_shared_session_creator,
                    resources,
                    None,
                    template.options.conversation_restoration,
                    user_default_shell_unsupported_banner_model_handle,
                    view_size,
                    model_event_sender.clone(),
                    template.options.shell,
                    template.options.direct_process,
                    template.options.environment_runtime_pty,
                    None,
                    ctx,
                );

                if !template.commands.is_empty()
                    && !matches!(template.pane_mode, PaneMode::AmbientAgent)
                {
                    let exec = template.commands.iter().map(String::as_str).join(" && ");
                    view.update(ctx, |terminal, ctx| {
                        terminal.set_pending_command(exec.as_str(), ctx);
                    });
                }

                if matches!(template.pane_mode, PaneMode::Agent) {
                    if template.commands.is_empty() {
                        view.update(ctx, |terminal_view, ctx| {
                            terminal_view.enter_agent_view_for_new_conversation(
                                None,
                ...
                let focus = InitialFocus {
                    focused_pane: template.is_focused.then_some(pane_id),
                    active_session: template.is_focused.then_some(terminal_pane_id),
                };
                let leftmost_pane_id = is_left_pane.then_some(LeftmostPaneId {
                    pane_id,
                    session_id: terminal_pane_id,
            ...
            EnvironmentRuntimePaneTemplateNode::Branch {
                split_direction,
                panes,
            } => {
                let mut len = 0;
                ...
                for (idx, node) in panes.into_iter().enumerate() {
                    let (child_leftmost_pane_id, child, child_focus) =
                        Self::pane_tree_from_environment_runtime_template_recursive(
                            node,
                            resources.clone(),
                            ctx,
                            pane_contents,
    ...
    fn restore_pane_leaf(
        ...
        let session_binding = leaf.session_binding.clone();
        ...
        let result = match leaf.contents {
            ...
            LeafContents::Terminal(terminal_snapshot) => {
                ...
                if let Some(active_profile_object_store_id) = &terminal_snapshot.active_profile_id {
                    log::info!(
                        "Attempting to restore active_profile '{active_profile_object_store_id}' for terminal {terminal_view_id:?}"
                    );

                    let profiles_model = AIExecutionProfilesModel::as_ref(ctx);

                    if let Some(profile_id) = profiles_model
                        .get_profile_id_by_object_store_id(active_profile_object_store_id)
                    {
            ...
            LeafContents::Notebook(snapshot) => {
                let pane: Box<dyn AnyPaneContent + 'static> = match snapshot {
                    ...
                    NotebookPaneSnapshot::CurrentAppFileNotebook { path } => {
                        Box::new(FilePane::new(
                            path,
                            None,
                            #[cfg(feature = "local_fs")]
                            None,
            ...
            LeafContents::ProviderConnection { .. } => {
                // Provider connection editor panes are intentionally not restored —
                // they're transient editor surfaces over the persistent
                // provider connection store. Users reopen via the Environment
                // provider manager tree in the left panel.
                Err(anyhow::anyhow!(
                    "Provider connection pane should not have been persisted, as it cannot be restored"
                ))
            }
            LeafContents::ProviderFileBrowser { .. } => {
                // Provider file browser panes are transient because environment
                // file-system access depends on an active provider connection.
                Err(anyhow::anyhow!(
                    "Provider file browser pane should not have been persisted, as it cannot be restored"
                ))
            }
            ...
            LeafContents::EnvironmentRuntimePlaceholder => {
                let pane: Box<dyn AnyPaneContent + 'static> =
                    Box::new(EnvironmentRuntimePlaceholderPane::new(ctx));
                let pane_id = pane.as_pane().id();
                pane_contents.insert(pane_id, pane);
                let focus = InitialFocus {
                    focused_pane: leaf.is_focused.then_some(pane_id),
                ...
                if let Some(pane) = pane_contents.get(pane_id) {
                    pane.as_pane()
                        ...
                        .update(ctx, |configuration, ctx| {
                            ...
                            configuration.restore_session_binding(session_binding.clone());
        ...
        match node {
            ...
            PaneNode::Leaf(pane_id) => {
                let pane_configuration = self
                    ...
                    .as_pane();
                let contents = pane_configuration.snapshot(app);
                let pane_configuration = pane_configuration.pane_configuration();
                ...
                PaneNodeSnapshot::Leaf(LeafSnapshot {
                    ...
                    session_binding: pane_configuration.session_binding(),
    ...
    pub(crate) fn restore_session_binding_for_pane_id(
        &self,
        pane_id: PaneId,
        ...
        pane.as_pane()
            .pane_configuration()
            .as_ref(app)
            .restore_session_binding(binding);
        true
    }

    pub(crate) fn session_binding_for_pane_id(
        &self,
        pane_id: PaneId,
        app: &AppContext,
    ) -> Option<PaneSessionBinding> {
        self.pane_contents
            .get(&pane_id)?
            .as_pane()
            .pane_configuration()
    ...
    /// Resolves the current terminal runtime carrier for a stable pane container.
    ///
    /// Session Navigator render-only runtime projections (for example CLI agent
    /// activity) must enter through this stable identity, never a tab/leaf
    /// locator or a persisted terminal EntityId.
    pub(crate) fn terminal_view_id_for_container_uuid(
        &self,
        container_uuid: &[u8],
        app: &AppContext,
    ) -> Option<EntityId> {
            ...
            (pane
                ...
                .then(|| {
                    self.downcast_pane_by_id::<TerminalPane>(*pane_id)
                        .map(|terminal_pane| terminal_pane.terminal_view(app).id())
                })
                .flatten()
        })
    }
    ...
    pub(crate) fn refresh_session_binding_for_terminal_view(
        &self,
        terminal_view_id: EntityId,
        ctx: &AppContext,
    ) -> bool {
        ...
        pane.refresh_session_binding_from_cli_agent_state(ctx);
        true
    }
    ...
    /// is used for intra-session prompt switching between Ashide prompt and PS1.
            ...
            Banner::<PaneGroupAction>::new_permanently_dismissible(
                BannerTextContent::formatted_text(vec![
                    FormattedTextFragment::plain_text(
                        "Ashide doesn't currently support your default shell, falling back to zsh.  ",
                    ),
                    ...
                    let loaded =
                        self.terminal_view_from_pane_id(pane_id, ctx)
                            .is_some_and(|target_view| {
                                Self::load_transcript_from_history(
                                    target_view,
                                    conversation_id,
                                    ambient_agent_task_id,
                                    ctx,
    ...
    /// Loads conversation data from local history into the given transcript viewer.
    ///
    ...
    fn load_transcript_from_history(
    ...
    fn initial_single_terminal_pane(
        ...
        let (view, terminal_manager) = PaneGroup::create_session(
            ...
            model_event_sender.clone(),
            options.shell,
            options.direct_process,
            options.environment_runtime_pty,
            None,
            ctx,
        );
        let uuid = Uuid::new_v4();
    ...
    /// Initial layout for an environment runtime tab while the Environment Runtime is
    /// resolving, connecting, or connected without a pending terminal intent.
    /// This intentionally is not a TerminalPane: until Workspace materializes a real
    /// runtime PTY, it must not appear as an active terminal session or render the
    /// conversation-transcript loading UI.
    fn initial_environment_runtime_placeholder_pane(
        pane_contents: &mut HashMap<PaneId, Box<dyn AnyPaneContent>>,
        pane_history: &mut Vec<PaneId>,
        ctx: &mut ViewContext<Self>,
    ) -> (PaneData, InitialFocus) {
        let pane = EnvironmentRuntimePlaceholderPane::new(ctx);
        let pane_id = pane.id();
        pane_contents.insert(pane_id, Box::new(pane));
        pane_history.push(pane_id);
        let focus = InitialFocus {
        ...
        let initial_layout = move |resources,
            ...
            match panes_layout {
                ...
                PanesLayout::EnvironmentRuntimeTemplate(template) => {
                    Self::pane_tree_from_environment_runtime_template(
                        template,
                        resources,
                        ctx,
                        pane_contents,
                ...
                PanesLayout::EnvironmentRuntimePlaceholder => {
                    Self::initial_environment_runtime_placeholder_pane(
                        pane_contents,
                        pane_history,
                        ctx,
                    )
    ...
    fn load_data_into_transcript_viewer(
        ...
        if let Some(ref terminal_manager) = terminal_manager {
            let status = if let Some(task_id) = ambient_agent_task_id {
                ...
                ConversationTranscriptViewerStatus::ViewingUserConversation
            };
        ...
        // Insert the conversation ended tombstone (includes Open in Ashide button on WASM).
    ...
    fn handle_focus_state_event(
        ...
        match event {
            PaneGroupFocusEvent::FocusChanged { new_focused, .. } => {
                self.update_pane_history(*new_focused);
    ...
    pub fn add_terminal_bootstrap_pane(
        &mut self,
        direction: Direction,
        chosen_shell: Option<AvailableShell>,
        ctx: &mut ViewContext<Self>,
    ) -> TerminalPaneId {
        let new_pane_id = self.add_terminal_bootstrap_session(
            direction,
    ...
    pub(crate) fn add_terminal_bootstrap_pane_with_agent_split_layout(
        &mut self,
        layout: AgentSplitLayoutIntent,
        chosen_shell: Option<AvailableShell>,
        ctx: &mut ViewContext<Self>,
    ) -> TerminalPaneId {
        let new_pane_id = self.add_terminal_bootstrap_session_with_default_session_mode_behavior(
            layout.direction,
            None,
            self.active_session_id(ctx),
            chosen_shell,
    ...
    /// Adds a terminal split pane with fully-specified terminal options.
    ///
    /// Workspace uses this for environment-owned environment runtime PTYs: PaneGroup does not
    /// know what the current Environment is, but it must preserve the
    /// `environment_runtime_pty` process option once Workspace has resolved it.
    pub fn add_terminal_pane_with_options(
        &mut self,
        direction: Direction,
        options: NewTerminalOptions,
        ctx: &mut ViewContext<Self>,
    ) -> TerminalPaneId {
        self.add_terminal_pane_with_options_and_layout(direction, options, None, ctx)
    }

    pub(crate) fn add_terminal_pane_with_options_and_agent_split_layout(
        &mut self,
        options: NewTerminalOptions,
        layout: AgentSplitLayoutIntent,
        ctx: &mut ViewContext<Self>,
    ) -> TerminalPaneId {
        self.add_terminal_pane_with_options_and_layout(layout.direction, options, Some(layout), ctx)
    }

    fn add_terminal_pane_with_options_and_layout(
        &mut self,
        direction: Direction,
        options: NewTerminalOptions,
        agent_split_layout: Option<AgentSplitLayoutIntent>,
        ...
        let should_immediately_enter_agent_view = agent_split_layout.is_none()
        ...
        if let Some(layout) = agent_split_layout {
            let _ = self.add_pane_with_agent_split_layout(Box::new(pane_data), layout, ctx);
        } else {
    ...
    pub fn add_terminal_bootstrap_pane_ignoring_default_session_mode(
        &mut self,
        direction: Direction,
        chosen_shell: Option<AvailableShell>,
        ctx: &mut ViewContext<Self>,
    ...
    /// The current working directory of the active current-app terminal session.
    ...
    fn session_path(&self, pane_id: &TerminalPaneId, ctx: &AppContext) -> Option<PathBuf> {
        self.terminal_view_from_pane_id(*pane_id, ctx)?
            .as_ref(ctx)
            .active_session_current_app_path(ctx)
    }
    ...
    pub fn close_pane_permanently(&mut self, pane_id: PaneId, ctx: &mut ViewContext<Self>) {
        if !self.pane_contents.contains_key(&pane_id) {
            return;
        }
    ...
    pub fn replace_pane<C: PaneContent>(
        ...
        self.replace_pane_with_focus_policy(
            original_pane_id,
            replacement_pane,
            is_temporary,
            PaneReplacementFocus::Replacement,
            ctx,
        )
    }

    fn replace_pane_with_focus_policy<C: PaneContent>(
        &mut self,
        original_pane_id: PaneId,
        replacement_pane: C,
        is_temporary: bool,
        focus_policy: PaneReplacementFocus,
        ctx: &mut ViewContext<Self>,
    ) -> bool {
        self.replace_pane_with_focus_policy_and_result(
            original_pane_id,
            replacement_pane,
            is_temporary,
            focus_policy,
    ...
    fn replace_pane_with_focus_policy_and_result<C, F>(
        &mut self,
        original_pane_id: PaneId,
        replacement_pane: C,
        is_temporary: bool,
        ...
        F: FnOnce(bool, PaneId),
    {
        ...
        let original_was_focused = self.focused_pane_id(ctx) == original_pane_id;
        // Permanent replacement changes pane implementation/runtime identity, not the
        // user-visible container identity. Capture the configuration before attaching
        // the replacement so every replacement path (runtime materialization, file →
        // code, loading → terminal, …) preserves the single durable identity chain.
        let preserved_configuration = (!is_temporary).then(|| {
            let configuration = self
                .pane_contents
                .get(&original_pane_id)
                .expect("existing pane must have content")
                .as_pane()
                .pane_configuration();
            let configuration = configuration.as_ref(ctx);
            (
                configuration.container_uuid().to_vec(),
                configuration.session_binding(),
                configuration
                    .custom_vertical_tabs_title()
                    .map(str::to_owned),
            )
        ...
        if let Some((container_uuid, session_binding, custom_vertical_tabs_title)) =
            preserved_configuration
        {
            let configuration = self
                .pane_contents
                .get(&replacement_pane_id)
                .expect("attached replacement pane must have content")
                .as_pane()
                .pane_configuration();
            configuration.update(ctx, |configuration, ctx| {
                configuration.restore_container_uuid(container_uuid);
                configuration.restore_session_binding(session_binding);
                if let Some(title) = custom_vertical_tabs_title.as_deref() {
                    configuration.set_custom_vertical_tabs_title(title, ctx);
                }
            });
        ...
        if success {
            ...
            if matches!(focus_policy, PaneReplacementFocus::Replacement) || original_was_focused {
        ...
        // Callers that bind external state to the replacement pane must commit or
        // roll back that state before observers can persist the pane tree.
        on_result(success, replacement_pane_id);
        ctx.notify();
        ctx.emit(Event::AppStateChanged);
        success
    }
    ...
    fn replace_code_pane_with_file_pane(
        ...
        let session = self.active_session_view(ctx).and_then(|view| {
            // Use the active session if it belongs to the current-app environment
            let view_ref = view.as_ref(ctx);
            if view_ref.active_session_uses_current_app_environment(ctx) == Some(true) {
                view_ref
    ...
    fn handle_pane_event(
        ...
        match event {
            ...
            PaneEvent::NewPaneInAIMode { initial_query } => {
                ctx.emit(Event::NewPaneInAIMode {
            ...
            PaneEvent::EnvironmentRepoNavigated {
                session_id,
                host_id,
                requested_path,
                indexed_path,
            } => {
                ctx.emit(Event::EnvironmentRepoNavigated {
    ...
    pub fn add_pane_for_replacement<C: PaneContent>(
        ...
        self.init_pane(Box::new(pane), ctx)
    }
    ...
    fn create_session(
        ...
        model_event_sender: Option<SyncSender<ModelEvent>>,
        chosen_shell: Option<AvailableShell>,
        direct_process: Option<DirectTerminalProcess>,
        environment_runtime_pty: Option<EnvironmentRuntimePtyProcess>,
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "remote_tty")] {
                ...
                let terminal_manager: ModelHandle<Box<dyn TerminalManager>> = if let Some(environment_runtime_pty) = environment_runtime_pty {
                    crate::terminal::remote_tty::TerminalManager::create_environment_runtime_model(
                        resources,
                        initial_size,
                        model_event_sender,
                        ctx.window_id(),
                        initial_input_config,
                        crate::terminal::remote_tty::EnvironmentRuntimePtyConfig {
                            client: environment_runtime_pty.client,
                            session_id: environment_runtime_pty.session_id,
                            working_directory: environment_runtime_pty.working_directory,
                            shell: environment_runtime_pty.shell,
                            startup_command: environment_runtime_pty.startup_command,
                            environment_variables: environment_runtime_pty.environment_variables,
                            honor_ps1_enabled: *SessionSettings::as_ref(ctx).honor_ps1,
                        },
                        ctx,
                    )
                } else {
                    crate::terminal::local_tty::TerminalManager::create_model(
                        startup_directory,
                        env_vars,
                        resources,
                        restored_blocks,
                        ...
                        ctx.window_id(),
                        chosen_shell,
                        direct_process.map(|process| {
                            ShellStarter::Direct(DirectShellStarter::new_direct_process(
    ...
    fn create_conversation_viewer(
        ...
        let viewer_status = ambient_agent_task_id
            ...
            .unwrap_or(ConversationTranscriptViewerStatus::ViewingUserConversation);
    ...
    pub fn add_loading_conversation_pane(
        ...
        self.add_loading_conversation_pane_with_layout(direction, base_pane_id, None, ctx)
    }

    pub(crate) fn add_loading_conversation_pane_with_agent_split_layout(
        &mut self,
        layout: AgentSplitLayoutIntent,
        ctx: &mut ViewContext<Self>,
    ) -> PaneId {
        self.add_loading_conversation_pane_with_layout(layout.direction, None, Some(layout), ctx)
    }

    fn add_loading_conversation_pane_with_layout(
        &mut self,
        direction: Direction,
        base_pane_id: Option<PaneId>,
        agent_split_layout: Option<AgentSplitLayoutIntent>,
        ...
        if let Some(layout) = agent_split_layout {
            assert!(
                base_pane_id.is_none(),
                "Agent loading split layout must insert at the pane-tree root"
            );
            let _ = self.add_pane_with_agent_split_layout(Box::new(pane_data), layout, ctx);
        } else {
    ...
    pub fn replace_pane_with_terminal_bootstrap(
        &mut self,
        pane_id: PaneId,
        startup_directory: Option<PathBuf>,
        ctx: &mut ViewContext<Self>,
    ) -> Option<TerminalPaneId> {
        self.replace_pane_with_terminal_options(
            pane_id,
            NewTerminalOptions::default().with_initial_directory_opt(startup_directory),
            ctx,
        )
    }

    pub fn replace_pane_with_terminal_options(
        &mut self,
        pane_id: PaneId,
        options: NewTerminalOptions,
        ctx: &mut ViewContext<Self>,
    ) -> Option<TerminalPaneId> {
        self.replace_pane_with_terminal_options_and_focus_policy(
            pane_id,
            options,
            PaneReplacementFocus::Replacement,
            ctx,
    ...
    pub fn replace_pane_with_terminal_options_following_original_focus(
        &mut self,
        pane_id: PaneId,
        options: NewTerminalOptions,
        ctx: &mut ViewContext<Self>,
        ...
        self.replace_pane_with_terminal_options_and_focus_policy(
            pane_id,
            options,
            PaneReplacementFocus::FollowOriginal,
            ctx,
        )
    }

    pub(crate) fn replace_pane_with_terminal_options_transactionally<F>(
        &mut self,
        pane_id: PaneId,
        options: NewTerminalOptions,
        mut transition: F,
    ...
    where
        F: FnMut(TerminalPaneReplacementPhase),
    {
        let (pane_data, view) = self.create_terminal_pane_data(
            options.initial_directory,
            options.env_vars,
        ...
        let replacement_pane_id = PaneId::from(terminal_pane_id);

        transition(TerminalPaneReplacementPhase::Prepared {
            terminal_pane_id: replacement_pane_id,
        });
        if !self.replace_pane_with_focus_policy_and_result(
            pane_id,
            pane_data,
            false,
            PaneReplacementFocus::FollowOriginal,
            |success, replacement_pane_id| {
                if !success {
                    transition(TerminalPaneReplacementPhase::RolledBack {
                        terminal_pane_id: replacement_pane_id,
                    });
                }
            },
        ...
        self.find_pane_id_for_terminal_view(terminal_view_id, ctx)
            .and_then(|pane_id| pane_id.as_terminal_pane_id())
            .or(Some(terminal_pane_id))
    }

    pub(crate) fn replace_pane_with_environment_runtime_placeholder_transactionally<F>(
        &mut self,
        pane_id: PaneId,
        transition: F,
        ctx: &mut ViewContext<Self>,
    ...
    where
        F: FnOnce(PaneId),
    {
        let placeholder = EnvironmentRuntimePlaceholderPane::new(ctx);
        let placeholder_pane_id = placeholder.id();
        let mut queued_pane_id = pane_id;
        self.replace_pane_with_focus_policy_and_result(
            pane_id,
            placeholder,
            false,
            ...
            |success, replacement_pane_id| {
                if success {
                    queued_pane_id = replacement_pane_id;
                }
                transition(queued_pane_id);
            },
            ctx,
        );
        debug_assert!(
            queued_pane_id == pane_id || queued_pane_id == placeholder_pane_id,
            "runtime placeholder replacement returned an unrelated pane identity"
        );
        queued_pane_id
    }

    fn replace_pane_with_terminal_options_and_focus_policy(
        &mut self,
        pane_id: PaneId,
        options: NewTerminalOptions,
        focus_policy: PaneReplacementFocus,
        ...
        if !self.replace_pane_with_focus_policy(pane_id, pane_data, false, focus_policy, ctx) {
            return None;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn add_terminal_bootstrap_session(
        &mut self,
        direction: Direction,
    ...
    #[allow(clippy::too_many_arguments)]
    fn add_terminal_bootstrap_session_with_default_session_mode_behavior(
        &mut self,
        direction: Direction,
        base_pane_id_for_split: Option<PaneId>,
        base_pane_id_for_context: Option<TerminalPaneId>,
        ...
        self.add_terminal_bootstrap_session_in_directory(
            direction,
            base_pane_id_for_split,
            chosen_shell,
            startup_directory,
    ...
    /// `add_terminal_bootstrap_session_in_directory` and `insert_terminal_pane_hidden_for_child_agent`.
    ...
    #[allow(clippy::too_many_arguments)]
    fn add_terminal_bootstrap_session_in_directory(
        &mut self,
        direction: Direction,
        base_pane_id: Option<PaneId>,
        chosen_shell: Option<AvailableShell>,
        ...
        if let Some(layout) = agent_split_layout {
            assert!(
                base_pane_id.is_none(),
                "Agent split layout must insert at the pane-tree root"
            );
            let _ = self.add_pane_with_agent_split_layout(Box::new(pane_data), layout, ctx);
        } else {
            let _ = self.add_pane(direction, base_pane_id, Box::new(pane_data), true, ctx);
    ...
    fn add_pane_with_options(
        ...
        let agent_split_flex = options
            .agent_split_layout
            .and_then(|layout| self.panes.flex_for_agent_split(layout, ctx));
        ...
        if let (Some(layout), Some(flex)) = (options.agent_split_layout, agent_split_flex) {
            assert!(
                self.panes.apply_agent_split_flex(pane_id, layout, flex),
                "Agent split layout requires the new pane to be a direct root child"
            );
        }
    ...
    fn mark_split_pane_used(&mut self, ctx: &mut ViewContext<Self>) {
    ...
    fn add_pane_with_agent_split_layout(
        &mut self,
        new_pane: Box<dyn AnyPaneContent>,
        layout: AgentSplitLayoutIntent,
        ctx: &mut ViewContext<Self>,
    ) -> Option<PaneId> {
        self.mark_split_pane_used(ctx);
        self.add_pane_with_options(
            ...
            AddPaneOptions {
                ...
                emit_app_state_changed: true,
                agent_split_layout: Some(layout),
            },
            ctx,
        )
    }
    ...
    ///
    /// This returns the active parent session's current-app directory when that
    /// session is done bootstrapping. Otherwise, it returns the current
    ...
    pub(crate) fn terminal_manager_for_pane_id(
        &self,
        pane_id: PaneId,
        app: &AppContext,
    ) -> Option<ModelHandle<Box<dyn TerminalManager>>> {
    ...
    /// Get all current-app terminal CWDs for this pane group.
    /// This is used by the Workspace to refresh the current-app file tree/global-search model.
    pub fn terminal_view_current_app_working_directories<'a>(
        &'a self,
        ctx: &'a AppContext,
        ...
        self.terminal_views(ctx).into_iter().map(|terminal_view| {
            ...
            let cwd = terminal_view.as_ref(ctx).current_app_pwd(ctx);
            (terminal_id, cwd)
        })
    }
    ...
    pub fn code_view_current_app_paths<'a>(
        &'a self,
        ctx: &'a AppContext,
    ) -> impl Iterator<Item = (EntityId, Option<String>)> + 'a {
        self.code_views(ctx).into_iter().map(move |code_view| {
            let id = code_view.id();
            let current_app_path = code_view
                .as_ref(ctx)
                .current_app_path(ctx)
                .map(|p| p.display().to_string());
            (id, current_app_path)
        })
    }

    pub fn code_diff_view_current_app_paths<'a>(
        &'a self,
        ctx: &'a AppContext,
    ) -> impl Iterator<Item = (EntityId, Option<String>)> + 'a {
        self.code_diff_views(ctx).into_iter().map(move |diff_view| {
            let id = diff_view.id();
            let current_app_path = diff_view.as_ref(ctx).primary_file_path(ctx);
            (id, current_app_path)
        })
    }

    pub fn file_notebook_current_app_paths<'a>(
        &'a self,
        ctx: &'a AppContext,
    ) -> impl Iterator<Item = (EntityId, Option<String>)> + 'a {
        self.file_notebook_views(ctx)
            ...
            .map(move |file_view| {
                let id = file_view.id();
                let current_app_path = file_view
                    .as_ref(ctx)
                    .current_app_path()
                    .map(|p| p.display().to_string());
                (id, current_app_path)
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            Add(direction) => {
                ...
                self.add_terminal_bootstrap_pane(*direction, chosen_shell, ctx);
            }
