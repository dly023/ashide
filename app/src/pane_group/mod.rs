...
pub struct PaneGroup {
    ...
    /// Workspace-owned default options for newly split terminal panes.
    /// Environment runtime tabs use this to make PaneGroupAction::Add create an
    /// environment PTY instead of silently falling back to a terminal-bootstrap shell.
    environment_new_terminal_options: Option<NewTerminalOptions>,
    /// True when the owning workspace tab is currently bound to an environment
    /// runtime. This is intentionally tracked separately from
    /// `environment_new_terminal_options`: while the runtime transport is
    /// connecting or temporarily unhealthy, options may be unavailable, but the
    /// split action must still not fall back to a terminal-bootstrap shell.
    environment_new_terminal_uses_runtime: bool,
}
...
impl PaneGroup {
    pub fn set_environment_new_terminal_options(
        &mut self,
        options: Option<NewTerminalOptions>,
        uses_environment_runtime: bool,
        ctx: &mut ViewContext<Self>,
    ) {
        self.environment_new_terminal_options = options;
        self.environment_new_terminal_uses_runtime = uses_environment_runtime;
        ctx.notify();
    }
    ...
    fn new_internal(
        ...
        let mut pane_group = Self {
            ...
            custom_title: None,
            environment_new_terminal_options: None,
            environment_new_terminal_uses_runtime: false,
        };
    ...
    pub fn add_terminal_pane_with_options(
        ...
        let should_immediately_enter_agent_view = options.conversation_restoration.is_none()
    ...
    pub fn replace_pane_with_terminal_options(
        ...
        if !self.replace_pane(pane_id, pane_data, false, ctx) {
            return None;
        }
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            Add(direction) => {
                ...
                if let Some(options) = self.environment_new_terminal_options.clone() {
                    self.add_terminal_pane_with_options(*direction, options, ctx);
                } else if self.environment_new_terminal_uses_runtime {
                    log::warn!(
                        "PaneGroupAction::Add ignored: environment runtime terminal options unavailable"
                    );
                    ctx.notify();
                } else {
