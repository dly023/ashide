use super::*;
use crate::pane_group::AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH;
...
#[test]
fn agent_split_prefers_requested_extent_when_existing_panes_keep_their_minimum() {
    let available_extent = 740.;
    let existing_minimum_extent = 2. * 50.;
    let target_extent = agent_split_target_extent(
        available_extent,
        existing_minimum_extent,
        AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH,
    )
    .expect("Agent pane should fit at its preferred width");
    assert_eq!(target_extent, AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH);

    let flex = agent_split_flex(available_extent, 2., target_extent)
        .expect("default thirds are narrower than the preferred Agent width");
    let realized_extent = available_extent * flex.0 / (2. + flex.0);
    assert!((realized_extent - target_extent).abs() < 0.001);
}

#[test]
fn agent_split_shrinks_only_to_preserve_existing_pane_minimums() {
    let available_extent = 299.;
    let existing_minimum_extent = 2. * 50.;
    let target_extent = agent_split_target_extent(
        available_extent,
        existing_minimum_extent,
    ...
    .expect("small layouts should still produce a feasible Agent target");
    assert_eq!(target_extent, 199.);

    let flex = agent_split_flex(available_extent, 2., target_extent)
        .expect("the feasible Agent target is wider than the default third");
    let realized_extent = available_extent * flex.0 / (2. + flex.0);
    assert!((realized_extent - target_extent).abs() < 0.001);
}
