...
impl FilePane {
    ...
    /// is `None` or the target session cannot expose a current-app path, the pane is created
    /// but left empty. If `path` is `Some`, but there's no target session, the pane is
    /// created using the next focused current-app session.
    pub fn new<V: View>(
        path: Option<PathBuf>,
        ...
        let view = ctx.add_typed_action_view(move |ctx| {
            ...
            if let Some(path) = path {
                if let Some(target_session) = target_session {
                    // If the target session is Some, but non-current-app, do not fall back - the path is
                    // in an environment runtime, so we can't reliably use the fallback behavior.
                    if target_session.uses_current_app_environment() {
                        view.open_current_app_file(path, Some(target_session), ctx);
                    }
                } else {
                    // If the active session is None or not current-app, the pane will wait for a current-app
                    // session to be activated.
                    let session = ActiveSession::as_ref(ctx)
                        .session(ctx.window_id())
                        .filter(|session| session.uses_current_app_environment());
                    view.open_current_app_file(path, session, ctx);
                }
            }
...
impl PaneContent for FilePane {
    ...
    fn attach(
        ...
        subscribe_to_link_model(&file_view.as_ref(ctx).links(), ctx);
    ...
    fn snapshot(&self, app: &AppContext) -> LeafContents {
        let path = self.file_view(app).as_ref(app).current_app_path();
        LeafContents::Notebook(NotebookPaneSnapshot::CurrentAppFileNotebook { path })
    }
