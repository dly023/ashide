...
use crate::{
    ...
    drive::{items::LocalDriveItemId, LocalDriveObjectSettings},
    notebooks::{
...
impl NotebookPane {
    ...
    /// Restore a notebook pane given its local object-store notebook ID.
    pub fn restore(
...
impl PaneContent for NotebookPane {
    ...
    fn attach(
        ...
        subscribe_to_link_model(&self.notebook_view(ctx).as_ref(ctx).links(), ctx);
...
pub(super) fn subscribe_to_link_model(
    ...
    ctx.subscribe_to_model(handle, move |_, _, event, ctx| match event {
        ...
        LinkEvent::StartEnvironmentSession { path } => {
            ctx.emit(crate::pane_group::Event::OpenDirectoryInNewTab { path: path.clone() });
        }
        ...
        LinkEvent::OpenFileWithTarget {
            ...
            // Emit event to workspace to handle opening in Ashide
...
fn handle_notebook_event(
    ...
    match event {
        ...
        NotebookEvent::EditWorkflow(id) => ctx.emit(
            crate::pane_group::Event::OpenObjectStoreWorkflowForEdit(*id),
        ),
        NotebookEvent::ViewInLocalDrive(id) => view_in_local_drive(*id, ctx),
...
fn view_in_local_drive(id: LocalDriveItemId, ctx: &mut ViewContext<PaneGroup>) {
    ctx.emit(crate::pane_group::Event::ViewInLocalDrive(id))
}
