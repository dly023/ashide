...
pub(crate) mod provider_connection_pane;
pub(crate) mod provider_file_browser_pane;
...
impl Display for IPaneType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            IPaneType::ProviderConnection => write!(f, "Provider Connection"),
            IPaneType::ProviderFileBrowser => write!(f, "Provider File Browser"),
            IPaneType::EnvironmentRuntimePlaceholder => {
                write!(f, "Environment Runtime Placeholder")
            }
...
impl PaneId {
    ...
    // Legacy environment-management pane context helper was removed with the ambient-agent UI subsystem.
    ...
    pub fn from_provider_connection_pane_ctx(ctx: &ViewContext<PaneView<SshServerView>>) -> Self {
        Self::new_from_ctx(IPaneType::ProviderConnection, ctx)
    }

    /// Creates a [`PaneId`] from a [`ViewContext<PaneView<SftpBrowserView>>`]
    pub fn from_provider_file_browser_pane_ctx(
        ctx: &ViewContext<PaneView<SftpBrowserView>>,
    ) -> Self {
        Self::new_from_ctx(IPaneType::ProviderFileBrowser, ctx)
    }
    ...
    // Legacy environment-management pane view helpers were removed with the ambient-agent UI subsystem.
    ...
    pub fn from_provider_connection_pane_view(
        provider_connection_pane_view: &ViewHandle<PaneView<SshServerView>>,
    ) -> Self {
        Self::new(IPaneType::ProviderConnection, provider_connection_pane_view)
    }

    /// Creates a [`PaneId`] from a [`PaneView<SftpBrowserView>`] entity ID.
    pub fn from_provider_file_browser_pane_view(
        provider_file_browser_pane_view: &ViewHandle<PaneView<SftpBrowserView>>,
    ) -> Self {
        Self::new(
            IPaneType::ProviderFileBrowser,
            provider_file_browser_pane_view,
        )
    }

    pub fn from_environment_runtime_placeholder_pane_view(
        placeholder_pane_view: &ViewHandle<PaneView<EnvironmentRuntimePlaceholderView>>,
    ) -> Self {
        Self::new(
            IPaneType::EnvironmentRuntimePlaceholder,
            placeholder_pane_view,
        )
    }

    pub fn from_environment_runtime_placeholder_pane_ctx(
        ctx: &ViewContext<PaneView<EnvironmentRuntimePlaceholderView>>,
    ) -> Self {
        Self::new_from_ctx(IPaneType::EnvironmentRuntimePlaceholder, ctx)
    }
    ...
    /// a pane — activation goes through `ActivateRestoredWorkspaceSession`
    /// instead — so its only requirement is to be constructible without a live
    /// view. Uses `DeferredPlaceholder` as the most semantically-appropriate
    /// non-live pane type.
    ...
    pub fn is_environment_management_pane(&self) -> bool {
        // Legacy environment-management pane type was removed with the ambient-agent UI subsystem.
        // This helper remains during gradual cleanup and always returns false.
        false
    }

    /// Returns true if this pane contains a Ashide Drive object (notebook, workflow, etc.).
    pub fn is_local_drive_object_pane(&self) -> bool {
        matches!(
            self.0.pane_type,
            IPaneType::Notebook
                | IPaneType::Workflow
    ...
    pub fn render(self, app: &AppContext) -> Box<dyn Element> {
        let mut element = match self.0.pane_type {
            ...
            IPaneType::EnvironmentRuntimePlaceholder => ChildView::<
                PaneView<EnvironmentRuntimePlaceholderView>,
...
pub struct PaneConfiguration {
    ...
    /// Stable provider/session metadata belongs to the semantic pane container,
    /// not to whichever placeholder or terminal runtime currently carries it.
    /// Interior mutability lets immutable snapshot projection atomically retain
    /// the last observed binding without introducing a second registry.
    session_binding: RefCell<Option<PaneSessionBinding>>,
    title: String,
...
impl PaneConfiguration {
    pub fn new(title: impl Into<String>) -> Self {
        Self {
            ...
            session_binding: RefCell::new(None),
            title: title.into(),
    ...
    pub(crate) fn session_binding(&self) -> Option<PaneSessionBinding> {
        self.session_binding.borrow().clone()
    }

    pub(crate) fn restore_session_binding(&self, binding: Option<PaneSessionBinding>) {
        *self.session_binding.borrow_mut() = binding;
    }
...
pub enum PaneEvent {
    ...
    RepoChanged,
    /// An environment runtime resolved the repo root for a session in this pane.
...
mod provider_pane_variant_tests {
    ...
    fn provider_pane_type_names_serialize_with_provider_neutral_variants() {
        assert_eq!(
            serde_json::to_string(&IPaneType::ProviderConnection).unwrap(),
            "\"ProviderConnection\""
        );
        assert_eq!(
            serde_json::to_string(&IPaneType::ProviderFileBrowser).unwrap(),
            "\"ProviderFileBrowser\""
        );
    }
}
