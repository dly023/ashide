...
impl WelcomeView {
    ...
    fn create_terminal_session(&mut self, ctx: &mut ViewContext<Self>) {
        update_workspace(ctx.window_id(), ctx, |workspace, ctx| {
            if let Some(path) = self.startup_directory.clone() {
                workspace.open_directory_tab_in_current_environment(path, false, ctx);
            } else {
    ...
    fn open_project_conversation(&mut self, path: &String, ctx: &mut ViewContext<Self>) {
        ...
        update_workspace(ctx.window_id(), ctx, |workspace, ctx| {
            // Create a project agent tab in the active Environment. Runtime-backed environments
            // must queue an EnvironmentRuntime agent intent instead of opening a current-app
            // terminal and then trying to enter agent mode locally.
            let _terminal_ready = workspace.open_agent_directory_tab_in_current_environment(
                path_buf.clone(),
                false,
                ctx,
            );
...
fn save_and_open_project(path: String, window_id: WindowId, ctx: &mut AppContext) {
    ProjectManagementModel::handle(ctx).update(ctx, |projects, ctx| {
        ...
        update_workspace(window_id, ctx, move |workspace, ctx| {
            workspace.open_directory_tab_in_current_environment(PathBuf::from(path), true, ctx);
        });
    });
}
