...
impl TerminalPane {
    /// Publish the terminal's latest CLI-agent observation into the pane-owned
    /// binding before Workspace projects or persists it.
    ///
    /// Snapshotting must remain read-only: mutating this owner from `snapshot`
    /// made persistence order observable and allowed a temporarily unavailable
    /// runtime cwd to erase the cold-restored binding.
    pub(in crate::pane_group) fn refresh_session_binding_from_cli_agent_state(
        &self,
        app: &AppContext,
    ) {
        let view = self.terminal_view(app).as_ref(app);
        let cli_agent_session = CLIAgentSessionsModel::as_ref(app)
            .session(view.id())
            .cloned();
        let detected_cli_agent_session = view.detected_cli_agent_session_metadata(app);
        ...
        let current_cli_command = cli_agent_session
            ...
            .or_else(|| {
                detected_cli_agent_session
                    .as_ref()
                    .map(|(agent, custom_command_prefix)| {
                        custom_command_prefix
                            .clone()
        ...
        let current_cli_agent_origin = cli_agent_session
            ...
            .map(|session| {
                if session.listener.is_some() {
                    CliAgentSessionOrigin::PluginObserved
                } else {
                    CliAgentSessionOrigin::CommandDetected
                }
            })
            .or_else(|| {
                detected_cli_agent_session
        ...
        let current_binding = PaneSessionBinding {
            ...
            cwd: view.workspace_session_snapshot_pwd(app),
...
impl PaneContent for TerminalPane {
    ...
    fn detach(
        ...
        for (manager, view) in contents {
            ...
            manager.update(ctx, |terminal_manager, ctx| {
                if matches!(detach_type, DetachType::Closed) {
                    // Retiring durable ownership intentionally keeps TerminalModel alive,
                    // so permanent teardown must explicitly stop the manager/event loop.
    ...
    fn snapshot(&self, app: &AppContext) -> LeafContents {
        ...
        if view.model.lock().shared_session_status().is_viewer() {
            ...
            // Conversation transcript viewers (opened from Session Navigator)
            ...
            let cwd = view.workspace_session_snapshot_pwd(app).or(persisted_cwd);
            ...
            LeafContents::Terminal(TerminalPaneSnapshot {
                ...
                shell_launch_data: view.current_app_shell_launch_data(app),
    ...
    if group.pane_contents.contains_key(&pane_id) {
        match event {
                            ...
                            && AppExecutionMode::as_ref(ctx).can_save_session()
                        {
                            if let Some(sender) = &group.model_event_sender {
                                let block_completed_event = ModelEvent::SaveBlock(BlockCompleted {
                                    ...
                                    block: block.clone(),
                                    is_local: *uses_current_app_environment,
                                });
            ...
            Event::SessionBootstrapped => {
                ctx.emit(pane_group::Event::TerminalSessionBootstrapped { pane_id });
                ctx.emit(pane_group::Event::ActiveSessionChanged);
            }
            Event::SessionBootstrapFailed => {
                ctx.emit(pane_group::Event::TerminalSessionBootstrapFailed { pane_id });
            }
            ...
            Event::OpenWorkflowModalWithWorkflowObject(workflow_id) => {
                ctx.emit(pane_group::Event::OpenObjectStoreWorkflowForEdit(
                    *workflow_id,
                ));
            }
            ...
            Event::LocalDriveObjectInPane(uid) => {
                ctx.emit(pane_group::Event::LocalDriveObjectInPane(uid.clone()));
            }
            ...
            Event::OpenAIFactCollection { object_store_id } => {
            ...
            // Ashide:把终端发出的“打开环境文件”事件透传给 pane_group → workspace。
            #[cfg(all(feature = "local_tty", feature = "local_fs"))]
            Event::OpenEnvironmentFileFromTerminal {
                environment_file_path,
                binding_session_id,
                line_col,
            } => {
                ctx.emit(pane_group::Event::OpenEnvironmentFileFromTerminal {
                    environment_file_path: environment_file_path.clone(),
                    binding_session_id: *binding_session_id,
                    line_col: *line_col,
                });
            }
            Event::CopyFileToEnvironment { command, upload_id } => {
                ...
                ctx.emit(pane_group::Event::FileUploadCommand {
                    ...
                    environment_pane_id: terminal_pane_id,
                    current_app_pane_id: new_pane_id,
                });
            ...
            Event::FileUploadPasswordPending => {
                ctx.emit(pane_group::Event::FileUploadPasswordPending {
                    current_app_pane_id: terminal_pane_id,
                });
            }
            ...
            Event::OpenRulesPane => {
                ctx.emit(crate::pane_group::Event::OpenAIFactCollection {
