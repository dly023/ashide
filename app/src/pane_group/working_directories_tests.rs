...
#[test]
fn refresh_working_directories_keeps_terminal_cwds_and_tracks_nearest_repo_root() {
    App::test((), |mut app| async move {
        ...
        let canonical_repo_a = dunce::canonicalize(&repo_a).expect("canonical repo/a");
        let canonical_repo_b = dunce::canonicalize(&repo_b).expect("canonical repo/b");
        ...
        let (roots, repos): (HashSet<PathBuf>, Vec<PathBuf>) =
        ...
        assert_eq!(roots, HashSet::from([canonical_repo_a, canonical_repo_b]));
