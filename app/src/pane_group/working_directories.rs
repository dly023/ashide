...
impl WorkingDirectoriesModel {
    ...
    pub fn refresh_working_directories_for_pane_group(
        ...
        current_app_paths: Vec<(EntityId, String)>,
        focused_terminal_id: Option<EntityId>,
        ctx: &mut ModelContext<Self>,
    ) {
        if terminal_cwds.is_empty() && current_app_paths.is_empty() {
            self.handle_empty_pane_group(pane_group_id, ctx);
            return;
        }
        ...
        let display_root_for_raw_path = |raw_path: &str| normalize_cwd(raw_path);
        ...
        let mut file_path_ancestors: HashSet<PathBuf> = terminal_cwds
            .iter()
            .filter_map(|(_, cwd)| display_root_for_raw_path(cwd))
            .collect();

        let local_cwds: Vec<(EntityId, String)> = current_app_paths
            .into_iter()
        ...
        //
        // Project Explorer follows the focused shell's actual cwd. Repository
        // roots are still tracked separately below for code-review/git features.
        ...
        let mut new_root_to_terminal: HashMap<PathBuf, EntityId> = terminal_cwds
            ...
            .filter_map(|(terminal_id, cwd)| {
                display_root_for_raw_path(cwd).map(|p| (p, *terminal_id))
            })
            .collect();
...
impl WorkingDirectoriesModel {
    ...
    pub fn refresh_working_directories_for_pane_group(
        ...
        _current_app_paths: Vec<(EntityId, String)>,
