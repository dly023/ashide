...
use crate::pane_group::{get_minimum_pane_size, DraggedBorder, PaneGroupAction, MINIMUM_PANE_SIZE};
...
impl PaneData {
    ...
    pub(super) fn flex_for_agent_split(
        &self,
        layout: super::AgentSplitLayoutIntent,
        ctx: &mut ViewContext<PaneGroup>,
    ) -> Option<PaneFlex> {
        let axis = layout.direction.axis();
        let root_size = self.root.pane_size(ctx);
        let root_extent = match axis {
            SplitDirection::Horizontal => root_size.x(),
            SplitDirection::Vertical => root_size.y(),
        };
        if root_extent <= 0. {
            return None;
        }

        let existing_minimum_extent =
            self.root
                .minimum_visible_extent(axis, MINIMUM_PANE_SIZE, &self.hidden_panes);
        let new_divider_extent = (get_divider_thickness() - 1.).max(0.);
        let available_extent = root_extent - new_divider_extent;
        let target_extent = agent_split_target_extent(
            available_extent,
            existing_minimum_extent,
            layout.minimum_width,
        )?;
        let root_flex_sum = match &self.root {
            PaneNode::Branch(branch) if branch.axis == axis => branch
                .nodes
                .iter()
                .filter(|(_, node)| node.has_visible_children(&self.hidden_panes))
                .map(|(flex, _)| flex.0)
                .sum(),
            PaneNode::Branch(_) | PaneNode::Leaf(_) => DEFAULT_FLEX_VALUE,
        };
        agent_split_flex(available_extent, root_flex_sum, target_extent)
    }

    pub(super) fn apply_agent_split_flex(
        &mut self,
        pane_id: PaneId,
        layout: super::AgentSplitLayoutIntent,
        flex: PaneFlex,
    ) -> bool {
        let PaneNode::Branch(root_branch) = &mut self.root else {
            return false;
        };
        if root_branch.axis != layout.direction.axis() {
            return false;
        }

        let Some((pane_flex, _)) = root_branch
            .nodes
            .iter_mut()
            .find(|(_, node)| matches!(node, PaneNode::Leaf(candidate) if *candidate == pane_id))
        else {
            return false;
        };
        *pane_flex = flex;
...
impl PaneNode {
    ...
    fn minimum_visible_extent(
        &self,
        axis: SplitDirection,
        minimum_pane_extent: f32,
        hidden_panes: &[HiddenPane],
    ) -> f32 {
        match self {
            PaneNode::Leaf(pane_id) => {
                if hidden_panes.iter().any(|pane| pane.pane_id == *pane_id) {
                    0.
                } else {
                    minimum_pane_extent
                }
            ...
            PaneNode::Branch(branch) if branch.axis == axis => branch
                ...
                .iter()
                .map(|(_, node)| {
                    node.minimum_visible_extent(axis, minimum_pane_extent, hidden_panes)
                })
                .sum(),
...
fn agent_split_target_extent(
    available_extent: f32,
    existing_minimum_extent: f32,
    requested_extent: f32,
) -> Option<f32> {
    let maximum_feasible_extent = available_extent - existing_minimum_extent;
    let target_extent = requested_extent.min(maximum_feasible_extent);
    (target_extent.is_normal() && target_extent > 0. && target_extent < available_extent)
        .then_some(target_extent)
}

fn agent_split_flex(
    available_extent: f32,
    existing_flex_sum: f32,
    target_extent: f32,
) -> Option<PaneFlex> {
    let default_new_pane_extent = available_extent / (existing_flex_sum + DEFAULT_FLEX_VALUE);
    let remaining_extent = available_extent - target_extent;
    let ratio = target_extent / remaining_extent;

    (default_new_pane_extent < target_extent
        && existing_flex_sum > 0.
        && ratio.is_normal()
        && ratio > 0.)
        .then_some(PaneFlex(existing_flex_sum * ratio))
}
