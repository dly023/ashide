...
use super::sftp_ops;
use super::sftp_ops::{normalize_remote_path, SftpOpsError};
use super::types::{
    ConnectionState, Dialog, FileEntry, TransferDirection, TransferState, TransferTask,
};
...
pub struct SftpBrowserView {
    ...
    /// Owns async listing completions across navigation and reconnects.
    ...
    loading_spinner: SpinnerStateHandle,
    // ---- 鼠标句柄 ----
    /// 刷新按钮
    ...
    /// 重新连接按钮
    reconnect_btn: MouseStateHandle,
    /// 对话框确认按钮
...
impl SftpBrowserView {
    ...
    pub fn new(node_id: String, ctx: &mut ViewContext<Self>) -> Self {
        let pane_configuration = ctx.add_model(|_ctx| PaneConfiguration::new("Files"));
        ...
        let mut me = Self {
            ...
            loading_spinner: SpinnerStateHandle::new(),
            ...
            reconnect_btn: MouseStateHandle::default(),
    ...
    fn refresh_dir_sync(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match sftp.list_dir(&path) {
            Ok(mut entries) => {
                ...
                entries.sort_by_key(|entry| !entry.is_directory_like());
    ...
    /// Backend-selection seam: returns a daemon-backed [`SftpBackend`] when the
    /// remote helper daemon is already connected for this SFTP browser's host,
    /// otherwise `None`.
    ///
    /// All remote file access is unified on the daemon (decision A): this
    /// resolves the browser's SSH config `node_id` to the active environment
    /// runtime's exact target, then builds a
    /// [`super::daemon_backend::DaemonSftpBackend`] from that runtime's
    /// `EnvironmentRuntimeClient` so file ops ride the single ControlMaster SSH
    /// connection shared by terminal + agent + transfers. When no connected
    /// runtime exists for the host this returns `None` and the browser shows a
    /// "connect the environment first" state (there is no libssh2 fallback).
    #[cfg(not(any(test, feature = "integration_tests")))]
    fn try_daemon_backend(
        &self,
        window_id: warpui::WindowId,
        ctx: &AppContext,
    ) -> Option<Arc<dyn SftpBackend>> {
        let target = self.daemon_target_for_node(window_id, ctx)?;
        let client = Self::daemon_client_for_target(&target, ctx)?;
        let handle = tokio::runtime::Handle::try_current().ok()?;
        Some(super::daemon_backend::DaemonSftpBackend::new(client, handle).into_backend())
    }

    fn daemon_client_for_target(
        target: &crate::workspace::environment_runtime::EnvironmentRuntimeTarget,
        ctx: &AppContext,
    ) -> Option<Arc<crate::workspace::environment_runtime::EnvironmentRuntimeClient>> {
    ...
    /// Resolves this browser's SSH `node_id` to the connected environment
    /// runtime's exact transport target.
    ///
    /// The environment system indexes runtimes by `authority_key`, and an SSH
    /// environment's authority is `ssh:{node_id}` (derived in
    /// `source_saved_ssh::runtime_transport_snapshot` /
    /// `provider_candidate_from_config`). `OpenEnvironment` and
    /// `OpenProviderFileBrowserPane` are emitted with the *same* `node_id` (see
    /// `ssh_manager::panel`), so this browser's `node_id` maps directly to that
    /// authority. The window's `Workspace` owns the environment-runtime
    /// registry; [`crate::workspace::connected_environment_runtime_target_for_node`]
    /// returns the target only when a runtime is Connected for that authority.
    #[cfg(not(any(test, feature = "integration_tests")))]
    fn daemon_target_for_node(
        &self,
        window_id: warpui::WindowId,
        ctx: &AppContext,
    ) -> Option<crate::workspace::environment_runtime::EnvironmentRuntimeTarget> {
        crate::workspace::connected_environment_runtime_target_for_node(
            window_id,
            &self.node_id,
            ctx,
        )
    ...
    /// Connects the file browser to the host's daemon-backed file RPCs.
    ///
    /// All remote file access is unified on the remote helper daemon (decision
    /// A): there is no separate libssh2 SFTP connection. We validate the SSH
    /// config node still exists (for clear error states), then resolve the host
    /// to an active environment runtime and drive file ops over the daemon's
    /// shared ControlMaster connection. If no connected environment runtime
    /// exists for this host, the browser surfaces a "connect the environment
    /// first" state instead of opening a second connection.
    fn connect_to_server(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match result {
            Ok(Some(_server)) => {
                ...
                // Route file ops through the daemon backend, which rides the
                // single ControlMaster SSH connection shared by terminal +
                // agent + transfers. Requires the host's environment to be
                // connected; there is no libssh2 fallback.
                let window_id = ctx.window_id();
                if let Some(backend) = self.try_daemon_backend(window_id, ctx) {
                    self.connection = ConnectionState::Connected;
                    self.sftp = Some(backend.clone());
                    self.connection_generation = self.connection_generation.wrapping_add(1);
                ...
                // No connected environment runtime for this host: ask the user
                // to open it as an environment first.
                self.is_loading = false;
                self.sftp = None;
                self.connection_generation = self.connection_generation.wrapping_add(1);
                self.listing_generation = self.listing_generation.wrapping_add(1);
                self.connection =
                    ConnectionState::Failed(crate::t!("sftp-environment-not-connected"));
                self.show_error_toast(crate::t!("sftp-environment-not-connected"), ctx);
                ctx.notify();
            }
            Ok(None) => {
                self.connection =
                    ConnectionState::Failed(crate::t!("sftp-server-config-not-found"));
                self.show_error_toast(crate::t!("sftp-server-config-not-found"), ctx);
                ctx.notify();
            }
            Err(e) => {
                self.connection = ConnectionState::Failed(crate::t!(
                    "sftp-read-server-config-failed",
                    error = e.to_string()
                ));
                self.show_error_toast(
                    crate::t!("sftp-read-server-config-failed", error = e.to_string()),
                    ctx,
                );
                ctx.notify();
            }
    ...
    fn refresh_dir(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.listing_generation = self.listing_generation.wrapping_add(1);
        let listing_generation = self.listing_generation;
        ...
        self.refresh_handle = self.run_blocking(
            ctx,
            move || sftp.list_dir(&operation_path),
            move |me, result, ctx| {
                if me.connection_generation != connection_generation
                    || me.listing_generation != listing_generation
                    || me.current_path != path
                {
                    return;
                }
                match result {
                    Ok(result) => me.apply_listing_completion(
    ...
    pub(crate) fn apply_listing_completion(
        &mut self,
        connection_generation: u64,
        path: PathBuf,
        listing_generation: u64,
        result: Result<Vec<FileEntry>, SftpOpsError>,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.connection_generation != connection_generation
            || self.listing_generation != listing_generation
        ...
        match result {
            ...
            Err(error) => self.show_error_toast(
                crate::t!("sftp-list-directory-failed", error = error.to_string()),
                ctx,
            ),
        }
    ...
    fn delete_selected(&mut self, index: usize, ctx: &mut ViewContext<Self>) {
        if let Some(entry) = self.entries.get(index) {
            let (paths, is_dirs, directory_identities) = if self.selected.contains(&index) {
                // 删除所有选中的
                self.selected
                    .iter()
                    .filter_map(|&i| {
                        self.entries.get(i).map(|e| {
                            (
                                e.path.clone(),
                                e.is_directory_entry(),
                                e.directory_identity.clone(),
                            )
                        })
                    })
                    .fold(
                        (Vec::new(), Vec::new(), Vec::new()),
                        |(mut paths, mut is_dirs, mut identities), (path, is_dir, identity)| {
                            paths.push(path);
                            is_dirs.push(is_dir);
                            identities.push(identity);
                            (paths, is_dirs, identities)
                        },
                    )
            } else {
                (
                    vec![entry.path.clone()],
                    vec![entry.is_directory_entry()],
                    vec![entry.directory_identity.clone()],
                )
            };
    ...
    fn confirm_delete(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let (paths, is_dirs, directory_identities) = match &self.dialog {
            Some(Dialog::DeleteConfirm {
                paths,
                is_dirs,
                directory_identities,
            }) => (paths.clone(), is_dirs.clone(), directory_identities.clone()),
            Some(Dialog::Rename { .. })
        ...
        self.run_blocking(
            ctx,
            move || {
                for ((path, is_dir), identity) in paths
                    .iter()
                    .zip(is_dirs.iter())
                    .zip(directory_identities.iter())
                {
                    let result = if *is_dir {
                        let identity = identity.as_ref().ok_or_else(|| {
                            "directory listing did not provide a delete identity".to_string()
                        })?;
                        sftp.delete_dir_recursive(path, identity)
                    } else {
            ...
            move |me, result, ctx| {
                ...
                match result {
                    ...
                    Ok(Err(e)) => {
                        me.show_error_toast(
                            crate::t!("sftp-delete-failed", error = e.to_string()),
                            ctx,
                        );
                        me.refresh_dir(ctx);
                    }
    ...
    fn render_connection_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        let theme = appearance.theme();
        let (msg, icon, icon_fill, text_color, spinner) = match &self.connection {
            ConnectionState::Connecting => (
                crate::t!("sftp-status-connecting"),
                Icon::Loading,
                theme.accent(),
                theme.accent().into(),
                Some(&self.loading_spinner),
            ),
            ...
            ConnectionState::Disconnected => (
                crate::t!("sftp-status-disconnected"),
                Icon::AlertCircle,
                theme.ui_error_color().into(),
                theme.ui_error_color(),
                None,
        ...
        let status =
            render_centered_status(icon, &msg, 12.0, icon_fill, text_color, spinner, appearance);
        if matches!(
            self.connection,
            ConnectionState::Failed(_) | ConnectionState::Disconnected
        ) {
            let reconnect_button = render_status_action_button(
                crate::t_static!("sftp-action-reconnect"),
                SftpBrowserAction::Reconnect,
                self.reconnect_btn.clone(),
                appearance,
            );
            Align::new(
                Flex::column()
    ...
    fn execute_upload(&mut self, local_path: &Path, ctx: &mut ViewContext<Self>) {
        ...
        let remote_path = match build_upload_remote_path(&self.current_path, &file_name) {
            Some(p) => p,
            None => {
                self.show_error_toast(crate::t!("sftp-file-name-invalid-characters"), ctx);
                return;
            }
        };
        let local_size = match super::sftp_backend::ensure_transferable_local_file(local_path) {
            ...
            Err(error) => {
                ...
                self.show_error_toast(
                    crate::t!("sftp-upload-failed", error = error.to_string()),
                    ctx,
                );
                ctx.notify();
                return;
        ...
        // 检查远程目录中是否已存在同名文件
        let existing = self.entries.iter().find(|e| e.name == file_name);

        if let Some(existing) = existing {
            if !existing.is_transferable() {
                self.show_error_toast(
                    crate::t!(
                        "server-file-browser-transfer-symlink-unsupported",
                        path = existing.path.to_string_lossy().to_string()
                    ),
                    ctx,
                );
                return;
    ...
    fn execute_upload_confirmed(
        ...
        if let Some(sftp) = &self.sftp {
                ...
                move |me, result, ctx| {
                    ...
                    match &result {
                        ...
                        Ok(Err(e)) => {
                            ...
                            me.show_error_toast(
                                crate::t!("sftp-upload-failed", error = e.to_string()),
                                ctx,
                            );
                            ctx.notify();
                        }
            ...
            if let Some(t) = self.transfers.iter_mut().find(|t| t.id == task_id) {
                t.state = TransferState::Failed(crate::t!("sftp-not-connected-to-server"));
            }
            log::error!("sftp: upload failed: not connected to server");
            self.show_error_toast(crate::t!("sftp-upload-not-connected"), ctx);
            ctx.notify();
        }
    }
    ...
    fn execute_download(
        ...
        if let Some(sftp) = &self.sftp {
            ...
            if let Some(handle) = self.run_blocking(
                ...
                move |me, result, ctx| {
                    ...
                    if let Ok(Err(e)) = &result {
                        ...
                        me.show_error_toast(
                            crate::t!("sftp-download-failed", error = e.to_string()),
                            ctx,
                        );
                    }
                    ctx.notify();
            ...
            self.show_error_toast(crate::t!("sftp-download-not-connected"), ctx);
            ctx.notify();
        }
    }
...
fn render_status_action_button(
    label: &str,
    action: SftpBrowserAction,
    ...
    Hoverable::new(mouse_state, move |state| {
        ...
        let text = Text::new(label.clone(), ui_font, ui_font_size)
...
fn render_centered_status(
    ...
    icon_fill: ThemeFill,
    text_color: ColorU,
    spinner: Option<&SpinnerStateHandle>,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let icon_el = spinner
        .map(|state| {
        ...
        .unwrap_or_else(|| icon.to_warpui_icon(icon_fill).finish());
    let icon_el = ConstrainedBox::new(icon_el)
        .with_width(24.0)
        .with_height(24.0)
        .finish();
        ...
        match action {
            ...
            SftpBrowserAction::ConfirmRename => {
                ...
                }) = &self.dialog
                {
                    ...
                    if new_name.is_empty() {
                        self.show_error_toast(crate::t!("sftp-name-empty"), ctx);
                        return;
                    }
                    let new_path = match build_rename_path(original_path, &new_name) {
                        ...
                        None => {
                            self.show_error_toast(
                                crate::t!("sftp-invalid-name-path-separators"),
                                ctx,
                            );
                            return;
                        }
                        ...
                        self.run_blocking(
                            ...
                            move |me, result, ctx| {
                                match result {
                                    ...
                                    Ok(Err(e)) => {
                                        me.show_error_toast(
                                            crate::t!("sftp-rename-failed", error = e.to_string()),
                                            ctx,
                                        );
                                    }
                                    Err(_) => {}
            ...
            SftpBrowserAction::ConfirmNewFolder => {
                if let Some(Dialog::CreateFolder { parent_path }) = &self.dialog {
                    ...
                    if folder_name.is_empty() {
                        self.show_error_toast(crate::t!("sftp-folder-name-empty"), ctx);
                        return;
                    }
            ...
            SftpBrowserAction::DownloadSaveAs {
                ...
                let Some(entry) = entry else {
                    self.show_error_toast(
                        crate::t!("common-resource-not-found-or-access-denied"),
                        ctx,
                    );
                    return;
                };
                ...
                self.execute_download(&entry.path, &local_path, entry.size, ctx);
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn sftp_runtime_lookup_rejects_connected_target_without_client() {
        warpui::App::test((), |mut app| async move {
            app.add_singleton_model(crate::workspace::environment_runtime::new_transport_manager);
            let target = crate::workspace::environment_runtime::EnvironmentRuntimeTarget {
                authority: "ssh:test".to_owned(),
                session_id: warp_core::SessionId::from(91),
                host_id: warp_core::HostId::new("connected-without-client".to_owned()),
                root: None,
            };

            assert!(app
                .update(|ctx| SftpBrowserView::daemon_client_for_target(&target, ctx))
                .is_none());
        });
    }
    ...
    fn test_action_download_save_as() {
        ...
        assert!(matches!(
            action,
            SftpBrowserAction::DownloadSaveAs { remote_path, .. }
                if remote_path == Path::new("/remote/file.txt")
        ));
    }
}
