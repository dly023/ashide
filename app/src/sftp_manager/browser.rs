...
impl SftpBrowserView {
    ...
    /// runtime's `HostId` (see [`Self::daemon_host_id_for_node`]), then builds a
    ...
    fn try_daemon_backend(
        ...
        let host_id = self.daemon_host_id_for_node(window_id, ctx)?;
        let client = crate::workspace::environment_runtime::client_for_host(&host_id, ctx)?;
    ...
    /// registry; [`crate::workspace::connected_environment_runtime_host_id_for_node`]
    /// returns the `HostId` only when a runtime is Connected for that authority.
    #[cfg(not(any(test, feature = "integration_tests")))]
    fn daemon_host_id_for_node(
        &self,
        window_id: warpui::WindowId,
        ctx: &AppContext,
    ) -> Option<warp_core::HostId> {
        crate::workspace::connected_environment_runtime_host_id_for_node(
            window_id,
            &self.node_id,
            ctx,
        )
        ...
        self.refresh_handle = self.run_blocking(
            ...
            |me, result, ctx| {
                ...
                match result {
                    ...
                    Ok(Err(e)) => {
                        me.show_error_toast(
                            crate::t!("sftp-list-directory-failed", error = e.to_string()),
                            ctx,
                        );
                    }
                    Err(_) => {}
