...
fn build_file_menu_items(entry_index: usize) -> Vec<MenuItem> {
    vec![
        MenuItem {
            label: crate::t!("common-open"),
            action: SftpBrowserAction::OpenEntry(entry_index),
        },
        MenuItem {
            label: crate::t!("server-file-browser-menu-download"),
...
mod tests {
    ...
    fn test_build_file_menu_items_labels() {
        let items = build_file_menu_items(0);
        // 用 `t!` 现算期望值,而不是硬编码原始 key 字符串。i18n 的 `LANGUAGE_LOADER`
        // 是进程级 OnceLock:别的测试一旦先 init,label 就会从 key 解析成译文,
        // 旧断言(label == "common-open")在并行下随机失败。比对 `item.label == t!(key)`
        // 与 locale 无关——验证的是"每个菜单项绑定了正确的 key",这才是测试意图。
        let expected_labels = [
            crate::t!("common-open"),
            crate::t!("server-file-browser-menu-download"),
            crate::t!("common-rename"),
            crate::t!("common-delete"),
            crate::t!("common-details"),
        ];
        for (item, expected) in items.iter().zip(expected_labels.iter()) {
            assert_eq!(&item.label, expected, "菜单项标签应解析自正确的 key");
        }
    }
    ...
    fn test_render_context_menu_via_browser() {
        ...
        warpui::App::test((), |mut app| async move {
            ...
            let temp_db = std::env::temp_dir().join("ashide_sftp_ctx_test.sqlite");
