...
use super::types::{
    ConnectionState, Dialog, FileEntry, FileEntryType, SymlinkTargetType, TransferDirection,
    TransferState,
};
...
fn initialize_app(app: &mut warpui::App) {
    ...
    let temp_db = std::env::temp_dir().join("ashide_sftp_test.sqlite");
...
/// 验证 DownloadSaveAs 路径不存在时不 panic 且不创建孤立任务
#[test]
fn test_download_save_as_missing_remote_path() {
    warpui::App::test((), |mut app| async move {
        initialize_app(&mut app);
        let (_, view) = create_view(&mut app);
        ...
        view.update(&mut app, |view, ctx| {
            view.handle_action(
                &SftpBrowserAction::DownloadSaveAs {
                    remote_path: PathBuf::from("/missing.txt"),
...
/// 验证 DownloadSaveAs 在空条目列表不 panic
#[test]
...
#[test]
fn download_save_as_revalidates_stable_path_after_listing_reorder() {
    warpui::App::test((), |mut app| async move {
        initialize_app(&mut app);
        let (_, view) = create_view(&mut app);
        let requested_path = PathBuf::from("/remote/requested.txt");

        view.update(&mut app, |view, ctx| {
            view.entries = vec![
                FileEntry {
                    name: "inserted-before-picker-return.txt".to_string(),
                    path: PathBuf::from("/remote/inserted-before-picker-return.txt"),
                ...
                FileEntry {
                    name: "requested.txt".to_string(),
                    path: requested_path.clone(),
                    file_type: FileEntryType::Symlink,
                    symlink_target_type: Some(SymlinkTargetType::File),
                    size: 1,
                    modified: None,
                    permissions: None,
                    directory_identity: None,
        ...
        view.read(&app, |view, _| {
            assert!(
                ...
                "picker return must revalidate the current entry kind by stable path"
            );
        });
    });
}
...
fn test_render_with_transfer_task() {
    warpui::App::test((), |mut app| async move {
        ...
        let local_file = local_dir.path().join("x.txt");
        std::fs::write(&local_file, b"content").expect("创建上传测试文件失败");
        initialize_app(&mut app);
        let (_, view) = create_view(&mut app);
...
fn test_render_all_overlays_combined() {
    warpui::App::test((), |mut app| async move {
        let local_dir = tempfile::tempdir().expect("创建本地临时目录失败");
        let local_file = local_dir.path().join("test.txt");
        std::fs::write(&local_file, b"content").expect("创建上传测试文件失败");
        initialize_app(&mut app);
        let (_, view) = create_view(&mut app);
...
#[test]
fn sftp_refresh_out_of_order_preserves_latest_snapshot() {
    warpui::App::test((), |mut app| async move {
        initialize_app(&mut app);
        let (_, view) = create_view(&mut app);
        ...
        view.update(&mut app, |view, ctx| {
            ...
            view.listing_generation = 2;
            view.apply_listing_completion(
                0,
        ...
        view.read(&app, |view, _| {
            ...
            assert_eq!(view.entries[0].name, "latest.txt");
            assert_eq!(view.selected, std::collections::HashSet::from([0]));
            assert!(view.is_loading);
        });
    });
}
