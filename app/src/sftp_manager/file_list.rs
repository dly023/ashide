...
/// 根据文件条目类型返回对应图标
pub fn file_icon(entry: &FileEntry) -> Icon {
    match entry.file_type {
        FileEntryType::Symlink => Icon::Link,
...
pub fn render_file_row(
    ...
    let icon_color = if entry.is_directory_like() {
    ...
    let is_directory_like = entry.is_directory_like();
    ...
    Hoverable::new(mouse_handle, move |_| {
        ...
        // 大小
        let size_text = if is_directory_like {
            String::from("--")
        } else {
            format_size(size)
        };
