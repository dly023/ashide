...
fn render_delete_confirm(
    ...
    let desc = if count == 1 {
        ...
        crate::t!("sftp-delete-description-one", name = name)
    } else {
        crate::t!("sftp-delete-description-many", count = count)
    };

    render_confirm_dialog(
        crate::t_static!("sftp-delete-title"),
        &desc,
        crate::t_static!("common-delete"),
...
fn render_rename(
    ...
    let title_bar = render_title_bar(
        crate::t_static!("common-rename"),
        appearance,
        close_btn_state,
    );

    // 当前名称提示
    let hint = crate::t!("sftp-current-name", name = original_name);
...
fn render_overwrite_confirm(
    ...
    let desc = match direction {
        TransferDirection::Upload => {
            format!("Remote file {target_name} already exists. Overwrite?")
        }
        TransferDirection::Download => {
            format!("Target file {target_name} already exists. Overwrite?")
        }
    };
...
pub fn render_dialog(
    ...
    match dialog {
        ...
        Dialog::CloseTransferPanelConfirm => render_confirm_dialog(
            "Close Transfers",
            "Transfers are still running. Closing will cancel them and clear the list. Close anyway?",
            "Close",
