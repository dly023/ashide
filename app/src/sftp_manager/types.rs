...
/// 符号链接解析后的目标类型。与 `FileEntryType` 分开存储，避免把
/// symlink-to-directory 降格成普通目录、或把 symlink-to-file 当目录打开。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub struct DeleteDirectoryIdentity {
    pub device: u64,
    pub inode: u64,
}
...
pub struct FileEntry {
    ...
    /// 仅符号链接存在；保留链接自身类型，同时描述可导航目标。
    pub symlink_target_type: Option<SymlinkTargetType>,
    /// 文件大小（字节）
    pub size: u64,
    /// 修改时间
    ...
    /// Listing-time identity required for recursive directory deletion.
    pub directory_identity: Option<DeleteDirectoryIdentity>,
}

impl FileEntry {
    pub fn is_directory_like(&self) -> bool {
        self.file_type == FileEntryType::Directory
            || (self.file_type == FileEntryType::Symlink
                && self.symlink_target_type == Some(SymlinkTargetType::Directory))
    }

    /// 当前 byte-transfer 协议只能表示普通文件内容，不能表示 link inode。
    /// target kind 只服务打开/导航，禁止据此把 symlink 降格成可传输文件。
    pub fn is_transferable(&self) -> bool {
        self.file_type == FileEntryType::File
    }

    /// 修改/删除针对目录项自身；指向目录的符号链接也只能按链接删除，禁止递归目标。
...
pub enum Dialog {
    DeleteConfirm {
        ...
        is_dirs: Vec<bool>,
        directory_identities: Vec<Option<DeleteDirectoryIdentity>>,
    },
    Rename {
        path: PathBuf,
...
mod tests {
    ...
    #[test]
    fn sftp_transfer_policy_rejects_symlink_entries() {
        let regular = FileEntry {
            name: "file".to_string(),
        ...
        assert!(regular.is_transferable());

        for target_type in [
            SymlinkTargetType::File,
            SymlinkTargetType::Directory,
            SymlinkTargetType::Missing,
            ...
            assert!(!symlink.is_transferable());
        }
    }
    ...
    fn test_dialog_delete_confirm_empty_paths() {
        let dialog = Dialog::DeleteConfirm {
            paths: vec![],
            is_dirs: vec![],
            directory_identities: vec![],
        };
        assert!(matches!(dialog, Dialog::DeleteConfirm { .. }));
    }

    #[test]
    fn symlink_target_type_controls_navigation_not_mutation() {
        let entry = FileEntry {
            name: "link".into(),
            path: PathBuf::from("/link"),
            file_type: FileEntryType::Symlink,
            symlink_target_type: Some(SymlinkTargetType::Directory),
            size: 0,
            modified: None,
            permissions: None,
            directory_identity: None,
