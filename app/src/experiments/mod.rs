//! Local feature rollout buckets for Ashide.
...
lazy_static! {
    ...
    /// it from their local identity key each time. Also keeps track of experiment overrides.
    ...
    // and local identity key persist even on logout, which may not be the correct behavior.
...
impl Layer {
    ...
    /// Determines the assigned bucket based on a hash of the local identity key. The
    /// returned bucket will be in the range 0-999 (inclusive) and is deterministic.
    fn assigned_bucket(&self, local_identity_key: &str) -> u16 {
        let (seed_1, seed_2) = self.hasher_seeds();
        let hash = {
            let mut hasher = siphasher::sip::SipHasher::new_with_keys(seed_1, seed_2);
            hasher.write(local_identity_key.as_bytes());
            hasher.finish()
        };
        (hash % 1000) as u16
    }

    /// Returns the experiment group that the local identity key is assigned to in this
    /// layer, if it exists. Returns None otherwise.
    fn get_assigned_group<T>(&self, local_identity_key: &str) -> Option<T>
    where
        T: Experiment<T>,
        <T as FromStr>::Err: fmt::Debug,
    {
        let bucket = self.assigned_bucket(local_identity_key);
...
pub trait Experiment<T: Experiment<T>>: FromStr {
        ...
        <T as FromStr>::Err: fmt::Debug,
    {
        ...
        // If there was no override, derive the assignment from the user's local identity key.
        if assigned_group.is_none() {
            let local_identity_key = AuthStateProvider::as_ref(ctx).get().local_identity_key();
            assigned_group = Self::layer().get_assigned_group(&local_identity_key);
        }
        ...
        if let Some(group) = assigned_group.as_ref() {
            ...
            {
                let tag_name = format!("ashide.experiments.{}", Self::name());
