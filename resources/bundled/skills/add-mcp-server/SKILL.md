---
name: agent-add-mcp
description: Use this skill when helping users add MCP servers to their Ashide configuration.
---

# Adding MCP Servers to Ashide

Ashide supports MCP servers via native config files. Follow these steps when helping a user add an MCP server.
...
Config file paths:
- **Global (user-scoped):** `~/.ashide/.mcp.json`
- **Project-scoped:** `{repo_root}/.ashide/.mcp.json`
...
By default, Ashide spawns stdio servers from the directory the config was discovered in:
- Project-scoped configs (`{repo_root}/.ashide/.mcp.json`) run from the repo root.
- Global configs (`~/.ashide/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
...
For environment variables containing secrets, use `${VAR_NAME}` syntax — Ashide will substitute the value from the user's environment at runtime.

## Notes

- Ashide auto-detects changes to `.mcp.json` files on save — no restart required.
- Configured servers appear in Ashide's Settings under MCP, labeled **"Detected from Ashide"**.
