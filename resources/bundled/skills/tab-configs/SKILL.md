---
name: tab-configs
description: Reference the Ashide tab config schema, validation rules, and examples. Use when creating or updating Ashide tab config TOML files or when another tab-config skill needs the canonical schema details.
---

# tab-configs

This skill is the canonical reference for Ashide tab config TOML files.

Tab configs live under `~/.ashide/tab_configs/` for standard Ashide builds. Non-stable builds (e.g. Preview or pre-release channels) use a channel-specific variant such as `~/.ashide-<channel>/tab_configs/`. Do NOT use `~/Library/Application Support/` — that is the wrong location.

To find the active data directory, inspect which `~/.ashide*` directories exist on the user's machine. Each `.toml` file defines a single tab layout that appears in the `+` menu. When the user selects one, Ashide opens a new tab with the configured pane layout and runs any specified commands.
...
- When creating or locating tab configs, use the correct tab config directory for the user's current Ashide build/channel rather than hardcoding a single base directory.
...
`{{autogenerated_branch_name}}` is a special-cased reserved template variable: if a tab config references it, Ashide generates a unique worktree branch name on each open instead of prompting the user for that value.
...
### With parameters
Ashide-generated worktree configs default to `~/.ashide/worktrees/<repo-name>/<worktree-name>`. When writing a reusable config by hand, derive the repo-name segment at shell runtime with `$(basename {{repo}})`.
...
directory = "{{repo}}"
commands = [
  "git worktree add -b {{branch_name}} $HOME/.ashide/worktrees/$(basename {{repo}})/{{branch_name}} {{base_branch}}",
  "cd $HOME/.ashide/worktrees/$(basename {{repo}})/{{branch_name}}",
]
