...
Param (
    ...
    # Builds only the Ashide binary, skips the installer.
    [Switch]$SKIP_BUILD_INSTALLER = $False,
    # Builds only the installer, skips the Ashide binary. Use this if the Ashide
...
if ($ARCH -eq 'arm64') {
    ...
    # If x64, then we just use the filename "AshideSetup.exe" for example
    $FILE_ENDING = 'Setup'
...
if ("$CHANNEL" -eq 'local') {
    ...
    # autoupdate 走 GitHub Release(OpenPawz/ashide),仅下载到 Downloads,不调 Inno Setup。
...
# 静默吞掉 toast。Ashide 所有 channel 都使用 `dev.ashide.<Name>`。
$AUMID = "dev.ashide.$APP_NAME"
$BUNDLE_ID = $AUMID
...
$PDB_PATH = "$CARGO_TARGET_OUTPUT_DIR\$ASHIDE_BIN.pdb"
...
if ($CHECK_ONLY) {
    cargo check -p warp --profile "$CARGO_PROFILE" --bin "$ASHIDE_BIN" --features "$FEATURES" --target $PLATFORM_TARGET
    if (-Not $?) {
        Write-Error "Failed to verify Ashide $ASHIDE_BIN compilation with profile $CARGO_PROFILE"
        exit 1
    }
    exit 0
}
...
if (-Not $SKIP_BUILD_BINARY) {
    ...
    if ("$ASHIDE_BIN.exe" -ne $BINARY_NAME) {
        $binarySource = "$CARGO_TARGET_OUTPUT_DIR\$ASHIDE_BIN.exe"
        Write-Output "Renaming executable $ASHIDE_BIN.exe to $BINARY_NAME"
...
Write-Output 'Building Ashide installer'
# Inno Setup `AppId` 决定注册表 Uninstall 条目与升级跟踪键。OSS 下固定为 `ashide`;
# 其他 channel 使用 Ashide 专属 AppId。
if ("$CHANNEL" -eq 'oss') {
    $INNO_APP_ID = 'ashide'
} else {
    $INNO_APP_ID = "ashide-terminal-$CHANNEL"
}
$ISCC_ARGS = @(
