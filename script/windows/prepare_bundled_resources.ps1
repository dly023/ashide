...
$CargoAboutVersion = if (Get-Command cargo-about -ErrorAction SilentlyContinue) { cargo about --version } else { '' }
if ($CargoAboutVersion -ne 'cargo-about 0.9.1') {
    Write-Error "cargo-about 0.9.1 is required, found: $CargoAboutVersion"
    exit 1
}
Write-Output 'Fetching complete locked workspace dependency graph for license metadata'
cargo fetch --locked --manifest-path "$RepoRoot\Cargo.toml"
if (-Not $?) {
    Write-Error 'Failed to fetch complete locked workspace dependency graph'
    exit 1
}
Write-Output "Generating third-party licenses at $LicensesOutput"
cargo about generate --workspace --frozen --fail --manifest-path "$RepoRoot\Cargo.toml" -c "$RepoRoot\about.toml" -o "$LicensesOutput" "$RepoRoot\about.hbs"
if (-Not $?) {
