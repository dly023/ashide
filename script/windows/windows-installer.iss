...
#define MyAppPublisher "Ashide"
#define MyAppURL "https://github.com/OpenPawz/ashide"
#ifndef MyAppName
  #define MyAppName "AshideDev"
#endif
#ifndef MyAppVersion
  #define MyAppVersion "0.1.0"
#endif
#ifndef MyAppExeName
  #define MyAppExeName "ashide-dev.exe"
#endif
#ifndef ReleaseChannel
  #define ReleaseChannel "dev"
#endif
#ifndef AppUserModelId
  #define AppUserModelId "dev.ashide." + MyAppName
#endif
...
#define ChannelPascalCase \
  ...
  "Unknown")))))
#define AppMutexName "Local\Ashide" + ChannelPascalCase + "_SingleInstance"


[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
; bundle.ps1 会为 OSS 传入 `InnoAppId=ashide`,其他 channel 走默认的 `ashide-terminal-{ReleaseChannel}`。
#ifndef InnoAppId
  #define InnoAppId "ashide-terminal-" + ReleaseChannel
#endif
AppId={#InnoAppId}
AppName={#MyAppName}
...
WizardStyle=modern
WizardSmallImageFile="installer-images\ashide-logo.bmp"
WizardImageFile="installer-images\ashide-banner.bmp"
SetupIconFile="..\..\app\channels\{#ReleaseChannel}\icon\padded\icon.ico"
UninstallDisplayIcon="{app}\icon.ico"
; Force close previous Ashide if it hasn't shut down yet.
; In the update flow we already warn the user if they have something running and make them confirm
; before running this installer. Therefore, we are good to force close Ashide without fear of losing
; unsaved work.
...
; For manual installs: if Ashide is running, show a dialog prompting the user to close it
...
[Registry]
Root: HKCU; Subkey: "SOFTWARE\Ashide.dev\{#MyAppName}"; Flags: uninsdeletekey
; cleanup "Open Ashide Here" registry entries
...
; Add "Open Ashide in new tab" to directory context menu
...
; Add "Open Ashide in new tab" to directory background context menu
...
; Add "Open Ashide in new window" to directory context menu
...
; Add "Open Ashide in new window" to directory background context menu
...
[Tasks]
Name: addToPath; Description: "Add Ashide to PATH"

[UninstallDelete]
Type: filesandordirs; Name: "{userappdata}\ashide\{#MyAppName}"
Type: filesandordirs; Name: "{localappdata}\ashide\{#MyAppName}"
...
{ Returns true when the installer was launched by Ashide's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Ashide process. Instead we
  wait for Ashide to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Ashide first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Ashide was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Ashide to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Ashide to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Ashide mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. ashide.exe, ashide-dev.exe) is unique
          ...
          rely on Ashide's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
          cascade-kill any child OpenConsole.exe processes when Ashide dies. }
        ...
        Log('Ashide has exited; proceeding with file installation.');
    end;
  end;

  { After a successful install, write a helper script for running the Ashide CLI. }
  { After a successful install, add Ashide to PATH if requested. }
  if CurStep = ssPostInstall then begin
    { Add Ashide to PATH if requested }
