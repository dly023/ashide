...
a configuration file for building an Ashide installer.
...
* Run `cargo build` to ensure the installer uses the latest version of Ashide.
...
```shell
.\script\windows\Output\Ashide-Windows-Setup.exe`.
```
...
* `MyAppExeName` (default: `ashide.exe`)
