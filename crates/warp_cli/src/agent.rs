...
#[derive(Debug, Copy, Clone, ValueEnum, Eq, PartialEq, Default)]
pub enum Harness {
    /// Use Ashide's built-in agent infrastructure (default).
    #[default]
    #[value(name = "oz")]
    Oz,
...
impl Harness {
    ...
    pub fn parse_current_app_child_harness(value: &str) -> Option<Self> {
...
pub struct RunAgentArgs {
    ...
    /// Display agent progress in the Ashide interface.
    ...
    // Ashide hard-cut:`--environment` 参数随 cloud ambient agent 主体子系统物理删。
    ...
    ///
    /// "oz" (default) uses Ashide's built-in agent infrastructure.
