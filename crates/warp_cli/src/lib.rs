...
pub mod completions;
pub mod config_file;
// Ashide hard-cut:`environment` CLI 随 cloud ambient agent 主体子系统物理删。
pub mod json_filter;
pub mod mcp;
pub mod memory;
pub mod model;
...
/// Options related to the parent process that spawned this Ashide instance.
#[derive(Debug, Default, Clone, clap::Args)]
pub struct ParentOpts {
    /// The ID of the Ashide process that spawned this one.
    ///
    /// Used by codepaths that attempt to detect when the parent Ashide process
    /// has terminated. Guaranteed to be [`None`] when this is the initial
    /// Ashide process, but may also be [`None`] for Ashide child processes if the
...
/// Hidden worker args used to scope environment-runtime proxy/daemon sockets by
/// Ashide identity without exposing credentials.
#[derive(Debug, Clone, Default, clap::Args)]
pub struct EnvironmentRuntimeIdentityArgs {
    /// Non-secret identity partition key for the environment-runtime daemon.
...
/// Command-line argument parser for the main Ashide binary. This is used across all channels.
#[derive(Debug, Default, Parser, Clone)]
#[command(
    name = "oz",
    display_name = "Ashide",
    about = r#"Ashide local agent CLI

The Ashide CLI is a tool for running and managing local coding agents.
Use the CLI to:
...
/// Flags for the Ashide application. Additional binaries, like test runners, may use this type
/// along with their own flags, or convert their flags into an `AppArgs` value.
#[derive(Debug, Default, clap::Args, Clone)]
pub struct AppArgs {
    /// True if this instance of Ashide was launched at the end of the auto-update process.
...
impl Args {
    ...
    pub fn from_env() -> Self {
        cfg_if::cfg_if! {
            ...
            if #[cfg(target_family = "wasm")] {
                ...
                // Ashide hard-cut:`warp environment` 子命令随 cloud ambient agent 主体物理删 ——
    ...
    pub fn clap_command() -> clap::Command {
        ...
        // Ashide hard-cut:`environment` 子命令与 `--environment` 参数随 cloud ambient agent
...
"#
    ...
    /// Args for the main Ashide application, if not running a subcommand.
    pub fn app_args(&self) -> &AppArgs {
        &self.args
    }

    /// Extract the main Ashide application args.
...
/// Ashide may spawn several worker processes - mostly servers that support the main application.
///
/// These subcommands run those worker processes, which are bundled into the Ashide binary.
...
pub enum WorkerCommand {
    ...
    /// Run the environment runtime proxy over SSH stdio.
    /// Ensures the daemon is running, then bridges its stdin/stdout
    /// to the daemon via a Unix domain socket.
    #[cfg(not(target_family = "wasm"))]
    #[clap(hide = true)]
    EnvironmentRuntimeProxy(EnvironmentRuntimeIdentityArgs),

    /// Run the long-lived environment runtime daemon.
    /// Listens on a Unix domain socket and accepts multiple concurrent
    /// connections from proxy processes.
    #[cfg(not(target_family = "wasm"))]
    #[clap(hide = true)]
    EnvironmentRuntimeDaemon(EnvironmentRuntimeIdentityArgs),
...
/// CLI-related subcommands. The command-line interface to Ashide isn't a full SDK (e.g. with language bindings),
/// but it allows scripting some Ashide functionality.
#[derive(Debug, Clone, Subcommand)]
pub enum CliCommand {
    /// Interact with Ashide Agent.
    #[command(subcommand)]
    Agent(crate::agent::AgentCommand),

    /// Recall project memory from `.agents/memory`.
    Recall(crate::memory::MemoryRecallArgs),

    /// Remember a durable project fact, decision, task, failure, or preference.
    Remember(crate::memory::RememberArgs),

    // Ashide hard-cut:`Environment` variant 随 cloud ambient agent 主体子系统物理删。
    /// Manage MCP servers.
    #[command(subcommand)]
    ...
    #[command(subcommand)]
    Memory(crate::memory::MemoryCommand),
    ...
    /// Inspect and export local Ashide sessions.
    #[command(name = "session-bridge", subcommand)]
    SessionBridge(crate::session_bridge::SessionBridgeCommand),
...
/// A subcommand of the main Ashide application. This includes all [`WorkerCommand`]s as well as app-specific debugging tools.
#[derive(Debug, Clone, Subcommand)]
pub enum Command {
    #[clap(flatten)]
    Worker(WorkerCommand),

    /// Commands that make up the Ashide CLI.
    #[clap(flatten)]
    ...
    ///
    /// If no shell is provided, this defaults to the shell that Ashide was run from.
...
/// Returns a flag that sets the current process as the parent of an Ashide subcommand to spawn.
