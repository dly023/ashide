...
pub struct RegistryBackedPreferences {
    app_key_path: String,
    /// 缓存 `HKCU\Software\Ashide\<channel>` 注册表 Key 句柄。
    ///
    /// Ashide 启动时会顺序调用 ~100 个 setting 的 `read_value`,
...
static REGISTRY_BASE_PATH: &str = "Software\\Ashide\\";
...
impl RegistryBackedPreferences {
    ...
    pub fn new(app_name: &str) -> Self {
        let app_key_path = REGISTRY_BASE_PATH.to_owned() + app_name;
    ...
    /// 用回调操作缓存的 Ashide 注册表 Key。第一次会 `CURRENT_USER.create(...)`,
        ...
        let mut guard = match self.cached_key.lock() {
            ...
            Err(_) => {
                let key = CURRENT_USER
                    ...
                    .map_err(|e| {
                        log::error!("unable to access Ashide app key in Windows Registry: {e:#}");
