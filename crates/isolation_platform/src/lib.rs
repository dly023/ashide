...
/// Environment variable containing the generic Ashide-managed workload token that we use
/// for isolation platforms that don't issue their own tokens.
#[cfg(not(target_family = "wasm"))]
const WARP_WORKLOAD_TOKEN_ENV: &str = "WARP_WORKLOAD_TOKEN";

/// A kind of isolation platform. For our usage, isolation platforms are different ways where Ashide
...
pub enum IsolationPlatformType {
    /// Ashide is running within a Docker container. Note that this does *not* mean this is a Ashide-hosted
    /// Docker Sandboxes environment. Instead, it's likely a self-hosted agent.
    #[cfg(not(target_family = "wasm"))]
    Docker,
    /// Ashide is running within a Docker Sandbox, likely as a Ashide-hosted agent.
    #[cfg(not(target_family = "wasm"))]
    DockerSandbox,
    /// Ashide is running within a Kubernetes pod, likely as a self-hosted agent.
    #[cfg(not(target_family = "wasm"))]
    Kubernetes,
    /// Ashide is running within a Namespace instance, likely as a Ashide-hosted agent.
    #[cfg(not(target_family = "wasm"))]
    Namespace,
}
