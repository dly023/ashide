...
pub enum FeatureFlag {
    ...
    /// Enable multiselect in Notebooks and rich text surfaces.
    RichTextMultiselect,
    ...
    /// Enables next action prediction within Ashide, powered by AI.
    AgentPredict,
    ...
    /// Playground for reducing Ashide UI clutter.
    MinimalistUI,
    ...
    /// Adds aliases for executing Ashide Drive workflows.
    WorkflowAliases,
    ...
    /// Enables support for Ashide packs.
    WarpPacks,
    ...
    /// Enables Ashide Drive objects (like workflows) as context in AI context menu
    ...
    /// Enables saving ambient plans into the local Drive.
    SaveAmbientPlansToLocalDrive,
    ...
    /// Enables the Ashide launch modal announcing the open-source build.
    ...
    /// Ctrl-G intercepts the keystroke and opens Ashide's input editor instead of $EDITOR.
    CLIAgentRichInput,
    ...
    /// Enables Ashide's vertical Session Navigator.
    VerticalTabs,
    ...
    /// CLIs (e.g. `claude`) to execute prompts instead of Ashide's agent harness.
    AgentHarness,

    /// Enables the upgraded CLI agent session tracking and notifications infrastructure.
    HOANotifications,

    /// Enables the install/update chip for the OpenCode Ashide notification plugin.
    /// Requires HOANotifications to also be enabled.
    OpenCodeNotifications,

    /// Enables the install/update chip for the Codex Ashide notification plugin.
    /// Requires HOANotifications to also be enabled.
    CodexNotifications,

    /// Enables the install/update chip for the Gemini CLI Ashide extension.
    /// Requires HOANotifications to also be enabled.
    GeminiNotifications,
    ...
    /// Replaces the in-block command-help banner with a footer.
    WarpifyFooter,
    ...
    /// Gates the environment runtime flow that installs and connects to a
    /// persistent helper on the environment machine instead of using
    /// ControlMaster for command execution.
    EnvironmentRuntime,
...
/// Features enabled for feature preview build users.
/// All PREVIEW_FLAGS are also automatically added to dogfood builds (AshideDev).
pub const PREVIEW_FLAGS: &[FeatureFlag] = &[
    FeatureFlag::MarkdownTables,
    FeatureFlag::GitOperationsInCodeReview,
];

/// Features enabled for all release builds (i.e.: everything but AshideLocal).
...
pub const RELEASE_FLAGS: &[FeatureFlag] = &[
    ...
    // 否则只能看到 OS 的候选窗,Ashide 会把 marked text 更新整体丢弃。
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    FeatureFlag::ImeMarkedText,
    FeatureFlag::BlocklistMarkdownTableRendering,
    // Ashide launches with the Session Navigator as the primary left tab rail.
    FeatureFlag::VerticalTabs,
    // Environment runtime helper is not yet supported on Windows.
    #[cfg(not(windows))]
    FeatureFlag::EnvironmentRuntime,
    // Ashide:远程文件浏览器 / 项目管理器对 oss release 默认开放。
    // 与 EnvironmentRuntime 同步开启:它依赖 environment runtime transport。
    #[cfg(not(windows))]
    FeatureFlag::ServerFileBrowser,
];
