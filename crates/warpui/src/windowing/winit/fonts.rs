...
// We use font-kit's family handle to load fonts that come with Ashide as
...
impl FontDB {
    pub fn new() -> Self {
        ...
        // 该回归(dly023/ashide#68)也仅出现在 Windows。macOS/Linux 走各自的 fallback 路径,
        // 不需预热。
        #[cfg(target_os = "windows")]
...
/// "restart Ashide" 提示驱动),用户已经预期会有一次中断。如果之后需要热重载低延迟,
