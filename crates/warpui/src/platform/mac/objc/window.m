...
static BOOL event_targets_standard_window_button(NSWindow *window, NSEvent *event) {
    NSWindowButton buttonKinds[] = {
        NSWindowCloseButton,
...
@implementation WarpWindow {
    ...
    // Left-click tracking that began on a native macOS traffic light. Those events must stay in
    // AppKit; routing the mouse-up through WarpHostView steals close/minimize/zoom actions.
    BOOL _routingNativeWindowButtonClick;
    // macOS 27: a left-click that began over a window resize edge must let AppKit
    // drive the resize (route subsequent drag/up to super), not WarpHostView.
    BOOL _leftMouseDownStartedOnResizeEdge;
}
...
- (void)sendEvent:(NSEvent *)event {
    switch (event.type) {
        case NSEventTypeLeftMouseDown:
            _routingNativeWindowButtonClick = event_targets_standard_window_button(self, event);
            _leftMouseDownStartedOnResizeEdge =
                !_routingNativeWindowButtonClick && [self eventIsOverResizeEdge:event];
        ...
        case NSEventTypeLeftMouseUp:
            if (_routingNativeWindowButtonClick) {
                [super sendEvent:event];
                _routingNativeWindowButtonClick = NO;
            } else if (_leftMouseDownStartedOnResizeEdge && @available(macOS 27, *)) {
                [super sendEvent:event];
            } else {
                [self.contentView mouseUp:event];
            }
            _leftMouseDownStartedOnResizeEdge = NO;
            break;
