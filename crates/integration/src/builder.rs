...
impl Builder {
    pub fn new() -> Self {
        ...
        Self {
            ...
            public_settings_toml: String::new(),
        }
    }
    ...
    pub fn with_private_user_defaults(mut self, user_defaults: HashMap<String, String>) -> Self {
        self.user_prefs.extend(user_defaults);
        self
    }

    /// 向测试 HOME 下的 `settings.toml` 写入公开设置。
    ///
    /// `with_private_user_defaults` 只适用于 private/native preferences；所有
    /// `private: false` 设置必须走本入口，避免测试写入错误的持久化权威。
    pub fn with_public_settings_toml(mut self, settings: impl AsRef<str>) -> Self {
        assert!(
            self.public_settings_toml.is_empty(),
            "with_public_settings_toml must receive one complete TOML document"
        );
        self.public_settings_toml = settings.as_ref().to_owned();
        self
    }
    ...
    pub fn build(self, test_name: &str, create_temp_dir_for_test: bool) -> TestDriver {
        ...
        if !public_settings_toml.is_empty() {
            let settings_path = warp::settings::user_preferences_toml_file_path();
            if let Some(parent) = settings_path.parent() {
                std::fs::create_dir_all(parent)
                    .expect("should create the public settings directory");
            }
            std::fs::write(&settings_path, public_settings_toml)
                .expect("should write initial public settings");
        }
