...
/// The Ashide integration test runner.
...
pub fn main() -> Result<()> {
    ...
    cfg_if::cfg_if! {
        if #[cfg(unix)] {
            ...
            let database_path = warp::integration_testing::persistence::database_file_path();
            assert!(
                database_path.starts_with(&home),
                "integration database must be isolated under HOME: HOME={home:?}, database_path={database_path:?}",
            );
        } else if #[cfg(windows)] {
            ...
            let database_path = warp::integration_testing::persistence::database_file_path();
            assert!(
                database_path.starts_with(&userprofile),
                "integration database must be isolated under USERPROFILE: USERPROFILE={userprofile:?}, database_path={database_path:?}",
            );
        } else {
...
fn register_tests() -> HashMap<&'static str, BoxedBuilderFn> {
    ...
    // Ashide 本地化: websocket 集成测试随 Listener 物理删除。
    ...
    register_test!(test_ai_code_block_uses_terminal_theme_screenshot);
