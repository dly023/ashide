...
use warp::{
    ...
    integration_testing::{
        ...
        terminal::{
            ...
            assert_input_not_at_either_edge_of_terminal,
            assert_long_running_block_executing_for_single_terminal_in_tab, assert_model_term_mode,
            assert_no_block_executing, assert_pane_group_has_state, assert_scroll_position,
            ...
            assert_snackbar_is_visible, assert_terminal_current_app_path,
            assert_view_has_text_selection, assert_waterfall_gap_empty_background_rendered,
...
use warp::integration_testing::local_drive::assert_is_left_panel_open;
...
use warp::{
    integration_testing::{
        ...
        terminal::{
            ...
            open_context_menu_for_selected_block, open_prompt_context_menu_for_input,
            open_rprompt_context_menu_for_input,
        },
    },
...
use warp::{
    ...
    workspace::{Workspace, NEW_SESSION_MENU_BUTTON_POSITION_ID},
};
...
pub fn test_suggestions_menu_positioning() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Open Ashide Drive")
...
pub fn test_waterfall_input_alt_grid() -> Builder {
    let mut builder = new_builder()
        .with_public_settings_toml(public_settings::input_mode(InputMode::Waterfall))
        .with_step(
...
pub fn test_waterfall_input() -> Builder {
    new_builder()
        .with_public_settings_toml(public_settings::input_mode(InputMode::Waterfall))
        .with_private_user_defaults(HashMap::from([
...
pub fn test_removing_tabs_out_of_order() -> Builder {
    new_builder()
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(
            new_step_with_default_assertions("Add a second tab")
                .with_keystrokes(&[cmd_or_ctrl_shift("t")])
                .add_assertion(assert_tab_count(2)),
        )
        .with_step(wait_until_bootstrapped_single_pane_for_tab(1))
        .with_step(
            new_step_with_default_assertions("Add a third tab")
                .with_keystrokes(&[cmd_or_ctrl_shift("t")])
...
pub fn test_add_and_close_session() -> Builder {
    ...
    new_builder()
        .with_setup(|_| {
            ...
            std::fs::write(&path, "[general.undo_close]\nenabled = false\n")
                .expect("undo-close test setting should be written");
        })
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(
            new_step_with_default_assertions(
...
pub fn test_ctrl_c() -> Builder {
    const CTRL_C_READY_MARKER: &str = "__ASHIDE_CTRL_C_READY__";
    let signal_target = format!(
        r#"sh -c "trap 'exit 130' INT; printf '%s\n' '{CTRL_C_READY_MARKER}'; while :; do sleep 1; done""#
    );
    ...
    new_builder()
        ...
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(
            TestStep::new("Run signal-ready command")
                .add_named_assertion("no pending model events", assert_no_pending_model_events())
                .with_input_string(&signal_target, Some(&["enter"]))
                .add_named_assertion(
                    "signal target ready",
                    assert_active_block_output_for_single_terminal_in_tab(
                        ExactLine::from(CTRL_C_READY_MARKER),
                        0,
                    ),
                )
                .add_assertion(
        ...
        .with_step(
            TestStep::new("Check ctrl-c terminates the command")
                .with_keystrokes(&["ctrl-c"])
                .add_named_assertion("no pending model events", assert_no_pending_model_events())
                .add_named_assertion("no block executing", assert_no_block_executing(0, 0))
                .add_assertion(|app, window_id| {
                    let terminal_view = single_terminal_view_for_tab(app, window_id, 0);
                    terminal_view.read(app, |view, _ctx| {
                        let model = view.model.lock();
...
pub fn test_hover_over_menu() -> Builder {
    const SAMPLES_PER_AXIS: usize = 25;
    let mut hover_window_grid =
        new_step_with_default_assertions("Hover over a deterministic window-sized grid");
    for x_index in 0..SAMPLES_PER_AXIS {
        for y_index in 0..SAMPLES_PER_AXIS {
            hover_window_grid = hover_window_grid.with_event_fn(move |app, window_id| {
                let size = app
                    .window_bounds(&window_id)
                    .expect("integration window must have bounds")
                    .size();
                let denominator = (SAMPLES_PER_AXIS - 1) as f32;
                Event::MouseMoved {
                    position: Vector2F::new(
                        (size.x() - 1.).max(0.) * x_index as f32 / denominator,
                        (size.y() - 1.).max(0.) * y_index as f32 / denominator,
                    ),
                    cmd: false,
                    shift: false,
                    is_synthetic: false,
...
precmd_functions+=(_p9k_precmd)
        ...
        // If the user then switches back to the Ashide prompt, we should close the banner.
        .with_step(
...
pub fn test_block_based_snackbar_appears_for_running_command_input_at_bottom() -> Builder {
    new_builder()
        .with_public_settings_toml(public_settings::input_mode(InputMode::PinnedToBottom))
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
...
pub fn test_block_based_snackbar_appears_for_running_command_pinned_to_top() -> Builder {
    new_builder()
        .with_public_settings_toml(public_settings::input_mode(InputMode::PinnedToTop))
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(execute_long_running_command(
            0,
            format!("{} && python3", long_block_command()),
...
/// Default auto title. We test that Ashide's auto title is used and verify that that
...
/// Validate that disabling Ashide's auto title feature will not mess with oh-my-zsh settings.
...
"#,
        ...
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        // If Ashide title is disabled, we don't set the DISABLE_AUTO_TITLE env variable
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "echo $DISABLE_AUTO_TITLE".to_string(),
            ExpectedExitStatus::Success,
...
/// Checks that the tab title set by the user takes precedence over Ashide's default title and
                        ...
                        .read(ctx, |pane_group, ctx| {
                            for pane_index in 0..=2 {
                                let actual_path = pane_group
                                    ...
                                    .read(ctx, |view, ctx| {
                                        view.active_session_current_app_path(ctx)
                                    });
...
pub fn test_create_session_with_new_tab_while_bootstrapping() -> Builder {
    ...
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Wait for parent working directory projection")
                .add_assertion(assert_terminal_current_app_path(0, parent_cwd)),
        )
        .with_step(
            new_step_with_default_assertions(
                    ...
                    for tab_view in workspace.tab_views() {
                        let tab_path = tab_view.read(ctx, |pane_group, ctx| {
                            pane_group
                                ...
                                .read(ctx, |view, ctx| view.active_session_current_app_path(ctx))
                        });
...
pub fn test_completions_as_you_type() -> Builder {
    new_builder()
        ...
        .with_public_settings_toml("[terminal.input]
...
",
        ...
        .with_step(
            new_step_with_default_assertions("Setup buffer to have dir with missing slash")
                .add_named_event_readiness_assertion(
                    "target input editor focused before typing",
                    assert_focused_editor_in_tab(0),
                )
                .with_typed_characters(&["cd app"])
                .add_named_assertion("terminal input received 'cd app'", |app, window_id| {
                    let input_view = single_input_view_for_tab(app, window_id, 0);
                    input_view.read(app, |view, ctx| {
                        async_assert_eq!(view.buffer_text(ctx), "cd app")
                    })
                })
                .add_assertion(|app, window_id| {
                    let input_view = single_input_view_for_tab(app, window_id, 0);
...
pub fn test_preferred_shell() -> Builder {
    ...
    new_builder()
        .with_public_settings_toml(format!(
            "[session]\nstartup_shell_override = {}\n",
            serde_json::to_string(&custom_shell).expect("custom shell should serialize to TOML")
        ))
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
...
pub fn test_find_query_not_evaluated_on_terminal_mode_change() -> Builder {
    new_builder()
        ...
        .with_step(
            // Don't perform default assertions here, as we expect to be in the middle of an
            // executing command. Use a deterministic pager instead of depending on the checkout's
            // current git diff size, and wait for the actual terminal-mode transition.
            TestStep::new("Open pager")
                .with_typed_characters(&["seq 1000 | env LESS=FR less"])
                .with_keystrokes(&["enter"])
                .add_assertion(assert_alt_grid_active(0, 0, true)),
        )
        .with_step(
            new_step_with_default_assertions("Quit pager")
                .with_typed_characters(&["q"])
                .add_assertion(assert_alt_grid_active(0, 0, false)),
        )
        .with_step(
...
/// Regression test: prompt color overrides must not crash the completion model.
...
"#
        ...
        .with_steps(open_context_menu_for_selected_block())
        .with_step(open_prompt_context_menu_for_input())
        .with_step(
            new_step_with_default_assertions("Copy prompt copies to clipboard properly")
                .with_click_on_saved_position("Copy prompt")
...
",
        ...
        .with_step(open_rprompt_context_menu_for_input())
        .with_step(
...
pub fn test_alt_screen_context_menu_with_sgr_with_mouse_reporting() -> Builder {
    let mut builder = new_builder()
        .with_public_settings_toml("[terminal]\nmouse_reporting_enabled = true\n")
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0));
            ...
            new_step_with_default_assertions("Check the new window size")
                .add_named_assertion_with_data_from_prior_step(
                    ...
                    move |app, _, step_data_map| {
                        ...
                        async_assert!(
                            (size.x() - 992.).abs() <= 2. && (size.y() - 644.).abs() <= 2.,
                            "expected laid-out window size near 992x644, got {}x{}",
                            size.x(),
                            size.y()
                        )
                    },
                ...
                .add_named_assertion_with_data_from_prior_step(
                    ...
                    |app, _, step_data_map| {
                        ...
                        pane_group.read(app, |view, app| {
                            ...
                            async_assert!(
                                (pane_width - AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH).abs() <= 4.,
                                "expected laid-out Agent Mode pane width near {}, got {}",
                                AGENT_MODE_PANE_DEFAULT_MINIMUM_WIDTH,
                                pane_width
                            )
                        })
                    },
...
// the workspace view, but we DO force Ashide Drive open to show the dialog, so we can look for that
...
pub fn test_context_chips_prompt_at_bootstrap() -> Builder {
    new_builder()
        ...
        .with_public_settings_toml("[terminal.input]\nhonor_ps1 = false\n")
        .with_private_user_defaults(HashMap::from([(
            String::from("SavedPrompt"),
            String::from("Default"),
        )]))
        .with_step(
            new_step_with_default_assertions("Check Ashide prompt")
...
pub fn test_undo_close_stack_timeout_cleanup() -> Builder {
    ...
    new_builder()
        ...
        .with_public_settings_toml("[general.undo_close]\ngrace_period = 5\n")
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(
            new_step_with_default_assertions("Assert we have only 1 window open at start")
                .add_assertion(move |app, _| async_assert_eq!(app.window_ids().len(), 1)),
