...
/// Test that clicking a file in the file tree opens it in Ashide's editor.
/// This is a regression test for the bug where files were being opened in
/// external editors instead of Ashide's built-in editor.
pub fn test_file_tree_opens_files_in_warp() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Verify file opened in Ashide editor").add_assertion(
...
pub fn test_file_tree_non_openable_files() -> Builder {
    new_builder()
        .with_setup(|utils| {
            ...
            // Create a binary file that shouldn't be opened in Ashide
        ...
        .with_step(
            new_step_with_default_assertions("Click on binary file")
                ...
                .add_assertion(|app, window_id| {
                    // The binary file should NOT open in a new pane in Ashide
                    ...
                    pane_group.read(app, |pane_group, _ctx| {
                        async_assert_eq!(
                            pane_group.pane_count(),
                            1,
                            "Binary file should not open in Ashide, should stay at 1 pane"
                        )
                    })
                }),
        )
