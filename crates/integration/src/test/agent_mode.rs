...
fn restored_code_block_user_query_message(task_id: &str, request_id: &str) -> api::Message {
    api::Message {
        id: "restored-code-block-user-query".to_string(),
        task_id: task_id.to_string(),
        server_message_data: String::new(),
        citations: vec![],
        message: Some(api::message::Message::UserQuery(api::message::UserQuery {
            query: "Render a large JSON code block in the terminal AI block".to_string(),
            context: Some(api::InputContext {
                directory: Some(api::input_context::Directory {
                    pwd: "~".to_string(),
                    home: String::new(),
...
fn restored_code_block_agent_output_message(task_id: &str, request_id: &str) -> api::Message {
    api::Message {
        id: "restored-code-block-agent-output".to_string(),
        task_id: task_id.to_string(),
        server_message_data: String::new(),
        citations: vec![],
        message: Some(api::message::Message::AgentOutput(
            api::message::AgentOutput {
                text: concat!(
                    "Here is a large JSON code block that should inherit the terminal theme:\n\n",
                    "```json\n",
                    "[\n",
                    "  {\n",
                    "    \"is_cover\": false,\n",
                    "    \"sub_plan\": \"1格|特写镜头|首选决定→闭眼品尝\",\n",
                    "    \"edit_from\": false,\n",
                    "    \"page_brief\": \"女孩闭眼享受巧克力的美味\",\n",
                    "    \"page_description\": \"画面：特写，女孩闭着眼睛，微微仰头，将一颗棕色的巧克力放入口中，表情显得无比陶醉和享受。\\n旁白：下一个是什么味道。\",\n",
                    "    \"characters_present\": [\"女孩\"],\n",
                    "    \"inspiration_ref_id\": \"\"\n",
                    "  },\n",
                    "  {\n",
                    "    \"is_cover\": false,\n",
                    "    \"sub_plan\": \"1格|远景|陶醉品尝→真相大白\",\n",
                    "    \"edit_from\": false,\n",
                    "    \"page_brief\": \"女孩惊恐地发现自己吃的不是巧克力\",\n",
                    "    \"page_description\": \"画面：全景，女孩睁开眼，表情变得极其惊恐和扭曲。在她面前的桌子上，根本没有巧克力盒，只有一盘看起来极其诡异、长满了触手的黑色不明生物。\\n气泡：呕——！\\n旁白：确实没想到...\",\n",
                    "    \"characters_present\": [\"女孩\"],\n",
                    "    \"inspiration_ref_id\": \"\"\n",
                    "  }\n",
                    "]\n",
                    "```\n\n",
                    "This text below the code block should stay on the same dark terminal surface.\n"
                )
                .to_string(),
            },
        )),
...
fn restored_code_block_conversation_data() -> api::ConversationData {
    let task_id = "restored-code-block-theme-task";
    let request_id = "restored-code-block-theme-request";
    api::ConversationData {
        tasks: vec![api::Task {
            id: task_id.to_string(),
            messages: vec![
                restored_code_block_user_query_message(task_id, request_id),
                restored_code_block_agent_output_message(task_id, request_id),
            ],
            dependencies: None,
            description: String::new(),
            summary: String::new(),
...
pub fn test_ai_code_block_uses_terminal_theme_screenshot() -> Builder {
    new_builder()
        .with_real_display()
        ...
        .with_step(
            new_step_with_default_assertions(
                "Restore AI conversation with a large JSON code block",
            )
            .with_action(|app, window_id, _| {
                let terminal_view = single_terminal_view_for_tab(app, window_id, 0);
                terminal_view.update(app, |view, ctx| {
                    view.load_conversation_from_tasks(restored_code_block_conversation_data(), ctx);
                });
            }),
        )
        .with_step(
            TestStep::new("Capture restored code block terminal theme")
                .set_timeout(Duration::from_secs(20))
                .set_post_step_pause(Duration::from_secs(3))
                .with_take_screenshot("ai_code_block_terminal_theme.png")
        ...
        .with_step(
            TestStep::new("Resize window and capture code block repaint")
                .set_timeout(Duration::from_secs(20))
                .set_post_step_pause(Duration::from_secs(1))
                .with_action(|app, window_id, _| {
                    let bounds = app
                        .window_bounds(&window_id)
                        .expect("integration window bounds should exist");
                    app.update(|ctx| {
                        ctx.set_and_cache_window_bounds(
                            window_id,
                            RectF::new(bounds.origin(), bounds.size() + vec2f(48.0, 32.0)),
                        );
                    });
                })
                .with_take_screenshot("ai_code_block_terminal_theme_after_resize.png")
                .add_assertion(|app, window_id| {
                    let terminal_view = single_terminal_view_for_tab(app, window_id, 0);
                    terminal_view.read(app, |view, _ctx| {
                        async_assert!(
                            view.last_ai_block().is_some(),
                            "Restored AI block should remain after resize"
                        )
                    })
                }),
        )
        .with_step(
            TestStep::new("Hold code block window for external screenshot")
                .set_post_step_pause(Duration::from_secs(
                    env::var("ASHIDE_UI010_HOLD_SECONDS")
                        .ok()
                ...
                .add_assertion(|app, window_id| {
                    ...
                    terminal_view.read(app, |view, _ctx| {
                        async_assert!(
                            view.last_ai_block().is_some(),
                            "Restored AI block should remain visible for external capture"
                        )
                    })
                }),
        )
