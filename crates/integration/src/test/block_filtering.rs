...
use warp::integration_testing::settings::{
    set_secret_display_mode_to_asterisks, toggle_safe_mode_setting,
};
            ...
            new_step_with_default_assertions("Open block filter editor via toolbelt icon")
                ...
                .add_named_assertion("Assert that block filter is open", |app, window_id| {
                    ...
                    terminal_view.read(app, |view, ctx| {
                        async_assert_eq!(
                            view.filter_editor_block_index(ctx),
            ...
            new_step_with_default_assertions("Toggle block filter off")
                ...
                .add_named_assertion(
                    ...
                    |app, window_id| {
                        let terminal_view = single_terminal_view_for_tab(app, window_id, 0);
                        terminal_view.read(app, |view, ctx| {
                            async_assert!(view.filter_editor_block_index(ctx).is_none())
                        })
                    },
                )
                .add_named_assertion(
