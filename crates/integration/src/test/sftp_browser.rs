...
fn open_provider_file_browser_pane(app: &mut warpui::App) {
    ...
    app.update(|ctx| {
        workspace.update(ctx, |ws, ctx| {
            ws.open_environment_file_browser("test-integration-node".to_string(), ctx);
        });
    });
}
...
",
        ...
        .with_step(
            TestStep::new("Open SFTP pane")
                .with_action(|app, _, _| open_provider_file_browser_pane(app))
...
fn open_sftp_with_mock_step(
    ...
    TestStep::new("Open SFTP pane with mock backend")
        .with_action(move |app, _, step_data: &mut StepDataMap| {
            let (_, temp_dir) = sftp::open_provider_file_browser_pane_with_mock(app, files);
