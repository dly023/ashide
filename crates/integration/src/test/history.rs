...
use warp::{
    integration_testing::{
        command_search::{
            assert_command_search_has_committed_results, assert_command_search_is_open,
    ...
    sqlite_testing::{create_history_test_database, set_user_and_hostname_for_commands},
    terminal::{input::Input, model::session::get_current_app_hostname, shell::ShellType},
};
...
fn select_ready_history_filter() -> warpui::integration::TestStep {
    new_step_with_default_assertions("Select history filter")
        ...
        .add_named_assertion(
            "History results are committed",
            assert_command_search_has_committed_results(),
        )
}

/// `create_history_test_database` creates the following command rows with the current schema:
///
/// |command                 |start_ts                  |completed_ts              |workflow_command                        |
/// |echo "foo"              |2023-07-11 16:29:32.092176|2023-07-11 16:29:33.124078|NULL                                    |
/// |[[ -n "foo" ]]          |2023-07-11 16:29:34.837961|2023-07-11 16:29:35.124078|[[ -n {{string}} ]]                    |
/// |echo "bar"              |2023-07-11 16:29:42.000000|2023-07-11 16:29:43.000000|NULL                                    |
/// |sed -i '' '/hello/d' foo|2023-07-12 16:29:42.000000|2023-07-12 16:29:43.000000|sed -i '' '/{{string}}/d' {{file}}    |
///
...
pub fn test_up_arrow_history_enters_shift_tab_for_workflow() -> Builder {
    new_builder()
        .with_setup(|utils| {
            create_history_test_database();

            let local_user = get_local_user();
            let local_hostname =
                get_current_app_hostname().expect("Failed to retrieve system hostname.");
...
pub fn test_command_search_loads_history() -> Builder {
    new_builder()
        ...
        .with_step(select_ready_history_filter())
        .with_step(
...
/// up in Ashide -- so we effectively do a "left join" on commands from the histfile with commands
/// loaded from sqlite.
