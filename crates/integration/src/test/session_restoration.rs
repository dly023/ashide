use warp::{
    ...
    sqlite_testing::{
        create_session_restoration_test_database, set_user_and_hostname_for_blocks,
        SessionRestorationTestFixture,
    },
    terminal::{
        model::{session::get_current_app_hostname, terminal_model::BlockIndex},
        shell::ShellType,
                ...
                move |app, window_id| {
                    ...
                    terminal_view.read(app, |terminal, ctx| {
                        History::handle(ctx).read(app, |history, ctx| {
                            ...
                            let local_user = get_local_user();
                            let local_hostname = get_current_app_hostname()
                                .expect("Failed to retrieve system hostname.");
...
pub fn test_restore_snapshot_with_notebooks() -> Builder {
    new_builder()
        .with_setup(|utils| {
            create_session_restoration_test_database(SessionRestorationTestFixture::Notebooks {
                terminal_cwd: utils.test_dir().to_owned(),
            });
        })
        .with_step(
...
pub fn test_restore_snapshot_with_workflows() -> Builder {
    new_builder()
        .with_setup(|_utils| {
            create_session_restoration_test_database(SessionRestorationTestFixture::Workflows);
        })
        .with_step(
...
pub fn test_restore_snapshot_with_markdown_file() -> Builder {
    new_builder()
        .with_setup(|utils| {
            ...
            create_session_restoration_test_database(SessionRestorationTestFixture::MarkdownFile {
                terminal_cwd: utils.test_dir().to_owned(),
                markdown_path: "./docs/test.md".into(),
            });
        })
...
pub fn test_restore_snapshot_with_code_file() -> Builder {
    new_builder()
        .with_setup(|utils| {
            ...
            create_session_restoration_test_database(SessionRestorationTestFixture::CodeFile {
                terminal_cwd: utils.test_dir().to_owned(),
                code_path: "./docs/test.rs".into(),
            });
        })
        // Wait for the terminal pane to bootstrap first - we need an active session to resolve the
        // home directory and context for the notebook pane.
...
/// The snapshot has a single window with one tab, containing:
/// * A terminal pane
/// * 设置面板,保存的是当前仍存在的 Appearance 页面。
pub fn test_restore_snapshot_with_settings_page() -> Builder {
    new_builder()
        .with_setup(|utils| {
            create_session_restoration_test_database(SessionRestorationTestFixture::Settings {
                terminal_cwd: utils.test_dir().to_owned(),
                current_page: "Appearance".to_owned(),
            });
        })
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(
