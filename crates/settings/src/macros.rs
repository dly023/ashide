...
//! of defining new settings within Ashide.
//!
...
//! ```
//!
//! ## Declaring a local setting.
//!
//! Each setting declares its supported platforms, whether it is private, and
//! where it appears in the local settings file.
//!
//! ```
//! # use settings::macros::*;
//! # use settings::*;
...
macro_rules! define_setting {
    ...
    (@base $name:ident: $type:ty, default: $default:tt, supported_platforms: $supported_platforms: expr, group: $group:path, private: $private:expr, storage_key: $storage_key:expr, toml_path_value: $toml_path_value:expr $(, max_table_depth_value: $mtd:literal)? $(, description: $desc:literal)? $(, feature_flag: $flag:path)?) => {
        ...
        impl $crate::Setting for $name {
            ...
            fn set_value_from_external_update(
                &mut self,
                ...
                if self.value() != &new_value || changed_in_storage {
                    ...
                    ctx.emit($crate::macros::concat_idents!(EventName = $group, ChangedEvent { EventName::$name {
                        change_event_reason: ChangeEventReason::ExternalUpdate,
                    }}));
                }
                Ok(())
            }
...
/// Registers SettingsManager callbacks for each typed setting.
#[macro_export]
...
/// Generates a function that registers storage-key callbacks for a setting.
#[macro_export]
            ...
            fn fn_name(
                ...
                SettingsManager::handle(ctx).update(ctx, |manager, _ctx| {
                    ...
                    manager.register_setting(
                        ...
                        $setting::is_private(),
                        move |value, from_external_update, ctx| {
                            use $crate::_settings_value::SettingsValue as _;
                            // Try SettingsValue first (handles snake_case enums etc.),
                            // then fall back to serde for externally imported values.
                            ...
                            settings_group_update_clone.update(ctx, |settings_group, ctx| {
                                if from_external_update {
                                    settings_group
                                        .$var
                                        .set_value_from_external_update(value, ctx)
                                } else {
