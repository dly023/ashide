...
#[test]
fn test_explicit_value_tracking_external_update() {
    warpui::App::test((), |mut app| async move {
        app.update(init_and_register_preferences);
        app.add_singleton_model(|_| SettingsManager::default());
        ...
        app.update(|ctx| {
            TestSettings::handle(ctx).update(ctx, |test_settings, ctx| {
                let _ = test_settings
                    .simple_setting
                    .set_value_from_external_update(true, ctx);
            });
        });
...
#[test]
fn test_manager_read_local_setting_value_routes() {
    warpui::App::test((), |mut app| async move {
        app.update(init_and_register_preferences);
        app.add_singleton_model(|_| SettingsManager::default());
        TestSettings::register(&mut app);
...
/// `read_local_setting_value` must forward the setting's hierarchy,
/// otherwise values stored under a section like `[account]` are invisible
