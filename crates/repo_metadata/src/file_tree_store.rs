...
use crate::{BuildTreeError, Entry, FileId, FileMetadata, Repository, SymlinkTargetKind};
...
impl FileTreeEntry {
    pub fn ignored(&self, path: &StandardizedPath) -> bool {
        ...
        match entry_state {
            ...
            FileTreeEntryState::Symlink(symlink) => symlink.ignored,
        }
    }
    ...
    pub fn find_or_insert_child(
        ...
        let new_entry = if child_path.is_symlink() {
            let target_kind = match std::fs::metadata(child_path) {
    ...
    pub fn ensure_parent_directories_exist(&mut self, target_parent: &StandardizedPath) {
        ...
        let Some(root_entry) = self.get(root_directory).cloned() else {
            return;
        };
        if !root_entry.is_directory_like() {
            return;
        }
        ...
        let ancestors: Vec<_> = target_parent
            .ancestors()
            .take_while(|ancestor| *ancestor != *root_directory.as_ref())
            .collect();

        // Create directories from root to target parent using find_or_insert_directory
        let mut current_parent_path = root_directory.clone();
        for ancestor in ancestors.iter().rev() {
            match self.find_or_insert_directory(&current_parent_path, ancestor) {
                Some(FileTreeEntryState::Directory(dir)) => {
                    current_parent_path = dir.path.clone();
                }
                Some(FileTreeEntryState::Symlink(link))
                    if link.target_kind == SymlinkTargetKind::Directory =>
                {
                    current_parent_path = link.path.clone();
                }
                Some(FileTreeEntryState::File(_) | FileTreeEntryState::Symlink(_)) => {
                    log::warn!("Found non-directory where directory expected: {ancestor:?}");
                    return;
                }
                None => {
    ...
    /// Applies the response to one explicit lazy-directory load and commits the
    /// requested lexical path as loaded, including empty directories and
    /// directory symlinks whose response contains no child entries.
    pub fn apply_repo_metadata_directory_load(
        &mut self,
        update: &crate::file_tree_update::RepoMetadataUpdate,
        dir_path: &StandardizedPath,
    ) -> bool {
        self.apply_repo_metadata_update(update);
        ...
        match self.get_mut(dir_path) {
            ...
            Some(FileTreeEntryState::Symlink(symlink))
                if symlink.target_kind == SymlinkTargetKind::Directory =>
            {
                symlink.loaded = true;
                true
            }
            Some(FileTreeEntryState::File(_) | FileTreeEntryState::Symlink(_)) | None => false,
        }
    }
        ...
        for node in &update.subtree_metadata {
            match node {
                ...
                RepoNodeMetadata::Symlink(symlink) => {
                    let state = FileTreeEntryState::Symlink(FileTreeSymlinkMetadata {
                        path: Arc::new(symlink.path.clone()),
                        target_kind: symlink.target_kind,
                        ignored: symlink.ignored,
                        loaded: symlink.loaded,
                    });
                    if let Some(parent) = self.find_parent_directory(&symlink.path) {
                        self.insert_child_state(&parent, state);
                    } else {
                        log::warn!(
                            "Could not find parent directory for node during incremental update: {:?}",
...
pub enum FileTreeEntryState {
    ...
    Symlink(FileTreeSymlinkMetadata),
}
...
impl FileTreeEntryState {
    pub fn set_ignored(&mut self, ignored: bool) {
        match self {
            ...
            Self::Symlink(symlink) => symlink.ignored = ignored,
        }
    }
    ...
    pub fn ignored(&self) -> bool {
        match self {
            ...
            FileTreeEntryState::Symlink(s) => s.ignored,
        }
    }
    ...
    pub fn path(&self) -> &StandardizedPath {
        match self {
            ...
            FileTreeEntryState::Symlink(symlink) => &symlink.path,
        }
    }
    ...
    fn path_arc(&self) -> Arc<StandardizedPath> {
        match self {
            ...
            FileTreeEntryState::Symlink(symlink) => symlink.path.clone(),
        }
    }
    ...
    pub fn loaded(&self) -> bool {
        match self {
            ...
            FileTreeEntryState::Symlink(s) => s.loaded,
        }
    }

    pub fn is_directory_like(&self) -> bool {
        matches!(self, Self::Directory(_))
            || matches!(self, Self::Symlink(s) if s.target_kind == SymlinkTargetKind::Directory)
    }
...
#[derive(Debug, Clone)]
pub struct FileTreeSymlinkMetadata {
    pub path: Arc<StandardizedPath>,
