...
use crate::entry::{DirectoryEntry, Entry, FileId, FileMetadata, SymlinkEntry, SymlinkTargetKind};
...
fn apply_mutations_generates_update_for_remove() {
    use crate::current_app_model::FileTreeMutation;
    ...
    let update =
        CurrentAppRepoMetadataModel::apply_file_tree_mutations(&mut tree, mutations, false, true)
...
fn apply_mutations_generates_update_for_add_file() {
    ...
    let mut tree = build_tree_from_entry(initial);
    let mutations = vec![FileTreeMutation::AddEntry {
        path: mutation_path("/repo/src/new.rs"),
        entry: file("/repo/src/new.rs"),
    }];
...
fn apply_mutations_generates_update_for_add_directory_subtree() {
    ...
    let mut tree = build_tree_from_entry(initial);
    let mutations = vec![FileTreeMutation::AddEntry {
        path: mutation_path("/repo/src/components"),
        entry: subtree,
    }];
...
fn apply_mutations_returns_none_when_emit_updates_is_false() {
    ...
    let update =
        CurrentAppRepoMetadataModel::apply_file_tree_mutations(&mut tree, mutations, false, false);
...
#[test]
fn explicit_empty_directory_load_marks_directory_and_symlink_loaded() {
    let empty_dir = std_path("/repo/empty");
    let empty_link = std_path("/repo/empty-link");
    ...
    let mut tree = build_tree_from_entry(initial);
    let empty_response = RepoMetadataUpdate {
        repo_path: std_path("/repo"),
        remove_entries: Vec::new(),
        update_entries: Vec::new(),
    };

    assert!(tree.apply_repo_metadata_directory_load(&empty_response, &empty_dir));
    assert!(tree.apply_repo_metadata_directory_load(&empty_response, &empty_link));

    assert!(tree.get(&empty_dir).is_some_and(FileTreeEntryState::loaded));
    assert!(tree
        .get(&empty_link)
        .is_some_and(FileTreeEntryState::loaded));
}
...
fn round_trip_apply_mutations_then_apply_update_produces_equivalent_tree() {
    ...
    let mutations = vec![
        ...
        FileTreeMutation::AddEntry {
            path: mutation_path("/repo/src/util"),
            entry: subtree,
        },
        FileTreeMutation::AddEntry {
            path: mutation_path("/repo/src/main.rs"),
            entry: file("/repo/src/main.rs"),
        },
    ];

    let update = CurrentAppRepoMetadataModel::apply_file_tree_mutations(
        &mut server_tree,
        mutations,
        false,
        true,
...
fn lazy_load_filters_mutations_for_unloaded_parents() {
    ...
    let mutations = vec![
        ...
        FileTreeMutation::AddEntry {
            ...
            entry: file("/repo/vendor/lib.rs"),
        },
    ];

    let update =
        CurrentAppRepoMetadataModel::apply_file_tree_mutations(&mut tree, mutations, true, true)
