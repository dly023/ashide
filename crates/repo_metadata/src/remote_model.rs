...
use crate::current_app_model::{GetContentsArgs, IndexedRepoState, RepoContent};
...
use super::current_app_model::collect_contents_recursive;
...
pub enum RemoteRepositoryMetadataEvent {
    ...
    /// One exact lazy-directory request reached a completion boundary.
...
impl RemoteRepoMetadataModel {
    ...
    /// Applies one explicit remote lazy-directory response and marks the exact
    /// lexical directory identity loaded even when it has no children.
    pub fn apply_directory_load(
        ...
        let Some(id) = matching_id else {
            log::warn!(
                "No remote repository found for directory load: {}/{}",
                update.repo_path,
                dir_path
            );
            return;
        ...
        if let Some(IndexedRepoState::Indexed(state)) = self.repositories.get_mut(&id) {
            if !state
                .entry
                .apply_repo_metadata_directory_load(update, dir_path)
            {
                log::warn!(
                    "Remote directory load completed for non-directory or missing path: {dir_path}"
                );
            }
            ctx.emit(RemoteRepositoryMetadataEvent::FileTreeEntryUpdated { id: id.clone() });
            ctx.emit(RemoteRepositoryMetadataEvent::DirectoryLoadFinished {
                id,
    ...
    /// Emits the exact failure boundary for a remote lazy-directory request.
    pub fn finish_directory_load(
        &mut self,
        host_id: &HostId,
        ...
        let id = self
            .repositories
            .keys()
            .find(|id| id.host_id == *host_id && id.path == *repo_path)
            .cloned()
            .unwrap_or_else(|| RemoteRepositoryIdentifier::new(host_id.clone(), repo_path.clone()));
        ctx.emit(RemoteRepositoryMetadataEvent::DirectoryLoadFinished {
            id,
            dir_path: dir_path.clone(),
            succeeded: false,
