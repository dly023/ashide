...
#[cfg(unix)]
#[test]
fn lexical_symlink_descendant_routes_to_owning_repository() {
    ...
    fs::write(external_target.join("child.txt"), "child").unwrap();
    let link = repository.join("linked-directory");
    symlink(&external_target, &link).unwrap();
    let lexical_child = link.join("child.txt");

    App::test((), |mut app| async move {
        let watcher_handle = app.add_model(DirectoryWatcher::new_for_testing);
        let repository_path = StandardizedPath::try_from_local(&repository).unwrap();
        let repository_handle = watcher_handle
            .update(&mut app, |watcher, ctx| {
                watcher.add_directory(repository_path, ctx)
            })
            .unwrap();

        watcher_handle.read(&app, |watcher, _ctx| {
            let routed = watcher
                .get_watched_directory_for_path(&lexical_child)
                .expect(
                "lexical link descendants must remain owned by the repository containing the link",
            );
            assert_eq!(routed, repository_handle);
        });
    });
}
