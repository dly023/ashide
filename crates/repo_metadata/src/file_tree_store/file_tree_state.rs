...
use crate::{BuildTreeError, DirectoryEntry, Entry, SymlinkEntry, SymlinkTargetKind};
    ...
    fn from(value: Entry) -> Self {
        match value {
            ...
            Entry::Symlink(symlink) => {
                let symlink_path: Arc<StandardizedPath> = Arc::new(symlink.path);
                let entry = FileTreeEntryState::Symlink(FileTreeSymlinkMetadata {
                ...
                let mut file_tree_map = Self::new_for_entry(entry);
                for child in symlink.children {
                    file_tree_map.append_entry(symlink_path.clone(), child);
                }
                file_tree_map
            }
        }
...
impl FileTreeMapStore {
    ...
    fn append_entry(&mut self, parent_path: Arc<StandardizedPath>, entry: Entry) {
        match entry {
            ...
            Entry::Symlink(symlink) => {
                ...
                self.insert_child(parent_path, entry);
                for child in symlink.children {
                    self.append_entry(symlink_path.clone(), child);
                }
            }
        }
    }
    ...
    pub fn children(
        ...
        if self
            .state_map
            .get(path)
            .is_none_or(|entry| !entry.is_directory_like())
        {
    ...
    fn is_directory_like(&self, path: &StandardizedPath) -> bool {
        self.state_map
            .get(path)
            .is_some_and(FileTreeEntryState::is_directory_like)
    }
    ...
    pub fn contains_child(&self, parent_path: &StandardizedPath, child: &StandardizedPath) -> bool {
        if !self.is_directory_like(parent_path) {
            return false;
        };
    ...
    pub fn insert_child(
        ...
        self.is_directory_like(&parent_path).then_some(())?;
    ...
    pub fn load_at_path(
        ...
        let mut entry = match self.state_map.get(path) {
            Some(FileTreeEntryState::Directory(directory)) => Entry::Directory(DirectoryEntry {
                path: path.clone(),
                children: Vec::new(),
            ...
            Some(FileTreeEntryState::File(_) | FileTreeEntryState::Symlink(_)) | None => {
                return Ok(());
            }
        };
    ...
    pub fn rename_path(&mut self, path: &StandardizedPath, new_path: &StandardizedPath) -> bool {
        ...
        match &mut item {
            ...
            FileTreeEntryState::Symlink(s) => s.path = new_path_arc.clone(),
        }
        let is_directory = item.is_directory_like();
    ...
    fn update_paths_recursive(
        ...
        if let Some(mut item) = self.remove_item_and_parent(&path) {
            match &mut item {
                ...
                FileTreeEntryState::Symlink(s) => s.path = new_child_path.clone(),
            }
            // Insert with new key.
