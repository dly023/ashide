...
impl DirectoryWatcher {
    ...
    pub fn get_watched_directory_for_path(&self, path: &Path) -> Option<ModelHandle<Repository>> {
        self.find_containing_directory_for_fs_path(path)
    }

    /// Route by lexical identity first so a link inside a repo stays owned by that repo,
    /// then fall back to canonical identity for repositories opened through a symlinked root.
    fn find_containing_directory_for_fs_path(
        &self,
        path: &Path,
    ) -> Option<ModelHandle<Repository>> {
        StandardizedPath::try_from_local(path)
            .ok()
            .and_then(|path| self.find_containing_directory(&path))
            .or_else(|| {
    ...
    fn handle_watcher_event(
        ...
        let mut process_upsert_paths = |paths: &HashSet<PathBuf>,
            ...
            for path in paths {
                ...
                if let Some(repo_handle) = self.find_containing_directory_for_fs_path(path) {
                    let is_ignored =
            ...
            if is_git_internal_path(path) {
                ...
                if let Some(existing_subpath) = Self::find_existing_subpath(path) {
                    if let Some(repo_handle) =
                        self.find_containing_directory_for_fs_path(&existing_subpath)
                    {
        ...
        for (to_path, from_path) in &event.moved {
            ...
            } else if let Some(repo_handle) = self.find_containing_directory_for_fs_path(to_path) {
                let to_is_ignored =
