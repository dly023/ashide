...
use crate::entry::{DirectoryEntry, Entry, FileMetadata, SymlinkEntry, SymlinkTargetKind};
...
/// Mirrors `SymlinkNodeMetadata` proto.
#[derive(Debug, Clone)]
pub struct SymlinkNodeMetadata {
    pub path: StandardizedPath,
...
fn collect_metadata(entry: &Entry, metadata: &mut Vec<RepoNodeMetadata>) {
    match entry {
        ...
        Entry::Symlink(symlink) => {
            collect_symlink_metadata(symlink, metadata);
        }
    }
}

fn collect_symlink_metadata(symlink: &SymlinkEntry, metadata: &mut Vec<RepoNodeMetadata>) {
    metadata.push(RepoNodeMetadata::Symlink(SymlinkNodeMetadata {
