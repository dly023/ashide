...
impl RepoMetadataModel {
    ...
    fn forward_remote_event(
        ...
        let unified = match event {
            ...
            RemoteRepositoryMetadataEvent::DirectoryLoadFinished {
                id,
                dir_path,
                succeeded,
    ...
    /// Applies one explicit remote lazy-directory response.
    pub fn apply_remote_directory_load(
        &self,
        host_id: &HostId,
        dir_path: &StandardizedPath,
        update: &RepoMetadataUpdate,
        ...
        let host_id = host_id.clone();
        self.remote.update(ctx, |remote, ctx| {
            remote.apply_directory_load(&host_id, dir_path, update, ctx);
        });
    }

    /// Emits the exact failure boundary for a remote lazy-directory request.
    pub fn finish_remote_directory_load(
        &self,
        host_id: &HostId,
        ...
        let host_id = host_id.clone();
        self.remote.update(ctx, |remote, ctx| {
            remote.finish_directory_load(&host_id, repo_path, dir_path, ctx);
        });
    }
