...
//!
//! 现状: Ashide 的主写入连接在专门的写线程里(see `app/src/persistence/sqlite.rs`)
...
//!
//! 不变量(RR-A4 已审计,勿再「路由进写线程」):这个独立写连接是**有意**的,不是
//! 待修的 bypass。把 SSH 写接进 app 层的 `ModelEvent` 总线会造成 crate 间循环依赖;
//! WAL + busy_timeout 下多写连接安全,且 panic 风险已在 9e8e50d 修掉(write 失败
//! 返回 `Result` 而非 expect)。
...
///
/// 开库失败(路径未初始化 / 文件被锁 / WAL 设置失败)返回 `Err` 而不是 panic
/// 整个应用。`OnceLock::get_or_try_init` 尚未稳定,这里手动 try-init:首个成功
/// 打开的连接被 `set` 进 OnceLock;若并发下另一线程抢先 `set`,本线程多开的连接
/// 无害地 drop。
pub fn with_conn<R>(f: impl FnOnce(&mut SqliteConnection) -> Result<R>) -> Result<R> {
    ...
    let mtx = CONN
        .get()
        .ok_or_else(|| anyhow!("warp_ssh_manager::db: connection not initialized"))?;
    let mut guard = mtx
        .lock()
