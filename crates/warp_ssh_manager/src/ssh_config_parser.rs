...
pub fn default_ssh_config_path() -> Option<PathBuf> {
    default_ssh_config_path_from_home(dirs::home_dir().as_deref())
}

fn default_ssh_config_path_from_home(home: Option<&std::path::Path>) -> Option<PathBuf> {
...
";
    ...
    fn include_directive_reported_as_unexpanded() {
        ...
        let (got, has_includes) =
            parse_ssh_config_reporting_includes("Include ~/.ssh/config.d/*\nHost a\n    User u\n");
    ...
    fn no_include_means_not_flagged() {
        ...
        let (_got, has_includes) = parse_ssh_config_reporting_includes("Host a\n    User u\n");
        assert!(!has_includes);
    }
    ...
    #[test]
    fn windows_home_path_is_preserved_as_config_open_target() {
        let home = PathBuf::from(r"C:\Users\Alice");
        let path = default_ssh_config_path_from_home(Some(&home))
            .expect("an explicit Windows home path must produce a config path");

        assert_eq!(path, home.join(".ssh").join("config"));
