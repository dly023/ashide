...
struct RuleAtPath {
    parent_path: PathBuf,
    rules_by_priority: Vec<Option<ProjectRule>>,
}
...
impl RuleAtPath {
    fn new(parent_path: PathBuf) -> Self {
        Self {
            parent_path,
            rules_by_priority: vec![None; RULES_FILE_PATTERN.len()],
        }
    }

    fn rule_index(file_name: &str) -> Option<usize> {
        RULES_FILE_PATTERN
            .iter()
            .position(|pattern| pattern.eq_ignore_ascii_case(file_name))
    }

    fn respected_rule(&self) -> Option<&ProjectRule> {
        self.rules_by_priority.iter().find_map(|rule| rule.as_ref())
    }

    fn all_rules(&self) -> impl Iterator<Item = &ProjectRule> {
        self.rules_by_priority
            .iter()
            .filter_map(|rule| rule.as_ref())
    }
}
...
impl ProjectRules {
    ...
    fn remove_rule(&mut self, path: &Path) -> Option<ProjectRule> {
        ...
        let rule_index = RuleAtPath::rule_index(file_name)?;
        rule.rules_by_priority.get_mut(rule_index)?.take()
    }
    ...
    fn upsert_rule(&mut self, path: &Path, content: String) {
        ...
        let Some(rule_index) = RuleAtPath::rule_index(file_name) else {
            return;
        };

        match existing_rule {
            Some(rule) => rule.rules_by_priority[rule_index] = rule_file,
            None => {
                let mut rule = RuleAtPath::new(parent.to_path_buf());
                rule.rules_by_priority[rule_index] = rule_file;
...
/// Supports rule files listed in `RULES_FILE_PATTERN`.
#[cfg_attr(not(feature = "local_fs"), allow(dead_code))]
#[derive(Debug, Default)]
...
impl ProjectContextModel {
    ...
    pub fn new_from_persisted(
        ...
        ctx.spawn(
            ...
            |me, mut res, ctx| {
                ...
                // (对 Ashide BYOP 是冷启动最大后台 CPU 耗费头)。
                //
                    ...
                    Ok(rule_files) => {
                        ...
                        let delta = RulesDelta {
                            discovered_rules: rule_files
                                .rules
                                .iter()
                                .flat_map(|rule_at_path| {
                                    rule_at_path.all_rules().map(|rule| ProjectRulePath {
    ...
    // Ashide:原 `try_initialize_and_register_watcher` 是从持久化 rule 路径启动时
