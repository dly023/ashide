...
fn fast_path_still_valid_when_nothing_changed() {
    ...
    // 并行测试会在系统临时目录下创建/删除 sibling tempdir，导致外层祖先目录
    // mtime 变化；这里聚焦验证命中的规则文件和 cwd 本身未变时仍可复用。
    entry.walked_dir_stamps.retain(|(dir, _)| dir == &cwd);
...
fn fast_path_invalidated_when_rule_file_mtime_changes() {
    ...
    let mut entry = ProjectContextModel::scan_fast_path(&cwd);
    // 并行测试会创建或删除同一系统临时目录下的 sibling，外层祖先目录
    // mtime 与本测试无关；这里仅验证命中的规则文件 stamp 会使缓存失效。
    entry.walked_dir_stamps.retain(|(dir, _)| dir == &cwd);
    assert!(ProjectContextModel::fast_path_entry_still_valid(&entry));
