...
//!
//! This module defines the supported skill providers (i.e. Agents, Ashide, Claude, Codex) and their
...
/// Represents a skill provider/origin (Agents, Ashide, Claude, Codex, or another tool).
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum SkillScope {
    ...
    #[default]
    Project,
    /// Bundled skills distributed with Ashide.
    Bundled,
}
    ...
    LazyLock::new(|| {
        vec![
            ...
            SkillProviderDefinition {
                ...
                skills_path: PathBuf::from(".ashide").join("skills"),
            },
            SkillProviderDefinition {
...
pub fn home_skills_path(provider: SkillProvider) -> Option<PathBuf> {
    if provider == SkillProvider::Ashide {
...
mod tests {
    ...
    #[test]
    fn ashide_home_skills_path_uses_ashide_home_path() {
        assert_eq!(
            home_skills_path(SkillProvider::Ashide),
            warp_core::paths::warp_home_skills_dir()
        );
    }

    #[test]
    fn ashide_home_skill_path_uses_ashide_provider() {
        let Some(ashide_home_skills_dir) = warp_core::paths::warp_home_skills_dir() else {
            eprintln!("Skipping test: home directory not available");
            return;
        };
        let path = ashide_home_skills_dir.join("my-skill").join("SKILL.md");

        assert_eq!(get_provider_for_path(&path), Some(SkillProvider::Ashide));
    }
}
