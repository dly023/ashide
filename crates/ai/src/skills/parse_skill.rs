...
pub struct ParsedSkill {
    ...
    /// The provider of the skill (Agents, Ashide, Claude, or Codex), determined from the path.
...
impl ParsedSkill {
    /// Returns true if this skill is bundled with Ashide (not a user-editable file).
...
/// skill provider directory. Bundled skills are always assigned `SkillProvider::Ashide`
...
pub fn parse_bundled_skill(path: &Path) -> Result<ParsedSkill> {
    parse_skill_internal(path, SkillProvider::Ashide, SkillScope::Bundled)
}
