...
#[test]
fn environment_file_runtime_uses_registered_session_binding() {
    App::test((), |mut app| async move {
        let files = app.add_singleton_model(FileModel::new);
        let (write_tx, write_rx) = async_channel::unbounded();
        files.update(&mut app, |model, _ctx| {
            model.set_environment_file_runtime(EnvironmentFileRuntime::new(
                move |host_id, session_id, path, content, _ctx| {
                    write_tx
                        .send_blocking((host_id.clone(), session_id, path, content))
                        .unwrap();
                    Ok(Box::pin(async { Ok(()) }))
                },
                |_host_id, _session_id, _path, _ctx| Ok(Box::pin(async { Ok(()) })),
            ));
        });

        let host_id = HostId::new("shared-host".to_owned());
        let session_id = SessionId::from(731);
        let path = StandardizedPath::try_new("/tmp/session-bound.txt").unwrap();
        let file_id = files.update(&mut app, |model, _ctx| {
            model.register_remote_file(host_id.clone(), session_id, path)
        });
        files
            .update(&mut app, |model, ctx| {
                model.save(
                    file_id,
                    "bound content".to_owned(),
                    ContentVersion::new(),
                    ctx,
                )
            })
        ...
        let (actual_host, actual_session, actual_path, actual_content) =
            write_rx.recv().await.unwrap();
        assert_eq!(actual_host, host_id);
        assert_eq!(actual_session, session_id);
        assert_eq!(actual_path, "/tmp/session-bound.txt");
        assert_eq!(actual_content, "bound content");
    });
}
