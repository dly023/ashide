...
pub type EnvironmentFileRuntimeFuture = warpui::r#async::BoxFuture<'static, Result<(), String>>;

type EnvironmentFileRuntimeWrite = Arc<
    dyn Fn(
            &HostId,
            String,
            String,
            &AppContext,
        ) -> Result<EnvironmentFileRuntimeFuture, FileSaveError>
        + 'static,
>;
type EnvironmentFileRuntimeDelete = Arc<
    dyn Fn(&HostId, String, &AppContext) -> Result<EnvironmentFileRuntimeFuture, FileSaveError>
        + 'static,
>;

/// File-operation facade supplied by the app-level environment runtime.
///
/// `warp_files` owns file state and local watching, but it should not know
/// whether non-local file operations are backed by SSH, a helper daemon, or a
/// future runtime transport. The app injects that boundary here.
#[derive(Clone)]
pub struct EnvironmentFileRuntime {
    write_file: EnvironmentFileRuntimeWrite,
    delete_file: EnvironmentFileRuntimeDelete,
}

impl EnvironmentFileRuntime {
    pub fn new(
        write_file: impl Fn(
                &HostId,
                String,
            ...
            + 'static,
        delete_file: impl Fn(&HostId, String, &AppContext) -> Result<EnvironmentFileRuntimeFuture, FileSaveError>
            + 'static,
    ) -> Self {
        Self {
            write_file: Arc::new(write_file),
            delete_file: Arc::new(delete_file),
        }
    }
    ...
    fn write_file(
        ...
        (self.write_file)(host_id, path, content, ctx)
    }
    ...
    fn delete_file(
        ...
        (self.delete_file)(host_id, path, ctx)
    }
}
...
/// Environment files dispatch through the injected [`EnvironmentFileRuntime`].
enum FileBackend {
    Local(LocalFile),
    Remote {
        /// Identifies the environment host. The app-level environment runtime
        /// resolves the active transport at call time, which naturally handles
        /// disconnect without holding a transport alive per file.
        host_id: HostId,
        /// Platform-aware path on the environment host.
...
pub struct FileModel {
    ...
    environment_file_runtime: Option<EnvironmentFileRuntime>,
...
impl FileModel {
    ...
    /// will dispatch through the injected [`EnvironmentFileRuntime`].
    ...
    pub fn set_environment_file_runtime(&mut self, runtime: EnvironmentFileRuntime) {
        self.environment_file_runtime = Some(runtime);
    }

    fn environment_file_runtime(&self) -> Result<EnvironmentFileRuntime, FileSaveError> {
        self.environment_file_runtime.clone().ok_or_else(|| {
            FileSaveError::RemoteError("Environment file runtime is not registered".to_string())
        })
    }
    ...
    pub fn save(
        ...
        match backend {
            ...
            FileBackend::Remote { host_id, path } => {
                let runtime = self.environment_file_runtime()?;
                let path = path.as_str().to_string();
                let future = runtime.write_file(host_id, path, content, ctx)?;
                ctx.spawn(
                    future,
    ...
    pub fn delete(
        ...
        match backend {
            ...
            FileBackend::Remote { host_id, path } => {
                let runtime = self.environment_file_runtime()?;
                let path = path.as_str().to_string();
                let future = runtime.delete_file(host_id, path, ctx)?;
                ctx.spawn(
                    future,
                    move |me, result: Result<(), String>, ctx| match result {
                        Ok(()) => {
