...
fn test_split_path() {
    ...
    assert_eq!(
        split_path,
        SplitPath {
            directory_absolute_path: TypedPathBuf::from("/Users/warpuser/Ashide.app/"),
            directory_relative_path_name: "Ashide.app/".to_owned(),
    ...
    assert_eq!(
        split_path,
        SplitPath {
            directory_absolute_path: TypedPathBuf::from("/Users/warpuser/Ashide.app/macOS/bin/"),
            directory_relative_path_name: "Ashide.app/macOS/bin/".to_owned(),
            file_name: "ashide.o".to_owned()
        }
    );
}
...
#[test]
fn platform_hidden_and_ignored_entries_are_filtered_from_path_completions() {
    let mut platform_hidden = file_entry("platform-secret");
    platform_hidden.platform_hidden = true;
    let mut ignored = file_entry("ignored-secret");
    ...
    let suggestions = warpui::r#async::block_on(sorted_paths_relative_to(
        &ParsedToken::empty(),
    ...
    assert_eq!(
        suggestions
            .into_iter()
            .map(|suggestion| suggestion.suggestion.display)
            .collect_vec(),
        vec!["visible"]
    );
}
