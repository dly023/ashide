...
Let's say we are interested in parsing the input `ashide --version`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "ashide --version";
let start_offset = 0;
...
let word1 = Span::new(0,6);
let word2 = Span::new(6,7);
let word3 = Span::new(7,16);

assert_eq!(word1.slice(input), "ashide");
assert_eq!(word2.slice(input), " ");
assert_eq!(word3.slice(input), "--version");
```
...
let example = Spanned { item: String::from("ashide"), span: Span::new(0,6) };
assert_eq!(example.item, "ashide".to_string());
assert_eq!(example.span, Span::new(0,6));

let example = String::from("ashide").spanned(Span::new(0,6));
assert_eq!(example.item, "ashide".to_string());
assert_eq!(example.span, Span::new(0,6));

let example = "ashide -p --version";

let full_span = Span::new(0, example.len());
let first_flag_span = Span::new(7,9);

assert_eq!(first_flag_span.slice(example), "-p");
assert_eq!(first_flag_span.until(full_span), Span::new(7,19));
assert_eq!(first_flag_span.until(full_span).slice(example), "-p --version");

```

This is useful because as soon as the `lite` parse step happens. We get as output everything we need with `spans` correctly calculated. Let's go ahead and do a lite parse (the function `parse_tokens`) to our original example using as input the tokens generated by the lexer when processing the input `ashide --version`, like so:
...
let expected_word1 = String::from("ashide").spanned(Span::new(0,6));
let expected_word2 = String::from("--version").spanned(Span::new(7,16));
...
For more involved inputs (say commands separated by `|` and/or having `;`) the lite parser will effectively create the necessary `LitePipeline`s to express it, let's explore what happens if we lite parse the input `ashide config-set --extension-path="/path/to/dir" ; echo $ASHIDE_VAR"` (*two pipelines here due to the ; character*), like so:

```rust
let input = "ashide config-set --extension-path=\"/path/to/dir\" ; echo $ASHIDE_VAR";
let start_offset = 0;
