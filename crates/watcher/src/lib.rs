...
#[derive(Debug)]
enum LexicalPathState {
    Present,
    Missing,
    Unknown(io::Error),
}

fn lexical_path_state(path: &Path) -> LexicalPathState {
    match std::fs::symlink_metadata(path) {
        Ok(_) => LexicalPathState::Present,
        Err(error) if error.kind() == io::ErrorKind::NotFound => LexicalPathState::Missing,
        Err(error) => LexicalPathState::Unknown(error),
    }
}
    ...
    for fs_event in raw_fs_events {
        match fs_event.event.kind {
            ...
            EventKind::Modify(ModifyKind::Name(RenameMode::Any)) => {
                for path in &fs_event.event.paths {
                    // RenameMode::Any 不区分 source 与 destination。这里必须观察 lexical inode，
                    // 不能跟随 symlink target；dangling symlink destination 仍然存在，必须投影为 added row。
                    match lexical_path_state(path) {
                        LexicalPathState::Present => {
                            created.insert(path.clone());
                            continue;
                        }
                        LexicalPathState::Missing => {}
                        LexicalPathState::Unknown(error) => {
                            log::warn!(
                                "Failed to inspect lexical rename path {}: {error}",
                                path.display()
                            );
