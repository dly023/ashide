...
pub struct CustomizeUISlide {
    ...
    chip_local_drive_mouse: MouseStateHandle,
    // Buttons
...
impl CustomizeUISlide {
    pub(crate) fn new(
        ...
        Self {
            ...
            chip_local_drive_mouse: MouseStateHandle::default(),
    ...
    fn render_tools_panel_card(
        ...
        if ui.tools_panel_enabled(&intention) {
            ...
            chips.push(ChipSpec {
                label: localized("onboarding-customize-local-drive", "Ashide Drive"),
                is_enabled: ui.show_local_drive,
                mouse_state: self.chip_local_drive_mouse.clone(),
                on_click: Box::new(|ctx, _, _| {
                    ctx.dispatch_typed_action(CustomizeSlideAction::ToggleToolsSubSetting {
                        setting: ToolsPanelSubSetting::LocalDrive,
                    });
                }),
                on_hover: Some(Box::new(|is_hovered, ctx, _, _| {
                    if is_hovered {
                ...
                if !ui.tools_panel_enabled(&intention) {
                    ...
                    if is_agent {
                        match (chip, vertical) {
                            ...
                            (ToolsPanelSubSetting::LocalDrive, true) => "async/png/onboarding/agent_intention/customize_warpdrive_vertical.png",
                            (ToolsPanelSubSetting::LocalDrive, false) => "async/png/onboarding/agent_intention/customize_warpdrive_horizontal.png",
                        }
                    } else {
                        match (chip, vertical) {
                            (ToolsPanelSubSetting::ProjectExplorer, true) => "async/png/onboarding/terminal_intention/terminal_customize_fileexplorer_vertical.png",
                            (ToolsPanelSubSetting::ProjectExplorer, false) => "async/png/onboarding/terminal_intention/terminal_customize_fileexplorer_horizontal.png",
                            ...
                            (ToolsPanelSubSetting::LocalDrive, true) => "async/png/onboarding/terminal_intention/terminal_customize_warpdrive_vertical.png",
                            (ToolsPanelSubSetting::LocalDrive, false) => "async/png/onboarding/terminal_intention/terminal_customize_warpdrive_horizontal.png",
                        }
                    }
                }
            }
            ...
            CustomizeSlideAction::ToggleToolsSubSetting { setting } => {
                ...
                self.onboarding_state
                    .update(ctx, |model, ctx| match setting {
                        ...
                        ToolsPanelSubSetting::LocalDrive => {
                            let current = model.ui_customization().show_local_drive;
                            model.set_show_local_drive(!current, ctx);
                        }
                    });
                ctx.notify();
            }
