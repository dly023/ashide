...
impl ThemePickerSlide {
    ...
    fn render_theme_picker_content(
        ...
        let local_drive_enabled = state.ui_customization().show_local_drive;
        if is_terminal && !local_drive_enabled {
    ...
    fn render_bottom_nav(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let next_label = localized("common-get-warping", "Get Warping");
            ...
            .with_child(
                ui_builder
                    .span(localized(
                        ...
                        "Ashide stores onboarding choices locally on this device.",
                    ))
