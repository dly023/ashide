...
impl UICustomizationSettings {
    ...
    pub fn tools_panel_enabled(&self, _intention: &OnboardingIntention) -> bool {
        self.show_project_explorer || self.show_global_search || self.show_local_drive
    }
}
...
impl SelectedSettings {
    ...
    pub fn is_local_drive_enabled(&self) -> bool {
        match self {
            SelectedSettings::AgentDrivenDevelopment {
                ui_customization, ..
            } => ui_customization
                .as_ref()
                .map(|ui| ui.show_local_drive)
                .unwrap_or(true),
...
impl OnboardingStateModel {
    ...
    pub(crate) fn set_tools_panel_enabled(&mut self, enabled: bool, ctx: &mut ModelContext<Self>) {
        ...
        self.ui_customization.show_local_drive = enabled;
        ctx.notify();
    }
    ...
    pub(crate) fn set_show_local_drive(&mut self, value: bool, ctx: &mut ModelContext<Self>) {
        if self.ui_customization.show_local_drive == value {
            return;
        }
        self.ui_customization.show_local_drive = value;
        ctx.notify();
    }
