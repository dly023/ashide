...
impl NotificationSendError {
    pub fn notifications_error_banner_title(&self) -> &str {
        match self {
            NotificationSendError::PermissionsDenied | NotificationSendError::PermissionsNotYetGranted => "Ashide tried to send you a notification for the last block but does not have permission.",
            NotificationSendError::Other { .. } => "Ashide tried to send you a notification for the last block, but something went wrong.",
        }
    }
}
