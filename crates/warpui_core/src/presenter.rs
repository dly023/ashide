...
pub struct PositionCache {
    ...
    /// Position ids that were painted while building the latest completed scene.
    ///
    /// `committed_positions` intentionally retains layout anchors across frames, but a retained
    /// rectangle is not proof that its element still exists in the current event hit tree.
    positions_painted_in_current_frame: HashSet<String>,
...
impl PositionCache {
    pub fn new() -> Self {
        PositionCache {
            ...
            positions_painted_in_current_frame: Default::default(),
            drop_target_positions: Default::default(),
        }
    }

    /// Starts ownership tracking for a newly built scene.
    pub fn begin_frame(&mut self) {
        self.clear_single_frame_positions();
        self.positions_painted_in_current_frame.clear();
    }
    ...
    pub fn cache_position_indefinitely(&mut self, position_id: String, bounds: RectF) {
        if let Some(last) = self.pending_positions.last_mut() {
            ...
            self.positions_painted_in_current_frame.insert(position_id);
        }
    }
    ...
    pub fn cache_position_for_one_frame(&mut self, position_id: String, bounds: RectF) {
        if let Some(last) = self.pending_positions.last_mut() {
            last.insert(position_id.clone(), bounds);
            self.single_frame_positions.insert(position_id.clone());
            self.positions_painted_in_current_frame.insert(position_id);
        }
    }
    ...
    /// Returns a position only when its element was painted into the latest completed scene.
    /// Use this for pointer interaction; layout anchors may continue using [`Self::get_position`].
    pub fn position_painted_in_current_frame<S>(&self, position_id: S) -> Option<RectF>
    where
        S: AsRef<str>,
    {
        let position_id = position_id.as_ref();
        self.positions_painted_in_current_frame
            .contains(position_id)
            .then(|| self.committed_positions.get(position_id).copied())
            .flatten()
    }

    /// Returns a current-scene position only when it has non-empty pointer hit geometry.
    ///
    /// A zero-width or zero-height rectangle can be emitted while an asynchronously laid-out
    /// element is materializing. Its presence proves paint ownership, but its center is not a
    /// valid pointer target and may route the event to an adjacent or underlying element.
    pub fn interactive_position_painted_in_current_frame<S>(&self, position_id: S) -> Option<RectF>
    where
        S: AsRef<str>,
    {
        self.position_painted_in_current_frame(position_id)
            .filter(|bounds| bounds.size().x() > 0.0 && bounds.size().y() > 0.0)
    }
...
impl Presenter {
    ...
    pub fn build_scene(
        ...
        self.position_cache.begin_frame();
