...
pub enum IntegrationTestEvent {
    ...
    WithSavedPosition(String, MouseEvent, SavedPositionRequirement),
    /// Similar to WithEventFn, but used to dispatch an event at the saved position.
    WithSavedPositionFn(SavedPositionFn, MouseEvent, SavedPositionRequirement),
}
...
#[derive(Clone, Copy)]
pub enum SavedPositionRequirement {
    /// The target must have been painted into the latest completed scene.
    CurrentScene,
    /// Reuse the last committed geometry after the target intentionally became non-interactive.
    ///
    /// This is only for negative interaction tests which must click the same screen point after a
    /// target disappears. Normal pointer automation must use [`Self::CurrentScene`].
    LastCommittedGeometry,
}
...
/// 一个测试步骤依次包含：
/// 1) setup 队列，例如文件系统准备。
/// 2) 必须在事件派发前通过的 readiness assertions。
/// 3) 派发到当前活动窗口的事件队列。
/// 4) 在结果 assertions 前执行的 action 队列；action 持有 `&mut App`，可以修改应用状态。
/// 5) 验证应用状态和最后一帧的结果 assertions。
/// 6) assertion 超时时间；超时前会持续检查，直到成功。
pub struct TestStep {
    name: String,
    setup_functions: VecDeque<IntegrationTestSetupFn>,
    event_readiness_assertions: Vec<Assertion>,
...
impl TestStep {
    pub fn new(name: &str) -> Self {
        ...
        Self {
            ...
            event_readiness_assertions: Default::default(),
    ...
    /// 在派发本步骤的事件前等待指定条件成立。
    ///
    /// 焦点、paint 和生命周期状态会决定输入事件最终投递到哪里，必须在事件前验证；
    /// 事件派发后的 assertion 无法阻止事件落到过期或错误的目标上。
    pub fn add_named_event_readiness_assertion<N, F>(mut self, name: N, callback: F) -> Self
    where
        N: Into<String>,
        F: FnMut(&mut App, WindowId) -> AssertionOutcome + 'static,
    {
        self.event_readiness_assertions.push(Assertion {
            name: Some(name.into()),
            callback: CallbackType::Assertion(Box::new(callback)),
        });
        self
    ...
    /// Waits until an element has completed paint and is available for pointer interaction.
    ///
    /// A view-model state such as "menu open" is not sufficient readiness for a follow-up click:
    /// the corresponding element must also have reached the presenter's position cache.
    pub fn add_saved_position_readiness_assertion(self, position_id: impl Into<String>) -> Self {
        let position_id = position_id.into();
        let assertion_name = format!("saved position '{position_id}' is interactive");
        self.add_named_assertion(assertion_name, move |app, window_id| {
            let Some(presenter) = app.presenter(window_id) else {
                return AssertionOutcome::failure(format!(
                    "Window {window_id:?} has no presenter while waiting for {position_id}"
                ));
            };
            let is_ready = presenter
                .borrow()
                .position_cache()
                .interactive_position_painted_in_current_frame(&position_id)
                .is_some();
            if is_ready {
                AssertionOutcome::Success
            } else {
                AssertionOutcome::failure(format!(
                    "Saved position {position_id} has not been painted yet"
                ))
            }
        })
    }
        ...
        F: Fn(&mut App, WindowId) -> String + 'static,
    {
        self.events
            .push_back(IntegrationTestEvent::WithSavedPositionFn(
                ...
                SavedPositionRequirement::CurrentScene,
            ));
        self
    }
    ...
    /// Clicks the last committed geometry after the original target intentionally stopped being
    /// interactive.
    ///
    /// Use this only for a negative assertion that verifies the old screen point no longer acts as
    /// the removed target. All ordinary clicks must use [`Self::with_click_on_saved_position`].
    pub fn with_click_on_last_committed_saved_position<S: Into<String>>(
        mut self,
        position_id: S,
    ) -> Self {
        self.events
            .push_back(IntegrationTestEvent::WithSavedPosition(
                position_id.into(),
                MouseEvent::ClickOnce,
                SavedPositionRequirement::LastCommittedGeometry,
            ));
        self
    }
...
async fn wait_for_event_readiness_assertions(
    ...
    'assertions: for assertion in assertions {
        let assertion_name = assertion.name.as_deref().unwrap_or("unnamed");
        let mut last_failure = None;
        let mut attempt = 0;

        while attempt == 0 || Instant::now() < deadline {
            if sigint_received.load(Ordering::Relaxed) {
                log::info!(
                    "Test interrupted by Ctrl+C during event readiness assertion '{assertion_name}'"
                );
            ...
            if attempt == 0 {
                log::info!("entering event readiness loop for '{assertion_name}'");
                if let Some(log) = action_log::get_action_log_mut(step_data_map) {
                    log.record(format!("Event readiness started: {assertion_name}"));
                }
            }
            attempt += 1;

            let outcome = match &mut assertion.callback {
                CallbackType::Assertion(callback) => callback(app, window_id),
                CallbackType::AssertionWithData(callback) => {
                    callback(app, window_id, step_data_map)
                }
            };
            match outcome {
                AssertionOutcome::Success => {
                    if let Some(log) = action_log::get_action_log_mut(step_data_map) {
                        log.record(format!("Event readiness passed: {assertion_name}"));
                    }
                    continue 'assertions;
                }
                ...
                AssertionOutcome::PreconditionFailed(message) => {
                    return AssertionOutcome::PreconditionFailed(message);
                }
                AssertionOutcome::Failure {
                    message,
                    backtrace,
                    ...
                    if let Err(error) = maybe_render_frame(app, window_id, deadline).await {
                        return error;
                    }
                }
                AssertionOutcome::ImmediateFailure {
                ...
                AssertionOutcome::Canceled => return AssertionOutcome::Canceled,
            }
        }

        return last_failure.unwrap_or_else(|| {
            AssertionOutcome::failure(format!(
                "Event readiness assertion '{assertion_name}' did not complete before the step deadline"
            ))
        });
    }
...
pub(super) async fn run_step(
    ...
    match wait_for_event_readiness_assertions(
        &mut step.event_readiness_assertions,
        app,
        window_id,
        step_data_map,
        sigint_received,
    ...
    for e in &step.events {
        ...
        match e {
            ...
            IntegrationTestEvent::WithSavedPosition(_, mouse_event, requirement)
            | IntegrationTestEvent::WithSavedPositionFn(_, mouse_event, requirement) => {
                let presenter = app.presenter(window_id).expect("Invalid window id");
                let position_id = match e {
                    IntegrationTestEvent::WithSavedPosition(position_id, _, _) => {
                        position_id.to_string()
                    }
                    IntegrationTestEvent::WithSavedPositionFn(position_id_fn, _, _) => {
                ...
                let bounds = loop {
                    if let Some(bounds) = {
                        ...
                        match requirement {
                            SavedPositionRequirement::CurrentScene => presenter_ref
                                .position_cache()
                                .interactive_position_painted_in_current_frame(&position_id),
                            SavedPositionRequirement::LastCommittedGeometry => {
                    ...
                    if Instant::now() >= deadline {
                        ...
                        return AssertionOutcome::failure(format!(
                            "No position for {position_id} before step deadline"
                        ));
                    }
