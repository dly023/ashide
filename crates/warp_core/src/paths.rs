...
/// The name of the directory in which to put non-global Ashide-specific files.
///
/// This should be used, for example, as the base directory under which
/// repository workflows are stored (in `./.ashide/workflows`).
pub const WARP_CONFIG_DIR: &str = ".ashide";

/// The name of the folder that stores Ashide execution logs and network logs.
...
/// Returns the home-relative Ashide config directory name for the current channel and data profile.
///
/// This keeps Ashide home config under the `.ashide*` directory family.
...
/// Returns the home-relative Ashide config directory for the current channel and data profile.
///
/// Unlike [`data_dir`] and [`config_local_dir`] on non-macOS platforms, this intentionally keeps
/// Ashide-authored, user-facing config under a `.ashide*` directory in the home directory instead of
...
///
/// Stable/Oss use `.ashide`, while other channels include a channel suffix
/// (e.g., `.ashide-dev`, `.ashide-local`).
#[cfg(target_os = "macos")]
fn macos_config_dir_name() -> String {
    match ChannelState::channel() {
        Channel::Stable => WARP_CONFIG_DIR.to_owned(),
        Channel::Preview => format!("{WARP_CONFIG_DIR}-preview"),
        Channel::Oss => ".ashide".to_owned(),
        Channel::Dev => format!("{WARP_CONFIG_DIR}-dev"),
        Channel::Integration => format!("{WARP_CONFIG_DIR}-integration"),
        Channel::Local => format!("{WARP_CONFIG_DIR}-local"),
    }
...
/// the user had open and cached state of known Ashide Drive objects.
pub fn state_dir() -> PathBuf {
    // 集成测试会把 HOME 替换为每个测试独享的目录。macOS 上
    // `directories::ProjectDirs` 仍会解析到真实用户的 Library,导致并行测试
    // 共享并损坏同一个 SQLite。所有平台的 Integration 状态必须统一落在
    // 已隔离的 channel data 目录下。
...
/// Returns an application-identity-scoped directory that intentionally ignores
/// [`ChannelState::data_profile`].
///
/// This is reserved for process-coordination resources whose ownership belongs
/// to the installed application itself, such as the desktop GUI singleton lock
/// and startup IPC endpoint. Persisted user data and ordinary caches must keep
/// using their profile-scoped directory helpers.
pub fn application_identity_dir() -> PathBuf {
    let Some(project_dirs) = project_dirs_for_app_id(ChannelState::app_id(), None) else {
        return PathBuf::new();
    };
    cfg_if! {
        if #[cfg(target_os = "macos")] {
...
fn project_dirs_for_app_id(
    ...
    cfg_if::cfg_if! {
        if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            // Adjust the base application name so Linux config/data paths match
            // the Ashide package name.
            let base_app_name = match app_id.application_name() {
                "Ashide" => "ashide".to_owned(),
                other if other.starts_with("Ashide") => other.replace("Ashide", "ashide-"),
...
/// Returns the path to resources included in the Ashide distribution.
///
/// Unlike [`warpui::AssetProvider`] assets, which are generally embedded in the binary, these are
/// stored on the filesystem alongside the rest of Ashide.
///
/// ## macOS
/// The resources directory is `$APP_DIR/Contents/Resources` (e.g. `/Applications/Ashide.app/Contents/Resources`).
///
/// ## Linux
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` depends on the
/// specific package manager. For example, on Ubuntu this might be `/opt/ashide/ashide/resources`.
///
/// ## Windows
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` is the directory
/// containing the Ashide executable (e.g. `C:\Program Files\Ashide\resources`).
