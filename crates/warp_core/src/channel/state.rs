...
impl ChannelState {
    pub fn init() -> Self {
        ...
        let app_id = AppId::new("dev", "ashide", "Ashide");
        Self {
            channel,
    ...
    pub fn new(mut channel: Channel, mut config: ChannelConfig) -> Self {
        if let Some(app_id) = app_id_from_bundle() {
            if let Some(bundle_channel) = channel_from_app_id(&app_id) {
                channel = bundle_channel;
                config.logfile_name = logfile_name_from_channel(bundle_channel).into();
            }
    ...
    ///
    /// This is a debugging tool for isolating development instances of Ashide, and is not
    ...
    pub fn url_scheme() -> &'static str {
        match Self::channel() {
            Channel::Stable | Channel::Oss => "ashide",
...
fn channel_from_app_id(app_id: &AppId) -> Option<Channel> {
    match app_id.application_name() {
        "AshideDev" => Some(Channel::Dev),
        "AshideLocal" => Some(Channel::Local),
        "AshidePreview" => Some(Channel::Preview),
        "AshideIntegration" => Some(Channel::Integration),
        "Ashide" => Some(Channel::Oss),
        _unknown_app_name => None,
    }
}

fn logfile_name_from_channel(channel: Channel) -> &'static str {
    match channel {
        Channel::Stable | Channel::Oss => "ashide.log",
