...
/// Execution mode that Ashide is running under.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExecutionMode {
    /// Ashide is running as a normal desktop app.
    App,
    /// Ashide is running as a CLI.
    Sdk,
}
...
impl ExecutionMode {
    ...
    pub fn client_id(&self) -> &'static str {
        match self {
            ExecutionMode::App => "ashide-app",
            ExecutionMode::Sdk => "ashide-cli",
        }
    }
}

/// Model tracking the mode that Ashide is running in.
///
/// This gates functionality that's disabled when Ashide is running in SDK mode.
#[derive(Clone, Debug)]
...
impl AppExecutionMode {
    ...
    /// If true, Ashide is running in a sandbox like a Docker container or VM, rather than directly
    /// on a user machine.
...
/// Returns the current global client ID string ("ashide-app" or "ashide-cli").
