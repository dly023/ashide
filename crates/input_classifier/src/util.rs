...
lazy_static! {
    ...
    ///
    /// `claude` and `codex` are not actually _really_ one-off shell command keywords,
    /// but false-positive NL classifications for these inputs (where the user was trying to use
    /// Claude Code or Codex CLI) suck, because the user often thinks we're
    ...
    static ref ONE_OFF_SHELL_COMMAND_KEYWORDS: HashSet<&'static str> = HashSet::from(["#", "echo", "man", "sudo", "claude", "codex"]);
