...
pub struct RemoteServerManager {
    ...
    /// Session aliases that borrow another session's remote-server connection.
    ///
    /// Environment-owned runtimes keep a long-lived synthetic owner session.
    /// Native PTYs opened inside that runtime later bootstrap their own shell
    /// session IDs. Those shell sessions must resolve to the owner for
    /// host/client lookups, but closing one PTY must not tear down the owner
    /// connection.
    session_aliases: HashMap<SessionId, SessionId>,
...
impl RemoteServerManager {
    ...
    fn canonical_session_id(&self, session_id: SessionId) -> SessionId {
        ...
        while let Some(next) = self.session_aliases.get(&current).copied() {
    ...
    pub fn register_session_alias(
        ...
        if alias_session_id == target_session_id {
            self.session_aliases.remove(&alias_session_id);
            return;
        }

        let target_session_id = self.canonical_session_id(target_session_id);
        if !self.is_session_potentially_active(target_session_id) {
            log::warn!(
                "register_session_alias: target session {target_session_id:?} is not active; \
                 refusing alias {alias_session_id:?}"
            );
            return;
        }

        self.session_aliases
            .insert(alias_session_id, target_session_id);
    }
    ...
    pub fn deregister_session(&mut self, session_id: SessionId, ctx: &mut ModelContext<Self>) {
        ...
        if self.session_aliases.remove(&session_id).is_some() {
        ...
        let removed_aliases = self
            ...
            .collect::<Vec<_>>();
        for alias_session_id in removed_aliases {
            self.session_aliases.remove(&alias_session_id);
            self.last_navigated_path.remove(&alias_session_id);
            self.session_bootstrap_info.remove(&alias_session_id);
