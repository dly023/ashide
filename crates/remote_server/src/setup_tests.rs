...
#[test]
fn parse_preinstall_glibc_too_old_is_unsupported() {
    ...
    assert_eq!(
        ...
        PreinstallStatus::Unsupported {
            reason: UnsupportedReason::GlibcTooOld {
                detected: GlibcVersion::new(2, 16),
                required: GlibcVersion::new(2, 17),
            },
        }
    );
    assert!(!result.is_supported());
...
#[test]
fn parse_preinstall_non_glibc_is_unsupported() {
    let stdout = "required_glibc=2.17\n\
                  libc_family=musl\n\
                  status=unsupported\n\
                  reason=non_glibc\n";
...
#[test]
fn oss_remote_server_dir_uses_ashide_namespace() {
    assert_eq!(remote_server_dir(), "~/.ashide/remote-server");
}

#[test]
fn oss_binary_name_matches_ashide_cli() {
    assert_eq!(binary_name(), "ashide");
}

#[test]
fn oss_source_build_remote_server_binary_uses_dev_protocol_slot_v10() {
    assert!(remote_server_binary().ends_with("/ashide-dev-pty-v10"));
}

#[test]
fn release_remote_server_slot_includes_protocol_revision() {
    assert_eq!(release_remote_server_slot("v0.3.0"), "v0.3.0-pty-v10");
}

#[test]
fn binary_check_requires_environment_runtime_proxy_identity_key_support() {
    let command = binary_check_command();

    assert!(command.contains("--version"));
    assert!(command.contains("environment-runtime-proxy --help"));
    assert!(command.contains("grep -q -- --identity-key"));
}

#[test]
fn release_helper_asset_url_targets_per_platform_tarball() {
    let url = release_helper_asset_url(&RemotePlatform {
        os: RemoteOs::Linux,
        arch: RemoteArch::X86_64,
    });

    // local-first 交付:本地 app 拉这个 URL,再 SSH 推给远端。
    assert!(url.starts_with("https://github.com/dly023/ashide/releases/"));
    assert!(url.ends_with("/ashide-linux-x86_64.tar.gz"));
}

#[test]
fn release_helper_asset_url_varies_by_arch() {
    let aarch64 = release_helper_asset_url(&RemotePlatform {
        os: RemoteOs::Linux,
        arch: RemoteArch::Aarch64,
    });
    assert!(aarch64.ends_with("/ashide-linux-aarch64.tar.gz"));
}
