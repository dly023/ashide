...
///
/// The slot is derived from the single wire-protocol revision owner, so a
/// protocol bump cannot accidentally keep reusing an incompatible uploaded
/// helper path.
fn dev_remote_server_slot() -> String {
    format!("dev-pty-v{}", crate::REMOTE_SERVER_PROTOCOL_REVISION)
}

/// Tag release remote-server install slot.
///
/// A release version identifies the product build, while the protocol revision
/// identifies wire compatibility. Both are part of the install identity so a
/// same-tag rebuild cannot silently reuse a helper from an older protocol.
fn release_remote_server_slot(version: &str) -> String {
    format!("{version}-pty-v{}", crate::REMOTE_SERVER_PROTOCOL_REVISION)
}
    ...
    match ChannelState::channel() {
        ...
        Channel::Stable | Channel::Preview | Channel::Dev | Channel::Integration | Channel::Oss => {
            format!(
                "{dir}/{}-{}",
                binary_name(),
                release_remote_server_slot(pinned_version())
            )
        }
    }
}
...
/// 返回检查远端 remote-server 二进制存在且暴露当前启动面的预检命令。
///
/// 该 help 探针只负责安装前快速判定，不是协议兼容证明。真正的 wire/semantic
/// compatibility 由 Initialize 双向 protocol revision fail-closed 校验。
...
pub const DEV_CROSS_COMPILE_TIMEOUT: Duration = Duration::from_secs(1800);
