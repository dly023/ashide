...
use repo_metadata::file_tree_update::{
    ...
    RepoNodeMetadata, SymlinkNodeMetadata,
};
...
impl From<&RepoNodeMetadata> for proto::RepoNodeMetadata {
    fn from(node: &RepoNodeMetadata) -> Self {
        let node_oneof = match node {
            ...
            RepoNodeMetadata::Symlink(symlink) => {
                proto::repo_node_metadata::Node::Symlink(proto::SymlinkNodeMetadata {
                    path: symlink.path.to_string(),
                    target_kind: symlink_target_kind_to_proto(symlink.target_kind),
                    ignored: symlink.ignored,
                    loaded: symlink.loaded,
                })
            }
        };
        Self {
        ...
        for child_path in &children {
            match entry.get(child_path) {
                ...
                Some(FileTreeEntryState::Symlink(symlink)) => {
                    subtree_metadata.push(proto::RepoNodeMetadata {
                        node: Some(proto::repo_node_metadata::Node::Symlink(
                            proto::SymlinkNodeMetadata {
                                path: symlink.path.to_string(),
                                target_kind: symlink_target_kind_to_proto(symlink.target_kind),
                                ignored: symlink.ignored,
                                loaded: symlink.loaded,
...
fn proto_to_repo_node_metadata(proto_node: &proto::RepoNodeMetadata) -> Option<RepoNodeMetadata> {
    match proto_node.node.as_ref()? {
        ...
        proto::repo_node_metadata::Node::Symlink(symlink) => {
            let path = StandardizedPath::try_new(&symlink.path).ok()?;
            let target_kind = proto_to_symlink_target_kind(symlink.target_kind)?;
            Some(RepoNodeMetadata::Symlink(SymlinkNodeMetadata {
                path,
                target_kind,
                ignored: symlink.ignored,
                loaded: symlink.loaded,
...
fn symlink_target_kind_to_proto(target_kind: SymlinkTargetKind) -> i32 {
    match target_kind {
        SymlinkTargetKind::File => proto::FileSystemEntryKind::File as i32,
        SymlinkTargetKind::Directory => proto::FileSystemEntryKind::Directory as i32,
        SymlinkTargetKind::Other => proto::FileSystemEntryKind::Other as i32,
        SymlinkTargetKind::Missing => proto::FileSystemEntryKind::Missing as i32,
    }
}

fn proto_to_symlink_target_kind(target_kind: i32) -> Option<SymlinkTargetKind> {
    match proto::FileSystemEntryKind::try_from(target_kind).ok()? {
        proto::FileSystemEntryKind::File => Some(SymlinkTargetKind::File),
        proto::FileSystemEntryKind::Directory => Some(SymlinkTargetKind::Directory),
        proto::FileSystemEntryKind::Other => Some(SymlinkTargetKind::Other),
        proto::FileSystemEntryKind::Missing => Some(SymlinkTargetKind::Missing),
        proto::FileSystemEntryKind::Unspecified | proto::FileSystemEntryKind::Symlink => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn symlink_metadata_round_trip_preserves_lexical_path_and_target_kind() {
        for target_kind in [
            ...
            let original = RepoNodeMetadata::Symlink(SymlinkNodeMetadata {
                path: StandardizedPath::try_new("/repo/link").unwrap(),
                target_kind,
                ignored: true,
                loaded: target_kind != SymlinkTargetKind::Directory,
            });
            let encoded = proto::RepoNodeMetadata::from(&original);
            let decoded = proto_to_repo_node_metadata(&encoded).unwrap();

            let RepoNodeMetadata::Symlink(decoded) = decoded else {
                panic!("decoded node must remain a symlink");
            };
            assert_eq!(decoded.path.as_str(), "/repo/link");
            assert_eq!(decoded.target_kind, target_kind);
            assert!(decoded.ignored);
            assert_eq!(decoded.loaded, target_kind != SymlinkTargetKind::Directory);
        }
    }
}
