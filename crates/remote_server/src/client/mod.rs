...
use crate::proto::{
    ...
    GetCliAgentSessionUserStateResponse, Initialize, InitializeResponse, ListDirectory,
    ListDirectoryResponse, LoadRepoMetadataDirectoryResponse, MutateCliAgentSession,
    MutateCliAgentSessionResponse, MutateCliAgentSessionUserState,
...
pub enum ClientError {
    ...
    #[error("Remote helper protocol revision mismatch: expected {expected}, received {received}")]
    ProtocolRevisionMismatch { expected: u32, received: u32 },
...
pub enum ClientEvent {
    ...
    PtyOutput { pty_id: u64, bytes: Vec<u8> },
    /// A native remote PTY exited or was closed.
    PtyExited { pty_id: u64, exit_code: Option<i32> },
    /// A server message could not be decoded and had no parseable request_id.
    MessageDecodingError,
}
#[cfg(test)]
const REQUIRED_RESULT_RESPONSE_NAMES: &[&str] = &[
...
fn validate_required_response_result(response: &ServerMessage) -> Result<(), ClientError> {
    let missing_result = match response.message.as_ref() {
        // BEGIN REQUIRED_RESULT_RESPONSE_VARIANTS
        Some(server_message::Message::AbortFileTransferResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::AppendFileResponse(response)) => response.result.is_none(),
        Some(server_message::Message::BeginFileTransferResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::CreateDirectoryResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::CreatePtyResponse(response)) => response.result.is_none(),
        Some(server_message::Message::DeleteDirectoryResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::DeleteFileResponse(response)) => response.result.is_none(),
        Some(server_message::Message::GetCliAgentSessionUserStateResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::ListDirectoryResponse(response)) => response.result.is_none(),
        Some(server_message::Message::MutateCliAgentSessionResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::MutateCliAgentSessionUserStateResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::ReadCliAgentSessionResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::ReadFileChunkResponse(response)) => response.result.is_none(),
        Some(server_message::Message::ExactRenameResponse(response)) => response.result.is_none(),
        Some(server_message::Message::FinishFileTransferResponse(response)) => {
            response.result.is_none()
        }
        ...
        Some(server_message::Message::RunCommandResponse(response)) => response.result.is_none(),
        Some(server_message::Message::SaveBufferResponse(response)) => response.result.is_none(),
        Some(server_message::Message::ScanCliAgentSessionsResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::WriteFileChunkResponse(response)) => {
            response.result.is_none()
        }
        Some(server_message::Message::WriteFileResponse(response)) => response.result.is_none(),
        // END REQUIRED_RESULT_RESPONSE_VARIANTS
        Some(
            server_message::Message::InitializeResponse(_)
            | server_message::Message::Error(_)
            | server_message::Message::NavigatedToDirectoryResponse(_)
            | server_message::Message::RepoMetadataSnapshot(_)
            | server_message::Message::RepoMetadataUpdate(_)
            | server_message::Message::LoadRepoMetadataDirectoryResponse(_)
            | server_message::Message::ReadFileContextResponse(_)
            | server_message::Message::OpenBufferResponse(_)
            | server_message::Message::BufferUpdated(_)
            | server_message::Message::PtyOutput(_)
            | server_message::Message::PtyExited(_),
        ) => false,
        None => false,
    };
    ...
    if missing_result {
        log::error!(
            "Remote server response omitted required oneof result: {:?}",
            response.message
        );
...
/// so that `kill_on_drop` fires when teardown occurs. In Ashide this is
...
impl RemoteServerClient {
    ...
    /// session's (in Ashide, on the `RemoteServerManager`'s
...
impl RemoteServerClient {
    ...
    pub async fn initialize(
        ...
        match response.message {
            Some(server_message::Message::InitializeResponse(resp)) => {
                ...
                if resp.protocol_revision != expected {
                    return Err(ClientError::ProtocolRevisionMismatch {
                        expected,
                        received: resp.protocol_revision,
                    });
                }
                Ok(resp)
            }
    ...
    /// Removes one exact terminal executor while keeping its transport owner
    /// and sibling aliases connected.
    pub fn notify_session_deregistered(&self, session_id: SessionId) {
        let msg = ClientMessage {
            request_id: String::new(),
            message: Some(
                client_message::Message::SessionExecutionContextDeregistered(
                    SessionExecutionContextDeregistered {
                        session_id: session_id.as_u64(),
                    },
                ),
            ),
    ...
    pub async fn write_file(&self, path: String, content: String) -> Result<(), ClientError> {
        ...
        match response.message {
            Some(server_message::Message::WriteFileResponse(resp)) => match resp.result {
                Some(crate::proto::write_file_response::Result::Success(_)) => Ok(()),
    ...
    pub async fn delete_file(&self, path: String) -> Result<(), ClientError> {
        ...
        match response.message {
            Some(server_message::Message::DeleteFileResponse(resp)) => match resp.result {
                Some(crate::proto::delete_file_response::Result::Success(_)) => Ok(()),
    ...
    /// Recursively deletes the selected remote directory after revalidating its identity.
    pub async fn delete_directory(
        ...
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::DeleteDirectory(DeleteDirectory {
                path,
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::DeleteDirectoryResponse(resp)) => match resp.result {
                Some(crate::proto::delete_directory_response::Result::Success(_)) => Ok(()),
                Some(crate::proto::delete_directory_response::Result::Error(e)) => {
                    Err(ClientError::FileOperationFailed(e.message))
                }
                None => Err(ClientError::UnexpectedResponse),
            },
            other => {
                log::error!("Unexpected response variant for DeleteDirectory: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }

    /// Renames to one exact target and returns the backend-committed path.
    pub async fn exact_rename(&self, from: String, to: String) -> Result<String, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::ExactRename(ExactRename {
                from_path: from,
                to_path: to,
            })),
        };
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::ExactRenameResponse(resp)) => match resp.result {
                Some(crate::proto::exact_rename_response::Result::Success(success)) => {
                    Ok(success.committed_path)
                }
                None => Err(ClientError::UnexpectedResponse),
                Some(crate::proto::exact_rename_response::Result::Error(e)) => {
                    Err(ClientError::FileOperationFailed(e.message))
                }
                Some(crate::proto::exact_rename_response::Result::Conflict(conflict)) => {
                    Err(ClientError::FileOperationFailed(format!(
            ...
            other => {
                log::error!("Unexpected response variant for ExactRename: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }

    /// Ashide:列举远端主机上某个目录的直接子项。
    ///
    ...
    /// Scans recent Claude/Codex session files on the remote host and returns
    /// a deduplicated, oldest-first list of session records (newest-trimmed to
    /// `limit`). The daemon does the directory walk and JSONL parsing natively.
    pub async fn scan_cli_agent_sessions(
        &self,
        limit: u32,
        ...
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::ScanCliAgentSessions(
                ScanCliAgentSessions {
                    limit,
                    roots: Some(roots),
                    enabled_agents,
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::ScanCliAgentSessionsResponse(resp)) => Ok(resp),
            other => {
                log::error!("Unexpected response variant for ScanCliAgentSessions: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }
    ...
    pub async fn read_cli_agent_session(
        &self,
        source: String,
        provider_session_id: Option<String>,
        roots: CliAgentSessionStoreRoots,
    ) -> Result<ReadCliAgentSessionResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::ReadCliAgentSession(
                ReadCliAgentSession {
                    source,
                    provider_session_id,
                    roots: Some(roots),
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::ReadCliAgentSessionResponse(resp)) => Ok(resp),
            other => {
                log::error!("Unexpected response variant for ReadCliAgentSession: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }

    /// Archives or deletes a session source (file or codex index entry) on the
    /// remote host. The daemon performs the mutation natively.
    pub async fn mutate_cli_agent_session(
        ...
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::MutateCliAgentSession(
                MutateCliAgentSession {
                    source,
                    mutation,
                    roots: Some(roots),
                },
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::MutateCliAgentSessionResponse(resp)) => Ok(resp),
            other => {
                log::error!("Unexpected response variant for MutateCliAgentSession: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }

    /// Reads Ashide-owned Session Navigator user state from the remote host.
    pub async fn get_cli_agent_session_user_state(
        &self,
    ) -> Result<GetCliAgentSessionUserStateResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::GetCliAgentSessionUserState(
                GetCliAgentSessionUserState {},
            )),
        };
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::GetCliAgentSessionUserStateResponse(resp)) => Ok(resp),
            other => {
                log::error!(
                    "Unexpected response variant for GetCliAgentSessionUserState: {other:?}"
                );
                Err(ClientError::UnexpectedResponse)
            }
        }
    ...
    /// Mutates Ashide-owned Session Navigator user state on the remote host.
    pub async fn mutate_cli_agent_session_user_state(
        &self,
        keys: Vec<String>,
        mutation: i32,
        alias: Option<String>,
    ) -> Result<MutateCliAgentSessionUserStateResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::MutateCliAgentSessionUserState(
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::MutateCliAgentSessionUserStateResponse(resp)) => Ok(resp),
            other => {
                log::error!(
                    "Unexpected response variant for MutateCliAgentSessionUserState: {other:?}"
                );
                Err(ClientError::UnexpectedResponse)
            }
        }
    ...
    pub async fn append_file(
        ...
        path: String,
        bytes: Vec<u8>,
    ) -> Result<AppendFileResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::AppendFile(AppendFile {
                path,
                bytes,
            })),
        };
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::AppendFileResponse(response)) => Ok(response),
            other => {
                log::error!("Unexpected response variant for AppendFile: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }

    pub async fn begin_file_transfer(
        &self,
        path: String,
        direction: crate::proto::FileTransferDirection,
        executable: Option<bool>,
    ) -> Result<BeginFileTransferResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::BeginFileTransfer(
                BeginFileTransfer {
                    path,
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::BeginFileTransferResponse(response)) => Ok(response),
            other => {
                log::error!("Unexpected response variant for BeginFileTransfer: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }

    /// Reads the next byte range from an inode-pinned transfer handle.
    ...
    /// Writes the next byte range to an inode-pinned transfer handle.
    ...
    pub async fn finish_file_transfer(
        &self,
        handle: FileTransferHandle,
    ) -> Result<FinishFileTransferResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::FinishFileTransfer(
                FinishFileTransfer {
                    handle: Some(handle),
                },
            )),
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::FinishFileTransferResponse(response)) => Ok(response),
            other => {
                log::error!("Unexpected response variant for FinishFileTransfer: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }
    ...
    pub async fn abort_file_transfer(
        &self,
        handle: FileTransferHandle,
    ) -> Result<AbortFileTransferResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::AbortFileTransfer(
                AbortFileTransfer {
                    handle: Some(handle),
                },
            )),
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::AbortFileTransferResponse(response)) => Ok(response),
            other => {
                log::error!("Unexpected response variant for AbortFileTransfer: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }
    ...
    pub async fn promote_files(
        ...
        overwrite: bool,
        directory_overwrite_roots: Vec<String>,
    ) -> Result<PromoteFilesResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::PromoteFiles(PromoteFiles {
                targets,
                overwrite,
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::PromoteFilesResponse(response)) => Ok(response),
            other => {
                log::error!("Unexpected response variant for PromoteFiles: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }
    ...
    ///
    /// Ashide:与其它 fire-and-forget 通知不同,buffer 编辑投递失败必须上报。
    ...
    fn push_message_to_event(msg: ServerMessage) -> Option<ClientEvent> {
        match msg.message? {
            ...
            server_message::Message::PtyOutput(push) => Some(ClientEvent::PtyOutput {
                pty_id: push.pty_id,
                bytes: push.bytes,
            }),
            server_message::Message::PtyExited(push) => Some(ClientEvent::PtyExited {
                pty_id: push.pty_id,
    ...
    /// Creates a persistent PTY on the remote host.
    pub async fn create_pty(
        ...
        cols: u32,
        environment_variables: HashMap<String, String>,
    ) -> Result<CreatePtyResponse, ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::CreatePty(CreatePty {
                working_directory,
                shell,
                rows,
                cols,
        ...
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::CreatePtyResponse(resp)) => Ok(resp),
            other => {
                log::error!("Unexpected response variant for CreatePty: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }

    /// Writes raw bytes to a persistent remote PTY.
    pub fn write_pty(&self, pty_id: u64, bytes: Vec<u8>) -> Result<(), ClientError> {
        let msg = ClientMessage {
            request_id: String::new(),
            message: Some(client_message::Message::WritePty(WritePty {
                pty_id,
                bytes,
            })),
        };
        self.outbound_tx.try_send(msg).map_err(|e| {
            log::error!("Failed to enqueue PTY input: {e}");
            ClientError::Disconnected
        })
    }

    /// Resizes a persistent remote PTY.
    pub fn resize_pty(
        &self,
        pty_id: u64,
        rows: u32,
        ...
        let msg = ClientMessage {
            request_id: String::new(),
            message: Some(client_message::Message::ResizePty(ResizePty {
                pty_id,
                rows,
                cols,
                width_px,
        ...
        self.outbound_tx.try_send(msg).map_err(|e| {
            log::error!("Failed to enqueue PTY resize: {e}");
            ClientError::Disconnected
        })
    }

    /// Closes a persistent remote PTY.
    pub fn close_pty(&self, pty_id: u64) -> Result<(), ClientError> {
        let msg = ClientMessage {
            request_id: String::new(),
            message: Some(client_message::Message::ClosePty(ClosePty { pty_id })),
        };
        self.outbound_tx.try_send(msg).map_err(|e| {
            log::error!("Failed to enqueue PTY close: {e}");
            ClientError::Disconnected
        })
    }
    ...
    async fn send_request(
        ...
        validate_required_response_result(&response)?;
        Ok(response)
    }
