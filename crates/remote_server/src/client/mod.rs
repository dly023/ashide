...
use crate::proto::{
    ...
    ClosePty, CreateDirectory, CreateDirectoryResponse, CreatePty, CreatePtyResponse, DeleteFile,
    ErrorCode, GetCliAgentSessionUserState, GetCliAgentSessionUserStateResponse, Initialize,
    InitializeResponse, ListDirectory, ListDirectoryResponse, LoadRepoMetadataDirectoryResponse,
    MutateCliAgentSession, MutateCliAgentSessionResponse, MutateCliAgentSessionUserState,
    MutateCliAgentSessionUserStateResponse, NavigatedToDirectoryResponse, OpenBuffer,
    OpenBufferResponse, ReadCliAgentSession, ReadCliAgentSessionResponse, ReadFileChunk,
    ReadFileChunkResponse, ReadFileContextRequest, ReadFileContextResponse, RenameFile, ResizePty,
    ResolveConflict, ResolveConflictResponse, ResolvePath, ResolvePathResponse, RunCommandRequest,
    RunCommandResponse, SaveBuffer, SaveBufferResponse, ScanCliAgentSessions,
    ScanCliAgentSessionsResponse, ServerMessage, SessionBootstrapped, TextEdit, WriteFile,
    WriteFileChunk, WriteFileChunkResponse, WritePty,
};
...
impl RemoteServerClient {
    ...
    /// Renames (moves) a file or directory on the remote host.
    pub async fn rename_file(&self, from: String, to: String) -> Result<(), ClientError> {
        let request_id = RequestId::new();
        let msg = ClientMessage {
            request_id: request_id.to_string(),
            message: Some(client_message::Message::RenameFile(RenameFile {
                from_path: from,
                to_path: to,
            })),
        };
        let response = self.send_request(request_id, msg).await?;
        match response.message {
            Some(server_message::Message::RenameFileResponse(resp)) => match resp.result {
                Some(crate::proto::rename_file_response::Result::Success(_)) | None => Ok(()),
                Some(crate::proto::rename_file_response::Result::Error(e)) => {
                    Err(ClientError::FileOperationFailed(e.message))
                }
            },
            other => {
                log::error!("Unexpected response variant for RenameFile: {other:?}");
                Err(ClientError::UnexpectedResponse)
            }
        }
    }
    ...
    pub async fn mutate_cli_agent_session(
        ...
        let msg = ClientMessage {
            ...
            message: Some(client_message::Message::MutateCliAgentSession(
                MutateCliAgentSession { source, mutation },
            )),
        };
        let response = self.send_request(request_id, msg).await?;
        match response.message {
