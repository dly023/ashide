...
use crate::proto::{
    client_message, create_pty_response, delete_directory_response, read_file_chunk_response,
    resolve_path_response, run_command_response, server_message, write_file_chunk_response,
    ClientMessage, CreatePtyResponse, CreatePtySuccess, DeleteDirectoryIdentity,
    DeleteDirectoryResponse, DeleteDirectorySuccess, DeleteFileResponse, ErrorCode,
    FileSystemEntryKind, FileTransferHandle, InitializeResponse, PtyExitedPush, PtyOutputPush,
    ReadFileChunkResponse, ReadFileChunkSuccess, ResolvePathResponse, ResolvePathSuccess,
    RunCommandResponse, RunCommandSuccess, ServerMessage, WriteFileChunkResponse,
...
async fn initialize_round_trip() {
    ...
    assert_eq!(
        resp.protocol_revision,
        crate::REMOTE_SERVER_PROTOCOL_REVISION
    );
}

#[tokio::test]
async fn initialize_sends_current_protocol_revision() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        let Some(client_message::Message::Initialize(init)) = &msg.message else {
            panic!("Expected Initialize, got {:?}", msg.message);
        };
        assert_eq!(
            init.protocol_revision,
            crate::REMOTE_SERVER_PROTOCOL_REVISION
        );
        server_message::Message::InitializeResponse(InitializeResponse {
            server_version: "test-0.1.0".to_string(),
...
#[tokio::test]
async fn session_bootstrapped_transports_target_execution_context() {
    let (client_stream, server_stream) = tokio::io::duplex(4096);
    ...
    let environment_variables = HashMap::from([
        ...
        ("HOME".to_owned(), "/home/target".to_owned()),
        ("PATH".to_owned(), "/target/bin".to_owned()),
        ("CODEX_HOME".to_owned(), "/target/codex".to_owned()),
        ("CLAUDE_CONFIG_DIR".to_owned(), "/target/claude".to_owned()),
    ]);
    ...
    let message = protocol::read_client_message(&mut server_read)
        .await
        .expect("SessionBootstrapped notification must be encoded");
    let Some(client_message::Message::SessionBootstrapped(notification)) = message.message else {
        panic!("expected SessionBootstrapped notification");
    };
    assert_eq!(notification.session_id, 73);
    assert_eq!(notification.shell_type, "zsh");
    assert_eq!(notification.shell_path.as_deref(), Some("/bin/zsh"));
    assert_eq!(
...
#[tokio::test]
async fn session_execution_context_deregistered_is_transported() {
    let (client_stream, server_stream) = tokio::io::duplex(4096);
    let (server_read, _server_write) = tokio::io::split(server_stream);
    let (client_read, client_write) = tokio::io::split(client_stream);
    let executor = executor::Background::default();
    ...
    client.notify_session_deregistered(SessionId::from(74));

    let mut server_read = server_read.compat();
    let message = protocol::read_client_message(&mut server_read)
        .await
        .expect("SessionExecutionContextDeregistered notification must be encoded");
    let Some(client_message::Message::SessionExecutionContextDeregistered(notification)) =
        message.message
    else {
        panic!("expected SessionExecutionContextDeregistered notification");
    };
    assert_eq!(notification.session_id, 74);
}

#[tokio::test]
async fn initialize_rejects_missing_server_protocol_revision() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|_| {
        server_message::Message::InitializeResponse(InitializeResponse {
            server_version: "old-helper".to_string(),
...
#[tokio::test]
async fn initialize_rejects_wrong_server_protocol_revision() {
    let wrong_revision = crate::REMOTE_SERVER_PROTOCOL_REVISION + 1;
    let (client, _disconnect_rx, _executor) = setup_mock_client(move |_| {
        server_message::Message::InitializeResponse(InitializeResponse {
            server_version: "wrong-helper".to_string(),
            host_id: "wrong-host".to_string(),
            protocol_revision: wrong_revision,
        })
    });
...
#[tokio::test]
async fn delete_directory_round_trip_requires_explicit_success() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        match &msg.message {
            Some(client_message::Message::DeleteDirectory(req)) => {
                assert_eq!(req.path, "/repo/empty");
                assert_eq!(
                    req.identity,
        ...
        server_message::Message::DeleteDirectoryResponse(DeleteDirectoryResponse {
            result: Some(delete_directory_response::Result::Success(
...
#[tokio::test]
async fn delete_directory_empty_response_is_not_false_success() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|_| {
        server_message::Message::DeleteDirectoryResponse(DeleteDirectoryResponse { result: None })
    });
...
#[tokio::test]
async fn delete_file_empty_response_is_not_false_success() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|_| {
        server_message::Message::DeleteFileResponse(DeleteFileResponse { result: None })
    });

    assert!(matches!(
        client.delete_file("/repo/file.txt".to_string()).await,
        Err(ClientError::UnexpectedResponse)
    ));
}
...
async fn resolve_path_round_trip() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        ...
        server_message::Message::ResolvePathResponse(ResolvePathResponse {
            result: Some(resolve_path_response::Result::Success(ResolvePathSuccess {
                ...
                resolved_path: Some("/home/me/project".to_string()),
    ...
    assert_eq!(success.path, "/home/me/project");
    assert_eq!(success.resolved_path.as_deref(), Some("/home/me/project"));
    ...
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        match &msg.message {
            Some(client_message::Message::ReadFileChunk(req)) => {
                assert_eq!(req.handle.as_ref().unwrap().id, "read-handle");
    ...
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        match &msg.message {
            Some(client_message::Message::WriteFileChunk(req)) => {
                assert_eq!(req.handle.as_ref().unwrap().id, "write-handle");
...
#[test]
fn required_result_response_names_match_proto_contract() {
    let proto = include_str!("../proto/remote_server.proto");
    let mut current_response = None;
    let mut proto_names = Vec::new();

    for line in proto.lines() {
        let line = line.trim();
        if let Some(declaration) = line.strip_prefix("message ") {
            if let Some((name, _)) = declaration.split_once(" {") {
                current_response = Some(name.to_owned());
            }
        }
        if line == "oneof result {" {
            proto_names.push(
                current_response
                    .take()
                    .expect("oneof result must belong to a named response"),
            );
        }
    }

    proto_names.sort_unstable();
    let mut validator_names = REQUIRED_RESULT_RESPONSE_NAMES.to_vec();
    validator_names.sort_unstable();
    assert_eq!(proto_names, validator_names);
}

#[test]
fn all_required_result_responses_reject_missing_result() {
    let responses = vec![
        server_message::Message::AbortFileTransferResponse(Default::default()),
        server_message::Message::AppendFileResponse(Default::default()),
        server_message::Message::BeginFileTransferResponse(Default::default()),
        server_message::Message::CreateDirectoryResponse(Default::default()),
        server_message::Message::CreatePtyResponse(Default::default()),
        server_message::Message::DeleteDirectoryResponse(Default::default()),
        server_message::Message::DeleteFileResponse(Default::default()),
        server_message::Message::GetCliAgentSessionUserStateResponse(Default::default()),
        server_message::Message::ListDirectoryResponse(Default::default()),
        server_message::Message::MutateCliAgentSessionResponse(Default::default()),
        server_message::Message::MutateCliAgentSessionUserStateResponse(Default::default()),
        server_message::Message::ReadCliAgentSessionResponse(Default::default()),
        server_message::Message::ReadFileChunkResponse(Default::default()),
        server_message::Message::ExactRenameResponse(Default::default()),
        ...
        server_message::Message::ScanCliAgentSessionsResponse(Default::default()),
        server_message::Message::WriteFileChunkResponse(Default::default()),
        server_message::Message::WriteFileResponse(Default::default()),
    ];

    assert_eq!(responses.len(), REQUIRED_RESULT_RESPONSE_NAMES.len());
    for response in responses {
        let response = ServerMessage {
            request_id: "required-result-contract".to_string(),
...
#[tokio::test]
async fn empty_required_result_is_rejected_at_transport_boundary() {
...
#[tokio::test]
async fn write_and_exact_rename_empty_response_are_not_false_success() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|message| match &message.message {
        Some(client_message::Message::WriteFile(_)) => {
            server_message::Message::WriteFileResponse(crate::proto::WriteFileResponse {
                result: None,
            })
        }
        Some(client_message::Message::ExactRename(_)) => {
            server_message::Message::ExactRenameResponse(crate::proto::ExactRenameResponse {
                result: None,
            })
        }
        other => panic!("Expected WriteFile or ExactRename, got {other:?}"),
    });

    assert!(matches!(
        client
            .write_file("/repo/file.txt".to_owned(), "content".to_owned())
            .await,
        Err(ClientError::UnexpectedResponse)
    ));
    assert!(matches!(
        client
            .exact_rename("/repo/file.txt".to_owned(), "/repo/renamed.txt".to_owned())
            .await,
        Err(ClientError::UnexpectedResponse)
    ));
}
